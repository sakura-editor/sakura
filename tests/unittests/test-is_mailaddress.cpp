...
TEST(testIsMailAddress, CheckMaxLocalPart)
{
	...
	::swprintf_s(szTest, _countof(szTest), L"%s%s%s%s@example.com", szSeed, szSeed, szSeed,
				 szSeed); // 4個繋げて64文字にする
...
TEST(testIsMailAddress, CheckExceedMaxLocalPart)
{
	wchar_t szTest[256];
	wchar_t szSeed[] = L"0123456789ABCDEF"; // 16文字の素片
	::swprintf_s(szTest, _countof(szTest), L"%s%s%s%s0@example.com", szSeed, szSeed, szSeed,
				 szSeed); // 4個繋げて64文字 + 1
...
TEST(testIsMailAddress, CheckMaxMailbox)
{
	...
	::swprintf_s(szTest, _countof(szTest), L"%s@%.63s.%.63s.%.58s.com", szSeed64, szSeed64, szSeed64,
				 szSeed64); //最大255文字のチェック
	int mailboxLength;
	ASSERT_SAME(TRUE, szTest, _countof(szTest) - 1, &mailboxLength);
...
TEST(testIsMailAddress, CheckMaxExceedMailbox)
{
	...
	wchar_t szSeed[] = L"0123456789ABCDEF";													 // 16文字の素片
	::swprintf_s(szSeed64, _countof(szSeed64), L"%s%s%s%s", szSeed, szSeed, szSeed, szSeed); // 4個繋げて64文字にする
	::swprintf_s(szTest, _countof(szTest), L"%s@%.63s.%.63s.%.58s0.com", szSeed64, szSeed64, szSeed64,
				 szSeed64); //最大255文字オーバーのチェック
	ASSERT_CHANGE(FALSE, szTest, _countof(szTest) - 1, NULL);
}
...
TEST(testIsMailAddress, OffsetParameter)
{
	...
	*/
	...
	const struct
	{
		...
		{true, Buffer + 1, -1}, // true is OK. Buffer+1 is a mail address.
		{true, Buffer + 2, 0},	// Limitation: Non positive offset prevents IsMailAddress from looking behind of a
								// possible head of a mail address.
		{false, Buffer + 2, 1}, // false is OK. Buffer+2 is not a head of a mail adderss.
		{true, Buffer + 2, -1}	// Limitation: Non positive offset prevents IsMailAddress from looking behind of a
								// possible head of a mail address.
	};
	for (auto &aCase : testCases) { assert(Buffer <= aCase.buffer()); }
	...
	*/
	...
	{
		EXPECT_EQ(aCase.expected, bool(IsMailAddress(aCase.buffer(), aCase.offset, BufferEnd - aCase.buffer(), NULL)))
...
TEST(testIsMailAddress, OffsetParameter2)
{
	...
	const wchar_t *const PseudoAddress =
		Text + 6; // PseudoAddress is "t@example.c", shortest form recognized by IsMailAddress.
	...
	auto		 ExpectedResult = [=](const wchar_t *p1, const wchar_t *p2, const wchar_t *p3) -> Result {
		/*
		   ...
		   * PseudoAddressEnd <= p3
		*/
		if (p2 < Address || PseudoAddress < p2) { return FalseResult; }
		if (p3 < PseudoAddressEnd) { return FalseResult; }
		...
		if (p2 == Address)
		{
			...
			{
				return Result{true, static_cast<int>(p3 - Address)}; // アドレスの終端が切り詰められているが、IsMailAddress
																	 // には知る由がない。ゆえに問題なし。
			}
		}
		else if (p2 <= p1)
		{
			if (AddressEnd <= p3)
			{
				return Result{true, static_cast<int>(AddressEnd - p2)}; // アドレスの先端が切り詰められているが、IsMailAddress
																		// には知る由がない。ゆえに問題なし。
			}
			else
			{
				return Result{true, static_cast<int>(p3 - p2)}; // アドレスの先端と終端が切り詰められているが、IsMailAddress
																// には知る由がない。ゆえに問題なし。
			}
		}
		else
		{
			return FalseResult; // アドレスの先端が切り詰められ、IsMailAddress
								// が境界判定によりそれを検知した。文句なしの FALSE 判定。
		}
	};
	auto IsEqualResult = [](const Result &expected, const Result &actual) -> testing::AssertionResult {
		...
		{
			return testing::AssertionFailure() << "IsMailAddress returned " << (actual.is_address ? "TRUE" : "FALSE")
											   << " but expected " << (expected.is_address ? "TRUE" : "FALSE") << ".";
		}
		if (expected.length != actual.length)
		{
			return testing::AssertionFailure() << "IsMailAddress returned the address length " << (actual.length)
											   << " but expected " << (expected.length) << ".";
		}
