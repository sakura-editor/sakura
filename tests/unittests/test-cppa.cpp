...
extern "C" {
	void CALLBACK ppaIntFunc(LPCSTR FuncName, int Index, LPCSTR* Argument, int ArgSize, int* Err_CD, int* ResultValue);
	void CALLBACK ppaStrFunc(LPCSTR FuncName, int Index, LPCSTR* Argument, int ArgSize, int* Err_CD, LPSTR* ResultValue);
	void CALLBACK ppaProc(LPCSTR FuncName, int Index, LPCSTR* Argument, int ArgSize, int* Err_CD);
	void CALLBACK ppaStrObj(LPCSTR ObjName, int Index, BYTE GS_Mode, int* Err_CD, LPSTR* Value);
	void CALLBACK ppaErrorProc(int Err_CD, LPCSTR Err_Mes);
...
public:
	CPpaStub() noexcept {
		InitDll(L"ppa_stub.dll");
	}

	LPCWSTR GetDllNameImp(int nIndex) override {
		UNREFERENCED_PARAMETER(nIndex);
		return L"ppa_stub.dll";
	}

	//! CPPA::stdProcを呼び出す 
	void CallProc(LPCSTR FuncName, int Index, LPCSTR* Argument, int ArgSize, int* Err_CD) const {
		if (const auto hModule = GetInstance()) {
			if (const auto pfnCallProc= (decltype(&ppaProc))::GetProcAddress(hModule, __func__)) {
				pfnCallProc(FuncName, Index, Argument, ArgSize, Err_CD);
			}
		}
	}

	//! CPPA::stdIntFuncを呼び出す 
	void CallIntFunc(LPCSTR FuncName, int Index, LPCSTR* Argument, int ArgSize, int* Err_CD, int* ResultValue) const {
		if (const auto hModule = GetInstance()) {
			if (const auto pfnCallIntFunc = (decltype(&ppaIntFunc))::GetProcAddress(hModule, __func__)) {
				pfnCallIntFunc(FuncName, Index, Argument, ArgSize, Err_CD, ResultValue);
			}
		}
	}

	//! CPPA::stdStrFuncを呼び出す 
	void CallStrFunc(LPCSTR FuncName, int Index, LPCSTR* Argument, int ArgSize, int* Err_CD, LPSTR* ResultValue) const {
		if (const auto hModule = GetInstance()) {
			if (const auto pfnCallStrFunc = (decltype(&ppaStrFunc))::GetProcAddress(hModule, __func__)) {
				pfnCallStrFunc(FuncName, Index, Argument, ArgSize, Err_CD, ResultValue);
			}
		}
	}
	...
	void CallStrObj(LPCSTR ObjName, int Index, BYTE GS_Mode, int* Err_CD, LPSTR* Value) const {
		if (const auto hModule = GetInstance()) {
			if (const auto pfnCallStrObj = (decltype(&ppaStrObj))::GetProcAddress(hModule, __func__)) {
				pfnCallStrObj(ObjName, Index, GS_Mode, Err_CD, Value);
			}
		}
	}
	...
	void CallErrorProc(int errCd, LPCSTR errMsg) const {
		if (const auto hModule = GetInstance()) {
			if (const auto pfnCallStdError = (decltype(&ppaErrorProc))::GetProcAddress(hModule, __func__)) {
				PpaExecInfo execInfo;
				const auto execInfoHolder = CPPA::RegisterExecInfo(execInfo);

				pfnCallStdError(errCd, errMsg);
			}
		}
	}

	//! CPPA::stdFinishProcを呼び出す 
	void CallFinishProc() const {
		if (const auto hModule = GetInstance()) {
			if (const auto pfnCallFinishProc = (decltype(&ppaFinishProc))::GetProcAddress(hModule, __func__)) {
				pfnCallFinishProc();
			}
		}
	}
...
{
	CPpaStub cPpa;
	EXPECT_TRUE(cPpa.Execute(nullptr, NULL));
}

/*!
	CPPA::Executeのテスト
 */
TEST(CPPA, Execute_mutex)
{
	...
	bool ret = false;
	EXPECT_ERROUT(ret = cPpa.Execute(nullptr, NULL), L"PPA実行中に新たにPPAマクロを呼び出すことはできません");
	EXPECT_FALSE(ret);
}
...
TEST(CPPA, ppaIntFunc)
{
	...
	LPCSTR ppszArgs[4] = { msg.data(), nullptr };
	int errCd = 0;
	int resultValue = 0;
	EXPECT_ERROUT(cPpa.CallIntFunc("OkCancelBox", (int)F_OKCANCELBOX, ppszArgs, 1, &errCd, &resultValue), L"something is wrong.");
...
TEST(CPPA, ppaStrFunc)
{
	...
	LPCSTR ppszArgs[] = { msg.data() };
	int errCd = 0;
	LPSTR resultValue = buffer.data();
	EXPECT_ERROUT(cPpa.CallStrFunc("OkCancelBox", (int)F_OKCANCELBOX, ppszArgs, _countof(ppszArgs), &errCd, &resultValue), L"something is wrong.");
}
...
/*!
	CPPA::stdErrorのテスト 
 */
TEST(CPPA, ppaErrorProc_funcCode_commands)
{
	std::string_view msg = "something is wrong.";
...
/*!
	CPPA::stdErrorのテスト
 */
TEST(CPPA, ppaErrorProc_funcCode_functions)
{
	std::string_view msg = "something is wrong.";
...
/*!
	CPPA::stdErrorのテスト
 */
TEST(CPPA, ppaErrorProc_funcCode_unknown)
{
	std::string_view msg = "something is wrong.";
...
/*!
	CPPA::stdErrorのテスト
 */
TEST(CPPA, ppaErrorProc_PPA_errors)
{
	std::string_view msg = "something is wrong.";
...
/*!
	CPPA::stdErrorのテスト
 */
TEST(CPPA, ppaErrorProc_user_errors)
{
	std::string_view msg = "something is wrong.";
	constexpr char chNul = '\0';
...
/*!
	CPPA::stdFinishProcのテスト
 */
TEST(CPPA, ppaFinishProc)
{
	CPpaStub cPpa;
