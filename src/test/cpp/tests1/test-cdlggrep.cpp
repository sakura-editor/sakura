...
{
	dialog::ModalDialogCloser closer(std::nullopt); // デフォルトで IDCANCEL を送信
	CDlgGrep dlg;
...
{
	dialog::ModalDialogCloser closer(std::nullopt, [](HWND hWnd) {
...
TEST_F(CDlgGrepGuiTest, GrepReplace_ComboText2DropDown_PopulatesFromHistory)
{
	...
	int nComboCount = -1;
	dialog::ModalDialogCloser closer(std::nullopt, [&nComboCount](HWND hWnd) {
