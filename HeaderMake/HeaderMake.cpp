...
inline bool is_token(char c) { return __iscsym(c) || c == '-'; }
...
int main_impl(const char *in_file, const char *out_file, const char *mode_name, const char *enum_name);
...
int main(int argc, char *argv[])
{
	...
	if (!in_file)
	{
		printf("Error: Specify <InputFile> argument.\n");
		return usage();
	}
	if (!out_file)
	{
		printf("Error: Specify <OutputFile> argument.\n");
		return usage();
	}
	if (!mode_name)
	{
		printf("Error: Specify <Mode> argument.\n");
		return usage();
	}
...
int main_impl(const char *in_file, const char *out_file, const char *mode_name, const char *enum_name)
{
	...
	if (_stricmp(mode_name, "DEFINE") == 0)
		mode = MODE_DEFINE;
	else if (_stricmp(mode_name, "ENUM") == 0)
		mode = MODE_ENUM;
	else
	{
...
next:
	...
	// 出力ファイルオープン
	FILE *out = NULL;
	fopen_s(&out, out_file, "wt");
	if (!out)
	{
		printf("Error: Failed to open OutputFile[%s] as write mode\n", out_file);
		return 1;
	}
	...
	while (NULL != fgets(line, _countof(line), in))
	{
		...
		if (*id && *value)
		{
			if (mode == MODE_ENUM) { fprintf(out, "\t%s = %s,\n", id, value); }
