...
void CMacro::Save( HINSTANCE hInstance, CTextOutputStream& out ) const
{
	...
	if (CSMacroMgr::GetFuncInfoByID( hInstance, m_nFuncID, szFuncName, szFuncNameJapanese)){
		...
		case F_REPLACE_ALL:
			...
			cmemWork.Replace( LTEXT("\'"), LTEXT("\\\'") );
			{
				CNativeW2 cmemWork2(m_pParamTop->m_pNext->m_pData);
		...
		case F_GREP:
			...
			cmemWork.Replace( LTEXT("\'"), LTEXT("\\\'") );
			{
				...
				CNativeW2 cmemWork3(m_pParamTop->m_pNext->m_pNext->m_pData);
...
//			int		nDataLen;
			...
			CNativeT cmWork1;	cmWork1.SetStringW2( Argument[0] );	cmWork1.Replace( _T("\""), _T("\"\"") );	//	検索文字列
			CNativeT cmWork2;	cmWork2.SetStringW2( Argument[1] );	cmWork2.Replace( _T("\""), _T("\"\"") );	//	ファイル名
			CNativeT cmWork3;	cmWork3.SetStringW2( Argument[2] );	cmWork3.Replace( _T("\""), _T("\"\"") );	//	フォルダ名
...
bool CMacro::HandleFunction(CEditView *View, int ID, VARIANT *Arguments, int ArgSize, VARIANT &Result)
{
	...
	case F_EXPANDPARAMETER:
		// 2003.02.24 Moca
		{
			...
			wchar_t *Source;
			int SourceLength;
			Wrap(&varCopy.Data.bstrVal)->GetW2(&Source, &SourceLength);
			wchar_t Buffer[2048];
