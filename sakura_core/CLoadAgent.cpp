...
next:
	...
	if (GetDllShareData().m_Common.m_sFile.GetAlertIfFileNotExist()) {
		if (!fexist(pLoadInfo->cFilePath)) {
			...
			TopInfoMessage(CEditWnd::getInstance()->GetHwnd(),
	...
	if (nFind != INVALID_HANDLE_VALUE) {
		...
		if (!CFileLoad::IsLoadableSize(nFileSize.QuadPart)) {
			...
			ErrorMessage(CEditWnd::getInstance()->GetHwnd(), LS(STR_LOADAGENT_BIG_ERROR), pLoadInfo->cFilePath.c_str(),
		...
		if (GetDllShareData().m_Common.m_sFile.m_bAlertIfLargeFile) {
			...
			if ((nFileSize.QuadPart >> 20) >= (GetDllShareData().m_Common.m_sFile.m_nAlertFileSize)) {
				...
				if (nRet != IDYES) { return CALLBACK_INTERRUPT; }
			}
		}
	}
...
ELoadResult CLoadAgent::OnLoad(const SLoadInfo &sLoadInfo)
{
	...
	if (fexist(sLoadInfo.cFilePath)) {
		...
		pcDoc->m_cDocFile.SetCodeSet(sLoadInfo.eCharCode, (sLoadInfo.eCharCode == types.m_encoding.m_eDefaultCodetype)
															  ? types.m_encoding.m_bDefaultBom
															  : CCodeTypeName(sLoadInfo.eCharCode).IsBomDefOn());
	}
	...
	CProgressSubject *pOld =
		CEditApp::getInstance()->m_pcVisualProgress->CProgressListener::Listen(&pcDoc->m_cLayoutMgr);
...
void CLoadAgent::OnFinalLoad(ELoadResult eLoadResult)
{
	...
	if (eLoadResult == LOADED_LOSESOME) { CAppMode::getInstance()->SetViewMode(true); }
