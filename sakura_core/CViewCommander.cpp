...
// 2002.02.08 hor EOFのみの行からも次検索しても再検索可能に (2/2)
	...
	if( b0Match ) {
		...
		if( nIdx < nLineLen ) {
			// 2005-09-02 D.S.Koba GetSizeOfChar
			nIdx += CLogicInt(CNativeW2::GetSizeOfChar(pLine, nLineLen, nIdx) == 2 ? 2 : 1);
		} else {
			// 念のため行末は別処理
			++nIdx;
		}
...
//			bBeginBoxSelect = m_pCommanderView->GetSelectionInfo().IsBoxSelecting();
	...
	const CNativeW2	cMemRepKey( GetShareData()->m_aReplaceKeys[0] );
	...
	if( m_pCommanderView->GetSelectionInfo().IsTextSelected() ){
		...
		if(nPaste){
			...
			CNativeW2 cMemMatchStr; cMemMatchStr.SetString(L"$&");
			CNativeW2 cMemRepKey2;
			...
			if( cRegexp.Replace(pLine, nLen, nIdx) ){
				...
				if (matchLen == 0) {
					...
					if (nIdxTo < nLen) {
						// 2005-09-02 D.S.Koba GetSizeOfChar
						nIdxTo += CLogicInt(CNativeW2::GetSizeOfChar(pLine, nLen, nIdxTo) == 2 ? 2 : 1);
					}
...
void CViewCommander::Command_REPLACE_ALL()
{
	...
	CNativeW2	cmemClip;		// 置換後文字列のデータ（データを格納するだけで、ループ内ではこの形ではデータを扱いません）。
	...
	if( bRegularExp )
	{
		...
		const CNativeW2	cMemRepKey( szREPLACEKEY, wcslen(szREPLACEKEY) );
		// Nov. 9, 2005 かろと 正規表現で選択始点・終点への挿入方法を変更(再)
		CNativeW2 cMemRepKey2;
		CNativeW2 cMemMatchStr;
