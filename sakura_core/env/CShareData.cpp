...
bool CShareData::InitShareData()
{
    ...
    if (GetLastError() != ERROR_ALREADY_EXISTS)
    {
        ...
        GetAppVersionInfo(NULL, VS_VERSION_INFO, &m_pShareData->m_sVersion.m_dwProductVersionMS, &m_pShareData->m_sVersion.m_dwProductVersionLS);
...
void CShareData::TraceOut(LPCWSTR lpFmt, ...)
{
    ...
    ::SendMessageTimeout(m_pShareData->m_sHandles.m_hwndDebug, MYWM_ADDSTRINGLEN_W, ret, 0, SMTO_NORMAL, 10000, &dwMsgResult);
}
...
void CShareData::TraceOutString(const wchar_t *pStr, int len)
{
    ...
    else
    {
        while (outPos < len)
        {
            ...
            {
                // CRLF(\r\n)とUTF-16が分離されないように
                if ((pStr[outPos + outLen - 1] == WCODE::CR && pStr[outPos + outLen] == WCODE::LF) || (IsUtf16SurrogHi(pStr[outPos + outLen - 1]) && IsUtf16SurrogLow(pStr[outPos + outLen])))
                {
                    --outLen;
                }
            }
            ...
            DWORD_PTR dwMsgResult;
            if (0 == ::SendMessageTimeout(m_pShareData->m_sHandles.m_hwndDebug, MYWM_ADDSTRINGLEN_W, outLen, 0, SMTO_NORMAL, 10000, &dwMsgResult))
            {
                // エラーかタイムアウト
                break;
            }
