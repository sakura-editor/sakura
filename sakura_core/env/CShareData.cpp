...
CMutex &CShareData::GetMutexShareWork() { return g_cMutexShareWork; }
...
bool CShareData::InitShareData()
{
	...
	if (GetLastError() != ERROR_ALREADY_EXISTS) {
		...
		GetAppVersionInfo(NULL, VS_VERSION_INFO, &m_pShareData->m_sVersion.m_dwProductVersionMS,
						  &m_pShareData->m_sVersion.m_dwProductVersionLS);
		...
		::SystemParametersInfo(SPI_GETICONTITLELOGFONT, // system parameter to query or set
		...
		INT nIconPointSize = lfIconTitle.lfHeight >= 0
								 ? lfIconTitle.lfHeight
								 : DpiPixelsToPoints(-lfIconTitle.lfHeight, 10); // フォントサイズ（1/10ポイント単位）
		...
		// [全般]タブ
		{
			...
			sGeneral.m_bCloseAllConfirm = FALSE;
			...
			sGeneral.m_nPageScrollByWheel	  = 0;
				/* キー/マウスボタン + ホイールスクロールでページスクロールする */ // 2009.01.17 nasukoji
			sGeneral.m_nHorizontalScrollByWheel = 0;
				/* キー/マウスボタン + ホイールスクロールで横スクロールする */ // 2009.01.17 nasukoji
			...
			sGeneral.m_bNoCaretMoveByActivation = FALSE; /* マウスクリックにてアクティベートされた時はカーソル位置を移動しない
		...
		// [タブバー]タブ
		{
			...
			wcscpy(								   //@@@ 2003.06.13 MIK
				sTabBar.m_szTabWndCaption, L"${w?【Grep】$h$:【アウトプット】$:$f$n$}${U?(更新)$}${R?(ビューモード)$:("
										   L"上書き禁止)$}${M?【キーマクロの記録中】$}");
			...
			sTabBar.m_bTab_CloseOneWin =
				FALSE; // タブモードでもウィンドウの閉じるボタンで現在のファイルのみ閉じる	// 2007.02.11 genta
		...
		// [書式]タブ
		{
			...
			/* 見出し記号 */
			wcscpy(sFormat.m_szMidashiKigou,
				   L"１２３４５６７８９０（(［["
				   L"「『【■□▲△▼▽◆◇○◎●§・※☆★第①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩ一二三四五六七八九十壱弐参伍");
			/* 引用符 */
		...
		// [検索]タブ
		{
			...
			sSearch.m_bCaretTextForSearch =
				TRUE; /* 2006.08.23 ryoji カーソル位置の文字列をデフォルトの検索文字列にする */
		...
		// [キー割り当て]タブ
		{
			...
			if (!InitKeyAssign(m_pShareData)) { return false; }
		}
		...
		// [支援]タブ
		{
			...
			sHelper.m_nPointSize =
				nIconPointSize; // フォントサイズ（1/10ポイント単位） ※古いバージョンからの移行を考慮して無効値で初期化
								// // 2009.10.01 ryoji
		...
		// [アウトライン]タブ
		{
			...
			sOutline.m_bAutoCloseDlgFuncList = FALSE;
				/* アウトライン ダイアログを自動的に閉じる */ // Nov. 18, 2000 JEPRO TRUE→FALSE に変更
			sOutline.m_bMarkUpBlankLineEnable =
				FALSE; //アウトラインダイアログでブックマークの空行を無視			2002.02.08 aroka,hor
			sOutline.m_bFunclistSetFocusOnJump =
				FALSE; //アウトラインダイアログでジャンプしたらフォーカスを移す	2002.02.08 hor
		...
		// [ビュー]タブ
		{
			...
			sView.m_lf		   = lf;
			sView.m_nPointSize = 0; // フォントサイズ（1/10ポイント単位）
									// ※古いバージョンからの移行を考慮して無効値で初期化	// 2009.10.01 ryoji
...

		{
			...
			m_pShareData->m_nExecFlgOpt = 1;
				/* 外部コマンド実行の「標準出力を得る」 */ // 2006.12.03 maru オプションの拡張のため
			...
			m_pShareData->m_bLineNumIsCRLF_ForJump =
...
BOOL CShareData::IsPathOpened(const WCHAR *pszPath, HWND *phwndOwner)
{
	...
	*phwndOwner = NULL;
	...
	if (GetLongFileName(pszPath, szBuf)) { pszPath = szBuf; }
...
{
	if (false == OpenDebugWindow(m_hwndTraceOutSource, false)) { return; }
	...
	::SendMessageTimeout(m_pShareData->m_sHandles.m_hwndDebug, MYWM_ADDSTRINGLEN_W, ret, 0, SMTO_NORMAL, 10000,
						 &dwMsgResult);
}
...
bool CShareData::OpenDebugWindow(HWND hwnd, bool bAllwaysActive)
{
	...
	/* 開いているウィンドウをアクティブにする */
	if (ret && bAllwaysActive) { ActivateFrameWindow(m_pShareData->m_sHandles.m_hwndDebug); }
	return ret;
}

/* iniファイルの保存先がユーザ別設定フォルダかどうか */ // 2007.05.25 ryoji
BOOL CShareData::IsPrivateSettings(void) { return m_pShareData->m_sFileNameManagement.m_IniFolder.m_bWritePrivate; }
...
{
	if (-1 != idx && !m_pShareData->m_Common.m_sMacro.m_MacroTable[idx].IsEnabled()) return 0;
	const WCHAR *ptr;
	const WCHAR *pszFile;
	...
	if (!_IS_REL_PATH(pszFile)											  // 絶対パス
		...
		if (pszPath == NULL || nBufLen <= nAllLen) { return -nAllLen; }

		wcscpy(pszPath, pszDir);
		WCHAR *ptr = pszPath + nDirLen;
		if (-1 == nFolderSep) { *ptr++ = L'\\'; }
		wcscpy(ptr, pszFile);
		return nAllLen;
	}
}
...
{
	if (!m_pShareData->m_Common.m_sMacro.m_MacroTable[idx].IsEnabled()) return false;
...
void CShareData::InitToolButtons(DLLSHAREDATA *pShareData)
{
	...
	static const int DEFAULT_TOOL_BUTTONS[] = {
		...
		266, //共通設定			//Sept. 16, 2000 JEPRO 10→37に変更 説明を「設定プロパティシート」から変更	//Oct. 25, 2000
			 //37→266
	...
	memcpy_raw(pShareData->m_Common.m_sToolBar.m_nToolBarButtonIdxArr, DEFAULT_TOOL_BUTTONS,
			   sizeof(DEFAULT_TOOL_BUTTONS));

	/* ツールバーボタンの数 */
	pShareData->m_Common.m_sToolBar.m_nToolBarButtonNum = _countof(DEFAULT_TOOL_BUTTONS);
	pShareData->m_Common.m_sToolBar.m_bToolBarIsFlat	= !IsVisualStyle();
		/* フラットツールバーにする／しない */ // 2006.06.23 ryoji ビジュアルスタイルでは初期値をノーマルにする
}
...
void CShareData::InitPopupMenu(DLLSHAREDATA *pShareData)
{
	...
	rMenu.m_nCustMenuItemKeyArr[0][n] =
		'G'; // Nov. 9, 2000 JEPRO 「コピー」とバッティングしていたアクセスキーを変更(T→G)
	n++;
	rMenu.m_nCustMenuItemFuncArr[0][n] = F_TAGJUMPBACK;
	rMenu.m_nCustMenuItemKeyArr[0][n]  = 'B';
	n++;
	rMenu.m_nCustMenuItemFuncArr[0][n] =
		F_0; // Oct. 15, 2000 JEPRO 以下に「選択範囲内全行コピー」と「引用符付きコピー」を追加
	rMenu.m_nCustMenuItemKeyArr[0][n] = '\0';
	n++;
	...
	rMenu.m_nCustMenuItemKeyArr[0][n] =
		'F'; // Nov. 9, 2000 JEPRO 「やり直し」とバッティングしていたアクセスキーを変更(R→F)
	n++;
...
void CShareData::RefreshString() { RefreshKeyAssignString(m_pShareData); }

void CShareData::CreateTypeSettings()
{
	if (NULL == m_pvTypeSettings) { m_pvTypeSettings = new std::vector<STypeConfig *>(); }
}

std::vector<STypeConfig *> &CShareData::GetTypeSettings() { return *m_pvTypeSettings; }
