...
void CSakuraEnvironment::ExpandParameter(const wchar_t *pszSource, wchar_t *pszBuffer, int nBufferLen)
{
    ...
    for (p = pszSource, q = pszBuffer, q_max = pszBuffer + nBufferLen; *p != '\0' && q < q_max;)
    {
        ...
        switch (*(++p))
        {
            ...
            {
                PRINTSETTING *ps = &GetDllShareData().m_PrintSettingArr[pcDoc->m_cDocType.GetDocumentAttribute().m_nCurrentPrintSetting];
            ...
            case L'V': // Apr. 4, 2003 genta
                // Version number
                {
                    ...
                    int len = auto_sprintf(buf, L"%d.%d.%d.%d", HIWORD(dwVersionMS), LOWORD(dwVersionMS), HIWORD(dwVersionLS), LOWORD(dwVersionLS));
...
int CSakuraEnvironment::_ExParam_Evaluate(const wchar_t *pCond)
{
    ...
    switch (*pCond)
    {
        ...
        case L'M': // $M キーボードマクロの記録中
            if (GetDllShareData().m_sFlags.m_bRecordingKeyMacro && GetDllShareData().m_sFlags.m_hwndRecordingKeyMacro == CEditWnd::getInstance()->GetHwnd())
            { /* ウィンドウ */
                return 0;
            }
            else
