												  ...
												  int cchMaxWidth)
{
	...
	{
		if (cchMaxWidth == 0) { cchMaxWidth = m_pShareData->m_Common.m_sFileName.m_nTransformShortMaxWidth; }
	...
	{
		GetFilePathFormat(pszSrc, pszDest, nDestLen, m_szTransformFileNameFromExp[0],
		...
		for (i = 1; i < m_nTransformFileNameCount; i++)
		{
			...
			GetFilePathFormat(szBuf, pszDest, nDestLen, m_szTransformFileNameFromExp[i],
...
*/
LPCWSTR CFileNameManager::GetFilePathFormat(LPCWSTR pszSrc, LPWSTR pszDest, int nDestLen, LPCWSTR pszFrom,
											LPCWSTR pszTo)
{
	int i, j;
	int nSrcLen;
...
#define _USE_META_ALIAS
	...
	static const MetaAlias AliasList[] = {{L"COMDESKTOP", 10, L"Common Desktop"},
	...
	for (ps = pszSrc, pd = pszDes; L'\0' != *ps; ps++)
	{
		...
		if (L'\0' != ps[1])
		{
			...
			{
				if (FALSE != GetLongFileName(szPath, szMeta)) { pStr2 = szMeta; }
			}
...
*/
bool CFileNameManager::GetMenuFullLabel(WCHAR *pszOutput, int nBuffSize, bool bEspaceAmp, const EditInfo *editInfo,
										int nId, bool bFavorite, int index, bool bAccKeyZeroOrigin, HDC hDC)
{
...
bool CFileNameManager::GetMenuFullLabel(WCHAR *pszOutput, int nBuffSize, bool bEspaceAmp, const WCHAR *pszFile, int nId,
										bool bModified, ECodeType nCharCode, bool bFavorite, int index,
										bool bAccKeyZeroOrigin, HDC hDC)
{
	WCHAR		 szAccKey[4];
	...
	int ret = auto_snprintf_s(pszOutput, nBuffSize, L"%s%s%s %s%s", szAccKey, (bFavorite ? L"★ " : L""), pszName,
...
void CFileNameManager::GetIniFileNameDirect(LPWSTR pszPrivateIniFile, LPWSTR pszIniFile, LPCWSTR pszProfName)
{
	...
	if (pszProfName[0] == '\0')
	{ auto_snprintf_s(pszIniFile, _MAX_PATH - 1, L"%s%s%s%s", szDrive, szDir, szFname, L".ini"); } else
	{
	...
	pszPrivateIniFile[0] = L'\0';
	{
		...
		if (nEnable)
		{
			...
			if (szDir[0] == L'\0') ::lstrcpy(szDir, L"sakura");
...
void CFileNameManager::GetIniFileName(LPWSTR pszIniFileName, LPCWSTR pszProfName, BOOL bRead /*=FALSE*/)
{
	if (!m_pShareData->m_sFileNameManagement.m_IniFolder.m_bInit)
	{
		...
		if (m_pShareData->m_sFileNameManagement.m_IniFolder.m_szPrivateIniFile[0] != L'\0')
		{
			...
			if (m_pShareData->m_sFileNameManagement.m_IniFolder.m_bWritePrivate)
			{
				...
				WCHAR szDrive[_MAX_DRIVE];
				WCHAR szDir[_MAX_DIR];
				_wsplitpath(m_pShareData->m_sFileNameManagement.m_IniFolder.m_szPrivateIniFile, szDrive, szDir, NULL,
							NULL);
	...
	bool bPrivate = bRead ? m_pShareData->m_sFileNameManagement.m_IniFolder.m_bReadPrivate
						  : m_pShareData->m_sFileNameManagement.m_IniFolder.m_bWritePrivate;
	::lstrcpy(pszIniFileName, bPrivate ? m_pShareData->m_sFileNameManagement.m_IniFolder.m_szPrivateIniFile
									   : m_pShareData->m_sFileNameManagement.m_IniFolder.m_szIniFile);
}
