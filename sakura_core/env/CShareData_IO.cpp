	...
	if( bRead ){
		if( !cProfile.ReadProfile( szIniFileName ) ){
			...
			if (langId != MAKELANGID( LANG_JAPANESE, SUBLANG_JAPANESE_JAPAN )) {
				...
				wcsncpy(pShareData->m_Common.m_sWindow.m_szLanguageDll, L"sakura_lang_en_US.dll");
...
void CShareData_IO::ShareData_IO_Mru( CDataProfile& cProfile )
{
	...
	for( i = 0; i < nSize; ++i ){
		...
		//お気に入り	//@@@ 2003.04.08 MIK
		wcsncat( szKeyName, LTEXT(".bFavorite") );
...
void CShareData_IO::ShareData_IO_Common( CDataProfile& cProfile )
{
	...
	if( !cProfile.IsReadingMode() ){
		...
		if( 1 == nCharChars && common.m_sBackup.m_szBackUpFolder[nDummy - 1] == '\\' ){
			...
			wcsncat( common.m_sBackup.m_szBackUpFolder, L"\\" );
		}
	}
...
void CShareData_IO::ShareData_IO_Print( CDataProfile& cProfile )
{
	...
	for( i = 0; i < MAX_PRINTSETTINGARR; ++i ){
		...
		if(0==wcsncmp(printsetting.m_szHeaderForm[0],_EDITL("&f")) &&
		   0==wcsncmp(printsetting.m_szFooterForm[0],_EDITL("&C- &P -"))
		){
			wcsncpy( printsetting.m_szHeaderForm[0], _EDITL("$f") );
			wcsncpy( printsetting.m_szFooterForm[0], _EDITL("") );
			wcsncpy( printsetting.m_szFooterForm[1], _EDITL("- $p -") );
		}
