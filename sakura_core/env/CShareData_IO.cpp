			...
			if (bOldVer) {
				...
				if (cProfile.IOProfileData(szSecName, szKeyName, MakeStringBufferW(szKeyData))) {
					...
					*pn  = 0;
					...
					if (tmpKeydata.m_nKeyCode
						<= 0) { // マウスコードは先頭に固定されている KeyCodeが同じなのでKeyNameで判別
...
void CShareData_IO::ShareData_IO_Other(CDataProfile &cProfile)
{
	...
	/* **** その他のダイアログ **** */
	/* 外部コマンド実行の「標準出力を得る」 */
	if (!cProfile.IOProfileData(pszSecName, LTEXT("nExecFlgOpt"),
								pShare->m_nExecFlgOpt)) { //	2006.12.03 maru オプション拡張
