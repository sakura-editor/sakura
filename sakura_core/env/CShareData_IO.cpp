	...
	if( bRead ){
		if( !cProfile.ReadProfile( szIniFileName ) ){
			...
			if (langId != MAKELANGID( LANG_JAPANESE, SUBLANG_JAPANESE_JAPAN )) {
				...
				cProfile.IOProfileData( L"Common", L"szLanguageDll", CStringBufW( pShareData->m_Common.m_sWindow.m_szLanguageDll ) );
		...
		if( cProfile.IOProfileData( L"Other", L"szVersion", CStringBufW( iniVer ) ) )
...
void CShareData_IO::ShareData_IO_Mru( CDataProfile& cProfile )
{
	...
	for( i = 0; i < nSize; ++i ){
		...
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(pfiWork->m_szPath) );
		auto_sprintf( szKeyName, LTEXT("MRU[%02d].szMark2"), i );
		if( !cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(pfiWork->m_szMarkLines) ) ){
			if( cProfile.IsReadingMode() ){
				auto_sprintf( szKeyName, LTEXT("MRU[%02d].szMark"), i ); // 旧ver互換
				cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(pfiWork->m_szMarkLines) );
			}
		}
...
void CShareData_IO::ShareData_IO_Nickname( CDataProfile& cProfile )
{
	...
	for( i = 0; i < nSize; ++i ){
		...
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(pShare->m_Common.m_sFileName.m_szTransformFileNameFrom[i]) );
		auto_sprintf( szKeyName, LTEXT("To%02d"), i );
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(pShare->m_Common.m_sFileName.m_szTransformFileNameTo[i]) );
	}
	// 読み込み時，残りをNULLで再初期化
	if( cProfile.IsReadingMode() ){
...
static bool ShareData_IO_RECT( CDataProfile& cProfile, const WCHAR* pszSecName, const WCHAR* pszKeyName, RECT& rcValue )
{
	...
	bool		ret = false;
	if( cProfile.IsReadingMode() ){
		ret = cProfile.IOProfileData( pszSecName, pszKeyName, CStringBufW( szKeyData ) );
		if( ret ){
			int buf[4];
...
void CShareData_IO::ShareData_IO_Common( CDataProfile& cProfile )
{
	...
	cProfile.IOProfileData( pszSecName, L"szRegexpLib", CStringBufW(common.m_sSearch.m_szRegexpLib) );
	...
	cProfile.IOProfileData( pszSecName, L"szExtHelp", CStringBufW(common.m_sHelper.m_szExtHelp) );
	cProfile.IOProfileData( pszSecName, L"szExtHtmlHelp", CStringBufW(common.m_sHelper.m_szExtHtmlHelp) );
	
	cProfile.IOProfileData( pszSecName, L"szMigemoDll", CStringBufW(common.m_sHelper.m_szMigemoDll) );
	cProfile.IOProfileData( pszSecName, L"szMigemoDict", CStringBufW(common.m_sHelper.m_szMigemoDict) );
	...
	cProfile.IOProfileData( pszSecName, L"szLanguageDll", CStringBufW( common.m_sWindow.m_szLanguageDll ) );
	...
	cProfile.IOProfileData( pszSecName, L"szTabWndCaption", CStringBufW(common.m_sTabBar.m_szTabWndCaption) );	//@@@ 2003.06.13 MIK
	...
	cProfile.IOProfileData( pszSecName, L"szMidashiKigou", CStringBufW(common.m_sFormat.m_szMidashiKigou) );
	cProfile.IOProfileData( pszSecName, L"szInyouKigou", CStringBufW(common.m_sFormat.m_szInyouKigou) );
...
//	cProfile.IOProfileData( pszSecName, LTEXT("bRulerDisp")					, common.m_bRulerDisp );/* ルーラー表示 */
	...
	cProfile.IOProfileData( pszSecName, L"szDateFormat", CStringBufW(common.m_sFormat.m_szDateFormat) );//日付書式
	cProfile.IOProfileData( pszSecName, LTEXT("nTimeFormatType")			, common.m_sFormat.m_nTimeFormatType );/* 時刻書式のタイプ */
	cProfile.IOProfileData( pszSecName, L"szTimeFormat", CStringBufW(common.m_sFormat.m_szTimeFormat) );//時刻書式
	...
	cProfile.IOProfileData( pszSecName, L"szWinCaptionActive", CStringBufW(common.m_sWindow.m_szWindowCaptionActive) );
	cProfile.IOProfileData( pszSecName, L"szWinCaptionInactive", CStringBufW(common.m_sWindow.m_szWindowCaptionInactive) );
	...
	cProfile.IOProfileData( pszSecName, L"eOutlineDockSide", common.m_sOutline.m_eOutlineDockSide );
	{
		...
		if( cProfile.IsReadingMode() ){
			if( cProfile.IOProfileData( pszSecName, pszKeyName, CStringBufW( szKeyData )) ){
				int buf[4];
				scan_ints( szKeyData, pszForm, buf );
			...
			cProfile.IOProfileData( pszSecName, pszKeyName, CStringBufW( szKeyData ) );
		}
	}
...
void CShareData_IO::ShareData_IO_Toolbar( CDataProfile& cProfile, CMenuDrawer* pcMenuDrawer )
{
	...
	for( i = 0; i < nSize; ++i ){
		...
		if( cProfile.IsReadingMode() ){
			//読み込み
			cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW( szText ) );
...
void CShareData_IO::IO_CustMenu( CDataProfile& cProfile, CommonSetting_CustomMenu& menu, bool bOutCmdName)
{
	...
	for( i = 0; i < MAX_CUSTOM_MENU; ++i ){
		...
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(menu.m_szCustMenuNameArr[i]) );	//	Oct. 15, 2001 genta 最大長指定
		...
		for( j = 0; j < nSize; ++j ){
			...
			if (cProfile.IsReadingMode()) {
				cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW( szFuncName ));
...
//	int		nSize = m_pShareData->m_nKeyNameArrNum;
	...
	if( cProfile.IsReadingMode() ){ 
		if (!cProfile.IOProfileData( szSecName, L"KeyBind[000]", CStringBufW( szKeyData ) ) ) {
			bOldVer = true;
		}
		else {
	...
	for( i = 0; i < sKeyBind.m_nKeyNameArrNum; ++i ){
		...
		if( cProfile.IsReadingMode() ){
			if (bOldVer) {
				...
				if( cProfile.IOProfileData( szSecName, szKeyName, CStringBufW( szKeyData ) ) ){
					int buf[8];
		...
		//	);
		//	cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW( szKeyData ) );
...
// start 新バージョン	2008/5/25 Uchi
			...
			wcscat(szKeyData, szWork);
			cProfile.IOProfileData( szSecName, szKeyName, CStringBufW( szKeyData ) );
//
		}
	}
...
void CShareData_IO::ShareData_IO_Print( CDataProfile& cProfile )
{
	...
	for( i = 0; i < MAX_PRINTSETTINGARR; ++i ){
		...
		if( cProfile.IsReadingMode() ){
			if( cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW( szKeyData ) ) ){
				int buf[19];
				scan_ints( szKeyData, pszForm, buf );
		...
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(printsetting.m_szPrintSettingName) );
		auto_sprintf( szKeyName, LTEXT("PS[%02d].szFF")	, i );
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(printsetting.m_szPrintFontFaceHan) );
		auto_sprintf( szKeyName, LTEXT("PS[%02d].szFFZ")	, i );
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(printsetting.m_szPrintFontFaceZen) );
		// ヘッダ/フッタ
		for( j = 0; j < 3; ++j ){
			auto_sprintf( szKeyName, LTEXT("PS[%02d].szHF[%d]") , i, j );
			cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(printsetting.m_szHeaderForm[j]) );
			auto_sprintf( szKeyName, LTEXT("PS[%02d].szFTF[%d]"), i, j );
			cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(printsetting.m_szFooterForm[j]) );
		}
		{ // ヘッダ/フッタ フォント設定
			WCHAR	szKeyName2[64];
			WCHAR	szKeyName3[64];
		...
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(printsetting.m_mdmDevMode.m_szPrinterDriverName) );
		auto_sprintf( szKeyName, LTEXT("PS[%02d].szDevice"), i );
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(printsetting.m_mdmDevMode.m_szPrinterDeviceName) );
		auto_sprintf( szKeyName, LTEXT("PS[%02d].szOutput"), i );
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(printsetting.m_mdmDevMode.m_szPrinterOutputName) );
...
) noexcept
{
	...
	bool ret = false;
	if( cProfile.IOProfileData( pszSectionName, pszEntryKeyFrom, CStringBufW( szFrom ) )
		&& cProfile.IOProfileData( pszSectionName, pszEntryKeyTo, CStringBufW( szTo ) ) ){
		//対になる設定が揃った場合のみ有効
		ret = true;
	}
...
) noexcept
{
	...
	bool ret = false;
	if( cProfile.IOProfileData( pszSectionName, pszEntryKeyComment, CStringBufW( lbuf ) )
...
void CShareData_IO::ShareData_IO_Type_One( CDataProfile& cProfile, STypeConfig& types, const WCHAR* pszSecName)
{
	...
	cProfile.IOProfileData( pszSecName, L"szTypeName", CStringBufW(types.m_szTypeName) );
	cProfile.IOProfileData( pszSecName, L"szTypeExts", CStringBufW(types.m_szTypeExts) );
	cProfile.IOProfileData( pszSecName, LTEXT("id"), types.m_id );
	if( types.m_id < 0 ){
		types.m_id *= -1;
	}
	cProfile.IOProfileData( pszSecName, L"szTabViewString", CStringBufW(types.m_szTabViewString) );
	...
	cProfile.IOProfileData( pszSecName, L"szIndentChars", CStringBufW(types.m_szIndentChars) );
...
//@@@ 2001.11.17 add end MIK
	...
	cProfile.IOProfileData( pszSecName, L"szKinsokuHead", CStringBufW(types.m_szKinsokuHead) );
	cProfile.IOProfileData( pszSecName, L"szKinsokuTail", CStringBufW(types.m_szKinsokuTail) );
	cProfile.IOProfileData( pszSecName, L"szKinsokuKuto", CStringBufW(types.m_szKinsokuKuto) );	// 2009.08.07 ryoji
...
void CShareData_IO::ShareData_IO_KeyWords( CDataProfile& cProfile )
{
	...
	if( cProfile.IsReadingMode() ){
		...
		for( i = 0; i < nSize; ++i ){
			auto_sprintf( szKeyName, LTEXT("szSN[%02d]"), i );
			cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(pCKeyWordSetMgr->m_szSetNameArr[i]) );
			auto_sprintf( szKeyName, LTEXT("nCASE[%02d]"), i );
			...
			*pMem = L'\0';
			auto_sprintf( szKeyName, LTEXT("szKW[%02d]"), i );
			cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW( pszMem, nMemLen ) );
			delete [] pszMem;
		}
	}
}
...
void CShareData_IO::ShareData_IO_Macro( CDataProfile& cProfile )
{
	...
	for( i = 0; i < MAX_CUSTMACRO; ++i ){
		...
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(macrorec.m_szName) );
		auto_sprintf( szKeyName, LTEXT("File[%03d]"), i );
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(macrorec.m_szFile) );
...
void CShareData_IO::ShareData_IO_Plugin( CDataProfile& cProfile, CMenuDrawer* pcMenuDrawer )
{
	...
	for( i = 0; i < MAX_PLUGIN; ++i ){
		...
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(pluginrec.m_szName) );
		auto_sprintf( szKeyName, LTEXT("P[%02d].Id"), i );
		cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW(pluginrec.m_szId) );
	...
	for (int i = 0; i < mainmenu.m_nMainMenuNum; i++) {
		...
		if (cProfile.IsReadingMode()) {
			...
			if( pData ){
				...
				cProfile.IOProfileData( pszSecName, szKeyName, CStringBufW( szLine ) );
			}
...
void CShareData_IO::ShareData_IO_Other( CDataProfile& cProfile )
{
	...
	cProfile.IOProfileData( pszSecName, L"szTagsCmdLine", CStringBufW(pShare->m_szTagsCmdLine) );
	...
	if( ! cProfile.IsReadingMode() ){
		...
		cProfile.IOProfileData( pszSecName, L"szVersion", CStringBufW( iniVer ) );
...
void CShareData_IO::IO_ColorSet( CDataProfile* pcProfile, const WCHAR* pszSecName, ColorInfo* pColorInfoArr )
{
	...
	for( j = 0; j < COLORIDX_LAST; ++j ){
		...
		if( pcProfile->IsReadingMode() ){
			if( pcProfile->IOProfileData( pszSecName, szKeyName, CStringBufW( szKeyData ) ) ){
				int buf[5];
				scan_ints( szKeyData, pszForm, buf);
			...
			pcProfile->IOProfileData( pszSecName, szKeyName, CStringBufW( szKeyData ) );
		}
	}
}
	...
	const WCHAR* pszKeyLf, const WCHAR* pszKeyPointSize, const WCHAR* pszKeyFaceName, LOGFONT& lf, INT& nPointSize )
{
	...
	if( cProfile.IsReadingMode() ){
		if( cProfile.IOProfileData( pszSecName, pszKeyLf, CStringBufW( szKeyData ) ) ){
			int buf[13];
			scan_ints( szKeyData, pszForm, buf );
			lf.lfHeight			= buf[ 0];
			lf.lfWidth			= buf[ 1];
		...
		cProfile.IOProfileData( pszSecName, pszKeyLf, CStringBufW( szKeyData ) );
	}
	
	cProfile.IOProfileData( pszSecName, pszKeyFaceName, CStringBufW(lf.lfFaceName) );
}
