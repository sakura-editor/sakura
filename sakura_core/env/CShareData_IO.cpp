...
void ShareData_IO_Sub_LogFont(CDataProfile &cProfile, const WCHAR *pszSecName, const WCHAR *pszKeyLf, const WCHAR *pszKeyPointSize, const WCHAR *pszKeyFaceName, LOGFONT &lf, INT &nPointSize);
...
bool CShareData_IO::ShareData_IO_2(bool bRead)
{
    ...
    if (bRead)
    {
        ...
        DLLSHAREDATA *pShareData = &GetDllShareData();
        if (pShareData->m_sVersion.m_dwProductVersionMS > dwMS || (pShareData->m_sVersion.m_dwProductVersionMS == dwMS && pShareData->m_sVersion.m_dwProductVersionLS > dwLS))
        {
...
void CShareData_IO::ShareData_IO_Common(CDataProfile &cProfile)
{
    ...
    if (!cProfile.IsReadingMode())
    {
        ...
        nCharChars = &common.m_sBackup.m_szBackUpFolder[nDummy] - CNativeW::GetCharPrev(common.m_sBackup.m_szBackUpFolder, nDummy, &common.m_sBackup.m_szBackUpFolder[nDummy]);
    ...
    // ai 02/05/23 Add S
    { // Keword Help Font
        ShareData_IO_Sub_LogFont(cProfile, pszSecName, L"khlf", L"khps", L"khlfFaceName", common.m_sHelper.m_lf, common.m_sHelper.m_nPointSize);
    } // Keword Help Font
    ...
    ShareData_IO_Sub_LogFont(cProfile, pszSecName, L"lfTabFont", L"lfTabFontPs", L"lfTabFaceName", common.m_sTabBar.m_lf, common.m_sTabBar.m_nPointSize);
...
void CShareData_IO::ShareData_IO_Print(CDataProfile &cProfile)
{
    ...
    for (i = 0; i < MAX_PRINTSETTINGARR; ++i)
    {
        ...
        {
            auto_sprintf(szKeyData, pszForm, printsetting.m_nPrintFontWidth, printsetting.m_nPrintFontHeight, printsetting.m_nPrintDansuu, printsetting.m_nPrintDanSpace, printsetting.m_nPrintLineSpacing, printsetting.m_nPrintMarginTY, printsetting.m_nPrintMarginBY, printsetting.m_nPrintMarginLX, printsetting.m_nPrintMarginRX, printsetting.m_nPrintPaperOrientation, printsetting.m_nPrintPaperSize, printsetting.m_bPrintWordWrap ? 1 : 0, printsetting.m_bPrintLineNumber ? 1 : 0, printsetting.m_bHeaderUse[0] ? 1 : 0, printsetting.m_bHeaderUse[1] ? 1 : 0, printsetting.m_bHeaderUse[2] ? 1 : 0, printsetting.m_bFooterUse[0] ? 1 : 0, printsetting.m_bFooterUse[1] ? 1 : 0, printsetting.m_bFooterUse[2] ? 1 : 0);
            cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szKeyData));
        }
        ...
        { // ヘッダ/フッタ フォント設定
            ...
            ShareData_IO_Sub_LogFont(cProfile, pszSecName, szKeyName, szKeyName2, szKeyName3, printsetting.m_lfHeader, printsetting.m_nHeaderPointSize);
            ...
            ShareData_IO_Sub_LogFont(cProfile, pszSecName, szKeyName, szKeyName2, szKeyName3, printsetting.m_lfFooter, printsetting.m_nFooterPointSize);
        }
...
void CShareData_IO::ShareData_IO_Type_One(CDataProfile &cProfile, STypeConfig &types, const WCHAR *pszSecName)
{
    ...
    {
        auto_sprintf(szKeyData, pszForm, types.m_nIdx, types.m_nMaxLineKetas, types.m_nColumnSpace, types.m_nTabSpace, types.m_nKeyWordSetIdx[0], types.m_nKeyWordSetIdx[1], types.m_nStringType, types.m_bLineNumIsCRLF ? 1 : 0, types.m_nLineTermType, types.m_bWordWrap ? 1 : 0, types.m_nCurrentPrintSetting, types.m_nTsvMode);
        cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szKeyData));
    }
    ...
    if (cProfile.IsReadingMode())
    {
        ...
        if (bRet1 && bRet2)
            types.m_cBlockComments[0].SetBlockCommentRule(buffer[0], buffer[1]);
        ...
        if (bRet1 && bRet2)
            types.m_cBlockComments[1].SetBlockCommentRule(buffer[0], buffer[1]);
        ...
        if (bRet1 && bRet2)
            types.m_cLineComment.CopyTo(0, lbuf, pos);
        ...
        if (bRet1 && bRet2)
            types.m_cLineComment.CopyTo(1, lbuf, pos);
        ...
        if (bRet1 && bRet2)
            types.m_cLineComment.CopyTo(2, lbuf, pos);
    }
    else
    { // write
        //	Block Comment
        cProfile.IOProfileData(pszSecName, LTEXT("szBlockCommentFrom"), MakeStringBufferW0(const_cast<wchar_t *>(types.m_cBlockComments[0].getBlockCommentFrom())));
        cProfile.IOProfileData(pszSecName, LTEXT("szBlockCommentTo"), MakeStringBufferW0(const_cast<wchar_t *>(types.m_cBlockComments[0].getBlockCommentTo())));

        //@@@ 2001.03.10 by MIK
        cProfile.IOProfileData(pszSecName, LTEXT("szBlockCommentFrom2"), MakeStringBufferW0(const_cast<wchar_t *>(types.m_cBlockComments[1].getBlockCommentFrom())));
        cProfile.IOProfileData(pszSecName, LTEXT("szBlockCommentTo2"), MakeStringBufferW0(const_cast<wchar_t *>(types.m_cBlockComments[1].getBlockCommentTo())));

        //	Line Comment
        cProfile.IOProfileData(pszSecName, LTEXT("szLineComment"), MakeStringBufferW0(const_cast<wchar_t *>(types.m_cLineComment.getLineComment(0))));
        cProfile.IOProfileData(pszSecName, LTEXT("szLineComment2"), MakeStringBufferW0(const_cast<wchar_t *>(types.m_cLineComment.getLineComment(1))));
    ...
    { //正規表現キーワード
        ...
        for (j = 0; j < _countof(types.m_RegexKeywordArr); j++)
        {
                ...
                if (cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szKeyData)))
                {
                    ...
                    if (p)
                    {
                        *p                                       = LTEXT('\0');
                        ...
                        if (types.m_RegexKeywordArr[j].m_nColorIndex < 0 || types.m_RegexKeywordArr[j].m_nColorIndex >= COLORIDX_LAST)
                        {
                            types.m_RegexKeywordArr[j].m_nColorIndex = COLORIDX_REGEX1;
                        }
                        if (pKeyword[nPos])
            ...
            {
                auto_sprintf(szKeyData, LTEXT("%ls,%ls"), GetColorNameByIndex(types.m_RegexKeywordArr[j].m_nColorIndex), &pKeyword[nPos]);
                cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szKeyData));
                nPos += wcslen(&pKeyword[nPos]) + 1;
            }
        }
    ...
    { /* キーワード辞書 */
        ...
        for (j = 0; j < MAX_KEYHELP_FILE; j++)
        {
            ...
            else
            {
                ...
                {
                    auto_sprintf(szKeyData, LTEXT("%d,%s,%s"), types.m_KeyHelpArr[j].m_bUse ? 1 : 0, types.m_KeyHelpArr[j].m_szAbout, types.m_KeyHelpArr[j].m_szPath.c_str());
                    cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szKeyData));
                }
            }
        }
    ...
    { // フォント設定
        ...
        ShareData_IO_Sub_LogFont(cProfile, pszSecName, L"lf", L"nPointSize", L"lfFaceName", types.m_lf, types.m_nPointSize);
    }
}
...
void CShareData_IO::ShareData_IO_Macro(CDataProfile &cProfile)
{
    ...
    for (i = 0; i < MAX_CUSTMACRO; ++i)
    {
        ...
        // 2002.02.08 hor 未定義値を無視
        if (!cProfile.IsReadingMode() && macrorec.m_szName[0] == L'\0' && macrorec.m_szFile[0] == L'\0')
            continue;
...
void CShareData_IO::IO_MainMenu(CDataProfile &cProfile, std::vector<std::wstring> *pData, CommonSetting_MainMenu &mainmenu, bool bOutCmdName)
{
    ...
    for (int i = 0; i < mainmenu.m_nMainMenuNum; i++)
    {
        ...
        else
        {
            ...
            auto_sprintf(szLine, L"%d,%d,%ls,%ls,%ls", pcMenu->m_nLevel, pcMenu->m_nType, szFuncName, pcMenu->m_sKey, pcMenu->m_nFunc == F_NODE ? pcMenu->m_sName : L"");
            cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szLine));
        }
...
void CShareData_IO::ShareData_IO_Other(CDataProfile &cProfile)
{
    ...
    {
        WCHAR iniVer[256];
        auto_sprintf(iniVer, L"%d.%d.%d.%d", HIWORD(pShare->m_sVersion.m_dwProductVersionMS), LOWORD(pShare->m_sVersion.m_dwProductVersionMS), HIWORD(pShare->m_sVersion.m_dwProductVersionLS), LOWORD(pShare->m_sVersion.m_dwProductVersionLS));
...
void CShareData_IO::IO_ColorSet(CDataProfile *pcProfile, const WCHAR *pszSecName, ColorInfo *pColorInfoArr)
{
    ...
    for (j = 0; j < COLORIDX_LAST; ++j)
    {
        ...
        {
            auto_sprintf(szKeyData, pszForm, pColorInfoArr[j].m_bDisp ? 1 : 0, pColorInfoArr[j].m_sFontAttr.m_bBoldFont ? 1 : 0, pColorInfoArr[j].m_sColorAttr.m_cTEXT, pColorInfoArr[j].m_sColorAttr.m_cBACK, pColorInfoArr[j].m_sFontAttr.m_bUnderLine ? 1 : 0);
...
void ShareData_IO_Sub_LogFont(CDataProfile &cProfile, const WCHAR *pszSecName, const WCHAR *pszKeyLf, const WCHAR *pszKeyPointSize, const WCHAR *pszKeyFaceName, LOGFONT &lf, INT &nPointSize)
{
    ...
    {
        auto_sprintf(szKeyData, pszForm, lf.lfHeight, lf.lfWidth, lf.lfEscapement, lf.lfOrientation, lf.lfWeight, lf.lfItalic, lf.lfUnderline, lf.lfStrikeOut, lf.lfCharSet, lf.lfOutPrecision, lf.lfClipPrecision, lf.lfQuality, lf.lfPitchAndFamily);
    ...
    CDataProfile &cProfile, SFileTreeItem &item, const WCHAR *pszSecName, int i)
{
    ...
    if (cProfile.IsReadingMode() || item.m_eFileTreeItemType == EFileTreeItemType_Grep || item.m_eFileTreeItemType == EFileTreeItemType_File)
    {
        auto_sprintf(szKey, L"FileTree(%d).szTargetPath", i);
        cProfile.IOProfileData(pszSecName, szKey, item.m_szTargetPath);
    }
    if (cProfile.IsReadingMode() || ((item.m_eFileTreeItemType == EFileTreeItemType_Grep || item.m_eFileTreeItemType == EFileTreeItemType_File) && item.m_szLabelName[0] != L'\0') || item.m_eFileTreeItemType == EFileTreeItemType_Folder)
    {
