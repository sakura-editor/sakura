...
void ShareData_IO_Sub_LogFont(CDataProfile &cProfile, const WCHAR *pszSecName, const WCHAR *pszKeyLf,
							  const WCHAR *pszKeyPointSize, const WCHAR *pszKeyFaceName, LOGFONT &lf, INT &nPointSize);
...
/* 共有データのロード */
bool CShareData_IO::LoadShareData() { return ShareData_IO_2(true); }

/* 共有データの保存 */
void CShareData_IO::SaveShareData() { ShareData_IO_2(false); }
...
bool CShareData_IO::ShareData_IO_2(bool bRead)
{
	...
	if (bRead) { cProfile.SetReadingMode(); }
	else
	{
	...
	CFileNameManager::getInstance()->GetIniFileName(szIniFileName, pszProfileName,
													bRead); // 2007.05.19 ryoji iniファイル名を取得する
	...
	if (bRead)
	{
		if (!cProfile.ReadProfile(szIniFileName))
		{
			...
			if (langId != MAKELANGID(LANG_JAPANESE, SUBLANG_JAPANESE_JAPAN))
			{
				...
				cProfile.IOProfileData(L"Common", L"szLanguageDll",
									   MakeStringBufferW(pShareData->m_Common.m_sWindow.m_szLanguageDll));
...
void CShareData_IO::ShareData_IO_Mru(CDataProfile &cProfile)
{
	...
	for (i = 0; i < nSize; ++i)
	{
		...
		if (cProfile.IsReadingMode()) { pfiWork->m_nTypeId = -1; }
...
void CShareData_IO::ShareData_IO_Keys(CDataProfile &cProfile)
{
	...
	cProfile.IOProfileData(pszSecName, LTEXT("_SEARCHKEY_Counts"),
						   pShare->m_sSearchKeywords.m_aSearchKeys._GetSizeRef());
	...
	cProfile.IOProfileData(pszSecName, LTEXT("_REPLACEKEY_Counts"),
						   pShare->m_sSearchKeywords.m_aReplaceKeys._GetSizeRef());
...
void CShareData_IO::ShareData_IO_Grep(CDataProfile &cProfile)
{
	...
	cProfile.IOProfileData(pszSecName, LTEXT("_GREPFOLDER_Counts"),
						   pShare->m_sSearchKeywords.m_aGrepFolders._GetSizeRef());
	...
	/* 除外ファイルパターン */
	cProfile.IOProfileData(pszSecName, LTEXT("_GREPEXCLUDEFILE_Counts"),
						   pShare->m_sSearchKeywords.m_aExcludeFiles._GetSizeRef());
	...
	/* 除外フォルダパターン */
	cProfile.IOProfileData(pszSecName, LTEXT("_GREPEXCLUDEFOLDER_Counts"),
						   pShare->m_sSearchKeywords.m_aExcludeFolders._GetSizeRef());
...
void CShareData_IO::ShareData_IO_Nickname(CDataProfile &cProfile)
{
	...
	cProfile.IOProfileData(pszSecName, LTEXT("nShortPathMaxWidth"),
						   pShare->m_Common.m_sFileName.m_nTransformShortMaxWidth);
	...
	for (i = 0; i < nSize; ++i)
	{
		...
		cProfile.IOProfileData(pszSecName, szKeyName,
							   MakeStringBufferW(pShare->m_Common.m_sFileName.m_szTransformFileNameFrom[i]));
		auto_sprintf(szKeyName, LTEXT("To%02d"), i);
		cProfile.IOProfileData(pszSecName, szKeyName,
							   MakeStringBufferW(pShare->m_Common.m_sFileName.m_szTransformFileNameTo[i]));
	}
	// 読み込み時，残りをNULLで再初期化
	if (cProfile.IsReadingMode())
	{
...
static bool ShareData_IO_RECT(CDataProfile &cProfile, const WCHAR *pszSecName, const WCHAR *pszKeyName, RECT &rcValue)
{
	...
	{
		auto_sprintf(szKeyData, pszForm, rcValue.left, rcValue.top, rcValue.right, rcValue.bottom);
		ret = cProfile.IOProfileData(pszSecName, pszKeyName, MakeStringBufferW(szKeyData));
	}
	return ret;
}
...
void CShareData_IO::ShareData_IO_Common(CDataProfile &cProfile)
{
	...
	cProfile.IOProfileData(pszSecName, LTEXT("bStopsBothEndsWhenSearchWord"),
						   common.m_sGeneral.m_bStopsBothEndsWhenSearchWord);
	cProfile.IOProfileData(pszSecName, LTEXT("bStopsBothEndsWhenSearchParagraph"),
						   common.m_sGeneral.m_bStopsBothEndsWhenSearchParagraph);
	//	Oct. 27, 2000 genta
	...
	cProfile.IOProfileData(
		pszSecName, LTEXT("szOpenDialogSelDir"),
		StringBufferW(common.m_sEdit.m_OpenDialogSelDir, _countof2(common.m_sEdit.m_OpenDialogSelDir)));
	...
	cProfile.IOProfileData(pszSecName, LTEXT("nPageScrollByWheel"),
						   common.m_sGeneral.m_nPageScrollByWheel); // 2009.01.17 nasukoji
	cProfile.IOProfileData(pszSecName, LTEXT("nHorizontalScrollByWheel"),
						   common.m_sGeneral.m_nHorizontalScrollByWheel); // 2009.01.17 nasukoji
	cProfile.IOProfileData(pszSecName, LTEXT("bCloseAllConfirm"), common.m_sGeneral.m_bCloseAllConfirm);
	...
	cProfile.IOProfileData(pszSecName, LTEXT("bSearchConsecutiveAll"),
						   common.m_sSearch.m_bConsecutiveAll); // 2007.01.16 ryoji
	...
	cProfile.IOProfileData(
		pszSecName, LTEXT("bCaretTextForSearch"),
		common.m_sSearch.m_bCaretTextForSearch); // 2006.08.23 ryoji カーソル位置の文字列をデフォルトの検索文字列にする
	...
	if (!cProfile.IsReadingMode())
	{
		...
		nCharChars = &common.m_sBackup.m_szBackUpFolder[nDummy]
					 - CNativeW::GetCharPrev(common.m_sBackup.m_szBackUpFolder, nDummy,
											 &common.m_sBackup.m_szBackUpFolder[nDummy]);
	...
	cProfile.IOProfileData(pszSecName, LTEXT("bBackUpDustBox"),
						   common.m_sBackup.m_bBackUpDustBox); //@@@ 2001.12.11 add MIK
	cProfile.IOProfileData(pszSecName, LTEXT("bBackUpPathAdvanced"),
						   common.m_sBackup.m_bBackUpPathAdvanced); /* 20051107 aroka */
	cProfile.IOProfileData(pszSecName, LTEXT("szBackUpPathAdvanced"),
						   common.m_sBackup.m_szBackUpPathAdvanced); /* 20051107 aroka */
	...
	// ai 02/05/23 Add S
	{ // Keword Help Font
		ShareData_IO_Sub_LogFont(cProfile, pszSecName, L"khlf", L"khps", L"khlfFaceName", common.m_sHelper.m_lf,
								 common.m_sHelper.m_nPointSize);
	} // Keword Help Font
	...
	cProfile.IOProfileData(
		pszSecName, LTEXT("nFUNCKEYWND_GroupNum"),
		common.m_sWindow.m_nFUNCKEYWND_GroupNum); // 2002/11/04 Moca ファンクションキーのグループボタン数
	...
	cProfile.IOProfileData(pszSecName, LTEXT("bDispTabWnd"),
						   common.m_sTabBar.m_bDispTabWnd); //タブウインドウ	//@@@ 2003.05.31 MIK
	cProfile.IOProfileData(pszSecName, LTEXT("bDispTabWndMultiWin"),
						   common.m_sTabBar.m_bDispTabWndMultiWin); //タブウインドウ	//@@@ 2003.05.31 MIK
	cProfile.IOProfileData(pszSecName, LTEXT("szTabWndCaption"),
						   MakeStringBufferW(common.m_sTabBar.m_szTabWndCaption)); //@@@ 2003.06.13 MIK
	cProfile.IOProfileData(pszSecName, LTEXT("bSameTabWidth"),
						   common.m_sTabBar.m_bSameTabWidth); // 2006.01.28 ryoji タブを等幅にする
	cProfile.IOProfileData(pszSecName, LTEXT("bDispTabIcon"),
						   common.m_sTabBar.m_bDispTabIcon); // 2006.01.28 ryoji タブにアイコンを表示する
	cProfile.IOProfileData_WrapInt(pszSecName, LTEXT("bDispTabClose"),
								   common.m_sTabBar.m_bDispTabClose); // 2012.04.14 syat
	cProfile.IOProfileData(pszSecName, LTEXT("bSortTabList"),
						   common.m_sTabBar.m_bSortTabList); // 2006.05.10 ryoji タブ一覧をソートする
	cProfile.IOProfileData(
		pszSecName, LTEXT("bTab_RetainEmptyWin"),
		common.m_sTabBar.m_bTab_RetainEmptyWin); // 最後のファイルが閉じられたとき(無題)を残す	// 2007.02.11 genta
	cProfile.IOProfileData(
		pszSecName, LTEXT("bTab_CloseOneWin"),
		common.m_sTabBar.m_bTab_CloseOneWin); // タブモードでもウィンドウの閉じるボタンで現在のファイルのみ閉じる
											  // // 2007.02.11 genta
	cProfile.IOProfileData(pszSecName, LTEXT("bTab_ListFull"),
						   common.m_sTabBar.m_bTab_ListFull); // タブ一覧をフルパス表示する	// 2007.02.28 ryoji
	cProfile.IOProfileData(pszSecName, LTEXT("bChgWndByWheel"),
						   common.m_sTabBar.m_bChgWndByWheel); // 2006.03.26 ryoji マウスホイールでウィンドウ切り替え
	cProfile.IOProfileData(pszSecName, LTEXT("bNewWindow"),
						   common.m_sTabBar.m_bNewWindow); // 外部から起動するときは新しいウインドウで開く
	...
	cProfile.IOProfileData_WrapInt(pszSecName, LTEXT("bSaveWindowSize"),
								   common.m_sWindow.m_eSaveWindowSize); //#####フラグ名が激しくきもい
	...
	cProfile.IOProfileData_WrapInt(pszSecName, LTEXT("nSaveWindowPos"),
								   common.m_sWindow.m_eSaveWindowPos); //#####フラグ名がきもい
	...
	cProfile.IOProfileData(pszSecName, LTEXT("bEnableUnmodifiedOverwrite"),
						   common.m_sFile.m_bEnableUnmodifiedOverwrite);
	cProfile.IOProfileData(pszSecName, LTEXT("bSelectClickedURL"), common.m_sEdit.m_bSelectClickedURL);
	cProfile.IOProfileData(pszSecName, LTEXT("bGrepExitConfirm"),
						   common.m_sSearch.m_bGrepExitConfirm); /* Grepモードで保存確認するか */
	...
	cProfile.IOProfileData(pszSecName, LTEXT("nRulerBottomSpace"),
						   common.m_sWindow.m_nRulerBottomSpace); /* ルーラーとテキストの隙間 */
	cProfile.IOProfileData(pszSecName, LTEXT("nRulerType"), common.m_sWindow.m_nRulerType); /* ルーラーのタイプ */
	//	Sep. 18, 2002 genta 追加
	cProfile.IOProfileData(pszSecName, LTEXT("nLineNumberRightSpace"),
						   common.m_sWindow.m_nLineNumRightSpace); /* 行番号の右側の隙間 */
	cProfile.IOProfileData(pszSecName, LTEXT("nVertLineOffset"), common.m_sWindow.m_nVertLineOffset); // 2005.11.10 Moca
	cProfile.IOProfileData(pszSecName, LTEXT("bUseCompotibleBMP"),
						   common.m_sWindow.m_bUseCompatibleBMP); // 2007.09.09 Moca
	cProfile.IOProfileData(pszSecName, LTEXT("bCopyAndDisablSelection"),
						   common.m_sEdit.m_bCopyAndDisablSelection); /* コピーしたら選択解除 */
	cProfile.IOProfileData(pszSecName, LTEXT("bEnableNoSelectCopy"), common.m_sEdit.m_bEnableNoSelectCopy);
	/* 選択なしでコピーを可能にする */ // 2007.11.18 ryoji
	cProfile.IOProfileData(pszSecName, LTEXT("bEnableLineModePaste"), common.m_sEdit.m_bEnableLineModePaste);
	/* ラインモード貼り付けを可能にする */ // 2007.10.08 ryoji
	cProfile.IOProfileData(pszSecName, LTEXT("bConvertEOLPaste"), common.m_sEdit.m_bConvertEOLPaste);
	/* 改行コードを変換して貼り付ける */ // 2009.02.28 salarm
	cProfile.IOProfileData(pszSecName, LTEXT("bEnableExtEol"), common.m_sEdit.m_bEnableExtEol);

	cProfile.IOProfileData(pszSecName, LTEXT("bHtmlHelpIsSingle"),
						   common.m_sHelper.m_bHtmlHelpIsSingle); /* HtmlHelpビューアはひとつ */
	cProfile.IOProfileData(pszSecName, LTEXT("bCompareAndTileHorz"), common.m_sCompare.m_bCompareAndTileHorz);
	/* 文書比較後、左右に並べて表示 */ // Oct. 10, 2000 JEPRO チェックボックスをボタン化すればこの行は不要のはず
	cProfile.IOProfileData(pszSecName, LTEXT("bDropFileAndClose"),
						   common.m_sFile.m_bDropFileAndClose); /* ファイルをドロップしたときは閉じて開く */
	cProfile.IOProfileData(pszSecName, LTEXT("nDropFileNumMax"),
						   common.m_sFile.m_nDropFileNumMax); /* 一度にドロップ可能なファイル数 */
	cProfile.IOProfileData(pszSecName, LTEXT("bCheckFileTimeStamp"),
						   common.m_sFile.m_bCheckFileTimeStamp);								  /* 更新の監視 */
	cProfile.IOProfileData(pszSecName, LTEXT("nAutoloadDelay"), common.m_sFile.m_nAutoloadDelay); /* 自動読込時遅延 */
	cProfile.IOProfileData(pszSecName, LTEXT("bUneditableIfUnwritable"),
						   common.m_sFile.m_bUneditableIfUnwritable); /* 上書き禁止検出時は編集禁止にする */
	cProfile.IOProfileData(pszSecName, LTEXT("bNotOverWriteCRLF"),
						   common.m_sEdit.m_bNotOverWriteCRLF); /* 改行は上書きしない */
	cProfile.IOProfileData(pszSecName, LTEXT("bOverWriteFixMode"),
						   common.m_sEdit.m_bOverWriteFixMode); // 文字幅に合わせてスペースを詰める
	cProfile.IOProfileData(pszSecName, LTEXT("bOverWriteBoxDelete"), common.m_sEdit.m_bOverWriteBoxDelete);
	cProfile.IOProfileData(pszSecName, LTEXT("bAutoCloseDlgFind"),
						   common.m_sSearch.m_bAutoCloseDlgFind); /* 検索ダイアログを自動的に閉じる */
	cProfile.IOProfileData(pszSecName, LTEXT("bAutoCloseDlgFuncList"),
						   common.m_sOutline.m_bAutoCloseDlgFuncList); /* アウトライン ダイアログを自動的に閉じる */
	cProfile.IOProfileData(pszSecName, LTEXT("bAutoCloseDlgReplace"),
						   common.m_sSearch.m_bAutoCloseDlgReplace); /* 置換 ダイアログを自動的に閉じる */
	cProfile.IOProfileData(pszSecName, LTEXT("bAutoColmnPaste"), common.m_sEdit.m_bAutoColumnPaste);
	/* 矩形コピーのテキストは常に矩形貼り付け */ // 2013.5.23 aroka iniファイルのtypo未修正
	cProfile.IOProfileData(
		pszSecName, LTEXT("NoCaretMoveByActivation"),
		common.m_sGeneral
			.m_bNoCaretMoveByActivation); /* マウスクリックにてアクティベートされた時はカーソル位置を移動しない
											 2007.10.02 nasukoji (add by genta) */
	cProfile.IOProfileData(pszSecName, LTEXT("bScrollBarHorz"),
						   common.m_sWindow.m_bScrollBarHorz); /* 水平スクロールバーを使う */

	cProfile.IOProfileData(pszSecName, LTEXT("bHokanKey_RETURN"),
						   common.m_sHelper.m_bHokanKey_RETURN); /* VK_RETURN 補完決定キーが有効/無効 */
	cProfile.IOProfileData(pszSecName, LTEXT("bHokanKey_TAB"),
						   common.m_sHelper.m_bHokanKey_TAB); /* VK_TAB    補完決定キーが有効/無効 */
	cProfile.IOProfileData(pszSecName, LTEXT("bHokanKey_RIGHT"),
						   common.m_sHelper.m_bHokanKey_RIGHT); /* VK_RIGHT  補完決定キーが有効/無効 */
	cProfile.IOProfileData(pszSecName, LTEXT("bHokanKey_SPACE"),
						   common.m_sHelper.m_bHokanKey_SPACE); /* VK_SPACE  補完決定キーが有効/無効 */

	cProfile.IOProfileData(pszSecName, LTEXT("nDateFormatType"),
						   common.m_sFormat.m_nDateFormatType); /* 日付書式のタイプ */
	cProfile.IOProfileData(pszSecName, LTEXT("szDateFormat"),
						   MakeStringBufferW(common.m_sFormat.m_szDateFormat)); //日付書式
	cProfile.IOProfileData(pszSecName, LTEXT("nTimeFormatType"),
						   common.m_sFormat.m_nTimeFormatType); /* 時刻書式のタイプ */
	cProfile.IOProfileData(pszSecName, LTEXT("szTimeFormat"),
						   MakeStringBufferW(common.m_sFormat.m_szTimeFormat)); //時刻書式

	cProfile.IOProfileData(pszSecName, LTEXT("bMenuIcon"), common.m_sWindow.m_bMenuIcon); //メニューにアイコンを表示する
	cProfile.IOProfileData(pszSecName, LTEXT("bAutoMIMEdecode"),
						   common.m_sFile.m_bAutoMIMEdecode); //ファイル読み込み時にMIMEのdecodeを行うか
	cProfile.IOProfileData(
		pszSecName, LTEXT("bQueryIfCodeChange"),
		common.m_sFile.m_bQueryIfCodeChange); //	Oct. 03, 2004 genta 前回と異なる文字コードのときに問い合わせを行うか
	cProfile.IOProfileData(
		pszSecName, LTEXT("bAlertIfFileNotExist"),
		common.m_sFile.m_bAlertIfFileNotExist); // Oct. 09, 2004 genta 開こうとしたファイルが存在しないとき警告する

	cProfile.IOProfileData(pszSecName, LTEXT("bNoFilterSaveNew"),
						   common.m_sFile.m_bNoFilterSaveNew); // 新規から保存時は全ファイル表示	// 2006.11.16 ryoji
	cProfile.IOProfileData(
		pszSecName, LTEXT("bNoFilterSaveFile"),
		common.m_sFile.m_bNoFilterSaveFile); // 新規以外から保存時は全ファイル表示	// 2006.11.16 ryoji
	cProfile.IOProfileData(pszSecName, LTEXT("bAlertIfLargeFile"),
						   common.m_sFile.m_bAlertIfLargeFile); // 開こうとしたファイルが大きい場合に警告する
	cProfile.IOProfileData(pszSecName, LTEXT("nAlertFileSize"),
						   common.m_sFile.m_nAlertFileSize); // 警告を開始するファイルサイズ(MB単位)
	...
	cProfile.IOProfileData(pszSecName, LTEXT("szWinCaptionActive"),
						   MakeStringBufferW(common.m_sWindow.m_szWindowCaptionActive));
	cProfile.IOProfileData(pszSecName, LTEXT("szWinCaptionInactive"),
						   MakeStringBufferW(common.m_sWindow.m_szWindowCaptionInactive));

	// アウトライン/トピックリスト の位置とサイズを記憶  20060201 aroka
	cProfile.IOProfileData(pszSecName, LTEXT("bRememberOutlineWindowPos"),
						   common.m_sOutline.m_bRememberOutlineWindowPos);
	...
	cProfile.IOProfileData_WrapInt(pszSecName, LTEXT("eOutlineDockSide"), common.m_sOutline.m_eOutlineDockSide);
	{
		...
		{
			auto_sprintf(szKeyData, pszForm, common.m_sOutline.m_cxOutlineDockLeft,
						 common.m_sOutline.m_cyOutlineDockTop, common.m_sOutline.m_cxOutlineDockRight,
...
EFunctionCode GetPlugCmdInfoByName(const WCHAR *pszFuncName //!< [in]  プラグインコマンド名
)
{
	...
	if ((psCmdName = wcschr(pszFuncName, L'/')) == NULL) { return F_INVALID; }
...
bool GetPlugCmdInfoByFuncCode(EFunctionCode eFuncCode, //!< [in]  機能コード
							  WCHAR *pszFuncName //!< [out] 機能名．この先にはMAX_PLUGIN_ID + 20文字のメモリが必要．
)
{
	CommonSetting_Plugin &plugin = GetDllShareData().m_Common.m_sPlugin;

	if (eFuncCode < F_PLUGCOMMAND_FIRST || eFuncCode > F_PLUGCOMMAND_LAST) { return false; }

	PluginId nID = CPlug::GetPluginId(eFuncCode);
	PlugId   nNo = CPlug::GetPlugId(eFuncCode);
	if (nID < 0 || nNo < 0) { return false; }
...
{
	EFunctionCode n;
	if (pszFuncName == NULL) { n = F_DEFAULT; }
	else if (wcschr(pszFuncName, L'/') != NULL)
	{
		// Plugin名
		n = GetPlugCmdInfoByName(pszFuncName);
	}
	else if (WCODE::Is09(pszFuncName[0]) && (pszFuncName[1] == L'\0' || WCODE::Is09(pszFuncName[1])))
	{
...
void CShareData_IO::ShareData_IO_Toolbar(CDataProfile &cProfile, CMenuDrawer *pcMenuDrawer)
{
	...
	for (i = 0; i < nSize; ++i)
	{
		...
		{
			//書き込み
			if (toolbar.m_nToolBarButtonIdxArr[i] <= MAX_TOOLBAR_ICON_COUNT + 1)
			{ // +1はセパレータ分
			...
			else
			{
				...
				if (eFunc == F_DEFAULT) { cProfile.IOProfileData(pszSecName, szKeyName, nInvalid); }
	...
	if (cProfile.IsReadingMode())
	{
		for (; i < MAX_TOOLBAR_BUTTON_ITEMS; ++i) { toolbar.m_nToolBarButtonIdxArr[i] = 0; }
	}
}
...
void CShareData_IO::IO_CustMenu(CDataProfile &cProfile, CommonSetting_CustomMenu &menu, bool bOutCmdName)
{
	...
	for (i = 0; i < MAX_CUSTOM_MENU; ++i)
	{
		...
		cProfile.IOProfileData(pszSecName, szKeyName,
							   MakeStringBufferW(menu.m_szCustMenuNameArr[i])); //	Oct. 15, 2001 genta 最大長指定
		...
		for (j = 0; j < nSize; ++j)
		{
			...
			else
			{
				...
				else
				{
					if (bOutCmdName)
					{
						WCHAR *p = CSMacroMgr::GetFuncInfoByID(G_AppInstance(), menu.m_nCustMenuItemFuncArr[i][j],
															   szFuncName, NULL);
						if (p == NULL) { auto_sprintf(szFuncName, L"%d", menu.m_nCustMenuItemFuncArr[i][j]); }
						cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szFuncName));
					}
					else
					{
...
void CShareData_IO::ShareData_IO_Font(CDataProfile &cProfile)
{
	...
	ShareData_IO_Sub_LogFont(cProfile, pszSecName, L"lf", L"nPointSize", L"lfFaceName", view.m_lf, view.m_nPointSize);
...
void CShareData_IO::IO_KeyBind(CDataProfile &cProfile, CommonSetting_KeyBind &sKeyBind, bool bOutCmdName)
{
	...
	if (cProfile.IsReadingMode())
	{
		if (!cProfile.IOProfileData(szSecName, L"KeyBind[000]", MakeStringBufferW(szKeyData))) { bOldVer = true; }
		else
		{
	...
	for (i = 0; i < sKeyBind.m_nKeyNameArrNum; ++i)
	{
		...
		if (cProfile.IsReadingMode())
		{
			...
			{ // 新バージョン(キー割り当てのImport,export の合わせた)	2008/5/25 Uchi
				KEYDATA tmpKeydata;
				...
				if (cProfile.IOProfileData(szSecName, szKeyName, MakeStringBufferW(szKeyData)))
				{
					...
					*pn  = 0;
					...
					if (tmpKeydata.m_nKeyCode <= 0)
					{ // マウスコードは先頭に固定されている KeyCodeが同じなのでKeyNameで判別
		...
		else
		{
			...
			for (int j = 0; j < 8; j++)
			{
				...
				else
				{
					if (bOutCmdName)
					{
						...
						WCHAR *p = CSMacroMgr::GetFuncInfoByID(0, keydata.m_nFuncCodeArr[j], szFuncName, NULL);
						if (p) { auto_sprintf(szWork, L",%ls", p); }
						else
						{
			...
			{ auto_sprintf(szWork, L",%s", jpVKEXNames[keydata.m_nKeyCode - 0x0100]); } else
			{
	...
	if (cProfile.IsReadingMode()) { sKeyBind.m_nKeyNameArrNum = nKeyNameArrUsed; }
}
...
void CShareData_IO::ShareData_IO_Print(CDataProfile &cProfile)
{
	...
	for (i = 0; i < MAX_PRINTSETTINGARR; ++i)
	{
		...
		{
			auto_sprintf(szKeyData, pszForm, printsetting.m_nPrintFontWidth, printsetting.m_nPrintFontHeight,
						 printsetting.m_nPrintDansuu, printsetting.m_nPrintDanSpace, printsetting.m_nPrintLineSpacing,
						 printsetting.m_nPrintMarginTY, printsetting.m_nPrintMarginBY, printsetting.m_nPrintMarginLX,
						 printsetting.m_nPrintMarginRX, printsetting.m_nPrintPaperOrientation,
						 printsetting.m_nPrintPaperSize, printsetting.m_bPrintWordWrap ? 1 : 0,
						 printsetting.m_bPrintLineNumber ? 1 : 0, printsetting.m_bHeaderUse[0] ? 1 : 0,
						 printsetting.m_bHeaderUse[1] ? 1 : 0, printsetting.m_bHeaderUse[2] ? 1 : 0,
						 printsetting.m_bFooterUse[0] ? 1 : 0, printsetting.m_bFooterUse[1] ? 1 : 0,
		...
		{ // ヘッダ/フッタ フォント設定
			...
			ShareData_IO_Sub_LogFont(cProfile, pszSecName, szKeyName, szKeyName2, szKeyName3, printsetting.m_lfHeader,
			...
			ShareData_IO_Sub_LogFont(cProfile, pszSecName, szKeyName, szKeyName2, szKeyName3, printsetting.m_lfFooter,
									 printsetting.m_nFooterPointSize);
		}

		auto_sprintf(szKeyName, LTEXT("PS[%02d].szDriver"), i);
		cProfile.IOProfileData(pszSecName, szKeyName,
							   MakeStringBufferW(printsetting.m_mdmDevMode.m_szPrinterDriverName));
		auto_sprintf(szKeyName, LTEXT("PS[%02d].szDevice"), i);
		cProfile.IOProfileData(pszSecName, szKeyName,
							   MakeStringBufferW(printsetting.m_mdmDevMode.m_szPrinterDeviceName));
		auto_sprintf(szKeyName, LTEXT("PS[%02d].szOutput"), i);
		cProfile.IOProfileData(pszSecName, szKeyName,
							   MakeStringBufferW(printsetting.m_mdmDevMode.m_szPrinterOutputName));
		...
		//禁則	//@@@ 2002.04.09 MIK
		auto_sprintf(szKeyName, LTEXT("PS[%02d].bKinsokuHead"), i);
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_bPrintKinsokuHead);
		auto_sprintf(szKeyName, LTEXT("PS[%02d].bKinsokuTail"), i);
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_bPrintKinsokuTail);
		auto_sprintf(szKeyName, LTEXT("PS[%02d].bKinsokuRet"), i);
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_bPrintKinsokuRet); //@@@ 2002.04.13 MIK
		auto_sprintf(szKeyName, LTEXT("PS[%02d].bKinsokuKuto"), i);
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_bPrintKinsokuKuto); //@@@ 2002.04.17 MIK

		//カラー印刷
		auto_sprintf(szKeyName, LTEXT("PS[%02d].bColorPrint"), i);
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_bColorPrint); // 2013/4/26 Uchi
	}
}
...
void CShareData_IO::ShareData_IO_Types(CDataProfile &cProfile)
{
	...
	for (i = 0; i < pShare->m_nTypesCount; ++i)
	{
		...
		{
			type.m_nIdx = i;
			if (i == 0) { pShare->m_TypeBasis = type; }
 ...
 */
static bool ShareData_IO_BlockComment(CDataProfile &cProfile, const WCHAR *pszSectionName, const WCHAR *pszEntryKeyFrom,
									  const WCHAR *pszEntryKeyTo, CBlockComment &cBlockComment) noexcept
{
	...
	// 読み込み後処理
	if (cProfile.IsReadingMode() && ret) { cBlockComment.SetBlockCommentRule(szFrom, szTo); }
 ...
 * @date 2020/01/01 berryzplus ShareData_IO_Type_Oneから分離
 */
static bool ShareData_IO_LineComment(CDataProfile &cProfile, const WCHAR *pszSectionName,
									 const WCHAR *pszEntryKeyComment, const WCHAR *pszEntryKeyColumn,
									 CLineComment &cLineComment, const int nDataIndex) noexcept
{
	...
	// 読み込み後処理
	if (cProfile.IsReadingMode() && ret) { cLineComment.CopyTo(nDataIndex, lbuf, pos); }
...
void CShareData_IO::ShareData_IO_Type_One(CDataProfile &cProfile, STypeConfig &types, const WCHAR *pszSecName)
{
	...
	if (cProfile.IsReadingMode())
	{
		...
		if (types.m_nMaxLineKetas < CKetaXInt(MINLINEKETAS)) { types.m_nMaxLineKetas = CKetaXInt(MINLINEKETAS); }
		if (types.m_nMaxLineKetas - 2 < types.m_nTabSpace) { types.m_nTabSpace = types.m_nMaxLineKetas - 2; }
	}
	else
	{
		auto_sprintf(szKeyData, pszForm, types.m_nIdx, types.m_nMaxLineKetas, types.m_nColumnSpace, types.m_nTabSpace,
					 types.m_nKeyWordSetIdx[0], types.m_nKeyWordSetIdx[1], types.m_nStringType,
					 types.m_bLineNumIsCRLF ? 1 : 0, types.m_nLineTermType, types.m_bWordWrap ? 1 : 0,
					 types.m_nCurrentPrintSetting, types.m_nTsvMode);
		cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szKeyData));
	}
	...
	if (cProfile.IsReadingMode())
	{
		if (types.m_nLineSpace < -LINESPACE_MAX) { types.m_nLineSpace = -LINESPACE_MAX; }
		if (types.m_nLineSpace > LINESPACE_MAX) { types.m_nLineSpace = LINESPACE_MAX; }
	}
	...
	if (cProfile.IsReadingMode())
	{
		if (types.m_nLineNumWidth < LINENUMWIDTH_MIN) { types.m_nLineNumWidth = LINENUMWIDTH_MIN; }
		if (types.m_nLineNumWidth > LINENUMWIDTH_MAX) { types.m_nLineNumWidth = LINENUMWIDTH_MAX; }
	}
	...
	// Block Comment
	ShareData_IO_BlockComment(cProfile, pszSecName, L"szBlockCommentFrom", L"szBlockCommentTo",
							  types.m_cBlockComments[0]);
	ShareData_IO_BlockComment(cProfile, pszSecName, L"szBlockCommentFrom2", L"szBlockCommentTo2",
							  types.m_cBlockComments[1]);
	...
	cProfile.IOProfileData(pszSecName, LTEXT("bOutlineDockDisp"),
						   types.m_bOutlineDockDisp); /* アウトライン解析表示の有無 */
	cProfile.IOProfileData_WrapInt(pszSecName, LTEXT("eOutlineDockSide"),
								   types.m_eOutlineDockSide); /* アウトライン解析ドッキング配置 */
	{
		const WCHAR *pszKeyName = LTEXT("xyOutlineDock");
		const WCHAR *pszForm	= LTEXT("%d,%d,%d,%d");
		WCHAR		 szKeyData[1024];
		...
		{
			auto_sprintf(szKeyData, pszForm, types.m_cxOutlineDockLeft, types.m_cyOutlineDockTop,
						 types.m_cxOutlineDockRight, types.m_cyOutlineDockBottom);
			cProfile.IOProfileData(pszSecName, pszKeyName, MakeStringBufferW(szKeyData));
		}
	}
	cProfile.IOProfileData_WrapInt(pszSecName, LTEXT("nDockOutline"), types.m_nDockOutline); /* アウトライン解析方法 */
	cProfile.IOProfileData_WrapInt(pszSecName, LTEXT("nDefaultOutline"),
								   types.m_eDefaultOutline); /* アウトライン解析方法 */
	cProfile.IOProfileData(pszSecName, LTEXT("szOutlineRuleFilename"),
						   types.m_szOutlineRuleFilename); /* アウトライン解析ルールファイル */
	cProfile.IOProfileData(pszSecName, LTEXT("nOutlineSortCol"),
						   types.m_nOutlineSortCol); /* アウトライン解析ソート列番号 */
	cProfile.IOProfileData(pszSecName, LTEXT("bOutlineSortDesc"),
						   types.m_bOutlineSortDesc); /* アウトライン解析ソート降順 */
	cProfile.IOProfileData(pszSecName, LTEXT("nOutlineSortType"),
						   types.m_nOutlineSortType); /* アウトライン解析ソート基準 */
	ShareData_IO_FileTree(cProfile, types.m_sFileTree, pszSecName);
	cProfile.IOProfileData_WrapInt(pszSecName, LTEXT("nSmartIndent"),
								   types.m_eSmartIndent); /* スマートインデント種別 */
	...
	cProfile.IOProfileData(pszSecName, LTEXT("bTypeHtmlHelpIsSingle"),
						   types.m_bHtmlHelpIsSingle); // 2012.06.30 Fix m_bHokanLoHiCase -> m_bHtmlHelpIsSingle
	...
	for (j = 0; j < MAX_VERTLINES; j++)
	{
		...
		if (types.m_nVertLineIdx[j] == 0) { break; }
	}
	...
	{ //正規表現キーワード
		WCHAR *p;
		cProfile.IOProfileData(pszSecName, LTEXT("bUseRegexKeyword"),
							   types.m_bUseRegexKeyword); /* 正規表現キーワード使用するか？ */
		...
		for (j = 0; j < _countof(types.m_RegexKeywordArr); j++)
		{
				...
				if (cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szKeyData)))
				{
					...
					if (p)
					{
						*p										 = LTEXT('\0');
						...
						p++;
						if (0 < nKeywordSize - nPos - 1) { wcscpyn(&pKeyword[nPos], p, nKeywordSize - nPos - 1); }
						if (types.m_RegexKeywordArr[j].m_nColorIndex < 0
							|| types.m_RegexKeywordArr[j].m_nColorIndex >= COLORIDX_LAST)
						{ types.m_RegexKeywordArr[j].m_nColorIndex = COLORIDX_REGEX1; } if (pKeyword[nPos])
			...
			{
				auto_sprintf(szKeyData, LTEXT("%ls,%ls"), GetColorNameByIndex(types.m_RegexKeywordArr[j].m_nColorIndex),
							 &pKeyword[nPos]);
				cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szKeyData));
				nPos += wcslen(&pKeyword[nPos]) + 1;
			}
		}
		if (cProfile.IsReadingMode()) { pKeyword[nPos] = L'\0'; }
	}
	...
	cProfile.IOProfileData(pszSecName, LTEXT("szKinsokuKuto"),
						   MakeStringBufferW(types.m_szKinsokuKuto)); // 2009.08.07 ryoji
	cProfile.IOProfileData(pszSecName, LTEXT("bUseDocumentIcon"),
						   types.m_bUseDocumentIcon); // Sep. 19 ,2002 genta 変数名誤り修正

	//@@@ 2006.04.10 fon ADD-start
	{					/* キーワード辞書 */
		WCHAR *pH, *pT; /* <pH>keyword<pT> */
		cProfile.IOProfileData(pszSecName, LTEXT("bUseKeyWordHelp"),
							   types.m_bUseKeyWordHelp); /* キーワード辞書選択を使用するか？ */
		//		cProfile.IOProfileData( pszSecName, LTEXT("nKeyHelpNum"), types.m_nKeyHelpNum );				/*
		//登録辞書数
		//*/
		cProfile.IOProfileData(pszSecName, LTEXT("bUseKeyHelpAllSearch"),
							   types.m_bUseKeyHelpAllSearch); /* ヒットした次の辞書も検索(&A) */
		cProfile.IOProfileData(pszSecName, LTEXT("bUseKeyHelpKeyDisp"),
							   types.m_bUseKeyHelpKeyDisp); /* 1行目にキーワードも表示する(&W) */
		cProfile.IOProfileData(pszSecName, LTEXT("bUseKeyHelpPrefix"),
							   types.m_bUseKeyHelpPrefix); /* 選択範囲で前方一致検索(&P) */
		...
		for (j = 0; j < MAX_KEYHELP_FILE; j++)
		{
			...
			else
			{
				...
				{
					auto_sprintf(szKeyData, LTEXT("%d,%s,%s"), types.m_KeyHelpArr[j].m_bUse ? 1 : 0,
								 types.m_KeyHelpArr[j].m_szAbout, types.m_KeyHelpArr[j].m_szPath.c_str());
					cProfile.IOProfileData(pszSecName, szKeyName, MakeStringBufferW(szKeyData));
				}
			}
		}
	...
	{ // フォント設定
		...
		ShareData_IO_Sub_LogFont(cProfile, pszSecName, L"lf", L"nPointSize", L"lfFaceName", types.m_lf,
								 types.m_nPointSize);
	}
}
...
void CShareData_IO::ShareData_IO_KeyWords(CDataProfile &cProfile)
{
	...
	if (cProfile.IsReadingMode())
	{
		...
		if (bIOSuccess)
		{
			...
			for (i = 0; i < nKeyWordSetNum; ++i)
			{
				...
				std::wstring sValue; // wstring のまま受ける（古い ini ファイルのキーワードは中身が NULL
									 // 文字区切りなので StringBufferW では NG だった）
	...
	else
	{
		...
		for (i = 0; i < nSize; ++i)
		{
			...
			wchar_t *pszMem =
				new wchar_t[nMemLen + 1]; //	May 25, 2003 genta 区切りをTABに変更したので，最後の\0の分を追加
			wchar_t *pMem = pszMem;
			for (j = 0; j < pCKeyWordSetMgr->m_nKeyWordNumArr[i]; ++j)
			{
...
void CShareData_IO::ShareData_IO_Macro(CDataProfile &cProfile)
{
	...
	cProfile.IOProfileData(pszSecName, LTEXT("nMacroOnOpened"), pShare->m_Common.m_sMacro.m_nMacroOnOpened);
	/* オープン後自動実行マクロ番号 */ //@@@ 2006.09.01 ryoji
	cProfile.IOProfileData(pszSecName, LTEXT("nMacroOnTypeChanged"), pShare->m_Common.m_sMacro.m_nMacroOnTypeChanged);
	/* タイプ変更後自動実行マクロ番号 */ //@@@ 2006.09.01 ryoji
	cProfile.IOProfileData(pszSecName, LTEXT("nMacroOnSave"), pShare->m_Common.m_sMacro.m_nMacroOnSave);
	/* 保存前自動実行マクロ番号 */ //@@@ 2006.09.01 ryoji
	cProfile.IOProfileData(
		pszSecName, LTEXT("nMacroCancelTimer"),
		pShare->m_Common.m_sMacro.m_nMacroCancelTimer); // マクロ停止ダイアログ表示待ち時間	// 2011.08.04 syat
}
...
void CShareData_IO::ShareData_IO_Statusbar(CDataProfile &cProfile)
{
	...
	// 表示文字コードの指定
	cProfile.IOProfileData(pszSecName, LTEXT("DispUnicodeInSjis"),
						   statusbar.m_bDispUniInSjis); // SJISで文字コード値をUnicodeで表示する
	cProfile.IOProfileData(pszSecName, LTEXT("DispUnicodeInJis"),
						   statusbar.m_bDispUniInJis); // JISで文字コード値をUnicodeで表示する
	cProfile.IOProfileData(pszSecName, LTEXT("DispUnicodeInEuc"),
						   statusbar.m_bDispUniInEuc); // EUCで文字コード値をUnicodeで表示する
	cProfile.IOProfileData(pszSecName, LTEXT("DispUtf8Codepoint"),
						   statusbar.m_bDispUtf8Codepoint); // UTF-8をコードポイントで表示する
	cProfile.IOProfileData(pszSecName, LTEXT("DispSurrogatePairCodepoint"),
						   statusbar.m_bDispSPCodepoint); // サロゲートペアをコードポイントで表示する
	cProfile.IOProfileData(pszSecName, LTEXT("DispSelectCountByByte"),
						   statusbar.m_bDispSelCountByByte); // 選択文字数を文字単位ではなくバイト単位で表示する
	cProfile.IOProfileData(pszSecName, LTEXT("DispColByChar"),
						   statusbar.m_bDispColByChar); // 現在桁をルーラー単位ではなく文字単位で表示する
}
...
void CShareData_IO::ShareData_IO_Plugin(CDataProfile &cProfile, CMenuDrawer *pcMenuDrawer)
{
	...
	for (i = 0; i < MAX_PLUGIN; ++i)
	{
		...
		if (pluginrec.m_szId[0] != '\0' && pluginrec.m_nCmdNum > 0)
		{
			...
			{
				pcMenuDrawer->AddToolButton(CMenuDrawer::TOOLBAR_ICON_PLUGCOMMAND_DEFAULT,
											CPlug::GetPluginFunctionCode(i, j));
			}
		}
	}
}
...
void CShareData_IO::ShareData_IO_MainMenu(CDataProfile &cProfile)
{
	...
	if (cProfile.IsReadingMode() && nVersion < nCurrentVer)
	{
		...
		SMainMenuAddItemInfo	addInfos[] = {
			   ...
			   {1, F_COPY_COLOR_HTML_LINENUMBER, F_COPY_COLOR_HTML, L'F', false,
				false}, // 選択範囲内行番号色付きHTMLコピー
			   // 矩形選択類は省略...
		...
		for (int i = 0; i < _countof(addInfos); i++)
		{
			...
			if (item.m_nVer <= nVersion) { continue; }
			CMainMenu *pcMenuTlb = mainmenu.m_cMainMenuTbl;
			int		   k		 = 0;
			for (; k < mainmenu.m_nMainMenuNum; k++)
			{
				if (pcMenuTlb[k].m_nFunc == item.m_nAddFuncCode) { break; }
			}
			int nAddSep = 0;
			if (item.m_bAddPrevSeparete) { nAddSep++; }
			if (item.m_bAddNextSeparete) { nAddSep++; }
			if (k == mainmenu.m_nMainMenuNum && mainmenu.m_nMainMenuNum + nAddSep < _countof(mainmenu.m_cMainMenuTbl))
			{
				...
				for (int r = 0; r < mainmenu.m_nMainMenuNum; r++)
				{
					if (pcMenuTlb[r].m_nFunc == item.m_nPrevFuncCode && 0 < pcMenuTlb[r].m_nLevel)
					{
						...
						{ pcMenuTlb[n + 1 + nAddSep] = pcMenuTlb[n]; } for (int n = 0; n < MAX_MAINMENU_TOP; n++)
						{
							if (r < mainmenu.m_nMenuTopIdx[n]) { mainmenu.m_nMenuTopIdx[n] += 1 + nAddSep; }
						}
...
*/
void CShareData_IO::IO_MainMenu(CDataProfile &cProfile, std::vector<std::wstring> *pData,
								CommonSetting_MainMenu &mainmenu, bool bOutCmdName)
{
	...
	{
		int menuNum = 0;
		if (pData) { menuNum = (int)data.size() - 1; }
		else
		{
	...
	if (pData) { mainmenu.m_bMainMenuKeyParentheses = (_wtoi(data[dataNum++].c_str()) != 0); }
	else
	{
	...
	for (int i = 0; i < mainmenu.m_nMainMenuNum; i++)
	{
		...
		if (cProfile.IsReadingMode())
		{
			...
			// 読み出し
			if (pData) { wcscpy(szLine, data[dataNum++].c_str()); }
			else
			{
		...
		else
		{
			...
			else
			{
				...
				{
					// マクロ名対応
					p = CSMacroMgr::GetFuncInfoByID(G_AppInstance(), pcMenu->m_nFunc, szFuncName, NULL);
				}
				if (!bOutCmdName || p == NULL) { auto_sprintf(szFuncName, L"%d", pcMenu->m_nFunc); }
			}
			// 書き込み
			// ラベル編集後のノードはノード名を出力する 2012.10.14 syat 各国語対応
			auto_sprintf(szLine, L"%d,%d,%ls,%ls,%ls", pcMenu->m_nLevel, pcMenu->m_nType, szFuncName, pcMenu->m_sKey,
		...
		{
			// Top Level設定
			if (nIdx < MAX_MAINMENU_TOP) { mainmenu.m_nMenuTopIdx[nIdx++] = i; }
		}
	}
}
...
void CShareData_IO::ShareData_IO_Other(CDataProfile &cProfile)
{
	...
	/* **** その他のダイアログ **** */
	/* 外部コマンド実行の「標準出力を得る」 */
	if (!cProfile.IOProfileData(pszSecName, LTEXT("nExecFlgOpt"), pShare->m_nExecFlgOpt))
	{ //	2006.12.03 maru オプション拡張
	...
	cProfile.IOProfileData(pszSecName, LTEXT("_TagJumpKeyword_Counts"),
						   pShare->m_sTagJump.m_aTagJumpKeywords._GetSizeRef());
	pShare->m_sHistory.m_aCommands.SetSizeLimit();
	int nSize = pShare->m_sTagJump.m_aTagJumpKeywords.size();
	for (i = 0; i < nSize; ++i)
	{
		auto_sprintf(szKeyName, LTEXT("TagJumpKeyword[%02d]"), i);
		if (i >= nSize) { pShare->m_sTagJump.m_aTagJumpKeywords[i][0] = LTEXT('\0'); }
	...
	{
		WCHAR iniVer[256];
		auto_sprintf(iniVer, L"%d.%d.%d.%d", HIWORD(pShare->m_sVersion.m_dwProductVersionMS),
					 LOWORD(pShare->m_sVersion.m_dwProductVersionMS), HIWORD(pShare->m_sVersion.m_dwProductVersionLS),
...
void CShareData_IO::IO_ColorSet(CDataProfile *pcProfile, const WCHAR *pszSecName, ColorInfo *pColorInfoArr)
{
	...
	for (j = 0; j < COLORIDX_LAST; ++j)
	{
		...
		if (pcProfile->IsReadingMode())
		{
			...
			if (0 != (fAttribute & COLOR_ATTRIB_FORCE_DISP)) pColorInfoArr[j].m_bDisp = true;
			if (0 != (fAttribute & COLOR_ATTRIB_NO_BOLD)) pColorInfoArr[j].m_sFontAttr.m_bBoldFont = false;
			if (0 != (fAttribute & COLOR_ATTRIB_NO_UNDERLINE)) pColorInfoArr[j].m_sFontAttr.m_bUnderLine = false;
		}
		else
		{
			auto_sprintf(szKeyData, pszForm, pColorInfoArr[j].m_bDisp ? 1 : 0,
						 pColorInfoArr[j].m_sFontAttr.m_bBoldFont ? 1 : 0, pColorInfoArr[j].m_sColorAttr.m_cTEXT,
						 pColorInfoArr[j].m_sColorAttr.m_cBACK, pColorInfoArr[j].m_sFontAttr.m_bUnderLine ? 1 : 0);
							  ...
							  const WCHAR *pszKeyPointSize, const WCHAR *pszKeyFaceName, LOGFONT &lf, INT &nPointSize)
{
	...
	{
		auto_sprintf(szKeyData, pszForm, lf.lfHeight, lf.lfWidth, lf.lfEscapement, lf.lfOrientation, lf.lfWeight,
...
void CShareData_IO::ShareData_IO_FileTreeItem(CDataProfile &cProfile, SFileTreeItem &item, const WCHAR *pszSecName,
											  int i)
{
	WCHAR szKey[64];
