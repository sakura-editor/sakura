...
public:
	...
	//!< いくつかのウィンドウへ終了要求を出す	// 2007.02.13 ryoji 「編集の全終了」を示す引数(bExit)を追加	//
...
public:
	...
	HWND SeparateGroup(HWND hwndSrc, HWND hwndDst, bool bSrcIsTop,
					   int notifygroups[]); /* タブ分離に伴うウィンドウ処理 2007.07.07 genta */

	//総合情報
	int GetOpenedWindowArr(EditNode **ppEditNode, BOOL bSort,
						   BOOL bGSort = FALSE); /* 現在開いている編集ウィンドウの配列を返す */

protected:
	int _GetOpenedWindowArrCore(EditNode **ppEditNode, BOOL bSort,
								BOOL bGSort = FALSE); /* 現在開いている編集ウィンドウの配列を返す（コア処理部） */
...
inline CAppNodeGroupHandle EditNode::GetGroup() const
{
	if (this)
		return m_nGroup;
	else
...
{
	if (node) return node->m_nGroup;
	return 0;
}

inline bool EditNode::IsTopInGroup() const
{
	return this && (CAppNodeGroupHandle(m_nGroup).GetEditNodeAt(0) == this);
} // TODO: Remove "this" check
inline bool EditNode::IsTopInGroup_Safe(const EditNode *node)
{
	if (node) return node->IsTopInGroup();
	return false;
}

inline CAppNodeHandle::CAppNodeHandle(HWND hwnd) { m_pNodeRef = CAppNodeManager::getInstance()->GetEditNode(hwnd); }
