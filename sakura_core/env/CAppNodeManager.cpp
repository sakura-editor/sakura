...
static int __cdecl cmpGetOpenedWindowArr(const void *e1, const void *e2)
{
	...
	if (s_bSort) return (((EditNodeEx *)e1)->p->m_nIndex - ((EditNodeEx *)e2)->p->m_nIndex); // ウィンドウ番号比較
...
EditNode *CAppNodeGroupHandle::GetEditNodeAt(int nIndex)
{
	...
	for (i = 0; i < pShare->m_sNodes.m_nEditArrNum; i++)
	{
		if (m_nGroup == 0 || m_nGroup == pShare->m_sNodes.m_pEditArr[i].m_nGroup)
		{
			...
			{
				if (iIndex == nIndex) return &pShare->m_sNodes.m_pEditArr[i];
				iIndex++;
			}
		}
	}
...
*/
BOOL CAppNodeGroupHandle::RequestCloseEditor(EditNode *pWndArr, int nArrCnt, BOOL bExit, BOOL bCheckConfirm,
											 HWND hWndFrom)
{
	/* クローズ対象ウィンドウを調べる */
	int  iGroup		 = -1;
	...
	if (bCheckConfirm && GetDllShareData().m_Common.m_sGeneral.m_bCloseAllConfirm)
	...
	HWND hWndActive;
	bool bTabGroup = (GetDllShareData().m_Common.m_sTabBar.m_bDispTabWnd
	...
	for (int i = 0; i < nArrCnt; i++)
	{
		if (m_nGroup == 0 || m_nGroup == pWndArr[i].m_nGroup)
		{
			if (IsSakuraMainWindow(pWndArr[i].m_hWnd))
			{
				...
				if (!::SendMessage(pWndArr[i].m_hWnd, MYWM_CLOSE, bExit ? PM_CLOSE_EXIT : 0, (LPARAM)hWndActive))
				{ // 2007.02.13 ryoji bExitを引き継ぐ
					return FALSE;
				}
			}
		}
...
int CAppNodeGroupHandle::GetEditorWindowsNum(bool bExcludeClosing /* = true */)
{
	...
	for (i = 0; i < pShare->m_sNodes.m_nEditArrNum; ++i)
	{
		if (IsSakuraMainWindow(pShare->m_sNodes.m_pEditArr[i].m_hWnd))
		{
			if (m_nGroup != 0
				&& m_nGroup
					   != CAppNodeManager::getInstance()
							  ->GetEditNode(pShare->m_sNodes.m_pEditArr[i].m_hWnd)
							  ->GetGroup())
				continue;
			if (bExcludeClosing && pShare->m_sNodes.m_pEditArr[i].m_bClosing) continue;
			j++;
		}
	}
	return j;
...
*/
BOOL CAppNodeGroupHandle::PostMessageToAllEditors(UINT   uMsg,	/*!< ポストするメッセージ */
...
/** 全編集ウィンドウへメッセージを送る
	...
	@date 2007.06.22 ryoji nGroup引数を追加、グループ単位で順番に送る
*/
BOOL CAppNodeGroupHandle::SendMessageToAllEditors(UINT   uMsg,	/* ポストするメッセージ */
												  WPARAM wParam,  /* 第1メッセージ パラメータ */
												  LPARAM lParam,  /* 第2メッセージ パラメータ */
												  HWND   hWndLast /* 最後に送りたいウィンドウ */
)
...
EditNode *CAppNodeManager::GetEditNode(HWND hWnd)
{
	...
	for (int i = 0; i < pShare->m_sNodes.m_nEditArrNum; i++)
	{
		...
		{
			if (IsSakuraMainWindow(pShare->m_sNodes.m_pEditArr[i].m_hWnd)) return &pShare->m_sNodes.m_pEditArr[i];
		}
	}
...
int CAppNodeManager::_GetOpenedWindowArrCore(EditNode **ppEditNode, BOOL bSort, BOOL bGSort /* = FALSE */)
{
	...
	//編集ウインドウ数を取得する。
	*ppEditNode = NULL;
	if (pShare->m_sNodes.m_nEditArrNum <= 0) return 0;
	...
	if (!bGSort)
	{
		...
		for (i = 0; i < nRowNum; i++)
		{
			if (pNode[i].nGroupMru == -1 && nGroup != pNode[i].p->m_nGroup)
			{
				...
				{
					if (pNode[j].p->m_nGroup == nGroup) pNode[j].nGroupMru = iGroupMru;
				}
			}
		}
	}
...
bool CAppNodeManager::ReorderTab(HWND hwndSrc, HWND hwndDst)
{
	...
	for (i = 0; i < nCount; i++)
	{
		if (hwndSrc == p[i].m_hWnd) nSrcTab = i;
		if (hwndDst == p[i].m_hWnd) nDstTab = i;
	}
...
bool CAppNodeManager::IsSameGroup(HWND hWnd1, HWND hWnd2)
{
	...
	if (cGroup1.IsValidGroup() && cGroup1 == cGroup2) { return true; }
		...
		&& !GetDllShareData().m_Common.m_sTabBar.m_bDispTabWndMultiWin)
	{
		...
		if (nCount > 1)
		{
			...
			for (i = 0; i < nCount; i++)
			{
				...
				{
					if (p[i].GetHwnd() == hWndCur) { bFound = true; }
					else
					{
						if (!bFound && hWnd == NULL || bFound) { hWnd = p[i].GetHwnd(); }
						if (bFound) { break; }
					}
				}
			}
