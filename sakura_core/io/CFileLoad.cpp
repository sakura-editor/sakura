...
EConvertResult CFileLoad::ReadLine(CNativeW *pUnicodeBuffer, CEol *pcEol)
{
    ...
    int nRetLineLen;
    CEol cEolTemp;
    const wchar_t *pRet = GetNextLineW(m_cLineTemp.GetStringPtr(), m_cLineTemp.GetStringLength(), &nRetLineLen, &m_nReadOffset2, &cEolTemp, GetDllShareData().m_Common.m_sEdit.m_bEnableExtEol);
...
)
{
    ...
    for (;;)
    {
        ...
        if (m_nReadDataLen <= m_nReadBufOffSet && FLMODE_READY == m_eMode)
        { // From Here Jun. 13, 2003 Moca
            int n             = 128;
        ...
        case ENCODING_TRAIT_ASCII:
        {
            ...
            for (i = nbgn; i < nDataLen; ++i)
            {
                ...
                if (m_bEolEx)
                {
                    ...
                    {
                        if (0 != m_memEols[k].GetRawLength() && i + m_memEols[k].GetRawLength() - 1 < nDataLen && 0 == memcmp(m_memEols[k].GetRawPtr(), pData + i, m_memEols[k].GetRawLength()))
                        {
        ...
        case ENCODING_TRAIT_EBCDIC:
            ...
            for (i = nbgn; i < nDataLen; ++i)
            {
                ...
                if (pData[i] == '\x0d' || pData[i] == '\x25')
                {
                    char szEof[3] = {
                        (pData[i] == '\x25' ? '\x0a' : '\x0d'),
                        (pData[i + 1] == '\x25' ? '\x0a' : (char)(pData[i + 1] == '\x0a' ? 0 : // EBCDIC の"\x0aがLFにならないように細工する
