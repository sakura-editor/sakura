...
public:
	...
	static std::wstring GetSizeStringForHuman(
		ULONGLONG size); //!< 人にとって見やすいサイズ文字列を作る (例: "2 GB", "10 GB", "400 MB", "32 KB")
...
public:
	...
	ECodeType FileOpen(LPCWSTR, bool bBigFile, ECodeType, int,
					   bool *pbBomExist = NULL); // 指定文字コードでファイルをオープンする
...
protected:
	...
	const char *   GetNextLineCharCode(const char *pData, int nDataLen, int *pnLineLen, int *pnBgn, CEol *pcEol,
...
{
	DWORD ReadSize;
	if (!::ReadFile(m_hFile, pBuf, nSize, &ReadSize, NULL)) throw CError_FileRead();
	return (int)ReadSize;
}
...
{
	DWORD fp;
	if (INVALID_SET_FILE_POINTER == (fp = ::SetFilePointer(m_hFile, offset, NULL, FILE_BEGIN))) throw CError_FileRead();
	return fp;
}
