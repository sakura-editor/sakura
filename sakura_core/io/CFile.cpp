...
{
	if (pszPath) { SetFilePath(pszPath); }
}

CFile::~CFile() { FileUnlock(); }

// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
//                         各種判定                            //
// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //

bool CFile::IsFileExist() const { return fexist(GetFilePath()); }

bool CFile::HasWritablePermission() const { return -1 != _waccess(GetFilePath(), 2); }
...
bool CFile::IsFileWritable() const
{
	...
	HANDLE hFile = CreateFile(this->GetFilePath(),				  //ファイル名
							  GENERIC_WRITE,					  //書きモード
...
bool CFile::FileLock(EShareMode eShareMode, bool bMsg)
{
	...
	//オープン
	m_hLockedFile = CreateFile(this->GetFilePath(),   //ファイル名
	...
	if (INVALID_HANDLE_VALUE == m_hLockedFile && bMsg)
	{
		...
		TopWarningMessage(CEditWnd::getInstance()->GetHwnd(), LS(STR_FILE_LOCK_ERR),
						  GetFilePathClass().IsValidPath() ? GetFilePath() : LS(STR_NO_TITLE1), pszMode);
		return false;
	}
