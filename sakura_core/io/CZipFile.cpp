    ...
    for (vari.lVal = 0; vari.lVal < lCount; vari.lVal++)
    {
        ...
        if (vFolder)
        {
            ...
            for (varj.lVal = 0; varj.lVal < lCount2; varj.lVal++)
            {
                ...
                // 定義ファイルか
                if (!vFolder && wcslen(bps) >= sDefFile.length() && (wmemicmp(bps, ((sFolderName + L"/" + sDefFile).c_str())) == 0 || wmemicmp(bps, ((sFolderName + L"\\" + sDefFile).c_str())) == 0 || wmemicmp(bps, ((sZipName + L"\\" + sFolderName + L"\\" + sDefFile).c_str())) == 0))
                {
                    bFoundDef = true;
                    break;
                }
