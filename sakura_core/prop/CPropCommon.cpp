...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CPropCommon::DlgProc(INT_PTR (CPropCommon::*DispatchPage)(HWND, UINT, WPARAM, LPARAM), HWND hwndDlg, UINT uMsg,
...
INT_PTR CPropCommon::DlgProc2(INT_PTR (CPropCommon::*DispatchPage)(HWND, UINT, WPARAM, LPARAM), HWND hwndDlg, UINT uMsg,
							  WPARAM wParam, LPARAM lParam)
{
	CPropCommon *pCPropCommon;
	switch (uMsg)
...
INT_PTR CPropCommon::DoPropertySheet(int nPageNum, bool bTrayProc)
{
	...
	static const ComPropSheetInfo ComPropSheetInfoList[] = {
		...
		{STR_PROPCOMMON_STATBAR, IDD_PROP_STATUSBAR,
		 CPropStatusbar::DlgProc_page}, // 文字コード表示指定	2008/6/21	Uchi
		...
		{STR_PROPCOMMON_SEARCH, IDD_PROP_GREP,
		 CPropGrep::DlgProc_page}, // 2006.08.23 ryoji タイトル変更（Grep -> 検索）
	...
	if (-1 == nRet)
	{
		...
		PleaseReportToAuthor(NULL, LS(STR_ERR_DLGPROPCOMMON24), psh.nStartPage, pszMsgBuf);
		::LocalFree(pszMsgBuf);
	}
...
void CPropCommon::InitData(const int *tempTypeKeywordSet, const WCHAR *name, const WCHAR *exts)
{
	...
	if (tempTypeKeywordSet)
	{
		...
		indexs.typeId = -1;
		for (int j = 0; j < MAX_KEYWORDSET_PER_TYPE; j++) { indexs.index[j] = tempTypeKeywordSet[j]; }
	...
	for (i = 0; i < GetDllShareData().m_nTypesCount; ++i)
	{
		...
		for (int j = 0; j < MAX_KEYWORDSET_PER_TYPE; j++) { indexs.index[j] = type.m_nKeyWordSetIdx[j]; }
		m_Types_nKeyWordSetIdx.push_back(indexs);
	}
}
...
void CPropCommon::OnHelp(HWND hwndParent, int nPageID)
{
	...
	case IDD_PROP_FILE:
		...
		//		nContextID = 84;
		//		break;
	case IDD_PROP_TOOLBAR: nContextID = ::FuncID_To_HelpContextID(F_OPTION_TOOLBAR); break;
	case IDD_PROP_KEYWORD: nContextID = ::FuncID_To_HelpContextID(F_OPTION_KEYWORD); break;
	case IDD_PROP_CUSTMENU: nContextID = ::FuncID_To_HelpContextID(F_OPTION_CUSTMENU); break;
	case IDD_PROP_HELPER: nContextID = ::FuncID_To_HelpContextID(F_OPTION_HELPER); break;

	// From Here Sept. 9, 2000 JEPRO 共通設定のヘルプボタンが効かなくなっていた部分を以下の追加によって修正
	case IDD_PROP_EDIT: nContextID = ::FuncID_To_HelpContextID(F_OPTION_EDIT); break;
	case IDD_PROP_BACKUP: nContextID = ::FuncID_To_HelpContextID(F_OPTION_BACKUP); break;
	case IDD_PROP_WIN: nContextID = ::FuncID_To_HelpContextID(F_OPTION_WINDOW); break;
	case IDD_PROP_TAB: nContextID = ::FuncID_To_HelpContextID(F_OPTION_TAB); break;
	case IDD_PROP_STATUSBAR: nContextID = ::FuncID_To_HelpContextID(F_OPTION_STATUSBAR); break;
	case IDD_PROP_GREP: nContextID = ::FuncID_To_HelpContextID(F_OPTION_GREP); break;
	case IDD_PROP_KEYBIND: nContextID = ::FuncID_To_HelpContextID(F_OPTION_KEYBIND); break;
	// To Here Sept. 9, 2000
