...
static const DWORD p_helpids[] = { //11700
    IDC_CHECK_PluginEnable,
    HIDC_CHECK_PluginEnable, //プラグインを有効にする
    IDC_PLUGINLIST,
    HIDC_PLUGINLIST, //プラグインリスト
    IDC_PLUGIN_INST_ZIP,
    HIDC_PLUGIN_INST_ZIP, //Zipプラグインを追加	// 2011/11/2 Uchi
    IDC_PLUGIN_SearchNew,
    HIDC_PLUGIN_SearchNew, //新規プラグインを追加
    IDC_PLUGIN_OpenFolder,
    HIDC_PLUGIN_OpenFolder, //フォルダを開く
    IDC_PLUGIN_Remove,
    HIDC_PLUGIN_Remove, //プラグインを削除
    IDC_PLUGIN_OPTION,
    HIDC_PLUGIN_OPTION, //プラグイン設定	// 2010/3/22 Uchi
    IDC_PLUGIN_README,
    HIDC_PLUGIN_README, //ReadMe表示		// 2011/11/2 Uchi
    IDC_PLUGIN_URL,
    HIDC_PLUGIN_URL, //配布先			// 2015/01/02 syat
    //	IDC_STATIC,			-1,
    0,
    0};
                        ...
                        case LVN_ITEMCHANGED:
                        {
                            ...
                            if (sel >= 0)
                            {
                                ...
                                ::EnableWindow(::GetDlgItem(hwndDlg, IDC_PLUGIN_README),
                                               (state == PLS_INSTALLED || state == PLS_UPDATED || state == PLS_LOADED || state == PLS_DELETED) && !GetReadMeFile(m_Common.m_sPlugin.m_PluginTable[sel].m_szName).empty());
...
void CPropPlugin::SetData_LIST(HWND hwndDlg)
{
    ...
    for (index = 0; index < MAX_PLUGIN; ++index)
    {
        ...
        {
            case PLS_INSTALLED:
                sItem.pszText = const_cast<WCHAR *>(LS(STR_PROPCOMPLG_STATE1));
                break;
            case PLS_UPDATED:
                sItem.pszText = const_cast<WCHAR *>(LS(STR_PROPCOMPLG_STATE2));
                break;
            case PLS_STOPPED:
                sItem.pszText = const_cast<WCHAR *>(LS(STR_PROPCOMPLG_STATE3));
                break;
            case PLS_LOADED:
                sItem.pszText = const_cast<WCHAR *>(LS(STR_PROPCOMPLG_STATE4));
                break;
            case PLS_DELETED:
                sItem.pszText = const_cast<WCHAR *>(LS(STR_PROPCOMPLG_STATE5));
                break;
            case PLS_NONE:
                sItem.pszText = const_cast<WCHAR *>(L"");
                break;
            default:
                sItem.pszText = const_cast<WCHAR *>(LS(STR_PROPCOMPLG_STATE6));
                break;
        }
        ListView_SetItem(hListView, &sItem);
