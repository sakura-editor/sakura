...
public:
	...
	*/
	...
	// 2002.12.11 Moca 追加
	void InitData(const int *tempTypeKeywordSet = NULL, const WCHAR *name = NULL,
				  const WCHAR *exts = NULL);		//!< DLLSHAREDATAから一時データ領域に設定を複製する
...
protected:
	...
	*/
	...
	//! 汎用ダイアログプロシージャ
	static INT_PTR DlgProc(INT_PTR (CPropCommon::*DispatchPage)(HWND, UINT, WPARAM, LPARAM), HWND hwndDlg, UINT uMsg,
						   WPARAM wParam, LPARAM lParam);
	static INT_PTR DlgProc2( //独立ウィンドウ用
		INT_PTR (CPropCommon::*DispatchPage)(HWND, UINT, WPARAM, LPARAM), HWND hwndDlg, UINT uMsg, WPARAM wParam,
		LPARAM lParam);
	...
	HFONT SetFontLabel(HWND hwndDlg, int idc_static, const LOGFONT &lf,
					   int nps); //!< フォントラベルにフォントとフォント名設定する	// 2013/4/24 Uchi
};
...
public:
	//!	Dialog Procedure
	static INT_PTR CALLBACK DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
	static INT_PTR CALLBACK DlgProc_dialog(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
