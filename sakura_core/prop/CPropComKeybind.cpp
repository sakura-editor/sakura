...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropKeybind::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
)
{
	...
	case WM_INITDIALOG:
		...
		::SendMessageCmd(hwndDlg, WM_COMMAND, MAKELONG(IDC_LIST_KEY, LBN_SELCHANGE),
						 (LPARAM)hwndKeyList); // Oct. 14, 2000 JEPRO ここはどっちでもいい？(わからん)
		//	To Here Oct. 14, 2000
		...
		else if (hwndFuncList == hwndCtl)
		{
			...
			case LBN_SELCHANGE:
				...
				nAssignedKeyNum = CKeyBind::GetKeyStrList(/* 機能に対応するキー名の取得(複数) */
														  G_AppInstance(), m_Common.m_sKeyBind.m_nKeyNameArrNum,
						...
						for (j = 0; j < m_Common.m_sKeyBind.m_nKeyNameArrNum; j++)
						{
							if (wcscmp(m_Common.m_sKeyBind.m_pKeyNameArr[j].m_szKeyName, p) == 0)
							{
								...
								if (i & _SHIFT)
									...
									::CheckDlgButton(hwndDlg, IDC_CHECK_SHIFT, BST_UNCHECKED); //チェックをはずす
								if (i & _CTRL)
									::CheckDlgButton(hwndDlg, IDC_CHECK_CTRL, BST_CHECKED); //チェック
								else
									::CheckDlgButton(hwndDlg, IDC_CHECK_CTRL, BST_UNCHECKED); //チェックをはずす
								if (i & _ALT)
									::CheckDlgButton(hwndDlg, IDC_CHECK_ALT, BST_CHECKED); //チェック
								else
									::CheckDlgButton(hwndDlg, IDC_CHECK_ALT, BST_UNCHECKED); //チェックをはずす
								::SendMessageCmd(hwndDlg, WM_COMMAND, MAKELONG(IDC_LIST_KEY, LBN_SELCHANGE),
												 (LPARAM)hwndKeyList);
	...
	case WM_TIMER:
		...
		::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_ASSIGN),
					   !(LB_ERR == nIndex || nIndex2 == CB_ERR || nIndex3 == LB_ERR));
...
/* ダイアログデータの取得 Keybind */
int CPropKeybind::GetData(HWND hwndDlg) { return TRUE; }
