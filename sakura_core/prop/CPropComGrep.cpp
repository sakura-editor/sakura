...
static const DWORD p_helpids[] = {			 // 10500
	...
	IDC_LABEL_REGEXP, HIDC_EDIT_REGEXPLIB, IDC_LABEL_REGEXP_VER,
	HIDC_LABEL_REGEXPVER, //正規表現ライブラリバージョン	// 2007.09.02 genta
	IDC_CHECK_bCaretTextForSearch,
	HIDC_CHECK_bCaretTextForSearch, //カーソル位置の文字列をデフォルトの検索文字列にする	// 2006.08.23 ryoji
	IDC_CHECK_INHERIT_KEY_OTHER_VIEW,
	HIDC_CHECK_INHERIT_KEY_OTHER_VIEW, // 次・前検索で他のビューの検索条件を引き継ぐ	// 2011.12.18 Moca
...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropGrep::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
INT_PTR CPropGrep::DispatchEvent(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
		...
		case PSN_SETACTIVE: m_nPageNum = ID_PROPCOM_PAGENUM_GREP; return TRUE;
		}
		//			break;	/* default */
		//		}
		break; /* WM_NOTIFY */
	case WM_COMMAND:
		...
		if (wParam == MAKEWPARAM(IDC_EDIT_REGEXPLIB, EN_KILLFOCUS)) { SetRegexpVersion(hwndDlg); }
		break;
...
void CPropGrep::SetData(HWND hwndDlg)
{
	...
	/* Grep結果のリアルタイム表示 */
	::CheckDlgButton(hwndDlg, IDC_CHECK_GREPREALTIME,
					 m_Common.m_sSearch.m_bGrepRealTimeView); // 2006.08.08 ryoji ID修正
	...
	struct tagTagJumpMode TagJumpMode1Arr[] = {{0, STR_TAGJUMP_0},
											   {1, STR_TAGJUMP_1},
											   //{ 2, STR_TAGJUMP_2 },
											   {3, STR_TAGJUMP_3}};
	...
	struct tagTagJumpMode TagJumpMode2Arr[] = {
		{0, STR_TAGJUMP_0}, {1, STR_TAGJUMP_1}, {2, STR_TAGJUMP_2}, {3, STR_TAGJUMP_3}};
...
int CPropGrep::GetData(HWND hwndDlg)
{
	...
	/* Grep結果のリアルタイム表示 */
	m_Common.m_sSearch.m_bGrepRealTimeView =
		::IsDlgButtonChecked(hwndDlg, IDC_CHECK_GREPREALTIME); // 2006.08.08 ryoji ID修正
	...
	//	2007.08.12 genta 正規表現DLL
	::DlgItem_GetText(hwndDlg, IDC_EDIT_REGEXPLIB, m_Common.m_sSearch.m_szRegexpLib,
