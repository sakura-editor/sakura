...
static const DWORD p_helpids[] = {IDC_COMBO_FUNCKIND,
								  HIDC_COMBO_FUNCKIND, //機能の種別
								  IDC_LIST_FUNC,
								  HIDC_LIST_FUNC, //機能一覧
								  IDC_TREE_RES,
								  HIDC_TREE_RES, //メニュー一覧
								  IDC_BUTTON_DELETE,
								  HIDC_BUTTON_TREE_DELETE, //メニューから機能削除
								  IDC_BUTTON_INSERT_NODE,
								  HIDC_BUTTON_TREE_INSERT_NODE, //メニューへノード追加
								  IDC_BUTTON_INSERTSEPARATOR,
								  HIDC_BUTTON_TREE_INSERT_SEPARATOR, //メニューへ区切線挿入
								  IDC_BUTTON_INSERT,
								  HIDC_BUTTON_TREE_INSERT, //メニューへ機能挿入(上)
								  IDC_BUTTON_INSERT_A,
								  HIDC_BUTTON_TREE_INSERT_A, //メニューへ機能挿入(下)
								  IDC_BUTTON_ADD,
								  HIDC_BUTTON_TREE_ADD, //メニューへ機能追加
								  IDC_BUTTON_UP,
								  HIDC_BUTTON_TREE_UP, //メニューの機能を上へ移動
								  IDC_BUTTON_DOWN,
								  HIDC_BUTTON_TREE_DOWN, //メニューの機能を下へ移動
								  IDC_BUTTON_RIGHT,
								  HIDC_BUTTON_TREE_RIGHT, //メニューの機能を右へ移動
								  IDC_BUTTON_LEFT,
								  HIDC_BUTTON_TREE_LEFT, //メニューの機能を左へ移動
								  IDC_BUTTON_IMPORT,
								  HIDC_BUTTON_IMPORT, //メニューのインポート
								  IDC_BUTTON_EXPORT,
								  HIDC_BUTTON_EXPORT, //メニューのエクスポート
								  IDC_BUTTON_CHECK,
								  HIDC_BUTTON_NENU_CHECK, //メニューの検査
								  IDC_BUTTON_CLEAR,
								  HIDC_BUTTON_TREE_CLEAR, //メニューをクリア
								  IDC_BUTTON_INITIALIZE,
								  HIDC_BUTTON_TREE_INITIALIZE, //メニューを初期状態に戻す
								  IDC_CHECK_KEY_PARENTHESES,
								  HIDC_CHECK_KEY_PARENTHESES, //アクセスキーを必ず( )付で表示(&P)
								  0,
								  0};
...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropMainMenu::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
static LRESULT CALLBACK TreeViewProc(HWND   hwndTree, // handle to dialog box
									 UINT   uMsg,	 // message
...
)
{
	...
	case WM_GETDLGCODE:
		...
		if (lParam == 0) { break; }
		pMsg = (MSG *)lParam;
		if (pMsg->wParam == wParam && (wParam == VK_RETURN || wParam == VK_ESCAPE || wParam == VK_TAB)) { break; }
	...
	case WM_KEYDOWN:
		...
		if (cKey > ' ')
		{
			...
			if (pFuncWk->m_nFunc == F_SEPARATOR) { return 0; }
		...
		case VK_DELETE: //	DELキーが押されたらダイアログボックスにメッセージを送信
			...
			return 0;
		case VK_F2: // F2で編集
			if (htiItem != NULL) { TreeView_EditLabel(hwndTree, htiItem); }
			return 0;
		}
		break;
...
static LRESULT CALLBACK WindowProcEdit(HWND   hwndEdit, // handle to dialog box
									   UINT   uMsg,		// message
									   WPARAM wParam,   // first message parameter
									   LPARAM lParam	// second message parameter
)
...
static void SetDlgItemsEnableState(HWND hwndDlg, HWND hwndTreeRes, HWND hwndComboFuncKind, HWND hwndListFunc,
...
/* Menu メッセージ処理 */
INT_PTR CPropMainMenu::DispatchEvent(HWND   hwndDlg, // handle to dialog box
									 UINT   uMsg,	// message
									 WPARAM wParam,  // first message parameter
									 LPARAM lParam   // second message parameter
)
{
	...
	case WM_NOTIFY:
		...
		switch (pNMHDR->code)
		{
		case PSN_HELP: OnHelp(hwndDlg, IDD_PROP_MAINMENU); return TRUE;
		case PSN_KILLACTIVE:
			/* ダイアログデータの取得 Menu */
			GetData(hwndDlg);
			return TRUE;
		case PSN_SETACTIVE:
			...
			if (nIdxFIdx != CB_ERR)
			{
				::SendMessage(hwndDlg, WM_COMMAND, MAKEWPARAM(IDC_COMBO_FUNCKIND, CBN_SELCHANGE),
							  (LPARAM)hwndComboFunkKind);
				if (nIdxFunc != LB_ERR) { List_SetCurSel(hwndListFunk, nIdxFunc); }
			}
			return TRUE;
		...
		case TVN_ENDLABELEDIT: //	アイテムの編集が終了
			if (pNMHDR->hwndFrom == hwndTreeRes && msMenu[ptdi->item.lParam].m_bIsNode)
			{
				// ノード有効
			...
			return TRUE;
		case TVN_DELETEITEM:
			if (!bInMove && !msMenu.empty() && pNMHDR->hwndFrom == hwndTreeRes
				&& (htiItem = TreeView_GetSelection(hwndTreeRes)) != NULL)
			{
				//付属情報を削除
				tvi.mask  = TVIF_HANDLE | TVIF_PARAM;
				tvi.hItem = htiItem;
				if (TreeView_GetItem(hwndTreeRes, &tvi)) { msMenu.erase(tvi.lParam); }
				return 0;
			}
			break;
		case NM_DBLCLK:
			...
			if (pNMHDR->hwndFrom == hwndTreeRes)
			{
				...
				tvi.mask  = TVIF_HANDLE | TVIF_PARAM;
				tvi.hItem = htiItem;
				if (!TreeView_GetItem(hwndTreeRes, &tvi)) { break; }
				pFuncWk = &msMenu[tvi.lParam];
				if (pFuncWk->m_nFunc != F_SEPARATOR)
				{
					auto_sprintf(szKey, L"%ls", pFuncWk->m_sKey);

					if (!cDlgInput1.DoModal(G_AppInstance(), hwndDlg, LS(STR_PROPCOMMAINMENU_ACCKEY1),
											LS(STR_PROPCOMMAINMENU_ACCKEY2), 1, szKey))
					{ return TRUE; } auto_sprintf(pFuncWk->m_sKey, L"%s", szKey);
					pFuncWk->m_bDupErr = false;
			...
			case BN_CLICKED:
				...
				case IDC_BUTTON_DELETE:
					...
					if (htiItem != NULL)
					{
						...
						{ return TRUE; } htiTemp = TreeView_GetNextSibling(hwndTreeRes, htiItem);
						if (htiTemp == NULL)
						{
							// 末尾ならば、前を取る
						...
						if (htiTemp != NULL) { TreeView_SelectItem(hwndTreeRes, htiTemp); }
					}
					break;
				...
				case IDC_BUTTON_ADD:			 // 追加
					...
					case IDC_BUTTON_ADD:	  // 追加
						// Function 取得
						if (CB_ERR == (nIdxFIdx = Combo_GetCurSel(hwndComboFunkKind))) { return FALSE; }
						if (LB_ERR == (nIdxFunc = List_GetCurSel(hwndListFunk))) { return FALSE; }
						if (nIdxFIdx == nSpecialFuncsNum)
						{
							// 特殊機能
					...
					// 展開
					if (htiParent != TVI_ROOT) { TreeView_Expand(hwndTreeRes, htiParent, TVE_EXPAND); }
				...
				case IDC_BUTTON_DOWN:
					...
					if (htiItem != NULL) { TreeView_SelectItem(hwndTreeRes, htiItem); }
					break;
				...
				{
					wstring sErrMsg;
					if (Check_MainMenu(hwndTreeRes, sErrMsg)) { InfoMessage(hwndDlg, LS(STR_PROPCOMMAINMENU_OK)); }
					else
					{
	...
	case WM_TIMER: SetDlgItemsEnableState(hwndDlg, hwndTreeRes, hwndComboFunkKind, hwndListFunk, m_cLookup); break;
	case WM_DESTROY:
		::KillTimer(hwndDlg, 1);
	...
	// Context Menu
	case WM_CONTEXTMENU: MyWinHelp(hwndDlg, HELP_CONTEXTMENU, (ULONG_PTR)(LPVOID)p_helpids); return TRUE;
	}
	return FALSE;
}
...
void CPropMainMenu::SetData(HWND hwndDlg)
{
	...
	for (i = 0; i < m_Common.m_sMainMenu.m_nMainMenuNum; i++)
	{
		...
		case T_LEAF:
			...
			pFuncWk->m_sName = szLabel;
			break;
		case T_SEPARATOR: pFuncWk->m_sName = LS(STR_PROPCOMMAINMENU_SEP); break;
		case T_SPECIAL:
			// 2014.05.04 各国語対応
		...
		case T_NODE:
			...
			if (pFuncWk->m_nFunc == F_NODE) { pFuncWk->m_sName = RemoveAmpersand(pcFunc->m_sName); }
			else
			{
...
bool CPropMainMenu::GetDataTree(HWND hwndTree, HTREEITEM htiTrg, int nLevel)
{
	...
	for (s = htiTrg; s != NULL; s = TreeView_GetNextSibling(hwndTree, s))
	{
		...
		if (nLevel == 0)
		{
			if (nTopCount >= MAX_MAINMENU_TOP) { continue; }
			// Top Levelの記録
		...
		default:
			...
			{
				if (pFuncWk->m_nFunc == F_OPTION) { bOptionOk = true; }
		...
		if (tvi.cChildren)
		{
			...
			if (ts != NULL)
			{
				if (!GetDataTree(hwndTree, ts, nLevel + 1)) { return false; }
			}
		}
	}
...
static HTREEITEM TreeCopy(HWND hwndTree, HTREEITEM dst, HTREEITEM src, bool fChild, bool fOnryOne)
{
	...
	for (s = src; s != NULL; s = fOnryOne ? NULL : TreeView_GetNextSibling(hwndTree, s))
	{
		...
		{
			ts = TreeView_GetChild(hwndTree, s); //	子の取得
			if (ts != NULL) { TreeCopy(hwndTree, td, ts, true, false); }
			// 展開
...
static const WCHAR *MakeDispLabel(SMainMenuWork *pFunc)
{
	...
	if (pFunc->m_sKey[0])
	{
		auto_sprintf_s(szLabel, MAX_MAIN_MENU_NAME_LEN + 10, L"%ls%ls(%ls)", pFunc->m_bDupErr ? L">" : L"",
					   pFunc->m_sName.substr(0, MAX_MAIN_MENU_NAME_LEN).c_str(), pFunc->m_sKey);
	}
	else
	{
		auto_sprintf_s(szLabel, MAX_MAIN_MENU_NAME_LEN + 10, L"%ls%ls", pFunc->m_bDupErr ? L">" : L"",
...
// メニューの検査
bool CPropMainMenu::Check_MainMenu(HWND		hwndTree, // handle to TreeView
...
bool CPropMainMenu::Check_MainMenu_Sub(HWND		 hwndTree, // handle to dialog box
									   HTREEITEM htiTrg,   // ターゲット
									   int nLevel, wstring &sErrMsg)
{
	// 検査用
	...
	for (s = htiTrg; s != NULL; s = TreeView_GetNextSibling(hwndTree, s))
	{
		...
		switch (pFuncWk->m_nFunc)
		{
		case F_NODE: nType = T_NODE; break;
		case F_SEPARATOR: nType = T_SEPARATOR; break;
		default:
			if (pFuncWk->m_nFunc >= F_SPECIAL_FIRST && pFuncWk->m_nFunc <= F_SPECIAL_LAST) { nType = T_SPECIAL; }
		...
		else
		{
			...
			else
			{
				...
				if (nDupErrNum == 0)
				{
					if (htiErr == NULL) { htiErr = mKey[pFuncWk->m_sKey[0]]; }
				}
