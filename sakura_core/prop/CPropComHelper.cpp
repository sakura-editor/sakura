...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropHelper::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
)
{
	...
	case WM_COMMAND:
		...
		case BN_CLICKED:
			...
			case IDC_BUTTON_OPENHELP1: /* 外部ヘルプ１の「参照...」ボタン */
			{
				...
				CDlgOpenFile::SelectFile(hwndDlg, GetDlgItem(hwndDlg, IDC_EDIT_EXTHELP1), L"*.hlp;*.chm;*.col", true,
										 EFITER_NONE);
			}
				return TRUE;
			case IDC_BUTTON_OPENEXTHTMLHELP: /* 外部HTMLヘルプの「参照...」ボタン */
			{
				// 2003.06.23 Moca 相対パスは実行ファイルからのパス
				// 2007.05.21 ryoji 相対パスは設定ファイルからのパスを優先
				CDlgOpenFile::SelectFile(hwndDlg, GetDlgItem(hwndDlg, IDC_EDIT_EXTHTMLHELP), L"*.chm;*.col", true,
										 EFITER_NONE);
			}
				return TRUE;
			// ai 02/05/21 Add S
			case IDC_BUTTON_KEYWORDHELPFONT: /* キーワードヘルプの「フォント」ボタン */
			{
				...
				if (MySelectFont(&lf, &nPointSize, hwndDlg, false)) {
					...
					HFONT hFont = SetFontLabel(hwndDlg, IDC_STATIC_KEYWORDHELPFONT, m_Common.m_sHelper.m_lf,
											   m_Common.m_sHelper.m_nPointSize);
			...
			case IDC_BUTTON_OPENMDLL: /* MIGEMODLL場所指定「参照...」ボタン */
			{
				// 2003.06.23 Moca 相対パスは実行ファイルからのパス
				// 2007.05.21 ryoji 相対パスは設定ファイルからのパスを優先
				CDlgOpenFile::SelectFile(hwndDlg, GetDlgItem(hwndDlg, IDC_EDIT_MIGEMO_DLL), L"*.dll", true,
										 EFITER_NONE);
			}
				return TRUE;
	...
	case WM_NOTIFY:
		...
		case PSN_SETACTIVE: m_nPageNum = ID_PROPCOM_PAGENUM_HELPER; return TRUE;
		}
		//			break;	/* default */
		//		}
...
{
	/* 補完候補決定キー */
	::CheckDlgButton(hwndDlg, IDC_CHECK_m_bHokanKey_RETURN,
					 m_Common.m_sHelper.m_bHokanKey_RETURN); // VK_RETURN 補完決定キーが有効/無効
	::CheckDlgButton(hwndDlg, IDC_CHECK_m_bHokanKey_TAB,
					 m_Common.m_sHelper.m_bHokanKey_TAB); // VK_TAB    補完決定キーが有効/無効
	::CheckDlgButton(hwndDlg, IDC_CHECK_m_bHokanKey_RIGHT,
					 m_Common.m_sHelper.m_bHokanKey_RIGHT); // VK_RIGHT  補完決定キーが有効/無効
	...
	/* HtmlHelpビューアはひとつ */
	::CheckDlgButton(hwndDlg, IDC_CHECK_HTMLHELPISSINGLE,
					 m_Common.m_sHelper.m_bHtmlHelpIsSingle ? BST_CHECKED : BST_UNCHECKED);

	// キーワードヘルプ フォント	// 2013/4/24 Uchi
	m_hKeywordHelpFont =
		SetFontLabel(hwndDlg, IDC_STATIC_KEYWORDHELPFONT, m_Common.m_sHelper.m_lf, m_Common.m_sHelper.m_nPointSize);
...
{
	/* 補完候補決定キー */
	m_Common.m_sHelper.m_bHokanKey_RETURN =
		::IsDlgButtonChecked(hwndDlg, IDC_CHECK_m_bHokanKey_RETURN); // VK_RETURN 補完決定キーが有効/無効
	m_Common.m_sHelper.m_bHokanKey_TAB =
		::IsDlgButtonChecked(hwndDlg, IDC_CHECK_m_bHokanKey_TAB); // VK_TAB    補完決定キーが有効/無効
	m_Common.m_sHelper.m_bHokanKey_RIGHT =
		::IsDlgButtonChecked(hwndDlg, IDC_CHECK_m_bHokanKey_RIGHT); // VK_RIGHT  補完決定キーが有効/無効

	/* 外部ヘルプ１ */
	::DlgItem_GetText(hwndDlg, IDC_EDIT_EXTHELP1, m_Common.m_sHelper.m_szExtHelp,
					  _countof(m_Common.m_sHelper.m_szExtHelp));

	/* 外部HTMLヘルプ */
	::DlgItem_GetText(hwndDlg, IDC_EDIT_EXTHTMLHELP, m_Common.m_sHelper.m_szExtHtmlHelp,
					  _countof(m_Common.m_sHelper.m_szExtHtmlHelp));

	/* HtmlHelpビューアはひとつ */
	m_Common.m_sHelper.m_bHtmlHelpIsSingle = ::IsDlgButtonChecked(hwndDlg, IDC_CHECK_HTMLHELPISSINGLE) != 0;

	// migemo dict
	::DlgItem_GetText(hwndDlg, IDC_EDIT_MIGEMO_DLL, m_Common.m_sHelper.m_szMigemoDll,
					  _countof(m_Common.m_sHelper.m_szMigemoDll));
	::DlgItem_GetText(hwndDlg, IDC_EDIT_MIGEMO_DICT, m_Common.m_sHelper.m_szMigemoDict,
					  _countof(m_Common.m_sHelper.m_szMigemoDict));
