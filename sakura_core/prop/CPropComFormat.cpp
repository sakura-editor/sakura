...
static const char *p_date_form[] = {"yyyy'年'M'月'd'日'",
									"yyyy'年'M'月'd'日('dddd')'",
									"yyyy'年'MM'月'dd'日'",
									"yyyy'年'M'月'd'日' dddd",
...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropFormat::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
void CPropFormat::ChangeDateExample(HWND hwndDlg)
{
	...
	CFormatManager().MyGetDateFormat(systime, szText, _countof(szText) - 1, m_Common.m_sFormat.m_nDateFormatType,
									 m_Common.m_sFormat.m_szDateFormat);
...
void CPropFormat::ChangeTimeExample(HWND hwndDlg)
{
	...
	CFormatManager().MyGetTimeFormat(systime, szText, _countof(szText) - 1, m_Common.m_sFormat.m_nTimeFormatType,
									 m_Common.m_sFormat.m_szTimeFormat);
...
)
{
	...
	case WM_INITDIALOG:
		...
		/* 見出し記号 */
		EditCtl_LimitText(::GetDlgItem(hwndDlg, IDC_EDIT_MIDASHIKIGOU),
						  _countof(m_Common.m_sFormat.m_szMidashiKigou) - 1);
	...
	case WM_NOTIFY:
		...
		case PSN_SETACTIVE: m_nPageNum = ID_PROPCOM_PAGENUM_FORMAT; return TRUE;
		}
		//			break;	/* default */
		//		}
...
{
	/* 見出し記号 */
	::DlgItem_GetText(hwndDlg, IDC_EDIT_MIDASHIKIGOU, m_Common.m_sFormat.m_szMidashiKigou,
					  _countof(m_Common.m_sFormat.m_szMidashiKigou));
	...
	/* 引用符 */
	::DlgItem_GetText(hwndDlg, IDC_EDIT_INYOUKIGOU, m_Common.m_sFormat.m_szInyouKigou,
	...
	//日付書式
	::DlgItem_GetText(hwndDlg, IDC_EDIT_DFORM, m_Common.m_sFormat.m_szDateFormat,
	...
	//時刻書式
	::DlgItem_GetText(hwndDlg, IDC_EDIT_TFORM, m_Common.m_sFormat.m_szTimeFormat,
					  _countof(m_Common.m_sFormat.m_szTimeFormat));
