...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropCustmenu::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	return DlgProc(reinterpret_cast<pDispatchPage>(&CPropCustmenu::DispatchEvent), hwndDlg, uMsg, wParam, lParam);
}
//	To Here Jun. 2, 2001 genta

static void SetDlgItemsEnableState(HWND hwndDlg, HWND hwndCOMBO_MENU, HWND hwndLIST_RES, HWND hwndCOMBO_FUNCKIND,
								   HWND hwndLIST_FUNC, const CFuncLookup &cLookup, const CommonSetting &common)
{
...
/* Custom menu メッセージ処理 */
INT_PTR CPropCustmenu::DispatchEvent(HWND   hwndDlg, // handle to dialog box
									 UINT   uMsg,	// message
...
)
{
	...
	case WM_INITDIALOG:
		...
		SetDlgItemsEnableState(hwndDlg, hwndCOMBO_MENU, hwndLIST_RES, hwndCOMBO_FUNCKIND, hwndLIST_FUNC, m_cLookup,
							   m_Common);
	...
	case WM_COMMAND:
		...
		case BN_CLICKED:
			...
			case IDC_BUTTON_MENUNAME:
				...
				::DlgItem_GetText(hwndDlg, IDC_EDIT_MENUNAME, m_Common.m_sCustomMenu.m_szCustMenuNameArr[nIdx1],
								  MAX_CUSTOM_MENU_NAME_LEN);
				//	Combo Boxも変更 削除＆再登録
				Combo_DeleteString(hwndCOMBO_MENU, nIdx1);
				Combo_InsertString(hwndCOMBO_MENU, nIdx1, m_cLookup.Custmenu2Name(nIdx1, buf, _countof(buf)));
		...
		else if (hwndLIST_RES == hwndCtl)
		{
			...
			case LBN_DBLCLK:
				...
				if (CB_ERR == nIdx1) { break; }
				nIdx2 = List_GetCurSel(hwndLIST_RES);
				if (LB_ERR == nIdx2) { break; }

				if (0 == m_Common.m_sCustomMenu.m_nCustMenuItemFuncArr[nIdx1][nIdx2]) { break; }
				...
				{
					BOOL bDlgInputResult = cDlgInput1.DoModal(G_AppInstance(), hwndDlg, LS(STR_PROPCOMCUSTMENU_AC1),
															  LS(STR_PROPCOMCUSTMENU_AC2), 1, szKey);
			...
			case LBN_SELCHANGE:
				...
				if (MAX_CUSTOM_MENU_ITEMS <= m_Common.m_sCustomMenu.m_nCustMenuItemNumArr[nIdx1]) { break; }
		...
		else
		{
			...
			case BN_CLICKED:
				...
				case IDC_BUTTON_INSERTSEPARATOR:
					...
					nIdx2 = List_GetCurSel(hwndLIST_RES);
					if (LB_ERR == nIdx2) { nIdx2 = 0; }
					nIdx2 = List_InsertString(
						hwndLIST_RES, nIdx2,
						LS(STR_PROPCOMCUSTMENU_SEP)); // Oct. 18, 2000 JEPRO
													  // 「ツールバー」タブで使っているセパレータと同じ線種に統一した
				...
				case IDC_BUTTON_DELETE:
					...
					if (0 == m_Common.m_sCustomMenu.m_nCustMenuItemNumArr[nIdx1]) { break; }
				...
				case IDC_BUTTON_INSERT:
					...
					if (LB_ERR == nIdx2 || LB_ERRSPACE == nIdx2) { break; }
					List_SetCurSel(hwndLIST_RES, nIdx2);
				...
				case IDC_BUTTON_UP:
...

					{
						...
						m_Common.m_sCustomMenu.m_nCustMenuItemFuncArr[nIdx1][nIdx2 - 1] =
							m_Common.m_sCustomMenu.m_nCustMenuItemFuncArr[nIdx1][nIdx2];
						m_Common.m_sCustomMenu.m_nCustMenuItemKeyArr[nIdx1][nIdx2 - 1] =
							m_Common.m_sCustomMenu.m_nCustMenuItemKeyArr[nIdx1][nIdx2];
				...
				case IDC_BUTTON_DOWN:
					...
					if (LB_ERR == nIdx2) { break; }
					nNum2 = List_GetCount(hwndLIST_RES);
					if (LB_ERR == nNum2) { break; }
					if (nNum2 - 1 <= nIdx2) { break; }
				...
				case IDC_CHECK_SUBMENU:
					nIdx1 = Combo_GetCurSel(hwndCOMBO_MENU);
					if (CB_ERR == nIdx1) { break; }
					m_Common.m_sCustomMenu.m_bCustMenuPopupArr[nIdx1] =
						IsDlgButtonCheckedBool(hwndDlg, IDC_CHECK_SUBMENU);
					break;
				}
...
void CPropCustmenu::SetData(HWND hwndDlg)
{
	...
	/* 種別の先頭の項目を選択（コンボボックス）*/
	Combo_SetCurSel(hwndCombo,
					0); // Oct. 14, 2000 JEPRO 「--未定義--」を表示させないように大元 Funcode.cpp で変更してある
	...
	//	/* カスタムメニューの先頭の項目を選択（リストボックス）*/	//Oct. 8, 2000 JEPRO
	//ここをコメントアウトすると先頭項目が選択されなくなる
...
/* ダイアログデータの取得 Custom menu */
int CPropCustmenu::GetData(HWND hwndDlg) { return TRUE; }
