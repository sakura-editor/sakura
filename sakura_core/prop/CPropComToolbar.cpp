...
static const DWORD p_helpids[] = {									 // 11000
	...
	IDC_LABEL_MENUFUNCKIND, (DWORD)-1, IDC_LABEL_MENUFUNC, (DWORD)-1, IDC_LABEL_TOOLBAR, (DWORD)-1,
	//	IDC_STATIC,						-1,
	0, 0};
//@@@ 2001.02.04 End
...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropToolbar::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
*/
int Listbox_INSERTDATA(HWND hWnd,  //!< handle to destination window
...
/*!
	...
	@date 2002.04.13 genta
*/
int Listbox_ADDDATA(HWND hWnd, //!< handle to destination window
					int  value)
{
...
static void SetDlgItemsEnableState(HWND hwndDlg, HWND hwndResList, HWND hwndFuncList)
{
	...
	else
	{
		...
		if (nIndex1 <= 0) { ::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_UP), FALSE); }
		else
		{
			::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_UP), TRUE);
		}
		if (nIndex1 + 1 >= i) { ::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_DOWN), FALSE); }
		else
		{
			::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_DOWN), TRUE);
		}
	}
	if (LB_ERR == nIndex1 || LB_ERR == nIndex2) { ::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_INSERT), FALSE); }
	else
	{
		::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_INSERT), TRUE);
	}
	if (LB_ERR == nIndex2) { ::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_ADD), FALSE); }
	else
	{
...
/* Toolbar メッセージ処理 */
INT_PTR CPropToolbar::DispatchEvent(HWND   hwndDlg, // handle to dialog box
									UINT   uMsg,	// message
...
)
{
	...
	case WM_NOTIFY:
		...
		switch (pNMHDR->code)
		{
		case PSN_HELP: OnHelp(hwndDlg, IDD_PROP_TOOLBAR); return TRUE;
		case PSN_KILLACTIVE:
		...
		case PSN_SETACTIVE: m_nPageNum = ID_PROPCOM_PAGENUM_TOOLBAR; return TRUE;
		}
		break;
	...
	case WM_COMMAND:
		...
		if (hwndResList == hwndCtl)
		{
			switch (wNotifyCode)
			{
			case LBN_SELCHANGE: return TRUE;
			}
		}
			...
			case CBN_SELCHANGE:
				...
				for (i = 0; i < nNum; ++i)
				{
					...
					if (nbarNo >= 0)
					{
						...
						if (lResult == LB_ERR || lResult == LB_ERRSPACE) { break; }
		...
		else
		{
			...
			case BN_CLICKED:
				...
				case IDC_BUTTON_INSERTSEPARATOR:
					...
					if (nIndex1 == LB_ERR || nIndex1 == LB_ERRSPACE) { break; }
				...
				case IDC_BUTTON_DELETE:
					nIndex1 = List_GetCurSel(hwndResList);
					if (LB_ERR == nIndex1) { break; }
					i = List_DeleteString(hwndResList, nIndex1);
					if (i == LB_ERR) { break; }
					if (nIndex1 >= i)
					{
						if (i == 0) { i = List_SetCurSel(hwndResList, 0); }
						else
						{
				...
				case IDC_BUTTON_UP:
					nIndex1 = List_GetCurSel(hwndResList);
					if (LB_ERR == nIndex1 || 0 >= nIndex1) { break; }
				...
				case IDC_BUTTON_DOWN:
					...
					nIndex1 = List_GetCurSel(hwndResList);
					if (LB_ERR == nIndex1 || nIndex1 + 1 >= i) { break; }
					i = List_GetItemData(hwndResList, nIndex1);
	...
	case WM_TIMER: SetDlgItemsEnableState(hwndDlg, hwndResList, hwndFuncList); break;
...
void CPropToolbar::SetData(HWND hwndDlg)
{
	...
	Combo_SetCurSel(hwndCombo,
					0); // Oct. 14, 2000 JEPRO JEPRO 「--未定義--」を表示させないように大元 Funcode.cpp で変更してある
...
void CPropToolbar::DrawToolBarItemList(DRAWITEMSTRUCT *pDis)
{
	...
	{
		// アイコンとテキストを表示する
		m_pcIcons->DrawToolIcon(pDis->hDC, rcItem.left + cxEdge,
								rcItem.top + cyEdge + (rcItem.bottom - rcItem.top - cySmIcon) / 2, tbb.iBitmap,
								ILD_NORMAL, cxSmIcon, cySmIcon);
	...
	::DrawText(pDis->hDC, szLabel, -1, &rcText, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
	...
	/* アイテムにフォーカスがある */
	if (pDis->itemState & ODS_FOCUS) { ::DrawFocusRect(pDis->hDC, &rcFrame); }
