...
static const DWORD p_helpids[] = {
	IDC_CHECK_DispTabWnd,
	HIDC_CHECK_DispTabWnd, //タブウインドウ表示	//@@@ 2003.05.31 MIK
	IDC_CHECK_DispTabWndMultiWin,
	HIDC_CHECK_DispTabWndMultiWin, //ウィンドウをまとめてグループ化する
	IDC_CHECK_RetainEmptyWindow,
	HIDC_CHECK_RetainEmptyWindow, //最後のファイルを閉じたとき(無題)文書を残す	// 2007.02.13 ryoji
	IDC_CHECK_CloseOneWin,
	HIDC_CHECK_CloseOneWin, //ウィンドウの閉じるボタンは現在のファイルのみ閉じる	// 2007.02.13 ryoji
	IDC_CHECK_OpenNewWin,
	HIDC_CHECK_OpenNewWin, //外部から起動するときは新しいウインドウで開く 2009.06.19
	IDC_CHECK_DispTabIcon,
	HIDC_CHECK_DispTabIcon, //アイコン表示	// 2006.08.06 ryoji
	IDC_CHECK_SameTabWidth,
	HIDC_CHECK_SameTabWidth, //等幅	// 2006.08.06 ryoji
	IDC_CHECK_DispTabClose,
	HIDC_CHECK_DispTabClose, //タブを閉じるボタン表示	// 2012.04.14 syat
	IDC_BUTTON_TABFONT,
	HIDC_BUTTON_TABFONT, //タブフォント
	IDC_CHECK_SortTabList,
	HIDC_CHECK_SortTabList, //タブ一覧ソート	// 2006.08.06 ryoji
	IDC_CHECK_TAB_MULTILINE,
	HIDC_CHECK_TAB_MULTILINE, //タブ多段
	IDC_COMBO_TAB_POSITION,
	HIDC_COMBO_TAB_POSITION, //タブ表示位置
	IDC_TABWND_CAPTION,
	HIDC_TABWND_CAPTION, //タブウインドウキャプション	//@@@ 2003.06.15 MIK
	IDC_CHECK_ChgWndByWheel,
	HIDC_CHECK_ChgWndByWheel, //マウスホイールでウィンドウ切り替え 2007.04.03 ryoji
	0,
	0};
...
*/
INT_PTR CALLBACK CPropTab::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
INT_PTR CPropTab::DispatchEvent(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
		...
		case PSN_SETACTIVE: m_nPageNum = ID_PROPCOM_PAGENUM_TAB; return TRUE;
		}
		//			break;	/* default */
		//		}
		break; /* WM_NOTIFY */
		...
		if (wNotifyCode == BN_CLICKED)
		{
			...
			case IDC_BUTTON_TABFONT:
				...
				if (MySelectFont(&lf, &nPointSize, hwndDlg, false))
				{
					...
					HFONT hFont = SetFontLabel(hwndDlg, IDC_STATIC_TABFONT, m_Common.m_sTabBar.m_lf,
											   m_Common.m_sTabBar.m_nPointSize);
...
void CPropTab::SetData(HWND hwndDlg)
{
	...
	::CheckDlgButton(hwndDlg, IDC_CHECK_DispTabWndMultiWin,
					 !m_Common.m_sTabBar.m_bDispTabWndMultiWin); //@@@ 2003.05.31 MIK
...
int CPropTab::GetData(HWND hwndDlg)
{
	...
	::DlgItem_GetText(hwndDlg, IDC_TABWND_CAPTION, m_Common.m_sTabBar.m_szTabWndCaption,
...
void CPropTab::EnableTabPropInput(HWND hwndDlg)
{
	...
	if (bTabWnd) { bMultiWin = ::IsDlgButtonChecked(hwndDlg, IDC_CHECK_DispTabWndMultiWin); }
