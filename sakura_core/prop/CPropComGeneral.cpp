...
static const DWORD p_helpids[] = {							   // 10900
	...
	IDC_CHECK_CLOSEALLCONFIRM,
	HIDC_CHECK_CLOSEALLCONFIRM, //[すべて閉じる]で他に編集用のウィンドウがあれば確認する	// 2006.12.25 ryoji
	...
	IDC_SPIN_REPEATEDSCROLLLINENUM, HIDC_EDIT_REPEATEDSCROLLLINENUM, IDC_SPIN_MAX_MRU_FILE, HIDC_EDIT_MAX_MRU_FILE,
	IDC_SPIN_MAX_MRU_FOLDER, HIDC_EDIT_MAX_MRU_FOLDER, IDC_CHECK_MEMDC, HIDC_CHECK_MEMDC, //画面キャッシュを使う
	IDC_COMBO_WHEEL_PAGESCROLL,
	HIDC_COMBO_WHEEL_PAGESCROLL, // 組み合わせてホイール操作した時ページスクロールする		// 2009.01.17 nasukoji
	IDC_COMBO_WHEEL_HSCROLL,
	HIDC_COMBO_WHEEL_HSCROLL, // 組み合わせてホイール操作した時横スクロールする			// 2009.01.17 nasukoji
							  //	IDC_STATIC,						-1,
	0, 0};
//@@@ 2001.02.04 End
...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropGeneral::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
static bool isImeUndesirable(int id)
{
	...
	case IDC_EDIT_MAX_MRU_FOLDER: return true;
...
)
{
	...
	case WM_COMMAND:
		...
		case BN_CLICKED:
			...
			case IDC_CHECK_STAYTASKTRAY: /* タスクトレイに常駐 */ return TRUE;
	...
	case WM_NOTIFY:
		...
		case IDC_SPIN_MAX_MRU_FOLDER:
			...
			if (nVal < 0) { nVal = 0; }
			if (nVal > MAX_OPENFOLDER) { nVal = MAX_OPENFOLDER; }
		...
		default:
			...
			case PSN_SETACTIVE:
				m_nPageNum = ID_PROPCOM_PAGENUM_GENERAL; // Oct. 25, 2000 JEPRO ZENPAN1→ZENPAN
														 // に変更(参照しているのはCPropCommon.cppのみの1箇所)
				return TRUE;
			}
			break;
		}
...
void CPropGeneral::SetData(HWND hwndDlg)
{
	...
	::CheckDlgButton(hwndDlg, IDC_CHECK_STOPS_BOTH_ENDS_WHEN_SEARCH_WORD,
					 m_Common.m_sGeneral.m_bStopsBothEndsWhenSearchWord);

	/* 段落単位で移動するときに、段落の両端で止まるか */
	::CheckDlgButton(hwndDlg, IDC_CHECK_STOPS_BOTH_ENDS_WHEN_SEARCH_PARAGRAPH,
	...
	HotKey_SetHotKey(::GetDlgItem(hwndDlg, IDC_HOTKEY_TRAYMENU), m_Common.m_sGeneral.m_wTrayMenuHotKeyCode,
					 m_Common.m_sGeneral.m_wTrayMenuHotKeyMods);
...
{
	/* カーソルのタイプ 0=win 1=dos  */
	if (::IsDlgButtonChecked(hwndDlg, IDC_RADIO_CARETTYPE0)) { m_Common.m_sGeneral.SetCaretType(0); }
	if (::IsDlgButtonChecked(hwndDlg, IDC_RADIO_CARETTYPE1)) { m_Common.m_sGeneral.SetCaretType(1); }
	...
	/* 段落単位で移動するときに、段落の両端で止まるか */
	m_Common.m_sGeneral.m_bStopsBothEndsWhenSearchParagraph =
		::IsDlgButtonChecked(hwndDlg, IDC_CHECK_STOPS_BOTH_ENDS_WHEN_SEARCH_PARAGRAPH);
	...
	if (m_Common.m_sGeneral.m_nMRUArrNum_MAX < 0) { m_Common.m_sGeneral.m_nMRUArrNum_MAX = 0; }
	if (m_Common.m_sGeneral.m_nMRUArrNum_MAX > MAX_MRU) { m_Common.m_sGeneral.m_nMRUArrNum_MAX = MAX_MRU; }
