...
static const DWORD p_helpids[] = {								 // 10000
	...
	HIDC_RADIO_BACKUP_DATETYPE1, //付加する日時の種類（作成日時）	//Jul. 05, 2001 JEPRO 追加
	IDC_RADIO_BACKUP_DATETYPE2,
	...
	HIDC_RADIO_BACKUP_DATETYPE2A, //付加する日時の種類（更新日時）※詳細設定ON用	// 2009.02.20 ryoji
								  //	IDC_STATIC,						-1,
	0, 0};
//@@@ 2001.02.04 End
...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropBackup::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
INT_PTR CPropBackup::DispatchEvent(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		EditCtl_LimitText(::GetDlgItem(hwndDlg, IDC_EDIT_BACKUPFOLDER),
						  _countof2(m_Common.m_sBackup.m_szBackUpFolder) - 1 - 1);
		// 20051107 aroka
		EditCtl_LimitText(::GetDlgItem(hwndDlg, IDC_EDIT_BACKUPFILE),
						  _countof2(m_Common.m_sBackup.m_szBackUpPathAdvanced) - 1 - 1);
		return TRUE;
	...
	case WM_NOTIFY:
		...
		default:
			switch (pNMHDR->code)
			{
			case PSN_HELP: OnHelp(hwndDlg, IDD_PROP_BACKUP); return TRUE;
			case PSN_KILLACTIVE:
				/* ダイアログデータの取得 Backup */
				GetData(hwndDlg);
				return TRUE;
				//@@@ 2002.01.03 YAZAKI 最後に表示していたシートを正しく覚えていないバグ修正
			case PSN_SETACTIVE: m_nPageNum = ID_PROPCOM_PAGENUM_BACKUP; return TRUE;
			}
			break;
		...
		//****	To Here Sept. 21, 2000 JEPRO
		//ダイアログ要素にスピンを入れるので以下のWM_NOTIFYをコメントアウトにし下に修正を置いた
		break;
	...
	case WM_COMMAND:
		...
		case EN_CHANGE: // 20051107 aroka フォルダが変更されたらリアルタイムにエディットボックス内を更新
			...
			case IDC_EDIT_BACKUPFOLDER:
				...
				::DlgItem_GetText(hwndDlg, IDC_EDIT_BACKUPFOLDER, m_Common.m_sBackup.m_szBackUpFolder,
								  _countof2(m_Common.m_sBackup.m_szBackUpFolder) - 1);
				UpdateBackupFile(hwndDlg);
				break;
			}
			break; /* EN_CHANGE */
...
void CPropBackup::SetData(HWND hwndDlg)
{
	...
	case 2:
		...
		break;
	case 3:
	case 6: ::CheckDlgButton(hwndDlg, IDC_RADIO_BACKUP_TYPE3, 1); break;
	case 4:
		::CheckDlgButton(hwndDlg, IDC_RADIO_BACKUP_DATETYPE2, 1); // 付加する日付のタイプ(前回の保存時刻)
		break;
	case 5:
	case 1:
	default: ::CheckDlgButton(hwndDlg, IDC_RADIO_BACKUP_TYPE1, 1); break;
	}
	...
	::CheckDlgButton(hwndDlg, IDC_CHECK_BACKUP_DUSTBOX,
					 m_Common.m_sBackup.m_bBackUpDustBox ? BST_CHECKED : BST_UNCHECKED); //@@@ 2001.12.11 add MIK

	/* バックアップ先フォルダを詳細設定する */ // 20051107 aroka
	::CheckDlgButton(hwndDlg, IDC_CHECK_BACKUP_ADVANCED,
					 m_Common.m_sBackup.m_bBackUpPathAdvanced ? BST_CHECKED : BST_UNCHECKED);
	...
	case 4:
		...
		break;
	default: ::CheckDlgButton(hwndDlg, IDC_RADIO_BACKUP_DATETYPE1A, 1); break;
	}
...
int CPropBackup::GetData(HWND hwndDlg)
{
	...
	m_Common.m_sBackup.m_bBackUpFolderRM =
		::IsDlgButtonCheckedBool(hwndDlg, IDC_CHECK_BACKUP_FOLDER_RM); // 2010/5/27 Uchi
	...
	/* バックアップファイルをごみ箱に放り込む */ //@@@ 2001.12.11 add MIK
	m_Common.m_sBackup.m_bBackUpDustBox =
		(BST_CHECKED == ::IsDlgButtonChecked(hwndDlg, IDC_CHECK_BACKUP_DUSTBOX)); //@@@ 2001.12.11 add MIK

	/* 指定フォルダにバックアップを作成する詳細設定 */ // 20051107 aroka
	m_Common.m_sBackup.m_bBackUpPathAdvanced =
		(BST_CHECKED == ::IsDlgButtonChecked(hwndDlg, IDC_CHECK_BACKUP_ADVANCED));
	/* バックアップを作成するフォルダ */ // 20051107 aroka
	::DlgItem_GetText(hwndDlg, IDC_EDIT_BACKUPFILE, m_Common.m_sBackup.m_szBackUpPathAdvanced,
					  _countof2(m_Common.m_sBackup.m_szBackUpPathAdvanced) - 1);
...
/*!	チェック状態に応じてダイアログボックス要素のEnable/Disableを
	...
	@date 2009.02.20 ryoji
   IDC_LABEL_BACKUP_HELPによる別コントロール隠しを廃止、if文制御をShowEnableフラグ制御に置き換えて簡素化して下記問題修正。
...
static inline void ShowEnable(HWND hWnd, BOOL bShow, BOOL bEnable)
{
	...
	::EnableWindow(
		hWnd,
		bEnable && bShow); // bShow=false,bEnable=trueの場合ショートカットキーが変な動きをするので修正	2010/5/27 Uchi
}
...
#define SHOWENABLE(id, show, enable) ShowEnable(::GetDlgItem(hwndDlg, id), show, enable)
	...
	//	SHOWENABLE( IDC_LABEL_BACKUP_HELP,		bAdvanced, bBackup );	//
	//不可視のまま放置（他コントロール隠しの方式は廃止） 2009.02.20 ryoji
	...
	SHOWENABLE(IDC_LABEL_BACKUP_4, TRUE,
			   bBackup && bFolder); // added Sept. 6, JEPRO フォルダ指定したときだけEnableになるように変更
...
void CPropBackup::UpdateBackupFile(HWND hwndDlg) //	バックアップファイルの詳細設定
{
	...
	else
	{
		...
		case 4: //	日付，時刻
			...
			if (m_Common.m_sBackup.GetBackupOpt(BKUP_YEAR))
			{ /* バックアップファイル名：日付の年 */
				wcscat(temp, LTEXT("%Y"));
			}
			if (m_Common.m_sBackup.GetBackupOpt(BKUP_MONTH))
			{ /* バックアップファイル名：日付の月 */
				wcscat(temp, LTEXT("%m"));
			}
			if (m_Common.m_sBackup.GetBackupOpt(BKUP_DAY))
			{ /* バックアップファイル名：日付の日 */
				wcscat(temp, LTEXT("%d"));
			}
			if (m_Common.m_sBackup.GetBackupOpt(BKUP_HOUR))
			{ /* バックアップファイル名：日付の時 */
				wcscat(temp, LTEXT("%H"));
			}
			if (m_Common.m_sBackup.GetBackupOpt(BKUP_MIN))
			{ /* バックアップファイル名：日付の分 */
				wcscat(temp, LTEXT("%M"));
			}
			if (m_Common.m_sBackup.GetBackupOpt(BKUP_SEC))
			{ /* バックアップファイル名：日付の秒 */
				wcscat(temp, LTEXT("%S"));
			}
	...
	if (!m_Common.m_sBackup.m_bBackUpPathAdvanced)
	{ // 詳細設定モードでないときだけ自動更新する
