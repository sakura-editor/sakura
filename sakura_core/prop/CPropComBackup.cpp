...
void CPropBackup::UpdateBackupFile(HWND hwndDlg)	//	バックアップファイルの詳細設定
{
	...
	if( !m_Common.m_sBackup.m_bBackUp ){
		...
		switch( m_Common.m_sBackup.GetBackupType() ){
		case 1: // .bak
			wcsncat( temp, LTEXT("$0.bak") );
			break;
		case 5: // .*.bak
			wcsncat( temp, LTEXT("$0.*.bak") );
			break;
		case 3: // .b??
			wcsncat( temp, LTEXT("$0.b??") );
			break;
		case 6: // .*.b??
			wcsncat( temp, LTEXT("$0.*.b??") );
			break;
