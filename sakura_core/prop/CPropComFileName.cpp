...
static const DWORD p_helpids[] = {
	// 13400
	IDC_CHECK_SHORTPATH, HIDC_CHECK_FNAME_SHORTPATH, IDC_EDIT_SHORTMAXWIDTH, HIDC_EDIT_FNAME_SHORTMAXWIDTH,
...
INT_PTR CALLBACK CPropFileName::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	WORD wNotifyCode;
	WORD wID;
	HWND hwndCtl;
...
INT_PTR CPropFileName::DispatchEvent(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY: {
		...
		default:
			...
			case PSN_SETACTIVE: m_nPageNum = ID_PROPCOM_PAGENUM_FILENAME; return TRUE;
			}
			break;
		}
	} break;
	...
	case WM_COMMAND: {
		...
		case BN_CLICKED:
			...
			case IDC_BUTTON_FNAME_INS: // 挿入
				...
				if (-1 != SetListViewItem_FILENAME(hListView, nIndex, szFrom, szTo, true)) { return TRUE; }
				break;
			case IDC_BUTTON_FNAME_ADD: // 追加
				...
				if (-1 != SetListViewItem_FILENAME(hListView, nCount, szFrom, szTo, true)) { return TRUE; }
				break;
			...
			case IDC_BUTTON_FNAME_TOP: // 先頭
				if (-1 != MoveListViewItem_FILENAME(hListView, nIndex, 0)) { return TRUE; }
				break;
			case IDC_BUTTON_FNAME_UP: // 上へ
				if (-1 != MoveListViewItem_FILENAME(hListView, nIndex, nIndex - 1)) { return TRUE; }
				break;
			...
			case IDC_BUTTON_FNAME_LAST: // 最終
				nCount = ListView_GetItemCount(hListView);
				if (-1 != MoveListViewItem_FILENAME(hListView, nIndex, nCount - 1)) { return TRUE; }
				break;
				// default:
			}
			break;
...
int CPropFileName::MoveListViewItem_FILENAME(HWND hListView, int nIndex, int nIndex2)
{
	...
	//	2004.03.24 dskoba
	if (nIndex > nCount - 1) { nIndex = nCount - 1; }
	if (nIndex2 > nCount - 1) { nIndex2 = nCount - 1; }
	if (nIndex < 0) { nIndex = 0; }
	if (nIndex2 < 0) { nIndex2 = 0; }

	if (nIndex == nIndex2) { return -1; }
