...
static const DWORD p_helpids[] = {								 // 11200
	...
	IDC_SPIN_nRulerBottomSpace, HIDC_EDIT_nRulerBottomSpace, IDC_SPIN_nRulerHeight, HIDC_EDIT_nRulerHeight,
	IDC_SPIN_nLineNumberRightSpace, HIDC_EDIT_nLineNumberRightSpace, IDC_SPIN_FUNCKEYWND_GROUPNUM,
	HIDC_EDIT_FUNCKEYWND_GROUPNUM, IDC_WINCAPTION_ACTIVE, HIDC_WINCAPTION_ACTIVE, //アクティブ時	//@@@ 2003.06.15 MIK
...
/*!
	...
	@param lParam パラメータ2
*/
INT_PTR CALLBACK CPropWin::DlgProc_page(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
static bool isImeUndesirable(int id)
{
	...
	case IDC_EDIT_FUNCKEYWND_GROUPNUM: return true;
...
)
{
	...
	case WM_NOTIFY:
		...
		default:
			...
			case PSN_SETACTIVE: m_nPageNum = ID_PROPCOM_PAGENUM_WIN; return TRUE;
			}
			break;
		case IDC_SPIN_nRulerHeight:
			...
			if (nVal > IDC_SPIN_nRulerHeight_MAX) { nVal = IDC_SPIN_nRulerHeight_MAX; }
...
void CPropWin::SetData(HWND hwndDlg)
{
	...
	EditCtl_LimitText(::GetDlgItem(hwndDlg, IDC_WINCAPTION_ACTIVE),
					  _countof(m_Common.m_sWindow.m_szWindowCaptionActive) - 1); //@@@ 2003.06.13 MIK
	EditCtl_LimitText(::GetDlgItem(hwndDlg, IDC_WINCAPTION_INACTIVE),
					  _countof(m_Common.m_sWindow.m_szWindowCaptionInactive) - 1); //@@@ 2003.06.13 MIK
...
int CPropWin::GetData(HWND hwndDlg)
{
	...
	/* ファンクションキー表示位置／0:上 1:下 */
	if (::IsDlgButtonChecked(hwndDlg, IDC_RADIO_FUNCKEYWND_PLACE1)) { m_Common.m_sWindow.m_nFUNCKEYWND_Place = 0; }
	if (::IsDlgButtonChecked(hwndDlg, IDC_RADIO_FUNCKEYWND_PLACE2)) { m_Common.m_sWindow.m_nFUNCKEYWND_Place = 1; }

	// 2002/11/04 Moca ファンクションキーのグループボタン数
	m_Common.m_sWindow.m_nFUNCKEYWND_GroupNum = ::GetDlgItemInt(hwndDlg, IDC_EDIT_FUNCKEYWND_GROUPNUM, NULL, FALSE);
	if (m_Common.m_sWindow.m_nFUNCKEYWND_GroupNum < 1) { m_Common.m_sWindow.m_nFUNCKEYWND_GroupNum = 1; }
	if (m_Common.m_sWindow.m_nFUNCKEYWND_GroupNum > 12) { m_Common.m_sWindow.m_nFUNCKEYWND_GroupNum = 12; }
	...
	if (m_Common.m_sWindow.m_nLineNumRightSpace < 0) { m_Common.m_sWindow.m_nLineNumRightSpace = 0; }
	if (m_Common.m_sWindow.m_nLineNumRightSpace > 32) { m_Common.m_sWindow.m_nLineNumRightSpace = 32; }
