...
inline const T *skip_field_prefix(const T *p)
{
    ...
    if (*p == _T2(T, 'h'))
        return p + 1;
    if (p[0] == _T2(T, 'l') && p[1] == _T2(T, 'l'))
        return p + 2;
    if (*p == _T2(T, 'l'))
        return p + 1;
    if (p[0] == _T2(T, 'I') && p[1] == _T2(T, '3') && p[2] == _T2(T, '2'))
        return p + 3;
    if (p[0] == _T2(T, 'I') && p[1] == _T2(T, '6') && p[2] == _T2(T, '4'))
        return p + 3;
    if (*p == _T2(T, 'I'))
        return p + 1;
    return p;
...
{
    if (strncmp_literal(src, "%ts") == 0 || strncmp_literal(src, "%tc") == 0)
    {
        src[1] = 'l';
    }
}
...
{
    if (wcsncmp_literal(src, L"%ts") == 0 || wcsncmp_literal(src, L"%tc") == 0)
    {
        src[1] = L'l';
    }
}
...
int tchar_vsprintf_s_imp(T *buf, size_t nBufCount, const T *format, va_list &v, bool truncate)
{
    ...
    while (*src)
    {
        if (nBufCount != MAX_BUF && dst >= buf_end - 1)
            break;
        //書式指定フィールドを取得
