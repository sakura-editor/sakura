...
const WCHAR *to_wchar(const ACHAR *pSrc, int nSrcLength)
{
	...
	//変換
	nDstLen		  = MultiByteToWideChar(CP_SJIS, // 2008/5/12 Uchi
									0, pSrc, nSrcLength, pDst, nDstLen);
	pDst[nDstLen] = L'\0';
...
const ACHAR *to_achar(const WCHAR *pSrc, int nSrcLength)
{
	...
	//変換
	nDstLen		  = WideCharToMultiByte(CP_SJIS, // 2008/5/12 Uchi
									0, pSrc, nSrcLength, pDst, nDstLen, NULL, NULL);
	pDst[nDstLen] = '\0';
