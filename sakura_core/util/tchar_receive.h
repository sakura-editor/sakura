...
/*!
	実際のデータ種に関わらず、WCHAR[]型の受け取りバッファを提供するクラス。
	使い方が特殊なので注意。
	...
	動作原理はソースを参照のこと。
	operator WCHAR* が GetWindowText に渡すポインタを提供し、
	~TcharReceiver において、必要であれば (ビルド種と受け取り型が異なれば)、
	WCHAR→wchar_t変換が発生します。
...
public:
	...
	operator WCHAR* (){ return GetBufferPointer(); }
	~TcharReceiver(){ Apply(); }
protected:
	WCHAR* GetBufferPointer();	//!< 一時バッファを提供。バッファ寿命は短いので注意。
