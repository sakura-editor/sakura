...
public:
	...
	{
		if (nMaxChar == -1) { nMaxChar = (DWORD)wcslen(buffer) * sizeof(WCHAR); }
		DWORD nError = 0;
		if ((nError = RegSetValueEx(_key, valueName, 0, REG_SZ, (LPBYTE)buffer, nMaxChar)) != 0) { return nError; }
		return ERROR_SUCCESS;
	}
	int SetValue(const WCHAR *valueName, const BYTE *buffer, int nMaxChar, DWORD dwType)
	{
		DWORD nError = 0;
		if ((nError = RegSetValueEx(_key, valueName, 0, dwType, (LPBYTE)buffer, nMaxChar)) != 0) { return nError; }
		return ERROR_SUCCESS;
	}

	int DeleteValue(const WCHAR *valueName) { return RegDeleteValue(_key, valueName); }
	int DeleteSubKey(const WCHAR *path) { return RegDeleteKey(_key, path); }
	...
	int EnumKey(int &index, WCHAR *pNameBuffer, int nMaxChar, int *pGetChar = NULL) const
	{
		...
		if (pGetChar) { *pGetChar = nMaxChar; }
		return nError;
	}

	int EnumValue(int &index, WCHAR *pNameBuffer, int nMaxChar, DWORD *lpType, BYTE *lpData, int nMaxData,
				  DWORD *pDataLen) const
	{
		if (index < 0) { return ERROR_NO_MORE_ITEMS; }
		...
		if (pDataLen) { *pDataLen = nDataSize; }
		return nError;
	}

	static int DeleteKey(HKEY root, const WCHAR *path) { return RegDeleteKey(root, path); }
};
