...
static LRESULT CALLBACK PropSheetWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		if (HIWORD(wParam) == BN_CLICKED && LOWORD(wParam) == 0x02000)
		{
			...
			int nId = ::TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON | TPM_RETURNCMD,
									   (pt.x > rc.left) ? pt.x : rc.left, (pt.y < rc.bottom) ? pt.y : rc.bottom, 0,
									   hwnd, NULL);
			::DestroyMenu(hMenu);
			...
			case 101: // インポート／エクスポートの起点リセット（起点を設定フォルダにする）
				...
				if (IDOK == nMsgResult)
				{
					...
					AddLastChar(pShareData->m_sHistory.m_szIMPORTFOLDER,
								_countof2(pShareData->m_sHistory.m_szIMPORTFOLDER), L'\\');
				}
				break;
			}
		}
	...
	case WM_DESTROY: ::SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR)s_pOldPropSheetWndProc); break;
	}
...
static int CALLBACK PropSheetProc(HWND hwndDlg, UINT uMsg, LPARAM lParam)
{
	...
	if (uMsg == PSCB_INITIALIZED)
	{
		...
		HWND	  hwndBtn =
			::CreateWindowEx(0, WC_BUTTON, LS(STR_SHELL_INIFOLDER), BS_PUSHBUTTON | WS_CHILD | WS_VISIBLE | WS_TABSTOP,
...
{
	if (DLL_SUCCESS == g_cHtmlHelp.InitDll()) { return g_cHtmlHelp.HtmlHelp(hWnd, szFile, uCmd, data); }
	if (msgflag) { ::MessageBox(hWnd, LS(STR_SHELL_HHCTRL), LS(STR_SHELL_INFO), MB_OK | MB_ICONEXCLAMATION); }
	return NULL;
}
...
BOOL ResolveShortcutLink(HWND hwnd, LPCWSTR lpszLinkFile, LPWSTR lpszPath)
{
	...
	*lpszPath	  = 0; // assume failure
	...
	if (!::GetLongFileName(lpszLinkFile, szAbsLongPath)) { return FALSE; }
...
static LPCWSTR GetHelpFilePath()
{
	...
	if (szHelpFile[0] == L'\0') { GetExedir(szHelpFile, L"sakura.chm"); }
	return szHelpFile;
}
...
BOOL MyWinHelp(HWND hwndCaller, UINT uCommand, DWORD_PTR dwData)
{
	...
	case HELP_WM_HELP: // [？]ボタンを押してコントロールをクリック／コントロールにフォーカスを置いて[F1]キー
		uCommand = HH_DISPLAY_TEXT_POPUP;
		{
			...
			else
			{
				...
				RECT rc;
				hwndCtrl = hwndCaller;
				if (!::GetWindowRect(hwndCtrl, &rc)) return FALSE;
	...
	else
	{
		...
		// オンラインヘルプを呼び出す
		if (uCommandOrg != HELP_CONTEXT) dwData = 1; // 目次ページ
