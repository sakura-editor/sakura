    ...
    switch (uMsg)
    {
        ...
        case WM_COMMAND:
            ...
            if (HIWORD(wParam) == BN_CLICKED && LOWORD(wParam) == 0x02000)
            {
                ...
                int nId = ::TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON | TPM_RETURNCMD, (pt.x > rc.left) ? pt.x : rc.left, (pt.y < rc.bottom) ? pt.y : rc.bottom, 0, hwnd, NULL);
                ::DestroyMenu(hMenu);
...
DWORD NetConnect(const WCHAR strNetWorkPass[])
{
    ...
    if (wcslen(strNetWorkPass) < 3)
        return ERROR_BAD_NET_NAME; //UNCではない。
    if (strNetWorkPass[0] != L'\\' && strNetWorkPass[1] != L'\\')
        return ERROR_BAD_NET_NAME; //UNCではない。
    ...
    if (!ChooseFont(&cf))
    {
        ...
        switch (nErr)
        {
            case CDERR_FINDRESFAILURE:
                MYTRACE(L"CDERR_FINDRESFAILURE \n");
                break;
            case CDERR_INITIALIZATION:
                MYTRACE(L"CDERR_INITIALIZATION \n");
                break;
            case CDERR_LOCKRESFAILURE:
                MYTRACE(L"CDERR_LOCKRESFAILURE \n");
                break;
            case CDERR_LOADRESFAILURE:
                MYTRACE(L"CDERR_LOADRESFAILURE \n");
                break;
            case CDERR_LOADSTRFAILURE:
                MYTRACE(L"CDERR_LOADSTRFAILURE \n");
                break;
            case CDERR_MEMALLOCFAILURE:
                MYTRACE(L"CDERR_MEMALLOCFAILURE\n");
                break;
            case CDERR_MEMLOCKFAILURE:
                MYTRACE(L"CDERR_MEMLOCKFAILURE \n");
                break;
            case CDERR_NOHINSTANCE:
                MYTRACE(L"CDERR_NOHINSTANCE \n");
                break;
            case CDERR_NOHOOK:
                MYTRACE(L"CDERR_NOHOOK \n");
                break;
            case CDERR_NOTEMPLATE:
                MYTRACE(L"CDERR_NOTEMPLATE \n");
                break;
            case CDERR_STRUCTSIZE:
                MYTRACE(L"CDERR_STRUCTSIZE \n");
                break;
            case CFERR_MAXLESSTHANMIN:
                MYTRACE(L"CFERR_MAXLESSTHANMIN \n");
                break;
            case CFERR_NOFONTS:
                MYTRACE(L"CFERR_NOFONTS \n");
                break;
        }
#endif
        return FALSE;
