...
bool fexist(LPCWSTR pszPath) { return _waccess(pszPath, 0) != -1; }
...
*/
bool IsFilePath(const wchar_t *pLine, //!< [in]  探査対象文字列
...
)
{
	...
	*pnBgn		   = i;
	...
	{
		//ファイル名終端を検知する
		if (WCODE::IsLineDelimiterExt(pLine[i]) || pLine[i] == L'\0') { break; }
		...
		{
			//	ファイル存在確認
			if (IsFileExists(szJumpToFile, bFileOnly)) { tmp_end = cur_pos; }
		}
		...
		if (!WCODE::IsValidFilenameChar(pLine[i])) { break; }
	...
	//	Jan. 04, 2002 genta
	//	ファイル存在確認方法変更
	if (szJumpToFile[0] != L'\0' && IsFileExists(szJumpToFile, bFileOnly)) { tmp_end = cur_pos; }
	if (tmp_end != 0)
	{
		*pnPathLen = tmp_end;
		return true;
...
bool IsLocalDrive(const WCHAR *pszDrive)
{
	...
	if (iswalpha(pszDrive[0]))
	{
		...
		if (lngRet == DRIVE_REMOVABLE || lngRet == DRIVE_CDROM || lngRet == DRIVE_REMOTE) { return false; }
	}
...
void CutLastYenFromDirectoryPath(WCHAR *pszFolder)
{
	...
	else
	{
		...
		if (0 < nFolderLen)
		{
			...
			if (1 == nCharChars && L'\\' == pszFolder[nFolderLen - 1]) { pszFolder[nFolderLen - 1] = L'\0'; }
		}
	}
	return;
}
...
void Concat_FolderAndFile(const WCHAR *pszDir, const WCHAR *pszTitle, WCHAR *pszPath)
{
	...
	//フォルダをコピー
	for (in = pszDir; *in != '\0';) { *out++ = *in++; }
	//円記号を付加
#if UNICODE
	if (*(out - 1) != '\\') { *out++ = '\\'; }
#else
	if (*(out - 1) != '\\' || (1 == out - CNativeW::GetCharPrev(pszDir, out - pszDir, out))) { *out++ = '\\'; }
#endif
	//ファイル名をコピー
	for (in = pszTitle; *in != '\0';) { *out++ = *in++; }
	*out = '\0';
	return;
}
...
BOOL GetLongFileName(const WCHAR *pszFilePathSrc, WCHAR *pszFilePathDes)
{
	...
	if (len <= 0 || _MAX_PATH <= len)
	{
		...
		if (len <= 0 || _MAX_PATH < len) { return FALSE; }
		return TRUE;
	}
	len = ::GetLongPathName(szBuf, pszFilePathDes, _MAX_PATH);
	if (len <= 0 || _MAX_PATH < len) { wcscpy(pszFilePathDes, szBuf); }
	return TRUE;
}
...
BOOL CheckEXT(const WCHAR *pszPath, const WCHAR *pszExt)
{
	...
	pszWork = szExt;
	if (pszWork[0] == L'.') { pszWork++; }
	if (0 == _wcsicmp(pszExt, pszWork)) { return TRUE; }
	else
	{
		return FALSE;
	}
...
{
	bool ret = true;
	if ((L'A' <= path[0] && path[0] <= L'Z' || L'a' <= path[0] && path[0] <= L'z') && path[1] == L':'
			&& path[2] == L'\\'
		|| path[0] == L'\\' && path[1] == L'\\')
...
*/
int CalcDirectoryDepth(const WCHAR *path //!< [in] 深さを調べたいファイル/ディレクトリのフルパス
)
{
	int depth = 0;
...
)
{
	...
	if (szFile == NULL) { SplitPath_FolderAndFile(szPath, pDir, NULL); }
	else
	{
...
LPCWSTR GetRelPath(LPCWSTR pszPath)
{
	...
	if (0 == wmemicmp(szPath, pszPath, nLen)) { pszFileName = pszPath + nLen; }
	else
	{
		GetExedir(szPath, L"");
		nLen = wcslen(szPath);
...
*/
bool GetLastWriteTimestamp(const WCHAR *pszFileName, //!< [in]  ファイルのパス
...
void my_splitpath_w(const wchar_t *comln, wchar_t *drv, wchar_t *dir, wchar_t *fnm, wchar_t *ext)
{
	...
	{ /* ファイル・ディレクトリが存在する場合 */
		...
		if (dir != NULL)
		{
			...
			for (ch = L'\\', pf = pd; *pf != L'\0'; pf++) { ch = *pf; }
		...
		for (int k = 0; k < len2 && score < (len2 - k);)
		{
			...
			for (int m = k; m < len2;)
			{
				...
				if (chars1 == chars2)
				{
					if (chars1 == 1)
					{
						if (skr_towlower(file1[pos1]) == skr_towlower(file2[m])) { tmpScore += chars1; }
						else
						{
							break;
						}
					...
					{
						if (0 == wmemicmp(&file1[pos1], &file2[m], chars1)) { tmpScore += chars1; }
						else
						{
							break;
						}
...
void GetShortViewPath(WCHAR *dest, int nSize, const WCHAR *path, HDC hDC, int nPxWidth, bool bFitMode)
{
	...
	else
	{
		...
		{
			// 「ほにゃらら:/」だった /が続いてる間飛ばす
			while (path[nTop] == L'/') { nTop += t_max(1, (int)(Int)CNativeW::GetSizeOfChar(path, nPathLen, nTop)); }
			nLeft = nTop;
		}
	}
	for (int i = 0; i < nSkipLevel; i++)
	{
		...
		{ nLeft += t_max(1, (int)(Int)CNativeW::GetSizeOfChar(path, nPathLen, nLeft)); } if (path[nLeft] != L'\0')
		{
			if (i + 1 < nSkipLevel) { nLeft++; }
		}
		else
		{
			if (bFitMode)
	...
	while (path[nRight] != L'\0')
	{
		...
		{ nNext += t_max(1, (int)(Int)CNativeW::GetSizeOfChar(path, nPathLen, nNext)); } if (path[nNext] != L'\0')
		{
			// サブフォルダ省略
			// C:\...\dir\file.ext
			std::wstring strTemp(path, nLeft + 1);
			if (nLeft + 1 < nRight) { strTemp += L"..."; }
			strTemp += &path[nRight];
	...
	// C:\...\file.ext
	int nLeftLen = nLeft;
	if (nLeftLen && nLeftLen != nRight) { nLeftLen++; }
	std::wstring strTemp(path, nLeftLen);
	if (nLeft != nRight) { strTemp += L"..."; }
	strTemp += &path[nRight];
	if (bFitMode)
	{
		if (calc.GetTextWidth(strTemp.c_str()) <= nPxWidth)
