...
{
	if (src_count >= dst_count) { src_count = dst_count - 1; }
...
{
	//必要な領域サイズ
	int nNewLength = MultiByteToWideChar(CP_SJIS, // 2008/5/12 Uchi
										 0, pSrc, nSrcLen, NULL, 0);

	//確保
	wchar_t *pNew = new wchar_t[nNewLength + 1];

	//変換
	nNewLength		 = MultiByteToWideChar(CP_SJIS, // 2008/5/12 Uchi
									   0, pSrc, nSrcLen, pNew, nNewLength);
	pNew[nNewLength] = L'\0';
	if (pnDstLen) { *pnDstLen = nNewLength; }
	return pNew;
}

// UNICODE→SJIS。戻り値はnew[]で確保して返す。
char *wcstombs_new(const wchar_t *src) { return wcstombs_new(src, wcslen(src)); }
//戻り値はnew[]で確保して返す。
char *wcstombs_new(const wchar_t *pSrc, int nSrcLen)
{
	//必要な領域サイズ
	int nNewLength = WideCharToMultiByte(CP_SJIS, // 2008/5/12 Uchi
										 0, pSrc, nSrcLen, NULL, 0, NULL, NULL);

	//確保
	char *pNew = new char[nNewLength + 1];

	//変換
	nNewLength		 = WideCharToMultiByte(CP_SJIS, // 2008/5/12 Uchi
									   0, pSrc, nSrcLen, pNew, nNewLength, NULL, NULL);
	pNew[nNewLength] = '\0';
...
void mbstowcs_vector(const char *src, std::vector<wchar_t> *ret) { mbstowcs_vector(src, strlen(src), ret); }
...
{
	//必要な容量
	int nNewLen = MultiByteToWideChar(CP_SJIS, // 2008/5/12 Uchi
									  0, pSrc, nSrcLen, NULL, 0);
	...
	//変換
	nNewLen			= MultiByteToWideChar(CP_SJIS, // 2008/5/12 Uchi
								  0, pSrc, nSrcLen, &(*ret)[0], nNewLen);
	(*ret)[nNewLen] = L'\0';
}

// UNICODE→SJIS。戻り値はvectorとして返す。
void wcstombs_vector(const wchar_t *src, std::vector<char> *ret) { wcstombs_vector(src, wcslen(src), ret); }
void wcstombs_vector(const wchar_t *pSrc, int nSrcLen, std::vector<char> *ret)
{
	//必要な容量
	int nNewLen = WideCharToMultiByte(CP_SJIS, // 2008/5/12 Uchi
									  0, pSrc, nSrcLen, NULL, 0, NULL, NULL);
	...
	//変換
	nNewLen			= WideCharToMultiByte(CP_SJIS, // 2008/5/12 Uchi
								  0, pSrc, nSrcLen, &(*ret)[0], nNewLen, NULL, NULL);
	(*ret)[nNewLen] = '\0';
}
...
int wmemicmp(const WCHAR *p1, const WCHAR *p2) { return wmemicmp(p1, p2, t_max(wcslen(p1), wcslen(p2))); }
