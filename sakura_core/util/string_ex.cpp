...
const char *strichr_j(const char *s1, char c2)
{
    ...
    for (const char *p1 = s1; *p1; p1++)
    {
        if (my_toupper(*p1) == C2)
            return p1;
        if (my_iskanji1(*(const unsigned char *)p1) && *(p1 + 1) != 0)
            p1++;
    }
    return NULL;
}
...
)
{
    ...
    do
    {
        ...
        for (; i < nLen; i++)
        {
            if (pBuffer[i] == Charset<CHAR_TYPE>::QUOT)
                bFlag = !bFlag;
            if (!bFlag)
            {
...
int my_mbisalpha2(int c)
{
    if ((c >= 0x60 && c <= 0x79) || (c >= 0x81 && c <= 0x9a))
        return 1;
    return 0;
}
#endif /* MY_ICMP_MBS */
...
int skr_towupper(int c)
{
    ...
    if (!bInit)
    {
        ...
        for (i = 0; i < 0x80; i++)
            szMap[i] = (wchar_t)my_towupper(i); // 自前で変換
        for (; i < 0xA0; i++)
            szMap[i] = (wchar_t)i; // 無変換（制御コード部）
        for (; i < 255; i++)
            szMap[i] = _towupper_l((wchar_t)i, locale); // "English"localeで変換
...
int skr_towlower(int c)
{
    ...
    if (!bInit)
    {
        ...
        _locale_t locale = _create_locale(LC_CTYPE, "English");
        for (i = 0; i < 0x80; i++)
            szMap[i] = (wchar_t)my_towlower(i); // 自前で変換
        for (; i < 0xA0; i++)
            szMap[i] = (wchar_t)i; // 無変換（制御コード部）
        for (; i < 256; i++)
            szMap[i] = _towlower_l((wchar_t)i, locale); // "English"localeで変換
        _free_locale(locale);
        bInit = true;
    }
