...
{
public:
	COutputAdapterDefault(CEditView *view, BOOL bToEditWindow)
...
{
public:
	COutputAdapterUTF8(CEditView *view, BOOL bToEditWindow)
		: COutputAdapterDefault(view, bToEditWindow)
...
bool CEditView::ExecCmd(const WCHAR *pszCmd, int nFlgOpt, const WCHAR *pszCurDir, COutputAdapter *customOa)
{
	...
	BOOL bToEditWindow =
		((nFlgOpt & 0x02) && bEditable) ? TRUE : FALSE; //	TRUE=編集中のウィンドウ / FALSAE=アウトプットウィンドウ
	...
	if (nFlgOpt & 0x08) { outputEncoding = CODE_UNICODE; }
	...
	if (nFlgOpt & 0x10) { sendEncoding = CODE_UNICODE; }
	...
	if (bBeforeTextSelected) { ptFrom = this->GetSelectionInfo().m_sSelect.GetFrom(); }
	...
	DuplicateHandle(
		GetCurrentProcess(), hStdOutRead, GetCurrentProcess(),
		&hStdOutRead, // 新しい継承不能ハンドルを受け取る	// 2007.01.31 ryoji
		0, FALSE,
		DUPLICATE_CLOSE_SOURCE
			| DUPLICATE_SAME_ACCESS); // 元の継承可能ハンドルは DUPLICATE_CLOSE_SOURCE で閉じる	// 2007.01.31 ryoji
	...
	{ /* 現在編集中のファイルを子プロセスの標準入力へ */
		...
		{
			// 子プロセスへの継承用にファイルを開く
			hStdIn = CreateFile(szTempFileName, GENERIC_READ, 0, &sa, OPEN_EXISTING,
								FILE_ATTRIBUTE_READONLY | FILE_ATTRIBUTE_TEMPORARY | FILE_FLAG_DELETE_ON_CLOSE, NULL);
			if (hStdIn == INVALID_HANDLE_VALUE) hStdIn = NULL;
		}
	}

	if (hStdIn == NULL)
	{ /* 標準入力を制御しない場合、または一時ファイルの生成に失敗した場合 */
		bSendStdin = FALSE;
		...
		if (hStdIn == NULL)
		{
			...
			if (hStdInWrite != NULL) { ::CloseHandle(hStdInWrite); }
		}
	}
	...
	if (CreateProcess(NULL, cmdline, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, bCurDir ? pszCurDir : NULL, &sui, &pi)
		== FALSE)
	{
		...
		//コマンドライン文字列作成
		auto_sprintf(cmdline, L"\"%s\\%s\" %s%s%s", szCmdDir, L"cmd.exe",
					 (outputEncoding == CODE_UNICODE ? L"/U" : L""), // Unicdeモードでコマンド実行	2008/6/17 Uchi
					 (bGetStdout ? L"/C " : L"/K "), pszCmd);
		if (CreateProcess(NULL, cmdline, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, bCurDir ? pszCurDir : NULL, &sui,
						  &pi)
			== FALSE)
		{
	...
	if (!bBeforeTextSelected && bSendStdin && bGetStdout && bToEditWindow)
	{
		GetSelectionInfo().SetSelectArea(
			CLayoutRange(CLayoutPoint(CLayoutInt(0), CLayoutInt(0)),
	...
	if (bGetStdout)
	{
		...
		oaInst			   = (customOa ? NULL
						   : (outputEncoding == CODE_UTF8 ? new COutputAdapterUTF8(this, bToEditWindow)
		...
		if (bOutputExtInfo)
		{
			...
			len = auto_snprintf_s(szOutTemp, _countof(szOutTemp), L"#CmdLine  : %s\r\n", pszCmd);
			oa.OutputW(szOutTemp, len);
			oa.OutputW(L"#============================================================\r\n");
		}
		...
		do
		{
			...
			{ //パイプの中の読み出し待機中の文字数を取得
				while (new_cnt > 0)
				{ //待機中のものがある

					if (new_cnt > MAX_WORK_READ)
					{ //パイプから読み出す量を調整
					...
					if (outputEncoding == CODE_UNICODE)
					{
						...
						if (read_cnt % (int)sizeof(wchar_t)) { byteCarry = work[read_cnt - 1]; }
						if (read_cntw)
						{
							workw[read_cntw] = L'\0';
							bCarry			 = false;
							...
							if (!oa.OutputW(workw, read_cntw)) { goto finish; }
							bufidx = 0;
							if (bCarry)
							{
					...
					else if (outputEncoding == CODE_SJIS)
					{
						...
						{
							//	2007.09.10 ryoji
							if (CShiftJis::GetSizeOfChar(work, read_cnt, j) == 2) { j++; }
							else
							{
						...
						{ //ぴったり出力できる場合
							...
							if (!oa.OutputA(work, read_cnt)) { goto finish; }
							bufidx = 0;
						}
						else
						{
							...
							work[read_cnt - 1] = '\0';
							//	2006.12.03 maru アウトプットウィンドウor編集中のウィンドウ分岐追加
							if (!oa.OutputA(work, read_cnt - 1)) { goto finish; }
							work[0] = tmp;
							bufidx  = 1;
					...
					else if (outputEncoding == CODE_UTF8)
					{
						...
						for (j = 0; j < (int)read_cnt;)
						{
							...
							if (echarset == CHARSET_BINARY2) { break; }
						...
						else
						{
							...
							work[j] = '\0';
							//	2006.12.03 maru アウトプットウィンドウor編集中のウィンドウ分岐追加
							if (!oa.OutputA(work, j)) { goto finish; }
							memcpy(work, tmp, len);
							bufidx = len;
							DEBUG_TRACE(L"ExecCmd: Carry last character [%x]\n", tmp[0]);
						}
					...
					new_cnt = 0;
					if (!PeekNamedPipe(hStdOutRead, NULL, 0, NULL, &new_cnt, NULL)) { break; }
					Sleep(0);
					...
					if (!::BlockingHook(cDlgCancel.GetHwnd()))
					{
						if (bLoopFlag) { ::TerminateProcess(pi.hProcess, 0); }
						goto finish;
					}
					if (cDlgCancel.IsCanceled())
					{
...
{
	if (m_bWindow) { m_pCommander->Command_INSTEXT(false, pBuf, CLogicInt(size), true); }
	else
	{
...
bool COutputAdapterDefault::OutputA(const ACHAR *pBuf, int size)
{
	CNativeA input;
	CNativeW buf;
	if (-1 == size) { input.SetString(pBuf); }
	else
	{
...
bool COutputAdapterUTF8::OutputA(const ACHAR *pBuf, int size)
{
	CMemory  input;
	CNativeW buf;
	if (-1 == size) { input.SetRawData(pBuf, strlen(pBuf)); }
	else
	{
