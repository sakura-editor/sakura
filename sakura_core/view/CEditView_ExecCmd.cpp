...
class COutputAdapterDefault : public COutputAdapter
{
  public:
    ...
    bool IsActiveDebugWindow()
    {
        return FALSE == m_bWindow;
    }
...
bool CEditView::ExecCmd(const WCHAR *pszCmd, int nFlgOpt, const WCHAR *pszCurDir, COutputAdapter *customOa)
{
    ...
    DuplicateHandle(GetCurrentProcess(), hStdOutRead, GetCurrentProcess(), &hStdOutRead, // 新しい継承不能ハンドルを受け取る	// 2007.01.31 ryoji
                    0,
                    FALSE,
    ...
    if (CreateProcess(NULL, cmdline, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, bCurDir ? pszCurDir : NULL, &sui, &pi) == FALSE)
    {
...
finish:
    //終了処理
    if (hStdIn != NULL)
        CloseHandle(hStdIn); /* 2007.03.18 maru 標準入力の制御のため */
    if (hStdOutWrite)
        CloseHandle(hStdOutWrite);
