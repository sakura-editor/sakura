...
{
	if (m_bHokan && !m_bExecutingKeyMacro)
	{ /* キーボードマクロの実行中 */
		CNativeW cmemData;
...
void CEditView::ShowHokanMgr(CNativeW &cmemData, BOOL bAutoDecided)
{
	...
	*/
	...
	// ※ Vista/7 の特定の PC でだけのちらつきか？ 該当 PC 以外の Vista/7 PC
	// でもたまに微妙に表示が乱れた感じになる程度の症状が見られたが、それらが同一原因かどうかは不明。
	if (!m_pcEditWnd->m_cHokanMgr.GetHwnd())
	{ m_pcEditWnd->m_cHokanMgr.DoModeless(G_AppInstance(), m_pcEditWnd->GetHwnd(), (LPARAM)this); }
	nKouhoNum = m_pcEditWnd->m_cHokanMgr.CHokanMgr::Search(
		&poWin, GetTextMetrics().GetHankakuHeight(), GetTextMetrics().GetHankakuDx(), cmemData.GetStringPtr(),
		m_pTypeData->m_szHokanFile, m_pTypeData->m_bHokanLoHiCase,
		m_pTypeData->m_bUseHokanByFile, // 2003.06.22 Moca
		m_pTypeData->m_nHokanType, m_pTypeData->m_bUseHokanByKeyword, pcmemHokanWord);
	/* 補完候補の数によって動作を変える */
	if (nKouhoNum <= 0)
	{ //	候補無し
		if (m_bHokan)
	...
	else if (bAutoDecided && nKouhoNum == 1)
	{ //	候補1つのみ→確定。
		if (m_bHokan)
		{
			m_pcEditWnd->m_cHokanMgr.Hide();
...
)
{
	...
	for (CLogicInt i = CLogicInt(0); i < nLines; i++)
	{
		...
		for (j = 0; j < nLineLen; j += nCharSize)
		{
			...
			// 全角記号は候補に含めない
			if (kindPre == CK_ZEN_SPACE || kindPre == CK_ZEN_NOBASU || kindPre == CK_ZEN_DAKU || kindPre == CK_ZEN_KIGO
			...
			for (j += nCharSize; j < nLineLen; j += nCharSize)
			{
				...
				// 全角記号は候補に含めない
				if (kindCur == CK_ZEN_SPACE || kindCur == CK_ZEN_KIGO || kindCur == CK_ZEN_SKIGO) { break; }

				// 文字種類が変わったら単語の切れ目とする
				ECharKind kindMerge = CWordParse::WhatKindOfTwoChars(kindPre, kindCur);
				if (kindMerge == CK_NULL)
				{ // kindPreとkindCurが別種
					if (kindCur == CK_HIRA)
					{
			...
			if (0 < nWordLenStop) { nWordLen = nWordLenStop; }

			// CDicMgr等の制限により長すぎる単語は無視する
			if (nWordLen > 1020) { continue; }
			...
			// カーソル位置の単語は候補からはずす
			if (ptCur.y == i && nWordBegin <= ptCur.x && ptCur.x <= nWordBegin + nWordLen)
			{ // 2010.02.20 syat 修正// 2008.11.09 syat 修正
				continue;
			}
			...
			if (0 != nMaxKouho && nMaxKouho <= (int)vKouho.size()) { return vKouho.size(); }
		}
	}
	return vKouho.size();
}
