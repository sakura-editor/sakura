...
*/
    ...
    if (m_pEditView->GetTextArea().GetAreaLeft() <= ptDrawPos.x && m_pEditView->GetTextArea().GetAreaTop() <= ptDrawPos.y && ptDrawPos.x < m_pEditView->GetTextArea().GetAreaRight() && ptDrawPos.y < m_pEditView->GetTextArea().GetAreaBottom())
    {
        // 画面内ならキャレットを表示する
        ;
    }
...
CLayoutInt CCaret::Cursor_UPDOWN(CLayoutInt nMoveLines, bool bSelect)
{
    ...
    if (nMoveLines > 0)
    { // 下移動。
        const bool existsEOFOnlyLine   = pLayoutMgr->GetBottomLayout() && pLayoutMgr->GetBottomLayout()->GetLayoutEol() != EOL_NONE || pLayoutMgr->GetLineCount() == 0;
...
{
    int nPosX = m_pEditView->GetTextArea().GetAreaLeft() + m_pEditView->GetTextMetrics().GetCharPxWidth(ptCaretPos.x - m_pEditView->GetTextArea().GetViewLeftCol());
    ...
    {
        nPosY = m_pEditView->GetTextArea().GetAreaTop() + (Int)(nY)*m_pEditView->GetTextMetrics().GetHankakuDy() + m_pEditView->GetTextMetrics().GetHankakuHeight() - GetCaretSize().cy; //下寄せ
    }
...
)
{
    ...
    if (ptNewXY.y >= m_pEditDoc->m_cLayoutMgr.GetLineCount() && (m_pEditView->GetSelectionInfo().IsMouseSelecting() && m_pEditView->GetSelectionInfo().IsBoxSelecting()))
    {
    ...
    else
    {
        ...
        if (i >= nLineLen)
        {
            ...
            /* フリーカーソルモードか */
            if (GetDllShareData().m_Common.m_sGeneral.m_bIsFreeCursorMode || (m_pEditView->GetSelectionInfo().IsMouseSelecting() && m_pEditView->GetSelectionInfo().IsBoxSelecting()) /* マウス範囲選択中 && 矩形範囲選択中 */
