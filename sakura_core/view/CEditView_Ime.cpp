    ...
    if (GetSelectionInfo().IsTextSelected())
    {
        ...
        if (ptSelectTo.y != ptSelect.y)
        {
            ...
            else
            {
                ...
                if (ptSelect.y == GetCaret().GetCaretLogicPos().y && pDocLine && pDocLine->GetLengthWithoutEOL() == GetCaret().GetCaretLogicPos().x)
                {
    ...
    if (NULL != pReconv)
    {
        ...
        if (bUnicode)
        {
            ...
            if (dwInsByteCount)
            {
                ...
                pb += dwInsByteCount;
                CopyMemory(pb, ((char *)pszReconv) + dwCompStrOffset, dwReconvTextLen * sizeof(wchar_t) - dwCompStrOffset);
            }
            else
            {
