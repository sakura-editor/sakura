	...
	if (hIMC)
	{
		if (::ImmGetOpenStatus(hIMC))
		{
			...
			CompForm.ptCurrentPos.y =
				(long)point.y + GetCaret().GetCaretSize().cy - GetTextMetrics().GetHankakuHeight();
...
void CEditView::SetIMECompFormFont(void)
{
	...
	//
	HIMC hIMC = ::ImmGetContext(GetHwnd());
	if (hIMC) { ::ImmSetCompositionFont(hIMC, const_cast<LOGFONT *>(&(m_pcEditWnd->GetLogfont()))); }
	::ImmReleaseContext(GetHwnd(), hIMC);
}
...
LRESULT CEditView::SetReconvertStruct(PRECONVERTSTRING pReconv, bool bUnicode, bool bDocumentFeed)
{
	...
	//矩形選択中は何もしない
	if (GetSelectionInfo().IsBoxSelecting()) return 0;

	// 2010.04.06 ビューモードでは何もしない
	if (CAppMode::getInstance()->IsViewMode()) { return 0; }
	...
	if ((nReconvLen + nReconvIndex - ptSelect.x) > nReconvMaxLen)
	{
		...
		while (p <= q) { p = t_max(p + 1, const_cast<const wchar_t *>(::CharNext(p))); }
	...
	if (bDocumentFeed)
	{
		...
		if (0 == nInsertCompLen)
		{
			...
			if (immRet == IMM_ERROR_NODATA || immRet == IMM_ERROR_GENERAL) { m_szComposition[0] = L'\0'; }
	...
	if (bDocumentFeed && pReconv) { m_szComposition[0] = L'\0'; }
...
LRESULT CEditView::SetSelectionFromReonvert(const PRECONVERTSTRING pReconv, bool bUnicode)
{
	...
	// 再変換情報が保存されているか
	if ((m_nLastReconvIndex < 0) || (m_nLastReconvLine < 0)) return 0;

	if (GetSelectionInfo().IsTextSelected()) GetSelectionInfo().DisableSelectArea(true);

	if (0 != pReconv->dwVersion) { return 0; }
	...
	else
	{
		...
		{
			if (pReconv->dwSize < (pReconv->dwStrOffset + pReconv->dwCompStrOffset)) { return 0; }
		...
		if (pReconv->dwCompStrLen > 0)
		{
			...
			{ return 0; } // 2010.03.17 sizeof(pReconv)+1ではなくdwStrOffsetを利用するように
			const char *p = ((const char *)pReconv) + pReconv->dwStrOffset;
	...
	//選択開始の位置を取得
	m_pcEditDoc->m_cLayoutMgr.LogicToLayout(CLogicPoint(m_nLastReconvIndex + dwOffset, m_nLastReconvLine),
											GetSelectionInfo().m_sSelect.GetFromPointer());

	//選択終了の位置を取得
	m_pcEditDoc->m_cLayoutMgr.LogicToLayout(CLogicPoint(m_nLastReconvIndex + dwOffset + dwLen, m_nLastReconvLine),
