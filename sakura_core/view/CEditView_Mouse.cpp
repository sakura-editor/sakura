    ...
    if (!tripleClickMode && GetDllShareData().m_Common.m_sEdit.m_bUseOLE_DragDrop)
    {
        ...
        { /* OLEによるドラッグ元にするか */
            ...
            if (0 == IsCurrentPositionSelected(ptNew))
            {
                ...
                {
                    DWORD dwEffects;
                    DWORD dwEffectsSrc = (!m_pcEditDoc->IsEditable()) ? DROPEFFECT_COPY : DROPEFFECT_COPY | DROPEFFECT_MOVE;
...
LRESULT CEditView::OnMOUSEWHEEL2(WPARAM wParam, LPARAM lParam, bool bHorizontalMsg, EFunctionCode nCmdFuncID)
{
...

    {
        ...
        bool bExecCmd = false;
        {
            ...
            if (nFuncID == F_WHEELLEFT || nFuncID == F_WHEELRIGHT || nFuncID == F_WHEELPAGELEFT || nFuncID == F_WHEELPAGERIGHT)
            {
                bHorizontal = true;
            }
            if (nFuncID == F_WHEELPAGEUP || nFuncID == F_WHEELPAGEDOWN || nFuncID == F_WHEELPAGELEFT || nFuncID == F_WHEELPAGERIGHT)
            {
                bKeyPageScroll = true;
            }
            if (nFuncID == F_WHEELUP || nFuncID == F_WHEELLEFT || nFuncID == F_WHEELPAGEUP || nFuncID == F_WHEELPAGELEFT)
            {
                if (nScrollCode != SB_LINEUP)
                {
                    zDelta *= -1;
            ...
            else if (nFuncID == F_WHEELDOWN || nFuncID == F_WHEELRIGHT || nFuncID == F_WHEELPAGEDOWN || nFuncID == F_WHEELPAGERIGHT)
            {
...
void CEditView::OnLBUTTONDBLCLK(WPARAM fwKeys, int _xPos, int _yPos)
{
    ...
    EFunctionCode nFuncID = GetDllShareData().m_Common.m_sKeyBind.m_pKeyNameArr[m_dwTripleClickCheck ? MOUSEFUNCTION_QUADCLICK : MOUSEFUNCTION_DOUBLECLICK].m_nFuncCodeArr[nIdx];
    if (m_dwTripleClickCheck)
    {
        ...
        if (m_pcEditDoc->m_cLayoutMgr.GetLineStr(ptCaretLayoutPos_Old.GetY2(), &nLineLen, &pcLayout))
        {
            ...
            { // 行終端より右の場合には nLineAllColLen に行全体の表示桁数が入っている
                ptCaretLogicPos_Old.SetX(
                    ptCaretLogicPos_Old.GetX2() + (Int)(ptCaretLayoutPos_Old.GetX2() - nLineAllColLen));
            }
        }
    ...
    if (bMove)
    {
        ...
        else
        {
            ...
            if (!bBoxData)
            {
                ...
                { // 選択開始が削除末尾と同一行
                    sSelLogic.SetFromX(
                        sSelLogic.GetFrom().GetX2() - (sDelLogic.GetTo().GetX2() - sDelLogic.GetFrom().GetX2()));
                }
                if (sSelLogic.GetTo().GetY2() == sDelLogic.GetTo().GetY2())
                { // 選択終了が削除末尾と同一行
                    sSelLogic.SetToX(
                        sSelLogic.GetTo().GetX2() - (sDelLogic.GetTo().GetX2() - sDelLogic.GetFrom().GetX2()));
                }
                // Note.
...
void CEditView::OnMyDropFiles(HDROP hDrop)
{
    ...
    if (nTid1 != nTid2)
        ::AttachThreadInput(nTid1, nTid2, TRUE);
    ...
    int nId = ::TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON | TPM_RETURNCMD, (pt.x > rcWork.left) ? pt.x : rcWork.left, (pt.y < rcWork.bottom) ? pt.y : rcWork.bottom, 0, hwnd, NULL);
    ::DestroyMenu(hMenu);
    ...
    // 入力状態をデタッチする
    if (nTid1 != nTid2)
        ::AttachThreadInput(nTid1, nTid2, FALSE);
