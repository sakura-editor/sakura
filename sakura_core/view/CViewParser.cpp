...
int CViewParser::GetLeftWord(CNativeW *pcmemWord, int nMaxWordLen) const
{
	...
	if (1 == nCharChars)
	{
		if (WCODE::IsWordDelimiter(pLine[nIdx])) { return 0; }
	}

	/* 現在位置の単語の範囲を調べる */
	CLayoutRange sRange;
	bool		 bResult =
		m_pEditView->m_pcEditDoc->m_cLayoutMgr.WhereCurrentWord(nCurLine, nIdx, &sRange, &cmemWord, pcmemWord);
	if (bResult)
	{
...
*/
bool CViewParser::GetCurrentWord(CNativeW *pcmemWord) const
{
	const CLayout *pcLayout =
		m_pEditView->m_pcEditDoc->m_cLayoutMgr.SearchLineByLayoutY(m_pEditView->GetCaret().GetCaretLayoutPos().GetY2());
	if (NULL == pcLayout) { return false; /* 単語選択に失敗 */ }
	...
	bool		 bResult = m_pEditView->m_pcEditDoc->m_cLayoutMgr.WhereCurrentWord(
		m_pEditView->GetCaret().GetCaretLayoutPos().GetY2(), nIdx, &sRange, pcmemWord, NULL);
