...
class CCaretUnderLine
{
  public:
    ...
    {
        if (!m_nLockCounter)
            m_bUnderLineDoNotOFF = flag;
    }
    void SetVertLineDoNotOFF(bool flag)
    {
        if (!m_nLockCounter)
            m_bVertLineDoNotOFF = flag;
    }
    inline bool GetUnderLineDoNotOFF() const
    {
        return m_bUnderLineDoNotOFF;
    }
    inline bool GetVertLineDoNotOFF() const
    {
        return m_bVertLineDoNotOFF;
    }
...
class CCaret
{
  public:
    ...
    //$ 設計思想的に微妙
    void SetCaretLayoutPos(const CLayoutPoint &pt)
    {
        m_ptCaretPos_Layout = pt;
    } //!< キャレット位置(レイアウト)を設定
    void SetCaretLogicPos(const CLogicPoint &pt)
    {
        m_ptCaretPos_Logic = pt;
    } //!< キャレット位置(ロジック)を設定
    ...
    CLayoutPoint GetCaretLayoutPos() const
    {
        return m_ptCaretPos_Layout;
    } //!< キャレット位置(レイアウト)を取得
    ...
    CLogicPoint GetCaretLogicPos() const
    {
        return m_ptCaretPos_Logic;
    } //!< キャレット位置(ロジック)を取得

    // -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
    //                  低頻度インターフェース                     //
    // -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
    bool GetCaretShowFlag() const
    {
        return m_bCaretShowFlag;
    }
