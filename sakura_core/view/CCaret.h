...
public:
	...
	// 表示非表示を切り替えられないようにする
	void Lock() { m_nLockCounter++; }
	// 表示非表示を切り替えられるようにする
	void UnLock()
	{
		m_nLockCounter--;
		if (m_nLockCounter < 0) { m_nLockCounter = 0; }
	}
	void UnderLineLock() { m_nUnderLineLockCounter++; }
	// 表示非表示を切り替えられるようにする
	void UnderLineUnLock()
	{
		m_nUnderLineLockCounter--;
		if (m_nUnderLineLockCounter < 0) { m_nUnderLineLockCounter = 0; }
	}
	void CaretUnderLineON(bool bDraw, bool bPaintDraw); // カーソル行アンダーラインのON
	void CaretUnderLineOFF(bool bDraw, bool bDrawPaint = true,
						   bool bResetFlag = false); // カーソル行アンダーラインのOFF
	void SetUnderLineDoNotOFF(bool flag)
	{
		if (!m_nLockCounter) m_bUnderLineDoNotOFF = flag;
	}
	void SetVertLineDoNotOFF(bool flag)
	{
		if (!m_nLockCounter) m_bVertLineDoNotOFF = flag;
	}
...
public:
	...
	void CreateEditCaret(COLORREF crCaret, COLORREF crBack, int nWidth, int nHeight);
	...
	//設定
	CLayoutInt MoveCursorToClientPoint(const POINT &ptClientPos, bool = false,
									   CLayoutPoint * = NULL); //!< マウス等による座標指定によるカーソル移動
	...
	CLayoutInt MoveCursorProperly(CLayoutPoint ptNewXY, bool, bool = false, CLayoutPoint * = NULL,
								  int = _CARETMARGINRATE, int = 0); /* 行桁指定によるカーソル移動（座標調整付き） */
	...
	CMySize GetCaretSize() const
	{
		return m_sizeCaret;
	} //!< キャレットサイズを取得。※正確には高さは違うらしい (この半分のこともある？)
	bool ExistCaretFocus() const
	{
		return m_sizeCaret.cx > 0;
	} //!< キャレットのフォーカスがあるか。※横幅値で判定してるらしい。
