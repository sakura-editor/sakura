...
public:
	...
	CPixelXInt GetCharSpacing() const { return GetHankakuDx() - GetHankakuWidth(); }
	// レイアウト幅分のピクセル幅を取得する
	int GetCharPxWidth(CLayoutXInt col) const { return (Int)col; }

	int GetCharPxWidth() const { return 1; }
	int GetCharHeightMarginByFontNo(int n) const { return m_aFontHeightMargin[n]; }

	// 固定文字x桁のレイアウト幅を取得する
	CLayoutXInt GetLayoutXDefault(CKetaXInt chars) const { return CLayoutXInt(GetHankakuDx() * (Int)chars); }
	...
	static const int *GenerateDxArray(std::vector<int> *vResultArray, //!< [out] 文字間隔配列の受け取りコンテナ
									  const wchar_t *pText,			  //!< [in]  文字列
									  int			 nLength,		  //!< [in]  文字列長
