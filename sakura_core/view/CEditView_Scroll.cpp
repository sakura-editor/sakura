...
CLayoutInt CEditView::OnVScroll(int nScrollCode, int nPos)
{
	...
	case SB_LINEDOWN:
		...
		//		}
		nScrollVal =
			ScrollAtV(GetTextArea().GetViewTopLine() + GetDllShareData().m_Common.m_sGeneral.m_nRepeatedScrollLineNum);
		break;
	case SB_LINEUP:
		//		for( i = 0; i < 4; ++i ){
		//			ScrollAtV( GetTextArea().GetViewTopLine() - 1 );
		...
		nScrollVal =
			ScrollAtV(GetTextArea().GetViewTopLine() - GetDllShareData().m_Common.m_sGeneral.m_nRepeatedScrollLineNum);
		break;
	case SB_PAGEDOWN: nScrollVal = ScrollAtV(GetTextArea().GetBottomLine()); break;
	case SB_PAGEUP: nScrollVal = ScrollAtV(GetTextArea().GetViewTopLine() - GetTextArea().m_nViewRowNum); break;
	case SB_THUMBPOSITION: nScrollVal = ScrollAtV(CLayoutInt(nPos), FALSE); break;
	case SB_THUMBTRACK: nScrollVal = ScrollAtV(CLayoutInt(nPos), FALSE); break;
	case SB_TOP: nScrollVal = ScrollAtV(CLayoutInt(0)); break;
	case SB_BOTTOM:
...
CLayoutInt CEditView::OnHScroll(int nScrollCode, int nPos)
{
	...
	case SB_PAGERIGHT: nScrollVal = ScrollAtH(GetTextArea().GetRightCol()); break;
	...
	case SB_THUMBTRACK:
		...
		//		MYTRACE( L"nPos=%d\n", nPos );
		break;
	case SB_LEFT: nScrollVal = ScrollAtH(CLayoutInt(0)); break;
	case SB_RIGHT:
...
static void setScrollInfoIfNeeded(HWND hWndScrollBar, int nMax, UINT nPage, int nPos, BOOL bRedraw)
{
	...
	if (!::GetScrollInfo(hWndScrollBar, SB_CTL, &siPrev)) { return; }
...
void CEditView::AdjustScrollBars(BOOL bRedraw)
{
	...
	if (NULL != m_hwndVScrollBar) {
		...
		// 64bit版用スクロール率
		while (nAllLines / nVScrollRate > INT_MAX) { ++nVScrollRate; }
#endif

		setScrollInfoIfNeeded(m_hwndVScrollBar, (Int)nAllLines / nVScrollRate - 1, /* 全行数 */
	...
	if (NULL != m_hwndHScrollBar) {
		...
		setScrollInfoIfNeeded(m_hwndHScrollBar,
							  (Int)GetRightEdgeForScrollBar()
								  - 1, // 2009.08.28 nasukoji	スクロールバー制御用の右端座標を取得
...
CLayoutInt CEditView::ScrollAtH(CLayoutInt nPos, BOOL bRedrawScrollBar)
{
	...
	if (GetTextArea().GetViewLeftCol() == nPos) { return CLayoutInt(0); }
	/* 水平スクロール量（文字数）の算出 */
...
void CEditView::ScrollDraw(CLayoutInt nScrollRowNum, CLayoutInt nScrollColNum, const RECT &rcScroll, const RECT &rcClip,
						   const RECT &rcClip2)
{
	...
	if (bBackImgFixed) {
		...
		if (m_hbmpCompatBMP) {
			...
			::BitBlt(m_hdcCompatDC, rcScroll.left + nScrollColPxWidth,
					 ...
					 rcScroll.right - rcScroll.left, rcScroll.bottom - rcScroll.top, m_hdcCompatDC, rcScroll.left,
					 rcScroll.top, SRCCOPY);
		}
	...
	if (nScrollColNum != 0) { InvalidateRect(&rcClip2, FALSE); }
}
