...
bool CEditView::GetCurrentTextForSearchDlg(CNativeW &cmemCurText, bool bGetHistory)
{
    ...
    { // テキストが選択されていない
        ...
        if (bGet)
        {
            if (0 < GetDllShareData().m_sSearchKeywords.m_aSearchKeys.size() && m_nCurSearchKeySequence < GetDllShareData().m_Common.m_sSearch.m_nSearchKeySequence)
            {
    ...
    *pnSearchStart = nPos; // 2002.02.08 hor
    ...
    { // 単語検索
        ...
        for (const wchar_t *p = m_strCurSearchKey.data(); p < searchKeyEnd;)
        {
            ...
            if (CWordParse::WhereCurrentWord_2(p, CLogicInt(searchKeyEnd - p), CLogicInt(0), &begin, &end, NULL, NULL) && begin == 0 && begin < end)
            {
