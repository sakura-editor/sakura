...
)
{
    ...
    if (nPaintFlag & PAINT_LINENUMBER)
        rcs.push_back(rcLineNumber);
    if (nPaintFlag & PAINT_RULER)
        rcs.push_back(rcRuler);
    if (nPaintFlag & PAINT_BODY)
        rcs.push_back(rcBody);
    if (rcs.size() == 0)
        return;
    CMyRect rc = rcs[0];
...
void CEditView::OnPaint2(HDC _hdc, PAINTSTRUCT *pPs, BOOL bDrawFromComptibleBmp)
{
    ...
    if (m_hdcCompatDC && NULL == m_hbmpCompatBMP || m_nCompatBMPWidth < (pPs->rcPaint.right - pPs->rcPaint.left) || m_nCompatBMPHeight < (pPs->rcPaint.bottom - pPs->rcPaint.top))
    {
        RECT rect;
    ...
    // -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
    CLayoutInt nLayoutLineTo = GetTextArea().GetViewTopLine() + CLayoutInt((pPs->rcPaint.bottom - GetTextArea().GetAreaTop() + (nLineHeight - 1)) / nLineHeight) - 1; // 2007.02.17 ryoji 計算を精密化
...
bool CEditView::DrawLayoutLine(SColorStrategyInfo *pInfo)
{
    ...
    CTypeSupport &cBackType = (cCaretLineBg.IsDisp() &&
                                   ...
                                   : cEvenLineBg.IsDisp() && pInfo->m_pDispPos->GetLayoutLineRef() % 2 == 1 && !m_bMiniMap
                                         ? cEvenLineBg
                                         : (cPageViewBg.IsDisp() && m_bMiniMap && cActiveView.GetTextArea().GetViewTopLine() <= pInfo->m_pDispPos->GetLayoutLineRef() && pInfo->m_pDispPos->GetLayoutLineRef() < cActiveView.GetTextArea().GetBottomLine())
                                               ? cPageViewBg
                                               : cTextType);
    bool bTransText         = IsBkBitmap();
    if (bTransText)
    ...
    if (pcLayout && GetSelectionInfo().IsTextSelected())
    {
        DispTextSelected(
            ...
            pcLayout->CalcLayoutWidth(m_pcEditDoc->m_cLayoutMgr) + CLayoutInt(pcLayout->GetLayoutEol().GetLen()
	...
	MYTRACE( L"CEditView::CreateOrUpdateCompatibleBitmap( %d, %d ): resized\n", cx, cy );
        ...
        if (m_hbmpCompatBMP)
        {
            ...
            if (hBitmapNew)
            {
                ...
                // 前の画面内容をコピーする
                ::BitBlt(hdcTemp, 0, 0, t_min(nBmpWidthNew, m_nCompatBMPWidth), t_min(nBmpHeightNew, m_nCompatBMPHeight), m_hdcCompatDC, 0, 0, SRCCOPY);
