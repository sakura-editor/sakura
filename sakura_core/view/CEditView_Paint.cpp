...
void CEditView_Paint::Call_OnPaint(int  nPaintFlag,  //!< 描画する領域を選択する
...
)
{
	...
	//各要素
	CMyRect rcLineNumber(0, pView->GetTextArea().GetAreaTop(), pView->GetTextArea().GetAreaLeft(),
						 pView->GetTextArea().GetAreaBottom());
	CMyRect rcRuler(pView->GetTextArea().GetAreaLeft(), 0, pView->GetTextArea().GetAreaRight(),
					pView->GetTextArea().GetAreaTop());
	CMyRect rcBody(pView->GetTextArea().GetAreaLeft(), pView->GetTextArea().GetAreaTop(),
				   pView->GetTextArea().GetAreaRight(), pView->GetTextArea().GetAreaBottom());
	...
	for (int i = 1; i < nSize; i++) rc = MergeRect(rc, rcs[i]);
...
void CEditView::RedrawLines(CLayoutYInt top, CLayoutYInt bottom)
{
	...
	if (bottom < GetTextArea().GetViewTopLine()) { return; }
	if (GetTextArea().GetBottomLine() <= top) { return; }
	HDC			hdc;
	PAINTSTRUCT ps;
...
void MyFillRect(HDC hdc, RECT &re) { ::ExtTextOut(hdc, re.left, re.top, ETO_OPAQUE | ETO_CLIPPED, &re, L"", 0, NULL); }
...
//	テスト背景パターン
	...
	case BGIMAGE_CENTER_LEFT: rcImagePos.left = area.GetAreaLeft(); break;
	case BGIMAGE_TOP_RIGHT:
	case BGIMAGE_BOTTOM_RIGHT:
	case BGIMAGE_CENTER_RIGHT: rcImagePos.left = area.GetAreaRight() - doc.m_nBackImgWidth; break;
	...
	case BGIMAGE_TOP_LEFT:
	case BGIMAGE_TOP_RIGHT:
	case BGIMAGE_TOP_CENTER: rcImagePos.top = area.GetAreaTop(); break;
	case BGIMAGE_BOTTOM_LEFT:
	case BGIMAGE_BOTTOM_RIGHT:
	case BGIMAGE_BOTTOM_CENTER: rcImagePos.top = area.GetAreaBottom() - doc.m_nBackImgHeight; break;
	...
	for (; rcImagePos.top <= nYEnd;)
	{
		for (; rcImagePos.left <= nXEnd;)
		{
			...
			if (::IntersectRect(&rcBlt, &rc, &rcImagePos))
			{
				...
				::AlphaBlend(hdc, rcBlt.left, rcBlt.top, width, height, hdcBgImg, rcBlt.left - rcImagePos.left,
							 rcBlt.top - rcImagePos.top, width, height, bf);
			...
			if (!typeConfig.m_backImgRepeatX) { break; }
		}
		...
		if (!typeConfig.m_backImgRepeatY) { break; }
	}
	if (rcBltAll.left != INT_MAX)
	{
		...
		if (y1 < y2)
		{
			rcFill.SetLTRB(x1, y1, x4, y2);
			MyFillRect(hdc, rcFill);
		}
		if (x1 < x2)
		{
			rcFill.SetLTRB(x1, y2, x2, y3);
			MyFillRect(hdc, rcFill);
		}
		if (x3 < x4)
		{
			rcFill.SetLTRB(x3, y2, x4, y3);
			MyFillRect(hdc, rcFill);
		}
		if (y3 < y4)
		{
			rcFill.SetLTRB(x1, y3, x4, y4);
			MyFillRect(hdc, rcFill);
		}
	}
	::SetBkColor(hdc, colorOld);
...
/*! 指定位置のColorIndexの取得
	...
	修正があった場合は、ここも修正が必要。
*/
CColor3Setting CEditView::GetColorIndex(const CLayout *pcLayout, CLayoutYInt nLineNum, int nIndex,
...
)
{
	...
	{
		if (bPrev && pInfo->m_nPosInLogic == nPosTo) break;

		//色切替
		pInfo->CheckChangeColor(cLineStr);

		// 1文字進む
		pInfo->m_nPosInLogic += CNativeW::GetSizeOfChar(cLineStr.GetPtr(), cLineStr.GetLength(), pInfo->m_nPosInLogic);
...
*/
void CEditView::SetCurrentColor(CGraphics &gr, EColorIndexType eColorIndex, EColorIndexType eColorIndex2,
...
{
	if (info.m_sColorAttr.m_cTEXT != info.m_sColorAttr.m_cBACK) { return info.m_sColorAttr.m_cTEXT; }
	// 反転表示
	if (info.m_sColorAttr.m_cBACK == m_crBack) { return info2.m_sColorAttr.m_cTEXT ^ 0x00FFFFFF; }
...
{
	if (info.m_sColorAttr.m_cTEXT != info.m_sColorAttr.m_cBACK) { return info.m_sColorAttr.m_cBACK; }
	// 反転表示
	if (info.m_sColorAttr.m_cBACK == m_crBack) { return info2.m_sColorAttr.m_cBACK ^ 0x00FFFFFF; }
	int alpha = 255 * 30 / 100; // 30%
...
{
	if (m_pcEditWnd->m_pPrintPreview) { return; }
	bool bChangeFont = m_bMiniMap;
	if (bChangeFont) { SelectCharWidthCache(CWM_FONT_MINIMAP, CWM_CACHE_LOCAL); }
	OnPaint2(_hdc, pPs, bDrawFromComptibleBmp);
	if (bChangeFont) { SelectCharWidthCache(CWM_FONT_EDIT, m_pcEditWnd->GetLogfontCacheMode()); }
}
...
void CEditView::OnPaint2(HDC _hdc, PAINTSTRUCT *pPs, BOOL bDrawFromComptibleBmp)
{
	...
	// 互換BMPからの転送のみによる作画
	if (bDrawFromComptibleBmp && m_hdcCompatDC && m_hbmpCompatBMP)
	{
		::BitBlt(gr, pPs->rcPaint.left, pPs->rcPaint.top, pPs->rcPaint.right - pPs->rcPaint.left,
				 pPs->rcPaint.bottom - pPs->rcPaint.top, m_hdcCompatDC, pPs->rcPaint.left, pPs->rcPaint.top, SRCCOPY);
	...
	if (m_hdcCompatDC && NULL == m_hbmpCompatBMP || m_nCompatBMPWidth < (pPs->rcPaint.right - pPs->rcPaint.left)
	...
	{
		nLayoutLine = GetTextArea().GetViewTopLine()
					  + CLayoutInt((nTop - GetTextArea().GetAreaTop()) / nLineHeight); //ビュー途中から描画
	}
	...
	// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
	CLayoutInt nLayoutLineTo =
		GetTextArea().GetViewTopLine()
		+ CLayoutInt((pPs->rcPaint.bottom - GetTextArea().GetAreaTop() + (nLineHeight - 1)) / nLineHeight)
		- 1; // 2007.02.17 ryoji 計算を精密化
	...
	sPos.InitDrawPos(
		CMyPoint(GetTextArea().GetAreaLeft() - (Int)GetTextArea().GetViewLeftCol() * nCharDx,
	...
	if (pPs->rcPaint.right <= GetTextArea().GetAreaLeft())
	{
		...
		{
			if (!sPos.GetLayoutRef()) break;

			// 1行描画（行番号のみ）
			GetTextDrawer().DispLineNumber(gr, sPos.GetLayoutLineRef(), sPos.GetDrawPos().y);
			//行を進める
	...
	else
	{
		while (sPos.GetLayoutLineRef() <= nLayoutLineTo)
		{
			...
			// 1行描画
			bool bDispResult = DrawLogicLine(gr, &sPos, nLayoutLineTo);
			...
			if (bDispResult)
			{
				...
				if (bClipping) { gr.SetClipping(pPs->rcPaint); }
				if (bTransText)
				{
					HDC		hdcBgImg = CreateCompatibleDC(gr);
					HBITMAP hOldBmp  = (HBITMAP)::SelectObject(hdcBgImg, m_pcEditDoc->m_hBackImg);
	...
	//                       ルーラー描画                          //
	// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
	if (pPs->rcPaint.top < GetTextArea().GetRulerHeight())
	{ // ルーラーが再描画範囲にあるときのみ再描画する 2002.02.25 Add By KK
	...
	if (bUseMemoryDC)
	{
		...
		::BitBlt(hdcOld, pPs->rcPaint.left, pPs->rcPaint.top, pPs->rcPaint.right - pPs->rcPaint.left,
				 pPs->rcPaint.bottom - pPs->rcPaint.top, gr, pPs->rcPaint.left, pPs->rcPaint.top, SRCCOPY);
	}
...
*/
bool CEditView::DrawLogicLine(HDC		 _hdc,		//!< [in]     作画対象
...
)
{
	...
	for (;;)
	{
		...
		if (nLineTo < pInfo->m_pDispPos->GetLayoutLineRef()) { break; }
		...
		// ロジック行を描画し終わったら抜ける
		if (pInfo->m_pDispPos->GetLayoutRef()->GetLogicLineNo() != nOldLogicLineNo) { break; }

		// nLineToを超えたら抜ける
		if (pInfo->m_pDispPos->GetLayoutLineRef() >= nLineTo + CLayoutInt(1)) { break; }
	}
...
bool CEditView::DrawLayoutLine(SColorStrategyInfo *pInfo)
{
	...
	const CLayout *pcLayout =
		pInfo->m_pDispPos
			->GetLayoutRef(); // m_pcEditDoc->m_cLayoutMgr.SearchLineByLayoutY( pInfo->pDispPos->GetLayoutLineRef() );

	// レイアウト情報
	if (pcLayout) { pInfo->m_pLineOfLogic = pcLayout->GetDocLineRef()->GetPtr(); }
	else
	{
	...
	if (pInfo->m_pDispPos->GetDrawPos().y < GetTextArea().GetAreaTop())
	{
		if (pcLayout)
		{
			...
			while (pInfo->m_nPosInLogic < nPosTo)
			{
				...
				// 1文字進む
				pInfo->m_nPosInLogic +=
					CNativeW::GetSizeOfChar(cLineStr.GetPtr(), cLineStr.GetLength(), pInfo->m_nPosInLogic);
			}
			if (bChange)
			{
				pInfo->DoChangeColor(&cColor);
	...
	CEditView &   cActiveView = m_pcEditWnd->GetActiveView();
	CTypeSupport &cBackType =
		(cCaretLineBg.IsDisp() && GetCaret().GetCaretLayoutPos().GetY() == pInfo->m_pDispPos->GetLayoutLineRef()
				 && !m_bMiniMap
			 ? cCaretLineBg
			 : cEvenLineBg.IsDisp() && pInfo->m_pDispPos->GetLayoutLineRef() % 2 == 1 && !m_bMiniMap
				   ...
				   : (cPageViewBg.IsDisp() && m_bMiniMap
					  ...
					  && pInfo->m_pDispPos->GetLayoutLineRef() < cActiveView.GetTextArea().GetBottomLine())
						 ...
						 : cTextType);
	bool bTransText = IsBkBitmap();
	if (bTransText) { bTransText = cBackType.GetBackColor() == cTextType.GetBackColor(); }

	// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
	//                        行番号描画                           //
	// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
	GetTextDrawer().DispLineNumber(pInfo->m_gr, pInfo->m_pDispPos->GetLayoutLineRef(),
	...
	if (pcLayout && pcLayout->GetIndent() != 0)
	{
		...
		{ cBackType.FillBack(pInfo->m_gr, rcClip); } //描画位置進める
	...
	if (pcLayout)
	{
		...
		if (NULL == pcLayoutNext) { bSkipRight = true; }
		else if (pcLayoutNext->GetLogicOffset() == 0)
		{
			bSkipRight = true; // 次の行は別のロジック行なのでスキップ可能
		}
		if (!bSkipRight) { bSkipRight = CColorStrategyPool::getInstance()->IsSkipBeforeLayout(); }
	}
	//行終端または折り返しに達するまでループ
	if (pcLayout)
	{
	...
	{ _DispWrap(pInfo->m_gr, pInfo->m_pDispPos, this, pInfo->m_pDispPos->GetLayoutLineRef()); } // 行末背景描画
	RECT rcClip;
	bool rcClipRet = GetTextArea().GenerateClipRectRight(&rcClip, *pInfo->m_pDispPos);
	if (rcClipRet)
	{
		if (!bTransText) { cBackType.FillBack(pInfo->m_gr, rcClip); }
		...
		if (GetSelectionInfo().IsTextSelected() && cSelectType.IsDisp())
		{
			...
			CLayoutRange selectArea =
				GetSelectionInfo().GetSelectAreaLine(pInfo->m_pDispPos->GetLayoutLineRef(), pcLayout);
			// 2010.10.04 スクロール分の足し忘れ
			CPixelXInt nSelectFromPx =
				GetTextMetrics().GetCharPxWidth(selectArea.GetFrom().x - GetTextArea().GetViewLeftCol());
			CPixelXInt nSelectToPx =
				GetTextMetrics().GetCharPxWidth(selectArea.GetTo().x - GetTextArea().GetViewLeftCol());
			if (nSelectFromPx < nSelectToPx && selectArea.GetTo().x != INT_MAX)
			{
				...
				if (::IntersectRect(&rcDraw, &rcClip, &rcSelect))
				{
					...
					if (color != cBackType.GetBackColor()) { pInfo->m_gr.FillSolidMyRect(rcDraw, color); }
				}
			}
		}
	}
	...
	if (!m_bMiniMap)
	{
		GetTextDrawer().DispNoteLine(pInfo->m_gr, pInfo->m_pDispPos->GetDrawPos().y,
									 pInfo->m_pDispPos->GetDrawPos().y + nLineHeight, GetTextArea().GetAreaLeft(),
									 GetTextArea().GetAreaRight());
	}

	// 指定桁縦線描画
	GetTextDrawer().DispVerticalLines(pInfo->m_gr, pInfo->m_pDispPos->GetDrawPos().y,
									  pInfo->m_pDispPos->GetDrawPos().y + nLineHeight, CLayoutInt(0), CLayoutInt(-1));

	// 折り返し桁縦線描画
	if (!m_bMiniMap)
	{
		GetTextDrawer().DispWrapLine(pInfo->m_gr, pInfo->m_pDispPos->GetDrawPos().y,
	...
	{
		DispTextSelected(
			pInfo->m_gr, pInfo->m_pDispPos->GetLayoutLineRef(),
					  ...
					  pcLayout->GetLayoutEol().GetLen()
						  ? (CTypeSupport(this, COLORIDX_EOL).IsDisp()
								 ? (GetTextMetrics().GetLayoutXDefault()
									+ CLayoutXInt(4)) // HACK:EOLの描画幅分だけ確保する。4pxはCRLFのはみ出している分
...
*/
void CEditView::DispTextSelected(HDC		hdc,	  //!< 作画対象ビットマップを含むデバイス
...
)
{
	...
	if (nLineNum >= sSelect.GetFrom().y && nLineNum <= sSelect.GetTo().y)
	{
		...
		if (nSelectFrom == INT_MAX) { nSelectFrom = nX_Layout; }
		if (nSelectTo == INT_MAX) { nSelectTo = nX_Layout; }

		// 2006.03.28 Moca 表示域外なら何もしない
		if (GetTextArea().GetRightCol() < nSelectFrom) { return; }
		if (nSelectTo < GetTextArea().GetViewLeftCol())
		{ // nSelectTo ==
		  // GetTextArea().GetViewLeftCol()のケースは後で０文字マッチでないことを確認してから抜ける
			return;
		}

		if (nSelectFrom < GetTextArea().GetViewLeftCol()) { nSelectFrom = GetTextArea().GetViewLeftCol(); }
		...
		if (GetSelectionInfo().IsTextSelected() && rcClip.right == rcClip.left && sSelect.IsLineOne()
		...
		if (rcClip.right > GetTextArea().GetAreaRight()) { rcClip.right = GetTextArea().GetAreaRight(); }

		// 選択色表示なら反転しない
		if (!bOMatch && CTypeSupport(this, COLORIDX_SELECT).IsDisp()) { return; }
...
{
	if (NULL == m_hdcCompatDC) { return false; }
	// サイズを64の倍数で整列
	...
	MYTRACE( L"CEditView::CreateOrUpdateCompatibleBitmap( %d, %d ): resized\n", cx, cy );
		...
		if (m_hbmpCompatBMP)
		{
			...
			if (hBitmapNew)
			{
				...
				// 前の画面内容をコピーする
				::BitBlt(hdcTemp, 0, 0, t_min(nBmpWidthNew, m_nCompatBMPWidth),
						 t_min(nBmpHeightNew, m_nCompatBMPHeight), m_hdcCompatDC, 0, 0, SRCCOPY);
