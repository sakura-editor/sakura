...
struct SColorStrategyInfo
{
	...
	//! 現在のスキャン位置
	CLogicInt		GetPosInLogic() const { return m_nPosInLogic; }
	const CDocLine *GetDocLine() const
	{
		const CLayout *layout = m_pDispPos->GetLayoutRef();

		if (layout) { return layout->GetDocLineRef(); }
		return NULL;
	}
...
public:
	...
	{
		if (strategy) { return strategy->GetStrategyColor(); }
		return COLORIDX_TEXT;
	}
	static CLayoutColorInfo *GetStrategyColorInfoSafe(const CColorStrategy *strategy)
	{
		if (strategy) { return strategy->GetStrategyColorInfo(); }
		return NULL;
	}
