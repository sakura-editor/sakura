...
bool CColor_BlockComment::BeginColor(const CStringRef& cStr, int nPos)
{
	...
	// ブロックコメント
	if( m_pTypeData->m_ColorInfoArr[COLORIDX_COMMENT].m_bDisp &&
		m_pcBlockComments->Match_CommentFrom( nPos, cStr )	//@@@ 2002.09.22 YAZAKI
	){
		/* この物理行にブロックコメントの終端があるか */	//@@@ 2002.09.22 YAZAKI
		this->m_nCOMMENTEND = m_pcBlockComments->Match_CommentTo(
			nPos + m_pcBlockComments->getBlockFromLen(),
			cStr
		);
