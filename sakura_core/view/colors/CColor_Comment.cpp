...
{
	//文字列終端
	if (nPos >= cStr.GetLength()) { return true; }

	//改行
	if (WCODE::IsLineDelimiter(cStr.At(nPos), GetDllShareData().m_Common.m_sEdit.m_bEnableExtEol)) { return true; }
...
bool CColor_BlockComment::BeginColor(const CStringRef &cStr, int nPos)
{
	...
	if (m_pcBlockComment->Match_CommentFrom(nPos, cStr) //@@@ 2002.09.22 YAZAKI
		...
		this->m_nCOMMENTEND = m_pcBlockComment->Match_CommentTo(nPos + m_pcBlockComment->getBlockFromLen(), cStr);
