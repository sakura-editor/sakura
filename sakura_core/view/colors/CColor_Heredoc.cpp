...
bool CColor_Heredoc::BeginColor(const CStringRef &cStr, int nPos)
{
    ...
    // ...
    // HEREDOC_ID
    if (m_pTypeData->m_nHeredocType == HEREDOC_PHP && cStr.At(nPos) == '<' && nPos + 3 < cStr.GetLength() && wmemcmp(cStr.GetPtr() + nPos + 1, L"<<", 2) == 0)
    {
...
bool CColor_Heredoc::EndColor(const CStringRef &cStr, int nPos)
{
    ...
    {
        if (m_pTypeData->m_nHeredocType == HEREDOC_PHP && nPos == 0 && m_nSize <= cStr.GetLength() && wmemcmp(cStr.GetPtr(), m_pszId, m_nSize) == 0)
        {
