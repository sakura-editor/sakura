...
bool CColor_Heredoc::EndColor(std::wstring_view cStr, int nPos)
{
	if (!m_nCOMMENTEND) {
		if( m_pTypeData->m_nHeredocType == HEREDOC_PHP
		 && nPos == 0 && m_nSize <= cStr.length()
		 && wmemcmp(cStr.data(), m_pszId, m_nSize) == 0 ){
			if( m_nSize== cStr.length() ){
				m_nCOMMENTEND = m_nSize;
				return false;
			}else{
				size_t i = m_nSize;
				if( i + 1 < cStr.length() && cStr[i] == L';' && WCODE::IsLineDelimiter(cStr[i+1], GetDllShareData().m_Common.m_sEdit.m_bEnableExtEol) ){
					// ID;
					m_nCOMMENTEND = i;
					return false;
				}else if( m_nSize < cStr.length() && WCODE::IsLineDelimiter(cStr[m_nSize], GetDllShareData().m_Common.m_sEdit.m_bEnableExtEol) ){
					// ID
					m_nCOMMENTEND = m_nSize;
					return false;
				}
