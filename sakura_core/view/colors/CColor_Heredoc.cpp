...
bool CColor_Heredoc::BeginColor(const CStringRef &cStr, int nPos)
{
	...
	// ...
	// HEREDOC_ID
	if (m_pTypeData->m_nHeredocType == HEREDOC_PHP && cStr.At(nPos) == '<' && nPos + 3 < cStr.GetLength()
		...
		for (; nPosIdStart < length; nPosIdStart++) {
			if (cStr.At(nPosIdStart) != L'\t' && cStr.At(nPosIdStart) != L' ') { break; }
		}
		wchar_t quote = L'\0';
		if (!(nPosIdStart < length)) { return false; }
		...
		int i = nPosIdStart;
		for (; i < length; i++) {
			if (!(WCODE::IsAZ(cStr.At(i)) || WCODE::Is09(cStr.At(i)) || cStr.At(i) == L'_')) { break; }
		}
...
bool CColor_Heredoc::EndColor(const CStringRef &cStr, int nPos)
{
	if (0 == this->m_nCOMMENTEND) {
		if (m_pTypeData->m_nHeredocType == HEREDOC_PHP && nPos == 0 && m_nSize <= cStr.GetLength()
