...
bool SColorStrategyInfo::CheckChangeColor(const CStringRef &cLineStr)
{
	...
	//選択範囲色開始
	if (!m_pStrategySelect) {
		if (pcSelect->BeginColorEx(cLineStr, this->GetPosInLogic(), m_pDispPos->GetLayoutLineRef(),
								   this->GetLayout())) {
...
CColorStrategyPool::~CColorStrategyPool()
{
	...
	for (int i = 0; i < size; i++) { delete m_vStrategies[i]; }
	m_vStrategies.clear();
}

CColorStrategy *CColorStrategyPool::GetStrategyByColor(EColorIndexType eColor) const
{
	if (COLORIDX_SEARCH <= eColor && eColor <= COLORIDX_SEARCHTAIL) { return m_pcFoundStrategy; }
	int size = (int)m_vStrategiesDisp.size();
	for (int i = 0; i < size; i++) {
		if (m_vStrategiesDisp[i]->GetStrategyColor() == eColor) { return m_vStrategiesDisp[i]; }
	}
	return NULL;
}
...
void CColorStrategyPool::NotifyOnStartScanLogic()
{
	...
	for (int i = 0; i < size; i++) { GetStrategy(i)->OnStartScanLogic(); }
}

// 2005.11.20 Mocaコメントの色分けがON/OFF関係なく行われていたバグを修正
void CColorStrategyPool::CheckColorMODE(CColorStrategy **ppcColorStrategy, //!< [in,out]
										int nPos, const CStringRef &cLineStr)
{
	//色終了
	if (*ppcColorStrategy) {
		if ((*ppcColorStrategy)->EndColor(cLineStr, nPos)) { *ppcColorStrategy = NULL; }
	}
	...
	if (!*ppcColorStrategy) {
		...
		if (m_pcHeredoc && m_pcHeredoc->BeginColor(cLineStr, nPos)) {
			*ppcColorStrategy = m_pcHeredoc;
			return;
		}
		if (m_pcBlockComment1 && m_pcBlockComment1->BeginColor(cLineStr, nPos)) {
			*ppcColorStrategy = m_pcBlockComment1;
			return;
		}
		if (m_pcBlockComment2 && m_pcBlockComment2->BeginColor(cLineStr, nPos)) {
			*ppcColorStrategy = m_pcBlockComment2;
			return;
		}
		if (m_pcLineComment && m_pcLineComment->BeginColor(cLineStr, nPos)) {
			*ppcColorStrategy = m_pcLineComment;
			return;
		}
		if (m_pcSingleQuote && m_pcSingleQuote->BeginColor(cLineStr, nPos)) {
			*ppcColorStrategy = m_pcSingleQuote;
			return;
		}
		if (m_pcDoubleQuote && m_pcDoubleQuote->BeginColor(cLineStr, nPos)) {
			*ppcColorStrategy = m_pcDoubleQuote;
			return;
		}
	}
}
...
void CColorStrategyPool::OnChangeSetting(void)
{
	...
	for (int i = 0; i < size; i++) {
		...
		// 色分け表示対象であれば登録
		if (m_vStrategies[i]->Disp()) { m_vStrategiesDisp.push_back(m_vStrategies[i]); }
	}
	...
	m_pcSingleQuote =
		static_cast<CColor_SingleQuote *>(GetStrategyByColor(COLORIDX_SSTRING)); // シングルクォーテーション文字列
	m_pcDoubleQuote =
		static_cast<CColor_DoubleQuote *>(GetStrategyByColor(COLORIDX_WSTRING)); // ダブルクォーテーション文字列
	...
	for (int n = 0; n < _countof(bSkipColorTypeTable); n++) {
		if (COLORIDX_KEYWORD1 == bSkipColorTypeTable[n]) { nKeyword1 = n; }
	...
	if (m_bSkipBeforeLayoutGeneral) {
		if (type.m_bUseRegexKeyword) { m_bSkipBeforeLayoutGeneral = false; }
	}
...
{
	if (!m_bSkipBeforeLayoutGeneral) { return false; }
	if (!m_bSkipBeforeLayoutFound && m_pcView->m_bCurSrchKeyMark) { return false; }
	return true;
}
...
const SColorAttributeData g_ColorAttributeArr[] = {
	...
	{L"CAR",
	 COLOR_ATTRIB_FORCE_DISP | COLOR_ATTRIB_NO_BACK | COLOR_ATTRIB_NO_EFFECTS}, // キャレット		// 2006.12.07 ryoji
	...
	{L"CVL",
	 COLOR_ATTRIB_NO_BACK | (COLOR_ATTRIB_NO_EFFECTS & ~COLOR_ATTRIB_NO_BOLD)}, // 2007.09.09 Moca カーソル位置縦線
 ...
 * インデックス番号からカラー名に変換する
 */
const WCHAR *GetColorNameByIndex(int index) { return g_ColorAttributeArr[index].szName; }
