...
*/
static int NextWordBreak( const CStringRefW& str, const int start );
...
bool CColor_KeywordSet::BeginColor(const CStringRefW& cStr, int nPos)
{
	if( ! cStr.IsValid() ) {
...
bool CColor_KeywordSet::EndColor(const CStringRefW& cStr, int nPos)
{
	return nPos == this->m_nCOMMENTEND;
}

static inline int NextWordBreak( const CStringRefW& str, const int start )
{
