      ...
      public CDocListenerEx
{
  ...
  public:
    ...
    {
        return NULL != m_pcEditDoc->m_hBackImg && 0 != GetDocument()->m_cDocType.GetDocumentAttribute().m_backImgOpacity;
    }
  ...
  public:
    ...
    CLayoutInt ScrollByV(CLayoutInt vl)
    {
        return ScrollAtV(GetTextArea().GetViewTopLine() + vl);
    } /* 指定行スクロール*/
  ...
  public:
    ...
    bool SearchBracketForward(CLogicPoint ptPos, CLayoutPoint *pptLayoutNew, const wchar_t *upChar, const wchar_t *dnChar, int *mode); //	対括弧の前方検索	// modeの追加 02/09/19 ai
    bool SearchBracketBackward(CLogicPoint ptPos, CLayoutPoint *pptLayoutNew, const wchar_t *dnChar, const wchar_t *upChar, int *mode); //	対括弧の後方検索	// modeの追加 02/09/19 ai
  ...
  public:
    ...
    bool TagJumpSub(const WCHAR *pszJumpToFile, CMyPoint ptJumpTo, bool bClose = false, bool bRelFromIni = false, bool *pbJumpToSelf = NULL);
  ...
  public:
    ...
    // -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
  public:
    BOOL OPEN_ExtFromtoExt(BOOL bCheckOnly, BOOL bBeepWhenMiss, const WCHAR *file_ext[], const WCHAR *open_ext[], int file_extno, int open_extno, const WCHAR *errmes); // 指定拡張子のファイルに対応するファイルを開く補助関数 // 2003.08.12 Moca
  ...
  public:
    ...
    //主要属性アクセス
    CTextMetrics &GetTextMetrics()
    {
        return m_cTextMetrics;
    }
    const CTextMetrics &GetTextMetrics() const
    {
        return m_cTextMetrics;
    }
    CViewSelect &GetSelectionInfo()
    {
        return m_cViewSelect;
    }
    const CViewSelect &GetSelectionInfo() const
    {
        return m_cViewSelect;
    }
    ...
    //主要ヘルパアクセス
    const CViewParser &GetParser() const
    {
        return m_cParser;
    }
    const CTextDrawer &GetTextDrawer() const
    {
        return m_cTextDrawer;
    }
...
class COutputAdapter
{
  public:
    ...
    virtual bool IsEnableRunningDlg()
    {
        return true;
    }
    virtual bool IsActiveDebugWindow()
    {
        return true;
    }
};
