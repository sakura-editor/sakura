...
class CEditView
	: public CViewCalc //$$
					   //これが親クラスである必要は無いが、このクラスのメソッド呼び出しが多いので、暫定的に親クラスとする。
	,
...
public:
	CEditView *		 GetEditView() { return this; }
	const CEditView *GetEditView() const { return this; }
...
public:
	//取得
	bool MyGetClipboardData(CNativeW &cmemBuf, bool *pbColumnSelect,
							bool *pbLineSelect = NULL); /* クリップボードからデータを取得 */

	//設定
	bool MySetClipboardData(const WCHAR *pszText, int nTextLen, bool bColumnSelect,
							bool bLineSelect = false); /* クリップボードにデータを設定 */
...
protected:
	...
	//色分け
public:
	CColor3Setting GetColorIndex(const CLayout *pcLayout, CLayoutYInt nLineNum, int nIndex, SColorStrategyInfo *pInfo,
								 bool bPrev = false); /* 指定位置のColorIndexの取得 02/12/13 ai */
...
public:
	...
	void CaretUnderLineOFF(bool bDraw, bool bDrawPaint, bool bResetFlag,
						   bool DisalbeUnderLine); /* カーソル行アンダーラインのOFF */
...
public:
	...
	CLayoutInt ScrollByH(CLayoutInt hl)
	{
		return ScrollAtH(GetTextArea().GetViewLeftCol() + hl);
	} /* 指定桁スクロール */
	void ScrollDraw(CLayoutInt nScrollRowNum, CLayoutInt nScrollColNum, const RECT &rcScroll, const RECT &rcClip,
					const RECT &rcClip2);
...
public:
	...
	bool GetSelectedData(CNativeW *cmemBuf, BOOL bLineOnly, const wchar_t *pszQuote, BOOL bWithLineNumber,
						 bool bAddCRLFWhenCopy, EEolType neweol = EOL_UNKNOWN); /* 選択範囲のデータを取得 */
	int IsCurrentPositionSelected(CLayoutPoint ptCaretPos); /* 指定カーソル位置が選択エリア内にあるか */
	int IsCurrentPositionSelectedTEST(const CLayoutPoint &ptCaretPos,
									  const CLayoutRange &sSelect) const; /* 指定カーソル位置が選択エリア内にあるか */
	...
	bool IsEmptyArea(CLayoutPoint ptFrom, CLayoutPoint ptTo = CLayoutPoint(CLayoutInt(-1), CLayoutInt(-1)),
					 bool bSelect = false, bool bBoxSelect = false) const;

	// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
	//                         各種判定                            //
	// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
public:
	bool IsCurrentPositionURL(const CLayoutPoint &ptCaretPos, CLogicRange *pUrlRange,
							  std::wstring *pstrURL); /* カーソル位置にURLが有る場合のその範囲を調べる */
	BOOL CheckTripleClick(CMyPoint ptMouse); /* トリプルクリックをチェックする */ // 2007.10.02 nasukoji

	bool ExecCmd(const WCHAR *pszCmd, int nFlgOpt, const WCHAR *pszCurDir,
				 COutputAdapter *customOa = NULL); // 子プロセスの標準出力をリダイレクトする
	void AddToCmdArr(const WCHAR *szCmd);
	BOOL ChangeCurRegexp(
		bool bRedrawIfChanged =
			true); // 2002.01.16 hor
				   // 正規表現の検索パターンを必要に応じて更新する(ライブラリが使用できないときはFALSEを返す)
	void SendStatusMessage(
		const WCHAR *msg); // 2002.01.26 hor 検索／置換／ブックマーク検索時の状態をステータスバーに表示する
	LRESULT SetReconvertStruct(PRECONVERTSTRING pReconv, bool bUnicode,
							   bool bDocumentFeed = false); /* 再変換用構造体を設定する 2002.04.09 minfu */
	LRESULT SetSelectionFromReonvert(const PRECONVERTSTRING pReconv,
									 bool bUnicode); /* 再変換用構造体の情報を元に選択範囲を変更する 2002.04.09 minfu */
...
public:
	/* 指定位置の指定長データ削除 */
	void DeleteData2(const CLayoutPoint &ptCaretPos, CLogicInt nDelLen, CNativeW *pcMem);

	/* 現在位置のデータ削除 */
	void DeleteData(bool bRedraw);

	/* 現在位置にデータを挿入 */
	void InsertData_CEditView(CLayoutPoint ptInsertPos, const wchar_t *pData, int nDataLen,
	...
	bool ReplaceData_CEditView3(CLayoutRange  sDelRange,		  // 削除範囲。レイアウト単位。
								...
								COpeLineData *pInsData, bool bRedraw, COpeBlk *pcOpeBlk, int nDelSeq, int *pnInsSeq,
								bool bFastMode = false, const CLogicRange *psDelRangeLogicFast = NULL);
...
public:
	...
	void TranslateCommand_isearch(EFunctionCode &nCommand, bool &bRedraw, LPARAM &lparam1, LPARAM &lparam2,
								  LPARAM &lparam3, LPARAM &lparam4);
	bool ProcessCommand_isearch(int nCommand, bool bRedraw, LPARAM lparam1, LPARAM lparam2, LPARAM lparam3,
								LPARAM lparam4);

	//	Jan. 10, 2005 genta HandleCommandからgrep関連処理を分離
	void TranslateCommand_grep(EFunctionCode &nCommand, bool &bRedraw, LPARAM &param1, LPARAM &param2, LPARAM &param3,
							   LPARAM &param4);
...
public:
	//	Jun. 16, 2000 genta
	bool SearchBracket(const CLayoutPoint &ptPos, CLayoutPoint *pptLayoutNew,
					   int *mode); // 対括弧の検索		// modeの追加 02/09/18 ai
	bool SearchBracketForward(CLogicPoint ptPos, CLayoutPoint *pptLayoutNew, const wchar_t *upChar,
							  const wchar_t *dnChar, int *mode); //	対括弧の前方検索	// modeの追加 02/09/19 ai
	bool SearchBracketBackward(CLogicPoint ptPos, CLayoutPoint *pptLayoutNew, const wchar_t *dnChar,
							   const wchar_t *upChar, int *mode); //	対括弧の後方検索	// modeの追加 02/09/19 ai
...
public:
	...
	int HokanSearchByFile(const wchar_t *pszKey, bool bHokanLoHiCase, vector_ex<std::wstring> &vKouho,
						  int nMaxKouho); // 2003.06.25 Moca
...
public:
	...
	bool TagJumpSub(const WCHAR *pszJumpToFile, CMyPoint ptJumpTo, bool bClose = false, bool bRelFromIni = false,
...
public:
	...
	CLogicPoint			 m_ptSrchStartPos_PHY; // 検索/置換開始時のカーソル位置 (改行単位行先頭からのバイト数(0開始),
									  // 改行単位行の行番号(0開始))
