...
*/
void CEditView::ViewDiffInfo(const WCHAR *pszFile1, const WCHAR *pszFile2, int nFlgOpt, bool bUTF8)
/*
...
*/
{
	...
	//今あるDIFF差分を消去する。
	if (CDiffManager::getInstance()->IsDiffUse()) GetCommander().Command_Diff_Reset();
...

	{
		...
		auto_sprintf(cmdline, L"\"%s\\%s\" %s \"%s\" \"%s\"",
					 szExeFolder, // sakura.exeパス
					 L"diff.exe", // diff.exe
					 szOption,	// diffオプション
					 (nFlgFile12 ? pszFile2 : pszFile1), (nFlgFile12 ? pszFile1 : pszFile2));
	}
	...
	if (nFlgOpt & 0x0040)
	{
		if (!CDiffManager::getInstance()->IsDiffUse()) { InfoMessage(this->GetHwnd(), LS(STR_ERR_DLGEDITVWDIFF5)); }
	}
...
/*!	DIFF差分情報を解析しマーク登録
	...
	@date	2002/05/25
*/
void CEditView::AnalyzeDiffInfo(const char *pszDiffInfo, int nFlgFile12)
{
	/*
	...
	{
		if (mode == 'a')
			CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr).SetDiffMarkRange(MARK_DIFF_DELETE, CLogicInt(s1), CLogicInt(e1));
		else if (mode == 'c')
			CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr)
				.SetDiffMarkRange(MARK_DIFF_CHANGE, CLogicInt(s1 - 1), CLogicInt(e1 - 1));
		else if (mode == 'd')
			CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr)
				.SetDiffMarkRange(MARK_DIFF_APPEND, CLogicInt(s1 - 1), CLogicInt(e1 - 1));
	}
	else //編集中ファイルは新ファイル
	{
		if (mode == 'a')
			CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr)
				.SetDiffMarkRange(MARK_DIFF_APPEND, CLogicInt(s2 - 1), CLogicInt(e2 - 1));
		else if (mode == 'c')
			CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr)
				.SetDiffMarkRange(MARK_DIFF_CHANGE, CLogicInt(s2 - 1), CLogicInt(e2 - 1));
		else if (mode == 'd')
			CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr).SetDiffMarkRange(MARK_DIFF_DELETE, CLogicInt(s2), CLogicInt(e2));
	}
...
BOOL CEditView::MakeDiffTmpFile(WCHAR *filename, HWND hWnd, ECodeType code, bool bBom)
{
	...
	if (NULL == hWnd)
	{
		EConvertResult eWriteResult = CWriteManager().WriteFile_From_CDocLineMgr(
			m_pcEditDoc->m_cDocLineMgr, SSaveInfo(filename, code, EOL_NONE, bBom));
	...
	try
	{
		if (!MakeDiffTmpFile_core(out, hWnd, *this, bBom)) { bError = true; }
	}
	catch (...)
	{
		bError = true;
...
BOOL CEditView::MakeDiffTmpFile2(WCHAR *tmpName, const WCHAR *orgName, ECodeType code, ECodeType saveCode)
{
	...
	{ return FALSE; } CFileLoad cfl(typeMini->m_encoding);
