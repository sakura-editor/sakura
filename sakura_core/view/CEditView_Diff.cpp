...
*/
{
	...
	wcsncpy( szOption, L"-" );
	if( nFlgOpt & 0x0001 ) wcsncat( szOption, L"i" );	//-i ignore-case         大文字小文字同一視
	if( nFlgOpt & 0x0002 ) wcsncat( szOption, L"w" );	//-w ignore-all-space    空白無視
	if( nFlgOpt & 0x0004 ) wcsncat( szOption, L"b" );	//-b ignore-space-change 空白変更無視
	if( nFlgOpt & 0x0008 ) wcsncat( szOption, L"B" );	//-B ignore-blank-lines  空行無視
	if( nFlgOpt & 0x0010 ) wcsncat( szOption, L"t" );	//-t expand-tabs         TAB-SPACE変換
	if( wcsncmp( szOption, L"-" ) == 0 ) szOption[0] = L'\0';	//オプションなし
...
bool COutputAdapterDiff::OutputA(const ACHAR* pBuf, int size)
{
	...
	{
		bFirst = false;
		if( strncmp( pBuf, "Binary files " ) == 0 )
		{
