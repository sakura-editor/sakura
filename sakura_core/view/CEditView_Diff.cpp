...
class COutputAdapterDiff final : public COutputAdapter
{
  public:
    ...
    bool IsEnableRunningDlg() override
    {
        return false;
    }
    bool IsActiveDebugWindow() override
    {
        return false;
    }
...
*/
{
    ...
    wcscpy(szOption, L"-");
    if (nFlgOpt & 0x0001)
        wcscat(szOption, L"i"); //-i ignore-case         大文字小文字同一視
    if (nFlgOpt & 0x0002)
        wcscat(szOption, L"w"); //-w ignore-all-space    空白無視
    if (nFlgOpt & 0x0004)
        wcscat(szOption, L"b"); //-b ignore-space-change 空白変更無視
    if (nFlgOpt & 0x0008)
        wcscat(szOption, L"B"); //-B ignore-blank-lines  空行無視
    if (nFlgOpt & 0x0010)
        wcscat(szOption, L"t"); //-t expand-tabs         TAB-SPACE変換
    if (wcscmp(szOption, L"-") == 0)
        szOption[0] = L'\0'; //オプションなし
    if (nFlgOpt & 0x0020)
        nFlgFile12 = 0;
    else
        nFlgFile12 = 1;
    ...
    int nFlgFile12)
{
    ...
    if (0 == nFlgFile12) //編集中ファイルは旧ファイル
    {
        if (mode == 'a')
            ...
            CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr).SetDiffMarkRange(MARK_DIFF_CHANGE, CLogicInt(s1 - 1), CLogicInt(e1 - 1));
        else if (mode == 'd')
            CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr).SetDiffMarkRange(MARK_DIFF_APPEND, CLogicInt(s1 - 1), CLogicInt(e1 - 1));
    }
    else //編集中ファイルは新ファイル
    {
        if (mode == 'a')
            CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr).SetDiffMarkRange(MARK_DIFF_APPEND, CLogicInt(s2 - 1), CLogicInt(e2 - 1));
        else if (mode == 'c')
            CDiffLineMgr(&m_pcEditDoc->m_cDocLineMgr).SetDiffMarkRange(MARK_DIFF_CHANGE, CLogicInt(s2 - 1), CLogicInt(e2 - 1));
        else if (mode == 'd')
