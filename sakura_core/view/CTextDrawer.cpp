...
const CTextArea *CTextDrawer::GetTextArea() const { return &m_pEditView->GetTextArea(); }
						   ...
						   bool bTransparent) const
{
	...
	if (pArea->IsRectIntersected(rcClip) && rcClip.top >= pArea->GetAreaTop())
	{
		...
		if (rcClip.Width() > pArea->GetAreaWidth()) { rcClip.right = rcClip.left + pArea->GetAreaWidth(); }
		...
		*/
		...
		if (nDrawLength < nDrawDataMaxLength && pDrawDxArray[nDrawLength] == 0) { nDrawLength++; }
									...
									) const
{
	...
	if (nRightCol < 0) { nRightCol = nWrapLayout; }
	const int nPosXOffset = GetDllShareData().m_Common.m_sWindow.m_nVertLineOffset + pView->GetTextArea().GetAreaLeft();
	const int nPosXLeft =
		t_max(pView->GetTextMetrics().GetCharPxWidth(pView->GetTextArea().GetAreaLeft() + (nLeftCol - nViewLeftCol)),
			  pView->GetTextArea().GetAreaLeft());
	const int nPosXRight =
	...
	for (k = 0; k < MAX_VERTLINES && typeData.m_nVertLineIdx[k] != 0; k++)
	{
		...
		{
			if (nWrapLayout < nXCol) { break; }
			int nPosX = nPosXOffset
						+ pView->GetTextMetrics().GetCharPxWidth(nXCol - pView->GetTextMetrics().GetLayoutXDefault()
																 - nViewLeftCol);
			// 2006.04.30 Moca 線の引く範囲・方法を変更
			// 太線の場合、半分だけ作画する可能性がある。
			int nPosXBold = nPosX;
			if (bBold) { nPosXBold -= 1; }
			if (nPosXRight <= nPosXBold) { break; }
			if (nPosXLeft <= nPosX)
			{
				if (bDot)
				{
							   ...
							   ) const
{
	...
	if (cNoteLine.IsDisp())
	{
		...
		while (offset < 0) { offset += nLineHeight; }
...
void CTextDrawer::DispLineNumber(CGraphics &gr, CLayoutInt nLineNum, int y) const
{
	...
	// 行がないとき・行の背景が透明のときの色
	CTypeSupport &cBackType = (cCaretLineBg.IsDisp() && pView->GetCaret().GetCaretLayoutPos().GetY() == nLineNum
								   ? cCaretLineBg
								   : cEvenLineBg.IsDisp() && nLineNum % 2 == 1 ? cEvenLineBg : cTextType);
	...
	if (pcLayout)
	{
		...
		if (pView->GetDocument()->m_cDocEditor.IsModified()
			&& CModifyVisitor().IsLineModified(pCDocLine,
											   pView->GetDocument()->m_cDocEditor.m_cOpeBuf.GetNoModifiedSeq()))
		{ /* 変更フラグ */
			if (CTypeSupport(pView, COLORIDX_GYOU_MOD).IsDisp())
			{									 // 2006.12.12 ryoji
	...
	if (pCDocLine)
	{
		...
		{
			if (CTypeSupport(pView, COLORIDX_MARK).IsDisp()) { nColorIndex = COLORIDX_MARK; }
		}
	}
	...
	else if (CTypeSupport(pView, COLORIDX_GYOU).IsDisp())
	{ /* 行番号表示／非表示 */
		...
		if (bGyouMod && nColorIndex != COLORIDX_GYOU_MOD)
		{
			...
			if (bChange) { sFont.m_hFont = pView->GetFontset().ChooseFontHandle(0, sFont.m_sFontAttr); }
		}
		...
		::ExtTextOut(gr, drawNumTop, y + nHeightMargin, ExtTextOutOption() & ~(bTrans ? ETO_OPAQUE : 0), &rcLineNum,
					 szLineNum, nLineCols, pView->GetTextMetrics().GetDxArray_AllHankaku());
	...
	if (pCDocLine)
	{
		...
		// DIFFマーク描画
		if (!pView->m_bMiniMap) { CDiffLineGetter(pCDocLine).DrawDiffMark(gr, y, nLineHeight, fgcolor); }
	}
