...
void CTextMetrics::Update(HDC hdc, HFONT hFont, int nLineSpace, int nColmSpace)
{
	...
	for (int i = 0; i < size; i++) {
		...
		if (i == 0 && GetHankakuWidth() < sz.cx) { SetHankakuWidth(sz.cx); }
	...
	for (int i = 0; i < size; i++) { m_aFontHeightMargin[i] = tmAscentMaxHeight - tmAscent[i] + minMargin; }
...
void CTextMetrics::SetHankakuWidth(int nHankakuWidth) { m_nCharWidth = nHankakuWidth; }

//! 半角文字の縦幅を設定。単位はピクセル。
void CTextMetrics::SetHankakuHeight(int nHankakuHeight) { m_nCharHeight = nHankakuHeight; }
...
void CTextMetrics::SetHankakuDy(int nDyBasis) { m_nDyBasis = nDyBasis; }
...
const int *CTextMetrics::GenerateDxArray(std::vector<int> *vResultArray, //!< [out] 文字間隔配列の受け取りコンテナ
...
//!文字列のピクセル幅を返す。
int CTextMetrics::CalcTextWidth(const wchar_t *pText,	//!< 文字列
								int			   nLength, //!< 文字列長
...
)
{
	...
	// UNICODE時代の動作
	int w = 0;
	for (int i = 0; i < nLength; i++) { w += pnDx[i]; }
	return w;
}

//!文字列のピクセル幅を返す。
int CTextMetrics::CalcTextWidth2(const wchar_t *pText,		 //!< 文字列
								 int			nLength,	 //!< 文字列長
...
)
{
	//文字間隔配列を生成
	vector<int> vDxArray;
	const int * pDxArray = CTextMetrics::GenerateDxArray(&vDxArray, pText, nLength, nHankakuDx, 8, 0, nCharSpacing);

	//ピクセル幅を計算
	return CalcTextWidth(pText, nLength, pDxArray);
}

int CTextMetrics::CalcTextWidth3(const wchar_t *pText,	//!< 文字列
								 int			nLength //!< 文字列長
) const
{
