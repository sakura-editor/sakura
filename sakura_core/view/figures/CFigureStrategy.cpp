...
bool CFigure_Text::DrawImp(SColorStrategyInfo *pInfo)
{
	...
	if (fontNo) {
		...
		bool		 blendColor = pInfo->GetCurrentColor() != pInfo->GetCurrentColor2()
						  && cCurrentType.GetTextColor() == cCurrentType.GetBackColor();
		SFONT sFont;
	...
	pInfo->m_pcView->GetTextDrawer().DispText(pInfo->m_gr, pInfo->m_pDispPos, nHeightMargin,
											  &pInfo->m_pLineOfLogic[nIdx], nLength, bTrans);
	if (fontNo) { pInfo->m_gr.PopMyFont(); }
...
bool CFigureSpace::DrawImp_StyleSelect(SColorStrategyInfo *pInfo)
{
	...
	CTypeSupport &cCurrentType1 =
		(cCurrentType.GetBackColor() == cTextType.GetBackColor() ? cCurrentTypeBg : cCurrentType);
