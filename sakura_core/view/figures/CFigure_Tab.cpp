...
{
	if (pText[0] == WCODE::TAB) { return true; }
	return false;
}
...
void CFigure_Tab::DispSpace(CGraphics &gr, DispPos *pDispPos, CEditView *pcView, bool bTrans) const
{
	...
	if (pArea->IsRectIntersected(rcClip2)) {
		if (cTabType.IsDisp() && TABARROW_STRING == m_pTypeData->m_bTabArrow) { //タブ通常表示	//@@@ 2003.03.26 MIK
			...
			//@@@ 2001.03.16 by MIK
			::ExtTextOut(gr, sPos.GetDrawPos().x, sPos.GetDrawPos().y + nHeightMargin,
						 ExtTextOutOption() & ~(bTrans ? ETO_OPAQUE : 0), &rcClip2, m_pTypeData->m_szTabViewString,
...
/*
	タブ矢印描画関数
*/
void _DrawTabArrow(CGraphics &gr,
				   int		  nPosX,   //ピクセルX
				   int		  nPosY,   //ピクセルY
				   int		  nWidth,  //ピクセルW
				   int		  nHeight, //ピクセルH
				   bool bBold, COLORREF pColor)
{
	// ペン設定
	gr.PushPen(pColor, 0);
