...
bool CFigure_Eol::DrawImp(SColorStrategyInfo *pInfo)
{
	...
	if (cEol.GetLen()) {
		...
		CTypeSupport &cCurrentType3 =
			(cCurrentType2.GetBackColor() == cTextType.GetBackColor() ? cCurrentTypeBg : cCurrentType2);
		COLORREF crText;
		COLORREF crBack;
...
void _DispWrap(CGraphics &gr, DispPos *pDispPos, const CEditView *pcView, CLayoutYInt nLineNum)
{
	...
	if (pcView->GetTextArea().GenerateClipRect(&rcClip2, *pDispPos, width)) {
		...
		::ExtTextOut(gr, pDispPos->GetDrawPos().x, pDispPos->GetDrawPos().y + nHeightMargin,
					 ExtTextOutOption() & ~(bTrans ? ETO_OPAQUE : 0), &rcClip2, szText, wcslen(szText), nDx);
		if (bChangeColor) { gr.PopTextBackColor(); }
	}
...
void _DrawEOL(CGraphics &gr, const CMyRect &rcEol, CEol cEol, bool bBold, COLORREF pColor);

// 2007.08.30 kobake 追加
void _DispEOL(CGraphics &gr, DispPos *pDispPos, CEol cEol, const CEditView *pcView, bool bTrans)
{
	const CLayoutXInt nCol =
		CTypeSupport(pcView, COLORIDX_EOL).IsDisp()
	...
	if (pcView->GetTextArea().GenerateClipRect(&rcClip2, *pDispPos, nCol)) {
		...
		::ExtTextOut(gr, pDispPos->GetDrawPos().x, pDispPos->GetDrawPos().y + nHeightMargin,
					 ExtTextOutOption() & ~(bTrans ? ETO_OPAQUE : 0), &rcClip2, L"  ", 2,
