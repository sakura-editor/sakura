...
void CFigure_HanSpace::DispSpace(CGraphics &gr, DispPos *pDispPos, CEditView *pcView, bool bTrans) const
{
	...
	if (pcView->GetTextArea().GenerateClipRect(&rcClip, *pDispPos, nCol)) {
		...
		::ExtTextOut(gr, pDispPos->GetDrawPos().x, pDispPos->GetDrawPos().y,
					 ExtTextOutOption() & ~(bTrans ? ETO_OPAQUE : 0), &rcClipBottom,
					 // FIXME:幅が違う
					 L"o", 1, &Dx);
		...
		::ExtTextOut(gr, pDispPos->GetDrawPos().x, pDispPos->GetDrawPos().y,
					 ExtTextOutOption() & ~(bTrans ? ETO_OPAQUE : 0), &rcClipTop, L" ", 1, &Dx);
	}
