        ...
        if (m_sSelect != m_sSelectOld)
        {
            ...
            else if (!IsBoxSelecting() && rcOld.top == rcNew.top && rcOld.bottom == rcNew.bottom)
            {
                if (m_sSelect.GetFrom() == m_sSelectOld.GetFrom() && m_sSelect.GetTo().x != m_sSelectOld.GetTo().x)
                {
                    ...
                    drawRight          = t_max(m_sSelect.GetTo().x, m_sSelectOld.GetTo().x) + pView->GetTextMetrics().GetLayoutXDefault() + 4; // 改行コード幅分余分に取る
                }
                else if (m_sSelect.GetTo() == m_sSelectOld.GetTo() && m_sSelect.GetFrom().x != m_sSelectOld.GetFrom().x)
                {
                    ...
                    drawRight          = t_max(m_sSelectOld.GetFrom().x, m_sSelect.GetFrom().x) + pView->GetTextMetrics().GetLayoutXDefault() + 4; // 改行コード幅分余分に取る
                }
                else
                {
                    rc.UnionStrictRect(rcOld, rcNew);
...
void CViewSelect::PrintSelectionInfoMsg() const
{
    ...
    else
    {
        ...
        BOOL bCountByByte       = (pView->m_pcEditWnd->m_nSelectCountMode == SELECT_COUNT_TOGGLE ? bCountByByteCommon : pView->m_pcEditWnd->m_nSelectCountMode == SELECT_COUNT_BY_BYTE);
        ...
        if (pcLayout)
        {
            ...
            else
            {
                ...
                {
                    select_sum =
                        pView->LineColumnToIndex(pcLayout, m_sSelect.GetTo().GetX2()) - pView->LineColumnToIndex(pcLayout, m_sSelect.GetFrom().GetX2());
                }
                else
                { //	2行以上選択されている場合
                    select_sum =
                        pcLayout->GetLengthWithoutEOL() + pcLayout->GetLayoutEol().GetLen() - pView->LineColumnToIndex(pcLayout, m_sSelect.GetFrom().GetX2());
...
#ifdef _DEBUG
        auto_sprintf(msg, L"%d %s (%d lines) selected. [%d:%d]-[%d:%d]", select_sum, (bCountByByte ? L"bytes" : L"chars"), select_line, m_sSelect.GetFrom().x, m_sSelect.GetFrom().y, m_sSelect.GetTo().x, m_sSelect.GetTo().y);
#else
