...
LRESULT CALLBACK EditViewWndProc(HWND   hwnd,   // handle of window
...
)
{
	...
	default:
		...
		if (NULL != pCEdit)
		{
			//	May 16, 2000 genta
			//	From Here
			if (uMsg == WM_COMMAND) { ::SendMessage(::GetParent(pCEdit->m_hwndParent), WM_COMMAND, wParam, lParam); }
			else
			{
				return pCEdit->DispatchEvent(hwnd, uMsg, wParam, lParam);
			}
...
*/
VOID CALLBACK EditViewTimerProc(HWND	 hwnd,	// handle of window for timer messages
...
)
{
	...
	if (NULL != pCEditView) { pCEditView->OnTimer(hwnd, uMsg, idEvent, dwTime); }
	return;
}
...
BOOL CEditView::Create(HWND		 hwndParent, //!< 親
					   ...
					   bool		 bMiniMap)
{
	...
	if (m_bMiniMap) { m_pcViewFont = m_pcEditWnd->m_pcViewFontMiniMap; }
	else
	{
	...
	m_ptSrchStartPos_PHY.Set(
		CLogicInt(-1),
		CLogicInt(
			-1)); //検索/置換開始時のカーソル位置  (改行単位行先頭からのバイト数(0開始), 改行単位行の行番号(0開始))
	m_bSearch = FALSE; // 検索/置換開始位置を登録するか */											// 02/06/26 ai

	m_ptBracketPairPos_PHY.Set(
		CLogicInt(-1), CLogicInt(-1)); // 対括弧の位置 (改行単位行先頭からのバイト数(0開始), 改行単位行の行番号(0開始))
	...
	GetTextArea().SetAreaTop(GetTextArea().GetAreaTop()
							 + GetDllShareData().m_Common.m_sWindow.m_nRulerHeight); /* ルーラー高さ */
	...
	/* エディタウィンドウの作成 */
	SetHwnd(::CreateWindowEx(WS_EX_STATICEDGE, // extended window style
	...
	if (bShow) { ShowWindow(GetHwnd(), SW_SHOW); }
...
CEditView::~CEditView() { Close(); }

void CEditView::Close()
{
	if (GetHwnd() != NULL) { ::DestroyWindow(GetHwnd()); }
...
// WCHAR→WCHAR変換。
inline wchar_t tchar_to_wchar(WCHAR tc) { return tc; }

/*
|| メッセージディスパッチャ
*/
LRESULT CEditView::DispatchEvent(HWND   hwnd,   // handle of window
								 UINT   uMsg,   // message identifier
								 WPARAM wParam, // first message parameter
								 LPARAM lParam  // second message parameter
)
{
	...
	{
	case WM_MOUSEWHEEL:
		if (m_pcEditWnd->DoMouseWheel(wParam, lParam)) { return 0L; }
		return OnMOUSEWHEEL(wParam, lParam);

	case WM_MOUSEHWHEEL: return OnMOUSEHWHEEL(wParam, lParam);
	...
	case WM_CREATE:
		...
		if (m_hwndSizeBox == NULL) { return -1; }
		m_hwndSizeBoxPlaceholder =
		...
		if (m_hwndSizeBoxPlaceholder == NULL) { return -1; }
		return 0L;

		// From Here 2007.09.09 Moca 互換BMPによる画面バッファ
	case WM_SHOWWINDOW:
		// ウィンドウ非表示の再に互換BMPを廃棄してメモリーを節約する
		if (hwnd == GetHwnd() && (BOOL)wParam == FALSE) { DeleteCompatibleBitmap(); }
		return 0L;
		// To Here 2007.09.09 Moca

	case WM_SIZE: OnSize(LOWORD(lParam), HIWORD(lParam)); return 0L;
	...
	case WM_CHAR:
		// コントロールコード入力禁止
		if (WCODE::IsControlCode((wchar_t)wParam)) { ErrorBeep(); }
		else
		{
	...
	case WM_IME_NOTIFY: // Nov. 26, 2006 genta
		if (wParam == IMN_SETCONVERSIONMODE || wParam == IMN_SETOPENSTATUS) { GetCaret().ShowEditCaret(); }
		return DefWindowProc(hwnd, uMsg, wParam, lParam);
	...
	case WM_IME_ENDCOMPOSITION: m_szComposition[0] = L'\0'; return DefWindowProc(hwnd, uMsg, wParam, lParam);
	...
	case WM_PASTE: return GetCommander().HandleCommand(F_PASTE, true, 0, 0, 0, 0);
	...
	case WM_MBUTTONDOWN: OnMBUTTONDOWN(wParam, (short)LOWORD(lParam), (short)HIWORD(lParam)); return 0L;
	...
	case WM_LBUTTONUP:
		...
		return 0L;
	case WM_MOUSEMOVE: OnMOUSEMOVE(wParam, (short)LOWORD(lParam), (short)HIWORD(lParam)); return 0L;
	...
	case WM_XBUTTONDOWN:
		...
		{
		case XBUTTON1: OnXLBUTTONDOWN(wParam, (short)LOWORD(lParam), (short)HIWORD(lParam)); break;
		case XBUTTON2: OnXRBUTTONDOWN(wParam, (short)LOWORD(lParam), (short)HIWORD(lParam)); break;
		}
	...
	case WM_XBUTTONUP:
		...
		switch (HIWORD(wParam))
		{
		case XBUTTON1: OnXLBUTTONUP(wParam, (short)LOWORD(lParam), (short)HIWORD(lParam)); break;
		case XBUTTON2: OnXRBUTTONUP(wParam, (short)LOWORD(lParam), (short)HIWORD(lParam)); break;
		}
	...
	case WM_VSCROLL:
		...
		{
			CLayoutInt Scroll = OnVScroll((int)LOWORD(wParam), ((int)HIWORD(wParam)) * m_nVScrollRate);

			//	シフトキーが押されていないときだけ同期スクロール
			if (!GetKeyState_Shift()) { SyncScrollV(Scroll); }
		}
	...
	case WM_HSCROLL:
		...
		//	Sep. 11, 2004 genta 同期スクロールの関数化
		{
			CLayoutInt Scroll = OnHScroll((int)LOWORD(wParam), ((int)HIWORD(wParam)));

			//	シフトキーが押されていないときだけ同期スクロール
			if (!GetKeyState_Shift()) { SyncScrollH(Scroll); }
		}
	...
	case WM_CLOSE:
		...
		return 0L;
	case WM_DESTROY:
		if (NULL != m_pcDropTarget) { m_pcDropTarget->Revoke_DropTarget(); }
	...
	case WM_IME_REQUEST: /* 再変換  by minfu 2002.03.27 */ // 20020331 aroka
		...
		switch (wParam)
		{
		case IMR_RECONVERTSTRING: return SetReconvertStruct((PRECONVERTSTRING)lParam, UNICODE_BOOL);

		case IMR_CONFIRMRECONVERTSTRING: return SetSelectionFromReonvert((PRECONVERTSTRING)lParam, UNICODE_BOOL);

		// 2010.03.16 MS-IME 2002 だと「カーソル位置の前後の内容を参照して変換を行う」の機能
		case IMR_DOCUMENTFEED: return SetReconvertStruct((PRECONVERTSTRING)lParam, UNICODE_BOOL, true);
	...
	case WM_MOUSEACTIVATE:
		...
		if (nRes == MA_NOACTIVATE || nRes == MA_NOACTIVATEANDEAT) { return nRes; }

		// マウスクリックによりバックグラウンドウィンドウがアクティベートされた
		//	2007.10.08 genta オプション追加
		if (GetDllShareData().m_Common.m_sGeneral.m_bNoCaretMoveByActivation && (!m_pcEditWnd->IsActiveApp()))
		{
		...
		if (::GetFocus() != GetHwnd())
		{
			...
			if (hwndCursorPos == GetHwnd())
			{
				...
				if (!m_bMiniMap) { m_pcEditWnd->SetActivePane(m_nMyIndex); }
			}
			...
			else
			{
				...
				if (!m_bMiniMap) { ::PostMessageAny(GetHwnd(), MYWM_SETACTIVEPANE, (WPARAM)m_nMyIndex, 0); }
			}
		}

		return nRes;

	case EM_GETLIMITTEXT: return INT_MAX;
	case EM_REPLACESEL:
	{
		// wParam RedoUndoフラグは無視する
		if (lParam) { GetCommander().HandleCommand(F_INSTEXT_W, true, lParam, wcslen((wchar_t *)lParam), TRUE, 0); }
		return 0L; // not use.
	}
...
void CEditView::OnSize(int cx, int cy)
{
	...
	if (NULL != m_hwndVScrollBar)
	{
		::MoveWindow(m_hwndVScrollBar, cx - nCxVScroll, 0 + nVSplitHeight, nCxVScroll, cy - nCyVScroll - nVSplitHeight,
					 TRUE);
	}
	/* 水平スクロールバー */
	if (NULL != m_hwndHScrollBar)
	{
		::MoveWindow(m_hwndHScrollBar, 0 + nHSplitWidth, cy - nCyHScroll, cx - nCxVScroll - nHSplitWidth, nCyHScroll,
					 TRUE);
	}
	...
	// エリア情報更新
	GetTextArea().TextArea_OnSize(CMySize(cx, cy), nCxVScroll, m_hwndHScrollBar ? nCyHScroll : 0);

	/* 再描画用メモリＢＭＰ */
	// From Here 2007.09.09 Moca 互換BMPによる画面バッファ
	if (m_hdcCompatDC != NULL) { CreateOrUpdateCompatibleBitmap(cx, cy); }
	// To Here 2007.09.09 Moca
	...
	if (IsBkBitmap())
	{
		...
		if (imgPos != BGIMAGE_TOP_LEFT)
		{
			...
			case BGIMAGE_CENTER: bUpdateWidth = true; break;
			}
			switch (imgPos)
			{
			case BGIMAGE_BOTTOM_CENTER:
			...
			case BGIMAGE_CENTER_RIGHT: bUpdateHeight = true; break;
			}
...
void CEditView::OnKillFocus(void)
{
	...
	if (m_nAutoScrollMode) { AutoScrollExit(); }
...
void CEditView::SetFont(void)
{
	...
	// メトリクス更新
	if (m_bMiniMap) { GetTextMetrics().Update(hdc, GetFontset().GetFontHan(), 0, 0); }
	else
	{
...
*/
void CEditView::MoveCursorSelecting(CLayoutPoint ptWk_CaretPos,   //!< [in] 移動先レイアウト位置
...
)
{
	...
	GetCaret().MoveCursor(ptWk_CaretPos, true,
						  nCaretMarginRate); // 2007.08.22 ryoji nCaretMarginRateが使われていなかった
	GetCaret().m_nCaretPosX_Prev = GetCaret().GetCaretLayoutPos().GetX2();
}
...
)
{
	...
	if (bUseRegexKeyword) { m_cRegexKeyword->RegexKeyLineStart(); }
	...
	*/
	CLogicPoint ptXY;
	m_pcEditDoc->m_cLayoutMgr.LayoutToLogic(ptCaretPos, &ptXY);
	CLogicInt	  nLineLen;
	const wchar_t *pLine = m_pcEditDoc->m_cDocLineMgr.GetLine(ptXY.GetY2())
							   ->GetDocLineStrWithEOL(&nLineLen); // 2007.10.09 kobake レイアウト・ロジック混在バグ修正
	...
	{
		bMatch =
			(bUseRegexKeyword && m_cRegexKeyword->RegexIsKeyword(CStringRef(pLine, nLineLen), i, &nUrlLen, &nMatchColor)
		...
		if (bMatch)
		{
			...
			{
				/* URLを返す場合 */
				if (pstrURL) { pstrURL->assign(&pLine[i], nUrlLen); }
...
VOID CEditView::OnTimer(HWND	 hwnd,	// handle of window for timer messages
						UINT	 uMsg,	// WM_TIMER message
						UINT_PTR idEvent, // timer identifier
						DWORD	dwTime   // current system time
)
{
	...
	if (GetDllShareData().m_Common.m_sEdit.m_bUseOLE_DragDrop)
	{ /* OLEによるドラッグ & ドロップを使う */
	...
	if (!GetSelectionInfo().IsMouseSelecting())
	{
		if (m_bMiniMap)
		{
			...
			{
				if (bHide && 0 == m_dwTipTimer) { m_cTipWnd.Hide(); }
			}
		}
		else
		{
			if (FALSE != KeyWordHelpSearchDict(LID_SKH_ONTIMER, &po, &rc))
			{ // 2006.04.10 fon
				/* 辞書Tipを表示 */
...
void CEditView::ConvSelectedArea(EFunctionCode nFuncCode)
{
	...
	m_pcEditDoc->m_cLayoutMgr.LayoutToLogic(GetSelectionInfo().m_sSelect.GetFrom(), &ptFromLogic);
	...
	if (GetSelectionInfo().IsBoxSelecting())
	{
		...
		for (nLineNum = rcSelLayout.bottom; nLineNum >= rcSelLayout.top - 1; nLineNum--)
		{
			...
			if (nLineNum < rcSelLayout.bottom && 0 < nDelLen)
			{
				...
				sPos.Set(LineIndexToColumn(pcLayout, nDelPos), nLineNum + 1);

				// 2007.10.18 COpe整理
				// 指定位置の指定長データ削除
				DeleteData2(sPos, nDelLen, &cmemBuf);

				{
					/* 機能種別によるバッファの変換 */
					int nStartColumn = (Int)sPos.GetX2() / (Int)GetTextMetrics().GetLayoutXDefault();
					CConvertMediator::ConvMemory(&cmemBuf, nFuncCode, m_pcEditDoc->m_cLayoutMgr.GetTabSpaceKetas(),
												 nStartColumn);

					/* 現在位置にデータを挿入 */
					CLayoutPoint ptLayoutNew; // 挿入された部分の次の位置
					InsertData_CEditView(sPos, cmemBuf.GetStringPtr(), cmemBuf.GetStringLength(), &ptLayoutNew,
										 false // 2009.07.18 ryoji true -> false 各行にアンダーラインが残る問題の修正
					);
		...
		{ /* アンドゥ・リドゥの実行中か */
			...
			m_cCommander.GetOpeBlk()->AppendOpe(
				new CMoveCaretOpe(GetCaret().GetCaretLogicPos() // 操作前後のキャレット位置
								  ));
		}
	}
	else
	{
		...
		/* 機能種別によるバッファの変換 */
		int nStartColum =
			(Int)GetSelectionInfo().m_sSelect.GetFrom().GetX2() / (Int)GetTextMetrics().GetLayoutXDefault();
		CConvertMediator::ConvMemory(&cmemBuf, nFuncCode, m_pcEditDoc->m_cLayoutMgr.GetTabSpaceKetas(), nStartColum);

		/* データ置換 削除&挿入にも使える */
		ReplaceData_CEditView(GetSelectionInfo().m_sSelect, cmemBuf.GetStringPtr(),
							  /* 挿入するデータ */ // 2002/2/10 aroka CMemory変更
							  cmemBuf.GetStringLength(),
							  /* 挿入するデータの長さ */ // 2002/2/10 aroka CMemory変更
							  false, m_bDoing_UndoRedo ? NULL : m_cCommander.GetOpeBlk());

		// From Here 2001.12.03 hor
		//	選択エリアの復元
		CLayoutPoint ptFrom; // 2009.07.18 ryoji LogicからLayoutに戻す
		m_pcEditDoc->m_cLayoutMgr.LogicToLayout(ptFromLogic, &ptFrom);
...
void CEditView::AddKeyHelpMenu(HMENU hMenu, EKeyHelpRMenuType eRmenuType)
{
	...
	if (!GetSelectionInfo().IsMouseSelecting() && eRmenuType != KEYHELP_RMENU_NONE)
	{
		POINT po;
		RECT  rc;
		if (FALSE != KeyWordHelpSearchDict(LID_SKH_POPUPMENU_R, &po, &rc))
		{ // 2006.04.10 fon
			if (eRmenuType == KEYHELP_RMENU_BOTTOM) { cMenuDrawer.MyAppendMenuSep(hMenu, MF_SEPARATOR, F_0, L""); }
			cMenuDrawer.MyAppendMenu(hMenu, 0, IDM_COPYDICINFO, LS(STR_MENU_KEYWORDINFO),
									 L"K"); // 2006.04.10 fon ToolTip内容を直接表示するのをやめた
			cMenuDrawer.MyAppendMenu(hMenu, 0, IDM_JUMPDICT, LS(STR_MENU_OPENKEYWORDDIC), L"L"); // 2006.04.10 fon
			if (eRmenuType == KEYHELP_RMENU_TOP) { cMenuDrawer.MyAppendMenuSep(hMenu, MF_SEPARATOR, F_0, L""); }
		}
	}
}
...
int CEditView::CreatePopUpMenuSub(HMENU hMenu, int nMenuIdx, int *pParentMenus, EKeyHelpRMenuType eRmenuType)
{
	...
	int *pNextParam  = nParentMenu;
	{
		...
		if (nMenuIdx == CUSTMENU_INDEX_FOR_RBUTTONUP) { nThisCode = F_MENU_RBUTTON; }
		else
		{
	...
	if (eRmenuType == KEYHELP_RMENU_TOP) { AddKeyHelpMenu(hMenu, KEYHELP_RMENU_TOP); }
	...
	for (i = 0; i < GetDllShareData().m_Common.m_sCustomMenu.m_nCustMenuItemNumArr[nMenuIdx]; ++i)
	{
		...
		{
			int nCustIdx = 0;
			if (F_MENU_RBUTTON == code) { nCustIdx = CUSTMENU_INDEX_FOR_RBUTTONUP; }
			else
			{
		...
		if (!bAppend)
		{
			...
			/* キー */
			if (F_SPECIAL_FIRST <= code && code <= F_SPECIAL_LAST) { m_pcEditWnd->InitMenu_Special(hMenu, code); }
			else
			{
				wchar_t keys[2];
				keys[0] = GetDllShareData().m_Common.m_sCustomMenu.m_nCustMenuItemKeyArr[nMenuIdx][i];
	...
	if (eRmenuType == KEYHELP_RMENU_BOTTOM) { AddKeyHelpMenu(hMenu, KEYHELP_RMENU_BOTTOM); }
	...
	::GetCursorPos(&po);
	po.y -= 4;
	nId = ::TrackPopupMenu(hMenu,
						   TPM_TOPALIGN | TPM_LEFTALIGN | TPM_RETURNCMD | TPM_LEFTBUTTON
						   /*| TPM_RIGHTBUTTON*/
						   ,
						   po.x, po.y, 0, ::GetParent(m_hwndParent) /*GetHwnd()*/, NULL);
	::DestroyMenu(hMenu);
	return nId;
}
...
void CEditView::OnChangeSetting()
{
	...
	/* 再描画 */
	if (!m_pcEditWnd->m_pPrintPreview) { ::InvalidateRect(GetHwnd(), NULL, TRUE); }
	CTypeSupport cTextType(this, COLORIDX_TEXT);
	m_crBack = cTextType.GetBackColor();
}
...
/* 選択範囲のデータを取得
	正常時はTRUE,範囲未選択の場合はFALSEを返す
*/
bool CEditView::GetSelectedData(CNativeW *cmemBuf, BOOL bLineOnly, const wchar_t *pszQuote, /* 先頭に付ける引用符 */
...
)
{
	...
	/* 範囲選択がされていない */
	if (!GetSelectionInfo().IsTextSelected()) { return false; }
	if (bWithLineNumber)
	{ /* 行番号を付与する */
		/* 行番号表示に必要な桁数を計算 */
		...
		for (nLineNum = rcSel.top; nLineNum <= rcSel.bottom; ++nLineNum)
		{
			...
			if (NULL != pLine)
			{
				...
				if (nIdxTo - nIdxFrom > 0)
				{
					...
					{ cmemBuf->AppendString(&pLine[nIdxFrom], nIdxTo - nIdxFrom - 1); } else
					{
	...
	else
	{
		...
		// 先頭に引用符を付けるとき。
		if (NULL != pszQuote) { nBufSize += wcslen(pszQuote); }

		// 行番号を付ける。
		if (bWithLineNumber) { nBufSize += nLineNumCols; }

		// 改行コードについて。
		if (neweol == EOL_UNKNOWN) { nBufSize += wcslen(WCODE::CRLF); }
		else
		{
		...
		for (nLineNum = GetSelectionInfo().m_sSelect.GetFrom().GetY2();
			 nLineNum <= GetSelectionInfo().m_sSelect.GetTo().y; ++nLineNum)
		{
			pLine = m_pcEditDoc->m_cLayoutMgr.GetLineStr(nLineNum, &nLineLen, &pcLayout);
			if (NULL == pLine) { break; }
			...
			if (nIdxTo - nIdxFrom == CLogicInt(0)) { continue; }

			if (NULL != pszQuote && pszQuote[0] != L'\0')
			{ /* 先頭に付ける引用符 */
			...
			if (EOL_NONE != pcLayout->GetLayoutEol())
			{
				if (nIdxTo >= nLineLen)
				{
					...
					//	Jul. 25, 2000 genta
					cmemBuf->AppendString((neweol == EOL_UNKNOWN) ? (pcLayout->GetLayoutEol()).GetValue2()
																  :   //	コード保存
...
bool CEditView::GetSelectedDataOne(CNativeW &cmemBuf, int nMaxLen)
{
	...
	if (GetSelectionInfo().IsBoxSelecting())
	{
		...
		if (NULL != pLine && NULL != pcLayout)
		{
			...
			if (0 < nSelectLen)
			{
				cmemBuf.AppendString(&pLine[nIdxFrom],
									 t_min<int>(nMaxLen, t_min<int>(nSelectLen, nLineLen - nIdxFrom)));
			}
		}
	}
	else
	{
		...
		if (NULL != pDocLine)
		{
			...
			nIdxFrom = ptFrom.x;
			if (targetY == ptTo.y) { nIdxTo = ptTo.x; }
			else
			{
				nIdxTo = nLineLen;
			}
...
/* 指定カーソル位置が選択エリア内にあるか
	...
	1	選択エリアより後方
*/
int CEditView::IsCurrentPositionSelected(CLayoutPoint ptCaretPos // カーソル位置
)
{
	if (!GetSelectionInfo().IsTextSelected())
	{ /* テキストが選択されているか */
	...
	if (GetSelectionInfo().IsBoxSelecting())
	{
		...
		if (rcSel.PtInRect(po)) { return 0; }
		if (rcSel.top > ptCaretPos.y) { return -1; }
		if (rcSel.bottom < ptCaretPos.y) { return 1; }
		if (rcSel.left > ptCaretPos.x) { return -1; }
		if (rcSel.right < ptCaretPos.x) { return 1; }
	}
	else
	{
		if (GetSelectionInfo().m_sSelect.GetFrom().y > ptCaretPos.y) { return -1; }
		if (GetSelectionInfo().m_sSelect.GetTo().y < ptCaretPos.y) { return 1; }
		if (GetSelectionInfo().m_sSelect.GetFrom().y == ptCaretPos.y)
		{
			if (IsDragSource())
			{
				if (GetKeyState_Control())
				{ /* Ctrlキーが押されていたか */
					if (GetSelectionInfo().m_sSelect.GetFrom().x >= ptCaretPos.x) { return -1; }
				}
				else
				{
					if (GetSelectionInfo().m_sSelect.GetFrom().x > ptCaretPos.x) { return -1; }
			...
			else if (GetSelectionInfo().m_sSelect.GetFrom().x > ptCaretPos.x)
			{
				return -1;
			}
		}
		if (GetSelectionInfo().m_sSelect.GetTo().y == ptCaretPos.y)
		{
			if (IsDragSource())
			{
				if (GetKeyState_Control())
				{ /* Ctrlキーが押されていたか */
					if (GetSelectionInfo().m_sSelect.GetTo().x <= ptCaretPos.x) { return 1; }
				}
				else
				{
					if (GetSelectionInfo().m_sSelect.GetTo().x < ptCaretPos.x) { return 1; }
			...
			else if (GetSelectionInfo().m_sSelect.GetTo().x <= ptCaretPos.x)
			{
				return 1;
			}
		}
...
int CEditView::IsCurrentPositionSelectedTEST(const CLayoutPoint &ptCaretPos, //カーソル位置
...
void CEditView::CopySelectedAllLines(const wchar_t *pszQuote,		//!< 先頭に付ける引用符
									 BOOL			bWithLineNumber //!< 行番号を付与する
)
{
	CNativeW cmemBuf;
	...
	if (!GetSelectedData(&cmemBuf, FALSE, pszQuote,							   /* 引用符 */
						 bWithLineNumber,									   /* 行番号を付与する */
...
{
	if (pbColumnSelect) *pbColumnSelect = false;

	if (pbLineSelect) *pbLineSelect = false;

	if (!CClipboard::HasValidData()) return false;

	CClipboard cClipboard(GetHwnd());
	if (!cClipboard) return false;

	CEol cEol = m_pcEditDoc->m_cDocEditor.GetNewLineCode();
	if (!cClipboard.GetText(&cmemBuf, pbColumnSelect, pbLineSelect, cEol)) { return false; }
...
inline bool CEditView::IsDrawCursorVLinePos(int posX)
{
	return posX
			   >= GetTextArea().GetAreaLeft()
					  - 2 // 2010.08.10 ryoji
						  // テキストと行番号の隙間が半角文字幅より大きいと隙間位置にあるカーソルの縦線が描画される問題修正
		   && posX > GetTextArea().GetAreaLeft()
						 - GetDllShareData()
							   .m_Common.m_sWindow.m_nLineNumRightSpace // 隙間(+1)がないときは線を引かない判定
...
void CEditView::CaretUnderLineON(bool bDraw, bool bDrawPaint, bool DisalbeUnderLine)
{
	...
	if (!bUnderLine && !bCursorVLine && !bCursorLineBg) { return; }
	...
	// カーソル行の描画
	if (bDraw && bCursorLineBg && GetDrawSwitch()
	...
	if (bCursorVLine)
	{
		...
		nCursorVLineX =
			GetTextArea().GetAreaLeft()
			+ GetTextMetrics().GetCharPxWidth(GetCaret().GetCaretLayoutPos().GetX2() - GetTextArea().GetViewLeftCol())
			- 1;
	}

	if (bDraw && GetDrawSwitch() && IsDrawCursorVLinePos(nCursorVLineX) && m_bDoing_UndoRedo == false
		&& !GetSelectionInfo().IsTextSelecting() && !DisalbeUnderLine)
	{
	...
	if (bDraw && GetDrawSwitch() && nUnderLineY >= GetTextArea().GetAreaTop()
		&& m_bDoing_UndoRedo == false /* アンドゥ・リドゥの実行中か */
		&& !GetSelectionInfo().IsTextSelecting() && !DisalbeUnderLine)
	{
		...
		HDC hdc = ::GetDC(GetHwnd());
		{
			...
			::MoveToEx(gr, GetTextArea().GetAreaLeft(), nUnderLineY, NULL);
			::LineTo(gr, GetTextArea().GetAreaRight(), nUnderLineY);
		} // ReleaseDC の前に gr デストラクト
		::ReleaseDC(GetHwnd(), hdc);
	}
}
...
void CEditView::CaretUnderLineOFF(bool bDraw, bool bDrawPaint, bool bResetFlag, bool DisalbeUnderLine)
{
	...
	{ return; } if (-1 != m_nOldUnderLineY)
	{
		if (bDraw && GetDrawSwitch() && m_nOldUnderLineY >= GetTextArea().GetViewTopLine()
		...
		)
		{
			...
			if (bDrawPaint) { m_nOldUnderLineYBg = -1; }
		}
		if (bResetFlag) { m_nOldUnderLineY = -1; }
		m_nOldUnderLineYHeightReal = 0;
	}
	...
	{
		if (bDraw && GetDrawSwitch() && IsDrawCursorVLinePos(m_nOldCursorLineX) && m_bDoing_UndoRedo == false
...
*/
void CEditView::SendStatusMessage(const WCHAR *msg) { m_pcEditWnd->SendStatusMessage(msg); }
...
*/
bool CEditView::IsInsMode(void) const { return m_pcEditDoc->m_cDocEditor.IsInsMode(); }

void CEditView::SetInsMode(bool mode) { m_pcEditDoc->m_cDocEditor.SetInsMode(mode); }
...
bool CEditView::ShowKeywordHelp(POINT po, LPCWSTR pszHelp, LPRECT prcHokanWin)
{
	...
	RECT	 rcTipWin, rcDesktop;

	if (m_pTypeData->m_bUseKeyWordHelp)
	{								  /* キーワードヘルプを使用する */
		...
		)
		{
			...
			{
				/* 該当するキーがなかった */
				if (!m_cTipWnd.m_KeyWasHit) { return false; }
			}
			else
			{
...
bool CEditView::IsEmptyArea(CLayoutPoint ptFrom, CLayoutPoint ptTo, bool bSelect, bool bBoxSelect) const
{
	...
	if (bSelect && !bBoxSelect && nLineFrom != nLineTo)
	{ // 複数行の範囲指定
	...
	else
	{
		...
		{
			// 範囲の調整
			if (nLineFrom > nLineTo) { std::swap(nLineFrom, nLineTo); }

			if (nColumnFrom > nColumnTo) { std::swap(nColumnFrom, nColumnTo); }
		}
		else
		{
			nLineTo = nLineFrom;
		...
		for (CLayoutInt nLineNum = nLineFrom; nLineNum <= nLineTo; nLineNum++)
		{
			if ((pcLayout = m_pcEditDoc->m_cLayoutMgr.SearchLineByLayoutY(nLineNum)) != NULL)
			{
				...
				if (nLineLen == 0)
				{ // 折り返しや改行コードより右の場合には nLineLen に行全体の表示桁数が入る
...
void CEditView::SetUndoBuffer(bool bPaintLineNumber)
{
	...
	{
		if (0 < m_cCommander.GetOpeBlk()->GetNum())
		{ /* 操作の数を返す */
			/* 操作の追加 */
			...
			//  if( bPaintLineNumber
			//   &&	m_pcEditDoc->m_cDocEditor.m_cOpeBuf.GetCurrentPointer() == 1 )	// 全Undo状態からの変更か？	//
			//   2009.03.26 ryoji
			//  	Call_OnPaint( PAINT_LINENUMBER, false );	// 自ペインの行番号（変更行）表示を更新 ←
			//  変更行のみの表示更新で済ませている場合があるため

			if (!m_pcEditWnd->UpdateTextWrap())
			{ // 折り返し方法関連の更新	// 2008.06.10 ryoji
