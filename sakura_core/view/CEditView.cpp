		...
		*/
        ...
        case WM_MOUSEACTIVATE:
            ...
            if (::GetFocus() != GetHwnd())
            {
                ...
                else if ((m_pcsbwVSplitBox && hwndCursorPos == m_pcsbwVSplitBox->GetHwnd()) || (m_pcsbwHSplitBox && hwndCursorPos == m_pcsbwHSplitBox->GetHwnd()))
                {
                    // 2010.01.19 ryoji
...
void CEditView::OnSize(int cx, int cy)
{
    ...
    if (m_pcEditWnd->GetMiniMap().GetHwnd())
    {
        ...
        if (miniMap.m_nPageViewTop != GetTextArea().GetViewTopLine() || miniMap.m_nPageViewBottom != GetTextArea().GetBottomLine())
        {
            MiniMapRedraw(true);
        }
    }
...
)
{
	...
	*/
    ...
    {
        bMatch = (bUseRegexKeyword && m_cRegexKeyword->RegexIsKeyword(CStringRef(pLine, nLineLen), i, &nUrlLen, &nMatchColor) && nMatchColor == COLORIDX_URL);
        if (!bMatch)
        {
            bMatch = (bDispUrl && (i == 0 || !IS_KEYWORD_CHAR(pLine[i - 1])) // 2009.05.22 ryoji CColor_Url::BeginColor()と同条件に
        ...
        for (nLineNum = GetSelectionInfo().m_sSelect.GetFrom().GetY2(); nLineNum <= GetSelectionInfo().m_sSelect.GetTo().y; ++nLineNum)
        {
            ...
            if (EOL_NONE != pcLayout->GetLayoutEol())
            {
                if (nIdxTo >= nLineLen)
                {
                    ...
                    //	Jul. 25, 2000 genta
                    cmemBuf->AppendString((neweol == EOL_UNKNOWN) ? (pcLayout->GetLayoutEol()).GetValue2() : //	コード保存
            ...
            else
            {
                ...
                if (nIdxTo >= nLineLen)
                {
                    ...
                    {
                        //	Jul. 25, 2000 genta
                        cmemBuf->AppendString((neweol == EOL_UNKNOWN) ? m_pcEditDoc->m_cDocEditor.GetNewLineCode().GetValue2() : //	コード保存
                                                  appendEol.GetValue2()); //	新規改行コード
                    }
                }
            }
    ...
    ) const
{
    ...
    if (PointCompare(ptCaretPos, sSelect.GetFrom()) < 0)
        return -1;
    if (PointCompare(ptCaretPos, sSelect.GetTo()) >= 0)
        return 1;
...
void CEditView::CaretUnderLineON(bool bDraw, bool bDrawPaint, bool DisalbeUnderLine)
{
    ...
    // カーソル行の描画
    if (bDraw && bCursorLineBg && GetDrawSwitch() && GetCaret().GetCaretLayoutPos().GetY2() >= GetTextArea().GetViewTopLine() && m_bDoing_UndoRedo == false /* アンドゥ・リドゥの実行中か */
    )
    {
    ...
    if (bCursorVLine)
    {
        ...
        nCursorVLineX = GetTextArea().GetAreaLeft() + GetTextMetrics().GetCharPxWidth(GetCaret().GetCaretLayoutPos().GetX2() - GetTextArea().GetViewLeftCol()) - 1;
    }

    if (bDraw && GetDrawSwitch() && IsDrawCursorVLinePos(nCursorVLineX) && m_bDoing_UndoRedo == false && !GetSelectionInfo().IsTextSelecting() && !DisalbeUnderLine)
    {
    ...
    if (bUnderLine)
    {
        nUnderLineY = GetTextArea().GetAreaTop() + (Int)(GetCaret().GetCaretLayoutPos().GetY2() - GetTextArea().GetViewTopLine()) * GetTextMetrics().GetHankakuDy() + GetTextMetrics().GetHankakuHeight();
    }
    // To Here 2007.09.09 Moca

    if (bDraw && GetDrawSwitch() && nUnderLineY >= GetTextArea().GetAreaTop() && m_bDoing_UndoRedo == false /* アンドゥ・リドゥの実行中か */
...
void CEditView::CaretUnderLineOFF(bool bDraw, bool bDrawPaint, bool bResetFlag, bool DisalbeUnderLine)
{
    ...
    {
        if (bDraw && GetDrawSwitch() && m_nOldUnderLineY >= GetTextArea().GetViewTopLine() && !m_bDoing_UndoRedo /* アンドゥ・リドゥの実行中か */
    ...
    {
        if (bDraw && GetDrawSwitch() && IsDrawCursorVLinePos(m_nOldCursorLineX) && m_bDoing_UndoRedo == false && !GetCaret().m_cUnderLine.GetVertLineDoNotOFF() // カーソル位置縦線を消去するか
            && !DisalbeUnderLine)
        {
            PAINTSTRUCT ps;
