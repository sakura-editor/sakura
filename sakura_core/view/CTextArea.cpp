...
void CTextArea::UpdateViewColRowNums()
{
	...
	m_nViewColNum =
		CLayoutInt(t_max(1, t_max(0, m_nViewCx - 1) / pView->GetTextMetrics().GetCharPxWidth())); // 表示域の桁数
	m_nViewRowNum =
		CLayoutInt(t_max(1, t_max(0, m_nViewCy - 1) / pView->GetTextMetrics().GetHankakuDy())); // 表示域の行数
}
...
{
	//左はみ出し調整
	if (rc->left < GetAreaLeft()) { rc->left = GetAreaLeft(); }
...
bool CTextArea::DetectWidthOfLineNumberArea(bool bRedraw)
{
	...
	if (nViewAlignLeftNew != GetAreaLeft())
	{
		...
		// m_nViewColNum = CLayoutInt(t_max(0, m_nViewCx - 1) / pView->GetTextMetrics().GetHankakuDx());	//
		// 表示域の桁数
		UpdateViewColRowNums();
		...
		if (bRedraw && pView2->GetDrawSwitch())
		{
			...
			pView2->Call_OnPaint(PAINT_LINENUMBER | PAINT_RULER | PAINT_BODY,
								 false); /* メモリＤＣを使用してちらつきのない再描画 */
...
void CTextArea::TextArea_OnSize(const CMySize &sizeClient, //!< ウィンドウのクライアントサイズ
...
{
	const CEditView *pView = m_pEditView;
	pptLayout->Set(GetViewLeftCol()
					   + CLayoutInt((ptClient.x - GetAreaLeft()) / pView->GetTextMetrics().GetCharPxWidth()),
...
{
	rc->left =
		m_nViewAlignLeft + m_nViewCx
		- m_pEditView->GetTextMetrics().GetCharPxWidth(nColCount); // 2008.01.26 kobake 符号が逆になってたのを修正
	rc->right  = m_nViewAlignLeft + m_nViewCx;
	rc->top	= m_nViewAlignTop;
	rc->bottom = m_nViewAlignTop + m_nViewCy;
}
