...
class CStringRef final : public IStringRef
{
  public:
    ...
    const wchar_t *GetPtr() const override
    {
        return m_pData;
    }
...
class CNativeW final : public CNative
{
  ...
  public:
    ...
    CNativeW operator+(const CNativeW &rhs) const
    {
        return (CNativeW(*this) += rhs);
    }
    CNativeW &operator+=(const CNativeW &rhs)
    {
    ...
    bool Equals(const CNativeW &rhs) const noexcept
    {
        return 0 == Compare(rhs);
    }
    bool Equals(const wchar_t *rhs) const noexcept
    {
        return 0 == Compare(rhs);
    }
