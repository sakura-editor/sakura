...
private:
	...
	//コンストラクタ・デストラクタ
public:
	CRecycledBuffer() { m_current = 0; }

	//インターフェース
public:
	//!一時的に確保されたメモリブロックを取得。このメモリブロックを解放してはいけない。
	template<class T>
	T *GetBuffer(size_t *nCount //!< [out] 領域の要素数を受け取る。T単位。
	)
	{
	...
	//!領域の要素数を取得。T単位
	template<class T> size_t GetMaxCount() const { return BLOCK_SIZE / sizeof(T); }
...
public:
	...
	{
		m_current = 0;
		for (int i = 0; i < _countof(m_buf); i++) { m_buf[i] = NULL; }
	}
	~CRecycledBufferDynamic()
	{
...
public:
	//!一時的に確保されたメモリブロックを取得。このメモリブロックを解放してはいけない。
	template<class T>
	T *GetBuffer(size_t nCount //!< [in] 確保する要素数。T単位。
	)
	{
		m_current = (m_current + 1) % CHAIN_COUNT;
