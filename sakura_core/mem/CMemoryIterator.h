...
public:
	...
	// CLayout用コンストラクタ
	CMemoryIterator(const CLayout *pcT, CLayoutInt nTabSpace, const CTsvModeInfo &tsvInfo, CPixelXInt nCharDx,
					CPixelXInt nSpacing)
		: m_pLine(pcT ? pcT->GetPtr() : NULL)
		, m_nLineLen(pcT ? pcT->GetLengthWithEOL() : 0)
		, m_nTabSpace(nTabSpace)
	 ...
	 */
	bool end() const { return (m_nLineLen <= m_nIndex); }
	...
	void scanNext()
	{
		...
		if (0 == m_nIndex_Delta) m_nIndex_Delta = CLogicInt(1);
