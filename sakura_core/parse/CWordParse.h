...
public:
	...
	*/
	...
	//! 現在位置の文字の種類を調べる
	static ECharKind WhatKindOfChar(const wchar_t *pData, int pDataLen, int nIdx);
	...
	static bool SearchNextWordPosition(const wchar_t *pLine, CLogicInt nLineLen,
									   CLogicInt  nIdx,			 //	桁数
	...
	static bool SearchNextWordPosition4KW(const wchar_t *pLine, CLogicInt nLineLen,
										  CLogicInt	 nIdx,			//	桁数
										  CLogicInt *pnColumnNew,	//	見つかった位置
										  BOOL		 bStopsBothEnds //	単語の両端で止まる
	);
	...
	static int GetWord(const CHAR_TYPE *pS, const int nLen, const CHAR_TYPE *pszSplitCharList, CHAR_TYPE **ppWordStart,
					   int *pnWordLen);
 ...
 */
inline BOOL IsURL(const wchar_t *pszLine, int nLineLen, int *pnMatchLen)
{
...
/** 指定アドレスがメールアドレスの先頭ならば TRUE とその長さを返す。
	...
	@param[out] pnAddressLength メールアドレスの長さを受け取る変数のアドレス。NULL可。長さとは pszBuf + offset
   からの距離。
...
/** @deprecated 互換性のために残されています。offset 引数が追加されたものを使用してください。
 */
inline BOOL IsMailAddress(const wchar_t *pszBuf, int nBufLen, int *pnAddressLength)
{
...
*/
template<class CHAR_TYPE>
int CWordParse::GetWord(const CHAR_TYPE *pS, const int nLen, const CHAR_TYPE *pszSplitCharList, CHAR_TYPE **ppWordStart,
						int *pnWordLen)
{
...
end_func:
	if (ppWordStart) { *ppWordStart = pwordstart; }
	if (pnWordLen) { *pnWordLen = nwordlen; }
	return pr - pS;
}
