...
struct KEYDATAINIT {
	...
	short			m_nFuncCodeArr[8];	//!< Key Function Number
...
//	To Here Sept. 19,2000
...
#endif
			sGeneral.m_wTrayMenuHotKeyCode = _T('Z');		/* タスクトレイ左クリックメニュー キー */
		...
		// [ウィンドウ]タブ
		{
			...
			sWindow.m_bMenuWChar = FALSE;		/* メニューの字化け対策を行う(Win2K以降のみ) */
		...
		// [編集]タブ
		{
			...
			_tcscpy(sEdit.m_OpenDialogSelDir, _T("%Personal%\\"));
		...
		// [書式]タブ
		{
			...
			/* 見出し記号 */
			_tcscpy( sFormat.m_szMidashiKigou, _T("１２３４５６７８９０（(［[「『【■□▲△▼▽◆◇○◎●§・※☆★第①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩ一二三四五六七八九十壱弐参伍") );
			/* 引用符 */
			_tcscpy( sFormat.m_szInyouKigou, _T("> ") );	/* 引用符 */
		...
		// [カスタムメニュー]タブ
		{
			...
			wsprintf( sCustomMenu.m_szCustMenuNameArr[0], _T("右クリックメニュー") );
			for( i = 1; i < MAX_CUSTOM_MENU; ++i ){
				wsprintf( sCustomMenu.m_szCustMenuNameArr[i], _T("メニュー%d"), i );
			...
			wsprintf( sCustomMenu.m_szCustMenuNameArr[CUSTMENU_INDEX_FOR_TABWND], _T("タブメニュー") );	//@@@ 2003.06.13 MIK
		...
		// [支援]タブ
		{
			...
			sHelper.m_nPointSize = 0;	// フォントサイズ（1/10ポイント単位） ※古いバージョンからの移行を考慮して無効値で初期化	// 2009.10.01 ryoji

			sHelper.m_szExtHelp[0] = _T('\0');			// 外部ヘルプ１
			sHelper.m_szExtHtmlHelp[0] = _T('\0');		// 外部HTMLヘルプ

			sHelper.m_szMigemoDll[0] = _T('\0');		/* migemo dll */
			sHelper.m_szMigemoDict[0] = _T('\0');		/* migemo dict */
...

		{
			...
			for( i = 0; i < MAX_TAGJUMP_KEYWORD; ++i ){
				m_pShareData->m_sTagJump.m_szTagJumpKeywordArr[i][0] = _T('\0');
			}
...
void CShareData::TraceOut( LPCTSTR lpFmt, ... )
{
	...
	|| !IsSakuraMainWindow( m_pShareData->m_sHandles.m_hwndDebug )
		...
		CControlTray::OpenNewEditor( NULL, m_hwndTraceOutSource, "-DEBUGMODE", CODE_SJIS, false, true );
...
const TCHAR* CShareData::MyGetDateFormat( const SYSTEMTIME& systime, TCHAR* pszDest, int nDestLen, int nDateFormatType, TCHAR* szDateFormat )
{
	const TCHAR* pszForm;
	DWORD dwFlags;
...
/* 時刻をフォーマット */
const TCHAR* CShareData::MyGetTimeFormat( const SYSTEMTIME& systime, TCHAR* pszDest, int nDestLen, int nTimeFormatType, TCHAR* szTimeFormat )
{
	const TCHAR* pszForm;
	DWORD dwFlags;
