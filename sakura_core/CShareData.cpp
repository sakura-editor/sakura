...
#if 0
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "C/C++", TRUE );		/* セット 0の追加 */
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "HTML", FALSE );		/* セット 1の追加 */
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "PL/SQL", FALSE );		/* セット 2の追加 */
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "COBOL", TRUE );		/* セット 3の追加 */
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "Java", TRUE );			/* セット 4の追加 */
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "CORBA IDL", TRUE );	/* セット 5の追加 */
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "AWK", TRUE );			/* セット 6の追加 */
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "MS-DOS batch", FALSE );/* セット 7の追加 */	//Oct. 31, 2000 JEPRO 'バッチファイル'→'batch' に短縮
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "Pascal", FALSE );		/* セット 8の追加 */	//Nov. 5, 2000 JEPRO 大・小文字の区別を'しない'に変更
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "TeX", TRUE );			/* セット 9の追加 */	//Sept. 2, 2000 jepro Tex →TeX に修正 Bool値は大・小文字の区別
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "TeX2", TRUE );			/* セット10の追加 */	//Jan. 19, 2001 JEPRO 追加
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "Perl", TRUE );			/* セット11の追加 */
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "Perl2", TRUE );		/* セット12の追加 */	//Jul. 10, 2001 JEPRO Perlから変数を分離・独立
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "Visual Basic", FALSE );/* セット13の追加 */	//Jul. 10, 2001 JEPRO
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "Visual Basic2", FALSE );/* セット14の追加 */	//Jul. 10, 2001 JEPRO
	pShareData->m_CKeyWordSetMgr.AddKeyWordSet( "リッチテキスト", TRUE );/* セット15の追加 */	//Jul. 10, 2001 JEPRO

	/* ｎ番目のセットにキーワードを追加 */
	const int nKeyWordsCPP_Num = sizeof( ppszKeyWordsCPP ) / sizeof( ppszKeyWordsCPP[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 0, nKeyWordsCPP_Num, ppszKeyWordsCPP );

	static const int nKeyWordsHTML_Num = sizeof( ppszKeyWordsHTML ) / sizeof( ppszKeyWordsHTML[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 1, nKeyWordsHTML_Num, ppszKeyWordsHTML );

	static const int nKeyWordsPLSQL_Num = sizeof( ppszKeyWordsPLSQL ) / sizeof( ppszKeyWordsPLSQL[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 2, nKeyWordsPLSQL_Num, ppszKeyWordsPLSQL );

	static const int nKeyWordsCOBOL_Num = sizeof( ppszKeyWordsCOBOL ) / sizeof( ppszKeyWordsCOBOL[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 3, nKeyWordsCOBOL_Num, ppszKeyWordsCOBOL );

	static const int nKeyWordsJAVA_Num = sizeof( ppszKeyWordsJAVA ) / sizeof( ppszKeyWordsJAVA[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 4, nKeyWordsJAVA_Num, ppszKeyWordsJAVA );

	static const int nKeyWordsCORBA_IDL_Num = sizeof( ppszKeyWordsCORBA_IDL ) / sizeof( ppszKeyWordsCORBA_IDL[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 5, nKeyWordsCORBA_IDL_Num, ppszKeyWordsCORBA_IDL );

	static int nKeyWordsAWK_Num = sizeof( ppszKeyWordsAWK ) / sizeof( ppszKeyWordsAWK[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 6, nKeyWordsAWK_Num, ppszKeyWordsAWK );

	static int nKeyWordsBAT_Num = sizeof( ppszKeyWordsBAT ) / sizeof( ppszKeyWordsBAT[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 7, nKeyWordsBAT_Num, ppszKeyWordsBAT );

	static int nKeyWordsPASCAL_Num = sizeof( ppszKeyWordsPASCAL ) / sizeof( ppszKeyWordsPASCAL[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 8, nKeyWordsPASCAL_Num, ppszKeyWordsPASCAL );

	static int nKeyWordsTEX_Num = sizeof( ppszKeyWordsTEX ) / sizeof( ppszKeyWordsTEX[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 9, nKeyWordsTEX_Num, ppszKeyWordsTEX );

	static int nKeyWordsTEX2_Num = sizeof( ppszKeyWordsTEX2 ) / sizeof( ppszKeyWordsTEX2[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 10, nKeyWordsTEX2_Num, ppszKeyWordsTEX2 );

	static int nKeyWordsPERL_Num = sizeof( ppszKeyWordsPERL ) / sizeof( ppszKeyWordsPERL[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 11, nKeyWordsPERL_Num, ppszKeyWordsPERL );

	static int nKeyWordsPERL2_Num = sizeof( ppszKeyWordsPERL2 ) / sizeof( ppszKeyWordsPERL2[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 12, nKeyWordsPERL2_Num, ppszKeyWordsPERL2 );

	static int nKeyWordsVB_Num = sizeof( ppszKeyWordsVB ) / sizeof( ppszKeyWordsVB[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 13, nKeyWordsVB_Num, ppszKeyWordsVB );

	static int nKeyWordsVB2_Num = sizeof( ppszKeyWordsVB2 ) / sizeof( ppszKeyWordsVB2[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 14, nKeyWordsVB2_Num, ppszKeyWordsVB2 );

	static int nKeyWordsRTF_Num = sizeof( ppszKeyWordsRTF ) / sizeof( ppszKeyWordsRTF[0] );
	pShareData->m_CKeyWordSetMgr.SetKeyWordArr( 15, nKeyWordsRTF_Num, ppszKeyWordsRTF );
#endif
}
