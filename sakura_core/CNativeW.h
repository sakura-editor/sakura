...
class CNativeW2 : public CNative{
public:
	//コンストラクタ・デストラクタ
	CNativeW2();
	CNativeW2( const CNativeW2& );
	CNativeW2( const wchar_t* pData, int nDataLen ); //!< nDataLenは文字単位。
	CNativeW2( const wchar_t* pData);
	...
	//CNativeW2
	void SetNativeData( const CNativeW2& pcNative );            //!< バッファの内容を置き換える
	void AppendNativeData( const CNativeW2& );                  //!< バッファの最後にデータを追加する

	//演算子
	const CNativeW2& operator+=( wchar_t wch )        { AppendString(&wch,1);   return *this; }
	const CNativeW2& operator=( wchar_t wch )         { SetString(&wch,1);      return *this; }
	const CNativeW2& operator+=( const CNativeW2& rhs ){ AppendNativeData(rhs); return *this; }
	const CNativeW2& operator=(const CNativeW2& rhs)   { SetNativeData(rhs);    return *this; }
	...
	//! 同一の文字列ならtrue
	static bool IsEqual( const CNativeW2& cmem1, const CNativeW2& cmem2 );
	...
	//WCHAR
	void SetStringW2(const wchar_t* pszData)				{ return SetString(pszData); }
	void SetStringW2(const wchar_t* pData, int nLength)		{ return SetString(pData,nLength); }
	void AppendStringW2(const wchar_t* pszData)				{ return AppendString(pszData); }
	void AppendStringW2(const wchar_t* pData, int nLength)	{ return AppendString(pData,nLength); }
	const wchar_t* GetStringW2() const						{ return GetStringPtr(); }
