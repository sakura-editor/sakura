...
public:
	...
	BOOL IsValid(WIN32_FIND_DATA &w32fd, LPCWSTR pFile = NULL) override
	{
		...
		{
			if (m_cGrepEnumExceptFolders.IsValid(w32fd, pFile)) { return TRUE; }
		}
		return FALSE;
	}

	int Enumerates(LPCWSTR lpBaseFolder, CGrepEnumKeys &cGrepEnumKeys, CGrepEnumOptions option,
				   CGrepEnumFolders &except)
	{
