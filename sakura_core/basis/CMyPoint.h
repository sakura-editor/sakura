...
public:
	...
	//算術演算子
	CMyPoint &operator+=(const POINT &rhs)
	{
		x += rhs.x;
		y += rhs.y;
		return *this;
	...
	//算術演算子２
	CMyPoint operator+(const POINT &rhs) const
	{
		CMyPoint tmp = *this;
		tmp += rhs;
		return tmp;
