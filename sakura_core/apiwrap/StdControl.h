...
//                      ウィンドウ共通                         //
// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
inline BOOL Wnd_SetText(HWND hwnd, const WCHAR *str) { return SetWindowText(hwnd, str); }
...
*/
inline BOOL Wnd_SetText(HWND hwnd, const CNativeW &str) { return SetWindowText(hwnd, str.GetStringPtr()); }
...
inline LRESULT Combo_GetText(HWND hwndCombo, WCHAR *str, int cchMax) { return ::GetWindowText(hwndCombo, str, cchMax); }
...
inline bool Combo_GetLBText(HWND hwndCombo, int nIndex, CNativeW &str)
{
	...
	if (length == CB_ERR || length < 0) { return false; }
	...
	if (actualCount == CB_ERR || actualCount < 0) { return false; }
...
inline LRESULT TrackBarCtl_GetPos(HWND hwndCtl) { return ::SendMessage(hwndCtl, TBM_GETPOS, (WPARAM)0, (LPARAM)0); }
