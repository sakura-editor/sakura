...
CMacro::~CMacro(void) { ClearMacroParam(); }
...
void CMacro::AddLParam(const LPARAM *lParams, const CEditView *pcEditView)
{
	...
	case F_1PageUp_BOX:
	case F_1PageDown_BOX: nOption = 1;
	case F_UP_BOX:
	case F_DOWN_BOX:
	case F_LEFT_BOX:
	case F_RIGHT_BOX:
	...
	case F_GOFILEEND_BOX:
	{
		if (nOption == 1)
		{
			...
			case F_HalfPageUp_BOX:
			case F_HalfPageDown_BOX:
				if (lParam == 0) { AddIntParam((Int)pcEditView->GetTextArea().m_nViewRowNum / 2); }
				else
				{
					AddIntParam(lParam);
				}
			...
			case F_1PageDown_BOX:
				if (lParam == 0) { AddIntParam((Int)pcEditView->GetTextArea().m_nViewRowNum - 1); }
				else
				{
					AddIntParam(lParam);
				}
				break;
			default: AddIntParam(lParam); break;
			}
		}
		int nParamOption;
		if (nOption == 1) { nParamOption = lParams[1]; }
		else
		{
			nParamOption = lParam;
		}
		if (nParamOption == 0)
		{
			if (GetDllShareData().m_Common.m_sEdit.m_bBoxSelectLock) { nParamOption = 0x01; }
			else
			{
				nParamOption = 0x02;
			}
	...
	case F_INSTEXT_W:
	case F_FILEOPEN:
	case F_EXECEXTMACRO: { AddStringParam((const wchar_t *)lParam); //	lParamを追加。
	}
	break;
	...
	case F_EXECMD:
	{
		...
		if (lParams[2] != 0) { AddStringParam((const wchar_t *)lParams[2]); }
	}
	break;
	...
	case F_GREP:
	{
		...
		lFlag |= (GetDllShareData().m_Common.m_sSearch.m_nGrepCharSet == CODE_AUTODETECT)
					 ? 0x10
					 : 0x00; //	2002/09/21 Moca 下位互換性のための処理
		lFlag |= GetDllShareData().m_Common.m_sSearch.m_nGrepOutputLineType == 1 ? 0x20 : 0x00;
		lFlag |=
			GetDllShareData().m_Common.m_sSearch.m_nGrepOutputLineType == 2 ? 0x400000 : 0x00; // 2014.09.23 否ヒット行
		lFlag |= (GetDllShareData().m_Common.m_sSearch.m_nGrepOutputStyle == 2)
					 ? 0x40
					 : 0x00; //	CShareDataに入れなくていいの？
		...
		if (IsValidCodeType(code) || CODE_AUTODETECT == code) { lFlag |= code << 8; }
...
void CMacroParam::SetStringParam(const WCHAR *szParam, int nLength)
{
	Clear();
	int nLen;
	if (nLength == -1) { nLen = wcslen(szParam); }
	else
	{
		nLen = nLength;
	}
...
static inline int wtoi_def(const WCHAR *arg, int def_val) { return (arg == NULL ? def_val : _wtoi(arg)); }

static inline const WCHAR *wtow_def(const WCHAR *arg, const WCHAR *def_val) { return (arg == NULL ? def_val : arg); }
...
void CMacro::Save(HINSTANCE hInstance, CTextOutputStream &out) const
{
	...
	if (CSMacroMgr::GetFuncInfoByID(hInstance, nFuncID, szFuncName, szFuncNameJapanese))
	{
		...
		while (pParam)
		{
			if (pParam != m_pParamTop) { out.WriteString(L", "); }
			switch (pParam->m_eType)
			{
			case EMacroParamTypeInt: out.WriteString(pParam->m_pData); break;
...
*/
bool CMacro::HandleCommand(CEditView *pcEditView, const EFunctionCode Index, const WCHAR *Argument[],
						   const int ArgLengths[], const int ArgSize)
{
	...
	case F_CTRL_CODE:
		...
		if (Argument[0] == NULL)
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO03));
			return false;
		}
	...
	case F_WORDRIGHT_BOX:
	case F_GOFILETOP_BOX:
	case F_GOFILEEND_BOX: nOptions = 1;
	case F_GOLOGICALLINETOP_BOX:
	case F_GOLINETOP_BOX:
	case F_GOLINEEND_BOX:
	case F_HalfPageUp_BOX:
	...
	case F_1PageDown_BOX:
	{
		...
		if (nOptions == 1) { pcEditView->GetCommander().HandleCommand(Index, true, nBoxLock, 0, 0, 0); }
		else
		{
	...
	case F_CHGMOD_EOL: //	入力改行コード指定。EEolTypeの数値を指定。2003.06.23 Moca
		...
		if (Argument[0] == NULL)
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO03_1));
			return false;
		}
		{
			...
			if (nEol != EOL_NONE) { pcEditView->GetCommander().HandleCommand(Index, true, nEol, 0, 0, 0); }
		}
		break;
	case F_SET_QUOTESTRING: // Jan. 29, 2005 genta 追加 テキスト引数1つを取るマクロはここに統合していこう．
	{
		if (Argument[0] == NULL)
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO04));
			return false;
		}
		{
	...
	case F_JUMP: //	指定行へジャンプ（ただしPL/SQLコンパイルエラー行へのジャンプは未対応）
		...
		if (Argument[0] == NULL)
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO05));
			return false;
		}
		{
	...
	case F_BOOKMARK_PATTERN: // 2002.02.08 hor
		if (Argument[0] == NULL)
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO06));
			return false;
		}
		/* NO BREAK */
	case F_SEARCH_NEXT:
	case F_SEARCH_PREV:
		...
		//		0x1000	(マクロ専用)検索オプションを元に戻す
		{
			...
			{
				/* 正規表現 */
				if (lFlag & 0x04 && !CheckRegexpSyntax(Argument[0], NULL, true)) { break; }
			...
			if (bBackupFlag)
			{
				...
				if (!backupKeyMark) { pcEditView->Redraw(); }
	...
	case F_EXECMD:
		...
		if (Argument[0] == NULL)
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO07));
			return false;
		}
		{
	...
	case F_TRACEOUT: // 2006.05.01 マクロ用アウトプットウインドウに出力
		...
		{
			pcEditView->GetCommander().HandleCommand(Index, true, (LPARAM)Argument[0], ArgLengths[0],
													 (LPARAM)(Argument[1] != NULL ? _wtoi(Argument[1]) : 0), 0);
		}
		break;
	...
	case F_INSFILE: // 2006.12.10 キャレット位置にファイル挿入
		...
		if (Argument[0] == NULL)
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO08));
			return false;
		}
		{
			pcEditView->GetCommander().HandleCommand(Index, true, (LPARAM)Argument[0],
													 (LPARAM)(Argument[1] != NULL ? _wtoi(Argument[1]) : 0),
													 (LPARAM)(Argument[2] != NULL ? _wtoi(Argument[2]) : 0), 0);
		}
		break;
	...
	case F_REPLACE_ALL:
		...
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO09));
			return false;
		}
		if (Argument[1] == NULL)
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO10));
			return false;
		}
		{
			...
			{ CSearchKeywordManager().AddToReplaceKeyArr(Argument[1]); } cDlgReplace.m_strText2 = Argument[1];
			...
			if (LOWORD(Index) == F_REPLACE)
			{ // 2007.07.08 genta コマンドは下位ワード
				//	置換する時は選べない
				cDlgReplace.m_bSelectedArea = 0;
			}
			else if (LOWORD(Index) == F_REPLACE_ALL)
			{ // 2007.07.08 genta コマンドは下位ワード
				//	全置換の時は選べる？
				cDlgReplace.m_bSelectedArea = lFlag & 0x80 ? 1 : 0;
			}
			cDlgReplace.m_nReplaceTarget =
				(lFlag >> 8) & 0x03; //	8bitシフト（0x100で割り算）	// 2007.01.16 ryoji 下位 2bitだけ取り出す
			if (bAddHistory)
			{
	...
	case F_GREP:
		...
		//		0x080000	フォルダ毎に表示
		{
			if (Argument[0] == NULL)
			{
				::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO11));
				return false;
			}
			int  ArgIndex	 = 0;
			...
			if (bGrepReplace)
			{
				...
				if (Argument[1] == NULL)
				{
					::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO17));
					return false;
				}
			}
			if (Argument[ArgIndex + 1] == NULL)
			{
				::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO12));
				return false;
			}
			if (Argument[ArgIndex + 2] == NULL)
			{
				::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO13));
				return false;
			}
			//	常に外部ウィンドウに。
			/*======= Grepの実行 =============*/
			/* Grep結果ウィンドウの表示 */
			CNativeW cmWork1;
			cmWork1.SetString(Argument[0]);
			cmWork1.Replace(L"\"", L"\"\""); //	検索文字列
			CNativeW cmWork4;
			if (bGrepReplace)
			{
				cmWork4.SetString(Argument[1]);
				cmWork4.Replace(L"\"", L"\"\""); //	置換後
			}
			CNativeW cmWork2;
			cmWork2.SetString(Argument[ArgIndex + 1]);
			cmWork2.Replace(L"\"", L"\"\""); //	ファイル名
			CNativeW cmWork3;
			cmWork3.SetString(Argument[ArgIndex + 2]);
			cmWork3.Replace(L"\"", L"\"\""); //	フォルダ名
			...
			ECodeType nCharSet;
			{
				...
				if (IsValidCodeTypeExceptSJIS(nCode) || CODE_AUTODETECT == nCode) { nCharSet = (ECodeType)nCode; }
				// 2013.06.11 5番目の引き数を文字コードにする
				if (ArgIndex + 5 <= ArgSize) { nCharSet = (ECodeType)_wtoi(Argument[ArgIndex + 4]); }
			}
			...
			if (0x20 == (lFlag & 0x400020))
				wcscat(pOpt, L"P"); // 行を出力する
			else if (0x400000 == (lFlag & 0x400020))
				wcscat(pOpt, L"N"); // 否ヒット行を出力する
			if (0x40 == (lFlag & 0xC0))
				wcscat(pOpt, L"2"); /* Grep: 出力形式 */
			else if (0x80 == (lFlag & 0xC0))
				wcscat(pOpt, L"3");
			else
				wcscat(pOpt, L"1");
			...
			CControlTray::OpenNewEditor(G_AppInstance(), pcEditView->GetHwnd(), sLoadInfo, cCmdLine.GetStringPtr());
			/*======= Grepの実行 =============*/
			/* Grep結果ウィンドウの表示 */
		}
		break;
	case F_FILEOPEN2:
		...
		if (Argument[0] == NULL)
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO14));
			return false;
		}
		{
			...
			pcEditView->GetCommander().HandleCommand(Index, true, (LPARAM)Argument[0], (LPARAM)nCharCode,
													 (LPARAM)nViewMode, (LPARAM)pDefFileName);
		}
		break;
	...
	case F_FILESAVEAS:
		...
		if (LOWORD(Index) == F_FILESAVEAS && (Argument[0] == NULL || L'\0' == Argument[0][0]))
		{
			...
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO15));
			return false;
		}
		{
			// 文字コードセット
			...
			if (Argument[1] != NULL) { nCharCode = (ECodeType)_wtoi(Argument[1]); }
			if (LOWORD(Index) == F_FILESAVEAS && IsValidCodeOrCPType(nCharCode)
				&& nCharCode != pcEditView->m_pcEditDoc->GetDocumentEncoding())
			{
				//	From Here Jul. 26, 2003 ryoji BOM状態を初期化
				pcEditView->m_pcEditDoc->SetDocumentEncoding(
					nCharCode, CCodeTypeName(pcEditView->m_pcEditDoc->GetDocumentEncoding()).IsBomDefOn());
				//	To Here Jul. 26, 2003 ryoji BOM状態を初期化
			}

			// 改行コード
			int nSaveLineCode = 0; //デフォルト値	//Sep. 11, 2004 genta 初期値を「変更しない」に
			if (Argument[2] != NULL) { nSaveLineCode = _wtoi(Argument[2]); }
			EEolType eEol;
			...
			pcEditView->GetCommander().HandleCommand(Index, true, (LPARAM)Argument[0], (LPARAM)nCharCode, (LPARAM)eEol,
													 0);
		}
		break;
	/* 2つの引数が文字列 */
	...
	{
		int noconfirm = 0;
		if (Argument[0] != NULL) { noconfirm = (_wtoi(Argument[0]) != 0); }
	...
	case F_NEXTWINDOW:
	case F_PREVWINDOW:
		pcEditView->GetDocument()->HandleCommand(
			Index); // 2009.04.11 ryoji F_NEXTWINDOW/F_PREVWINDOWが動作しなかったのを修正
		break;
	...
	case F_MOVECURSOR:
	{
		...
		{
			::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, EXEC_ERROR_TITLE, LS(STR_ERR_DLGMACRO16));
			return false;
		}
	}
	break;
	...
	case F_STATUSMSG:
	{
		...
		int			 val1 = Argument[1] != NULL ? _wtoi(Argument[1]) : 0;
		if ((val1 & 0x03) == 0) { pcEditView->SendStatusMessage(val0.c_str()); }
		else if ((val1 & 0x03) == 1)
		{
			if (NULL != pcEditView->m_pcEditWnd->m_cStatusBar.GetStatusHwnd())
			{ pcEditView->SendStatusMessage(val0.c_str()); } else
			{
	...
	break;
	case F_SETUNDOBUFFER: { pcEditView->SetUndoBuffer();
	}
	break;
	case F_APPENDUNDOBUFFERCURSOR:
	{
		...
		opeBlk->AddRef();
		opeBlk->AppendOpe(new CMoveCaretOpe(pcEditView->GetCaret().GetCaretLogicPos() // 操作前後のキャレット位置
											));
		pcEditView->SetUndoBuffer();
	}
	break;
	...
	case F_SETVIEWTOP:
	{
		...
		if (ArgLengths[0] <= 0) { return false; }
		if (!WCODE::Is09(Argument[0][0])) { return false; }
		CLayoutYInt nLineNum = CLayoutYInt(_wtoi(Argument[0]) - 1);
		if (nLineNum < 0) { nLineNum = CLayoutYInt(0); }
	...
	case F_SETVIEWLEFT:
	{
		...
		if (nColumn < 0) { nColumn = CLayoutXInt(0); }
...
bool CMacro::HandleFunction(CEditView *View, EFunctionCode ID, const VARIANT *Arguments, int ArgSize, VARIANT &Result)
{
	...
	case F_EXPANDPARAMETER:
		// 2003.02.24 Moca
		{
			...
			if (VariantChangeType(&varCopy.Data, const_cast<VARIANTARG *>(&(Arguments[0])), 0, VT_BSTR) != S_OK)
	...
	case F_GETLINESTR:
		//	2003.06.01 Moca マクロ追加
		{
			...
			if (-1 < varCopy.Data.lVal)
			{
				...
				CLogicInt	  nLength;
				CLogicInt	  nLine;
				if (0 == varCopy.Data.lVal) { nLine = View->GetCaret().GetCaretLogicPos().GetY2(); }
				else
				{
	...
	case F_CHGTABWIDTH:
		//	2004.03.16 zenryaku マクロ追加
		{
			...
			if (0 < varCopy.Data.iVal && nTab != varCopy.Data.iVal)
			{
				...
				View->m_pcEditWnd->ChangeLayoutParam(false, CKetaXInt(varCopy.Data.iVal),
	...
	case F_GETLINECODE:
		//	2005.08.04 maru マクロ追加
		{
			...
			{
			case EOL_CRLF: n = 0; break;
			case EOL_CR: n = 1; break;
			case EOL_LF: n = 2; break;
			case EOL_NEL: n = 3; break;
			case EOL_LS: n = 4; break;
			case EOL_PS: n = 5; break;
			}
	...
	case F_CHGWRAPCOLUMN:
		//	2008.06.19 ryoji マクロ追加
		{
			...
			if (varCopy.Data.iVal < MINLINEKETAS || varCopy.Data.iVal > MAXLINEKETAS) return true;
			View->m_pcEditDoc->m_nTextWrapMethodCur = WRAP_SETTING_WIDTH;
			View->m_pcEditDoc->m_bTextWrapMethodCurTemp =
				!(View->m_pcEditDoc->m_nTextWrapMethodCur
				  == View->m_pcEditDoc->m_cDocType.GetDocumentAttribute().m_nTextWrapMethod);
			View->m_pcEditWnd->ChangeLayoutParam(false, View->m_pcEditDoc->m_cLayoutMgr.GetTabSpaceKetas(),
												 View->m_pcEditDoc->m_cLayoutMgr.m_tsvInfo.m_nTsvMode,
	...
	case F_ISSAMETYPEEXT:
		//	2006.09.04 ryoji ２つの拡張子が同じタイプ別設定に含まれているかどうかを調べる
		{
			...
			if (VariantChangeType(&varCopy.Data, const_cast<VARIANTARG *>(&(Arguments[1])), 0, VT_BSTR) != S_OK)
				return false; // VT_BSTRとして解釈
			Wrap(&varCopy.Data.bstrVal)->GetW(&Source, &SourceLength);
	...
	case F_INPUTBOX:
		//	2011.03.18 syat テキスト入力ダイアログの表示
		{
			...
			if (ArgSize >= 3)
			{
				if (VariantChangeType(&varCopy.Data, const_cast<VARIANTARG *>(&(Arguments[2])), 0, VT_I4) != S_OK)
					return false;			   // VT_I4として解釈
				nMaxLen = varCopy.Data.intVal; // 最大入力長
				if (nMaxLen <= 0) { nMaxLen = _MAX_PATH; }
			}
			...
			if (cDlgInput1.DoModal(G_AppInstance(), View->GetHwnd(), L"sakura macro", sMessage.c_str(), nMaxLen,
								   Buffer))
			{
				SysString S(Buffer, wcslen(Buffer));
				Wrap(&Result)->Receive(S);
	...
	case F_YESNOBOX:	// メッセージボックス（確認：はい／いいえ）の表示
		//	2011.03.18 syat メッセージボックスの表示
		{
			...
			case F_MESSAGEBOX:
				if (ArgSize >= 2)
				{
					if (VariantChangeType(&varCopy.Data, const_cast<VARIANTARG *>(&(Arguments[1])), 0, VT_I4) != S_OK)
						return false; // VT_I4として解釈
				...
				break;
			case F_ERRORMSG: uType |= MB_OK | MB_ICONSTOP; break;
			case F_WARNMSG: uType |= MB_OK | MB_ICONEXCLAMATION; break;
			case F_INFOMSG: uType |= MB_OK | MB_ICONINFORMATION; break;
			case F_OKCANCELBOX: uType |= MB_OKCANCEL | MB_ICONQUESTION; break;
			case F_YESNOBOX: uType |= MB_YESNO | MB_ICONQUESTION; break;
			}
	...
	case F_MACROSLEEP:
		//	2011.03.18 syat 指定した時間（ミリ秒）停止する
		{
			...
			if (VariantChangeType(&varCopy.Data, const_cast<VARIANTARG *>(&(Arguments[0])), 0, VT_UI4) != S_OK)
	...
	case F_FILESAVEDIALOG:
		//	2011.03.18 syat ファイルダイアログの表示
		{
			...
			cDlgOpenFile.Create(G_AppInstance(), View->GetHwnd(), sFilter.c_str(), sDefault.c_str());
			bool  bRet;
			WCHAR szPath[_MAX_PATH];
			wcscpy(szPath, sDefault.c_str());
			if (LOWORD(ID) == F_FILEOPENDIALOG) { bRet = cDlgOpenFile.DoModal_GetOpenFileName(szPath); }
			else
			{
	...
	case F_LAYOUTTOLOGICLINENUM:
		// レイアウト→ロジック行
		{
			...
			{ ret = (Int)View->m_pcEditDoc->m_cDocLineMgr.GetLineCount() + 1; } else
			{
				const CLayout *pcLayout = View->m_pcEditDoc->m_cLayoutMgr.SearchLineByLayoutY(nLineNum);
				if (pcLayout != NULL) { ret = pcLayout->GetLogicLineNo() + 1; }
				else
				{
					return false;
				}
	...
	case F_LINECOLUMNTOINDEX:
		// レイアウト→ロジック桁
		{
			...
			if (!VariantToI4(varCopy, Arguments[0])) { return false; }
			CLayoutInt nLineNum = CLayoutInt(varCopy.Data.lVal - 1);
			if (!VariantToI4(varCopy, Arguments[1])) { return false; }
	...
	case F_GETCOOKIE:
	{
		...
		if (ArgSize >= 2)
		{
			if (VariantChangeType(&varCopy.Data, const_cast<VARIANTARG *>(&(Arguments[0])), 0, VT_BSTR) != S_OK)
				return false;
			if (VariantChangeType(&varCopy2.Data, const_cast<VARIANTARG *>(&(Arguments[1])), 0, VT_BSTR) != S_OK)
				return false;
	...
	case F_GETCOOKIEDEFAULT:
	{
		...
		if (ArgSize >= 3)
		{
			...
			if (VariantChangeType(&varCopy2.Data, const_cast<VARIANTARG *>(&(Arguments[1])), 0, VT_BSTR) != S_OK)
				return false;
			if (VariantChangeType(&varCopy3.Data, const_cast<VARIANTARG *>(&(Arguments[2])), 0, VT_BSTR) != S_OK)
				return false;
	...
	case F_SETCOOKIE:
	{
		...
		if (ArgSize >= 3)
		{
			...
			if (VariantChangeType(&varCopy3.Data, const_cast<VARIANTARG *>(&(Arguments[2])), 0, VT_BSTR) != S_OK)
				return false;
			int ret =
				View->GetDocument()->m_cCookie.SetCookie(varCopy.Data.bstrVal, varCopy2.Data.bstrVal,
	...
	case F_GETSTRWIDTH:
	{
		...
		if (1 <= ArgSize)
		{
			...
			if (2 <= nLen)
			{
				if (pLine[nLen - 2] == WCODE::CR && pLine[nLen - 1] == WCODE::LF) { nLen--; }
			}
			const int nTabWidth = (Int)View->GetDocument()->m_cLayoutMgr.GetTabSpaceKetas();
			int		  nPosX		= varCopy2.Data.lVal - 1;
			for (int i = 0; i < nLen;)
			{
				if (pLine[i] == WCODE::TAB) { nPosX += nTabWidth - (nPosX % nTabWidth); }
				else
				{
	...
	case F_GETSTRLAYOUTLENGTH:
	{
		...
		if (1 <= ArgSize)
		{
			...
			const CLayout tmpLayout(&tmpDocLine, CLogicPoint(0, 0), CLogicXInt(tmpLenWithEol1), COLORIDX_TEXT, offset,
									NULL);
	...
	case F_SETCLIPBOARDBYFORMAT:
	{
		...
		if (3 <= ArgSize)
		{
			...
			bool	   bret =
				cClipboard.SetClipboradByFormat(cstr, varCopy2.Data.bstrVal, varCopy3.Data.lVal, varCopy4.Data.lVal);
			Wrap(&Result)->Receive(bret ? 1 : 0);
			return true;
		}
		return false;
	...
	case F_GETLINEATTRIBUTE:
	{
		...
		CLogicInt nLine;
		if (0 == nLineNum) { nLine = View->GetCaret().GetCaretLogicPos().GetY2(); }
		else if (nLineNum < 0)
		{
			return false;
		}
		...
		if (pcDocLine == NULL) { return false; }
		int nRet;
		switch (nAttType)
		{
		case 0:
			nRet = (CModifyVisitor().IsLineModified(pcDocLine,
													View->GetDocument()->m_cDocEditor.m_cOpeBuf.GetNoModifiedSeq())
						? 1
						: 0);
			break;
		case 1: nRet = pcDocLine->m_sMark.m_cModified.GetSeq(); break;
		case 2: nRet = (pcDocLine->m_sMark.m_cBookmarked ? 1 : 0); break;
		case 3: nRet = pcDocLine->m_sMark.m_cDiffmarked; break;
		case 4: nRet = (pcDocLine->m_sMark.m_cFuncList.GetFuncListMark() ? 1 : 0); break;
		...
		if (2 <= ArgSize)
		{
			...
			while (p = my_strtok(&vStrMenu[0], nLen, &nPos, L","))
			{
				...
				if (p[0] == L'[')
				{
					...
					while (*r != L']' && *r != L'\0')
					{
						...
						case L'S':
							...
							break;
						case L'E': nBreakNum++; break;
						case L'C': nFlags |= MF_CHECKED; break;
						case L'D': nFlags |= MF_GRAYED; break;
						case L'R': bRadio = true; break;
						case L'B': nFlagBreak |= MF_MENUBARBREAK; break;
						default: break;
						}
						r++;
					}
				...
				else
				{
					...
					if (bRadio) { ::CheckMenuRadioItem(hMenuCurrent, i, i, i, MF_BYCOMMAND); }
					i++;
				}
				for (int n = 0; n < nBreakNum; n++)
				{
					if (1 < vHmenu.size()) { vHmenu.resize(vHmenu.size() - 1); }
					hMenuCurrent = vHmenu.back();
				}
			}
			POINT pt;
			...
			if (nViewPointType == 1)
			{
				...
				{ ::ClientToScreen(View->GetHwnd(), &pt); } else
				{
					::GetCursorPos(&pt);
				}
			}
			...
			int nId = ::TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON | TPM_RETURNCMD,
									   ...
									   (pt.y < rcWork.bottom) ? pt.y : rcWork.bottom, 0, View->GetHwnd(), NULL);
			::DestroyMenu(hMenu);
