    ...
    switch (LOWORD(Index))
    {
        ...
        case F_GREP:
            ...
            //		0x080000	フォルダ毎に表示
            {
                ...
                pOpt[0] = '\0';
                if (lFlag & 0x01)
                    wcscat(pOpt, L"S"); /* サブフォルダからも検索する */
                if (lFlag & 0x04)
                    wcscat(pOpt, L"L"); /* 英大文字と英小文字を区別する */
                if (lFlag & 0x08)
                    wcscat(pOpt, L"R"); /* 正規表現 */
        ...
        case F_MSGBEEP:
        {
            ...
            switch (val0)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    val0 = MB_ICONQUESTION;
                    break;
                case 3:
                    val0 = MB_ICONWARNING;
                    break;
                case 4:
                    val0 = MB_ICONINFORMATION;
                    break;
                default:
                    val0 = MB_OK;
                    break;
...
bool CMacro::HandleFunction(CEditView *View, EFunctionCode ID, const VARIANT *Arguments, int ArgSize, VARIANT &Result)
{
    ...
    switch (LOWORD(ID))
    {
        ...
        case F_GETSELECTED:
        {
            ...
            {
                CNativeW cMem;
                if (!View->GetSelectedDataSimple(cMem))
                    return false;
        ...
        case F_FILESAVEDIALOG:
            //	2011.03.18 syat ファイルダイアログの表示
            {
                ...
                cDlgOpenFile.Create(
                    G_AppInstance(), View->GetHwnd(), sFilter.c_str(), sDefault.c_str());
                bool bRet;
        ...
        case F_GETCOOKIEDEFAULT:
        {
            ...
            if (ArgSize >= 3)
            {
                ...
                if (VariantChangeType(&varCopy3.Data, const_cast<VARIANTARG *>(&(Arguments[2])), 0, VT_BSTR) != S_OK)
                    return false;
                SysString ret = View->GetDocument()->m_cCookie.GetCookieDefault(varCopy.Data.bstrVal, varCopy2.Data.bstrVal, varCopy3.Data.bstrVal, SysStringLen(varCopy3.Data.bstrVal));
                Wrap(&Result)->Receive(ret);
                return true;
            }
            return false;
        ...
        case F_SETCOOKIE:
        {
            ...
            if (ArgSize >= 3)
            {
                ...
                if (VariantChangeType(&varCopy3.Data, const_cast<VARIANTARG *>(&(Arguments[2])), 0, VT_BSTR) != S_OK)
                    return false;
                int ret = View->GetDocument()->m_cCookie.SetCookie(varCopy.Data.bstrVal, varCopy2.Data.bstrVal, varCopy3.Data.bstrVal, SysStringLen(varCopy3.Data.bstrVal));
                Wrap(&Result)->Receive(ret);
                return true;
            }
            return false;
        ...
        case F_GETSTRWIDTH:
        {
            ...
            if (1 <= ArgSize)
            {
                if (!VariantToBStr(varCopy, Arguments[0]))
                {
                    return false;
                }
                if (2 <= ArgSize)
                {
                    if (!VariantToI4(varCopy2, Arguments[1]))
                    {
                        return false;
                    }
                }
        ...
        case F_GETCLIPBOARDBYFORMAT:
        {
            ...
            if (2 <= ArgSize)
            {
                ...
                if (3 <= ArgSize)
                {
                    if (!VariantToI4(varCopy3, Arguments[2]))
                        return false;
                }
                else
                {
        ...
        case F_SETCLIPBOARDBYFORMAT:
        {
            ...
            if (3 <= ArgSize)
            {
                if (!VariantToBStr(varCopy, Arguments[0]))
                    return false;
                if (!VariantToBStr(varCopy2, Arguments[1]))
                    return false;
                if (!VariantToI4(varCopy3, Arguments[2]))
                    return false;
                if (3 <= ArgSize)
                {
                    if (!VariantToI4(varCopy4, Arguments[3]))
                        return false;
                }
                else
                {
        ...
        case F_GETLINEATTRIBUTE:
        {
            ...
            if (!variant_to_int(Arguments[0], nLineNum))
                return false;
            if (!variant_to_int(Arguments[1], nAttType))
                return false;
            CLogicInt nLine;
            if (0 == nLineNum)
            {
        ...
        case F_CREATEMENU:
        {
            ...
            if (2 <= ArgSize)
            {
                ...
                if (nViewPointType == 1)
                {
                    ...
                    if (View->GetTextArea().GetAreaLeft() <= pt.x && View->GetTextArea().GetAreaTop() <= pt.y && pt.x < View->GetTextArea().GetAreaRight() && pt.y < View->GetTextArea().GetAreaBottom())
                    {
                ...
                int nId = ::TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON | TPM_RETURNCMD, (pt.x > rcWork.left) ? pt.x : rcWork.left, (pt.y < rcWork.bottom) ? pt.y : rcWork.bottom, 0, View->GetHwnd(), NULL);
                ::DestroyMenu(hMenu);
