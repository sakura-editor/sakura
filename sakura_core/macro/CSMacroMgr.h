...
public:
	...
	*/
	...
	{
		return (0 <= idx && idx < MAX_CUSTMACRO) ? m_pShareData->m_Common.m_sMacro.m_MacroTable[idx].IsEnabled()
												 : false;
	}
	...
	{
		return (0 <= idx && idx < MAX_CUSTMACRO) ? m_pShareData->m_Common.m_sMacro.m_MacroTable[idx].GetTitle()
												 : NULL; // 2007.11.02 ryoji
	}

	//!	表示名の取得
	const WCHAR *GetName(int idx) const
	{
		return (0 <= idx && idx < MAX_CUSTMACRO) ? m_pShareData->m_Common.m_sMacro.m_MacroTable[idx].m_szName : NULL;
	}
	...
	*/
	const WCHAR *GetFile(int idx) const
	{
		return (0 <= idx && idx < MAX_CUSTMACRO)
				   ...
				   : ((idx == STAND_KEYMACRO || idx == TEMP_KEYMACRO) && m_sMacroPath != L"") ? m_sMacroPath.c_str()
																							  : NULL;
	}
	...
	||  Attributes & Operations
	*/
	static WCHAR *		 GetFuncInfoByID(HINSTANCE hInstance, int nFuncID, WCHAR *pszFuncName,
										 WCHAR *pszFuncNameJapanese); /* 機能ID→関数名，機能名日本語 */
	static EFunctionCode GetFuncInfoByName(HINSTANCE hInstance, const WCHAR *pszFuncName,
										   WCHAR *pszFuncNameJapanese); /* 関数名→機能ID，機能名日本語 */
