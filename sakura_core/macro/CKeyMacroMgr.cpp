...
*/
void CKeyMacroMgr::Append(EFunctionCode nFuncID, const LPARAM *lParams, CEditView *pcEditView)
{
...
BOOL CKeyMacroMgr::LoadKeyMacro(HINSTANCE hInstance, const WCHAR *pszPath)
{
	...
	for (; in.Good(); ++line)
	{
		...
		{
			if (szLine[i] != SPACE && szLine[i] != TAB) { break; }
		}
		nBgn = i;
		//	Jun. 16, 2002 genta 空行を無視する
		if (nBgn == nLineLen || szLine[nBgn] == LTEXT('\0')) { continue; }
		// コメント行の検出
		//# パフォーマンス：'/'のときだけ２文字目をテスト
		if (szLine[nBgn] == LTEXT('/') && nBgn + 1 < nLineLen && szLine[nBgn + 1] == LTEXT('/')) { continue; }
		...
		if (-1 != nFuncID)
		{
			...
			if (mInfo->m_pData) { nArgSizeMax = mInfo->m_pData->m_nArgMaxSize; }
			for (nArgs = 0; szLine[i]; ++nArgs)
			{
				// Jun. 16, 2002 genta プロトタイプチェック
				if (nArgs >= nArgSizeMax)
				{
					::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, MACRO_ERROR_TITLE, LS(STR_ERR_DLGKEYMACMGR3),
								   line, i + 1);
					m_nReady = false;
				}
				...
				{ //	'で始まったら文字列だよきっと。
					...
					{ //	最後の文字+1までスキャン
						...
						{ //	行末に来てしまった
							::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, MACRO_ERROR_TITLE,
										   LS(STR_ERR_DLGKEYMACMGR5), line, szFuncName, nArgs + 1, cQuote);
							m_nReady = false;
					...
					cmemWork.Replace(L"\\t", L"\t");
					{
						...
						for (size_t n = 0; n < nLen; n++)
						{
							if (n + 1 < nLen && p[n] == L'\\' && p[n + 1] == L'u')
							{
								...
								for (k = n + 2; k < nLen && k < n + 2 + 4
												&& (WCODE::Is09(p[k]) || (L'a' <= p[k] && p[k] <= L'f')
													|| (L'A' <= p[k] && p[k] <= L'F'));
									 k++)
						...
						if (nBegin != 0)
						{
							if (0 < nLen - nBegin) { cmemTemp.AppendString(p + nBegin, nLen - nBegin); }
				...
				else if (Is09(szLine[i]) || szLine[i] == L'-')
				{ //	数字で始まったら数字列だ(-記号も含む)。
					// Jun. 16, 2002 genta プロトタイプチェック
					// Jun. 27, 2002 genta 余分な引数を無視するよう，VT_EMPTYを許容する．
					...
					{
						::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, MACRO_ERROR_TITLE,
									   LS(STR_ERR_DLGKEYMACMGR6), line, i + 1, szFuncName, nArgs + 1);
						m_nReady = false;
						break;
					}
					nBgn = nEnd = i; //	nBgnは引数の先頭の文字
					//	行末の検出のため，ループ回数を1増やした
					for (i = nBgn + 1; i <= nLineLen; ++i)
					{ //	最後の文字+1までスキャン
						if (Is09(szLine[i]))
						{ // まだ数値
						  //							++i;
				...
				else
				{
					...
					//	Jun. 16, 2002 genta
					nBgn = nEnd = i;
					::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, MACRO_ERROR_TITLE, LS(STR_ERR_DLGKEYMACMGR7),
								   line, i + 1);
					m_nReady = false;
					break;
				}
				...
				for (; i < nLineLen; ++i)
				{ //	最後の文字までスキャン
					if (szLine[i] == LTEXT(')') || szLine[i] == LTEXT(','))
					{ //	,もしくは)を読み飛ばす
						i++;
						break;
					}
				}
				if (szLine[i - 1] == LTEXT(')')) { break; }
			}
			//	Jun. 16, 2002 genta
			if (!m_nReady)
			{
...
{
	if (wcscmp(ext, L"mac") == 0) { return new CKeyMacroMgr; }
	return NULL;
}
