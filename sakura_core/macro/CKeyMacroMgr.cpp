...
BOOL CKeyMacroMgr::LoadKeyMacro(HINSTANCE hInstance, const WCHAR *pszPath)
{
    ...
    for (; in.Good(); ++line)
    {
        ...
        if (-1 != nFuncID)
        {
            ...
            for (nArgs = 0; szLine[i]; ++nArgs)
            {
                ...
                { //	'で始まったら文字列だよきっと。
                    ...
                    cmemWork.Replace(L"\\t", L"\t");
                    {
                        ...
                        for (size_t n = 0; n < nLen; n++)
                        {
                            ...
                            {
                                size_t k;
                                for (k = n + 2; k < nLen && k < n + 2 + 4 && (WCODE::Is09(p[k]) || (L'a' <= p[k] && p[k] <= L'f') || (L'A' <= p[k] && p[k] <= L'F')); k++)
                                {
                                }
                ...
                else
                {
                    ...
                    //	Jun. 16, 2002 genta
                    nBgn = nEnd = i;
                    ::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, MACRO_ERROR_TITLE, LS(STR_ERR_DLGKEYMACMGR7), line, i + 1);
                    m_nReady = false;
                    break;
                }
        ...
        {
            ::MYMESSAGEBOX(NULL, MB_OK | MB_ICONSTOP | MB_TOPMOST, MACRO_ERROR_TITLE, LS(STR_ERR_DLGKEYMACMGR8), line, szFuncName);
            //	Jun. 16, 2002 genta
            m_nReady = false;
            break;
        }
...
BOOL CKeyMacroMgr::LoadKeyMacroStr(HINSTANCE hInstance, const WCHAR *pszCode)
{
    ...
    if (0 == ::GetTempPath(_MAX_PATH, szTempDir))
        return FALSE;
    if (0 == ::GetTempFileName(szTempDir, L"mac", 0, szTempFile))
        return FALSE;
    // 一時ファイルに書き込む
