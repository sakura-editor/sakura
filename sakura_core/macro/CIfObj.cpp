...
public:
		...
		/* [out] */ BSTR __RPC_FAR *pBstrHelpFile)
	{
		...
		if (pBstrHelpFile) { *pBstrHelpFile = SysAllocString(L""); }
		return S_OK;
	}
...
#endif
	*pcNames = 1;
	if (cMaxNames > 0) *rgBstrNames = SysAllocString(m_MethodsRef[memid].Name);
	return S_OK;
}
...
CIfObj::~CIfObj()
{
	if (m_TypeInfo != NULL) m_TypeInfo->Release();
}
...
// IDispatch実装
HRESULT STDMETHODCALLTYPE CIfObj::Invoke(DISPID dispidMember, REFIID riid, LCID lcid, WORD wFlags,
										 DISPPARAMS FAR *pdispparams, VARIANT FAR *pvarResult,
										 EXCEPINFO FAR *pexcepinfo, UINT FAR *puArgErr)
{
	if ((unsigned)dispidMember < m_Methods.size())
		return (this->*(m_Methods[dispidMember].Method))(m_Methods[dispidMember].ID, pdispparams, pvarResult,
														 m_Owner->GetData());
	else
		return E_UNEXPECTED;
}
...
//型情報にメソッドを追加する
void CIfObj::AddMethod(const wchar_t *Name, int ID, VARTYPE *ArgumentTypes, int ArgumentCount, VARTYPE ResultType,
					   CIfObjMethod Method)
{
	/*
