...
LPCWSTR CPPA::GetDllNameImp(int nIndex) { return L"PPA.DLL"; }
...
{
	if (false != m_CurInstance->m_bError) { return; }
	...
	else
	{
		...
		else
		{
			...
			case 0:
				...
				break;
			default: auto_sprintf(szMes, LS(STR_ERR_DLGPPA5), Err_CD, to_wchar(Err_Mes));
			}
		}
	}
...
*/
void __stdcall CPPA::stdProc(const char *FuncName, const int _Index, const char *Argument[], const int ArgSize,
							 int *Err_CD)
{
	...
	*Err_CD = 0;
	...
	//処理
	bool bRet =
		CMacro::HandleCommand(m_CurInstance->m_pcEditView, (EFunctionCode)(Index | m_CurInstance->m_commandflags),
							  tmpArguments, tmpArgLengths, ArgSize);
...
*/
void __stdcall CPPA::stdIntFunc(const char *FuncName, const int Index, const char *Argument[], const int ArgSize,
								int *Err_CD, int *ResultValue)
{
	NEVER_USED_PARAM(FuncName);
	VARIANT Ret;
	::VariantInit(&Ret);
	...
	*Err_CD		 = 0;
	if (false != CallHandleFunction(Index, Argument, ArgSize, &Ret))
	{
		...
		default: *Err_CD = -2; // 2003.06.01 Moca 値変更
		}
		::VariantClear(&Ret);
		return;
	}
...
*/
void __stdcall CPPA::stdStrFunc(const char *FuncName, const int Index, const char *Argument[], const int ArgSize,
...
/*!
	...
	@author Moca
*/
bool CPPA::CallHandleFunction(const int Index, const char *Arg[], int ArgSize, VARIANT *Result)
{
	int					 i, ArgCnt;
