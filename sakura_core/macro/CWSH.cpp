...
#ifdef USE_JSCRIPT9
const GUID CLSID_JSScript9 = {0x16d51579, 0xa30b, 0x4c8b, {0xa2, 0x76, 0x0f, 0xf4, 0xdc, 0x41, 0xe7, 0x55}};
#endif
		...
		/* [out] */ ITypeInfo **ppti)
	{
		...
		for (CWSHClient::ListIter it = objects.begin(); it != objects.end(); it++) {
			...
			if (_wcsicmp(pstrName, (*it)->m_sName.c_str()) == 0) {
				...
				if (dwReturnMask & SCRIPTINFO_ITYPEINFO) { (*it)->GetTypeInfo(0, 0, ppti); }
				return S_OK;
			}
		}
...
{
	//インタフェースオブジェクトを解放
	for (ListIter it = m_IfObjArr.begin(); it != m_IfObjArr.end(); it++) { (*it)->Release(); }
...
static unsigned __stdcall AbortMacroProc(LPVOID lpParameter)
{
	...
	if (::WaitForSingleObject(pParam->hEvent, pParam->nCancelTimer * 1000) == WAIT_TIMEOUT) {
		...
		HWND	   hwndDlg = cDlgCancel.DoModeless(G_AppInstance(), NULL,
											   IDD_MACRORUNNING); // エディタビジーでも表示できるよう、親を指定しない
		// ダイアログタイトルとファイル名を設定
		::SendMessage(hwndDlg, WM_SETTEXT, 0, (LPARAM)GSTR_APPNAME);
		::SendMessage(GetDlgItem(hwndDlg, IDC_STATIC_CMD), WM_SETTEXT, 0,
...
{
	if (m_OnError != NULL) m_OnError(Description, Source, m_Data);
}
