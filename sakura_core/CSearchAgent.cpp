    ...
    const CSearchStringPattern &pattern)
{
    ...
    else
    {
        ...
        {
            int n = bLoHiCase ? wmemcmp(&pLine[nPos], pszPattern, nPatternLen) : wmemicmp(&pLine[nPos], pszPattern, nPatternLen);
            if (n == 0)
            {
                return &pLine[nPos];
            }
    ...
    int nPatternLen)
{
    for (CLogicInt pos = CLogicInt(0); pos < nPatternLen;)
    {
        ...
        if (CWordParse::WhereCurrentWord_2(pszPattern + pos, nPatternLen - pos, CLogicInt(0), &begin, &end, NULL, NULL) && begin == 0 && begin < end)
        {
    ...
    for (; i >= pArg->sDelRange.GetFrom().y && NULL != pCDocLine; i--)
    {
        ...
        else
        {
            ...
            { // 20020119 aroka ブロック内に pWork を閉じ込めた
                ...
                if (pCDocLine->_GetDocLineData().capacity() * 9 / 10 < nNewLen && nNewLen <= pCDocLine->_GetDocLineData().capacity())
                {
                    CNativeW &ref = pCDocLine->_GetDocLineData();
    ...
    for (nCount = 0; nCount < nInsSize; nCount++)
    {
...
#endif
        {
            ...
            else
            {
                ...
                {
                    if (cmemCurLine.GetStringLength() - cPrevLine.GetLength() < cmemCurLine.GetStringLength() / 100 && cPrevLine.GetLength() + cmemLine.GetStringLength() <= cmemCurLine.GetStringLength() && cmemCurLine.capacity() / 2 <= cPrevLine.GetLength() + cmemLine.GetStringLength())
                    {
