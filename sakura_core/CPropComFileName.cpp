...
void CPropCommon::SetData_PROP_FILENAME( HWND hwndDlg )
{
	...
	// リストにデータをセット
	for( i = 0, nIndex = 0; i < m_Common.m_nTransformFileNameArrNum; i++ ){
		if( '\0' == m_Common.m_szTransformFileNameFrom[i][0] ) continue;

		::ZeroMemory( &lvItem, sizeof( lvItem ));
		lvItem.mask     = LVIF_TEXT;
		lvItem.iItem    = nIndex;
		lvItem.iSubItem = 0;
		lvItem.pszText  = m_Common.m_szTransformFileNameFrom[i];
		...
		lvItem.iItem    = nIndex;
		lvItem.iSubItem = 1;
		lvItem.pszText  = m_Common.m_szTransformFileNameTo[i];
...
//	}
	...
	// ファイル名置換リスト
	HWND hListView = ::GetDlgItem( hwndDlg, IDC_LIST_FNAME );
	m_Common.m_nTransformFileNameArrNum = ListView_GetItemCount( hListView );

	for( nIndex = 0, nCount = 0; nIndex < MAX_TRANSFORM_FILENAME ; ++nIndex ){
		if( nIndex < m_Common.m_nTransformFileNameArrNum ){
			ListView_GetItemText( hListView, nIndex, 0, m_Common.m_szTransformFileNameFrom[nCount], _MAX_PATH );

			// 置換前文字列がNULLだったら捨てる
			if( '\0' == m_Common.m_szTransformFileNameFrom[nCount][0] ){
				m_Common.m_szTransformFileNameTo[nIndex][0] = '\0';
			}else{
				ListView_GetItemText( hListView, nIndex, 1, m_Common.m_szTransformFileNameTo[nCount], _MAX_PATH );
				nCount++;
			}
		}else{
			m_Common.m_szTransformFileNameFrom[nIndex][0] = '\0';
			m_Common.m_szTransformFileNameTo[nIndex][0] = '\0';
		}
	}
