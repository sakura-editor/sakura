...
ECallbackResult CBackupAgent::OnPreBeforeSave(SSaveInfo *pSaveInfo)
{
	...
	if (GetDllShareData().m_Common.m_sBackup.m_bBackUp)
	{
		...
		case 3: //	ファイルエラー
			if (IDYES
				!= ::MYMESSAGEBOX(CEditWnd::getInstance()->GetHwnd(), MB_YESNO | MB_ICONQUESTION | MB_TOPMOST,
								  LS(STR_BACKUP_ERR_TITLE), LS(STR_BACKUP_ERR_MSG)))
			{ return CALLBACK_INTERRUPT; }
			break;
		}
	}
...
*/
int CBackupAgent::MakeBackUp(const WCHAR *target_file)
{
	int nRet;
	...
	//	ファイルがない場合と同様に何もしない
	if ((_waccess(target_file, 2)) == -1) { return 0; }
	...
	{
		int nMsgResult = ::TopConfirmMessage(CEditWnd::getInstance()->GetHwnd(), LS(STR_BACKUP_ERR_PATH_CRETE));
		if (nMsgResult == IDYES)
		{
			return 0; //	保存継続
		}
	...
	bool dustflag = false;
	if (bup_setting.m_bBackUpDustBox) { dustflag = !IsLocalDrive(szPath); }
	//@@@ 2002.03.23 end

	if (bup_setting.m_bBackUpDialog)
	{ /* バックアップの作成前に確認 */
		ConfirmBeep();
		if (bup_setting.m_bBackUpDustBox && !dustflag)
		{ //共通設定：バックアップファイルをごみ箱に放り込む	//@@@ 2001.12.11 add start MIK	//2002.03.23
			nRet =
	...
	if (bup_setting.GetBackupType() == 3 || bup_setting.GetBackupType() == 6)
	{
		//	既に存在するBackupをずらす処理
		int i;
	...
	if (::CopyFile(target_file, szPath, FALSE))
	{
		...
		if (bup_setting.m_bBackUpDustBox && !dustflag)
		{ //@@@ 2002.03.23 ネットワーク・リムーバブルドライブでない
...
/*! バックアップパスの作成
	...
	@todo Advanced modeでの世代管理
*/
bool CBackupAgent::FormatBackUpPath(WCHAR *szNewPath, size_t newPathCount, const WCHAR *target_file)
{
	WCHAR  szDrive[_MAX_DIR];
	WCHAR  szDir[_MAX_DIR];
	WCHAR  szFname[_MAX_FNAME];
	...
	if (bup_setting.m_bBackUpFolder && (!bup_setting.m_bBackUpFolderRM || !IsLocalDrive(target_file)))
	...
	if (!bup_setting.m_bBackUpPathAdvanced)
	{
		...
		{
		case 1:
			if (-1 == auto_snprintf_s(pBase, nBaseCount, L"%s.bak", szFname)) { return false; }
			break;
		case 5: //	Jun.  5, 2005 genta 1の拡張子を残す版
			if (-1 == auto_snprintf_s(pBase, nBaseCount, L"%s%s.bak", szFname, szExt)) { return false; }
			break;
		case 2: //	日付，時刻
			_tzset();
			_wstrdate(szTime);
			...
			szForm[0] = L'\0';
			if (bup_setting.GetBackupOpt(BKUP_YEAR))
			{ /* バックアップファイル名：日付の年 */
				wcscat(szForm, L"%Y");
			}
			if (bup_setting.GetBackupOpt(BKUP_MONTH))
			{ /* バックアップファイル名：日付の月 */
				wcscat(szForm, L"%m");
			}
			if (bup_setting.GetBackupOpt(BKUP_DAY))
			{ /* バックアップファイル名：日付の日 */
				wcscat(szForm, L"%d");
			}
			if (bup_setting.GetBackupOpt(BKUP_HOUR))
			{ /* バックアップファイル名：日付の時 */
				wcscat(szForm, L"%H");
			}
			if (bup_setting.GetBackupOpt(BKUP_MIN))
			{ /* バックアップファイル名：日付の分 */
				wcscat(szForm, L"%M");
			}
			if (bup_setting.GetBackupOpt(BKUP_SEC))
			{ /* バックアップファイル名：日付の秒 */
				wcscat(szForm, L"%S");
			}
			/* YYYYMMDD時分秒 形式に変換 */
			wcsftime(szTime, _countof(szTime) - 1, szForm, &result);
			if (-1 == auto_snprintf_s(pBase, nBaseCount, L"%s_%ls%s", szFname, szTime, szExt)) { return false; }
			break;
		...
		case 4: //	日付，時刻
		{
			...
			if (-1 == auto_sprintf_s(pBase, nBaseCount, L"%s_%ls%s", szFname, szTime, szExt)) { return false; }
		}
		break;
			// 2001/06/12 End
	...
	{ // 詳細設定使用する
		...
		case 4: //	ファイルの日付，時刻
		{
			...
			CFileTime ctimeLastWrite;
			GetLastWriteTimestamp(target_file, &ctimeLastWrite);
			if (!GetDateTimeFormat(szFormat, _countof(szFormat), bup_setting.m_szBackUpPathAdvanced,
								   ctimeLastWrite.GetSYSTEMTIME()))
			{ return false; }
		}
		break;
		case 2: //	現在の日付，時刻
...

		{
			...
			WCHAR *folders[10];
			{
				...
				int idx;
				for (idx = 0; idx < 10; ++idx) { folders[idx] = 0; }
				folders[0] = szFname;
		...
		}
		{
			...
			while ((cp = wcschr(szNewPath, L'?')) != NULL) { *cp = L'_'; }
		}
	}
	return true;
}
