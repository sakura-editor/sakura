...
EDiffMark CDiffLineGetter::GetLineDiffMark() const
{
    return (EDiffMark)m_pcDocLine->m_sMark.m_cDiffmarked;
}
...
void CDiffLineSetter::SetLineDiffMark(EDiffMark mark)
{
    m_pcDocLine->m_sMark.m_cDiffmarked = mark;
}
...
void CDiffLineMgr::SetDiffMarkRange(EDiffMark nMode, CLogicInt nStartLine, CLogicInt nEndLine)
{
    ...
    if (nStartLine < CLogicInt(0))
        nStartLine = CLogicInt(0);
    ...
    if (nLines <= nEndLine)
    {
        ...
        if (pCDocLine)
            CDiffLineSetter(pCDocLine).SetLineDiffMark(MARK_DIFF_DEL_EX);
    }
