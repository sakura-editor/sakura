...
/*!
	...
	@date 2011.04.10 nasukoji	新規作成
*/
LPCWSTR CSelectLang::getDefaultLangString(void) { return m_psLangInfo->szLangName; }

// 言語IDを返す
WORD CSelectLang::getDefaultLangId(void)
{
	if (m_psLangInfo == NULL) { return ::GetUserDefaultLangID(); }
...
HINSTANCE CSelectLang::InitializeLanguageEnvironment(void)
{
	...
	if (m_psLangInfoList.size() == 0)
	{
		...
		auto nCount = ::LoadString(psLangInfo->hInstance, STR_SELLANG_NAME, psLangInfo->szLangName,
								   _countof(psLangInfo->szLangName));
		assert(0 < nCount);
...
{
	if (lang.szDllName[0] == L'\0') return NULL; // DLLが指定されていなければNULLを返す
	...
	if (hInstance)
	{
		...
		if (nCount > 0)
		{
			...
			if (nCount > 0)
			{
				...
				if (lang.wLangId > 0) lang.bValid = TRUE; // メッセージリソースDLLとして有効
			}
		}
	}
...
// 文字列リソース読み込み用グローバル
CLoadString::CLoadStrBuffer
	CLoadString::m_acLoadStrBufferTemp[]; // 文字列読み込みバッファの配列（CLoadString::LoadStringSt() が使用する）
int CLoadString::m_nDataTempArrayIndex =
	0; // 最後に使用したバッファのインデックス（CLoadString::LoadStringSt() が使用する）
...
int CLoadString::CLoadStrBuffer::LoadString(UINT uid)
{
	...
	while (1)
	{
		...
		else if (nRet >= m_nBufferSize - 2)
		{ // ANSI版は1小さい長さで再読み込みを判定する
#endif
...
HINSTANCE CSelectLang::ChangeLang(UINT nIndex)
{
	...
	{ return m_psLangInfo->hInstance; } SSelLangInfo *psLangInfo = m_psLangInfoList.at(nIndex);
