...
CClipboard::~CClipboard() { Close(); }

// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
//                     インターフェース                        //
// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //

void CClipboard::Empty() { ::EmptyClipboard(); }
						 ...
						 bool bColumnSelect, bool bLineSelect, UINT uFormat)
{
	...
	*/
	...
	{
		//領域確保
		hgClipText = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, (nDataLen + 1) * sizeof(wchar_t));
		if (!hgClipText) break;
	...
	while (bSakuraText)
	{
		...
		//領域確保
		hgClipSakura = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, sizeof(int) + (nDataLen + 1) * sizeof(wchar_t));
		if (!hgClipSakura) break;

		//確保した領域にデータをコピー
		BYTE *pClip		= GlobalLockBYTE(hgClipSakura);
		*((int *)pClip) = nDataLen;
		pClip += sizeof(int); //データの長さ
		wmemcpy((wchar_t *)pClip, pData, nDataLen);
		pClip += nDataLen * sizeof(wchar_t); //データ
		*((wchar_t *)pClip) = L'\0';
		pClip += sizeof(wchar_t); //終端ヌル
	...
	if (bColumnSelect)
	{
		...
		if (0 != uFormat)
		{
			hgClipMSDEVColumn = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, 1);
	...
	if (bLineSelect)
	{
		...
		if (0 != uFormat)
		{
			hgClipMSDEVLine = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, 1);
			if (hgClipMSDEVLine)
			{
	...
	if (bLineSelect)
	{
		...
		if (0 != uFormat)
		{
			hgClipMSDEVLine2 = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, 1);
			if (hgClipMSDEVLine2)
			{
	...
	if (bColumnSelect && !hgClipMSDEVColumn) { return false; }
	if (bLineSelect && !(hgClipMSDEVLine && hgClipMSDEVLine2)) { return false; }
	if (!(hgClipText && hgClipSakura)) { return false; }
	return true;
}
...
bool CClipboard::SetHtmlText(const CNativeW &cmemBUf)
{
	...
	memcpy_raw(pszClip + cmemHeader.GetStringLength() + cmemUtf8.GetStringLength(), cmemFooter.GetStringPtr(),
			   cmemFooter.GetStringLength()); //データ
	pszClip[nLen] = '\0';					  //終端ヌル
	::GlobalUnlock(hgClipText);
...
{
	if (!m_bOpenResult) { return false; }
	if (NULL != pbColumnSelect) { *pbColumnSelect = false; }
	if (NULL != pbLineSelect) { *pbLineSelect = false; }
		...
		while ((uFormat = ::EnumClipboardFormats(uFormat)) != 0)
		{
			...
			if (::GetClipboardFormatName(uFormat, szFormatName, _countof(szFormatName) - 1))
			{
				...
				if (NULL != pbLineSelect
					&& 0 == lstrcmpi(L"VisualStudioEditorOperationsLineCutCopyClipboardTag", szFormatName))
				{
					*pbLineSelect = true;
					break;
				}
	...
	//サクラ形式のデータがあれば取得
	CLIPFORMAT uFormatSakuraClip = CClipboard::GetSakuraFormat();
	if ((uGetFormat == -1 || uGetFormat == uFormatSakuraClip) && ::IsClipboardFormatAvailable(uFormatSakuraClip))
	{
	...
	if (uGetFormat == -1 || uGetFormat == CF_UNICODETEXT) { hUnicode = ::GetClipboardData(CF_UNICODETEXT); }
	if (hUnicode != NULL)
	{
	...
	// OEMTEXT形式のデータがあれば取得
	HGLOBAL hText = NULL;
	if (uGetFormat == -1 || uGetFormat == CF_OEMTEXT) { hText = ::GetClipboardData(CF_OEMTEXT); }
	if (hText != NULL)
	{
	...
	if ((uGetFormat == -1 || uGetFormat == CF_HDROP) && ::IsClipboardFormatAvailable(CF_HDROP))
	{
		...
		if (hDrop != NULL)
		{
			...
			for (int nLoop = 0; nLoop < nMaxCnt; nLoop++)
			{
				...
				if (nMaxCnt > 1) { cmemBuf->AppendString(cEol.GetValue2()); }
			}
			return true;
		}
	}
...
static CLIPFORMAT GetClipFormat(const wchar_t *pFormatName)
{
	...
	if (pFormatName[0] == L'\0') { return uFormat; }
	for (int i = 0; i < _countof(sClipFormatNames); i++)
	{
		if (0 == _wcsicmp(pFormatName, sClipFormatNames[i].m_pszName)) { uFormat = sClipFormatNames[i].m_nClipFormat; }
	}
...
bool CClipboard::IsIncludeClipboradFormat(const wchar_t *pFormatName)
{
	...
	if (::IsClipboardFormatAvailable(uFormat)) { return true; }
	return false;
}
...
{
	CLIPFORMAT uFormat = GetClipFormat(pFormatName);
	if (uFormat == (CLIPFORMAT)-1) { return false; }
	if (nMode == -2)
	{
		if (uFormat == CF_UNICODETEXT || uFormat == GetSakuraFormat())
		{ return SetText(cstr.GetPtr(), cstr.GetLength(), false, false, uFormat); } return false;
	}
	...
	HGLOBAL hgClipText = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, nTextByteLen + nulLen);
	if (!hgClipText) { return false; }
	char *pszClip = GlobalLockChar(hgClipText);
	memcpy(pszClip, pBuf, nTextByteLen);
	if (nulLen) { memset(&pszClip[nTextByteLen], 0, nulLen); }
	::GlobalUnlock(hgClipText);
	::SetClipboardData(uFormat, hgClipText);
									  ...
									  const CEol &cEol)
{
	...
	CLIPFORMAT uFormat = GetClipFormat(pFormatName);
	if (uFormat == (CLIPFORMAT)-1) { return false; }
	if (!::IsClipboardFormatAvailable(uFormat)) { return false; }
	if (nMode == -2)
	{
		bool bret = false;
		if (-1 != GetDataType())
