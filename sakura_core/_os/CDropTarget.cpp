...
{
	if (bEscapePressed || (dwKeyState & (m_bLeft ? MK_RBUTTON : MK_LBUTTON))) return DRAGDROP_S_CANCEL;
	if (!(dwKeyState & (m_bLeft ? MK_LBUTTON : MK_RBUTTON))) return DRAGDROP_S_DROP;
	return S_OK;
}

STDMETHODIMP CDropSource::GiveFeedback(DWORD dropEffect) { return DRAGDROP_S_USEDEFAULTCURSORS; }
...
void CDataObject::SetText(LPCWSTR lpszText, int nTextLen, BOOL bColumnSelect)
{
	...
	if (lpszText != NULL)
	{
		...
		*((wchar_t *)m_pData[0].data + nTextLen) = L'\0';
		...
		m_pData[i].size = ::WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)m_pData[0].data, m_pData[0].size / sizeof(wchar_t),
												NULL, 0, NULL, NULL);
		m_pData[i].data = new BYTE[m_pData[i].size];
		::WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)m_pData[0].data, m_pData[0].size / sizeof(wchar_t),
							  (LPSTR)m_pData[i].data, m_pData[i].size, NULL, NULL);
...
DWORD CDataObject::DragDrop(BOOL bLeft, DWORD dwEffects)
{
	...
	if (SUCCEEDED(::DoDragDrop(this, &drop, dwEffects, &dwEffect))) return dwEffect;
...
{
	// Feb. 26, 2001, fixed by yebisuya sugoroku
	if (lpfe == NULL || lpsm == NULL) return E_INVALIDARG;
	if (m_pData == NULL) return OLE_E_NOTRUNNING;
	if (lpfe->lindex != -1) return DV_E_LINDEX;
	if ((lpfe->tymed & TYMED_HGLOBAL) == 0) return DV_E_TYMED;
	if (lpfe->dwAspect != DVASPECT_CONTENT) return DV_E_DVASPECT;
	if (!(lpfe->tymed & TYMED_HGLOBAL) || lpfe->lindex != -1 || lpfe->dwAspect != DVASPECT_CONTENT)
...
{
	// Feb. 26, 2001, fixed by yebisuya sugoroku
	if (lpfe == NULL || lpsm == NULL || lpsm->hGlobal == NULL) return E_INVALIDARG;
	if (m_pData == NULL) return OLE_E_NOTRUNNING;

	if (lpfe->lindex != -1) return DV_E_LINDEX;
	if (lpfe->tymed != TYMED_HGLOBAL || lpsm->tymed != TYMED_HGLOBAL) return DV_E_TYMED;
	if (lpfe->dwAspect != DVASPECT_CONTENT) return DV_E_DVASPECT;
	...
	if (i == m_nFormat) return DV_E_FORMATETC;
	if (m_pData[i].size > ::GlobalSize(lpsm->hGlobal)) return STG_E_MEDIUMFULL;
...
{
	if (lpfe == NULL) return E_INVALIDARG;
	// Feb. 26, 2001, fixed by yebisuya sugoroku
	if (m_pData == NULL) return OLE_E_NOTRUNNING;

	if (lpfe->ptd != NULL || lpfe->dwAspect != DVASPECT_CONTENT || lpfe->lindex != -1 || !(lpfe->tymed & TYMED_HGLOBAL))
...
STDMETHODIMP CDataObject::GetCanonicalFormatEtc(LPFORMATETC, LPFORMATETC) { return DATA_S_SAMEFORMATETC; }

STDMETHODIMP CDataObject::SetData(LPFORMATETC, LPSTGMEDIUM, BOOL) { return E_NOTIMPL; }
...
STDMETHODIMP CDataObject::DAdvise(LPFORMATETC, DWORD, LPADVISESINK, LPDWORD) { return OLE_E_ADVISENOTSUPPORTED; }

STDMETHODIMP CDataObject::DUnadvise(DWORD) { return OLE_E_ADVISENOTSUPPORTED; }

STDMETHODIMP CDataObject::EnumDAdvise(LPENUMSTATDATA *) { return OLE_E_ADVISENOTSUPPORTED; }
...
{
	if (celt <= 0 || rgelt == NULL || m_nIndex >= m_pcDataObject->m_nFormat) return S_FALSE;
	...
	if (pceltFetched != NULL) *pceltFetched = celt - i;
...
/** IEnumFORMATETC::Clone
	@date 2008.03.26 ryoji 新規作成
*/
STDMETHODIMP CEnumFORMATETC::Clone(IEnumFORMATETC **ppenum) { return E_NOTIMPL; }
