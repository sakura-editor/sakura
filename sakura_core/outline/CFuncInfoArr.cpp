...
{
	if (nIdx >= m_nFuncInfoArrNum) { return NULL; }
...
)
{
	CFuncInfo *pcFuncInfo =
		new CFuncInfo(nFuncLineCRLF, nFuncColCRLF, nFuncLineLAYOUT, nFuncColLAYOUT, pszFuncName, pszFileName, nInfo);
...
void CFuncInfoArr::DUMP(void)
{
	...
	for (i = 0; i < m_nFuncInfoArrNum; i++) {
		...
		MYTRACE(L"m_cmemFileName	=[%s]\n",
				(m_ppcFuncInfoArr[i]->m_cmemFileName.GetStringPtr() ? m_ppcFuncInfoArr[i]->m_cmemFileName.GetStringPtr()
																	: L"NULL"));
...
{
	// キーが存在する場合、値を返す
	if (m_AppendTextArr.find(info) != m_AppendTextArr.end()) { return m_AppendTextArr[info]; }
	// キーが存在しない場合、空文字列を返す
	return {};
}
