...
const DWORD p_helpids[] = { // 13300
	IDC_CHECK_LOADINI, HIDC_CHECK_FILETREE_LOADINI, IDC_EDIT_DEFINI, HIDC_EDIT_FILETREE_DEFINI, IDC_BUTTON_REF1,
	HIDC_BUTTON_FILETREE_REF1, IDC_BUTTON_LOAD, HIDC_BUTTON_FILETREE_LOAD, IDC_RADIO_GREP, HIDC_RADIO_FILETREE_GREP,
	IDC_RADIO_FILE, HIDC_RADIO_FILETREE_FILE, IDC_RADIO_FOLDER, HIDC_RADIO_FILETREE_FOLDER, IDC_STATIC_PATH,
	HIDC_STATIC_FILETREE_PATH, IDC_EDIT_PATH, HIDC_EDIT_FILETREE_PATH, IDC_BUTTON_REF2, HIDC_BUTTON_FILETREE_REF2,
	IDC_BUTTON_PATH_MENU, HIDC_BUTTON_FILETREE_PATH_MENU, IDC_EDIT_LABEL, HIDC_EDIT_FILETREE_LABEL, IDC_STATIC_FILE,
	HIDC_STATIC_FILETREE_FILE, IDC_EDIT_FILE, HIDC_EDIT_FILETREE_FILE, IDC_CHECK_HIDDEN, HIDC_CHECK_FILETREE_HIDDEN,
	IDC_CHECK_READONLY, HIDC_CHECK_FILETREE_READONLY, IDC_CHECK_SYSTEM, HIDC_CHECK_FILETREE_SYSTEM, IDC_BUTTON_DELETE,
	HIDC_BUTTON_FILETREE_DELETE, IDC_BUTTON_INSERT, HIDC_BUTTON_FILETREE_INSERT, IDC_BUTTON_INSERT_A,
	HIDC_BUTTON_FILETREE_INSERT_A, IDC_BUTTON_ADD, HIDC_BUTTON_FILETREE_ADD, IDC_BUTTON_UPDATE,
	HIDC_BUTTON_FILETREE_UPDATE, IDC_BUTTON_FILEADD, HIDC_BUTTON_FILETREE_FILEADD, IDC_BUTTON_REPLACE,
	HIDC_BUTTON_FILETREE_REPLACE, IDC_BUTTON_UP, HIDC_BUTTON_FILETREE_UP, IDC_BUTTON_DOWN, HIDC_BUTTON_FILETREE_DOWN,
	IDC_BUTTON_RIGHT, HIDC_BUTTON_FILETREE_RIGHT, IDC_BUTTON_LEFT, HIDC_BUTTON_FILETREE_LEFT, IDC_TREE_FL,
	HIDC_TREE_FILETREE_FL, IDC_BUTTON_IMPORT, HIDC_BUTTON_FILETREE_IMPORT, HIDC_BUTTON_EXPORT,
	HIDC_BUTTON_FILETREE_EXPORT, IDOK, HIDC_FILETREE_IDOK, IDCANCEL, HIDC_FILETREE_IDCANCEL, IDC_BUTTON_HELP,
	HIDC_BUTTON_FILETREE_HELP,
	//	IDC_STATIC,				-1,
	0, 0};

CDlgFileTree::CDlgFileTree() { m_bInMove = false; }
...
/*! モーダルダイアログの表示
	...
	呼び出し元は、再表示で設定される
*/
int CDlgFileTree::DoModal(HINSTANCE hInstance, HWND hwndParent, LPARAM lParam)
{
...
static WCHAR *GetFileTreeLabel(const SFileTreeItem &item)
{
	...
	if (item.m_eFileTreeItemType != EFileTreeItemType_Folder)
	{
		...
		if (item.m_szLabelName[0] == L'\0')
		{
			...
			if (0 == wcscmp(pszLabel, L".") || 0 == wcscmp(pszLabel, L".\\") || 0 == wcscmp(pszLabel, L"./"))
...
void CDlgFileTree::SetData()
{
	...
	for (int i = 0; i < (int)m_fileTreeSetting.m_aItems.size(); i++)
	{
		...
		if (bSaveShareData && nMaxCount < i + 1) { ::InfoMessage(GetHwnd(), LS(STR_FILETREE_MAXCOUNT), nMaxCount); }
		...
		if (item.m_eFileTreeItemType == EFileTreeItemType_Folder) { hParentTree.push_back(hParent); }
		if (hSelect == NULL) { hSelect = hParent; }
	}
	if (hSelect != NULL) { TreeView_SelectItem(hwndTree, hSelect); }
...
void CDlgFileTree::SetDataItem(int nItemIndex)
{
	...
	bool bDummy  = false;
	if (nItemIndex < 0 || (int)m_fileTreeSetting.m_aItems.size() <= nItemIndex) { bDummy = true; }
	...
	{
	case EFileTreeItemType_Grep:
		nID1 = 0;
		nID2 = 1;
		nID3 = 2;
		break;
	case EFileTreeItemType_File:
		nID1 = 1;
		nID2 = 0;
		nID3 = 2;
		break;
	case EFileTreeItemType_Folder:
		nID1 = 2;
		nID2 = 0;
		nID3 = 1;
		break;
...
void CDlgFileTree::ChangeEnableAddInsert()
{
	...
	if (bSaveShareData)
	{
		...
		bool bEnable   = true;
		int  nMaxCount = _countof(GetDllShareData().m_Common.m_sOutline.m_sFileTree.m_aItems);
		if (nMaxCount < nCount) { bEnable = false; }
...
int CDlgFileTree::GetData()
{
	...
	{ pFileTree = &GetDllShareData().m_Common.m_sOutline.m_sFileTree; } else
	{
	...
	if (!GetDataTree(items, TreeView_GetRoot(GetItemHwnd(IDC_TREE_FL)), 0,
					 (bSaveShareData ? _countof(pFileTree->m_aItems) : 0)))
	{ InfoMessage(GetHwnd(), LS(STR_FILETREE_MAXCOUNT)); } if (pFileTree)
	{
		pFileTree->m_bProject = IsDlgButtonCheckedBool(hwndDlg, IDC_CHECK_LOADINI);
		DlgItem_GetText(hwndDlg, IDC_EDIT_DEFINI, pFileTree->m_szProjectIni,
						pFileTree->m_szProjectIni.GetBufferCount());
		if (bSaveShareData)
		{
	...
	if (false == bSaveShareData)
	{
		...
		if (false == cImpExp.Export(strIni, strError)) { ErrorMessage(hwndDlg, L"%ls", strError.c_str()); }
	}
	return TRUE;
}
...
bool CDlgFileTree::GetDataTree(std::vector<SFileTreeItem> &data, HTREEITEM hItem, int nLevel, int nMaxCount)
{
	...
	for (HTREEITEM s = hItem; s != NULL; s = TreeView_GetNextSibling(hwndTree, s))
	{
		...
		if (0 < nMaxCount && nMaxCount <= (int)data.size()) { return false; }
		...
		if (0 < tvi.cChildren)
		{
			...
			if (ts != NULL)
			{
				if (!GetDataTree(data, ts, nLevel + 1, nMaxCount)) { return false; }
			}
		}
	}
	return true;
...
void CDlgFileTree::SetDataInit()
{
	...
	if (m_fileTreeSetting.m_eFileTreeSettingLoadType != EFileTreeSettingFrom_File)
	{
		...
		{ id = STR_FILETREE_FROM_COMMON; } else
		{
	...
	else
	{
		...
		CFileNameManager::getInstance()->GetTransformFileNameFast(pFile, szFilePath, _countof(szFilePath), calc.GetDC(),
																  true, nMaxCch);
...
static HTREEITEM FileTreeCopy(HWND hwndTree, HTREEITEM dst, HTREEITEM src, bool fChild, bool fOnryOne)
{
	...
	for (s = src; s != NULL; s = fOnryOne ? NULL : TreeView_GetNextSibling(hwndTree, s))
	{
		...
		if (tvi.cChildren)
		{
			...
			if (ts != NULL) { FileTreeCopy(hwndTree, td, ts, true, false); }
			// 展開
			if (tvi.state & TVIS_EXPANDEDONCE) { TreeView_Expand(hwndTree, td, TVE_EXPAND); }
		}
		n++;
	}
...
BOOL CDlgFileTree::OnBnClicked(int wID)
{
	...
	{
		DlgItem_GetText(GetHwnd(), IDC_EDIT_DEFINI, m_fileTreeSetting.m_szDefaultProjectIni,
						m_fileTreeSetting.m_szDefaultProjectIni.GetBufferCount());
	...
	case IDC_BUTTON_REF2:
	{
		...
		else
		{
			...
			GetInidir(szDir);
			dlg.Create(G_AppInstance(), hwndDlg, L"*.*", szDir, std::vector<LPCWSTR>(), std::vector<LPCWSTR>());
	...
	case IDC_BUTTON_DELETE:
	{
		...
		if (htiItem != NULL)
		{
			...
			{ return TRUE; } HTREEITEM htiTemp = TreeView_GetNextSibling(hwndTree, htiItem);
			if (htiTemp == NULL)
			{
				// 末尾ならば、前を取る
			...
			if (htiTemp != NULL) { TreeView_SelectItem(hwndTree, htiTemp); }
		}
		ChangeEnableAddInsert();
	}
		return TRUE;
	...
	case IDC_BUTTON_ADD:
	{
		...
		else
		{
			...
			// 挿入先を探る
			htiParent = TreeView_GetParent(hwndTree, htiTemp);
			if (htiParent == NULL) { htiInsert = TVI_FIRST; }
			else
			{
				htiInsert = TreeView_GetPrevSibling(hwndTree, htiTemp);
				if (htiInsert == NULL) { htiInsert = TVI_FIRST; }
			}
		}
		if (htiParent == NULL) { htiParent = TVI_ROOT; }
		if (htiInsert == NULL) { htiInsert = TVI_LAST; }
		SFileTreeItem item;
		GetDataItem(item);
		HTREEITEM htiItem = InsertTreeItem(item, htiParent, htiInsert);
		// 展開
		if (htiParent != TVI_ROOT) { TreeView_Expand(hwndTree, htiParent, TVE_EXPAND); }
	...
	case IDC_BUTTON_FILEADD:
	{
		...
		dlg.Create(G_AppInstance(), GetHwnd(), L"*.*", L".", std::vector<LPCWSTR>(), std::vector<LPCWSTR>());
		if (dlg.DoModalOpenDlg(&sLoadInfo, &aFileNames, false))
		{
			if (0 < aFileNames.size())
			{
				...
				for (int i = 0; i < (int)aFileNames.size(); i++)
				{
					...
					if (htiItemFirst == NULL) { htiItemFirst = htiInsert; }
				}
				// 展開
				if (htiParent != TVI_ROOT) { TreeView_Expand(hwndTree, htiParent, TVE_EXPAND); }
	...
	case IDC_BUTTON_REPLACE:
	{
		...
		szPathFrom[0] = L'\0';
		if (dlgInput.DoModal(G_AppInstance(), GetHwnd(), strTitle.c_str(), strMsg.c_str(), _countof(szPathFrom),
							 szPathFrom))
		{
			WCHAR szPathTo[_MAX_PATH];
			szPathTo[0] = L'\0';
			strMsg		= LS(STR_FILETREE_REPLACE_PATH_TO);
			if (dlgInput.DoModal(G_AppInstance(), GetHwnd(), strTitle.c_str(), strMsg.c_str(), _countof(szPathTo),
								 szPathTo))
			{
			...
			if (htiItem != NULL)
			{
				...
				tvi.mask  = TVIF_HANDLE | TVIF_PARAM;
				tvi.hItem = htiItem;
				if (TreeView_GetItem(hwndTree, &tvi)) { SetDataItem(tvi.lParam); }
			}
		}
	}
		return TRUE;
	...
	case IDC_BUTTON_DOWN:
	{
		...
		if (htiItem != NULL) { TreeView_SelectItem(hwndTree, htiItem); }
	}
		return TRUE;
	...
	case IDOK: ::EndDialog(GetHwnd(), GetData()); return TRUE;
...
BOOL CDlgFileTree::OnNotify(WPARAM wParam, LPARAM lParam)
{
	...
	{
	case TVN_DELETEITEM:
		if (!m_bInMove && pNMHDR->hwndFrom == hwndTree && (htiItem = TreeView_GetSelection(hwndTree)) != NULL)
		{
			//付属情報を削除
			TV_ITEM tvi;
			tvi.mask  = TVIF_HANDLE | TVIF_PARAM;
		...
		break;
	case TVN_SELCHANGED:
		if (!m_bInMove && pNMHDR->hwndFrom == hwndTree)
		{
...
LPVOID CDlgFileTree::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
