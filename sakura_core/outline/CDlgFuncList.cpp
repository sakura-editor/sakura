...
INT_PTR CDlgFuncList::DispatchEvent(HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NCPAINT: return OnNcPaint(hWnd, wMsg, wParam, lParam);
	case WM_NCCALCSIZE: return OnNcCalcSize(hWnd, wMsg, wParam, lParam);
	case WM_NCHITTEST: return OnNcHitTest(hWnd, wMsg, wParam, lParam);
	case WM_NCMOUSEMOVE: return OnNcMouseMove(hWnd, wMsg, wParam, lParam);
	case WM_MOUSEMOVE: return OnMouseMove(hWnd, wMsg, wParam, lParam);
 ...
 */
HWND CDlgFuncList::DoModeless(HINSTANCE hInstance, HWND hwndParent, LPARAM lParam, CFuncInfoArr *pcFuncInfoArr,
							  CLayoutInt nCurLine, CLayoutInt nCurCol, int nOutlineType, int nListType,
...
void CDlgFuncList::SetData()
{
	...
	//	2007.02.08 genta Python追加
		...
		for (i = 0; i < m_pcFuncInfoArr->GetNum(); ++i) {
			...
			if (1 == pcFuncInfo->m_nInfo) {
				item.pszText = const_cast<WCHAR *>(LS(STR_DLGFNCLST_REMARK01));
	...
	::CheckDlgButton(GetHwnd(), IDC_CHECK_bAutoCloseDlgFuncList,
					 m_pShareData->m_Common.m_sOutline.m_bAutoCloseDlgFuncList);
	/* アウトライン ブックマーク一覧で空行を無視する */
	::CheckDlgButton(GetHwnd(), IDC_CHECK_bMarkUpBlankLineEnable,
					 m_pShareData->m_Common.m_sOutline.m_bMarkUpBlankLineEnable);
	/* アウトライン ジャンプしたらフォーカスを移す */
	::CheckDlgButton(GetHwnd(), IDC_CHECK_bFunclistSetFocusOnJump,
...
void CDlgFuncList::SetTreeJava(HWND hwndDlg, BOOL bAddClass)
{
	...
	for (i = 0; i < m_pcFuncInfoArr->GetNum(); ++i) {
			...
			for (k = 0; k < nWorkLen; ++k) {
				...
				if (1 == nCharChars && 0 == nNestTemplate && L':' == pWork[k]) {
					...
					if (k < nWorkLen - 1 && L':' == pWork[k + 1]) {
						...
						if (wcsncmp_literal(pWork + m, L"operator ") == 0) { break; }
					} else
						break;
		...
		case FL_OBJ_GLOBAL:		//「グローバル」は別の場所で処理してるので除外
			break;
		default: strFuncName += m_pcFuncInfoArr->GetAppendText(pcFuncInfo->m_nInfo);
		}
		...
		// "%s%ls\r\n"
		m_cmemClipText.AppendNativeData(pcFuncInfo->m_cmemFuncName);
		m_cmemClipText.AppendString(FL_OBJ_DECLARE == pcFuncInfo->m_nInfo
										? m_pcFuncInfoArr->GetAppendText(FL_OBJ_DECLARE).c_str()
										: L""); //	Jan. 04, 2001 genta C++で使用
		m_cmemClipText.AppendString(L"\r\n");
...
void CDlgFuncList::SetListVB(void)
{
	...
	for (i = 0; i < m_pcFuncInfoArr->GetNum(); ++i) {
		...
		case 3: // フレンド(Friend)
			wcsncat(szOption, LS(STR_DLGFNCLST_VB_FRIEND),
					_countof(szOption) - wcslen(szOption)); //	2006.12.17 genta サイズ誤り修正
			break;

		default: // パブリック(Public)
			wcsncat(szOption, LS(STR_DLGFNCLST_VB_PUBLIC),
					_countof(szOption) - wcslen(szOption)); //	2006.12.17 genta サイズ誤り修正
		}
...
void CDlgFuncList::SetTree(bool tagjump, bool nolabel)
{
	...
	for (i = 0; i < nFuncInfoArrNum; i++) {
		...
		*/
		...
		cTVInsertStruct.item.lParam =
			i; //	あとでこの数値（＝m_pcFuncInfoArrの何番目のアイテムか）を見て、目的地にジャンプするぜ!!。
...
{
	if (m_nOutlineType == OUTLINE_BOOKMARK) { return; }
	if (m_nOutlineType == OUTLINE_FILETREE) { return; }
	CEditView *	 pcEditView	  = (CEditView *)m_lParam;
...
void CDlgFuncList::SetTreeFile()
{
	...
	for (int i = 0; i < (int)m_fileTreeSetting.m_aItems.size(); i++) {
		...
		if (item.m_szLabelName[0] != L'\0') { pszLabel = item.m_szLabelName; }
		// lvis.item.lParam
...
void CDlgFuncList::SetTreeFileSub(HTREEITEM hParent, const WCHAR *pszFile)
{
	...
	if (NULL != TreeView_GetChild(hwndTree, hParent)) { return; }
...
BOOL CDlgFuncList::OnInitDialog(HWND hwndDlg, WPARAM wParam, LPARAM lParam)
{
	...
	ListView_SetExtendedListViewStyle(hwndList, ListView_GetExtendedListViewStyle(hwndList) | LVS_EX_FULLROWSELECT
													| LVS_EX_INFOTIP);
	...
	if (IsDocking()) {
		...
		m_hwndToolTip =
			::CreateWindowEx(0, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, CW_USEDEFAULT,
							 CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, GetHwnd(), NULL, m_hInstance, NULL);
...
BOOL CDlgFuncList::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		/* 「アウトライン解析」のヘルプ */
		// Apr. 5, 2001 JEPRO 修正漏れを追加 (Stonee, 2001/03/12
		// 第四引数を、機能番号からヘルプトピック番号を調べるようにした)
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_OUTLINE)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
	...
	case IDC_CHECK_bFunclistSetFocusOnJump:
		m_pShareData->m_Common.m_sOutline.m_bAutoCloseDlgFuncList =
			::IsDlgButtonChecked(GetHwnd(), IDC_CHECK_bAutoCloseDlgFuncList);
		m_pShareData->m_Common.m_sOutline.m_bMarkUpBlankLineEnable =
...
BOOL CDlgFuncList::OnNotify(WPARAM wParam, LPARAM lParam)
{
	...
	if (hwndTree == pnmh->hwndFrom) {
		...
		case LVN_COLUMNCLICK:
			...
			if (m_nSortCol == m_nSortColOld) { m_bSortDesc = !m_bSortDesc; }
			m_nSortColOld = m_nSortCol;
			{
...
void CDlgFuncList::SortListView(HWND hwndList, int sortcol)
{
	...
	//	if( sortcol == 1 ){
	{
		...
		col.mask = LVCF_TEXT;
		col.pszText =
			const_cast<WCHAR *>(sortcol == col_no ? LS(STR_DLGFNCLST_LIST_LINE_M) : LS(STR_DLGFNCLST_LIST_LINE));
		col.iSubItem = 0;
		ListView_SetColumn(hwndList, col_no, &col);
		...
		col.mask = LVCF_TEXT;
		col.pszText =
			const_cast<WCHAR *>(sortcol == col_no ? LS(STR_DLGFNCLST_LIST_COL_M) : LS(STR_DLGFNCLST_LIST_COL));
		col.iSubItem = 0;
		ListView_SetColumn(hwndList, col_no, &col);
	...
	ListView_EnsureVisible(hwndList, keypos >= 0 ? keypos : 0, FALSE);
}
...
{
	// 今のところ CEditWnd::OnSize() からの呼び出しでは lParam は CEditWnd 側 の lParam のまま渡される	// 2010.06.05
	// ryoji
	RECT rcDlg;
	...
	for (int i = 0; i < _countof(anchorList); i++) {
		...
		if (anchorList[i].anchor == ANCHOR_ALL) { ::UpdateWindow(hwndCtrl); }
	}
...
BOOL CDlgFuncList::OnDestroy(void)
{
	...
	if (IsDocking() && hwndEdit) pcEditView->m_pcEditWnd->EndLayoutBars();
...
static void SortTree_Sub(HWND hWndTree, HTREEITEM htiParent, STreeViewSortData &data, int nSortType)
{
	if (SORTTYPE_ATOZ == nSortType || SORTTYPE_ZTOA == nSortType) {
		for (HTREEITEM htiItem = TreeView_GetChild(hWndTree, htiParent); NULL != htiItem;
			...
			item.hItem = htiItem;
			std::vector<WCHAR> vecStr;
			if (TreeView_GetItemTextVector(hWndTree, item, vecStr)) { data.m_vecText[item.lParam].assign(&vecStr[0]); }
		}
	}
	TVSORTCB sort;
...
void CDlgFuncList::SortTree(HWND hWndTree, HTREEITEM htiParent)
{
	...
	if (m_bDummyLParamMode) { size = m_nTreeItemCount; }
...
LPVOID CDlgFuncList::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end
...
void CDlgFuncList::Key2Command(WORD KeyCode)
{
	...
	case F_BOOKMARK_SET:
		...
		break;
	case F_COPY:
	case F_CUT: OnBnClicked(IDC_BUTTON_COPY); break;
	}
}
...
*/
void CDlgFuncList::Redraw(int nOutLineType, int nListType, CFuncInfoArr *pcFuncInfoArr, CLayoutInt nCurLine,
						  CLayoutInt nCurCol)
{
	CEditView *pcEditView = (CEditView *)m_lParam;
	m_nDocType			  = pcEditView->GetDocument()->m_cDocType.GetDocumentType().GetIndex();
...
//ダイアログタイトルの設定
void CDlgFuncList::SetWindowText(const WCHAR *szTitle) { ::SetWindowText(GetHwnd(), szTitle); }
...
void CDlgFuncList::SyncColor(void)
{
	...
	::SetWindowPos(
		hwndTree, NULL, 0, 0, 0, 0,
		SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER
			| SWP_FRAMECHANGED); // なぜかこうしないと四辺１ドット幅分だけ色変更が即時適用されない（←スタイル再設定とは無関係）
...
{
	if (!IsDocking()) return false;
	if (nButton >= DOCK_BUTTON_NUM) return false;
...
INT_PTR CDlgFuncList::OnTimer(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam == 1) {
		...
		if (!::PtInRect(&rc, pt)) { ::SendMessageAny(hwnd, WM_NCMOUSEMOVE, 0, MAKELONG(pt.x, pt.y)); }
	}
...
INT_PTR CDlgFuncList::OnMouseMove(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (m_bStretching) { // マウスのドラッグ位置にあわせてサイズを変更する
		...
		if (::EqualRect(&rcOld, &rc)) return 0L;

		// 移動する
		::SetWindowPos(GetHwnd(), NULL, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top,
	...
	if (!IsDocking()) {
		if (GetDockSide() == DOCKSIDE_FLOAT) {
			if (wParam == HTCAPTION && !::IsZoomed(GetHwnd()) && !::IsIconic(GetHwnd())) {
				...
				// SetActiveWindow() 後にはボタンが離されていることがある。その場合は Track() を開始せずに抜ける。
...
INT_PTR CDlgFuncList::OnNcPaint(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	// ↓DC_SMALLCAP 指定のものを Win7(64bit版)
	// で動かしてみたら描画位置が下にずれて上半分しか見えなかった（x86ビルド/x64ビルドのどちらも NG）
	::DrawCaption(hwnd, gr, &rcWk,
				  DC_TEXT | (bGradient ? DC_GRADIENT : 0) /*| DC_SMALLCAP*/ | (bActive ? DC_ACTIVE : 0));
	...
	ncm.cbSize = CCSIZEOF_STRUCT(
		NONCLIENTMETRICS,
		lfMessageFont); // 以前のプラットフォームに WINVER >= 0x0600 で定義される構造体のフルサイズを渡すと失敗する
	...
	static const WCHAR szBtn[DOCK_BUTTON_NUM] = {(WCHAR)0x72 /* 閉じる */, (WCHAR)0x36 /* メニュー */,
												 (WCHAR)0x71 /* 更新 */};
...
void CDlgFuncList::DoMenu(POINT pt, HWND hwndFrom)
{
	...
	if (bDropDown == false) {
		...
		if (m_nViewType == VIEWTYPE_TREE) {
			...
			flag		  = 0;
			HWND hwndList = GetItemHwnd(IDC_LIST_FL);
			if (ListView_GetSelectedCount(hwndList) == 0) { flag |= MF_GRAYED; }
		...
		::InsertMenu(hMenu, iPos++, MF_BYPOSITION | MF_SEPARATOR, 0, NULL);
		::InsertMenu(hMenu, iPos++, MF_BYPOSITION | MF_STRING | MF_POPUP, (UINT_PTR)hMenuSub,
					 LS(STR_DLGFNCLST_MENU_WINPOS));
	}
...
/** 現在の設定に応じて配置を変更する（できる限り再解析しない）
	...
	@param nId [in] 動作指定．OUTLINE_LAYOUT_FOREGROUND: 前面用の動作 / OUTLINE_LAYOUT_BACKGROUND: 背後用の動作 /
   OUTLINE_LAYOUT_FILECHANGED: ファイル切替用の動作（前面だが特殊）
...
bool CDlgFuncList::ChangeLayout(int nId)
{
	...
	if (!GetHwnd()) {	 // 現在は非表示
		if (bDockDisp) { // 新設定は表示
			...
			EOutlineType nOutlineType = GetOutlineTypeRedraw(
				m_nOutlineType); // ブックマークかアウトライン解析かは最後に開いていた時の状態を引き継ぐ（初期状態はアウトライン解析）
			pcEditView->GetCommander().Command_FUNCLIST(
				SHOW_NORMAL, nOutlineType); // 開く	※ HandleCommand(F_OUTLINE,...)
											// だと印刷プレビュー状態で実行されないので Command_FUNCLIST()
		...
		::SetWindowPos(GetHwnd(), NULL, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top,
					   SWP_NOOWNERZORDER | SWP_NOZORDER | SWP_NOACTIVATE
						   | ((eDockSideOld == eDockSideNew)
								  ? 0
								  : SWP_FRAMECHANGED)); // SWP_FRAMECHANGED 指定で
														// WM_NCCALCSIZE（非クライアント領域の再計算）に誘導する
...
{
	return CAppNodeGroupHandle(0).PostMessageToAllEditors(MYWM_OUTLINE_NOTIFY, (WPARAM)wParam, (LPARAM)lParam,
														  GetHwnd());
}
	...
	if (!IsDocking()) { // フローティング → フローティング
		...
		*pRect = rcWnd;
		...
		if (m_pShareData->m_Common.m_sOutline.m_bRememberOutlineWindowPos
			...
			rcFloat.right += ::GetSystemMetrics(SM_CXDLGFRAME)
							 * 2; // ※ Create 時のスタイル変更でサイズ変更不可からサイズ変更可能にしている
...
void CDlgFuncList::LoadFileTreeSetting(CFileTreeSetting &data, SFilePath &IniDirPath)
{
	...
	if (data.m_szLoadProjectIni[0] == L'\0') {
		...
		if (!bReadIni) {
			...
			for (int i = 0; i < pFileTree->m_nItemCount; i++) { data.m_aItems[i] = pFileTree->m_aItems[i]; }
		}
	}
}
