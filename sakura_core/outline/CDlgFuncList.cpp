...
const DWORD p_helpids[] = { //12200
    IDC_BUTTON_COPY,
    HIDC_FL_BUTTON_COPY, //コピー
    IDOK,
    HIDOK_FL, //ジャンプ
    IDCANCEL,
    HIDCANCEL_FL, //キャンセル
    IDC_BUTTON_HELP,
    HIDC_FL_BUTTON_HELP, //ヘルプ
    IDC_CHECK_bAutoCloseDlgFuncList,
    HIDC_FL_CHECK_bAutoCloseDlgFuncList, //自動的に閉じる
    IDC_LIST_FL,
    HIDC_FL_LIST1, //トピックリスト	IDC_LIST1->IDC_LIST_FL	2008/7/3 Uchi
    IDC_TREE_FL,
    HIDC_FL_TREE1, //トピックツリー	IDC_TREE1->IDC_TREE_FL	2008/7/3 Uchi
    IDC_CHECK_bFunclistSetFocusOnJump,
    HIDC_FL_CHECK_bFunclistSetFocusOnJump, //ジャンプでフォーカス移動する
    IDC_CHECK_bMarkUpBlankLineEnable,
    HIDC_FL_CHECK_bMarkUpBlankLineEnable, //空行を無視する
    IDC_COMBO_nSortType,
    HIDC_COMBO_nSortType, //順序
    IDC_BUTTON_WINSIZE,
    HIDC_FL_BUTTON_WINSIZE, //ウィンドウ位置保存	// 2006.08.06 ryoji
    IDC_BUTTON_MENU,
    HIDC_FL_BUTTON_MENU, //ウィンドウの位置メニュー
    IDC_BUTTON_SETTING,
    HIDC_FL_BUTTON_SETTING, //設定
    //	IDC_STATIC,							-1,
    0,
...
void CDlgFuncList::SetData()
{
    ...
    else
    {
        ...
        for (i = 0; i < m_pcFuncInfoArr->GetNum(); ++i)
        {
            ...
            if (!bSelected)
            {
                if (pcFuncInfo->m_nFuncLineLAYOUT < nFuncLineTop || (pcFuncInfo->m_nFuncLineLAYOUT == nFuncLineTop && pcFuncInfo->m_nFuncColLAYOUT <= nFuncColTop))
                {
            ...
            {
                if ((nFuncLineOld < pcFuncInfo->m_nFuncLineLAYOUT || (nFuncLineOld == pcFuncInfo->m_nFuncColLAYOUT && nFuncColOld <= pcFuncInfo->m_nFuncColLAYOUT)) && (pcFuncInfo->m_nFuncLineLAYOUT < m_nCurLine || (pcFuncInfo->m_nFuncLineLAYOUT == m_nCurLine && pcFuncInfo->m_nFuncColLAYOUT <= m_nCurCol)))
                {
    ...
    ::DlgItem_SetText(GetHwnd(), IDC_BUTTON_WINSIZE, m_pShareData->m_Common.m_sOutline.m_bRememberOutlineWindowPos ? L"■" : L"□");
        ...
        if (pcFuncInfo->IsAddClipText())
        {
            ...
            if (tagjump)
            {
                ...
                text.AllocStringBuffer(
                    pcFuncInfo->m_cmemFuncName.GetStringLength() + nStackPointer * 2 + 1 + wcslen(pszFileName) + 20);
...
BOOL CDlgFuncList::OnInitDialog(HWND hwndDlg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (pcEditView != NULL)
    {
        ...
        else if (IsDocking())
        {
            ...
            {
                case DOCKSIDE_LEFT:
                    m_nWidth = ProfDockLeft();
                    break;
                case DOCKSIDE_TOP:
                    m_nHeight = ProfDockTop();
                    break;
                case DOCKSIDE_RIGHT:
                    m_nWidth = ProfDockRight();
                    break;
                case DOCKSIDE_BOTTOM:
                    m_nHeight = ProfDockBottom();
                    break;
            }
            if (eDockSide == DOCKSIDE_LEFT || eDockSide == DOCKSIDE_RIGHT)
            {
                ...
                if (m_nWidth > rc.right - rc.left - DOCK_MIN_SIZE)
                    m_nWidth = rc.right - rc.left - DOCK_MIN_SIZE;
                if (m_nWidth < DOCK_MIN_SIZE)
                    m_nWidth = DOCK_MIN_SIZE;
            }
            else
            {
                if (m_nHeight == 0) // 初回
                    m_nHeight = (rc.bottom - rc.top) / 3;
                if (m_nHeight > rc.bottom - rc.top - DOCK_MIN_SIZE)
                    m_nHeight = rc.bottom - rc.top - DOCK_MIN_SIZE;
                if (m_nHeight < DOCK_MIN_SIZE)
                    m_nHeight = DOCK_MIN_SIZE;
            }
        }
    }
...
bool CDlgFuncList::HitTestSplitter(int xPos, int yPos)
{
    ...
    {
        case DOCKSIDE_LEFT:
            bRet = (rc.right - xPos < DOCK_SPLITTER_WIDTH);
            break;
        case DOCKSIDE_TOP:
            bRet = (rc.bottom - yPos < DOCK_SPLITTER_WIDTH);
            break;
        case DOCKSIDE_RIGHT:
            bRet = (xPos - rc.left < DOCK_SPLITTER_WIDTH);
            break;
        case DOCKSIDE_BOTTOM:
            bRet = (yPos - rc.top < DOCK_SPLITTER_WIDTH);
            break;
    }
...
INT_PTR CDlgFuncList::OnNcCalcSize(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case DOCKSIDE_LEFT:
            pNCS->rgrc[0].right -= DOCK_SPLITTER_WIDTH;
            break;
        case DOCKSIDE_TOP:
            pNCS->rgrc[0].bottom -= DOCK_SPLITTER_WIDTH;
            break;
        case DOCKSIDE_RIGHT:
            pNCS->rgrc[0].left += DOCK_SPLITTER_WIDTH;
            break;
        case DOCKSIDE_BOTTOM:
            pNCS->rgrc[0].top += DOCK_SPLITTER_WIDTH;
            break;
    }
    return 1L;
}
...
INT_PTR CDlgFuncList::OnNcMouseMove(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (nHilightedBtn != m_nHilightedBtn)
    {
        ...
        {
            case 0:
                ti.lpszText = const_cast<WCHAR *>(LS(STR_DLGFNCLST_TIP_CLOSE));
                break;
            case 1:
                ti.lpszText = const_cast<WCHAR *>(LS(STR_DLGFNCLST_TIP_WIN));
                break;
            case 2:
                ti.lpszText = const_cast<WCHAR *>(LS(STR_DLGFNCLST_TIP_UPDATE));
                break;
            default:
                ti.lpszText = NULL; // 消す
        }
...
INT_PTR CDlgFuncList::OnMouseMove(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    { // マウスのドラッグ位置にあわせてサイズを変更する
        ...
        if (pt.x > rc.right - DOCK_MIN_SIZE)
            pt.x = rc.right - DOCK_MIN_SIZE;
        if (pt.x < rc.left + DOCK_MIN_SIZE)
            pt.x = rc.left + DOCK_MIN_SIZE;
        if (pt.y > rc.bottom - DOCK_MIN_SIZE)
            pt.y = rc.bottom - DOCK_MIN_SIZE;
        if (pt.y < rc.top + DOCK_MIN_SIZE)
            pt.y = rc.top + DOCK_MIN_SIZE;
        ...
        {
            case DOCKSIDE_LEFT:
                rc.right = pt.x - DOCK_SPLITTER_WIDTH / 2 + DOCK_SPLITTER_WIDTH;
                break;
            case DOCKSIDE_TOP:
                rc.bottom = pt.y - DOCK_SPLITTER_WIDTH / 2 + DOCK_SPLITTER_WIDTH;
                break;
            case DOCKSIDE_RIGHT:
                rc.left = pt.x - DOCK_SPLITTER_WIDTH / 2;
                break;
            case DOCKSIDE_BOTTOM:
                rc.top = pt.y - DOCK_SPLITTER_WIDTH / 2;
                break;
        }
        ...
        // 移動する
        ::SetWindowPos(GetHwnd(), NULL, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, SWP_NOOWNERZORDER | SWP_NOZORDER | SWP_NOACTIVATE);
        ...
        {
            case DOCKSIDE_LEFT:
                ProfDockLeft() = rc.right - rc.left;
                break;
            case DOCKSIDE_TOP:
                ProfDockTop() = rc.bottom - rc.top;
                break;
            case DOCKSIDE_RIGHT:
                ProfDockRight() = rc.right - rc.left;
                break;
            case DOCKSIDE_BOTTOM:
                ProfDockBottom() = rc.bottom - rc.top;
                break;
        }
        if (bType)
        {
...
INT_PTR CDlgFuncList::OnNcPaint(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case DOCKSIDE_LEFT:
            rcWk.left = rcWk.right - DOCK_SPLITTER_WIDTH;
            break;
        case DOCKSIDE_TOP:
            rcWk.top = rcWk.bottom - DOCK_SPLITTER_WIDTH;
            break;
        case DOCKSIDE_RIGHT:
            rcWk.right = rcWk.left + DOCK_SPLITTER_WIDTH;
            break;
        case DOCKSIDE_BOTTOM:
            rcWk.bottom = rcWk.top + DOCK_SPLITTER_WIDTH;
            break;
    }
...
void CDlgFuncList::DoMenu(POINT pt, HWND hwndFrom)
{
    ...
    { // 設定コピー
                                   ...
                                   LS(STR_DLGFNCLST_UNIFY)))
        {
            ...
            if (GetHwnd())
            {
                ...
                { // 現在のドッキングモード
                    case DOCKSIDE_LEFT:
                        CommonSet().m_cxOutlineDockLeft = rc.right - rc.left;
                        break;
                    case DOCKSIDE_TOP:
                        CommonSet().m_cyOutlineDockTop = rc.bottom - rc.top;
                        break;
                    case DOCKSIDE_RIGHT:
                        CommonSet().m_cxOutlineDockRight = rc.right - rc.left;
                        break;
                    case DOCKSIDE_BOTTOM:
                        CommonSet().m_cyOutlineDockBottom = rc.bottom - rc.top;
                        break;
                }
            }
            STypeConfig *type = new STypeConfig();
    ...
    { // ドッキングモード （※ DOCKSIDE_UNDOCKABLE は -1 です） */
        ...
        if (eDockSide > DOCKSIDE_FLOAT)
        {
            ...
            {
                case DOCKSIDE_LEFT:
                    pnWidth = &ProfDockLeft();
                    break;
                case DOCKSIDE_TOP:
                    pnHeight = &ProfDockTop();
                    break;
                case DOCKSIDE_RIGHT:
                    pnWidth = &ProfDockRight();
                    break;
                case DOCKSIDE_BOTTOM:
                    pnHeight = &ProfDockBottom();
                    break;
            }
            if (eDockSide == DOCKSIDE_LEFT || eDockSide == DOCKSIDE_RIGHT)
            {
...
bool CDlgFuncList::ChangeLayout(int nId)
{
    ...
    { // 現在は表示
        ...
        {
            case DOCKSIDE_LEFT:
                rc.right = rc.left + ProfDockLeft();
                break;
            case DOCKSIDE_TOP:
                rc.bottom = rc.top + ProfDockTop();
                break;
            case DOCKSIDE_RIGHT:
                rc.left = rc.right - ProfDockRight();
                break;
            case DOCKSIDE_BOTTOM:
                rc.top = rc.bottom - ProfDockBottom();
                break;
        }
...
BOOL CDlgFuncList::OnContextMenu(WPARAM wParam, LPARAM lParam)
{
    ...
    if (::SendMessage(GetHwnd(), WM_NCHITTEST, 0, lParam) == HTCAPTION || hwndFrom == GetItemHwnd(IDC_LIST_FL) || hwndFrom == GetItemHwnd(IDC_TREE_FL))
    {
        POINT pt;
        pt.x = MAKEPOINTS(lParam).x;
        pt.y = MAKEPOINTS(lParam).y;
...
EDockSide CDlgFuncList::GetDropRect(POINT ptDrag, POINT ptDrop, LPRECT pRect, bool bForceFloat)
{
    ...
    { // ドッキング → フローティング
        ...
        rcFloat.left = 0;
        rcFloat.top  = 0;
        if (m_pShareData->m_Common.m_sOutline.m_bRememberOutlineWindowPos && m_pShareData->m_Common.m_sOutline.m_widthOutlineWindow // 初期値だと 0 になっている
