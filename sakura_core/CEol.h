...
struct SEolDefinition
{
    ...
    bool StartsWith(const WCHAR *pData, int nLen) const
    {
        return m_nLen <= nLen && 0 == wmemcmp(pData, m_szDataW, m_nLen);
    }
    bool StartsWith(const ACHAR *pData, int nLen) const
    {
        return m_nLen <= nLen && m_szDataA[0] != '\0' && 0 == memcmp(pData, m_szDataA, m_nLen);
    }
};
...
class CEol
{
  public:
    ...
    //比較
    bool operator==(EEolType t) const
    {
        return GetType() == t;
    }
    bool operator!=(EEolType t) const
    ...
    {
        m_eEolType = t.m_eEolType;
        return *this;
    }

    //型変換
    operator EEolType() const
    {
        return GetType();
    }
    ...
    //設定（ファイル読み込み時に使用）
    void SetTypeByStringForFile(const char *pszData, int nDataLen)
    {
        SetTypeByString(pszData, nDataLen);
    }
    ...
    //取得
    EEolType GetType() const
    {
        return m_eEolType;
    } //!< 現在のTypeを取得
    CLogicInt GetLen() const
    {
