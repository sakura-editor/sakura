...
void CControlTray::DoGrepCreateWindow(HINSTANCE hinst, HWND msgParent, CDlgGrep& cDlgGrep)
{
	...
	//GOPTオプション
	WCHAR pOpt[64] = L"";
	if( cDlgGrep.m_bSubFolder					)wcsncat( pOpt, L"S" );	// サブフォルダからも検索する
	if( cDlgGrep.m_sSearchOption.bLoHiCase		)wcsncat( pOpt, L"L" );	// 英大文字と英小文字を区別する
	if( cDlgGrep.m_sSearchOption.bRegularExp	)wcsncat( pOpt, L"R" );	// 正規表現
	if( cDlgGrep.m_nGrepOutputLineType == 1     )wcsncat( pOpt, L"P" );	// 行を出力する
	if( cDlgGrep.m_nGrepOutputLineType == 2     )wcsncat( pOpt, L"N" );	// 否ヒット行を出力する 2014.09.23
	if( cDlgGrep.m_sSearchOption.bWordOnly		)wcsncat( pOpt, L"W" );	// 単語単位で探す
	if( 1 == cDlgGrep.m_nGrepOutputStyle		)wcsncat( pOpt, L"1" );	// Grep: 出力形式
	if( 2 == cDlgGrep.m_nGrepOutputStyle		)wcsncat( pOpt, L"2" );	// Grep: 出力形式
	if( 3 == cDlgGrep.m_nGrepOutputStyle		)wcsncat( pOpt, L"3" );
	if( cDlgGrep.m_bGrepOutputFileOnly			)wcsncat( pOpt, L"F" );
	if( cDlgGrep.m_bGrepOutputBaseFolder		)wcsncat( pOpt, L"B" );
	if( cDlgGrep.m_bGrepSeparateFolder			)wcsncat( pOpt, L"D" );
	if( pOpt[0] != L'\0' ){
