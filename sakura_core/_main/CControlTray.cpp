...
void CControlTray::DoGrep()
{
    ...
    if (0 < m_pShareData->m_sSearchKeywords.m_aSearchKeys.size() && m_nCurSearchKeySequence < GetDllShareData().m_Common.m_sSearch.m_nSearchKeySequence)
    {
...
void CControlTray::DoGrepCreateWindow(HINSTANCE hinst, HWND msgParent, CDlgGrep &cDlgGrep)
{
    ...
    //GOPTオプション
    WCHAR pOpt[64] = L"";
    if (cDlgGrep.m_bSubFolder)
        wcscat(pOpt, L"S"); // サブフォルダからも検索する
    if (cDlgGrep.m_sSearchOption.bLoHiCase)
        wcscat(pOpt, L"L"); // 英大文字と英小文字を区別する
    if (cDlgGrep.m_sSearchOption.bRegularExp)
        wcscat(pOpt, L"R"); // 正規表現
    if (cDlgGrep.m_nGrepOutputLineType == 1)
        wcscat(pOpt, L"P"); // 行を出力する
    if (cDlgGrep.m_nGrepOutputLineType == 2)
        wcscat(pOpt, L"N"); // 否ヒット行を出力する 2014.09.23
    if (cDlgGrep.m_sSearchOption.bWordOnly)
        wcscat(pOpt, L"W"); // 単語単位で探す
    if (1 == cDlgGrep.m_nGrepOutputStyle)
        wcscat(pOpt, L"1"); // Grep: 出力形式
    if (2 == cDlgGrep.m_nGrepOutputStyle)
        wcscat(pOpt, L"2"); // Grep: 出力形式
    if (3 == cDlgGrep.m_nGrepOutputStyle)
        wcscat(pOpt, L"3");
    if (cDlgGrep.m_bGrepOutputFileOnly)
        wcscat(pOpt, L"F");
    if (cDlgGrep.m_bGrepOutputBaseFolder)
        wcscat(pOpt, L"B");
    if (cDlgGrep.m_bGrepSeparateFolder)
        wcscat(pOpt, L"D");
    if (pOpt[0] != L'\0')
    {
    ...
    OpenNewEditor(hinst, msgParent, sLoadInfo, cCmdLine.GetStringPtr(), false, NULL, GetDllShareData().m_Common.m_sTabBar.m_bNewWindow ? true : false);
}
                            ...
                            do
                            {
                                ...
                                else
                                {
                                    ErrorBeep();
                                                  ...
                                                  NULL, MB_YESNOCANCEL | MB_ICONEXCLAMATION | MB_APPLMODAL | MB_TOPMOST, GSTR_APPNAME, LS(STR_TRAY_EXTHELP1))); /*do-while*/
                    ...
                    switch (nId)
                    {
                        ...
                        case F_FILEOPEN: /* 開く */
                        {
                            ...
                            for (size_t f = 0; f < nSize; f++)
                            {
                                ...
                                CControlTray::OpenNewEditor(m_hInstance, GetTrayHwnd(), sLoadInfo, NULL, true, NULL, m_pShareData->m_Common.m_sTabBar.m_bNewWindow ? true : false);
                            }
                        }
                        break;
                        case F_GREP_DIALOG:
...
void CControlTray::OnNewEditor(bool bNewWindow)
{
    ...
    bNewWindow = bNewWindow && m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd != FALSE && m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin == FALSE;
...
)
{
    ...
    // ファイル名
    if (sLoadInfo.cFilePath.c_str()[0] != L'\0')
        cCmdLineBuf.AppendF(L" \"%s\"", sLoadInfo.cFilePath.c_str());

    // コード指定
    if (IsValidCodeOrCPType(sLoadInfo.eCharCode))
        cCmdLineBuf.AppendF(L" -CODE=%d", sLoadInfo.eCharCode);

    // ビューモード指定
    if (sLoadInfo.bViewMode)
        cCmdLineBuf.AppendF(L" -R");
...
)
{
    ...
    if (pfi != NULL)
    {
        if (pfi->m_ptCursor.x >= 0)
            cCmdLine.AppendF(L" -X=%d", pfi->m_ptCursor.x + 1);
        if (pfi->m_ptCursor.y >= 0)
            cCmdLine.AppendF(L" -Y=%d", pfi->m_ptCursor.y + 1);
        if (pfi->m_nViewLeftCol >= CLayoutInt(0))
            cCmdLine.AppendF(L" -VX=%d", (Int)pfi->m_nViewLeftCol + 1);
        if (pfi->m_nViewTopLine >= CLayoutInt(0))
            cCmdLine.AppendF(L" -VY=%d", (Int)pfi->m_nViewTopLine + 1);
    }
    SLoadInfo sLoadInfo;
