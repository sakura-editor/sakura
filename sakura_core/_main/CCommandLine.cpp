...
)
{
	...
	*/
	...
	/*!
		...
		後ろに引数を取らないもの
	*/
	static const _CmdLineOpt _COptWoA[] = {{L"R", 1, CMDLINEOPT_R, false},
	...
	/*!
		...
		後ろに引数を取るもの
	*/
	static const _CmdLineOpt _COptWithA[] = {{L"@", 1, CMDLINEOPT_AT, false},
...
void CCommandLine::ParseCommandLine(LPCWSTR pszCmdLineSrc, bool bResponse)
{
	...
	while (pszToken != NULL)
	{
		...
		if ((bParseOptDisabled || !(pszToken[0] == '-' || pszToken[0] == '"' && pszToken[1] == '-')))
		{
			...
			for (int i = 0; i < len;)
			{
				if (!TCODE::IsValidFilenameChar(szPath[i]))
				{
					...
					swprintf(msg_str, _countof(msg_str), LS(STR_CMDLINE_PARSECMD1), szPath);
		...
		else
		{
			...
			case CMDLINEOPT_GREPMODE: //	GREPMODE
				m_bGrepMode = true;
				if (L'\0' == m_fi.m_szDocType[0]) { wcscpy(m_fi.m_szDocType, L"grepout"); }
				break;
			...
			case CMDLINEOPT_GOPT: //	GOPT
				for (; *arg != '\0'; ++arg)
				{
					...
					case 'S':
						// サブフォルダからも検索する
						m_gi.bGrepSubFolder = true;
						break;
					case 'L':
						// 英大文字と英小文字を区別する
						m_gi.sGrepSearchOption.bLoHiCase = true;
						break;
					case 'R':
						// 正規表現
						m_gi.sGrepSearchOption.bRegularExp = true;
						break;
					case 'K':
						// 文字コード自動判別
						// 2002/09/21 Moca 互換性保持のための処理
						m_gi.nGrepCharSet = CODE_AUTODETECT;
						break;
					case 'P':
						// 結果出力：[行を出力]/該当部分/否マッチ行
						m_gi.nGrepOutputLineType = 1;
						break;
					case 'N':
						// 結果出力：行を出力/該当部分/[否マッチ行]
						m_gi.nGrepOutputLineType = 2;
						break;
					case 'W':
						// 単語単位で探す
						m_gi.sGrepSearchOption.bWordOnly = true;
						break;
					case '1':
						// Grep: 出力形式
					...
					case '2':
						// Grep: 出力形式
						m_gi.nGrepOutputStyle = 2;
						break;
					case '3':
						// Grep: 出力形式
						m_gi.nGrepOutputStyle = 3;
						break;
					case 'F': m_gi.bGrepOutputFileOnly = true; break;
					case 'B': m_gi.bGrepOutputBaseFolder = true; break;
					case 'D': m_gi.bGrepSeparateFolder = true; break;
					case 'C': m_gi.bGrepPaste = true; break;
					case 'O': m_gi.bGrepBackup = true; break;
					}
				}
				break;
			// 2002/09/21 Moca Grepでの文字コードセット 追加
			case CMDLINEOPT_GCODE: m_gi.nGrepCharSet = (ECodeType)AtoiOptionInt(arg); break;
			...
			case CMDLINEOPT_DEBUGMODE:
				...
				if (L'\0' == m_fi.m_szDocType[0]) { wcscpy(m_fi.m_szDocType, L"output"); }
				break;
			...
			case CMDLINEOPT_PROF: // 2013.12.20 Moca 追加
				...
				m_bSetProfile = true;
				break;
			case CMDLINEOPT_PROFMGR: m_bProfileMgr = true; break;
			}
		}
	...
	if (cmResponseFile.GetStringLength() && bResponse)
	{
		...
		while (input) { responseData += input.ReadLineW(); }
