...
static void		 Load()
{
	if (!hDll) { hDll = LoadLibraryExedir(L"DebugMonitorLib.dll"); }
}
...
int DebugMonitor_Output(const wchar_t *szInstanceId, const wchar_t *szText)
{
	...
	if (!f) { f = (FN_DebugMonitor_Output)GetProcAddress(hDll, "DebugMonitor_Output"); }
	if (!f) return -1;
...
LPCWSTR GetWindowsMessageName(UINT msg)
{
	...
	if (!f) { f = (FN_GetWindowsMessageName)GetProcAddress(hDll, "GetWindowsMessageName"); }
	if (!f) return L"?";
