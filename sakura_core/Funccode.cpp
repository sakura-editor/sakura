...
/* 機能が利用可能か調べる */
bool IsFuncEnable( const CEditDoc* pcEditDoc, const DLLSHAREDATA* pShareData, int nId )
{
	/* 書き換え禁止のときを一括チェック */
...
/* 機能がチェック状態か調べる */
bool IsFuncChecked( const CEditDoc* pcEditDoc, const DLLSHAREDATA* pShareData, int nId )
{
	CEditWnd* pCEditWnd;
...
//@@@ 2002.01.14 YAZAKI 印刷プレビューをCPrintPreviewに独立させたことにより、プレビュー判定削除
	...
	case F_TOPMOST:					return ((DWORD)::GetWindowLongPtr( pCEditWnd->m_hWnd, GWL_EXSTYLE ) & WS_EX_TOPMOST) != 0;	// 2004.09.21 Moca
