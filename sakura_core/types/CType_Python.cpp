...
{
	return (L'_' == c || (L'a' <= c && c <= L'z') || (L'A' <= c && c <= L'Z') || (L'0' <= c && c <= L'9'));
}
...
int COutlinePython::EnterString(const wchar_t *data, int linelen, int start_offset)
{
	...
	if (data[col] == '\"' || data[col] == '\'') {
		...
		//	文字列の開始
		if (col >= 1 && (data[col - 1] == 'r' || data[col - 1] == 'R')) {
	...
	for (int col = start_offset; col < linelen; ++col) {
		...
		//	文字列
			...
			if ((linelen - 2 == col && (data[col + 1] == WCODE::CR && data[col + 2] == WCODE::LF))
				|| (linelen - 1 == col && (WCODE::IsLineDelimiter(data[col + 1], bExtEol)))) {
	...
	for (int col = start_offset; col < linelen; ++col) {
		...
		if (data[col] == '\\' && col + 1 < linelen) {
			...
			if (WCODE::IsLineDelimiter(key, bExtEol)) {
				// \r\nをまとめて\nとして扱う必要がある
				if (col + 1 >= linelen || data[col + 2] == key) {
					// 本当に行末
					++col;
					continue;
...
void CDocOutline::MakeFuncList_python(CFuncInfoArr *pcFuncInfoArr)
{
	...
	for (nLineCount = CLogicInt(0); nLineCount < m_pcDocRef->m_cDocLineMgr.GetLineCount(); ++nLineCount) {
		...
		if (python_analyze_state.IsLogicalLineTop()) {
			...
			if (WCODE::IsLineDelimiter(pLine[col], bExtEol) || pLine[col] == L'\0' || pLine[col] == L'#') {
			...
			//	skip whitespace
			while (col < nLineLen && C_IsSpace(pLine[col], bExtEol)) ++col;

			int w_end;
			for (w_end = col; w_end < nLineLen && Python_IsWordChar(pLine[w_end]); ++w_end)
				;
			...
			*/
			...
			pcFuncInfoArr->AppendData(nLineCount + CLogicInt(1), ptPosXY.GetY2() + CLayoutInt(1), szWord, nItemFuncId,
									  depth_index);
