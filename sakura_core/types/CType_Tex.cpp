...
class TagProcessor
{
  ...
  public:
    ...
    )
    {
        ...
        {
            if (wcslen(TagHierarchy[depth]) == pTagEnd - pTag && 0 == wcsncmp(TagHierarchy[depth], pTag, pTagEnd - pTag))
            {
                break;
            }
        }
...
template <int HierarchyCount>
inline TagProcessor<HierarchyCount>
...
void CDocOutline::MakeTopicList_tex(CFuncInfoArr *pcFuncInfoArr)
{
    const wchar_t *TagHierarchy[] = {
        ...
        L"subsection",
        L"subsubsection"};
    TagIterator(m_pcDocRef->m_cDocLineMgr).each(MakeTagProcessor(*pcFuncInfoArr, m_pcDocRef->m_cLayoutMgr, TagHierarchy));
}
