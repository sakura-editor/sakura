...
struct SEncodingConfig{
	bool				m_bPriorCesu8 = false;					//!< 自動判別時に CESU-8 を優先するかどうか
	ECodeType			m_eDefaultCodetype = CODE_UTF8;				//!< デフォルト文字コード
	EEolType			m_eDefaultEoltype = EEolType::cr_and_lf;				//!< デフォルト改行コード	// 2011.01.24 ryoji
	bool				m_bDefaultBom = false;					//!< デフォルトBOM			// 2011.01.24 ryoji
};
...
struct STypeConfig{
	...
	int					m_nTextWrapMethod = WRAP_NO_TEXT_WRAP;				//!< テキストの折り返し方法		// 2008.05.30 nasukoji
	CKetaXInt			m_nMaxLineKetas = 10;				//!< 折り返し桁数
	int					m_nColumnSpace = 0;					//!< 文字と文字の隙間
	int					m_nLineSpace = 1;					//!< 行間のすきま
	CKetaXInt			m_nTabSpace = 4;					//!< TABの文字数
	ETabArrow			m_bTabArrow = TABARROW_STRING;					//!< タブ矢印表示		//@@@ 2003.03.26 MIK
	EDIT_CHAR			m_szTabViewString[8+1];			//!< TAB表示文字列	// 2003.1.26 aroka サイズ拡張	// 2009.02.11 ryoji サイズ戻し(17->8+1)
	bool				m_bInsSpace = false;					//!< スペースの挿入	// 2001.12.03 hor
	int					m_nTsvMode = 0;						//!< TSVモード	// 2015.05.02 syat
	// 2005.01.13 MIK 配列化
	...
	int					m_nStringType = STRING_LITERAL_CPP;					//!< 文字列区切り記号エスケープ方法  0=[\"][\'] 1=[""]['']
	bool				m_bStringLineOnly = false;				//!< 文字列は行内のみ
	bool				m_bStringEndLine = false;				//!< (終了文字列がない場合)行末まで色分け
	int					m_nHeredocType = HEREDOC_PHP;
	wchar_t				m_szIndentChars[64]{};			//!< その他のインデント対象文字

	int					m_nColorInfoArrNum = COLORIDX_LAST;				//!< 色設定配列の有効数
	ColorInfo			m_ColorInfoArr[64]{};				//!< 色設定配列

	SFilePath			m_szBackImgPath;				//!< 背景画像
	EBackgroundImagePos m_backImgPos = BGIMAGE_TOP_LEFT;					//!< 背景画像表示位置
	bool				m_backImgRepeatX = true;				//!< 背景画像表示横方向繰り返し
	bool				m_backImgRepeatY = true;				//!< 背景画像表示縦方向繰り返し
	bool				m_backImgScrollX = true;				//!< 背景画像表示横方向スクロール
	bool				m_backImgScrollY = true;				//!< 背景画像表示縦方向スクロール
	POINT				m_backImgPosOffset = { 0, 0 };				//!< 背景画像表示オフセット
	BYTE				m_backImgOpacity = 0xFF;				//!< 背景画像表示不透明度

	bool				m_bLineNumIsCRLF = true;				//!< 行番号の表示 false=折り返し単位／true=改行単位
	int					m_nLineTermType = 1;				//!< 行番号区切り  0=なし 1=縦線 2=任意
	wchar_t				m_cLineTermChar = L':';				//!< 行番号区切り文字
	CKetaXInt			m_nVertLineIdx[MAX_VERTLINES]{};	//!< 指定桁縦線
	int 				m_nNoteLineOffset = 0;				//!< ノート線のオフセット

	bool				m_bWordWrap = false;					//!< 英文ワードラップをする
	bool				m_bKinsokuHead = false;					//!< 行頭禁則をする		//@@@ 2002.04.08 MIK
	bool				m_bKinsokuTail = false;					//!< 行末禁則をする		//@@@ 2002.04.08 MIK
	bool				m_bKinsokuRet = false;					//!< 改行文字のぶら下げ	//@@@ 2002.04.13 MIK
	bool				m_bKinsokuKuto = false;					//!< 句読点のぶらさげ	//@@@ 2002.04.17 MIK
	...
	int					m_nCurrentPrintSetting = 0;			//!< 現在選択している印刷設定
	...
	EOutlineType		m_eDefaultOutline = OUTLINE_TEXT;				//!< アウトライン解析方法
	SFilePath			m_szOutlineRuleFilename;		//!< アウトライン解析ルールファイル
	int					m_nOutlineSortCol = 0;				//!< アウトライン解析ソート列番号
	bool				m_bOutlineSortDesc = false;				//!< アウトライン解析ソート降順
	int					m_nOutlineSortType = 0;				//!< アウトライン解析ソート基準
	SFileTree			m_sFileTree;					/*!< ファイルツリー設定 */

	ESmartIndentType	m_eSmartIndent = SMARTINDENT_NONE;					//!< スマートインデント種別
	bool				m_bIndentCppStringIgnore = true;		//!< C/C++インデント：文字列を無視する
	bool				m_bIndentCppCommentIgnore = true;		//!< C/C++インデント：コメントを無視する
	bool				m_bIndentCppUndoSep = false;			//!< C/C++インデント：Undoバッファを分ける

	bool				m_bDetectIndentationStyleOnFileLoad = false;	//!< ファイル読み込み時にインデントスタイルを検出して一時設定する

	int					m_nImeState = IME_CMODE_NOCONVERSION;					//!< 初期IME状態	Nov. 20, 2000 genta

	//	2001/06/14 asa-o 補完のタイプ別設定
	SFilePath			m_szHokanFile;					//!< 入力補完 単語ファイル
	int					m_nHokanType = 0;					//!< 入力補完 種別(プラグイン)
	//	2003.06.23 Moca ファイル内からの入力補完機能
	bool				m_bUseHokanByFile = true;				//!< 入力補完 開いているファイル内から候補を探す
	bool				m_bUseHokanByKeyword = false;			//!< 強調キーワードから入力補完
	
	//	2001/06/19 asa-o
	bool				m_bHokanLoHiCase = false;				//!< 入力補完機能：英大文字小文字を同一視する

	SFilePath			m_szExtHelp;					//!< 外部ヘルプ１
	SFilePath			m_szExtHtmlHelp;				//!< 外部HTMLヘルプ
	bool				m_bHtmlHelpIsSingle = true;			//!< HtmlHelpビューアはひとつ

	bool				m_bChkEnterAtEnd = true;				//!< 保存時に改行コードの混在を警告する	2013/4/14 Uchi

	SEncodingConfig		m_encoding{};						//!< エンコードオプション

//@@@ 2001.11.17 add start MIK
	bool				m_bUseRegexKeyword = false;								//!< 正規表現キーワードを使うか
	DWORD				m_nRegexKeyMagicNumber = 0;							//!< 正規表現キーワード更新マジックナンバー
...
//@@@ 2006.04.10 fon ADD-start
	bool				m_bUseKeyWordHelp = false;				//!< キーワード辞書セレクト機能を使うか
	int					m_nKeyHelpNum = 0;					//!< キーワード辞書の冊数
	KeyHelpInfo			m_KeyHelpArr[MAX_KEYHELP_FILE]{};	//!< キーワード辞書ファイル
	bool				m_bUseKeyHelpAllSearch = false;			//!< ヒットした次の辞書も検索(&A)
	bool				m_bUseKeyHelpKeyDisp = false;			//!< 1行目にキーワードも表示する(&W)
	bool				m_bUseKeyHelpPrefix = false;			//!< 選択範囲で前方一致検索(&P)
	EKeyHelpRMenuType	m_eKeyHelpRMenuShowType = KEYHELP_RMENU_TOP;		//!< 右クリックメニュー表示
//@@@ 2006.04.10 fon ADD-end

	//	2002/04/30 YAZAKI Commonから移動。
	bool				m_bAutoIndent = true;					//!< オートインデント
	bool				m_bAutoIndent_ZENSPACE = true;			//!< 日本語空白もインデント
	bool				m_bRTrimPrevLine = false;				//!< 2005.10.11 ryoji 改行時に末尾の空白を削除
	int					m_nIndentLayout = 0;				//!< 折り返しは2行目以降を字下げ表示

	//	Sep. 10, 2002 genta
	bool				m_bUseDocumentIcon = false;				//!< ファイルに関連づけられたアイコンを使う

	bool				m_bUseTypeFont = false;					//!< タイプ別フォントの使用
	LOGFONT				m_lf{};							//!< フォント // 2013.03.18 aroka
	INT					m_nPointSize = 10;					//!< フォントサイズ（1/10ポイント単位）

	int					m_nLineNumWidth = LINENUMWIDTH_MIN;				//!< 行番号の最小桁数 2014.08.02 katze

	STypeConfig() noexcept;
