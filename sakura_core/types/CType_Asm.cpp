...
{
	//名前と拡張子
	wcscpy_literal( pType->m_szTypeName, L"アセンブラ" );
	wcscpy_literal( pType->m_szTypeExts, L"asm" );
		...
		if( token[ 0 ] != NULL ){	//トークンが1個以上ある
			...
			if( length >= 2
			 && token[ 0 ][ length - 1 ] == L':' ){	//ラベル
				...
				if( wcsicmp_literal( token[ 1 ], L"proc" ) == 0 ){	//関数
					nFuncId = 50;
					entry_token = token[ 0 ];
				}else
				if( wcsicmp_literal( token[ 1 ], L"endp" ) == 0 ){	//関数終了
					nFuncId = 52;
					entry_token = token[ 0 ];
				//}else
