...
{
	//名前と拡張子
	wcsncpy( pType->m_szTypeName, L"テキスト" );
	wcsncpy( pType->m_szTypeExts, L"txt,log,1st,err,ps" );
	...
	wcsncpy( pType->m_szKinsokuHead, L"!%),.:;?]}\xa2°’”‰′″℃、。々〉》」』】〕゛゜ゝゞ・ヽヾ！％），．：；？］｝｡｣､･ﾞﾟ￠" );		/* 行頭禁則 */	//@@@ 2002.04.13 MIK 
	wcsncpy( pType->m_szKinsokuTail, L"$([\\{\xa3\xa5‘“〈《「『【〔＄（［｛｢￡￥" );		/* 行末禁則 */	//@@@ 2002.04.08 MIK 
...
void CDocOutline::MakeTopicList_txt( CFuncInfoArr* pcFuncInfoArr )
{
	...
	*/
	...
	for( nLineCount = CLogicInt(0); nLineCount <  m_pcDocRef->m_cDocLineMgr.GetLineCount(); ++nLineCount )
	{
		...
		//見出し種類の判別 -> szTitle
		if( pLine[i] == L'(' ){
			     if ( IsInRange(pLine[i + 1], L'0', L'9') ) wcsncpy( szTitle, L"(0)" ); //数字
			else if ( IsInRange(pLine[i + 1], L'A', L'Z') ) wcsncpy( szTitle, L"(A)" ); //英大文字
			else if ( IsInRange(pLine[i + 1], L'a', L'z') ) wcsncpy( szTitle, L"(a)" ); //英小文字
			else continue; //※「(」の次が英数字で無い場合、見出しとみなさない
		}
		else if( IsInRange(pLine[i], L'０', L'９') ) wcsncpy( szTitle, L"０" ); // 全角数字
		else if( IsInRange(pLine[i], L'①', L'⑳') || pLine[i] == L'\u24ea'
			|| IsInRange(pLine[i], L'\u3251', L'\u325f') || IsInRange(pLine[i], L'\u32b1', L'\u32bf') ) wcsncpy( szTitle, L"①" ); // ①～⑳ ○0　○21○35　○36○50
		else if( IsInRange(pLine[i], L'Ⅰ', L'\u216f') ) wcsncpy( szTitle, L"Ⅰ" ); // Ⅰ～Ⅹ　XIXIILCDM
		else if( IsInRange(pLine[i], L'ⅰ', L'\u217f') ) wcsncpy( szTitle, L"Ⅰ" ); // Ⅰ～Ⅹ　xixiilcdm
		else if( IsInRange(pLine[i], L'\u2474', L'\u2487') ) wcsncpy( szTitle, L"\u2474" ); // (1)-(20)
		else if( IsInRange(pLine[i], L'\u2488', L'\u249b') ) wcsncpy( szTitle, L"\u2488" ); // 1.-20.
		else if( IsInRange(pLine[i], L'\u249c', L'\u24b5') ) wcsncpy( szTitle, L"\u249c" ); // (a)-(z)
		else if( IsInRange(pLine[i], L'\u24b6', L'\u24cf') ) wcsncpy( szTitle, L"\u24b6" ); // ○A-○Z
		else if( IsInRange(pLine[i], L'\u24d0', L'\u24e9') ) wcsncpy( szTitle, L"\u24d0" ); // ○a-○z
		else if( IsInRange(pLine[i], L'\u24eb', L'\u24f4') ){ // ●11-●20
			if(b278a){ wcsncpy( szTitle, L"\u278a" ); }
			else{ wcsncpy( szTitle, L"\u2776" ); } }
		else if( IsInRange(pLine[i], L'\u24f5', L'\u24fe') ) wcsncpy( szTitle, L"\u24f5" ); // ◎1-◎10
		else if( IsInRange(pLine[i], L'\u2776', L'\u277f') ) wcsncpy( szTitle, L"\u2776" ); // ●1-●10
		else if( IsInRange(pLine[i], L'\u2780', L'\u2789') ) wcsncpy( szTitle, L"\u2780" ); // ○1-○10
		else if( IsInRange(pLine[i], L'\u278a', L'\u2793') ){ wcsncpy( szTitle, L"\u278a" ); b278a = true; } // ●1-●10(SANS-SERIF)
		else if( IsInRange(pLine[i], L'\u3220', L'\u3229') ) wcsncpy( szTitle, L"\ua3220" ); // (一)-(十)
		else if( IsInRange(pLine[i], L'\u3280', L'\u3289') ) wcsncpy( szTitle, L"\u3220" ); // ○一-○十
		else if( IsInRange(pLine[i], L'\u32d0', L'\u32fe') ) wcsncpy( szTitle, L"\u32d0" ); // ○ア-○ヲ
		else if( wcschr(L"〇一二三四五六七八九十百零壱弐参伍", pLine[i]) ) wcsncpy( szTitle, L"一" ); //漢数字
		else{
