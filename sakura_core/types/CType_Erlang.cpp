...
private:
	// helper functions
	bool IS_ATOM_HEAD(wchar_t wc) { return (L'a' <= wc && wc <= L'z') || (wc == L'_') || (wc == L'@'); }

	bool IS_ALNUM(wchar_t wc) { return IS_ATOM_HEAD(wc) || (L'A' <= wc && wc <= L'Z') || (L'0' <= wc && wc <= L'9'); }

	bool IS_COMMENT(wchar_t wc) { return (wc == L'%'); }

	bool IS_SPACE(wchar_t wc) { return (wcschr(L" \t\r\n", wc) != 0); }
...
const wchar_t *COutlineErlang::ScanFuncName(const wchar_t *buf, const wchar_t *end, const wchar_t *p)
{
	...
	if (p > buf || !(IS_ATOM_HEAD(*p) || *p == L'\'')) { return end; }
...
const wchar_t *COutlineErlang::EnterArgs(const wchar_t *end, const wchar_t *p)
{
	...
	while (IS_SPACE(*p) && p < end) p++;

	if (p >= end) return end;

	if (IS_COMMENT(*p)) { return end; }
	else if (*p == L'(') { // )
...
const wchar_t *COutlineErlang::ScanArgs(const wchar_t *end, const wchar_t *p)
{
	...
	for (const wchar_t *head = p; p < end; p++) {
		if (quote) {
			if (*p == quote) quote = L'\0';
		}
		else {
			if (wcschr(L"([{", *p)) { //)
				// level up
				if (m_parenthesis_ptr < parptr_max) { m_parenthesis[m_parenthesis_ptr] = *p; }
				m_parenthesis_ptr++;
			}
			else if (wcschr(L")]}", *p)) { //)
				wchar_t op;
...
bool COutlineErlang::parse(const wchar_t *buf, int linelen, CLogicInt linenum)
{
	...
	if (m_state == STATE_FUNC_FOUND) { m_state = STATE_NORMAL; }
	if (m_state == STATE_NORMAL) {
		pos = ScanFuncName(buf, end, pos);
		if (m_state != STATE_NORMAL) { m_lnum = linenum; }
	}
	while (pos < end) {
		switch (m_state) {
		case STATE_FUNC_CANDIDATE_FIN: pos = EnterArgs(end, pos); break;
		case STATE_FUNC_ARGS1: pos = ScanArgs1(end, pos); break;
		case STATE_FUNC_ARGS: pos = ScanArgs(end, pos); break;
		case STATE_FUNC_ARGS_FIN: pos = EnterCond(end, pos); break;
		default: PleaseReportToAuthor(NULL, L"COutlineErlang::parse Unknown State: %d", m_state); break;
		}
...
void COutlineErlang::build_arity(int arity)
{
	...
	if (len + 1 >= buf_size) return; // no room
	...
	for (nLineCount = CLogicInt(0); nLineCount < m_pcDocRef->m_cDocLineMgr.GetLineCount(); ++nLineCount) {
		...
		if (erl_state_machine.parse(pLine, nLineLen, nLineCount)) {
			...
			*/
			...
			m_pcDocRef->m_cLayoutMgr.LogicToLayout(CLogicPoint(CLogicInt(0), erl_state_machine.GetFuncLine()),
												   &ptPosXY);
			pcFuncInfoArr->AppendData(erl_state_machine.GetFuncLine() + CLogicInt(1), ptPosXY.GetY2() + CLayoutInt(1),
									  erl_state_machine.GetFuncName(), 0, 0);
		}
	}
}
