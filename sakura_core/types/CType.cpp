...
#define PopulateKeyword(name, case_sensitive, aryname)                                                                 \
	...
	pShareData->m_Common.m_sSpecialKeyword.m_CKeyWordSetMgr.SetKeyWordArr(++nSetCount, g_nKeywords##aryname,           \
																		  g_ppszKeywords##aryname);
...
void _DefaultConfig(STypeConfig *pType)
{
	...
	for (int i = 0; i < MAX_VERTLINES; i++) { pType->m_nVertLineIdx[i] = CKetaXInt(0); }
