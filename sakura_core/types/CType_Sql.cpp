				...
				if ((1 == nCharChars
					...
					if (0 == nParseCnt && 0 == _wcsicmp(szWord, L"FUNCTION")) {
						...
						if (0 == _wcsicmp(szWord, L"IS")) {
							...
							*/
							CLayoutPoint ptPos;
							m_pcDocRef->m_cLayoutMgr.LogicToLayout(CLogicPoint(0, nFuncLine - 1), &ptPos);
						...
						if (0 == _wcsicmp(szWord, L"AS")) {
							if (3 == nFuncOrProc) {
								...
								*/
								CLayoutPoint ptPos;
								m_pcDocRef->m_cLayoutMgr.LogicToLayout(CLogicPoint(0, nFuncLine - 1), &ptPos);
								pcFuncInfoArr->AppendData(nFuncLine, ptPos.GetY2() + CLayoutInt(1), szFuncName,
														  nFuncId);
								nParseCnt = 0;
							} else if (4 == nFuncOrProc) {
								nFuncId = 41; /* パッケージ本体 */
...
const wchar_t *g_ppszKeywordsPLSQL[] = {L"AND",
										L"AS",
										L"BEGIN",
										L"BINARY_INTEGER",
										L"BODY",
