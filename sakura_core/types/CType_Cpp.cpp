...
{
    if (wcsncmp_literal(pData, L"case") == 0)
        return true;
    if (wcsncmp_literal(pData, L"default:") == 0)
        return true;
    if (wcsncmp_literal(pData, L"public:") == 0)
        return true;
    if (wcsncmp_literal(pData, L"private:") == 0)
        return true;
    if (wcsncmp_literal(pData, L"protected:") == 0)
        return true;
    return false;
}
...
{
    if (len > 0 && WCODE::IsLineDelimiter(s[len - 1], GetDllShareData().m_Common.m_sEdit.m_bEnableExtEol))
        len--;
...
/*!
	...
	ここに関数のリストを登録する。
*/
void CDocOutline::MakeFuncList_C(CFuncInfoArr *pcFuncInfoArr, EOutlineType &nOutlineType, const WCHAR *pszFileName, bool bVisibleMemberFunc)
{
#ifdef _DEBUG
// #define TRACE_OUTLINE
	...
	*/
    ...
    for (nLineCount = CLogicInt(0); nLineCount < m_pcDocRef->m_cDocLineMgr.GetLineCount(); ++nLineCount)
    {
        ...
        {
#ifdef TRACE_OUTLINE
            DEBUG_TRACE(L"%2d [%lc] %d %x %d %d %d wd[%ls] pre[%ls] tmp[%ls] til[%ls] %d\n", int((Int)i), pLine[i], nMode, nMode2, nNestLevel_global, nNestLevel_func, nNestLevel_fparam, szWord, szWordPrev, szTemplateName, szItemName, nWordIdx);
#endif
                ...
                if (2 == nMode)
            {
                ...
                else
                {
                    ...
                    if (nMode2 == M2_OPERATOR_WORD && L'<' == pLine[i])
                    {
                        ...
                        if ((8 <= nWordIdx && wcsncmp_literal(p, L"operator<") == 0) || ((9 <= nWordIdx && wcsncmp_literal(p - 1, L"operator<<") == 0) && 0 < i && L'<' == pLine[i - 1]))
                        {
                ...
                if (0 == nMode)
            {
                ...
                else if ('(' == pLine[i])
                {
                    ...
                    if (!bOperator && nNestLevel_func == 0 && (nMode2 == M2_NORMAL || nMode2 == M2_NAMESPACE_SAVE || nMode2 == M2_OPERATOR_WORD || nMode2 == M2_KR_FUNC))
                    {
                        if (nNestLevel_fparam == 0)
                        {
                            bool bAdd = true;
                            if (wcscmp(szWordPrev, L"__declspec") == 0 || wcscmp(szWordPrev, L"alignas") == 0 || wcscmp(szWordPrev, L"decltype") == 0 || wcscmp(szWordPrev, L"_Alignas") == 0 || wcscmp(szWordPrev, L"__attribute__") == 0)
                            {
                                bAdd = false;
                            }
                            else
                            {
                                if (!(nMode2 == M2_KR_FUNC && bInInitList) && !(nMode2 == M2_KR_FUNC && CPP_IsFunctionAfterKeyword(szWordPrev))) // 2010.07.08 ryoji 初期化リストに入る以前までは後発の名前を優先的に関数名候補とする
                                {
                                    bNoFunction = false;
                                }
                                else
                    ...
                    if ('[' == pLine[i])
                {
                    ...
                    if (nMode2 == M2_NORMAL && C_IsOperator(szWordPrev, nLen))
                    {
                        ...
                        nMode2 = M2_OPERATOR_WORD;
                             ...
                             (nMode2 == M2_NORMAL || nMode2 == M2_NAMESPACE_SAVE || nMode2 == M2_TEMPLATE_SAVE || nMode2 == M2_OPERATOR_WORD || nMode2 == M2_TEMPLATE_WORD))
                    {
                        nMode2AttOld = nMode2;
                        nMode2       = M2_ATTRIBUTE;
                    }
                ...
                else if (nNestLevel_fparam == 0 && nMode2 != M2_ATTRIBUTE)
                {
                    ...
                    else
                    {
                        ...
                        int nLen = (int)wcslen(szWordPrev);
                        if (pLine[i] == L'=' && nNestLevel_func == 0 && nMode2 == M2_NORMAL && !C_IsOperator(szWordPrev, nLen))
                        {
