...
EOutlineType GetCLangOutlineType(const wchar_t *pszFileName)
{
	...
	if (CheckEXT(pszFileName, L"c")) { eOutlineType = OUTLINE_C; }
	else if (CheckEXT(pszFileName, L"cpp") || CheckEXT(pszFileName, L"c++") || CheckEXT(pszFileName, L"cxx")
			 || CheckEXT(pszFileName, L"hpp") || CheckEXT(pszFileName, L"h++") || CheckEXT(pszFileName, L"hxx"))
	{
...
void CType_Cpp::InitTypeConfigImp(STypeConfig *pType)
{
	...
	pType->m_cBlockComments[1].SetBlockCommentRule(L"#if 0", L"#endif");
	/* ブロックコメントデリミタ2 */									 // Jul. 11, 2001 JEPRO
...
{
	return (L'_' == c || L':' == c || L'~' == c || (L'a' <= c && c <= L'z') || (L'A' <= c && c <= L'Z')
			|| (L'0' <= c && c <= L'9') || (0xa1 <= c && !iswcntrl(c) && !iswspace(c)));
}
//	To Here
...
static bool C_IsOperator(wchar_t *szStr, int nLen)
{
	...
	{
		if (nLen > 8 ? wcscmp(szStr + nLen - 9, L":operator") == 0 : // メンバー関数による定義
...
static bool C_IsLineEsc(const wchar_t *s, int len)
{
	...
	if (len > 0 && s[len - 1] == L'\\')
	{
		...
		{
			if (CNativeW::GetSizeOfChar(s, 2, 0) == 1) return (true);
		}
		else
		{ //残り３バイト以上
			if (CNativeW::GetSizeOfChar(s, len, len - 2) == 1) return (true);
			if (CNativeW::GetSizeOfChar(s, len, len - 3) == 2) return (true);
		}
	}
	return (false);
}
...
{
	return wcscmp(s, L"const") == 0 || wcscmp(s, L"volatile") == 0 || wcscmp(s, L"throw") == 0
		   || wcscmp(s, L"default") == 0 || wcscmp(s, L"delete") == 0 || wcscmp(s, L"override") == 0
		   || wcscmp(s, L"final") == 0 || wcscmp(s, L"noexcept") == 0;
}
...
CLogicInt CCppPreprocessMng::ScanLine(const wchar_t *str, CLogicInt _length)
{
	...
	for (p = str; C_IsSpace(*p, bExtEol) && p < lastptr; ++p)
		;
	if (lastptr <= p) return CLogicInt(length); //	空行のため処理不要
	...
	if (p + 2 + 2 < lastptr && wcsncmp_literal(p, L"if") == 0)
	{
		...
		else if ((p + 3 < lastptr && wcsncmp_literal(p, L"def") == 0)
		...
		if (enable > 0)
		{
			...
			++m_stackptr;
			if (enable == 1) { m_enablebuf |= m_bitpattern; }
		}
	}
	else if (p + 4 < lastptr && wcsncmp_literal(p, L"else") == 0)
	{
		//	2007.12.14 genta : #ifが無く#elseが出たときのガード追加
		if (0 < m_stackptr && m_stackptr < m_maxnestlevel) { m_enablebuf ^= m_bitpattern; }
	}
...
/*!
	...
	ここに関数のリストを登録する。
*/
void CDocOutline::MakeFuncList_C(CFuncInfoArr *pcFuncInfoArr, EOutlineType &nOutlineType, const WCHAR *pszFileName,
...
// #define TRACE_OUTLINE
	...
	int nNestLevel_func   = 0; //	nNestLevel_func 関数の定義、および関数内の	{}のレベル
							   //	int			nNestLevel2;			//	nNestLevel2	()に対する位置 // 2002/10/27 frozen
							   // nNastLevel_fparamとnMode2のM2_FUNC_NAME_ENDで代用
	int nNestLevel_fparam = 0; // ()のレベル
	int nNestPoint_class  = 0; // 外側から何番目の{がクラスの定義を囲む{か？
							  // (一番外側なら1、0なら無し。bVisibleMemberFuncがfalseの時のみ有効。trueでは常に0)
	// 2002/10/27 frozen　ここまで

	bool bInInitList =
		false; // 2010.07.08 ryoji 関数名調査の際、現在位置が初期化リスト（':'以後）に到達したかどうかを示す
	...
	*/
	...
	enum MODE2
	{
		...
		M2_NAMESPACE_SAVE =
			0x11, //!< ネームスペース名調査中
				  // 「通常」状態で単語 "class" "struct" "union" "enum" "namespace", "__interface"
				  // を読み込むと、この状態になり、';' '{' ',' '>' '='を読み込むと「通常」になる。
				  //	2007.05.26 genta キーワードに__interface追加
				  //
				  // ':' を読み込むと「ネームスペース名調査完了」へ移行すると同時に
				  // szWordをszItemNameに保存し、あとで ':' 又は '{' の直前の単語が調べられるようにしている。
				  // これは "__declspec( dllexport
				  // )"のように"class"とクラス名の間にキーワードが書いてある場合でもクラス名を取得できるようにするため。
				  //
		...
		M2_TEMPLATE_WORD = 0x15, //!< テンプレート特殊化を調査中 func<int>()等 '単語
								 //!< <'でM2_TEMPLATE_WORDになり、'>'(ネスト認識)でM2_NORMAL/M2_OPERATOR_WORDに戻る
	...
	wchar_t   szItemName[nItemNameLenMax]; //!< すぐ前の 関数名 or クラス名 or 構造体名 or 共用体名 or 列挙体名 or
										 //!< ネームスペース名
	wchar_t szTemplateName[nItemNameLenMax]; //!< すぐ前の 関数名 or クラス名 or 構造体名 or 共用体名 or 列挙体名 or
											 //!< ネームスペース名
	// 例えば下のコードの←の部分での
	...
	for (nLineCount = CLogicInt(0); nLineCount < m_pcDocRef->m_cDocLineMgr.GetLineCount(); ++nLineCount)
	{
		...
		for (; i < nLineLen; ++i)
		{
			...
			DEBUG_TRACE(L"%2d [%lc] %d %x %d %d %d wd[%ls] pre[%ls] tmp[%ls] til[%ls] %d\n", int((Int)i), pLine[i],
						nMode, nMode2, nNestLevel_global, nNestLevel_func, nNestLevel_fparam, szWord, szWordPrev,
						szTemplateName, szItemName, nWordIdx);
#endif
			...
			{
				if (!C_IsLineEsc(pLine, nLineLen)) { nMode = 0; }
				i = nLineLen;
				continue;
			}
			...
			else if (21 == nMode)
			{
				...
				if ('"' == pLine[i])
				{
					if (nRawStringTagLen)
					{
						// R"abc(test
						// )abc"
						if (nRawStringTagLen <= i
							&& wcsncmp(szRawStringTag, &pLine[i - nRawStringTagLen], nRawStringTagCompLen) == 0)
						{
							nRawStringTagLen = 0;
							nMode			 = 0;
							continue;
			...
			else if (1 == nMode)
			{
					...
					else
					{
						if (pLine[i] == L':')
						{
							...
							else
							{
								...
								{
									if (szWord[0] != '\0') wcscpy(szItemName, szWord);
				...
				else
				{
					...
					else if (nMode2 == M2_TEMPLATE_SAVE || nMode2 == M2_TEMPLATE_WORD)
					{
						...
						if (wcscmp(L"final", szWord) != 0 || nNestLevel_template != 0)
						{
							...
							if (0 < nLen && C_IsWordChar(szTemplateName[nLen - 1]) && szTemplateName[nLen - 1] != L':'
								&& szWord[nWordIdx] != L':')
							{
						...
						&& (nMode2 == M2_NORMAL
							|| nMode2
								   == M2_FUNC_NAME_END)) // 2010.07.08 ryoji
														 // 関数型マクロ呼出しを関数と誤認することがある問題対策として
														 // nMode2 == M2_FUNC_NAME_END
														 // 条件を追加し、補正がかかるようにした。
					{
						if (nMode2 == M2_NORMAL) nItemFuncId = 0;
						...
						else if (wcscmp(szWord, L"__interface")
								 == 0) // 2007.05.26 genta "__interface" をクラスに類する扱いにする
							nItemFuncId = FL_OBJ_INTERFACE;
						else if (wcscmp(szWord, L"typedef") == 0)
							bDefinedTypedef = true;
						if (nItemFuncId != 0
							&& nItemFuncId != FL_OBJ_FUNCTION) //  2010.07.08 ryoji nMode2 == M2_FUNC_NAME_END のときは
															   //  nItemFuncId == 2 のはず
						{
					...
					if (!(wcscmp(L"PARAMS", szWord) == 0 || wcscmp(L"_", szWord) == 0)) wcscpy(szWordPrev, szWord);
					nWordIdx  = 0;
					szWord[0] = L'\0';
					nMode	 = 0;
					i--;
				...
				if (2 == nMode)
			{
				if (C_IsWordChar(pLine[i]) || C_IsSpace(pLine[i], bExtEol) || '{' == pLine[i] || '}' == pLine[i]
					|| '(' == pLine[i] || ')' == pLine[i] || ';' == pLine[i] || '\'' == pLine[i] || '"' == pLine[i]
				...
				else
				{
					...
					if (nMode2 == M2_OPERATOR_WORD && L'<' == pLine[i])
					{
						...
						if ((8 <= nWordIdx && wcsncmp_literal(p, L"operator<") == 0)
							|| ((9 <= nWordIdx && wcsncmp_literal(p - 1, L"operator<<") == 0) && 0 < i
								&& L'<' == pLine[i - 1]))
						{
					...
					{
						if (pLine[i] == L'<') { nNestLevel_template++; }
						else if (pLine[i] == L'>')
						{
							...
							{
								if (nMode2 == M2_TEMPLATE) { nMode2 = M2_NORMAL; }
			...
			{
				/* 空白やタブ記号等を飛ばす */
				if (C_IsSpace(pLine[i], bExtEol)) continue;
				...
				else if ('"' == pLine[i])
				{
					...
					{
						if (wcscmp(szWordPrev, L"R") == 0 || wcscmp(szWordPrev, L"LR") == 0
							|| wcscmp(szWordPrev, L"uR") == 0 || wcscmp(szWordPrev, L"UR") == 0
					...
					if ('{' == pLine[i])
				{
					...
					if (nMode2 == M2_FUNC_NAME_END || nMode2 == M2_KR_FUNC) { bAddFunction = true; }
					int nItemNameLen = 0;
					int nLenDefPos   = wcslen(LS(STR_OUTLINE_CPP_DEFPOS));
					if (nNestLevel_func != 0 || (szWordPrev[0] == L'=' && szWordPrev[1] == L'\0')
						|| nMode2 == M2_AFTER_EQUAL)
						++nNestLevel_func;
					else if ((nMode2 & M2_AFTER_ITEM) != 0 && nNestLevel_global < nNamespaceNestMax
							 && (nNamespaceLen[nNestLevel_global] + (nItemNameLen = wcslen(szItemName)) + nLenDefPos
								 + 1)
									< nNamespaceLenMax
							 && (nItemLine > 0))
					// ３番目の(&&の後の)条件
					...
					// +1は終端NUL文字
					{
						...
						//	中括弧に遭遇する．
						if (bAddFunction) { ++nNestLevel_func; }
						else
						{
							++nNestLevel_global;
						...
						*/
						...
						m_pcDocRef->m_cLayoutMgr.LogicToLayout(CLogicPoint(0, nItemLine - 1), &ptPosXY);
#ifdef TRACE_OUTLINE
					...
					if ('}' == pLine[i])
				{
					...
					if (nNestLevel_func == 0)
					{
						...
						{
							if (nNestLevel_global == nNestPoint_class) nNestPoint_class = 0;
							--nNestLevel_global;
						}
					}
					else
				...
				else if ('(' == pLine[i])
				{
					...
					if (!bOperator && nNestLevel_func == 0
						&& (nMode2 == M2_NORMAL || nMode2 == M2_NAMESPACE_SAVE || nMode2 == M2_OPERATOR_WORD
							|| nMode2 == M2_KR_FUNC))
					{
						if (nNestLevel_fparam == 0)
						{
							bool bAdd = true;
							if (wcscmp(szWordPrev, L"__declspec") == 0 || wcscmp(szWordPrev, L"alignas") == 0
								|| wcscmp(szWordPrev, L"decltype") == 0 || wcscmp(szWordPrev, L"_Alignas") == 0
								|| wcscmp(szWordPrev, L"__attribute__") == 0)
							{ bAdd = false; } else
							{
								if (!(nMode2 == M2_KR_FUNC && bInInitList)
									&& !(nMode2 == M2_KR_FUNC
										 && CPP_IsFunctionAfterKeyword(
												szWordPrev))) // 2010.07.08 ryoji
															  // 初期化リストに入る以前までは後発の名前を優先的に関数名候補とする
								{ bNoFunction = false; } else
								{
									bAdd = false;
								}
							}
							if (bAdd)
							{
								if (szTemplateName[0]) { wcscpy(szItemName, szTemplateName); }
								else
								{
				...
				else if (')' == pLine[i])
				{
					...
					if (nNestLevel_fparam > 0)
					{
						...
						{
							if (nMode2 == M2_NORMAL) bInInitList = false;
							if (!bNoFunction)
							{
								nMode2		= M2_FUNC_NAME_END;
					...
					if ('[' == pLine[i])
				{
					...
					else if (nNestLevel_func == 0 && nNestLevel_fparam == 0
					...
					if (';' == pLine[i])
				{
					...
					if (nMode2 == M2_FUNC_NAME_END && nNestLevel_global < nNamespaceNestMax
				...
				else if (nNestLevel_fparam == 0 && nMode2 != M2_ATTRIBUTE)
				{
					...
					else
					{
						...
						int nLen = (int)wcslen(szWordPrev);
						if (pLine[i] == L'=' && nNestLevel_func == 0 && nMode2 == M2_NORMAL
							&& !C_IsOperator(szWordPrev, nLen))
						{ nMode2 = M2_AFTER_EQUAL; } else if (nMode2 == M2_NORMAL && C_IsOperator(szWordPrev, nLen))
						{
							// 演算子のオペレータだった operator +
							wcscpy(szWord, szWordPrev);
							nWordIdx = (int)nLen - 1;
...
#endif
							if (pLine[i] == L'<') nMode2 = M2_TEMPLATE_SAVE;
						}
						nMode = 2;
						i--;
					}
...
static bool IsCommentBlock(std::vector<SCommentBlock> &arr, int pos)
{
	...
	{
		if (arr[i].nFirst <= pos && pos <= arr[i].nLast) { return true; }
	}
	return false;
}
...
void CEditView::SmartIndent_CPP(wchar_t wcChar)
{
	...
	case L'(':
		...
		pLine =
			m_pcEditDoc->m_cDocLineMgr.GetLine(GetCaret().GetCaretLogicPos().GetY2())->GetDocLineStrWithEOL(&nLineLen);
		if (NULL == pLine)
		{
			if (WCODE::CR != wcChar) { return; }
			/* 調整によって置換される箇所 */
		...
		else
		{
			...
			//	nWorkに処理の基準桁位置を設定する
			if (WCODE::CR != wcChar) { nWork = nCaretPosX_PHY - 1; }
			else
			{
				/*
				...
				*/
				...
				{
					if (WCODE::TAB != pLine[i] && WCODE::SPACE != pLine[i]) { break; }
				}
				if (i < nLineLen)
				{
					// 2005-09-02 D.S.Koba GetSizeOfChar
					nCharChars = CNativeW::GetSizeOfChar(pLine, nLineLen, i);
					if (1 == nCharChars && (pLine[i] == L')' || pLine[i] == L'}')) { wcChar = pLine[i]; }
					nCaretPosX_PHY = i;
			...
			if (i < nWork)
			{
				if ((L':' == wcChar && IsHeadCppKeyword(&pLine[i]))
					//	Sep. 18, 2002 かろと
					|| (L'{' == wcChar && L'#' != pLine[i]) || (L'(' == wcChar && L'#' != pLine[i]))
				{} else
				{
					return;
				}
		...
		for (j = GetCaret().GetCaretLogicPos().GetY2(); j >= CLogicInt(0); --j)
		{
			...
			{
				if (NULL == pLine2) break;
				nCharChars =
					CLogicInt(&pLine2[nLineLen2] - CNativeW::GetCharPrev(pLine2, nLineLen2, &pLine2[nLineLen2]));
			...
			if (bCommentStringCheck)
			{
				...
				for (int n = 0; n < nLineLen2; n++)
				{
					...
					case 0:
						...
						else if (m_pTypeData->m_bIndentCppCommentIgnore && n + 1 < nLineLen2 && '/' == pLine2[n]
								 && '*' == pLine2[n + 1])
						{
							nMode  = 3;
							nBegin = n;
			...
			for (; k >= 0; /*k--*/)
			{
				...
				{
					if (0 < k && L'\'' == pLine2[k - 1] && nLineLen2 - 1 > k && L'\'' == pLine2[k + 1])
					{
						//						MYTRACE( L"▼[%ls]\n", pLine2 );
					}
				...
				if (0 == nCharChars) { nCharChars = CLogicInt(1); }
				k -= nCharChars;
			}
			if (k < 0)
			{
...

			{
				...
				{
					if (WCODE::TAB != pLine2[m] && WCODE::SPACE != pLine2[m]) { break; }
				}
				nDataLen = CLogicInt(m);
			}

			nCharChars = (m_pcEditDoc->m_cDocType.GetDocumentAttribute().m_bInsSpace)
							 ? (Int)m_pcEditDoc->m_cLayoutMgr.GetTabSpaceKetas()
							 : Int(1);
			...
			if (WCODE::CR == wcChar || L'{' == wcChar || L'(' == wcChar)
			{
				...
				if (m_pcEditDoc->m_cDocType.GetDocumentAttribute().m_bInsSpace)
				{ // SPACE挿入設定
					int		  i;
					...
					for (int i = 0; i < nCharChars; i++) pszData[nDataLen + i] = WCODE::SPACE;
		...
		if ((0 == nDataLen && sRangeLayout.IsOne()) || (nDataLen == nSrcLen && 0 == wmemcmp(pszSrc, pszData, nDataLen)))
		{ bChange = FALSE; } else
		{
			bChange = TRUE;
			if (m_pTypeData->m_bIndentCppUndoSep)
			{
				...
				SetUndoBuffer();
				if (m_cCommander.GetOpeBlk() == NULL) { m_cCommander.SetOpeBlk(new COpeBlk); }
				m_cCommander.GetOpeBlk()->AddRef();
			}

			/* データ置換 削除&挿入にも使える */
			ReplaceData_CEditView(sRangeLayout, pszData, /* 挿入するデータ */
								  nDataLen,				 /* 挿入するデータの長さ */
								  true, m_bDoing_UndoRedo ? NULL : m_cCommander.GetOpeBlk());
		}
