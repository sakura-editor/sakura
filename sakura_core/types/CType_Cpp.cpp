	...
	case L'(':
		...
		for( j = GetCaret().GetCaretLogicPos().GetY2(); j >= CLogicInt(0); --j ){
			...
			if( bCommentStringCheck ){
				...
				for( int n = 0; n < nLineLen2; n++ ){
					...
					case 0:
						if( m_pTypeData->m_bIndentCppStringIgnore && L'"' == pLine2[n] ){
							...
							arrCommentBlock.emplace_back(SCommentBlock{ n, nLineLen2 - 1 });
							n = nLineLen2 - 1;
						}
						break;
					case 1:
						if( L'\\' == pLine2[n] ){
							...
							arrCommentBlock.emplace_back(SCommentBlock{ nBegin, n });
							nBegin = -1;
							nMode = 0;
						}
						break;
					...
					case 3:
						if( n + 1 < nLineLen2 && '*' == pLine2[n] && '/' == pLine2[n+1] ){
							arrCommentBlock.emplace_back(SCommentBlock{ nBegin, n + 1 });
							nBegin = -1;
							nMode = 0;
							n++;
						}
				...
				if( 0 < nBegin ){
					// 終わりのない文字列orコメント
					arrCommentBlock.emplace_back(SCommentBlock{ nBegin, nLineLen2 });
				}
