...
{
    CSplitBoxWnd::Draw3dRect(hdc, prc->left, prc->top, prc->right, prc->bottom, ::GetSysColor(COLOR_3DSHADOW), ::GetSysColor(COLOR_3DHILIGHT));
    CSplitBoxWnd::Draw3dRect(hdc, prc->left + 1, prc->top + 1, prc->right - 2, prc->bottom - 2, RGB(0, 0, 0), ::GetSysColor(COLOR_3DFACE));
    return;
}
...
void CSplitterWnd::DoSplit(int nHorizontal, int nVertical)
{
	...
	*/
    if (NULL == pCEditWnd || (NULL != pCEditWnd->m_cFuncKeyWnd.GetHwnd() && 1 == m_pShareData->m_Common.m_sWindow.m_nFUNCKEYWND_Place /* ファンクションキー表示位置／0:上 1:下 */
                              ))
    {
        bSizeBox = FALSE;
    }
    else if (NULL != pCEditWnd->m_cTabWnd.GetHwnd() && m_pShareData->m_Common.m_sTabBar.m_eTabPosition == TabPosition_Bottom)
    {
        bSizeBox = FALSE;
    }
    else
    ...
    if (nVertical == 0 && nHorizontal == 0)
    {
        ...
        if (m_ChildWndArr[0] != NULL)
            ::ShowWindow(m_ChildWndArr[0], SW_SHOW);
        if (m_ChildWndArr[1] != NULL)
            ::ShowWindow(m_ChildWndArr[1], SW_HIDE);
        if (m_ChildWndArr[2] != NULL)
            ::ShowWindow(m_ChildWndArr[2], SW_HIDE);
        if (m_ChildWndArr[3] != NULL)
            ::ShowWindow(m_ChildWndArr[3], SW_HIDE);
    ...
    else if (nVertical > 0 && nHorizontal == 0)
    {
        ...
        if (m_ChildWndArr[1] != NULL)
            ::ShowWindow(m_ChildWndArr[1], SW_HIDE);
        if (m_ChildWndArr[2] != NULL)
            ::ShowWindow(m_ChildWndArr[2], SW_SHOW);
        if (m_ChildWndArr[3] != NULL)
            ::ShowWindow(m_ChildWndArr[3], SW_HIDE);
        if (NULL != pcViewArr[0])
    ...
    else if (nVertical == 0 && nHorizontal > 0)
    {
        ...
        if (m_ChildWndArr[0] != NULL)
            ::ShowWindow(m_ChildWndArr[0], SW_SHOW);
        if (m_ChildWndArr[1] != NULL)
            ::ShowWindow(m_ChildWndArr[1], SW_SHOW);
        if (m_ChildWndArr[2] != NULL)
            ::ShowWindow(m_ChildWndArr[2], SW_HIDE);
        if (m_ChildWndArr[3] != NULL)
            ::ShowWindow(m_ChildWndArr[3], SW_HIDE);
    ...
    else
    {
        ...
        if (m_ChildWndArr[3] != NULL)
        {
            ::ShowWindow(m_ChildWndArr[3], SW_SHOW);
        }
        if (NULL != pcViewArr[0])
        {
