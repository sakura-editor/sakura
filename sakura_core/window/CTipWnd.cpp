    ...
    for (size_t i = 0, nLineBgn = 0; i <= cchText;)
    {
        ...
        if (isEndOfText || (i + 1 < cchText && pszText[i] == '\\' && pszText[i + 1] == 'n'))
        {
            ...
            if (0 < i - nLineBgn)
            {
                ...
                // テキスト描画に必要な矩形を計測する
                ::DrawText(hdc, &pszText[nLineBgn], i - nLineBgn, &rc, DT_CALCRECT | DT_WORDBREAK | DT_EXPANDTABS | DT_EXTERNALLEADING);
    ...
    const RECT &rcPaint)
{
    ...
    for (size_t i = 0, nLineBgn = 0; i <= cchText;)
    {
        ...
        if (isEndOfText || (i + 1 < cchText && pszText[i] == '\\' && pszText[i + 1] == 'n'))
        {
            ...
            {
                // 指定されたテキストを描画する
                nHeight = ::DrawText(hdc, &pszText[nLineBgn], i - nLineBgn, &rc, DT_WORDBREAK | DT_EXPANDTABS | DT_EXTERNALLEADING);
            }
            else
            {
                // ダミー文字列の高さを取得する
