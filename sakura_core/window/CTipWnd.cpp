...
/* ウィンドウのサイズを決める */
void CTipWnd::ComputeWindowSize(const HDC hdc, RECT *prcResult)
{
	assert(m_hFont != NULL);
	assert(hdc != NULL);
	...
	for (size_t i = 0, nLineBgn = 0; i <= cchText;) {
		...
		// iの位置にNUL終端、または"\n"がある場合
		if (isEndOfText || (i + 1 < cchText && pszText[i] == '\\' && pszText[i + 1] == 'n')) {
			// 計測結果を格納する矩形
			CMyRect rc;
			// 計測対象の文字列がブランクでない場合
			if (0 < i - nLineBgn) {
...
/* ウィンドウのテキストを表示 */
void CTipWnd::DrawTipText(const HDC hdc, const RECT &rcPaint)
{
	assert(m_hFont != NULL);
	assert(hdc != NULL);
