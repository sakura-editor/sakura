...
void Use()
{
	if (++g_nCnt >= 1 && g_hHook == NULL) { g_hHook = ::SetWindowsHookEx(WH_CBT, CBTProc, NULL, GetCurrentThreadId()); }
}
...
/* デフォルトメッセージ処理 */
LRESULT CWnd::CallDefWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) { return ::DefWindowProc(hwnd, msg, wp, lp); }
