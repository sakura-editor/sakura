...
public:
	...
	*/
	...
	// ウィンドウクラス登録
	ATOM RegisterWC(HINSTANCE hInstance,
					...
					LPCWSTR   lpszMenuName, // Pointer to a null-terminated character string that specifies the resource
...
protected:
	/* 仮想関数 */
	virtual LRESULT DispatchEvent_WM_APP(HWND hwnd, UINT msg, WPARAM wp,
										 LPARAM lp); /* アプリケーション定義のメッセージ(WM_APP <= msg <= 0xBFFF) */
