...
void CEditWnd::MessageLoop(void)
{
	...
	while (GetHwnd())
	{
		...
		//ダイアログメッセージ
		if (MyIsDialogMessage(CPrintPreview::GetPrintPreviewBarHANDLE_Safe(m_pPrintPreview), &msg)) {
		} //!< 印刷プレビュー 操作バー
...
void CEditWnd::InitMenu_Function(HMENU hMenu, EFunctionCode eFunc, const wchar_t *pszName, const wchar_t *pszKey)
{
	...
	else if (eFunc >= F_PLUGCOMMAND_FIRST && eFunc < F_PLUGCOMMAND_LAST)
	{
		...
		{ m_cMenuDrawer.MyAppendMenu(hMenu, MF_BYPOSITION | MF_STRING, eFunc, szLabel, pszKey, TRUE, eFunc); }
		else
		{
			// not found
...
void CEditWnd::RestorePhysPosOfAllView(CLogicPointEx *pptPosArray)
{
	...
	for (int i = 0; i < NUM_OF_VIEW; ++i)
	{
		...
		if (nLeft < GetView(i).GetTextArea().GetViewLeftCol()) { GetView(i).GetTextArea().SetViewLeftCol(nLeft); }
