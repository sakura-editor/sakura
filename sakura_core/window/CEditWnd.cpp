    ...
    if (pLine)
    {
        ...
        if (nIdx < nLineLen)
        {
            if (nIdx < nLineLen - (pcLayout->GetLayoutEol().GetLen() ? 1 : 0))
            {
                ...
                // メッセージボックス表示
                auto_sprintf(szMsg, LS(STR_ERR_DLGEDITWND13), szChar, szCodeCP, szCode[CODE_SJIS], szCode[CODE_JIS], szCode[CODE_EUC], szCode[CODE_LATIN1], szCode[CODE_UNICODE], szCode[CODE_UTF8], szCode[CODE_CESU8]);
...
void CEditWnd::_AdjustInMonitor(const STabGroupInfo &sTabGroupInfo)
{
    ...
    if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd && !m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin && sTabGroupInfo.hwndTop)
    {
    ...
    {
        ::SetWindowPos(
            GetHwnd(), 0, rcOrg.left, rcOrg.top, rcOrg.right - rcOrg.left, rcOrg.bottom - rcOrg.top, SWP_NOOWNERZORDER | SWP_NOZORDER);
...
)
{
    ...
    m_pcEditDoc->m_cLayoutMgr.SetLayoutInfo(true, false, m_pcEditDoc->m_cDocType.GetDocumentAttribute(), m_pcEditDoc->m_cLayoutMgr.GetTabSpaceKetas(), m_pcEditDoc->m_cLayoutMgr.m_tsvInfo.m_nTsvMode, m_pcEditDoc->m_cLayoutMgr.GetMaxLineKetas(), CLayoutXInt(-1), &GetLogfont());
...
void CEditWnd::LayoutMainMenu()
{
    ...
    {
        nCount    = (i >= MAX_MAINMENU_TOP || pcMenu->m_nMenuTopIdx[i + 1] < 0 ? pcMenu->m_nMainMenuNum : pcMenu->m_nMenuTopIdx[i + 1]) - pcMenu->m_nMenuTopIdx[i]; // メニュー項目数
        ...
        switch (cMainMenu->m_nType)
        {
            case T_NODE:
                ...
                pszName = (cMainMenu->m_sName[0] == L'\0' && cMainMenu->m_nFunc != F_NODE)
                              ? LS(cMainMenu->m_nFunc)
                              : cMainMenu->m_sName;
                ::AppendMenu(hMenu, MF_POPUP | MF_STRING | (nCount <= 1 ? MF_GRAYED : 0), (UINT_PTR)CreatePopupMenu(), CKeyBind::MakeMenuLabel(pszName, cMainMenu->m_sKey));
                break;
            case T_LEAF:
                /* メニューラベルの作成 */
            ...
            case T_SPECIAL:
                ...
                ::AppendMenu(hMenu, MF_POPUP | MF_STRING | (nCount <= 0 ? MF_GRAYED : 0), (UINT_PTR)CreatePopupMenu(), CKeyBind::MakeMenuLabel(LS(cMainMenu->m_nFunc), cMainMenu->m_sKey));
                break;
        }
    }
        ...
        case WM_NOTIFY:
            ...
            if (m_cStatusBar.GetStatusHwnd() && pnmh->hwndFrom == m_cStatusBar.GetStatusHwnd())
            {
                ...
                else if (pnmh->code == NM_RCLICK)
                {
                    ...
                    { //	入力改行モード
                        ...
                        m_cMenuDrawer.MyAppendMenu(hMenuPopUp, MF_BYPOSITION | MF_STRING, F_CHGMOD_EOL_CRLF, LS(F_CHGMOD_EOL_CRLF), L"C"); // 入力改行コード指定(CRLF)
                        m_cMenuDrawer.MyAppendMenu(hMenuPopUp, MF_BYPOSITION | MF_STRING, F_CHGMOD_EOL_LF, LS(F_CHGMOD_EOL_LF), L"L"); // 入力改行コード指定(LF)
                        m_cMenuDrawer.MyAppendMenu(hMenuPopUp, MF_BYPOSITION | MF_STRING, F_CHGMOD_EOL_CR, LS(F_CHGMOD_EOL_CR), L"R"); // 入力改行コード指定(CR)
                        // 拡張EOLが有効の時だけ表示
                        if (GetDllShareData().m_Common.m_sEdit.m_bEnableExtEol)
                        {
                            m_cMenuDrawer.MyAppendMenu(hMenuPopUp, MF_BYPOSITION | MF_STRING, F_CHGMOD_EOL_NEL, LS(STR_EDITWND_MENU_NEL), L"", TRUE, -2); // 入力改行コード指定(NEL)
                            m_cMenuDrawer.MyAppendMenu(hMenuPopUp, MF_BYPOSITION | MF_STRING, F_CHGMOD_EOL_LS, LS(STR_EDITWND_MENU_LS), L"", TRUE, -2); // 入力改行コード指定(LS)
                            m_cMenuDrawer.MyAppendMenu(hMenuPopUp, MF_BYPOSITION | MF_STRING, F_CHGMOD_EOL_PS, LS(STR_EDITWND_MENU_PS), L"", TRUE, -2); // 入力改行コード指定(PS)
                        }
                        ...
                        switch (nId)
                        {
                            ...
                            case F_CHGMOD_EOL_LF:
                                nEOLCode = EOL_LF;
                                break;
                            case F_CHGMOD_EOL_NEL:
                                nEOLCode = EOL_NEL;
                                break;
                            case F_CHGMOD_EOL_PS:
                                nEOLCode = EOL_PS;
                                break;
                            case F_CHGMOD_EOL_LS:
                                nEOLCode = EOL_LS;
                                break;
                            default:
                                nEOLCode = -1;
            ...
            switch (pnmh->code)
            {
                ...
                case TBN_DROPDOWN:
                {
                    ...
                    if (nId != 0)
                        OnCommand((WORD)0 /*メニュー*/, (WORD)nId, (HWND)0);
                }
                    return FALSE;
        ...
        case MYWM_CHANGESETTING:
            ...
            switch ((e_PM_CHANGESETTING_SELECT)lParam)
            {
                    ...
                    if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd && !m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin)
                    {
                        if (CAppNodeManager::getInstance()->GetEditNode(GetHwnd())->IsTopInGroup())
                        {
                            if (!::IsWindowVisible(GetHwnd()))
                            {
                                ...
                                ::SetWindowPos(GetHwnd(), NULL, 0, 0, 0, 0, SWP_SHOWWINDOW | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER);
                ...
                case PM_CHANGESETTING_FONTSIZE:
                    if ((-1 == wParam && CWM_CACHE_SHARE == GetLogfontCacheMode()) || GetDocument()->m_cDocType.GetDocumentType().GetIndex() == wParam)
                    {
                        // 文字幅で幅も変わるので再構築する
                ...
                switch (wParam)
                {
                    ...
                    case MYBCN_TAB:
                        ...
                        else
                        {
                            ...
                            ::SetWindowPos(hwnd, NULL, 0, 0, 0, 0, SWP_SHOWWINDOW | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER);
                        }
                        break;
        ...
        for (i = nIdxStr + 1; i < nIdxEnd; i++)
        {
            ...
            switch (cMainMenu->m_nType)
            {
                case T_NODE:
                    ...
                    m_cMenuDrawer.MyAppendMenu(hMenu, MF_BYPOSITION | MF_STRING | MF_POPUP, (UINT_PTR)hMenuPopUp, pMenuName, cMainMenu->m_sKey);
                    if (hSubMenu.size() > (size_t)nLv)
                    {
                ...
                case T_SPECIAL:
                    ...
                    if (!bInList)
                    {
                        ...
                        if ((i == nIdxStr + 1 || (pcMenu->m_cMainMenuTbl[i - 1].m_nType == T_SEPARATOR && pcMenu->m_cMainMenuTbl[i - 1].m_nLevel == cMainMenu->m_nLevel)) && i + 1 < nIdxEnd && pcMenu->m_cMainMenuTbl[i + 1].m_nType == T_SEPARATOR && pcMenu->m_cMainMenuTbl[i + 1].m_nLevel == cMainMenu->m_nLevel)
                        {
                            i++; // スキップ
                        }
                    }
...
void CEditWnd::InitMenu_Function(HMENU hMenu, EFunctionCode eFunc, const wchar_t *pszName, const wchar_t *pszKey)
{
    ...
    if (eFunc == F_MENU_RBUTTON || eFunc >= F_CUSTMENU_1 && eFunc <= F_CUSTMENU_24)
    {
        ...
        m_cMenuDrawer.MyAppendMenu(hMenu, nFlag, eFunc, GetDocument()->m_cFuncLookup.Custmenu2Name(j, buf, _countof(buf)), pszKey);
    }
    // マクロ
    ...
    else
    {
        switch (eFunc)
        {
            ...
            case F_LOADKEYMACRO:
            case F_EXECKEYMACRO:
                SetMenuFuncSel(hMenu, eFunc, pszKey, !m_pShareData->m_sFlags.m_bRecordingKeyMacro);
                break;
            case F_SPLIT_V:
                SetMenuFuncSel(hMenu, eFunc, pszKey, m_cSplitterWnd.GetAllSplitRows() == 1);
                break;
            case F_SPLIT_H:
                SetMenuFuncSel(hMenu, eFunc, pszKey, m_cSplitterWnd.GetAllSplitCols() == 1);
                break;
            case F_SPLIT_VH:
                SetMenuFuncSel(hMenu, eFunc, pszKey, m_cSplitterWnd.GetAllSplitRows() == 1 || m_cSplitterWnd.GetAllSplitCols() == 1);
                break;
            case F_TAB_CLOSEOTHER:
                SetMenuFuncSel(hMenu, eFunc, pszKey, m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd != 0);
                break;
            case F_TOPMOST:
                SetMenuFuncSel(hMenu, eFunc, pszKey, ((DWORD)::GetWindowLongPtr(GetHwnd(), GWL_EXSTYLE) & WS_EX_TOPMOST) == 0);
                break;
            case F_BIND_WINDOW:
                SetMenuFuncSel(hMenu, eFunc, pszKey, (!m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd || m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin));
                break;
            case F_SHOWTOOLBAR:
                SetMenuFuncSel(hMenu, eFunc, pszKey, !m_pShareData->m_Common.m_sWindow.m_bMenuIcon | !m_cToolbar.GetToolbarHwnd());
                break;
            case F_SHOWFUNCKEY:
                SetMenuFuncSel(hMenu, eFunc, pszKey, !m_pShareData->m_Common.m_sWindow.m_bMenuIcon | !m_cFuncKeyWnd.GetHwnd());
                break;
            case F_SHOWTAB:
                SetMenuFuncSel(hMenu, eFunc, pszKey, !m_pShareData->m_Common.m_sWindow.m_bMenuIcon | !m_cTabWnd.GetHwnd());
                break;
            case F_SHOWSTATUSBAR:
                SetMenuFuncSel(hMenu, eFunc, pszKey, !m_pShareData->m_Common.m_sWindow.m_bMenuIcon | !m_cStatusBar.GetStatusHwnd());
                break;
            case F_SHOWMINIMAP:
                SetMenuFuncSel(hMenu, eFunc, pszKey, !m_pShareData->m_Common.m_sWindow.m_bMenuIcon | !GetMiniMap().GetHwnd());
                break;
            case F_TOGGLE_KEY_SEARCH:
                SetMenuFuncSel(hMenu, eFunc, pszKey, !m_pShareData->m_Common.m_sWindow.m_bMenuIcon | !IsFuncChecked(GetDocument(), m_pShareData, F_TOGGLE_KEY_SEARCH));
                break;
            ...
            break;
            default:
                m_cMenuDrawer.MyAppendMenu(hMenu, MF_BYPOSITION | MF_STRING, eFunc, pszName, pszKey);
                break;
        }
    }
}
...
bool CEditWnd::InitMenu_Special(HMENU hMenu, EFunctionCode eFunc)
{
    ...
    switch (eFunc)
    {
        ...
        case F_CUSTMENU_LIST: // カスタムメニューリスト
            ...
            if (m_pShareData->m_Common.m_sCustomMenu.m_nCustMenuItemNumArr[0] > 0)
            {
                m_cMenuDrawer.MyAppendMenu(hMenu, MF_BYPOSITION | MF_STRING, F_MENU_RBUTTON, GetDocument()->m_cFuncLookup.Custmenu2Name(0, buf, _countof(buf)), L"");
                bInList = true;
            }
            //	カスタムメニュー
            for (j = 1; j < MAX_CUSTOM_MENU; ++j)
            {
                if (m_pShareData->m_Common.m_sCustomMenu.m_nCustMenuItemNumArr[j] > 0)
                {
                    m_cMenuDrawer.MyAppendMenu(hMenu, MF_BYPOSITION | MF_STRING, F_CUSTMENU_BASE + j, GetDocument()->m_cFuncLookup.Custmenu2Name(j, buf, _countof(buf)), L"");
                    bInList = true;
                }
            }
            break;
        ...
        case F_PLUGIN_LIST: // プラグインコマンドリスト
            //プラグインコマンドを提供するプラグインを列挙する
            {
                ...
                for (CPlug::ArrayIter it = plugs.begin(); it != plugs.end(); it++)
                {
                    ...
                    //コマンドを登録
                    m_cMenuDrawer.MyAppendMenu(hMenuPlugin, MF_BYPOSITION | MF_STRING, (*it)->GetFunctionCode(), (*it)->m_sLabel.c_str(), L"", TRUE, (*it)->GetFunctionCode());
                }
...
LRESULT CEditWnd::OnSize2(WPARAM wParam, LPARAM lParam, bool bUpdateStatus)
{
    ...
    if (m_cTabWnd.GetHwnd())
    {
        ...
        else if (tabPosition == TabPosition_Bottom)
        {
            ...
            ::GetWindowRect(m_cTabWnd.GetHwnd(), &rc);
            nTabWndHeight = rc.bottom - rc.top;
            ::MoveWindow(m_cTabWnd.GetHwnd(), 0, cy - nFuncKeyWndHeight - nStatusBarHeight - nTabWndHeight, cx, nTabWndHeight, TRUE);
            m_cTabWnd.OnSize();
            ::GetWindowRect(m_cTabWnd.GetHwnd(), &rc);
            if (nTabWndHeight != rc.bottom - rc.top)
            {
...
void CEditWnd::ChangeFileNameNotify(const WCHAR *pszTabCaption, const WCHAR *_pszFilePath, bool bIsGrep)
{
    ...
    if (NULL == pszTabCaption)
        pszTabCaption = L""; //ガード
    if (NULL == pszFilePath)
        pszFilePath = _FT(""); //ガード 2006.01.28 ryoji
...
LRESULT CEditWnd::PopupWinList(bool bMousePos)
{
    ...
    else
    {
        ...
        int nId = ::TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON | TPM_RETURNCMD, (pt.x > rcWork.left) ? pt.x : rcWork.left, (pt.y < rcWork.bottom) ? pt.y : rcWork.bottom, 0, GetHwnd(), NULL);
        delete[] pEditNodeArr;
        ::DestroyMenu(hMenu);
