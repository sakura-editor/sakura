                ...
                case TBSTYLE_COMBOBOX: //コンボボックス
                {
                    ...
                    switch (tbb.idCommand)
                    {
                        case F_SEARCH_BOX:
                            ...
                            //コンボボックスを作る
                            m_hwndSearchBox = CreateWindow(L"COMBOBOX", L"Combo", WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_VSCROLL | CBS_DROPDOWN
                                                                                      /*| CBS_SORT*/
...
void CMainToolBar::AcceptSharedSearchKey()
{
    if (m_hwndSearchBox)
    {
        ...
        if (GetDllShareData().m_Common.m_sSearch.m_bInheritKeyOtherView && m_pOwner->GetActiveView().m_nCurSearchKeySequence < GetDllShareData().m_Common.m_sSearch.m_nSearchKeySequence || 0 == m_pOwner->GetActiveView().m_strCurSearchKey.size())
        {
            if (0 < nSize)
            {
