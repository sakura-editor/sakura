...
/* 描画処理 */
void CSplitBoxWnd::Draw3dRect(HDC hdc, int x, int y, int cx, int cy, COLORREF clrTopLeft, COLORREF clrBottomRight)
{
	RECT rc;
...
LRESULT CSplitBoxWnd::OnPaint(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (m_bVertical) {
		...
		Draw3dRect(hdc, 1, 1, nHSplitWidth - 2, nCyHScroll - 2, ::GetSysColor(COLOR_3DHILIGHT),
				   ::GetSysColor(COLOR_3DSHADOW));
	}
...
LRESULT CSplitBoxWnd::OnMouseMove(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (m_bVertical) {
		...
		if (po.y < po_top.y) { po.y = po_top.y; }

		po_top.x = 0;
		po_top.y = rc.bottom;
		::ClientToScreen(::GetParent(GetParentHwnd()), &po_top);
		if (po.y > po_top.y - 6) { po.y = po_top.y - 6; }
		...
		po_top.y = 0;
		::ClientToScreen(::GetParent(GetParentHwnd()), &po_top);
		if (po.x < po_top.x) { po.x = po_top.x; }

		po_top.x = rc.right;
		po_top.y = 0;
		::ClientToScreen(::GetParent(GetParentHwnd()), &po_top);
		if (po.x > po_top.x - 6) { po.x = po_top.x - 6; }
