...
void CMainStatusBar::CreateStatusBar()
{
	...
	/* ステータスバー */
	m_hwndStatusBar = ::CreateStatusWindow(WS_CHILD /* | WS_VISIBLE*/ | WS_EX_RIGHT
											   | SBARS_SIZEGRIP, // 2007.03.08 ryoji WS_VISIBLE 除去
										   L"", m_pOwner->GetHwnd(), IDW_STATUSBAR);

	/* プログレスバー */
	m_hwndProgressBar = ::CreateWindowEx(WS_EX_TOOLWINDOW, PROGRESS_CLASS, NULL, WS_CHILD /*|  WS_VISIBLE*/, 3, 5, 150,
										 13, m_hwndStatusBar, NULL, CEditApp::getInstance()->GetAppInstance(), 0);

	if (NULL != m_pOwner->m_cFuncKeyWnd.GetHwnd()) { m_pOwner->m_cFuncKeyWnd.SizeBox_ONOFF(FALSE); }
...
{
	if (NULL != m_hwndStatusBar) { SetStatusText(0, SBT_NOBORDERS, msg); }
}
...
void CMainStatusBar::SetStatusText(int nIndex, int nOption, const WCHAR *pszText, size_t textLen /* = SIZE_MAX */)
{
	...
	[&]() -> bool {
		...
		// 判定が難しいので諦める
		if (nOption == SBT_OWNERDRAW) { return true; }
		...
		if (HIWORD(res) != nOption) { return true; }
		size_t prevTextLen = LOWORD(res);
		WCHAR  prev[1024];
		// 設定済みの文字列長が長過ぎて取得できない場合は、SB_SETTEXT メッセージを発行
		if (prevTextLen >= _countof(prev)) { return true; }
