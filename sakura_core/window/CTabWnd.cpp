...
static int compTABMENU_DATA(const void *arg1, const void *arg2)
{
	...
	if (0 == ret) ret = ((TABMENU_DATA *)arg1)->iItem - ((TABMENU_DATA *)arg2)->iItem;
	return ret;
}
...
LRESULT CALLBACK TabWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		// return
		if (0L == pcTabWnd->TabWndDispatchEvent(hwnd, uMsg, wParam, lParam)) return 0L;
	}
...
LRESULT CTabWnd::TabWndDispatchEvent(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
	case WM_LBUTTONDOWN: return OnTabLButtonDown(wParam, lParam);

	case WM_LBUTTONUP: return OnTabLButtonUp(wParam, lParam);

	case WM_MOUSEMOVE: return OnTabMouseMove(wParam, lParam);

	case WM_TIMER: return OnTabTimer(wParam, lParam);

	case WM_CAPTURECHANGED: return OnTabCaptureChanged(wParam, lParam);

	case WM_RBUTTONDOWN: return OnTabRButtonDown(wParam, lParam);

	case WM_RBUTTONUP: return OnTabRButtonUp(wParam, lParam);

	case WM_MBUTTONDOWN: return OnTabMButtonDown(wParam, lParam);

	case WM_MBUTTONUP: return OnTabMButtonUp(wParam, lParam);

	case WM_NOTIFY: return OnTabNotify(wParam, lParam);

	case WM_HSCROLL:
		::InvalidateRect(GetHwnd(), NULL,
						 TRUE); // アクティブタブの位置が変わるのでトップバンドを更新する	// 2006.03.27 ryoji
		break;
...
LRESULT CTabWnd::OnTabLButtonUp(WPARAM wParam, LPARAM lParam)
{
	...
	// タブの閉じるボタン押下処理
	if (m_nTabCloseCapture >= 0)
	{ // タブ内の閉じるボタンが押し下げられていた?
		...
		if (::PtInRect(&rcClose, hitinfo.pt)) { ExecTabCommand(F_WINCLOSE, MAKEPOINTS(lParam)); }
		// キャプチャー解除
		BreakDrag();
		return 0L;
	}
...
LRESULT CTabWnd::OnTabMouseMove(WPARAM wParam, LPARAM lParam)
{
	...
	if (nDstTab == -1 && m_nTabBorderArray)
	{
		if (hitinfo.pt.x >= m_nTabBorderArray[nTabCount - 1])
		{
			...
			if (hitinfo.pt.y >= rc.top && hitinfo.pt.y < rc.bottom) { nDstTab = nTabCount - 1; }
		}
	}
	...
	if (bDispTabClose && ::GetCapture() != m_hwndTab)
	{
		...
		{ // カーソルがどれかタブ内にある
			...
			{ // 閉じるボタン上
				if (!m_bTabCloseHover || nTabHoverCur != nTabHoverPrev)
				{ // 以前はマウス下の閉じるボタンをハイライトしていなかった
	...
	case DRAG_CHECK:
		...
		if (m_nTabBorderArray) { delete[] m_nTabBorderArray; }
	...
	case DRAG_DRAG:
		...
		if (0 <= nDstTab)	  // タブの上にカーソルがある
		{
			...
			{
				if (hitinfo.pt.x < m_nTabBorderArray[nDstTab]) { break; }
			}
			if (nDstTab >= nTabCount) nDstTab = nTabCount - 1;
			...
			if (m_nSrcTab != nDstTab)
			{
				...
				if (rc.left > 0)
				{
					...
					// 今回の WM_MOUSEMOVE
					// が移動後のタブ上で発生したかのように偽装してマウスオーバーハイライトも移動する
					TabCtrl_GetItemRect(m_hwndTab, nDstTab, &rc);
					DefTabWndProc(m_hwndTab, WM_MOUSEMOVE, wParam,
								  MAKELPARAM((rc.left + rc.right) / 2, HIWORD(lParam)));
				}
			}
		}
		else
...
LRESULT CTabWnd::OnTabTimer(WPARAM wParam, LPARAM lParam)
{
	if (wParam == 1)
	{
		...
		int nDstTab = TabCtrl_HitTest(m_hwndTab, (LPARAM)&hitinfo);
		if (nDstTab < 0) ::SendMessageAny(m_hwndTab, WM_MOUSEMOVE, 0, MAKELONG(hitinfo.pt.x, hitinfo.pt.y));
	}
...
{
	if (m_eDragState != DRAG_NONE) m_eDragState = DRAG_NONE;
	return 0L;
}
...
BOOL CTabWnd::ReorderTab(int nSrcTab, int nDstTab)
{
	...
	if (0 > nSrcTab || 0 > nDstTab || nSrcTab == nDstTab) return FALSE;
	...
	if (!CAppNodeManager::getInstance()->ReorderTab(hwndSrc, hwndDst)) { return FALSE; }
	return TRUE;
}
...
void CTabWnd::BroadcastRefreshToGroup()
{
	...
	CAppNodeGroupHandle(nGroup).PostMessageToAllEditors(MYWM_TAB_WINDOW_NOTIFY, (WPARAM)TWNT_REFRESH, (LPARAM)FALSE,
														GetParentHwnd());
}
...
BOOL CTabWnd::SeparateGroup(HWND hwndSrc, HWND hwndDst, POINT ptDrag, POINT ptDrop)
{
	...
	if (pTopEditNode == NULL) return FALSE;
	if (hWnd != pTopEditNode->m_hWnd || hWnd != ::GetForegroundWindow()) return FALSE;
	if (hWnd != CAppNodeManager::getInstance()->GetEditNode(hwndSrc)->GetGroup().GetTopEditNode()->GetHwnd())
		return FALSE;
	if (hwndDst != NULL
		&& hwndDst != CAppNodeManager::getInstance()->GetEditNode(hwndDst)->GetGroup().GetTopEditNode()->GetHwnd())
		return FALSE;
	if (hwndSrc == hwndDst) return TRUE;
	...
	{ // 既存グループのウィンドウ処理
		...
		{ // 先頭ウィンドウの既存グループへの移動
			...
			HWND hWndInsertAfter =
				(::GetWindowLongPtr(hwndDst, GWL_EXSTYLE) & WS_EX_TOPMOST) ? HWND_TOPMOST : HWND_NOTOPMOST;
			::SetWindowPos(hwndSrc, hWndInsertAfter, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);

			// ウィンドウを移動先に表示する
			::GetWindowPlacement(hwndDst, &wp);
			if (wp.showCmd == SW_SHOWMINIMIZED) wp.showCmd = showCmdRestore;
			SetCarmWindowPlacement(hwndSrc, &wp);
	...
	{
		CAppNodeGroupHandle(notifygroups[group])
			.PostMessageToAllEditors(MYWM_TAB_WINDOW_NOTIFY, (WPARAM)TWNT_REFRESH, (LPARAM)bSrcIsTop, NULL);
	}
...
LRESULT CTabWnd::ExecTabCommand(int nId, POINTS pts)
{
	...
	tcitem.mask   = TCIF_PARAM;
	tcitem.lParam = 0;
	if (!TabCtrl_GetItem(m_hwndTab, nTab, &tcitem)) return 1L;
...
HWND CTabWnd::Open(HINSTANCE hInstance, HWND hwndParent)
{
	...
	RegisterWC(hInstance,
			   ...
			   NULL, // Pointer to a null-terminated character string that specifies the resource name of the class
	...
	m_hwndTab = ::CreateWindow(WC_TABCONTROL, L"",
							   ...
							   // 2006.01.30 ryoji 初期配置見直し
							   TAB_MARGIN_LEFT, TAB_MARGIN_TOP,
							   rcParent.right - rcParent.left - (TAB_MARGIN_LEFT + TAB_MARGIN_RIGHT), TAB_WINDOW_HEIGHT,
							   GetHwnd(), (HMENU)NULL, GetAppInstance(), (LPVOID)NULL);
	if (m_hwndTab)
	{
		// Modified by KEITA for WIN64 2003.9.6
		...
		if (m_pShareData->m_Common.m_sTabBar.m_bTabMultiLine) { lngStyle |= TCS_MULTILINE; }
		else
		{
...
LRESULT CTabWnd::OnSize(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (m_hwndSizeBox)
	{
		...
		::MoveWindow(m_hwndSizeBox, rcWnd.right - rcWnd.left - nSizeBoxWidth, rcWnd.bottom - rcWnd.top - nSizeBoxHeight,
					 nSizeBoxWidth, nSizeBoxHeight, TRUE);
	}
...
{
	if (m_eCaptureSrc != CAPT_NONE) m_eCaptureSrc = CAPT_NONE;
...
LRESULT CTabWnd::OnMeasureItem(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (lpmis->CtlType == ODT_MENU)
	{
		...
		if (NULL != m_hIml) { ImageList_GetIconSize(m_hIml, &cxIcon, &cyIcon); }
...
LRESULT CTabWnd::OnDrawItem(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	else if (lpdis->CtlType == ODT_TAB)
	{
		...
		// 背景描画
		if (!IsVisualStyle()) { ::MyFillRect(gr, rcItem, COLOR_BTNFACE); }
		else
		{
			...
			if (hTheme)
			{
				if (!bSelected)
				{
					...
					if (nTabIndex == nSelIndex - 1) { rcFullItem.right -= DpiScaleX(1); }
				...
				if (bSelected)
				{
					...
					if (nTabIndex == 0)
					{
						if (nTabIndex == nTabCount - 1) { iPartId = TABP_TOPTABITEMBOTHEDGE; }
						else
						{
				...
				else
				{
					...
					if (nTabIndex == 0)
					{
						if (nTabIndex == nTabCount - 1) { iPartId = TABP_TABITEMBOTHEDGE; }
						else
						{
		...
		if (NULL != m_hIml)
		{
			...
			if (0 <= item.iImage)
			{
				...
				ImageList_Draw(m_hIml, item.iImage, lpdis->hDC, rcItem.left, top + (bSelected ? 0 : DpiScaleY(3)),
							   ILD_TRANSPARENT);
		...
		// タブを閉じるボタンの矩形は他の箇所と同様 TabCtrl_GetItemRect の矩形から取得（lpdis->rcItem
		// の矩形だと若干ずれる）
		EDispTabClose bDispTabClose = m_pShareData->m_Common.m_sTabBar.m_bDispTabClose;
		bool		  bDrawTabCloseBtn =
			(bDispTabClose == DISPTABCLOSE_ALLWAYS || (bDispTabClose == DISPTABCLOSE_AUTO && nTabIndex == m_nTabHover));
		RECT rcGetItemRect;
	...
	if (bHovering != m_bCloseBtnHilighted)
	{
		...
		if (m_bCloseBtnHilighted) // ボタンに入ってきた?
		{
			...
			if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd
				&& !m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin)
			{
				if (!m_pShareData->m_Common.m_sTabBar.m_bTab_CloseOneWin) { wcscpy(szText, LS(STR_TABWND_CLOSETAB)); }
				else
				{
...
LRESULT CTabWnd::OnTimer(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (wParam == 1)
	{
		...
		::GetClientRect(hwnd, &rc);
		if (!::PtInRect(&rc, pt)) ::SendMessageAny(hwnd, WM_MOUSEMOVE, 0, MAKELONG(pt.x, pt.y));
	}
...
LRESULT CTabWnd::OnPaint(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (!m_bVisualStyle)
	{
		...
		if (nCurSel >= 0)
		{
			...
			if (rcCurSel.left < rc.left + TAB_MARGIN_LEFT) rcCurSel.left = rc.left + TAB_MARGIN_LEFT; // 左端限界値
			...
			if (hwndUpDown && ::IsWindowVisible(hwndUpDown))
			{
				...
				if (rcCurSel.right > ptREnd.x) rcCurSel.right = ptREnd.x; // 右端限界値
			}

			if (rcCurSel.left < rcCurSel.right) { ::MyFillRect(gr, rcCurSel, RGB(255, 128, 0)); }
		}
	}

	// サイズボックスを描画する
	if (!m_pShareData->m_Common.m_sWindow.m_bDispSTATUSBAR && !m_pShareData->m_Common.m_sWindow.m_bDispFUNCKEYWND
		&& m_pShareData->m_Common.m_sTabBar.m_eTabPosition == TabPosition_Bottom)
	{ SizeBox_ONOFF(true); } ::EndPaint(hwnd, &ps);
...
void CTabWnd::TabWindowNotify(WPARAM wParam, LPARAM lParam)
{
	...
	if (nCount <= 0)
	{
		...
		if (wParam == TWNT_ADD) Refresh(); // 続けてTWNT_ADD処理で自分以外のウィンドウを隠す
	}
	...
	case TWNT_DEL: //ウインドウ削除
		...
		if (-1 != nIndex)
		{
			...
			if (hwndUpDown != NULL && ::IsWindowVisible(hwndUpDown)) // 2007.09.24 ryoji hwndUpDown可視の条件追加
			{
				...
				::SendMessageAny(m_hwndTab, WM_HSCROLL, MAKEWPARAM(SB_THUMBPOSITION, LOWORD(nScrollPos)),
								 (LPARAM)NULL); // 設定位置にタブをスクロール
			}
		}
		break;
	...
	case TWNT_ORDER: //ウインドウ順序変更
		...
		if (-1 != nIndex)
		{
			...
			{
				//自分ならアクティブに
				if (!::IsWindowVisible(GetParentHwnd())) { ShowHideWindow(GetParentHwnd(), TRUE); }
				//ここに来たということはすでにアクティブ

				// 自タブアイテムを強制的に可視位置にするために、
				// 自タブアイテム選択前に一時的に画面左端のタブアイテムを選択する
				hwndUpDown = ::FindWindowEx(m_hwndTab, NULL, UPDOWN_CLASS, 0); // タブ内の Up-Down コントロール
				nScrollPos = (hwndUpDown != NULL && ::IsWindowVisible(hwndUpDown))
								 ? LOWORD(UpDown_GetPos(hwndUpDown))
								 : 0; // 2007.09.24 ryoji hwndUpDown可視の条件追加
		...
		{
			//指定のウインドウがないので再表示
			if (!CAppNodeManager::IsSameGroup(GetParentHwnd(), (HWND)lParam)) Refresh();
		}
		break;
	...
	case TWNT_FILE: //ファイル名変更
		...
		if (-1 != nIndex)
		{
			...
			if (0 != wcscmp(szNameOld, szName) || tcitem.iImage != GetImageIndex(p))
			{
	...
	//タブの表示・非表示を切りかえる。
	nCount = TabCtrl_GetItemCount(m_hwndTab);
	if (nCount <= 0) { ::ShowWindow(m_hwndTab, SW_HIDE); }
	else
	{
...
void CTabWnd::Refresh(BOOL bEnsureVisible /* = TRUE*/, BOOL bRebuild /* = FALSE*/)
{
	...
	else
	{
		...
		if (bRebuild) TabCtrl_DeleteAllItems(m_hwndTab); // 作成しなおす

		// 作成するタブ数と選択状態にするタブ位置（自ウィンドウの位置）を調べる
		for (i = 0, j = 0; i < nCount; i++)
		{
			if (pEditNode[i].m_nGroup != nGroup) continue;
			if (pEditNode[i].m_bClosing) // このあとすぐに閉じるウィンドウなのでタブ表示しない
				continue;
			if (pEditNode[i].m_hWnd == GetParentHwnd()) nSel = j; // 選択状態にするタブ位置
			j++;
		}
		nTab = j; // 作成するタブ数
		...
		if (nCurSel > nSel)
		{
			for (i = 0; i < nCurSel - nSel; i++) TabCtrl_DeleteItem(m_hwndTab, nCurSel - 1 - i); // 余分を削除
		}
		else
		{
			for (i = 0; i < nSel - nCurSel; i++) TabCtrl_InsertItem(m_hwndTab, nCurSel + i, &tcitem); // 不足を追加
		}
		...
		if (nCurTab > nTab)
		{
			for (i = 0; i < nCurTab - nTab; i++) TabCtrl_DeleteItem(m_hwndTab, nSel + 1); // 余分を削除
		}
		else
		{
			for (i = 0; i < nTab - nCurTab; i++) TabCtrl_InsertItem(m_hwndTab, nSel + 1, &tcitem); // 不足を追加
		}
...
void CTabWnd::AdjustWindowPlacement(void)
{
	...
	if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd && !m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin)
	{
		...
		if (!::IsWindowVisible(hwnd)) // 可視化するときだけ引き継ぐ
		{
			...
			if (wp.showCmd == SW_SHOWMINIMIZED) wp.showCmd = pEditNode->m_showCmdRestore;
...
int CTabWnd::SetCarmWindowPlacement(HWND hwnd, const WINDOWPLACEMENT *pWndpl)
{
	...
	if (wp.showCmd == SW_SHOWMAXIMIZED && ::IsZoomed(hwnd))
	{
		...
		if (!::EqualRect(&wp.rcNormalPosition, &wpCur.rcNormalPosition))
		{
			...
			// Note.
			// マルチモニタで以前に別モニタで最大化されていた画面は一旦通常サイズに戻しておかないと元の別モニタ側に表示されてしまう
...
void CTabWnd::ShowHideWindow(HWND hwnd, BOOL bDisp)
{
	...
	if (bDisp)
	{
		if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd && !m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin)
		{
			if (m_pShareData->m_sFlags.m_bEditWndChanging) return; // 切替の最中(busy)は要求を無視する
...
void CTabWnd::HideOtherWindows(HWND hwndExclude)
{
	if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd && !m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin)
	{
		...
		for (i = 0; i < m_pShareData->m_sNodes.m_nEditArrNum; i++)
		{
			...
			{
				if (!CAppNodeManager::IsSameGroup(hwndExclude, hwnd)) continue;
				if (hwnd != hwndExclude && ::IsWindowVisible(hwnd)) { ::ShowWindow(hwnd, SW_HIDE); }
			}
		}
	}
}
...
void CTabWnd::TabWnd_ActivateFrameWindow(HWND hwnd, bool bForeground)
{
	...
	{
		// 2005.09.01 ryoji ::ShowWindow( hwnd, SW_SHOWNA ) だと非表示から表示に切り替わるときに Z-order
		// がおかしくなることがあるので ::SetWindowPos に変更
...
void CTabWnd::LayoutTab(void)
{
	...
	int  nSizeBoxWidth = 0;
	if (m_hwndSizeBox) { nSizeBoxWidth = ::GetSystemMetrics(SM_CXVSCROLL); }
	if (bChgFont)
	{
	...
	{
		if (NULL != InitImageList()) Refresh(TRUE, TRUE);
	}
	...
	if (bSameTabWidth && !(lStyle & TCS_FIXEDWIDTH)) { lStyle |= (TCS_FIXEDWIDTH | TCS_FORCELABELLEFT); }
	...
	if (bOwnerDraw && !(lStyle & TCS_OWNERDRAWFIXED)) { lStyle |= TCS_OWNERDRAWFIXED; }
	...
	if (0 < nCount)
	{
		...
		if (m_pShareData->m_Common.m_sTabBar.m_bTabMultiLine) { min = MIN_TABITEM_WIDTH_MULTI; }
	...
	if (bDispTabClose == DISPTABCLOSE_ALLWAYS)
	{
		...
		cx += bDispTabIcon ? (nWidth + 2) / 3
						   : (nWidth + 1) / 2; // それっぽく調整: ボタン幅の 1/3（アイコン有） or 1/2（アイコン無）
	}
	...
	if (m_pShareData->m_Common.m_sTabBar.m_bTabMultiLine && TabCtrl_GetItemCount(m_hwndTab))
	{
...
HIMAGELIST CTabWnd::InitImageList(void)
{
	...
	if (m_pShareData->m_Common.m_sTabBar.m_bDispTabIcon)
	{
		...
		hImlSys = (HIMAGELIST)::SHGetFileInfo(L"C:\\", FILE_ATTRIBUTE_DIRECTORY, &sfi, sizeof(sfi),
											  SHGFI_SYSICONINDEX | SHGFI_SMALLICON | SHGFI_USEFILEATTRIBUTES);
		if (NULL == hImlSys) goto l_end;
		m_iIconApp = sfi.iIcon;

		hImlSys = (HIMAGELIST)::SHGetFileInfo(L".1", FILE_ATTRIBUTE_DIRECTORY, &sfi, sizeof(sfi),
											  SHGFI_SYSICONINDEX | SHGFI_SMALLICON | SHGFI_USEFILEATTRIBUTES
												  | SHGFI_OPENICON);
		if (NULL == hImlSys) goto l_end;
...
l_end:
	...
	// 新しいイメージリストを記憶する
	if (NULL != m_hIml) ImageList_Destroy(m_hIml);
	m_hIml = hImlNew;
...
int CTabWnd::GetImageIndex(EditNode *pNode)
{
	...
	if (NULL == m_hIml) return -1; // イメージリストを使っていない
	...
	if (pNode)
	{
		if (pNode->m_szFilePath[0])
		{
			...
			hImlSys = (HIMAGELIST)::SHGetFileInfo(szExt, FILE_ATTRIBUTE_NORMAL, &sfi, sizeof(sfi),
												  SHGFI_SYSICONINDEX | SHGFI_SMALLICON | SHGFI_USEFILEATTRIBUTES);
			if (NULL == hImlSys) return -1;
			if (ImageList_GetImageCount(m_hIml) > sfi.iIcon) return sfi.iIcon; // インデックスを返す

			// システムイメージリストを複製する
			hImlNew = ImageList_Duplicate(hImlSys);
			if (NULL == hImlNew) return -1;
			ImageList_SetBkColor(hImlNew, CLR_NONE);

			// イメージリストにアプリケーションアイコンと Grepアイコンを登録する
			// （利用しないアイコンと差し替える）
			ImageList_ReplaceIcon(hImlNew, m_iIconApp, m_hIconApp);
			if (m_iIconApp != m_iIconGrep) { ImageList_ReplaceIcon(hImlNew, m_iIconGrep, m_hIconGrep); }

			// タブにアイコンイメージを設定する
			if (m_pShareData->m_Common.m_sTabBar.m_bDispTabIcon) TabCtrl_SetImageList(m_hwndTab, hImlNew);
...
void CTabWnd::DrawCloseFigure(CGraphics &gr, const RECT &rcBtn)
{
	static const POINT ptBase1[6][2] = // [x]描画イメージ形状（直線6本）
		{{{4, 5}, {12, 13}}, {{4, 4}, {13, 13}}, {{5, 4}, {13, 12}},
		 {{11, 4}, {3, 12}}, {{12, 4}, {3, 13}}, {{12, 5}, {4, 13}}};
	POINT pt[2];
	int   i;
...
void CTabWnd::DrawCloseBtn(CGraphics &gr, const LPRECT lprcClient)
{
	static const POINT ptBase2[10][2] = // [xx]描画イメージ形状（矩形10個）
		{{{3, 4}, {5, 6}},  {{6, 4}, {8, 6}},   {{4, 6}, {7, 10}},   {{3, 10}, {5, 12}},  {{6, 10}, {8, 12}},
		 {{9, 4}, {11, 6}}, {{12, 4}, {14, 6}}, {{10, 6}, {13, 10}}, {{9, 10}, {11, 12}}, {{12, 10}, {14, 12}}};
	...
	if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd && !m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin
		&& !m_pShareData->m_Common.m_sTabBar
				.m_bTab_CloseOneWin // 2007.02.13 ryoji 条件追加（ウィンドウの閉じるボタンは全部閉じる）
	)
	{ DrawCloseFigure(gr, rcBtn); } else
	{
		// [xx]を描画（矩形10個）
...
void CTabWnd::GetListBtnRect(const LPRECT lprcClient, LPRECT lprc)
{
	*lprc = rcBtnBase;
	...
	int nSizeBoxWidth = 0;
	if (m_hwndSizeBox) { nSizeBoxWidth = ::GetSystemMetrics(SM_CXVSCROLL); }
	::OffsetRect(lprc, lprcClient->right - TAB_MARGIN_RIGHT - nSizeBoxWidth + DpiScaleX(4),
...
void CTabWnd::GetCloseBtnRect(const LPRECT lprcClient, LPRECT lprc)
{
	*lprc = rcBtnBase;
	...
	if (m_hwndSizeBox) { nSizeBoxWidth = ::GetSystemMetrics(SM_CXVSCROLL); }
	::OffsetRect(lprc,
				 lprcClient->right - TAB_MARGIN_RIGHT - nSizeBoxWidth + DpiScaleX(4)
					 + (DpiScaleX(rcBtnBase.right) - DpiScaleX(rcBtnBase.left)) + DpiScaleX(7),
				 lprcClient->top + TAB_MARGIN_TOP + DpiScaleY(2));
}
...
void CTabWnd::GetTabCloseBtnRect(const LPRECT lprcTab, LPRECT lprc, bool selected)
{
	*lprc = rcBtnBase;
	DpiScaleRect(lprc); // 2009.10.01 ryoji 高DPI対応スケーリング
	::OffsetRect(lprc,
				 (lprcTab->right + (selected ? 0 : -2))
					 - ((DpiScaleX(rcBtnBase.right) - DpiScaleX(rcBtnBase.left)) + DpiScaleX(2)),
...
void CTabWnd::GetTabName(EditNode *pEditNode, BOOL bFull, BOOL bDupamp, LPWSTR pszName, int nLen)
{
	...
	if (pEditNode == NULL) { ::lstrcpyn(pszText, LS(STR_NO_TITLE1), nLen); }
	else if (!bFull || pEditNode->m_szFilePath[0] == '\0')
	{
		if (pEditNode->m_szTabCaption[0]) { ::lstrcpyn(pszText, pEditNode->m_szTabCaption, nLen); }
		else
		{
...
LRESULT CTabWnd::TabListMenu(POINT pt, BOOL bSel /* = TRUE*/, BOOL bFull /* = FALSE*/, BOOL bOtherGroup /* = TRUE*/)
{
	...
	if (bSel) bFull = m_pShareData->m_Common.m_sTabBar.m_bTab_ListFull;
	...
	do
	{
		...
		for (i = 0; i < nSelfTab; i++)
		{
			...
			if (pData[i].hwnd == GetParentHwnd()) iMenuSel = i;
		}

		// 自ウィンドウに対応するメニューをチェック状態にする
		if (iMenuSel >= 0) { ::CheckMenuRadioItem(hMenu, 0, nSelfTab - 1, iMenuSel, MF_BYPOSITION); }
		...
		if (bSel)
		{
			::InsertMenu(hMenu, 0, MF_BYPOSITION | MF_STRING, 100,
						 bFull ? LS(STR_TABWND_SHOWTABNAME) : LS(STR_TABWND_SHOWPATHNAME));
	...
	if (bSel) m_pShareData->m_Common.m_sTabBar.m_bTab_ListFull = bFull;
...
HWND CTabWnd::GetNextGroupWnd(void)
{
	...
	if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd && !m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin)
	{
		...
		for (i = 0; i < n; i++)
		{
			if (pWndArr[i].m_hWnd == GetParentHwnd()) break;
		}
		if (i < n)
		{
			for (j = i + 1; j < n; j++)
			{
				...
				{
					hwndRet = CAppNodeManager::getInstance()
								  ->GetEditNode(pWndArr[j].m_hWnd)
								  ->GetGroup()
								  .GetTopEditNode()
								  ->GetHwnd();
					break;
...
void CTabWnd::NextGroup(void)
{
	...
	if (hWnd) { ActivateFrameWindow(hWnd); }
}
...
void CTabWnd::MoveRight(void)
{
	if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd)
	{
		...
		if (-1 != nIndex)
		{
			...
			if (nCount - 1 > nIndex)
			{
				if (ReorderTab(nIndex, nIndex + 1)) { BroadcastRefreshToGroup(); }
			}
		}
	}
}
