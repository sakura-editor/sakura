...
BOOL CTabWnd::SeparateGroup(HWND hwndSrc, HWND hwndDst, POINT ptDrag, POINT ptDrop)
{
    ...
    if (bSrcIsTop)
    {
        ...
        {
            DWORD_PTR dwResult;
            ::SendMessageTimeout(pNextEditNode->m_hWnd, MYWM_TAB_WINDOW_NOTIFY, TWNT_WNDPL_ADJUST, (LPARAM)NULL, SMTO_ABORTIFHUNG | SMTO_BLOCK, 10000, &dwResult);
        }
    }
    ...
    {
        CAppNodeGroupHandle(notifygroups[group]).PostMessageToAllEditors(MYWM_TAB_WINDOW_NOTIFY, (WPARAM)TWNT_REFRESH, (LPARAM)bSrcIsTop, NULL);
    }
...
{
    if (NULL == GetHwnd() || NULL == m_hwndTab)
        return 0L;
    ...
    if (m_hwndSizeBox)
    {
        ...
        ::MoveWindow(m_hwndSizeBox, rcWnd.right - rcWnd.left - nSizeBoxWidth, rcWnd.bottom - rcWnd.top - nSizeBoxHeight, nSizeBoxWidth, nSizeBoxHeight, TRUE);
    }
    ...
    else if (lpdis->CtlType == ODT_TAB)
    {
        ...
        if (NULL != m_hIml)
        {
            ...
            if (0 <= item.iImage)
            {
                ...
                ImageList_Draw(m_hIml, item.iImage, lpdis->hDC, rcItem.left, top + (bSelected ? 0 : DpiScaleY(3)), ILD_TRANSPARENT);
...
LRESULT CTabWnd::OnPaint(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    // サイズボックスを描画する
    if (!m_pShareData->m_Common.m_sWindow.m_bDispSTATUSBAR && !m_pShareData->m_Common.m_sWindow.m_bDispFUNCKEYWND && m_pShareData->m_Common.m_sTabBar.m_eTabPosition == TabPosition_Bottom)
    {
        SizeBox_ONOFF(true);
    }
...
void CTabWnd::ShowHideWindow(HWND hwnd, BOOL bDisp)
{
    ...
    if (bDisp)
    {
        if (m_pShareData->m_Common.m_sTabBar.m_bDispTabWnd && !m_pShareData->m_Common.m_sTabBar.m_bDispTabWndMultiWin)
        {
            ...
            DWORD_PTR dwResult;
            ::SendMessageTimeout(hwnd, MYWM_TAB_WINDOW_NOTIFY, TWNT_WNDPL_ADJUST, (LPARAM)NULL, SMTO_ABORTIFHUNG | SMTO_BLOCK, 10000, &dwResult);
        }
