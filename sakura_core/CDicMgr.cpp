...
*/
BOOL CDicMgr::Search(const wchar_t *pszKey,  //!< 検索キーワード
...
)
{
	...
	/* 辞書ファイル */
	if (pszKeyWordHelpFile[0] == L'\0') { return FALSE; }
...
||
*/
int CDicMgr::HokanSearch(const wchar_t *		  pszKey,
						 ...
						 const WCHAR *			  pszKeyWordFile)
{
	int nKeyLen;
	int nRet;
	if (pszKeyWordFile[0] == L'\0') { return 0; }
	...
	{
		szLine = in.ReadLineW();
		if (nKeyLen > (int)szLine.length()) { continue; }
		...
		if (0 == nRet)
		{
			...
			if (0 != nMaxKouho && nMaxKouho <= (int)vKouho.size()) { break; }
		}
	}
	in.Close();
