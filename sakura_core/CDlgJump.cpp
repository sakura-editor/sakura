...
BOOL CDlgJump::OnBnClicked( int wID )
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp( m_hWnd, m_pszHelpFile, HELP_CONTEXT, ::FuncID_To_HelpContextID(F_JUMP_DIALOG) );	// 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
...
//To Here Feb. 20, 2001
		{	//@@@ 2002.2.2 YAZAKI 指定行へジャンプを、ダイアログを表示するコマンドと、実際にジャンプするコマンドに分離。
			...
			pCEditDoc->m_pcEditViewArr[pCEditDoc->m_nActivePaneIndex]->HandleCommand(F_JUMP, true, 0, 0, 0, 0);	//	ジャンプコマンド発行
		}
		return TRUE;
	case IDCANCEL:
