...
HWND CHokanMgr::DoModeless(HINSTANCE hInstance, HWND hwndParent, LPARAM lParam)
{
	...
	// Modified by KEITA for WIN64 2003.9.6
	::gm_wpHokanListProc =
		(WNDPROC)::SetWindowLongPtr(GetItemHwnd(IDC_LIST_WORDS), GWLP_WNDPROC, (LONG_PTR)HokanList_SubclassProc);
...
*/
int CHokanMgr::Search(POINT *ppoWin, int nWinHeight, int nColumnWidth, const wchar_t *pszCurWord,
...
)
{
	...
	*/
	m_vKouho.clear();
	CDicMgr::HokanSearch(pszCurWord,
	...
	if (bHokanByKeyword) { HokanSearchByKeyword(pszCurWord, bHokanLoHiCase, m_vKouho); }

	{
		int nOption = ((bHokanLoHiCase ? 0x01 : 0) | (bHokanByFile ? 0x02 : 0));
		...
		if (nHokanType != 0) {
			...
			if (0 < plugType.size()) { plugs.push_back(plugType[0]); }
		}
	...
	List_ResetContent(hwndList);
	{
		...
		for (size_t i = 0; i < kouhoNum; ++i) { ::List_AddString(hwndList, m_vKouho[i].c_str()); }
	}
	...
	/* 下に入るなら */
	if (nY + nCY < rcDesktop.bottom) { /* 何もしない */ }
	else
		/* 上に入るなら */
...
void CHokanMgr::HokanSearchByKeyword(const wchar_t *pszCurWord, bool bHokanLoHiCase, vector_ex<std::wstring> &vKouho)
{
...
INT_PTR CHokanMgr::DispatchEvent(HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (wMsg >= WM_IME_STARTCOMPOSITION && wMsg <= WM_IME_KEYLAST
		|| wMsg >= WM_IME_SETCONTEXT && wMsg <= WM_IME_KEYUP) {
		...
		// ::SetCapture() して自前でサイズ変更する
		{
			...
			while (::GetCapture() == GetHwnd()) {
				...
				case WM_MOUSEMOVE:
					...
					{
						switch (wParam) {
						case HTTOP: rc.top += pt.y - ptStart.y; break;
						case HTBOTTOM: rc.bottom += pt.y - ptStart.y; break;
						case HTLEFT: rc.left += pt.x - ptStart.x; break;
						case HTRIGHT: rc.right += pt.x - ptStart.x; break;
						case HTTOPLEFT:
					...
					break;
				case WM_LBUTTONUP:
				case WM_RBUTTONDOWN: ::ReleaseCapture(); break;
				case WM_KEYDOWN:
...
BOOL CHokanMgr::OnSize(WPARAM wParam, LPARAM lParam)
{
	...
	for (i = 0; i < nControls; ++i) {
		...
		if (Controls[i] == IDC_LIST_WORDS) {
			::SetWindowPos(hwndCtrl, NULL, rc.left, rc.top, nWidth - rc.left * 2, nHeight - rc.top * 2 /* - 20*/,
...
BOOL CHokanMgr::DoHokan(int nVKey)
{
	...
	/* 補完候補決定キー */
	if (VK_RETURN == nVKey && !m_pShareData->m_Common.m_sHelper.m_bHokanKey_RETURN)
		return FALSE; /* VK_RETURN 補完決定キーが有効/無効 */
	if (VK_TAB == nVKey && !m_pShareData->m_Common.m_sHelper.m_bHokanKey_TAB)
		return FALSE; /* VK_TAB    補完決定キーが有効/無効 */
	if (VK_RIGHT == nVKey && !m_pShareData->m_Common.m_sHelper.m_bHokanKey_RIGHT)
		return FALSE; /* VK_RIGHT  補完決定キーが有効/無効 */
	...
	// Until here
	//	pcEditView->GetCommander().HandleCommand( F_INSTEXT_W, true, (LPARAM)(pszLabel + m_cmemCurWord.GetLength()),
	// TRUE, 0, 0 );
	Hide();
...
#endif
		/* 補完実行 */
		if (DoHokan(vkey)) { return -1; }
		else {
			return -2;
		}
	case VK_ESCAPE:
	case VK_LEFT: return -2;
	}
	return -2;
}
...
{
	INT		   nItem, nTopItem, nItemHeight;
	POINT	  point;
	CEditView *pcEditView;
	HWND	   hwndCtrl;
	RECT	   rcHokanWin;
...
const DWORD p_helpids[] = {0, 0};

LPVOID CHokanMgr::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end
