...
public:
	...
	bool Compile(const wchar_t *szPattern, int nOption = 0) { return Compile(szPattern, NULL, nOption); }
	...
	*/
	CLogicInt GetIndex(void) { return CLogicInt(m_pRegExp->startp[0] - m_szTarget); }
	/*!
		検索に一致した文字列の次の位置を返す
		@retval 検索に一致した文字列の次の位置
	*/
	CLogicInt GetLastIndex(void) { return CLogicInt(m_pRegExp->endp[0] - m_szTarget); }
	/*!
		検索に一致した文字列の長さを返す
		@retval 検索に一致した文字列の長さ
	*/
	CLogicInt GetMatchLen(void) { return CLogicInt(m_pRegExp->endp[0] - m_pRegExp->startp[0]); }
	/*!
		置換された文字列の長さを返す
		@retval 置換された文字列の長さ
	*/
	...
	*/
	const wchar_t *GetString(void) { return m_pRegExp->outp; }
	/*! @} */
	//-----------------------------------------
	...
	*/
	bool IsLookAhead(void) { return m_ePatType & PAT_LOOKAHEAD ? true : false; }
...
bool CheckRegexpSyntax(const wchar_t *szPattern, HWND hWnd, bool bShowMessage, int nOption = -1,
					   bool bKakomi = false); // 2002/2/1 hor追加
