...
{
	if (m_bInitialized) return IsAvailable();
...
/*!
	UxTheme のファイル名を渡す
*/
LPCWSTR CUxTheme::GetDllNameImp(int nIndex) { return L"UxTheme.dll"; }

bool CUxTheme::InitDllImp()
{
	const ImportTable table[] = {{&m_pfnIsThemeActive, "IsThemeActive"},
...
{
	if (!InitThemeDll()) return S_FALSE;
