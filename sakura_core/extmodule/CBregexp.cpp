...
)
{
	...
	*pPat++ = L'm'; // 複数行対応(但し、呼び出し側が複数行対応でない)
	...
	{					// 2002/2/1 hor IgnoreCase オプション追加 マージ：aroka
	...
	if ((nOption & optExtend)) { *pPat++ = L'x'; }
	if ((nOption & optASCII)) { *pPat++ = L'a'; }
	if ((nOption & optUnicode)) { *pPat++ = L'u'; }
	if ((nOption & optDefault)) { *pPat++ = L'd'; }
	if ((nOption & optLocale)) { *pPat++ = L'l'; }
	if ((nOption & optR)) { *pPat++ = L'R'; }
	...
	if ((m_ePatType & PAT_BOTTOM) != 0)
	{
		...
		if (matched >= 0)
		{
			...
			if (matched > 0)
			{
				...
				{
					if (BMatch(NULL, szCR, szCR + wcslen(szCR), &sReg, szMsg) > 0 && sReg->startp[0] == &szCR[1]
						&& sReg->endp[0] == &szCR[1])
					{
						if (BMatch(NULL, szLF, szLF + wcslen(szLF), &sReg, szMsg) > 0 && sReg->startp[0] == &szLF[0]
							&& sReg->endp[0] == &szLF[0])
						{
							// 検索文字列は 行末($)のみだった
							bJustDollar = true;
...
wchar_t *CBregexp::MakePatternAlternate(const wchar_t *const szSearch, const wchar_t *const szReplace, int nOption)
{
	...
	for (; *right; ++right)
	{ // CNativeW::GetSizeOfChar()は使わなくてもいいかな？
...
bool CBregexp::Match(const wchar_t *target, int len, int nStart)
{
	...
	if ((!IsAvailable()) || m_pRegExp == NULL) { return false; }
...
const WCHAR *CBregexp::GetLastMessage() const { return m_szMsg; }
...
)
{
	...
	{
		if (hWnd != NULL) { ::DlgItem_SetText(hWnd, nCmpId, L" "); }
		return false;
	}
	if (hWnd != NULL) { ::DlgItem_SetText(hWnd, nCmpId, cRegexp.GetVersionW()); }
	return true;
}
...
*/
bool CheckRegexpSyntax(const wchar_t *szPattern, HWND hWnd, bool bShowMessage, int nOption, bool bKakomi)
{
	CBregexp cRegexp;

	if (!InitRegexp(hWnd, cRegexp, bShowMessage)) { return false; }
	if (nOption == -1) { nOption = CBregexp::optCaseSensitive; }
	if (!cRegexp.Compile(szPattern, NULL, nOption, bKakomi))
	{ // 2002/2/1 hor追加
		if (bShowMessage)
		{ ::MessageBox(hWnd, cRegexp.GetLastMessage(), LS(STR_BREGONIG_TITLE), MB_OK | MB_ICONEXCLAMATION); }
		return false;
	}
	return true;
}
