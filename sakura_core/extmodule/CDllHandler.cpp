...
EDllResult CDllImp::InitDll(LPCWSTR pszSpecifiedDllName)
{
	...
	for (int i = -1;; i++) {
		...
		if (i == -1) { //まずは引数で指定された名前から。
			...
			if (pszLastName && _wcsicmp(pszLastName, pszName) == 0) { break; }
		}
		pszLastName = pszName;
...
LPCWSTR CDllImp::GetLoadedDllName() const { return m_strLoadedDllName.c_str(); }
...
*/
bool CDllImp::DeinitDllImp() { return true; }
