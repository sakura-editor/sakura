...
/////////////////////////////////////////////
// CPlug メンバ関数
bool CPlug::Invoke(CEditView *view, CWSHIfObj::List &params) { return m_cPlugin.InvokePlug(view, *this, params); }

EFunctionCode CPlug::GetFunctionCode() const { return GetPluginFunctionCode(m_cPlugin.m_id, m_id); }
...
{
	for (CPluginOption::ArrayIter it = m_options.begin(); it != m_options.end(); it++) { delete *it; }
}
	...
	for (i = 0; i < jacks.size(); i++)
	{
		...
		for (int nCount = 0; nCount < MAX_PLUG_CMD; nCount++)
		{
			...
			if (cProfile->IOProfileData(PII_PLUG, (sKey + szIndex).c_str(), sHandler))
			{
				...
				if (cProfileMlang) { cProfileMlang->IOProfileData(PII_PLUG, sKeyLabel.c_str(), sLabel); }
				if (sLabel == L"")
				{
...
bool CPlugin::ReadPluginDefCommand(CDataProfile *cProfile, CDataProfile *cProfileMlang)
{
	...
	for (int nCount = 1; nCount < MAX_PLUG_CMD; nCount++)
	{ //添え字は１から始める
		...
		if (cProfile->IOProfileData(PII_COMMAND, bufKey, sHandler))
		{
			...
			if (cProfileMlang) { cProfileMlang->IOProfileData(PII_COMMAND, bufKey, sLabel); }
			if (sLabel == L"")
			{
				sLabel = sHandler; // Labelが無ければハンドラ名で代用
			}
...
bool CPlugin::ReadPluginDefOption(CDataProfile *cProfile, CDataProfile *cProfileMlang)
{
	...
	sSection = L"";
	for (int nCount = 1; nCount < MAX_PLUG_OPTION; nCount++)
	{ //添え字は１から始める
		...
		if (cProfile->IOProfileData(PII_OPTION, bufKey, sKey))
		{
			...
			if (cProfileMlang) { cProfileMlang->IOProfileData(PII_OPTION, bufKey, sLabel); }
			// Typeの取得
			...
			if (cProfileMlang) { cProfileMlang->IOProfileData(PII_OPTION, bufKey, sSelect); }
			// デフォルト値
...
std::wstring CPlugin::GetFilePath(const wstring &sFileName) const { return m_sBaseDir + L"\\" + sFileName; }

std::wstring CPlugin::GetFolderName() const { return GetFileTitlePointer(m_sBaseDir.c_str()); }
...
int CPlugin::AddCommand(const WCHAR *handler, const WCHAR *label, const WCHAR *icon, bool doRegister)
{
	...
	if (icon) { newPlug->m_sIcon = icon; }

	m_plugs.push_back(newPlug);

	if (doRegister) { CJackManager::getInstance()->RegisterPlug(PP_COMMAND_STR, newPlug); }
...
std::vector<std::wstring> wstring_split(std::wstring sTrg, wchar_t cSep)
{
	...
	if (sTrg != L"") { splitVec.push_back(sTrg); }
...
bool CPlugin::ReadPluginDefString(CDataProfile *cProfile, CDataProfile *cProfileMlang)
{
	...
	for (int nCount = 1; nCount < MAX_PLUG_STRING; nCount++)
	{ //添え字は１から始める
		wstring sVal = L"";
		_swprintf(bufKey, L"S[%d]", nCount);
		if (cProfile->IOProfileData(PII_STRING, bufKey, sVal))
		{
			if (cProfileMlang) { cProfileMlang->IOProfileData(PII_STRING, bufKey, sVal); }
		}
