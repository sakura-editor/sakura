...
public:
	...
	int		 InstallPlugin(CommonSetting &common, const WCHAR *pszPluginName, HWND hWndOwner, wstring &errorMsg,
						   bool bUpdate = false); //プラグインの初期導入をする
	bool	 InstZipPlugin(CommonSetting &common, HWND hWndOwner, const wstring &sZipName,
						   bool bInSearch = false);			 // Zipプラグインを追加する
...
public:
	...
	bool		  SearchNewPluginDir(CommonSetting &common, HWND hWndOwner, const wstring &sSearchDir,
									 bool &bCancel); //新規プラグインを追加する(下請け)
	bool		  SearchNewPluginZip(CommonSetting &common, HWND hWndOwner, const wstring &sSearchDir,
									 bool &bCancel); //新規プラグインを追加する(下請け)Zip File
	bool InstZipPluginSub(CommonSetting &common, HWND hWndOwner, const wstring &sZipName, const wstring &sDispName,
						  bool bInSearch, bool &bCancel); // Zipプラグインを導入する(下請け)
