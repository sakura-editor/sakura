...
//ジャック定義一覧を返す
std::vector<JackDef> CJackManager::GetJackDef() const { return m_Jacks; }
...
ERegisterPlugResult CJackManager::RegisterPlug(wstring pszJack, CPlug *plug)
{
	...
	{
		int nMethod = CPlug::GetOutlineType(
	...
	{
		int nMethod = CPlug::GetSmartIndentType(
			plug->GetFunctionCode()); // 2011/8/20 syat プラグ複数化のためGetOutlineType仕様変更// 2010/5/1 Uchi 関数化
...
EJack CJackManager::GetJackFromName(wstring sName)
{
	...
	{
		if (wcscmp(m_Jacks[i].szName, szName) == 0) { return m_Jacks[i].ppId; }
	}
...
//利用可能なプラグを検索する
bool CJackManager::GetUsablePlug(EJack		   jack,   //!< [in] ジャック番号
...
)
{
	...
	{
		if (plugId == 0 || plugId == (*it)->GetFunctionCode()) { plugs->push_back(*it); }
	}
	return true;
}
...
void CJackManager::InvokePlugins(EJack jack, CEditView *view)
{
	...
	for (CPlug::ArrayIter it = plugs.begin(); it != plugs.end(); it++) { (*it)->Invoke(view, params); }
}
...
//プラグインコマンドの数を返す
int CJackManager::GetCommandCount() const { return m_Jacks[PP_COMMAND].plugs.size(); }
...
//プラグを返す
const CPlug::Array &CJackManager::GetPlugs(EJack jack) const { return m_Jacks[jack].plugs; }
