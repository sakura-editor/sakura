...
DEF_MACROFUNC(	F_GETFILENAME,			L"GetFilename",			VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// 編集中のファイル名を取得する
DEF_MACROFUNC(	F_GETSAVEFILENAME,		L"GetSaveFilename",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// 保存時のファイル名を取得する	// 2006.09.04 ryoji
DEF_MACROFUNC(	F_GETSELECTED,			L"GetSelectedString",	VT_I4,    VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// 選択範囲の取得
DEF_MACROFUNC(	F_EXPANDPARAMETER,		L"ExpandParameter",		VT_BSTR,  VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// コマンドラインパラメータ展開
DEF_MACROFUNC(	F_GETLINESTR,			L"GetLineStr",			VT_I4,    VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// 指定行論理データを取得する
DEF_MACROFUNC(	F_GETLINECOUNT,			L"GetLineCount",		VT_I4,    VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 論理行数を取得する
DEF_MACROFUNC(	F_CHGTABWIDTH,			L"ChangeTabWidth",		VT_I4,    VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// タブサイズを取得、設定する
DEF_MACROFUNC(	F_ISTEXTSELECTED,		L"IsTextSelected",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// テキストが選択されているか
DEF_MACROFUNC(	F_GETSELLINEFROM,		L"GetSelectLineFrom",	VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 選択開始行の取得
DEF_MACROFUNC(	F_GETSELCOLUMNFROM,		L"GetSelectColumnFrom",	VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 選択開始桁の取得
DEF_MACROFUNC2( F_GETSELCOLUMNFROM,		L"GetSelectColmFrom",	VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 選択開始桁の取得
DEF_MACROFUNC(	F_GETSELLINETO,			L"GetSelectLineTo",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 選択終了行の取得
DEF_MACROFUNC(	F_GETSELCOLUMNTO,		L"GetSelectColumnTo",	VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 選択終了桁の取得
DEF_MACROFUNC2(	F_GETSELCOLUMNTO,		L"GetSelectColmTo",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 選択終了桁の取得
DEF_MACROFUNC(	F_ISINSMODE,			L"IsInsMode",			VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 挿入／上書きモードの取得
DEF_MACROFUNC(	F_GETCHARCODE,			L"GetCharCode",			VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 文字コード取得
DEF_MACROFUNC(	F_GETLINECODE,			L"GetLineCode",			VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 改行コード取得
DEF_MACROFUNC(	F_ISPOSSIBLEUNDO,		L"IsPossibleUndo",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// Undo可能か調べる
DEF_MACROFUNC(	F_ISPOSSIBLEREDO,		L"IsPossibleRedo",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// Redo可能か調べる
DEF_MACROFUNC(	F_CHGWRAPCOLUMN,		L"ChangeWrapColumn",	VT_I4,    VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 折り返し桁を取得、設定する 2008.06.19 ryoji
...
DEF_MACROFUNC(	F_ISCURTYPEEXT,			L"IsCurTypeExt",		VT_BSTR,  VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 指定した拡張子が現在のタイプ別設定に含まれているかどうかを調べる 2006.09.04 ryoji
DEF_MACROFUNC(	F_ISSAMETYPEEXT,		L"IsSameTypeExt",		VT_BSTR,  VT_BSTR,  VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// ２つの拡張子が同じタイプ別設定に含まれているかどうかを調べる 2006.09.04 ryoji
DEF_MACROFUNC(	F_INPUTBOX,				L"InputBox",			VT_BSTR,  VT_BSTR,  VT_I4,    VT_EMPTY,	VT_BSTR,	NULL  )		// テキスト入力ダイアログの表示
DEF_MACROFUNC(	F_MESSAGEBOX,			L"MessageBox",			VT_BSTR,  VT_I4,    VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// メッセージボックスの表示
DEF_MACROFUNC(	F_ERRORMSG,				L"ErrorMsg",			VT_BSTR,  VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// メッセージボックス（エラー）の表示
DEF_MACROFUNC(	F_WARNMSG,				L"WarnMsg",				VT_BSTR,  VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// メッセージボックス（警告）の表示
DEF_MACROFUNC(	F_INFOMSG,				L"InfoMsg",				VT_BSTR,  VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// メッセージボックス（情報）の表示
DEF_MACROFUNC(	F_OKCANCELBOX,			L"OkCancelBox",			VT_BSTR,  VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// メッセージボックス（確認：OK／キャンセル）の表示
DEF_MACROFUNC(	F_YESNOBOX,				L"YesNoBox",			VT_BSTR,  VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// メッセージボックス（確認：はい／いいえ）の表示
DEF_MACROFUNC(	F_COMPAREVERSION,		L"CompareVersion",		VT_BSTR,  VT_BSTR,  VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// バージョン番号の比較
DEF_MACROFUNC(	F_MACROSLEEP,			L"Sleep",				VT_I4,    VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 指定した時間（ミリ秒）停止
DEF_MACROFUNC(	F_FILEOPENDIALOG,		L"FileOpenDialog",		VT_BSTR,  VT_BSTR,  VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// ファイルを開くダイアログの表示
DEF_MACROFUNC(	F_FILESAVEDIALOG,		L"FileSaveDialog",		VT_BSTR,  VT_BSTR,  VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// ファイルを保存ダイアログの表示
DEF_MACROFUNC(	F_FOLDERDIALOG,			L"FolderDialog",		VT_BSTR,  VT_BSTR,  VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// フォルダを開くダイアログの表示
DEF_MACROFUNC(	F_GETCLIPBOARD,			L"GetClipboard",		VT_I4,    VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// クリップボードの文字列を取得
DEF_MACROFUNC(	F_SETCLIPBOARD,			L"SetClipboard",		VT_I4,    VT_BSTR,  VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// クリップボードに文字列を設定
DEF_MACROFUNC(	F_LAYOUTTOLOGICLINENUM,	L"LayoutToLogicLineNum",VT_I4,    VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// ロジック行番号取得
DEF_MACROFUNC(	F_LOGICTOLAYOUTLINENUM,	L"LogicToLayoutLineNum",VT_I4,    VT_I4,    VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// レイアウト行番号取得
DEF_MACROFUNC(	F_LINECOLUMNTOINDEX,	L"LineColumnToIndex",	VT_I4,    VT_I4,    VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// ロジック桁番号取得
DEF_MACROFUNC(	F_LINEINDEXTOCOLUMN,	L"LineIndexToColumn",	VT_I4,    VT_I4,    VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// レイアウト桁番号取得
DEF_MACROFUNC(	F_GETCOOKIE,			L"GetCookie",			VT_BSTR,  VT_BSTR,  VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// Cookie取得
DEF_MACROFUNC(	F_GETCOOKIEDEFAULT,		L"GetCookieDefault",	VT_BSTR,  VT_BSTR,  VT_BSTR,  VT_EMPTY,	VT_BSTR,	NULL  )		// Cookie取得デフォルト値
DEF_MACROFUNC(	F_SETCOOKIE,			L"SetCookie",			VT_BSTR,  VT_BSTR,  VT_BSTR,  VT_EMPTY,	VT_I4,		NULL  )		// Cookie設定
DEF_MACROFUNC(	F_DELETECOOKIE,			L"DeleteCookie",		VT_BSTR,  VT_BSTR,  VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// Cookie削除
DEF_MACROFUNC(	F_GETCOOKIENAMES,		L"GetCookieNames",		VT_BSTR,  VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_BSTR,	NULL  )		// Cookie名前取得
DEF_MACROFUNC(	F_SETDRAWSWITCH,		L"SetDrawSwitch",		VT_I4,    VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 再描画スイッチ設定
DEF_MACROFUNC(	F_GETDRAWSWITCH,		L"GetDrawSwitch",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 再描画スイッチ取得
DEF_MACROFUNC(	F_ISSHOWNSTATUS,		L"IsShownStatus",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// ステータスウィンドウが表示されているか
DEF_MACROFUNC(	F_GETSTRWIDTH,			L"GetStrWidth",			VT_BSTR,  VT_I4,    VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 文字列幅取得
DEF_MACROFUNC(	F_GETSTRLAYOUTLENGTH,	L"GetStrLayoutLength",	VT_BSTR,  VT_I4,    VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 文字列のレイアウト幅取得
DEF_MACROFUNC(	F_GETDEFAULTCHARLENGTH,	L"GetDefaultCharLength",VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// デフォルト文字幅の取得
DEF_MACROFUNC(	F_ISINCLUDECLIPBOARDFORMAT,L"IsIncludeClipboardFormat",VT_BSTR,  VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// クリップボードの形式取得
DEF_MACROFUNC(	F_GETCLIPBOARDBYFORMAT,	L"GetClipboardByFormat",VT_BSTR,  VT_I4,    VT_I4,    VT_EMPTY,	VT_BSTR,	NULL  )		// クリップボードの指定形式で取得
DEF_MACROFUNC(	F_SETCLIPBOARDBYFORMAT,	L"SetClipboardByFormat",VT_BSTR,  VT_BSTR,  VT_I4,    VT_I4,    VT_I4,		NULL  )		// クリップボードの指定形式で設定
DEF_MACROFUNC(	F_GETLINEATTRIBUTE,		L"GetLineAttribute",	VT_I4,    VT_I4,    VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 行属性取得
DEF_MACROFUNC(	F_ISTEXTSELECTINGLOCK,	L"IsTextSelectingLock",	VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// 選択状態のロックを取得
DEF_MACROFUNC(	F_GETVIEWLINES,			L"GetViewLines",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// ビューの行数取得
DEF_MACROFUNC(	F_GETVIEWCOLUMNS,		L"GetViewColumns",		VT_EMPTY, VT_EMPTY, VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// ビューの列数取得
DEF_MACROFUNC(	F_CREATEMENU,			L"CreateMenu",			VT_I4,    VT_BSTR,  VT_EMPTY, VT_EMPTY,	VT_I4,		NULL  )		// メニュー作成
