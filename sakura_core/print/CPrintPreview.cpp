...
CPrintPreview::~CPrintPreview()
{
	...
	// 再描画用コンパチブルDC
	if (m_hdcCompatDC != NULL) { ::DeleteDC(m_hdcCompatDC); }
}
...
*/
LRESULT CPrintPreview::OnPaint(HWND   hwnd,   // handle of window
...
)
{
	...
	WCHAR szText[1024];
	::DlgItem_SetText(m_hwndPrintPreviewBar, IDC_STATIC_PRNDEV, m_pPrintSetting->m_mdmDevMode.m_szPrinterDeviceName);
	...
	auto_sprintf(szText, L"%s  %s", szPaperName,
				 (m_pPrintSetting->m_mdmDevMode.dmOrientation & DMORIENT_LANDSCAPE) ? LS(STR_ERR_DLGPRNPRVW1)
																					: LS(STR_ERR_DLGPRNPRVW2));
	...
	::SetViewportOrgEx(hdc, ((-1 * m_nPreviewHScrollPos) * m_nbmpCompatScale) / COMPAT_BMP_BASE,
					   ((nToolBarHeight + m_nPreviewVScrollPos) * m_nbmpCompatScale) / COMPAT_BMP_BASE, &poViewPortOld);
	...
	::Rectangle(hdc, m_nPreview_ViewMarginLeft, nDirectY * (m_nPreview_ViewMarginTop),
	...
	::Rectangle(hdc, m_nPreview_ViewMarginLeft + m_pPrintSetting->m_nPrintMarginLX,
	...
	cRect.bottom =
		nDirectY * (m_nPreview_ViewMarginTop + m_nPreview_PaperAllHeight - (m_pPrintSetting->m_nPrintMarginBY + 5));
	...
	// ヘッダ
	if (nHeaderHeight) { DrawHeaderFooter(hdc, cRect, true); }

	CColorStrategy *pStrategyStart = DrawPageTextFirst(m_nCurPageNum);

	// 印刷/印刷プレビュー ページテキストの描画
	DrawPageText(hdc, m_nPreview_ViewMarginLeft + m_pPrintSetting->m_nPrintMarginLX,
				 m_nPreview_ViewMarginTop + m_pPrintSetting->m_nPrintMarginTY + nHeaderHeight * 2, m_nCurPageNum, NULL,
				 pStrategyStart);

	// フッタ
	if (CPrint::CalcFooterHeight(m_pPrintSetting)) { DrawHeaderFooter(hdc, cRect, false); }
	...
	{
		::BitBlt(hdcOld, ps.rcPaint.left, ps.rcPaint.top, ps.rcPaint.right - ps.rcPaint.left,
				 ps.rcPaint.bottom - ps.rcPaint.top, hdc, ps.rcPaint.left, ps.rcPaint.top, SRCCOPY);
	}
	else
	{
		int stretchModeOld = SetStretchBltMode(hdcOld, STRETCH_HALFTONE);
		::StretchBlt(hdcOld, ps.rcPaint.left, ps.rcPaint.top, ps.rcPaint.right - ps.rcPaint.left,
					 ps.rcPaint.bottom - ps.rcPaint.top, hdc, (ps.rcPaint.left * m_nbmpCompatScale) / COMPAT_BMP_BASE,
					 (ps.rcPaint.top * m_nbmpCompatScale) / COMPAT_BMP_BASE,
					 ((ps.rcPaint.right - ps.rcPaint.left) * m_nbmpCompatScale) / COMPAT_BMP_BASE,
					 ((ps.rcPaint.bottom - ps.rcPaint.top) * m_nbmpCompatScale) / COMPAT_BMP_BASE, SRCCOPY);
...
LRESULT CPrintPreview::OnSize(WPARAM wParam, LPARAM lParam)
{
	...
	{
		::MoveWindow(m_hwndVScrollBar, cx - nCxVScroll, nToolBarHeight, nCxVScroll, cy - nCyVScroll - nToolBarHeight,
					 TRUE);
	}
	...
	{ m_nbmpCompatScale = COMPAT_BMP_SCALE; } else
	{
	...
	if (NULL != m_hwndSizeBox)
	{
		...
		{
			if (::IsZoomed(m_pParentWnd->GetHwnd())) { ::ShowWindow(m_hwndSizeBox, SW_HIDE); }
			else
			{
...
LRESULT CPrintPreview::OnVScroll(WPARAM wParam, LPARAM lParam)
{
	...
	switch (nScrollCode)
	{
	case SB_LINEUP: nMove = -1 * LINE_RANGE_Y; break;
	case SB_LINEDOWN: nMove = LINE_RANGE_Y; break;
	case SB_PAGEUP: nMove = -1 * PAGE_RANGE_Y; break;
	case SB_PAGEDOWN: nMove = PAGE_RANGE_Y; break;
	case SB_THUMBPOSITION:
	case SB_THUMBTRACK: nMove = nPos - nNowPos; break;
	// 2006.08.14 Moca SB_TOP, SB_BOTTOMへの対応
	case SB_TOP: nMove = -1 * nNowPos; break;
	case SB_BOTTOM: nMove = si.nMax - nNowPos; break;
	default: return 0;
	}
	nNewPos = nNowPos + nMove;
	if (nNewPos < 0) { nNewPos = 0; }
	else if (nNewPos > (int)(si.nMax - si.nPage + 1))
	{
...
LRESULT CPrintPreview::OnHScroll(WPARAM wParam, LPARAM lParam)
{
	...
	case SB_LEFT: nMove = -1 * nNowPos; break;
	case SB_RIGHT: nMove = si.nMax - nNowPos; break;
	default: return 0;
	}
	nNewPos = nNowPos + nMove;
	if (nNewPos < 0) { nNewPos = 0; }
...
LRESULT CPrintPreview::OnMouseMove(WPARAM wParam, LPARAM lParam)
{
	...
	if (!m_pParentWnd->GetDragMode()) { return 0; }
	...
	if (m_SCROLLBAR_VERT)
	{
		...
		else if (nNewPosY > (int)(siV.nMax - siV.nPage + 1))
		{
	...
	if (m_SCROLLBAR_HORZ)
	{
		...
		else if (nNewPosX > (int)(siH.nMax - siH.nPage + 1))
		{
...
LRESULT CPrintPreview::OnMouseWheel(WPARAM wParam, LPARAM lParam)
{
	...
	short zDelta =
		(short)HIWORD(wParam); // wheel rotation
							   //	short	xPos = (short) LOWORD(lParam);		// horizontal position of pointer
							   //	short	yPos = (short) HIWORD(lParam);		// vertical position of pointer

	int nScrollCode;
	if (0 < zDelta) { nScrollCode = SB_LINEUP; }
	else
	{
...
void CPrintPreview::OnChangePrintSetting(void)
{
	...
	::EnumFontFamilies(hdc, NULL, (FONTENUMPROC)CPrintPreview::MyEnumFontFamProc, (LPARAM)this);
	...
	if (m_pPrintSetting->m_mdmDevMode.m_szPrinterDeviceName[0] == L'\0') { GetDefaultPrinterInfo(); }
	...
	{
		/* 行番号表示に必要な桁数を計算 */
		m_nPreview_LineNumberColumns =
			m_pParentWnd->GetActiveView().GetTextArea().DetectWidthOfLineNumberArea_calculate(m_pLayoutMgr_Print);
	}
	...
	/* 印刷/プレビューに必要な情報を取得 */
	WCHAR szErrMsg[1024];
	if (!m_cPrint.GetPrintMetrics(&m_pPrintSetting->m_mdmDevMode, /* プリンタ設定 DEVMODE用*/
	...
	else
	{
		if (m_pPrintSetting->m_nPrintPaperSize != m_pPrintSetting->m_mdmDevMode.dmPaperSize)
		{
			...
			TopWarningMessage(m_pParentWnd->GetHwnd(), LS(STR_ERR_DLGPRNPRVW3),
							  m_pPrintSetting->m_mdmDevMode.m_szPrinterDeviceName, szPaperNameOld, szPaperNameNew);
		}
	}
	/* 現在のページ設定の、用紙サイズと用紙方向を反映させる(エラーでA4縦になった場合も考慮してif文の外へ移動 2003.07.03
	 * かろと) */
	m_pPrintSetting->m_nPrintPaperSize = m_pPrintSetting->m_mdmDevMode.dmPaperSize;
	m_pPrintSetting->m_nPrintPaperOrientation =
		m_pPrintSetting->m_mdmDevMode.dmOrientation; // 用紙方向の反映忘れを修正 2003/07/03 かろと
	...
	*m_pPrintSettingOrg = *m_pPrintSetting;
	...
	/* 行あたりの文字数(行番号込み) */
	m_bPreview_EnableColumns = CKetaXInt(CPrint::CalculatePrintableColumns(
		m_pPrintSetting, m_nPreview_PaperAllWidth, m_nPreview_LineNumberColumns)); /* 印字可能桁数/ページ */
	/* 縦方向の行数 */
	m_bPreview_EnableLines =
		CPrint::CalculatePrintableLines(m_pPrintSetting, m_nPreview_PaperAllHeight); /* 印字可能行数/ページ */
	...
	ref.m_bKinsokuHead							 = m_pPrintSetting->m_bPrintKinsokuHead,
	/* 行頭禁則する */ //@@@ 2002.04.08 MIK
		ref.m_bKinsokuTail = m_pPrintSetting->m_bPrintKinsokuTail,
	/* 行末禁則する */ //@@@ 2002.04.08 MIK
		ref.m_bKinsokuRet = m_pPrintSetting->m_bPrintKinsokuRet,
	/* 改行文字をぶら下げる */ //@@@ 2002.04.13 MIK
		ref.m_bKinsokuKuto = m_pPrintSetting->m_bPrintKinsokuKuto,
	/* 句読点をぶら下げる */ //@@@ 2002.04.17 MIK
		m_pLayoutMgr_Print->SetLayoutInfo(true, false, ref, ref.m_nTabSpace, ref.m_nTsvMode, ref.m_nMaxLineKetas,
										  CLayoutXInt(m_pPrintSetting->m_nPrintFontWidth), NULL);
	m_nAllPageNum = (WORD)((Int)m_pLayoutMgr_Print->GetLineCount()
						   / (m_bPreview_EnableLines * m_pPrintSetting->m_nPrintDansuu)); /* 全ページ数 */
	if (0 < m_pLayoutMgr_Print->GetLineCount() % (m_bPreview_EnableLines * m_pPrintSetting->m_nPrintDansuu))
	{ m_nAllPageNum++; } if (m_nAllPageNum <= m_nCurPageNum)
	{ /* 現在のページ */
		m_nCurPageNum = 0;
	}
	...
	if (m_pPrintSetting->m_bPrintLineNumber)
	{
		...
		int tempLineNum =
			m_pParentWnd->GetActiveView().GetTextArea().DetectWidthOfLineNumberArea_calculate(m_pLayoutMgr_Print);
		if (m_nPreview_LineNumberColumns != tempLineNum) { OnChangeSetting(); }
	}
...
void CPrintPreview::OnPreviewGoDirectPage(void)
{
	...
	BOOL bDlgInputPageResult = cDlgInputPage.DoModal(CEditApp::getInstance()->GetAppInstance(), m_hwndPrintPreviewBar,
													 LS(STR_ERR_DLGPRNPRVW5), szMessage, INPUT_PAGE_NUM_LEN, szPageNum);
	if (FALSE != bDlgInputPageResult)
	{
		...
		{
			if (!(L'0' <= szPageNum[i] && szPageNum[i] <= L'9')) { return; }
		}
...
void CPrintPreview::OnPreviewZoom(BOOL bZoomUp)
{
	if (bZoomUp)
	{
		...
		if (MAX_PREVIEW_ZOOM < m_nPreview_Zoom) { m_nPreview_Zoom = MAX_PREVIEW_ZOOM; }
	}
	else
	{
		/* スクロール位置を調整 */
		...
		m_nPreview_Zoom -= 10; /* 印刷プレビュー倍率 */
		if (MIN_PREVIEW_ZOOM > m_nPreview_Zoom) { m_nPreview_Zoom = MIN_PREVIEW_ZOOM; }
	}
	...
	if (MIN_PREVIEW_ZOOM == m_nPreview_Zoom)
	{
		...
		HWND focus = ::GetFocus();
		if (focus == GetDlgItem(m_hwndPrintPreviewBar, IDC_BUTTON_ZOOMDOWN)) { ::SetFocus(m_pParentWnd->GetHwnd()); }
	...
	if (MAX_PREVIEW_ZOOM == m_nPreview_Zoom)
	{
		// 2013.05.30 FocusがDisableなウィンドウだとマウススクロールできない対策
		HWND focus = ::GetFocus();
		if (focus == GetDlgItem(m_hwndPrintPreviewBar, IDC_BUTTON_ZOOMUP)) { ::SetFocus(m_pParentWnd->GetHwnd()); }
...
void CPrintPreview::OnPrint(void)
{
	...
	/* プリンタに渡すジョブ名を生成 */
	if (!m_pParentWnd->GetDocument()->m_cDocFile.GetFilePathClass().IsValidPath())
	{ /* 現在編集中のファイルのパス */
	...
	{
		m_bLockSetting = false;
		if (m_bDemandUpdateSetting) { OnChangePrintSetting(); }
		return;
	}
	if (m_pPrintSettingOrg->m_mdmDevMode != m_pPrintSetting->m_mdmDevMode)
	{
		...
		// 自分はLockで更新しない
		CAppNodeGroupHandle(0).PostMessageToAllEditors(MYWM_CHANGESETTING, (WPARAM)0, (LPARAM)PM_PRINTSETTING,
	...
	/* 印刷 ジョブ開始 */
	if (!m_cPrint.PrintOpen(szJobName, &m_pPrintSetting->m_mdmDevMode, /* プリンタ設定 DEVMODE用*/
							&hdc, szErrMsg							   /* エラーメッセージ格納場所 */
							))
	{
		//		MYTRACE( L"%s\n", szErrMsg );
	}
	...
	cRect.bottom =
		nDirectY * (m_nPreview_PaperAllHeight - (m_pPrintSetting->m_nPrintMarginBY + m_nPreview_PaperOffsetTop + 5));
	...
	for (i = 0; i < nNum; ++i)
	{
		...
		const CLayoutInt nPageTopLineNum =
			CLayoutInt(((nFrom + i) * m_pPrintSetting->m_nPrintDansuu) * m_bPreview_EnableLines);
		const CLayout *pcPageTopLayout = m_pLayoutMgr_Print->SearchLineByLayoutY(nPageTopLineNum);
		if (m_pPrintSetting->m_bColorPrint && !(i == 0) && pcPageTopLayout->GetLogicOffset() == 0)
		{
		...
		// 印刷/印刷プレビュー ページテキストの描画
		pStrategy = DrawPageText(hdc, m_pPrintSetting->m_nPrintMarginLX - m_nPreview_PaperOffsetLeft,
								 m_pPrintSetting->m_nPrintMarginTY - m_nPreview_PaperOffsetTop + nHeaderHeight * 2,
								 nFrom + i, &cDlgPrinting, pStrategy);
		...
		/* 中断ボタン押下チェック */
		if (cDlgPrinting.IsCanceled()) { break; }
	}
...
{
	bool bFontSetting =
		(bHeader ? m_pPrintSetting->m_lfHeader.lfFaceName[0] : m_pPrintSetting->m_lfFooter.lfFaceName[0]) != L'\0';
	...
	if (bFontSetting)
	{
		...
		lf.lfHeight = -(bHeader ? m_pPrintSetting->m_nHeaderPointSize : m_pPrintSetting->m_nFooterPointSize) * 254
					  / 720; // フォントのサイズ計算(pt->1/10mm)
		...
		// 左寄せ
		CSakuraEnvironment::ExpandParameter(bHeader ? m_pPrintSetting->m_szHeaderForm[POS_LEFT]
													: m_pPrintSetting->m_szFooterForm[POS_LEFT],
											szWork, nWorkLen);
		Tab2Space(szWork);
		::ExtTextOut(hdc, rect.left, nY, 0, NULL, szWork, wcslen(szWork), NULL);

		// 中央寄せ
		CSakuraEnvironment::ExpandParameter(bHeader ? m_pPrintSetting->m_szHeaderForm[POS_CENTER]
													: m_pPrintSetting->m_szFooterForm[POS_CENTER],
											szWork, nWorkLen);
		Tab2Space(szWork);
		SIZE Size;
		nLen = wcslen(szWork);
		::GetTextExtentPoint32W(hdc, szWork, nLen, &Size); //テキスト幅
		::ExtTextOut(hdc, (rect.right + rect.left - Size.cx) / 2, nY, 0, NULL, szWork, nLen, NULL);

		// 右寄せ
		CSakuraEnvironment::ExpandParameter(bHeader ? m_pPrintSetting->m_szHeaderForm[POS_RIGHT]
													: m_pPrintSetting->m_szFooterForm[POS_RIGHT],
											szWork, nWorkLen);
		Tab2Space(szWork);
		nLen = wcslen(szWork);
		::GetTextExtentPoint32W(hdc, szWork, nLen, &Size); //テキスト幅
		::ExtTextOut(hdc, rect.right - Size.cx, nY, 0, NULL, szWork, nLen, NULL);
		// フォントの戻し
		::SelectObject(hdc, hFontOld);
	...
	else
	{
		...
		CSakuraEnvironment::ExpandParameter(bHeader ? m_pPrintSetting->m_szHeaderForm[POS_LEFT]
											...
											szWork, nWorkLen);
		nLen = wcslen(szWork);
		Print_DrawLine(hdc, CMyPoint(rect.left, nY), szWork, nLen, 0, nLen, CLayoutInt(0));
		...
		Print_DrawLine(hdc, CMyPoint((rect.right + rect.left - nTextWidth) / 2, nY), szWork, nLen, 0, nLen,
					   CLayoutInt(0));
		...
		nLen	   = wcslen(szWork);
		nTextWidth = CTextMetrics::CalcTextWidth2(szWork, nLen, nDx, spaceing); //テキスト幅
		Print_DrawLine(hdc, CMyPoint(rect.right - nTextWidth, nY), szWork, nLen, 0, nLen, CLayoutInt(0));
	}
}
...
CColorStrategy *CPrintPreview::DrawPageTextFirst(int nPageNum)
{
	...
	if (m_pPrintSetting->m_bColorPrint)
	{
		...
		const CLayoutInt nPageTopLineNum =
			CLayoutInt((nPageNum * m_pPrintSetting->m_nPrintDansuu) * m_bPreview_EnableLines);
		const CLayout *pcPageTopLayout = m_pLayoutMgr_Print->SearchLineByLayoutY(nPageTopLineNum);
		...
		if (pcPageTopLayout != NULL)
		{
			...
			// ページトップの物理行の先頭を検索
			while (pcPageTopLayout->GetLogicOffset()) { pcPageTopLayout = pcPageTopLayout->GetPrevLayout(); }
...
*/
CColorStrategy *CPrintPreview::DrawPageText(HDC hdc, int nOffX, int nOffY, int nPageNum, CDlgCancel *pCDlgCancel,
											CColorStrategy *pStrategyStart)
{
	int nDirectY = -1;

	const int nLineHeight = m_pPrintSetting->m_nPrintFontHeight
							+ (m_pPrintSetting->m_nPrintFontHeight * m_pPrintSetting->m_nPrintLineSpacing / 100);
	// 段と段の間隔の幅
	const int nDanWidth =
		(Int)m_bPreview_EnableColumns * m_pPrintSetting->m_nPrintFontWidth + m_pPrintSetting->m_nPrintDanSpace;
	// 行番号の幅
	...
	for (nDan = 0; nDan < m_pPrintSetting->m_nPrintDansuu; ++nDan)
	{
		...
		for (i = 0; i < m_bPreview_EnableLines; ++i)
		{
			...
			{
				/* 処理中のユーザー操作を可能にする */
				if (!::BlockingHook(pCDlgCancel->GetHwnd())) { return NULL; }
			}
			...
			*/
			const CLayoutInt nLineNum =
				CLayoutInt((nPageNum * m_pPrintSetting->m_nPrintDansuu + nDan) * m_bPreview_EnableLines + i);
			const CLayout *pcLayout = m_pLayoutMgr_Print->SearchLineByLayoutY(nLineNum);
			if (NULL == pcLayout) { break; }
			/* 行番号を表示するか */
			if (m_pPrintSetting->m_bPrintLineNumber)
			{
				...
				if (2 == m_pParentWnd->GetDocument()->m_cDocType.GetDocumentAttribute().m_nLineTermType)
				{
					...
					szLineTerm[0] = m_pParentWnd->GetDocument()
										->m_cDocType.GetDocumentAttribute()
										.m_cLineTermChar; /* 行番号区切り文字
														   */
					szLineTerm[1] = L'\0';
				...
				vector<int> vDxArray;
				int			spacing  = 0;
				const int * pDxArray = CTextMetrics::GenerateDxArray(&vDxArray, szLineNum, nLineCols,
																	 m_pPrintSetting->m_nPrintFontWidth, spacing);

				::ExtTextOut(hdc, nBasePosX - nLineCols * charWidth,
							 nDirectY
								 * (nOffY + nLineHeight * i + (m_pPrintSetting->m_nPrintFontHeight - m_nAscentHan)),
							 0, NULL, szLineNum, nLineCols, pDxArray);
			}

			const int nLineLen = pcLayout->GetLengthWithoutEOL();
			if (0 == nLineLen) { continue; }

			// 物理行頭の色指定を取得
			if (m_pPrintSetting->m_bColorPrint && !(nDan == 0 && i == 0) && pcLayout->GetLogicOffset() == 0)
			{
			...
			// 印刷／プレビュー 行描画
			pStrategy = Print_DrawLine(hdc, CMyPoint(nBasePosX, nDirectY * (nOffY + nLineHeight * i)),
									   pcLayout->GetDocLineRef()->GetPtr(), // pcLayout->GetPtr(),
									   (Int)pcLayout->GetDocLineRef()->GetLengthWithEOL(),
									   (Int)pcLayout->GetLogicOffset(), nLineLen,
									   pcLayout->GetIndent(), // 2006.05.16 Add Moca. レイアウトインデント分ずらす。
									   m_pPrintSetting->m_bColorPrint ? pcLayout : NULL, pStrategy);
		}
			...
			&& 1 == m_pParentWnd->GetDocument()->m_cDocType.GetDocumentAttribute().m_nLineTermType)
		{
			...
			::MoveToEx(hdc, nBasePosX - (m_pPrintSetting->m_nPrintFontWidth / 2), nDirectY * nOffY, NULL);
			::LineTo(hdc, nBasePosX - (m_pPrintSetting->m_nPrintFontWidth / 2), nDirectY * (nOffY + nLineHeight * i));
		}
	}
	return pStrategy;
}
...
*/
CColorStrategy *CPrintPreview::Print_DrawLine(HDC			 hdc,
											  ...
											  CColorStrategy *pStrategyStart)
{
	if (nLineLen == 0) { return pStrategyStart; }
	...
	//タブ幅取得
	//	CLayoutInt nTabSpace = m_pParentWnd->GetDocument()->m_cLayoutMgr.GetTabSpace(); //	Sep. 23, 2002 genta
	// LayoutMgrの値を使う
	...
	//文字間隔配列を生成
	vector<int> vDxArray;
	const int * pDxArray = CTextMetrics::GenerateDxArray(&vDxArray, pLine + nLineStart, nLineLen, nDx, (Int)nTabSpace,
														 (Int)nIndent, space);
	...
	for (iLogic = nLineStart; iLogic < nLineStart + nLineLen; ++iLogic, nKindLast = nKind)
	{
		//文字の種類
		if (pLine[iLogic] == WCODE::TAB) { nKind = 2; }
		...
		if (nKind != nKindLast || bChange)
		{
			...
			if (0 < iLogic - nBgnLogic)
			{
				Print_DrawBlock(hdc,
								ptDraw, //!< 描画座標。HDC内部単位。
								pLine + nLineStart, iLogic - nBgnLogic, nKindLast,
								pcLayout, //!< 色設定用Layout
								nColorIdx, nBgnLogic - nLineStart, nLayoutX, nDx, pDxArray);
				...
				{
					int nIncrement = 0;
					for (int i = nBgnLogic - nLineStart; i < iLogic - nLineStart; i++) { nIncrement += pDxArray[i]; }
					nLayoutX += nIncrement;
				}
				//ロジック進め
				nBgnLogic = iLogic;
	...
	if (0 < nLineStart + nLineLen - nBgnLogic)
	{
		Print_DrawBlock(hdc,
						ptDraw, //!< 描画座標。HDC内部単位。
						pLine + nLineStart, nLineStart + nLineLen - nBgnLogic, nKindLast,
						pcLayout, //!< 色設定用Layout
						nColorIdx, nBgnLogic - nLineStart, nLayoutX, nDx, pDxArray);
	}
	...
	if (pcLayout)
	{
		...
		if (-1 != nColorIdx)
		{
			const ColorInfo &info =
				m_pParentWnd->GetDocument()->m_cDocType.GetDocumentAttribute().m_ColorInfoArr[nColorIdx];
...
*/
void CPrintPreview::Print_DrawBlock(HDC			   hdc,
									POINT		   ptDraw, //!< 描画座標。HDC内部単位。
									...
									CLayoutInt nLayoutX, int nDx, const int *pDxArray)
{
	...
	::ExtTextOut(hdc, ptDraw.x + (Int)nLayoutX * charWidth,
				 ptDraw.y - (m_pPrintSetting->m_nPrintFontHeight - (nKind == 1 ? m_nAscentZen : m_nAscentHan)), 0, NULL,
				 &pPhysicalLine[nBgnPhysical], nBlockLen, &pDxArray[nBgnPhysical]);
}
...
*/
CColorStrategy *CPrintPreview::GetColorStrategy(const CStringRef &cStringLine, int iLogic, CColorStrategy *pStrategy,
												bool &bChange)
{
	if (pStrategy)
	{
...
int CALLBACK CPrintPreview::MyEnumFontFamProc(ENUMLOGFONT *  pelf,		// pointer to logical-font data
...
)
{
	...
	{ pCPrintPreview->SetPreviewFontZen(&pelf->elfLogFont); } return 1;
}
...
void CPrintPreview::CreatePrintPreviewControls(void)
{
	...
	m_hwndPrintPreviewBar =
		::CreateDialogParam(CSelectLang::getLangRsrcInstance(),		// handle to application instance
	...
	/* 縦スクロールバーの作成 */
	m_hwndVScrollBar = ::CreateWindowEx(0L,										   /* no extended styles			*/
	...
	/* サイズボックスの作成 */
	m_hwndSizeBox = ::CreateWindowEx(WS_EX_CONTROLPARENT /*0L*/,						 /* no extended styles			*/
									 L"SCROLLBAR",										 /* scroll bar control class		*/
									 NULL,												 /* text for window title bar	*/
...
/* ダイアログプロシージャ */
INT_PTR CALLBACK CPrintPreview::PrintPreviewBar_DlgProc(HWND   hwndDlg, // handle to dialog box
														UINT   uMsg,	// message
														WPARAM wParam,  // first message parameter
														LPARAM lParam   // second message parameter
)
{
	...
	case WM_INITDIALOG:
		...
		if (NULL != pCPrintPreview) { return pCPrintPreview->DispatchEvent_PPB(hwndDlg, uMsg, wParam, lParam); }
		return TRUE;
	default:
		// Modified by KEITA for WIN64 2003.9.6
...
{
	if (NULL != m_hwndPrintPreviewBar) { ::SetFocus(m_hwndPrintPreviewBar); }
}

/* 印刷プレビュー 操作バー ダイアログのメッセージ処理 */
// IDD_PRINTPREVIEWBAR
INT_PTR CPrintPreview::DispatchEvent_PPB(HWND   hwndDlg, // handle to dialog box
										 UINT   uMsg,	// message
										 WPARAM wParam,  // first message parameter
										 LPARAM lParam   // second message parameter
)
{
	...
	case WM_INITDIALOG: ::EnableWindow(::GetDlgItem(hwndDlg, IDC_CHECK_ANTIALIAS), TRUE); return TRUE;
	case WM_COMMAND:
		...
		case BN_CLICKED:
			...
			case IDC_BUTTON_PRINTERSELECT:
				...
				if (m_cPrint.PrintDlg(&pd, &m_pPrintSettingOrg->m_mdmDevMode))
				{
					...
					CAppNodeGroupHandle(0).SendMessageToAllEditors(
						MYWM_CHANGESETTING, (WPARAM)0, (LPARAM)PM_PRINTSETTING, CEditWnd::getInstance()->GetHwnd());
					// OnChangePrintSetting();
					// ::InvalidateRect( m_pParentWnd->GetHwnd(), NULL, TRUE );
				}
				// To Here 2003.05.03 かろと
				break;
			case IDC_BUTTON_PRINTSETTING: m_pParentWnd->OnPrintPageSetting(); break;
			...
			case IDC_BUTTON_DIRECTPAGE: OnPreviewGoDirectPage(); break;
			case IDC_CHECK_ANTIALIAS: OnCheckAntialias(); break;
			// To Here 2007.02.11 Moca
			case IDC_BUTTON_HELP:
				/* 印刷プレビューのヘルプ */
				// Stonee, 2001/03/12 第四引数を、機能番号からヘルプトピック番号を調べるようにした
				MyWinHelp(hwndDlg, HELP_CONTEXT,
						  ::FuncID_To_HelpContextID(F_PRINT_PREVIEW)); // 2006.10.10 ryoji MyWinHelpに変更に変更
				break;
			case IDOK:
				/* 印刷実行 */
				OnPrint();
...
#ifdef _DEEBUG
	m_hFontHan = m_hFontHan_b = m_hFontHan_u = m_hFontHan_bu = m_hFontZen = m_hFontZen_b = m_hFontZen_u =
		m_hFontZen_bu																	 = NULL;
#endif
}
