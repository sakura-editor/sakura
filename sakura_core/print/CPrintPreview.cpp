...
LRESULT CPrintPreview::OnSize(WPARAM wParam, LPARAM lParam)
{
    ...
    m_hbmpCompatBMP    = ::CreateCompatibleBitmap(hdc, (cx * m_nbmpCompatScale + COMPAT_BMP_BASE - 1) / COMPAT_BMP_BASE, (cy * m_nbmpCompatScale + COMPAT_BMP_BASE - 1) / COMPAT_BMP_BASE);
...
void CPrintPreview::OnChangePrintSetting(void)
{
    ...
    *m_pPrintSettingOrg = *m_pPrintSetting;
    ...
    /* 句読点をぶら下げる */ //@@@ 2002.04.17 MIK
        m_pLayoutMgr_Print->SetLayoutInfo(true, false, ref, ref.m_nTabSpace, ref.m_nTsvMode, ref.m_nMaxLineKetas, CLayoutXInt(m_pPrintSetting->m_nPrintFontWidth), NULL);
         ...
         ++iLogic, nKindLast = nKind)
    {
        ...
        if (nKind != nKindLast || bChange)
        {
            ...
            if (0 < iLogic - nBgnLogic)
            {
                ...
                if (nKindLast == 2)
                {
                    nLayoutX += (nTabSpace + tabPadding - (nLayoutX + tabPadding) % nTabSpace) + nTabSpace * (iLogic - nBgnLogic - 1);
                }
                else
                {
                    int nIncrement = 0;
