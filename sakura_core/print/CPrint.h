...
struct MYDEVMODE {
	...
	bool operator==(const MYDEVMODE &rhs) const noexcept
	{
		...
		return m_bPrinterNotFound == rhs.m_bPrinterNotFound
			   ...
			   && dmFields == rhs.dmFields && dmOrientation == rhs.dmOrientation && dmPaperSize == rhs.dmPaperSize
			   && dmPaperLength == rhs.dmPaperLength && dmPaperWidth == rhs.dmPaperWidth;
	}
	//! 否定の等価比較演算子
	bool operator!=(const MYDEVMODE &rhs) const noexcept { return !(*this == rhs); }
};
...
public:
	...
	*/
	...
	/* 用紙の幅、高さ */
	static BOOL GetPaperSize(short *pnPaperAllWidth, short *pnPaperAllHeight, MYDEVMODE *pDEVMODE);
	/* 印字可能桁・行の計算 */
...
public:
	...
	*/
	...
	/* 印刷/プレビューに必要な情報を取得 */
	BOOL GetPrintMetrics(MYDEVMODE *pMYDEVMODE, short *pnPaperAllWidth, /* 用紙幅 */
	...
	/* 印刷 ジョブ開始 */
	BOOL PrintOpen(WCHAR *pszJobName, MYDEVMODE *pMYDEVMODE, HDC *phdc, WCHAR *pszErrMsg /* エラーメッセージ格納場所 */
	);
