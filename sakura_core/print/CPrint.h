...
struct MYDEVMODE
{
    ...
    bool operator==(const MYDEVMODE &rhs) const noexcept
    {
        if (this == &rhs)
            return true;
        return m_bPrinterNotFound == rhs.m_bPrinterNotFound && 0 == wcsncmp(m_szPrinterDriverName, rhs.m_szPrinterDriverName, _countof(m_szPrinterDriverName)) && 0 == wcsncmp(m_szPrinterDeviceName, rhs.m_szPrinterDeviceName, _countof(m_szPrinterDeviceName)) && 0 == wcsncmp(m_szPrinterOutputName, rhs.m_szPrinterOutputName, _countof(m_szPrinterOutputName)) && dmFields == rhs.dmFields && dmOrientation == rhs.dmOrientation && dmPaperSize == rhs.dmPaperSize && dmPaperLength == rhs.dmPaperLength && dmPaperWidth == rhs.dmPaperWidth;
    }
    //! 否定の等価比較演算子
    bool operator!=(const MYDEVMODE &rhs) const noexcept
    {
