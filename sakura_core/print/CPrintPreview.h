...
class CPrintPreview
{
  ...
  public:
	...
	*/
    ...
    void OnPreviewGoPreviousPage()
    {
        OnPreviewGoPage(m_nCurPageNum - 1);
    } //	前のページへ
    void OnPreviewGoNextPage()
    {
        OnPreviewGoPage(m_nCurPageNum + 1);
    } //	前のページへ
	...
	*/
    ...
    HWND GetPrintPreviewBarHANDLE(void)
    {
        return m_hwndPrintPreviewBar;
    }
    HWND GetPrintPreviewBarHANDLE_Safe() const
    {
        if (!this)
            return NULL;
        else
            return m_hwndPrintPreviewBar;
    } //!< thisがNULLでも実行できる版。2007.10.29 kobake
  ...
  protected:
	...
	*/
    ...
    CColorStrategy *DrawPageText(HDC hdc, int nOffX, int nOffY, int nPageNum, CDlgCancel *pCDlgCancel, CColorStrategy *pStrategyStart);
  ...
  public:
	...
	*/
    ...
    {
        return m_cPrint.GetDefaultPrinter(&m_pPrintSetting->m_mdmDevMode);
    }
    int GetCurPageNum()
    {
