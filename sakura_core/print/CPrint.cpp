...
int CPrint::CalculatePrintableColumns(PRINTSETTING *pPS, int nPaperAllWidth, int nLineNumberColumns)
{
    ...
    if (nPrintablePaperWidth < 0)
    {
        return 0;
    }

    int nPrintSpaceWidth = (pPS->m_nPrintDansuu - 1) * pPS->m_nPrintDanSpace + (pPS->m_nPrintDansuu) * (nLineNumberColumns * pPS->m_nPrintFontWidth);
    if (nPrintablePaperWidth < nPrintSpaceWidth)
    {
        return 0;
    }

    int nEnableColumns =
        (nPrintablePaperWidth - nPrintSpaceWidth) / pPS->m_nPrintFontWidth / pPS->m_nPrintDansuu; /* 印字可能桁数/ページ */
