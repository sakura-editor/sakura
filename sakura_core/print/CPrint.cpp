...
BOOL CPrint::PrintDlg(PRINTDLG *pPD, MYDEVMODE *pMYDEVMODE)
{
	...
	if (m_hDevMode == NULL)
	{
		if (!GetDefaultPrinter(pMYDEVMODE)) { return FALSE; }
	}
	...
	// プリンタドライバ名
	wcscpy_s(pMYDEVMODE->m_szPrinterDriverName, _countof(pMYDEVMODE->m_szPrinterDriverName),
			 (const WCHAR *)pDEVNAMES + pDEVNAMES->wDriverOffset);
	// プリンタデバイス名
	wcscpy_s(pMYDEVMODE->m_szPrinterDeviceName, _countof(pMYDEVMODE->m_szPrinterDeviceName),
			 (const WCHAR *)pDEVNAMES + pDEVNAMES->wDeviceOffset);
	// プリンタポート名
	wcscpy_s(pMYDEVMODE->m_szPrinterOutputName, _countof(pMYDEVMODE->m_szPrinterOutputName),
...
*/
HDC CPrint::CreateDC(MYDEVMODE *pMYDEVMODE, WCHAR *pszErrMsg /* エラーメッセージ格納場所 */
)
{
	HDC		 hdc	  = NULL;
	...
	if (m_hDevMode == NULL) { return NULL; }

	//
	// OpenPrinter()で、デバイス名でプリンタハンドルを取得
	//
	if (!::OpenPrinter(pMYDEVMODE->m_szPrinterDeviceName, /* プリンタデバイス名 */
					   &hPrinter,						  /* プリンタハンドルのポインタ */
					   NULL))
	{
		auto_sprintf(pszErrMsg, LS(STR_ERR_CPRINT01), pMYDEVMODE->m_szPrinterDeviceName /* プリンタデバイス名 */
		);
		goto end_of_func;
	}
	...
	//
	::DocumentProperties(NULL, hPrinter, pMYDEVMODE->m_szPrinterDeviceName /* プリンタデバイス名 */, pDEVMODE, pDEVMODE,
						 DM_OUT_BUFFER | DM_IN_BUFFER);
	/* 指定デバイスに対するデバイス コンテキストを作成します。 */
	hdc = ::CreateDC(pMYDEVMODE->m_szPrinterDriverName, /* プリンタドライバ名 */
...
/* 印刷/プレビューに必要な情報を取得 */
BOOL CPrint::GetPrintMetrics(MYDEVMODE *pMYDEVMODE, short *pnPaperAllWidth, /* 用紙幅 */
...
)
{
	...
	if (NULL == (hdc = CreateDC(pMYDEVMODE, pszErrMsg))) { return FALSE; }
...
/* 用紙の幅、高さ */
BOOL CPrint::GetPaperSize(short *pnPaperAllWidth, short *pnPaperAllHeight, MYDEVMODE *pDEVMODE)
{
	short nWork;
...
/* 印刷 ジョブ開始 */
BOOL CPrint::PrintOpen(WCHAR *pszJobName, MYDEVMODE *pMYDEVMODE, HDC *phdc,
					   WCHAR *pszErrMsg /* エラーメッセージ格納場所 */
)
{
	BOOL	bRet;
	...
	{
		auto_sprintf(pszErrMsg, LS(STR_ERR_CPRINT02), pMYDEVMODE->m_szPrinterDeviceName /* プリンタデバイス名 */
		);
		bRet = FALSE;
		goto end_of_func;
	}
...
/* 印刷 ページ開始 */
void CPrint::PrintStartPage(HDC hdc) { ::StartPage(hdc); }

/* 印刷 ページ終了 */
void CPrint::PrintEndPage(HDC hdc) { ::EndPage(hdc); }
...
WCHAR *CPrint::GetPaperName(int nPaperSize, WCHAR *pszPaperName)
{
	...
	if (NULL != paperInfo) { wcscpy(pszPaperName, paperInfo->m_pszName); }
	else
	{
...
const PAPER_INFO *CPrint::FindPaperInfo(int id)
{
	...
	{
		if (m_paperInfoArr[i].m_nId == id) { return &(m_paperInfoArr[i]); }
	}
	return NULL;
}
...
void CPrint::SettingInitialize(PRINTSETTING &pPrintSetting, const WCHAR *settingName)
{
	...
	pPrintSetting.m_nPrintPaperOrientation =
		DMORIENT_PORTRAIT; /* 用紙方向 DMORIENT_PORTRAIT (1) または DMORIENT_LANDSCAPE (2) */
...
int CPrint::CalculatePrintableColumns(PRINTSETTING *pPS, int nPaperAllWidth, int nLineNumberColumns)
{
	...
	int nEnableColumns = (nPrintablePaperWidth - nPrintSpaceWidth) / pPS->m_nPrintFontWidth
						 / pPS->m_nPrintDansuu; /* 印字可能桁数/ページ */
...
{
	if (pPS->m_szHeaderForm[0][0] == L'\0' && pPS->m_szHeaderForm[1][0] == L'\0' && pPS->m_szHeaderForm[2][0] == L'\0')
	{
		// 使ってなければ 0
		return 0;
	}
...
{
	if (pPS->m_szFooterForm[0][0] == L'\0' && pPS->m_szFooterForm[1][0] == L'\0' && pPS->m_szFooterForm[2][0] == L'\0')
	{
		// 使ってなければ 0
		return 0;
	}
