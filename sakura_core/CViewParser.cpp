...
/*
	...
	単語区切り
*/
int CViewParser::GetLeftWord( CNativeW2* pcmemWord, int nMaxWordLen ) const
{
	const wchar_t*	pLine;
	CLogicInt		nLineLen;
	CLogicInt		nIdx;
...
//		return 0;
	...
	if( 0 == nIdxTo || NULL == pLine ){
		...
		nCharChars = &pLine[nLineLen] - CNativeW2::GetCharPrev( pLine, nLineLen, &pLine[nLineLen] );
		if( 0 == nCharChars ){
			return 0;
		}
		nIdxTo = nLineLen;
		...
		nCharChars = &pLine[nIdxTo] - CNativeW2::GetCharPrev( pLine, nLineLen, &pLine[nIdxTo] );
		if( 0 == nCharChars ){
			return 0;
		}
		nIdx = nIdxTo - CLogicInt(nCharChars);
