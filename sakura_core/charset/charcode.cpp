...
public:
	...
	{
		return &lhs == &rhs || 0 == memcmp(&lhs, &rhs, sizeof(lhs));
	}
	...
	void Init(const LOGFONT &lf, const LOGFONT &lfFull, HDC hdcOrg)
	{
		...
		for (int i = 0; i < size; i++) {
			...
			sz.cx = (sz.cx / 26 + 1) / 2;
			if (m_han_size.cx < sz.cx) { m_han_size.cx = sz.cx; }
			if (m_han_size.cy < sz.cy) { m_han_size.cy = sz.cy; }
		}
	}
	...
	void  SetCachePx(wchar_t c, short cache_pxwidth) { m_pCache->m_nCharPxWidthCache[c] = cache_pxwidth; }
	short GetCachePx(wchar_t c) const { return _GetRawPx(c); }
	...
	bool GetMultiFont() const { return m_bMultiFont; }

protected:
	int _GetRawPx(wchar_t c) const { return m_pCache->m_nCharPxWidthCache[c]; }
	HDC SelectHDC(wchar_t c) const { return m_hdcFull && WCODE::GetFontNo(c) ? m_hdcFull : m_hdc; }
...
public:
	LocalCacheSelector()
	{
		...
		for (int i = 0; i < CWM_FONT_MAX; i++) { m_parCache[i] = 0; }
