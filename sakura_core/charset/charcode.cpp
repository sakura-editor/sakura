...
bool IsHankaku(wchar_t wc)
{
    ...
    if (wc >= 0x007F && wc <= 0x00A0)
        return true; // Control Code ISO/IEC 6429
...
class LocalCacheSelector
{
  public:
    ...
    void Select(ECharWidthFontMode fMode, ECharWidthCacheMode cMode)
    {
        ...
        if (fMode == CWM_FONT_EDIT)
        {
            m_eLastEditCacheMode = cmode;
        }
