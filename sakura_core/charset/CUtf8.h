...
public:
	...
	EConvertResult _UnicodeToHex(const wchar_t *cSrc, const int iSLen, WCHAR *pDst,
								 const CommonSetting_Statusbar *psStatusbar,
								 const bool						CESU8Mode); //!< UNICODE → Hex 変換
	EConvertResult UnicodeToHex(const wchar_t *ps, const int nsl, WCHAR *pd,
								const CommonSetting_Statusbar *psStatusbar) override
	{
...
public:
	...
	inline static EConvertResult UTF8ToUnicode(const CMemory &cSrc, CNativeW *pDst)
	{
		return _UTF8ToUnicode(cSrc, pDst, false);
	} // UTF-8 -> Unicodeコード変換
	inline static EConvertResult CESU8ToUnicode(const CMemory &cSrc, CNativeW *pDst)
	{
		return _UTF8ToUnicode(cSrc, pDst, true);
	} // CESU-8 -> Unicodeコード変換
	inline static EConvertResult UnicodeToUTF8(const CNativeW &cSrc, CMemory *pDst)
	{
		return _UnicodeToUTF8(cSrc, pDst, false);
	} // Unicode → UTF-8コード変換
	inline static EConvertResult UnicodeToCESU8(const CNativeW &cSrc, CMemory *pDst)
	{
		return _UnicodeToUTF8(cSrc, pDst, true);
	} // Unicode → CESU-8コード変換
...
protected:
	...
	inline static int _UniToUtf8_char(const unsigned short *pSrc, const int nSrcLen, unsigned char *pDst,
									  bool bCESU8Mode);
