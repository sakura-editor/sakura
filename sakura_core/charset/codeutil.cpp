...
unsigned int __fastcall SjisFilter_basis(const unsigned int uCode)
{
	...
	if (nleft <= nright) { return TABLE_SjisPoorcodeDef[TABLE_SjisPoorcodeIndex[ni][1]]; }
	return uCode;
}
...
unsigned int __fastcall SjisFilter_ibm2nec(const unsigned int uCode)
{
	...
	*/
	...
	if (c1 == 0xfa || c1 == 0xfb || (c1 == 0xfc && c2 <= 0x4b))
	{
		...
		{
			code -= 0x0d1c;
		} /* fb5c～fb7e → ee40～ee62 (犾～神) */
		else if (code <= 0xfb9b)
		{
			code -= 0x0d1d;
		} /* fb80～fb9b → ee63～ee7e (祥～蕙) */
		else if (code <= 0xfbfc)
		{
			code -= 0x0d1c;
...
unsigned int __fastcall SjisFilter_nec2ibm(const unsigned int uCode)
{
	...
	*/
	...
	if (c1 == 0xed || c1 == 0xee)
	{
		...
		{
			code += 0x0d1c;
		} /* ed80～ede0 → fa9c～fafc (塚～浯) */
		else if (code <= 0xedfc)
		{
			code += 0x0d5f;
		} /* ede1～edfc → fb40～fb5b (涖～犱) */
		else if (code <= 0xee62)
		{
			code += 0x0d1c;
		} /* ee40～ee62 → fb5c～fb7e (犾～神) */
		else if (code <= 0xee7e)
		{
			code += 0x0d1d;
		} /* ee63～ee7e → fb80～fb9b (祥～蕙) */
		else if (code <= 0xeee0)
		{
			code += 0x0d1c;
		} /* ee80～eee0 → fb9c～fbf0 (蕫～髙) */
		else if (code <= 0xeeec)
		{
			code += 0x0d5f;
