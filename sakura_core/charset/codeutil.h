...
{
	if (wc == 0x3094 /* ←平仮名の う に濁点をつけたやつ。 */) { return true; }
	if (0x00a0 <= wc && wc <= 0x00ff && TABLE_WctombSpec[wc - 0x00a0]) { return true; }
	return false;
}
...
/*!
	...
	nSrcLen は 1 か 2
*/
inline int MyMultiByteToWideChar_JP(const unsigned char *pSrc, const int nSrcLen, unsigned short *pDst,
									bool bKeepRt = true)
{
	int			  nret;
