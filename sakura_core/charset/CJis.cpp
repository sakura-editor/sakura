...
const char CJis::JISESCDATA_ASCII7[] = "\x1b"
									   "(B"; // ☆
const char CJis::JISESCDATA_JISX0201Latin[] = "\x1b"
											  "(J";
const char CJis::JISESCDATA_JISX0201Latin_OLD[] = "\x1b"
												  "(H";
const char CJis::JISESCDATA_JISX0201Katakana[] = "\x1b"
												 "(I"; // ☆
const char CJis::JISESCDATA_JISX0208_1978[] = "\x1b"
											  "$@";
const char CJis::JISESCDATA_JISX0208_1983[] = "\x1b"
											  "$B"; // ☆
const char CJis::JISESCDATA_JISX0208_1990[] = "\x1b"
											  "&@"
											  "\x1b"
											  "$B";
...
*/
int CJis::_JisToUni_block(const unsigned char *pSrc, const int nSrcLen, unsigned short *pDst,
						  const EMyJisEscseq eMyJisesc, bool *pbError)
{
...
int CJis::JisToUni(const char *pSrc, const int nSrcLen, wchar_t *pDst, bool *pbError)
{
	...
	do {
		...
		case MYJISESC_ASCII7:
			// ASCII7 ブロックをチェック
			nblocklen = CheckJisAscii7Part(reinterpret_cast<const char *>(pr), pr_end - pr,
										   reinterpret_cast<const char **>(&pr_next), &next_esctype, NULL);
			break;
		case MYJISESC_HANKATA:
			// 半角カタカナブロックをチェック
			nblocklen = CheckJisHankataPart(reinterpret_cast<const char *>(pr), pr_end - pr,
											reinterpret_cast<const char **>(&pr_next), &next_esctype, NULL);
			break;
		case MYJISESC_ZENKAKU:
			// 全角ブロックをチェック
			nblocklen = CheckJisZenkakuPart(reinterpret_cast<const char *>(pr), pr_end - pr,
											reinterpret_cast<const char **>(&pr_next), &next_esctype, NULL);
			break;
		default: // MYJISESC_UNKNOWN:
			// 不明なエスケープシーケンスから始まるブロックをチェック
			nblocklen = CheckJisUnknownPart(reinterpret_cast<const char *>(pr), pr_end - pr,
											reinterpret_cast<const char **>(&pr_next), &next_esctype, NULL);
		}
		// 変換実行
...
EConvertResult CJis::UnicodeToHex(const wchar_t *cSrc, const int iSLen, WCHAR *pDst,
