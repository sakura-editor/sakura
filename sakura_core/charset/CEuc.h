...
public:
	//実装
	static EConvertResult EUCToUnicode(const CMemory &cSrc,
									   CNativeW *pDstMem); // EUC       → Unicodeコード変換  //2007.08.13 kobake 追加
	static EConvertResult UnicodeToEUC(const CNativeW &cSrc, CMemory *pDstMem); // Unicode   → EUCコード変換

public:
	// 実装
	// 2008.11.10 変換ロジックを書き直す
	inline static int _EucjpToUni_char(const unsigned char *pSrc, unsigned short *pDst, const ECharSet eCharset,
									   bool *pbError, bool *pbHex);

protected:
	static int		  EucjpToUni(const char *pSrc, const int nSrcLen, wchar_t *pDst, bool *pbError);
	inline static int _UniToEucjp_char(const unsigned short *pSrc, unsigned char *pDst, const ECharSet eCharset,
									   bool *pbError);
...
/*!
	...
	高速化のため、インライン化
*/
inline int CEuc::_EucjpToUni_char(const unsigned char *pSrc, unsigned short *pDst, const ECharSet eCharset,
								  bool *pbError, bool *pbHex = NULL)
{
	int			  nret;
...
/*
	...
	高速化のため、インライン化
*/
inline int CEuc::_UniToEucjp_char(const unsigned short *pSrc, unsigned char *pDst, const ECharSet eCharset,
								  bool *pbError)
{
	int			  nret = 0, nclen;
