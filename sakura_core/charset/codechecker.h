...
inline bool IsUnicodeNoncharacter(const wchar32_t wc)
{
	...
	if (wc_ == 0xfffe || wc_ == 0xffff) { return true; }
	if (wc >= 0xfdd0 && wc <= 0xfdef) { return true; }
	return false;
}
...
//! SJIS 全角文字 1 バイト目か
inline bool IsSjisZen1(const char c) { return CHARCODE__IS_SJIS_ZEN1(static_cast<unsigned char>(c)); }
//! SJIS 全角文字 2 バイト目か
inline bool IsSjisZen2(const char c) { return CHARCODE__IS_SJIS_ZEN2(static_cast<unsigned char>(c)); }
//! SJIS 全角文字か
...
inline bool _IS_SJIS_1(unsigned char c) { return IsSjisZen1(static_cast<char>(c)); }
...
//! EUCJP 全角文字 1 バイト目か
inline bool IsEucjpZen1(const char c) { return CHARCODE__IS_EUCJP_ZEN1(static_cast<unsigned char>(c)); }
//! EUCJP 全角文字 2 バイト目か
inline bool IsEucjpZen2(const char c) { return CHARCODE__IS_EUCJP_ZEN2(static_cast<unsigned char>(c)); }
//! EUCJP 全角文字か
...
inline bool IsEucZen_hirakata(const char *pC)
{
	...
	return ((c0 == 0xa4 && (c1 >= 0xa1 && c1 <= 0xf3)) || (c0 == 0xa5 && (c1 >= 0xa1 && c1 <= 0xf6)));
}
//! EUCJP 全角文字　補助漢字か
inline bool IsEucjpSupplemtal(const char *pC)
{
	return (static_cast<unsigned char>(pC[0]) == 0x8f && CHARCODE__IS_EUCJP_ZEN1(static_cast<unsigned char>(pC[1]))
			&& CHARCODE__IS_EUCJP_ZEN2(static_cast<unsigned char>(pC[2])));
}
//! EUCJP 半角カタカナ文字 2 バイト目か  added by genta
inline bool IsEucjpHankata2(const char c) { return CHARCODE__IS_EUCJP_HANKATA2(static_cast<unsigned char>(c)); }
//! EUCJP 半角カタカナ文字か
inline bool IsEucjpHankata(const char *pC)
{
	return (static_cast<unsigned char>(pC[0]) == 0x8e
			&& CHARCODE__IS_EUCJP_HANKATA2(static_cast<unsigned char>(pC[1])));
}
//! ISO-2022-JP(JIS) か
inline bool IsJis(const char c) { return CHARCODE__IS_JIS(static_cast<unsigned char>(c)); }
...
{
	return (CHARCODE__IS_JIS(static_cast<unsigned char>(pC[0])) && CHARCODE__IS_JIS(static_cast<unsigned char>(pC[1])));
}
...
inline bool IsUtf8SurrogHi(const char *pS)
{
	...
	if ((ps[0] & 0xff) == 0xed && (ps[1] & 0xf0) == 0xa0) { return true; }
	return false;
}
//! UtF-8版 下位サロゲートか
...
{
	const unsigned char *ps = reinterpret_cast<const unsigned char *>(pS);
	if ((ps[0] & 0xff) == 0xed && (ps[1] & 0xf0) == 0xb0) { return true; }
	return false;
}
//! UTF-7 Set D の文字か
...
inline bool IsUTF16High(wchar_t c) { return IsUtf16SurrogHi(c); }
//! 下位サロゲートエリアか？	from ssrc_2004-06-05wchar00703b	2008/5/15 Uchi
inline bool IsUTF16Low(wchar_t c) { return IsUtf16SurrogLow(c); }
...
inline int CheckJisAscii7Part(const char *pS, const int nLen, const char **ppNextChar, EMyJisEscseq *peNextEsc,
							  int *pnErrorCount)
{
	return _CheckJisAnyPart(pS, nLen, ppNextChar, peNextEsc, pnErrorCount, JISCHECK_ASCII7);
}
inline int CheckJisHankataPart(const char *pS, const int nLen, const char **ppNextChar, EMyJisEscseq *peNextEsc,
							   int *pnErrorCount)
{
	return _CheckJisAnyPart(pS, nLen, ppNextChar, peNextEsc, pnErrorCount, JISCHECK_HANKATA);
}
inline int CheckJisZenkakuPart(const char *pS, const int nLen, const char **ppNextChar, EMyJisEscseq *peNextEsc,
							   int *pnErrorCount)
{
	return _CheckJisAnyPart(pS, nLen, ppNextChar, peNextEsc, pnErrorCount, JISCHECK_ZENKAKU);
}
inline int CheckJisUnknownPart(const char *pS, const int nLen, const char **ppNextChar, EMyJisEscseq *peNextEsc,
							   int *pnErrorCount)
{
...
inline int CheckUtf16leChar(const wchar_t *p, const int n, ECharSet *e, const int o)
{
	return _CheckUtf16Char(p, n, e, o, false);
}
inline int CheckUtf16beChar(const wchar_t *p, const int n, ECharSet *e, const int o)
{
	return _CheckUtf16Char(p, n, e, o, true);
}
