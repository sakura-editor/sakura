...
inline bool IsLineDelimiterBasic(wchar_t wc) { return wc == CR || wc == LF; }
...
//!単語の区切り文字であるかどうか
inline bool IsWordDelimiter(wchar_t wc) { return wc == SPACE || wc == TAB || IsZenkakuSpace(wc); }
...
//!空白かどうか
inline bool IsBlank(wchar_t wc) { return wc == TAB || wc == SPACE || IsZenkakuSpace(wc); }
...
inline bool IsValidFilenameChar(const wchar_t wc)
{
	...
	if (wcschr(table, wc) != NULL)
		return false; // table内の文字が含まれていたら、ダメ。
	else
		return true;
}
...
inline bool IsZenkakuKigou(wchar_t c)
{
	...
	// 2009.06.26 syat
	// 「ゝゞ（ひらがな）」「ヽヾ（カタカナ）」「゛゜（全角濁点）」「仝々〇（漢字）」「ー（長音）」を除外 2009.10.10
	// syat ANSI版の修正にあわせて「〆」を記号→漢字にする
...
//! ギリシャ文字かどうか
inline bool IsGreek(wchar_t c) { return c >= 0x0391 && c <= 0x03C9; }
...
//! BOX DRAWING 文字 かどうか
inline bool IsBoxDrawing(wchar_t c) { return c >= 0x2500 && c <= 0x257F; }
