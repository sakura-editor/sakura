...
{
	if (codepageEx == CODE_CPACP) { return CP_ACP; }
...
*/
EConvertResult CCodePage::CPToUni(const char *pSrc, const int nSrcLen, wchar_t *pDst, int nDstCchLen, int &nRetLen,
								  UINT codepage)
{
	if (nSrcLen < 1)
	{
...
EConvertResult CCodePage::CPToUnicode(const CMemory &cSrc, CNativeW *pDst, int codepageEx)
{
	...
	if (pDstBuffer == NULL) { return RESULT_FAILURE; }
...
EConvertResult CCodePage::UniToCP(const wchar_t *pSrc, const int nSrcLen, char *pDst, int nDstByteLen, int &nRetLen,
								  UINT codepage)
{
	if (nSrcLen < 1)
	{
	...
	if (nRetLen <= 0)
	{
		...
		switch (errorCd)
		{
		case ERROR_INSUFFICIENT_BUFFER: DEBUG_TRACE(L"UniToCP::ERROR_INSUFFICIENT_BUFFER\n"); break;
		case ERROR_INVALID_FLAGS: DEBUG_TRACE(L"UniToCP::ERROR_INVALID_FLAGS\n"); break;
		case ERROR_INVALID_PARAMETER:
			DEBUG_TRACE(L"UniToCP::ERROR_INVALID_PARAMETER\n");
			break;
			//		case ERROR_NO_UNICODE_TRANSLATION:
			//			DEBUG_TRACE(L"ERROR_NO_UNICODE_TRANSLATION"); break;
		default: DEBUG_TRACE(L"UniToCP %x\n", errorCd); break;
		}
#endif
		return RESULT_FAILURE;
	}
	if (bDefaultChar) { return RESULT_LOSESOME; }
	return RESULT_COMPLETE;
}
...
EConvertResult CCodePage::UnicodeToCP(const CNativeW &cSrc, CMemory *pDst, int codepageEx)
{
	...
	if (0 == nSrcLen) { return RESULT_COMPLETE; }
	UINT codepage = CodePageExToMSCP(codepageEx);
	...
	if (0 == nBuffSize)
	{
		...
		switch (errorCd)
		{
		case ERROR_INSUFFICIENT_BUFFER: DEBUG_TRACE(L"UnicodeToCP::ERROR_INSUFFICIENT_BUFFER\n"); break;
		case ERROR_INVALID_FLAGS: DEBUG_TRACE(L"UnicodeToCP::ERROR_INVALID_FLAGS\n"); break;
		case ERROR_INVALID_PARAMETER:
			DEBUG_TRACE(L"UnicodeToCP::ERROR_INVALID_PARAMETER\n");
			break;
			//		case ERROR_NO_UNICODE_TRANSLATION:
			//			DEBUG_TRACE(L"ERROR_NO_UNICODE_TRANSLATION"); break;
		default: DEBUG_TRACE(L"UnicodeToCP::UniToCP %x\n", errorCd); break;
		}
#endif
		return RESULT_FAILURE;
	}
...
void CCodePage::GetEol(CMemory *pcmemEol, EEolType eEolType)
{
	...
	// 双方向変換ができる場合だけ設定
	if (!CNativeW::IsEqual(temp, temp2)) { pcmemEol->Clear(); }
}
...
void CCodePage::GetBom(CMemory *pcmemBom)
{
	...
	if (!CNativeW::IsEqual(temp, temp2)) { pcmemBom->Clear(); }
}

// 文字コード表示用	UNICODE → Hex 変換
EConvertResult CCodePage::UnicodeToHex(const wchar_t *cSrc, const int iSLen, WCHAR *pDst,
...
int CCodePage::GetNameNormal(LPWSTR outName, int charcodeEx)
{
	...
	if (codepage == CP_ACP) { wcscpy(outName, L"CP_ACP"); }
...
int CCodePage::GetNameLong(LPWSTR outName, int charcodeEx)
{
	...
	else
	{
		...
		if (::GetCPInfoEx(codepage, 0, &cpInfo)) { wcscpy(outName, cpInfo.CodePageName); }
		else
		{
			auto_sprintf(outName, L"CP%d", codepage);
		}
...
EEncodingTrait CCodePage::GetEncodingTrait(int charcodeEx)
{
	...
	case CODE_UTF8:
	case CODE_UTF7:
	case CODE_CESU8: return ENCODING_TRAIT_ASCII;
	case CODE_UNICODE: return ENCODING_TRAIT_UTF16LE;
	case CODE_UNICODEBE: return ENCODING_TRAIT_UTF16BE;
	case CODE_CODEMAX:
	case CODE_AUTODETECT:
	case CODE_ERROR: return ENCODING_TRAIT_ERROR;
	}
	...
	switch (codepage)
	{
	case 1200: return ENCODING_TRAIT_UTF16BE;
	case 1201: return ENCODING_TRAIT_UTF16BE;
	case 12000: return ENCODING_TRAIT_UTF32LE;
	case 12001: return ENCODING_TRAIT_UTF32BE;
	}
	CHAR testCrlf[10];
	...
	case 2:
		if (0 == memcmp(testCrlf, "\x0d\x25", 2))
		{
			...
			if (nRetNel && 0 == memcmp(testNel, "\x15", 1)) { return ENCODING_TRAIT_EBCDIC; }
	...
	case 8:
		...
		return ENCODING_TRAIT_ERROR;
	default: return ENCODING_TRAIT_ERROR;
	}
}
...
CCodePage::CodePageList &CCodePage::GetCodePageList()
{
	...
	{
		cpInfo.CodePageName[0] = L'\0';
		if (::GetCPInfoEx(it->first, 0, &cpInfo)) { it->second = cpInfo.CodePageName; }
		else
		{
...
{
	if (codepage == 12000) { return S_UTF32LEToUnicode(pSrc, nSrcLen, pDst, nDstLen); }
...
{
	if (codepage == 12000) { return S_UnicodeToUTF32LE(pSrc, nSrcLen, pDst, nDstLen); }
	else if (codepage == 12001)
	{
		...
		if (i + 3 < nSrcLen)
		{
			if (pSrcByte[i + 3] == 0x00 && pSrcByte[i + 2] == 0x00)
			{
				...
				if (IsUtf16SurrogHi(c) || IsUtf16SurrogLow(c))
				{
					...
					{ BinToText(pSrcByte + i, nDstUseCharLen, pDstShort + nDstUseLen); } else
					{
						return 0;
					}
				}
				...
				{
					nDstUseCharLen = 1;
					if (nDstUseLen + nDstUseCharLen <= nDstLen) { pDst[nDstUseLen] = c; }
					else
					{
						return 0;
					}
...
static bool BinToUTF32(const unsigned short *pSrc, int Len, char *pDst, int nDstLen)
{
	if (4 <= nDstLen)
	{
		for (int i = 0; i < Len; i++) { pDst[i] = CCodeBase::TextToBin(pSrc[i]); }
		for (int k = Len; k < 4; k++) { pDst[k] = 0; }
	}
	else
	{
		return false;
...
int CCodePage::S_UnicodeToUTF32LE(const wchar_t *pSrc, int nSrcLen, char *pDst, int nDstLen)
{
	...
	if (nBinaryLen != 0)
	{
		...
		{
			for (int k = 0; k < nBinaryLen; k++) { pDstByte[nDstUseLen + k] = TextToBin(pSrcShort[nBinaryPos]); }
		}
		else
		{
			return 0;
