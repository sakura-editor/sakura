...
void CESI::GetEncodingInfo_sjis(const char *pS, const int nLen)
{
    ...
    for (; 0 != (nret = CheckSjisChar(pr, pr_end - pr, &echarset)); pr += nret)
    {
        ...
        {
            if (pr_end - pr < GuessSjisCharsz(pr[0]))
            {
                break;
            }
            nillbytes += nret;
...
void CESI::GetEncodingInfo_eucjp(const char *pS, const int nLen)
{
    ...
    for (; 0 != (nret = CheckEucjpChar(pr, pr_end - pr, &echarset)); pr += nret)
    {
        ...
        {
            if (pr_end - pr < GuessEucjpCharsz(pr[0]))
            {
                break;
            }
            nillbytes += nret;
...
void CESI::GetEncodingInfo_utf8(const char *pS, const int nLen)
{
    ...
    for (; 0 != (nret = CheckUtf8Char(pr, pr_end - pr, &echarset, true, 0)); pr += nret)
    {
        ...
        {
            if (pr_end - pr < GuessUtf8Charsz(pr[0]))
            {
                break;
            }
            nillbytes += nret;
...
void CESI::GetEncodingInfo_cesu8(const char *pS, const int nLen)
{
    ...
    for (; 0 != (nret = CheckCesu8Char(pr, pr_end - pr, &echarset, 0)); pr += nret)
    {
        ...
        {
            if (pr_end - pr < GuessCesu8Charsz(pr[0]))
            {
                break;
            }
            nillbytes += nret;
        ...
        if (nret1 != 0)
        {
            if (echarset1 != CHARSET_BINARY)
            {
                if (nret1 == 1)
                {
                    ...
                    if (_CheckUtf16EolLE(pr1, pr_end - pr1))
                    {
                        nnewlinew1++;
                    }
                }
        ...
        if (nret2 != 0)
        {
            if (echarset2 != CHARSET_BINARY)
            {
                if (nret2 == 1)
                {
                    ...
                    if (_CheckUtf16EolBE(pr2, pr_end - pr2))
                    {
                        nnewlinew2++;
                    }
                }
        ...
        for (i = 5; i < nSize - 12; i++)
        {
            ...
            if (IsXMLWhiteSpace(pBuf[i]) && 0 == memcmp(pBuf + i + 1, "encoding", 8))
            {
                ...
                for (k = 0; encodingNameToCode[k].name != NULL; k++)
                {
                    ...
                    if (i + nLen < nSize - 1 && pBuf[i + nLen] == quoteChar && 0 == _memicmp(encodingNameToCode[k].name, pBuf + i, nLen))
                    {
        ...
        if (pBuf[i] == '<' && 0 == _memicmp(&pBuf[i + 1], "meta", 4) && IsXMLWhiteSpace(pBuf[i + 5]))
        {
            ...
            while (i < nSize)
            {
                if (IsXMLWhiteSpace(pBuf[i]) && i + 1 < nSize && !IsXMLWhiteSpace(pBuf[i + 1]))
                {
                    int nAttType = 0;
                    i++;
                    if (i + 12 < nSize && 0 == _memicmp(pBuf + i, "http-equiv", 10) && (IsXMLWhiteSpace(pBuf[i + 10]) || '=' == pBuf[i + 10]))
                    {
                        i += 10;
                        nAttType = 1; // http-equiv
                    }
                    else if (i + 9 < nSize && 0 == _memicmp(pBuf + i, "content", 7) && (IsXMLWhiteSpace(pBuf[i + 7]) || '=' == pBuf[i + 7]))
                    {
                        i += 7;
                        nAttType = 2; // content
                    }
                    else if (i + 9 < nSize && 0 == _memicmp(pBuf + i, "charset", 7) && (IsXMLWhiteSpace(pBuf[i + 7]) || '=' == pBuf[i + 7]))
                    {
                        i += 7;
                        nAttType = 3; // charset
                    }
                    ...
                    {
                        // その他の属性名読み飛ばし
                        while (i < nSize && !IsXMLWhiteSpace(pBuf[i]))
                        {
                            i++;
                        }
                    }
                    ...
                    while (IsXMLWhiteSpace(pBuf[i]) && i < nSize)
                    {
                        i++;
                    }
                    if (nSize <= i)
                    ...
                    if ('\'' == pBuf[i] || '"' == pBuf[i])
                    {
                        ...
                        while (i < nSize && quoteChar != pBuf[i] && '<' != pBuf[i] && '>' != pBuf[i])
                        {
                            i++;
                        }
                        nEndAttVal = i;
                    ...
                    {
                        while (i < nSize && !IsXMLWhiteSpace(pBuf[i]) && '<' != pBuf[i] && '>' != pBuf[i])
                        {
                            i++;
                        }
                        nEndAttVal = i;
                    ...
                    else if (2 == nAttType)
                    {
                        ...
                        i++; // Skip ';'
                        while (i < nEndAttVal && IsXMLWhiteSpace(pBuf[i]))
                        {
                            i++;
                        }
                        if (nEndAttVal <= i)
                        ...
                        if (i + 7 < nEndAttVal && 0 == _memicmp(pBuf + i, "charset", 7))
                        {
                            ...
                            {
                                const int nLen = encodingNameToCode[k].nLen;
                                if (i - nCharsetBegin == nLen && 0 == _memicmp(encodingNameToCode[k].name, pBuf + nCharsetBegin, nLen))
                                {
                                    if (bContentType)
                                    {
                                        return static_cast<ECodeType>(encodingNameToCode[k].nCode);
                    ...
                    else if (3 == nAttType)
                    {
                        ...
                        {
                            const int nLen = encodingNameToCode[k].nLen;
                            if (nEndAttVal - nBeginAttVal == nLen && 0 == _memicmp(encodingNameToCode[k].name, pBuf + nBeginAttVal, nLen))
                            {
                                return static_cast<ECodeType>(encodingNameToCode[k].nCode);
                            }
                        }
...
void CESI::GetDebugInfo(const char *pS, const int nLen, CNativeW *pcmtxtOut)
{
    ...
    for (i = 0; i < NUM_OF_MBCODE; ++i)
    {
        ...
        auto_sprintf(szWork, LS(STR_ESI_OTHER_B_AND_P), // "\t\t%d.%s\t固有バイト数 %d\tポイント数 %d\r\n"
                     i + 1,
                     CCodeTypeName(cesi.m_apMbcInfo[i]->eCodeID).Normal(),
                     v1,
                     v2);
        pcmtxtOut->AppendString(szWork);
    }
