...
public:
	//CCodeBaseインターフェース
	EConvertResult CodeToUnicode(const CMemory* pSrc, CNativeW2* pDst){ *pDst->_GetMemory()=*pSrc; return SJISToUnicode(pDst->_GetMemory()); }	//!< 特定コード → UNICODE    変換
	EConvertResult UnicodeToCode(const CNativeW2* pSrc, CMemory* pDst){ *pDst=*pSrc->_GetMemory(); return UnicodeToSJIS(pDst); }	//!< UNICODE    → 特定コード 変換
