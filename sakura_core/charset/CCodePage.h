...
public:
	...
	{
		return CPToUnicode(cSrc, pDst, m_nCodePageEx);
	} //!< 特定コード → UNICODE    変換
	EConvertResult UnicodeToCode(const CNativeW &cSrc, CMemory *pDst) override
	{
		return UnicodeToCP(cSrc, pDst, m_nCodePageEx);
	}																	  //!< UNICODE    → 特定コード 変換
	void		   GetEol(CMemory *pcmemEol, EEolType eEolType) override; //!< 改行データ取得
	void		   GetBom(CMemory *pcmemBom) override;					  //!< BOMデータ取得
	EConvertResult UnicodeToHex(const wchar_t *cSrc, const int iSLen, WCHAR *pDst,
								const CommonSetting_Statusbar *psStatusbar); //!< UNICODE → Hex 変換

public:
	//実装
	static EConvertResult CPToUnicode(const CMemory &cSrc, CNativeW *pDst,
									  int codepageEx); // CodePage  → Unicodeコード変換
	static EConvertResult UnicodeToCP(const CNativeW &cSrc, CMemory *pDst,
									  int codepageEx); // Unicode   → CodePageコード変換
...
protected:
	// 実装
	static EConvertResult CPToUni(const char *pSrc, const int nSrcLen, wchar_t *pDst, int nDstCchLen, int &nRetLen,
								  UINT codepage);
	static EConvertResult UniToCP(const wchar_t *pSrc, const int nSrcLen, char *pDst, int nDstByteLen, int &nRetLen,
								  UINT codepage);
	...
	static int MultiByteToWideChar2(UINT codepage, int flags, const char *pSrc, int nSrcLen, wchar_t *pDst,
									int nDstLen);
	static int WideCharToMultiByte2(UINT codepage, int flags, const wchar_t *pSrc, int nSrcLen, char *pDst,
									int nDstLen);
