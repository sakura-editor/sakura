...
int CShiftJis::SjisToUni(const char *pSrc, const int nSrcLen, wchar_t *pDst, bool *pbError)
{
	...
	for (; (nclen = CheckSjisChar(reinterpret_cast<const char *>(pr), pr_end - pr, &echarset)) != 0; pr += nclen) {
		...
		case CHARSET_JIS_HANKATA:
			// 保護コード
			if (echarset == CHARSET_JIS_ZENKAKU && nclen != 2) { nclen = 2; }
			if (echarset == CHARSET_JIS_HANKATA && nclen != 1) { nclen = 1; }
			// 全角文字または半角カタカナ文字を変換
...
EConvertResult CShiftJis::UnicodeToHex(const wchar_t *cSrc, const int iSLen, WCHAR *pDst,
									   const CommonSetting_Statusbar *psStatusbar)
{
	...
	if (res != RESULT_COMPLETE) { return RESULT_LOSESOME; }
