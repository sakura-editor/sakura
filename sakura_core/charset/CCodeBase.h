...
//	virtual bool IsCode(const CMemory* pMem){return false;}  //!< 特定コードであればtrue
	virtual EConvertResult CodeToUnicode(const CMemory* pSrc, CNativeW2* pDst)=0; //!< 特定コード → UNICODE    変換
	virtual EConvertResult UnicodeToCode(const CNativeW2* pSrc, CMemory* pDst)=0; //!< UNICODE    → 特定コード 変換
};
