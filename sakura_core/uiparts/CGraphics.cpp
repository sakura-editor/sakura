...
{
	//元のクリッピングに戻す
	if (!m_vClippingRgns.empty()) { ::SelectClipRgn(m_hdc, m_vClippingRgns[0]); }
	//領域をすべて削除
	int nSize = (int)m_vClippingRgns.size();
	for (int i = 0; i < nSize; i++) { ::DeleteObject(m_vClippingRgns[i]); }
...
void CGraphics::PushTextForeColor(COLORREF color)
{
	...
	//記録
	if (m_vTextForeColors.empty()) { m_vTextForeColors.push_back(cOld); }
...
void CGraphics::PushTextBackColor(COLORREF color)
{
	...
	//記録
	if (m_vTextBackColors.empty()) { m_vTextBackColors.push_back(cOld); }
...
void CGraphics::PopPen()
{
	...
	//選択
	if (hpnNew) { SelectObject(m_hdc, hpnNew); }
	...
	//オリジナル
	if (m_vPens.empty()) { m_hpnOrg = NULL; }
}
...
void CGraphics::ClearPen()
{
	...
	for (int i = 0; i < nSize; i++) { DeleteObject(m_vPens[i]); }
	m_vPens.clear();
}
...
{
	//元のブラシに戻す
	if (!m_vBrushes.empty()) { ::SelectObject(m_hdc, m_vBrushes[0]); }
	//ブラシをすべて削除 (0番要素以外)
	int nSize = (int)m_vBrushes.size();
	for (int i = 1; i < nSize; i++) { ::DeleteObject(m_vBrushes[i]); }
...
void CGraphics::DrawDropRect(LPCRECT lpRectNew, SIZE sizeNew, LPCRECT lpRectLast, SIZE sizeLast)
{
	...
	if (lpRectNew) { hRgnNew = CreateDropRectRgn(lpRectNew, sizeNew); }
	if (lpRectLast) {
