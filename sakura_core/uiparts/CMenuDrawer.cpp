...
)
{
    ...
    mii.fType        = 0;
    if (MF_OWNERDRAW & (nFlag | nFlagAdd))
        mii.fType |= MFT_OWNERDRAW;
    if (MF_SEPARATOR & (nFlag | nFlagAdd))
        mii.fType |= MFT_SEPARATOR;
    if (MF_STRING & (nFlag | nFlagAdd))
        mii.fType |= MFT_STRING;
    if (MF_MENUBREAK & (nFlag | nFlagAdd))
        mii.fType |= MFT_MENUBREAK;
    if (MF_MENUBARBREAK & (nFlag | nFlagAdd))
        mii.fType |= MFT_MENUBARBREAK;

    mii.fState = 0;
    if (MF_GRAYED & (nFlag | nFlagAdd))
        mii.fState |= MFS_GRAYED;
    if (MF_CHECKED & (nFlag | nFlagAdd))
...
void CMenuDrawer::DrawItem(DRAWITEMSTRUCT *lpdis)
{
    ...
    else if (bMenuIconDraw)
    {
        ...
        // 明度らしきもの
        if (64 < t_abs(t_max(t_max(GetRValue(colFace), GetGValue(colFace)), GetBValue(colFace)) - t_max(t_max(GetRValue(colMenu), GetGValue(colMenu)), GetBValue(colMenu))))
        {
            colIconBack = colFace;
        }
        else
    ...
    if (lpdis->itemID == F_0)
    {
        ...
        if (bBackSurface)
        {
            ::BitBlt(hdcOrg, lpdis->rcItem.left, lpdis->rcItem.top, nTargetWidth, nTargetHeight, hdc, lpdis->rcItem.left, lpdis->rcItem.top, SRCCOPY);
        }
        return; // セパレータ。作画終了
    }
    ...
    if (0 != ::GetMenuItemInfo((HMENU)lpdis->hwndItem, lpdis->itemID, FALSE, &mii) && NULL == mii.hSubMenu && 0 == ::FuncID_To_HelpContextID((EFunctionCode)lpdis->itemID) /* 機能IDに対応するメニューコンテキスト番号を返す */
    )
    {
        //@@@ 2001.12.21 YAZAKI
        if (lpdis->itemState & ODS_SELECTED)
...
int CMenuDrawer::FindIndexFromCommandId(int idCommand, bool bOnlyFunc) const
{
    if (bOnlyFunc)
    {
        ...
        if (!(F_MENU_FIRST <= idCommand && idCommand < F_MENU_NOT_USED_FIRST) && !(F_PLUGCOMMAND_FIRST <= idCommand && idCommand < F_PLUGCOMMAND_LAST))
        {
            return -1;
        }
    }
