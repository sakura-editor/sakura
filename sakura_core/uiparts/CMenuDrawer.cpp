...
CMenuDrawer::CMenuDrawer()
{
	...
	*/
	...
	//
	//	Sept. 16, 2000 JEPRO できるだけ系ごとに集まるように順番を大幅に入れ替えた
	//それに伴いCShareData.cppで設定している初期設定値も変更 	Oct. 22, 2000 JEPRO
	//アイコンのビットマップリソースの2次元配置が可能になったため根本的に配置転換した
	//	・配置の基本は「コマンド一覧」に入っている機能(コマンド)順
	//なお「コマンド一覧」自体は「メニューバー」の順におおよそ準拠している
	...
	static const int tbd[] = {
		...
		/* 64 */ F_FILE_REOPEN_LATIN1 /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */, // Latin1で開きなおす	// 2010/3/20
																					 // Uchi
		...
		/* 79 */ F_HalfPageDown /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */, //半ページダウン	//Oct. 6, 2000 JEPRO
																			   //名称をPC-AT互換機系に変更(ROLL→PAGE)
																			   ////Oct. 10, 2000 JEPRO 名称変更
		/* 80 */ F_1PageUp /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		, //１ページアップ	//Oct. 10, 2000 JEPRO 従来のページアップを半ページアップと名称変更し１ページアップを追加
		/* 81 */ F_1PageDown /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		, //１ページダウン	//Oct. 10, 2000 JEPRO 従来のページダウンを半ページダウンと名称変更し１ページダウンを追加
		/* 82 */ F_OPEN_COMMAND_PROMPT_AS_ADMIN /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		,										//管理者としてコマンドプロンプトを開く
		/* 83 */ F_OPEN_POWERSHELL_AS_ADMIN		/* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		,										//管理者としてPowerShellを開く
		...
		/* 114 */ F_DISABLE /*F_DISPLAYTOP_SEL*/ /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		,										 //(範囲選択)画面の先頭に移動(未実装)
		/* 115 */ F_DISABLE /*F_DISPLAYEND_SEL*/ /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		,										 //(範囲選択)画面の最後に移動(未実装)
		...
		,										 //(矩形選択)画面の先頭に移動(未実装)
		/* 147 */ F_DISABLE /*F_DISPLAYEND_BOX*/ /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		,										 //(矩形選択)画面の最後に移動(未実装)
		...
		/* クリップボード系(6段目24個: 161-184) */
		/* 161 */ F_CUT /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		...
		/* 166 */ F_DISABLE /*F_INSTEXT_W*/ /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */, //テキストを貼り付け
																						   //(未公開コマンド？未完成？)
		...
		/* 177 */ F_COPY_COLOR_HTML_LINENUMBER /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		,									   //選択範囲内行番号色付きHTMLコピー
		/* 178 */ F_COPYDIRPATH				   /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		, //このファイルのフォルダ名をクリップボードにコピー
		...
		/* 211 */ F_BASE64DECODE /* , TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */, // Base64デコードして保存	//Sept. 28,
																				// 2000 JEPRO 追加
		...
		/* 321 */ F_SPLIT_V /*, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */, //上下に分割	//Sept. 16, 2000 jepro
																		  //説明を「縦」から「上下に」に変更
		/* 322 */ F_SPLIT_H /*, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */, //左右に分割	//Sept. 16, 2000 jepro
																		  //説明を「横」から「左右に」に変更
		/* 323 */ F_SPLIT_VH /*, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */, //縦横に分割	//Sept. 17, 2000 jepro
																		   //説明に「に」を追加
		/* 324 */ F_WINCLOSE /*, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */, //ウィンドウを閉じる
		/* 325 */ F_WIN_CLOSEALL										   /*, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		, //すべてのウィンドウを閉じる	//Sept. 18, 2000 JEPRO 追加	//Oct. 17, 2000 JEPRO
		...
		/* 384 */ F_TOOLBARWRAP /*, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */
		,						//追加マクロ用icon位置兼、折返ツールバーボタンID
		...
		/* 481 */ F_DISABLE /*, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0 */ //最終行用ダミー(Jepro note:
																		 //最終行末にはカンマを付けないこと)
	};
	...
	SetTBBUTTONVal(&m_tbMyButton[0], -1, F_SEPARATOR, 0, TBSTYLE_SEP, 0,
				   0); //セパレータ	// 2007.11.02 ryoji アイコンの未定義化(-1)
	...
	for (int i = INDEX_GAP; i < myButtonEnd; i++)
	{
		...
		if (funcCode == F_TOOLBARWRAP)
		{
			...
			SetTBBUTTONVal(&m_tbMyButton[i],
						   -1, // 2007.11.02 ryoji アイコンの未定義化(-1)
						   F_MENU_NOT_USED_FIRST, TBSTATE_ENABLED | TBSTATE_WRAP, TBSTYLE_SEP, 0, 0);
			continue;
		}
	...
	if (MAX_CUSTMACRO_ICO < MAX_CUSTMACRO)
	{
		...
		for (int k = 0; k < nAddFuncs; k++)
		{
			...
			SetTBBUTTONVal(&m_tbMyButton[k + nBaseIndex], TOOLBAR_ICON_MACRO_INTERNAL - INDEX_GAP, macroFuncCode,
...
void CMenuDrawer::ResetContents(void)
{
	...
	{
		// 最低アイコン分の高さを確保
		if (20 > m_nMenuHeight) { m_nMenuHeight = 20; }
	}
...
/* メニュー項目を追加 */
void CMenuDrawer::MyAppendMenu(HMENU hMenu, int nFlag, UINT_PTR nFuncId, const WCHAR *pszLabel,
...
)
{
	...
	{
		/* メニューラベルの作成 */
		CKeyBind::GetMenuLabel(m_hInstance, m_pShareData->m_Common.m_sKeyBind.m_nKeyNameArrNum,
							   m_pShareData->m_Common.m_sKeyBind.m_pKeyNameArr, nFuncId, szLabel, szKey, bAddKeyStr,
							   _countof(szLabel));
		...
		/* アイコン用ビットマップを持つものは、オーナードロウにする */
		{
			...
			if (m_pShareData->m_Common.m_sWindow.m_bMenuIcon) { nFlagAdd = MF_OWNERDRAW; }
			/* 機能のビットマップの情報を覚えておく */
...
{
	if (nToolbarNo < 0) { return -1; }
	// 固定アクセス分のみ直接番号でアクセスさせる。m_nMyButtonNum は使わない
	if (0 <= nToolbarNo && nToolbarNo < m_nMyButtonFixSize) { return nToolbarNo; }
...
void CMenuDrawer::DrawItem(DRAWITEMSTRUCT *lpdis)
{
	...
	// アイコンとテキストの間の縦線の位置
	const int nIndentLeft = bMenuIconDraw ? cxSmIcon + cxEdge * 6 + cxBorder : cxEdge * 2 + nCxCheck;
...
#ifdef DRAW_MENU_ICON_BACKGROUND_3DFACE
#else
				&& -1 != m_menuItems[nItemIndex].m_nBitmapIdx
	...
	if (lpdis->itemID == F_0)
	{
		...
		if (bBackSurface)
		{
			::BitBlt(hdcOrg, lpdis->rcItem.left, lpdis->rcItem.top, nTargetWidth, nTargetHeight, hdc,
					 lpdis->rcItem.left, lpdis->rcItem.top, SRCCOPY);
		}
		return; // セパレータ。作画終了
	}
	...
	{
		if (pszItemStr[j] == L'\t') { break; }
	}
	/* TAB文字の前側のテキストを描画する */
	::DrawText(hdc, pszItemStr, j, &rcText, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
	/* TAB文字の後ろ側のテキストを描画する */
	if (j < nItemStrLen)
	{ ::DrawText(hdc, &pszItemStr[j + 1], nItemStrLen - (j + 1), &rcText, DT_RIGHT | DT_VCENTER | DT_SINGLELINE); }
...
// チェックマークも自分で書く場合
				...
				const int nScale =
					t_max(100, t_min(210, int((lpdis->rcItem.bottom - lpdis->rcItem.top - 2) * 100) / (16 - 2)));
			...
			else
			{
				...
				::BitBlt(hdc, lpdis->rcItem.left + 2, lpdis->rcItem.top + 2, nCxCheck, nCyCheck, hdcMem, 0, 0,
						 SRCPAINT);
	...
	if (bBackSurface)
	{
		::BitBlt(hdcOrg, lpdis->rcItem.left, lpdis->rcItem.top, nTargetWidth, nTargetHeight, hdc, lpdis->rcItem.left,
				 lpdis->rcItem.top, SRCCOPY);
	}
	return;
}
...
*/
void CMenuDrawer::EndDrawMenu() { DeleteCompDC(); }
...
TBBUTTON CMenuDrawer::getButton(int nToolbarNo) const
{
	...
	if (0 <= index && index < m_nMyButtonNum) { return m_tbMyButton[index]; }
...
{
	int i;
	if (-1 == (i = Find(nFuncID))) { return NULL; }
...
LRESULT CMenuDrawer::OnMenuChar(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	for (i = 0; i < ::GetMenuItemCount(hmenu); i++)
	{
		...
		if (0 == ::GetMenuItemInfo(hmenu, i, TRUE, &mii)) { continue; }
		const WCHAR *pszLabel;
		if (NULL == (pszLabel = GetLabel(mii.wID))) { continue; }
		if (chUser == GetAccelCharFromLabel(pszLabel))
		{
			WorkData work;
			work.idx = i;
			work.mii = mii;
			if (/*-1 == nAccelSel ||*/ MFS_HILITE & mii.fState) { nAccelSel = vecAccel.size(); }
			vecAccel.push_back(work);
		}
	}
	//	MYTRACE( L"%d\n", (int)mapAccel.size() );
	if (0 == vecAccel.size()) { return MAKELONG(0, MNC_IGNORE); }
	if (1 == vecAccel.size()) { return MAKELONG(vecAccel[0].idx, MNC_EXECUTE); }
...
/* TBBUTTON構造体にデータをセット */
void CMenuDrawer::SetTBBUTTONVal(TBBUTTON *ptb, int iBitmap, int idCommand, BYTE fsState, BYTE fsStyle,
								 DWORD_PTR dwData, INT_PTR iString) const
{
	/*
typedef struct _TBBUTTON {
	int iBitmap;	// ボタン イメージの 0 から始まるインデックス
...
void CMenuDrawer::AddToolButton(int iBitmap, int iCommand)
{
	...
	if (m_pShareData->m_maxTBNum < m_nMyButtonNum) { m_pShareData->m_maxTBNum = m_nMyButtonNum; }
