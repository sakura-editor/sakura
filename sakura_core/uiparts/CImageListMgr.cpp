...
bool CImageListMgr::Create(HINSTANCE hInstance)
{
    ...
    do
    {
        ...
        { // ローカルファイルの読み込み失敗時はリソースから取得
            ...
            hRscbmp = (HBITMAP)::LoadImage(hInstance, MAKEINTRESOURCE(IDB_MYTOOL), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION /* | LR_LOADMAP3DCOLORS */);
            if (hRscbmp == NULL)
            {
...
int CImageListMgr::Add(const WCHAR *szPath)
{
    ...
    ::TransparentBlt(hdcDst, (imageNo % MAX_X) * cx(), (imageNo / MAX_X) * cy(), cx(), cy(), hdcSrc, 0, 0, nWidth, nHeight, cTransParent);
