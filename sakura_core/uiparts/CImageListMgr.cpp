 ...
 */
static inline HBITMAP LoadMyToolFromModule(HINSTANCE hInstance)
{
	//	リソースからBitmapを読み込む
	HANDLE hRscbmp = ::LoadImageW(hInstance, MAKEINTRESOURCE(IDB_MYTOOL), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
...
CImageListMgr::~CImageListMgr()
{
	...
	if (m_hIconBitmap != NULL) { DeleteObject(m_hIconBitmap); }
}
...
struct MyRGBQUAD : tagRGBQUAD
{
		...
		: tagRGBQUAD()
	{
		...
		for (auto it = a.begin(); it != a.end(); ++it, ++p) { *p = *it; }
	}
	...
	bool operator!=(const RGBQUAD &rhs) const noexcept { return !(*this == rhs); }
		 operator COLORREF(void) const noexcept { return RGB(rgbRed, rgbGreen, rgbBlue); }
};
...
COLORREF FromHLS(const _HlsTuple &hls)
{
	...
	// 色相が無効値（＝白黒）の場合
	if (std::isinf(H)) { return RGB(L * 255, L * 255, L * 255); }
...
void CImageListMgr::MyBitBlt(HDC drawdc, int nXDest, int nYDest, int nWidth, int nHeight, int nXSrc, int nYSrc) const
{
	...
	// 透過色を考慮して転送
	::TransparentBlt(drawdc, nXDest, nYDest, nWidth, nHeight, hdcSrc, nXSrc, nYSrc, cx(), cy(), cTransparent);
...
void CImageListMgr::MyDitherBlt(HDC drawdc, int nXDest, int nYDest, int nWidth, int nHeight, int nXSrc, int nYSrc) const
{
	...
	// 背景を透過させつつ転送
	::TransparentBlt(drawdc, nXDest, nYDest, nWidth, nHeight, hdcWork, 0, 0, nWidth, nHeight, cTransparent);
 ...
 */
bool CImageListMgr::DrawToolIcon(HDC drawdc, LONG x, LONG y, int imageNo, DWORD fStyle, LONG cx, LONG cy) const
{
...
{
	if ((m_nIconCount % MAX_X) == 0) { Extend(); }

	//アイコンを読み込む
	HBITMAP bmpSrc = (HBITMAP)::LoadImage(NULL, szPath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);

	if (bmpSrc == NULL) { return -1; }
...
// ツールイメージをリサイズする
HBITMAP CImageListMgr::ResizeToolIcons(HBITMAP	 bmpSrc,		//!< [in] 変換前Bmpのハンドル
...
void CImageListMgr::Extend(bool bExtend)
{
	...
	//拡張した部分は透過色で塗る
	if (bExtend) { FillSolidRect(hDestDC, 0, curY * cy(), MAX_X * cx(), cy(), m_cTrans); }
