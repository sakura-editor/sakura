...
public:
	...
	virtual ~CGrepEnumFileBase() { ClearItems(); }
	...
	{
		if (!IsExist(pFile ? pFile : w32fd.cFileName)) { return TRUE; }
		return FALSE;
	}

	int GetCount(void) { return (int)m_vpItems.size(); }
	...
	int Enumerates(LPCWSTR lpBaseFolder, VGrepEnumKeys &vecKeys, CGrepEnumOptions &option,
		...
		for (int i = 0; i < (int)vecKeys.size(); i++) {
			...
			if (INVALID_HANDLE_VALUE != handle) {
				do {
					...
					if (option.m_bIgnoreSystem && (w32fd.dwFileAttributes & FILE_ATTRIBUTE_SYSTEM)) { continue; }
