...
#define CTAGS_COMMAND L"ctags.exe"
    ...
    if (!cDlgTagsMake.DoModal(G_AppInstance(), m_pCommanderView->GetHwnd(), 0, szTargetPath))
        return false;
    ...
    if (cDlgTagsMake.m_nTagsOpt & 0x0001)
        wcscat(options, L" -R"); //サブフォルダも対象
    ...
    BOOL bProcessResult = CreateProcess(
        NULL, cmdline, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, cDlgTagsMake.m_szPath, &sui, &pi);
    if (!bProcessResult)
    {
