...
static bool IsFileExists2(const wchar_t *pszFile)
{
	...
	{
		if (!WCODE::IsValidFilenameChar(pszFile[i])) { return false; }
	}
	if (_IS_REL_PATH(pszFile)) { return false; }
	...
	*/
	...
	do
	{
		...
		for (; 0 <= ptXY.y; ptXY.y--)
		{
			...
			else if (3 <= nLineLen && 0 == wmemcmp(pLine, L"■\"", 2))
			{
				...
				{
					if (szFile[0]) { AddLastYenFromDirectoryPath(szPath); }
	...
	if (szJumpToFile[0] == L'\0')
	{
		...
		while (p < p_end)
		{
			...
			for (; p < p_end
				   && (*p == L' ' || *p == L'\t'
					   || WCODE::IsLineDelimiter(*p, GetDllShareData().m_Common.m_sEdit.m_bEnableExtEol));
				 ++p)
				;
			if (p >= p_end) break;
	...
	if (szJumpToFile[0])
	{
		if (m_pCommanderView->TagJumpSub(szJumpToFile, CMyPoint(nJumpToColumn, nJumpToLine), bClose))
		{ //@@@ 2003.04.13
			return true;
		}
	}
...
#define CTAGS_COMMAND L"ctags.exe"
	...
	//継承不能にする
	DuplicateHandle(GetCurrentProcess(), hStdOutRead, GetCurrentProcess(), NULL, 0, FALSE, DUPLICATE_SAME_ACCESS);
	...
	//コマンドライン実行
	BOOL bProcessResult =
		CreateProcess(NULL, cmdline, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, cDlgTagsMake.m_szPath, &sui, &pi);
	if (!bProcessResult)
	{
...

	{
		...
		do
		{
			...
			case WAIT_OBJECT_0 + 1:
				//処理中のユーザー操作を可能にする
				if (!::BlockingHook(cDlgCancel.GetHwnd())) { break; }
				break;
			default: break;
			}
			...
			if (PeekNamedPipe(hStdOutRead, NULL, 0, NULL, &new_cnt, NULL)) //パイプの中の読み出し待機中の文字数を取得
			{
				if (new_cnt > 0) //待機中のものがある
				{
					...
					else
					{
						...
						WarningMessage(m_pCommanderView->GetHwnd(), LS(STR_ERR_CEDITVIEW_CMD06),
									   work); // 2003.11.09 じゅうじ
...
bool CViewCommander::Command_TagJumpByTagsFileMsg(bool bMsg)
{
	...
	if (false == ret && bMsg) { m_pCommanderView->SendStatusMessage(LS(STR_ERR_TAGJMP1)); }
	return ret;
}
...
bool CViewCommander::Command_TagJumpByTagsFile(bool bClose)
{
	...
	if (0 == cmemKeyW.GetStringLength()) { return false; }

	WCHAR szDirFile[1024];
	if (false == Sub_PreProcTagJumpByTagsFile(szDirFile, _countof(szDirFile))) { return false; }
	...
	if (0 < nMatchAll)
	{
		...
		{ return false; } return m_pCommanderView->TagJumpSub(fileName, CMyPoint(0, fileLine), bClose);
	}
...
bool CViewCommander::Command_TagJumpByTagsFileKeyword(const wchar_t *keyword)
{
	...
	if (false == Sub_PreProcTagJumpByTagsFile(szCurrentPath, _countof(szCurrentPath))) { return false; }
	...
	//タグジャンプする。
	if (false == cDlgTagJumpList.GetSelectedFullPathAndLine(fileName, _countof(fileName), &fileLine, NULL))
	{ return false; } return m_pCommanderView->TagJumpSub(fileName, CMyPoint(0, fileLine));
}
...
bool CViewCommander::Sub_PreProcTagJumpByTagsFile(WCHAR *szCurrentPath, int count)
{
	...
	// 基準ファイル名の設定
	if (GetDocument()->m_cDocFile.GetFilePathClass().IsValidPath())
	{ wcscpy(szCurrentPath, GetDocument()->m_cDocFile.GetFilePath()); } else
	{
