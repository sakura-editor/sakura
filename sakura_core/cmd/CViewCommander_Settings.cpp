...
void CViewCommander::Command_SELECT_COUNT_MODE(int nMode)
{
    ...
    if (nMode == SELECT_COUNT_TOGGLE)
    {
        ...
        {
            nCurrentMode = (GetDllShareData().m_Common.m_sStatusbar.m_bDispSelCountByByte ? SELECT_COUNT_BY_BYTE : SELECT_COUNT_BY_CHAR);
        }
        else
        {
...
void CViewCommander::Command_SET_QUOTESTRING(const wchar_t *quotestr)
{
    ...
    wcsncpy(GetDllShareData().m_Common.m_sFormat.m_szInyouKigou, quotestr, _countof(GetDllShareData().m_Common.m_sFormat.m_szInyouKigou));
