...
void CViewCommander::Command_SHOWTOOLBAR(void)
{
	...
	GetDllShareData().m_Common.m_sWindow.m_bDispTOOLBAR =
		((NULL == pCEditWnd->m_cToolbar.GetToolbarHwnd()) ? TRUE : FALSE); /* ツールバー表示 */
	pCEditWnd->LayoutToolBar();
	pCEditWnd->EndLayoutBars();

	//全ウインドウに変更を通知する。
	CAppNodeGroupHandle(0).PostMessageToAllEditors(MYWM_BAR_CHANGE_NOTIFY, (WPARAM)MYBCN_TOOLBAR,
												   (LPARAM)pCEditWnd->GetHwnd(), pCEditWnd->GetHwnd());
}
...
void CViewCommander::Command_SHOWFUNCKEY(void)
{
	...
	GetDllShareData().m_Common.m_sWindow.m_bDispFUNCKEYWND =
		((NULL == pCEditWnd->m_cFuncKeyWnd.GetHwnd()) ? TRUE : FALSE); /* ファンクションキー表示 */
	pCEditWnd->LayoutFuncKey();
	pCEditWnd->EndLayoutBars();

	//全ウインドウに変更を通知する。
	CAppNodeGroupHandle(0).PostMessageToAllEditors(MYWM_BAR_CHANGE_NOTIFY, (WPARAM)MYBCN_FUNCKEY,
												   (LPARAM)pCEditWnd->GetHwnd(), pCEditWnd->GetHwnd());
}
...
void CViewCommander::Command_SHOWTAB(void)
{
	...
	GetDllShareData().m_Common.m_sTabBar.m_bDispTabWnd =
		((NULL == pCEditWnd->m_cTabWnd.GetHwnd()) ? TRUE : FALSE); /* タブバー表示 */
	...
	CAppNodeGroupHandle(0).PostMessageToAllEditors(MYWM_BAR_CHANGE_NOTIFY, (WPARAM)MYBCN_TAB,
												   (LPARAM)pCEditWnd->GetHwnd(), pCEditWnd->GetHwnd());
}
...
void CViewCommander::Command_SHOWSTATUSBAR(void)
{
	...
	GetDllShareData().m_Common.m_sWindow.m_bDispSTATUSBAR =
		((NULL == pCEditWnd->m_cStatusBar.GetStatusHwnd()) ? TRUE : FALSE); /* ステータスバー表示 */
	pCEditWnd->LayoutStatusBar();
	pCEditWnd->EndLayoutBars();

	//全ウインドウに変更を通知する。
	CAppNodeGroupHandle(0).PostMessageToAllEditors(MYWM_BAR_CHANGE_NOTIFY, (WPARAM)MYBCN_STATUSBAR,
												   (LPARAM)pCEditWnd->GetHwnd(), pCEditWnd->GetHwnd());
}
...
void CViewCommander::Command_SHOWMINIMAP(void)
{
	...
	//全ウインドウに変更を通知する。
	CAppNodeGroupHandle(0).PostMessageToAllEditors(MYWM_BAR_CHANGE_NOTIFY, (WPARAM)MYBCN_MINIMAP,
												   (LPARAM)pCEditWnd->GetHwnd(), pCEditWnd->GetHwnd());
}
...
#define USE_UNFIXED_FONT
	...
	if (MySelectFont(&lf, &nPointSize, CEditWnd::getInstance()->m_cSplitterWnd.GetHwnd(), bFixedFont))
	{
		...
		CAppNodeGroupHandle(0).PostMessageToAllEditors(MYWM_CHANGESETTING, (WPARAM)0, (LPARAM)PM_CHANGESETTING_FONT,
													   hwndFrame);
...
void CViewCommander::Command_SETFONTSIZE(int fontSize, int shift, int mode)
{
	...
	static const INT sizeTable[] = {8 * 10,	 9 * 10,  10 * 10, (INT)(10.5 * 10), 11 * 10, 12 * 10,
									14 * 10, 16 * 10, 18 * 10, 20 * 10,			 22 * 10, 24 * 10,
									26 * 10, 28 * 10, 36 * 10, 48 * 10,			 72 * 10};
	const LOGFONT &lf = (mode == 0 ? GetDllShareData().m_Common.m_sView.m_lf : GetEditWindow()->GetLogfont(mode == 2));
	INT			   nPointSize;

	// TrueTypeのみ対応
	if (OUT_STROKE_PRECIS != lf.lfOutPrecision && OUT_PS_ONLY_PRECIS != lf.lfOutPrecision) { return; }
	...
	else if (0 != shift)
	{
		...
		for (i = 0; i < _countof(sizeTable); i++)
		{
			if (nPointSize <= sizeTable[i])
			{
				...
				// フォントサイズが変わらないので終了
				if (nPointSize == nNewPointSize) { return; }
...
void CViewCommander::Command_WRAPWINDOWWIDTH(void) //	Oct. 7, 2000 JEPRO WRAPWINDIWWIDTH を WRAPWINDOWWIDTH に変更
{
	...
	GetEditWindow()->ChangeLayoutParam(true, GetDocument()->m_cLayoutMgr.GetTabSpaceKetas(),
									   GetDocument()->m_cLayoutMgr.m_tsvInfo.m_nTsvMode, newKetas);
...
void CViewCommander::Command_Favorite(void)
{
	...
	//ダイアログを表示する
	if (!cDlgFavorite.DoModal(G_AppInstance(), m_pCommanderView->GetHwnd(), (LPARAM)GetDocument())) { return; }
...
void CViewCommander::Command_TEXTWRAPMETHOD(int nWrapMethod)
{
	...
	// 現在の設定値と同じなら何もしない
	if (pcDoc->m_nTextWrapMethodCur == nWrapMethod) return;
	...
	// 折り返し位置を変更
	GetEditWindow()->ChangeLayoutParam(false, pcDoc->m_cLayoutMgr.GetTabSpaceKetas(),
...
void CViewCommander::Command_SET_QUOTESTRING(const wchar_t *quotestr)
{
	...
	GetDllShareData()
		.m_Common.m_sFormat.m_szInyouKigou[_countof(GetDllShareData().m_Common.m_sFormat.m_szInyouKigou) - 1] = L'\0';
}
