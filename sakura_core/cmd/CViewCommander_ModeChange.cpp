...
)
{
    if (eCharSet == CODE_NONE || eCharSet == CODE_AUTODETECT)
    {
        ...
        int nRet = GetEditWindow()->m_cDlgSetCharSet.DoModal(G_AppInstance(), m_pCommanderView->GetHwnd(), &eCharSet, &bBom);
        if (!nRet)
        {
            return;
        }
...
void CViewCommander::Command_CANCEL_MODE(int whereCursorIs)
{
    ...
    else
    {
        ...
        if (m_pCommanderView->GetSelectionInfo().IsTextSelecting() || m_pCommanderView->GetSelectionInfo().IsBoxSelecting())
        {
            m_pCommanderView->GetSelectionInfo().DisableSelectArea(true);
