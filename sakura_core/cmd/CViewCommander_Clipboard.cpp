...
void CViewCommander::Command_PASTE(int option)
{
    ...
    bool bLineSelectOption =
        ((option & 0x04) == 0x04) ? true : ((option & 0x08) == 0x08) ? false : GetDllShareData().m_Common.m_sEdit.m_bEnableLineModePaste;
    ...
    bool bConvertEol =
        ((option & 0x01) == 0x01) ? true : ((option & 0x02) == 0x02) ? false : GetDllShareData().m_Common.m_sEdit.m_bConvertEOLPaste;

    bool bAutoColumnPaste =
        ((option & 0x10) == 0x10) ? true : ((option & 0x20) == 0x20) ? false : GetDllShareData().m_Common.m_sEdit.m_bAutoColumnPaste != FALSE;
...
static bool AppendHTMLColor(
    const SColorAttr &sColorAttrLast, SColorAttr &sColorAttrLast2, const SFontAttr &sFontAttrLast, SFontAttr &sFontAttrLast2, const WCHAR *pAppendStr, int nLen, CNativeW &cmemClip)
{
    if (sFontAttrLast.m_bBoldFont != sFontAttrLast2.m_bBoldFont || sFontAttrLast.m_bUnderLine != sFontAttrLast2.m_bUnderLine || sColorAttrLast.m_cTEXT != sColorAttrLast2.m_cTEXT || sColorAttrLast.m_cBACK != sColorAttrLast2.m_cBACK)
    {
...
void CViewCommander::Command_COPY_COLOR_HTML(bool bLineNumber)
{
    ...
    bool bLineNumLayout     = GetDllShareData().m_Common.m_sEdit.m_bAddCRLFWhenCopy || m_pCommanderView->GetSelectionInfo().IsBoxSelecting();
    CLayoutRect rcSel;
    TwoPointToRect(
        &rcSel,
        ...
        for (; pcLayout->GetLogicLineNo() == nLineNum; nLayoutLineNum++, pcLayout = pcLayout->GetNextLayout())
        {
            ...
            for (; iLogic < nLineStart + nLineLen; ++iLogic)
            {
                ...
                if (nIdxFrom != -1 && nIdxFrom + nLineStart <= iLogic && iLogic <= nIdxTo + nLineStart)
                {
                    ...
                    else if (nIdxFrom + nLineStart < iLogic && (sFontAttr.m_bBoldFont != sFontAttrNext.m_bBoldFont || sFontAttr.m_bUnderLine != sFontAttrNext.m_bUnderLine || sColorAttr.m_cTEXT != sColorAttrNext.m_cTEXT || sColorAttr.m_cBACK != sColorAttrNext.m_cBACK))
                    {
                        bAddCRLF       = AppendHTMLColor(sColorAttrLast, sColorAttrLast2, sFontAttrLast, sFontAttrLast2, pLine + nBgnLogic, iLogic - nBgnLogic, cmemClip);
                        sColorAttrLast = sColorAttrNext;
                        sFontAttrLast  = sFontAttrNext;
                        nBgnLogic      = iLogic;
                    }
