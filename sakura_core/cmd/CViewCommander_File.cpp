...
void CViewCommander::Command_FILENEW(void)
{
	...
	CControlTray::OpenNewEditor(G_AppInstance(), m_pCommanderView->GetHwnd(), sLoadInfo, NULL, false, curDir.c_str(),
								false);
	return;
}
...
{
	if (!IsValidCodeType(nCharCode) && nCharCode != CODE_AUTODETECT) { nCharCode = CODE_AUTODETECT; }
	//ロード情報
...
{
	CAppNodeGroupHandle(0).SendMessageToAllEditors(WM_COMMAND, MAKELONG(F_FILESAVE_QUIET, 0), 0, NULL);
	return TRUE;
}

/* 閉じて(無題) */ // Oct. 17, 2000 jepro 「ファイルを閉じる」というキャプションを変更
void CViewCommander::Command_FILECLOSE(void) { GetDocument()->m_cDocFileOperation.FileClose(); }
...
BOOL CViewCommander::Command_OPEN_HHPP(BOOL bCheckOnly, BOOL bBeepWhenMiss)
{
	...
	return m_pCommanderView->OPEN_ExtFromtoExt(bCheckOnly, bBeepWhenMiss, source_ext, header_ext, _countof(source_ext),
											   _countof(header_ext), LS(STR_ERR_CEDITVIEW_CMD08));
}
...
BOOL CViewCommander::Command_OPEN_CCPP(BOOL bCheckOnly, BOOL bBeepWhenMiss)
{
	...
	static const WCHAR *source_ext[] = {L"c", L"cpp", L"cxx", L"cc", L"cp", L"c++"};
	static const WCHAR *header_ext[] = {L"h", L"hpp", L"hxx", L"hh", L"hp", L"h++"};
	return m_pCommanderView->OPEN_ExtFromtoExt(bCheckOnly, bBeepWhenMiss, header_ext, source_ext, _countof(header_ext),
											   _countof(source_ext), LS(STR_ERR_CEDITVIEW_CMD09));
}
...
*/
BOOL CViewCommander::Command_PUTFILE(LPCWSTR   filename,  //!< [in] filename 出力ファイル名
...
)
{
	...
	if (nFlgOpt & 0x01) { /* 選択範囲を出力 */
		...
		EConvertResult eRet = CWriteManager().WriteFile_From_CDocLineMgr(
			GetDocument()->m_cDocLineMgr, SSaveInfo(filename, nSaveCharCode, EOL_NONE, bBom));
