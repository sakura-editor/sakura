...
void CViewCommander::Command_FILENEW_NEWWINDOW(void)
{
    ...
    sLoadInfo.bViewMode = false;
    std::wstring curDir = CSakuraEnvironment::GetDlgInitialDir();
    CControlTray::OpenNewEditor(G_AppInstance(), m_pCommanderView->GetHwnd(), sLoadInfo, NULL, false, curDir.c_str(), true);
    return;
}
...
bool CViewCommander::Command_FILESAVE(bool warnbeep, bool askname)
{
    ...
    //上書き処理
    if (!warnbeep)
        CEditApp::getInstance()->m_cSoundSet.MuteOn();
    bool bRet = pcDoc->m_cDocFileOperation.DoSaveFlow(&sSaveInfo);
    if (!warnbeep)
        CEditApp::getInstance()->m_cSoundSet.MuteOff();
...
{
    if (Command_OPEN_HHPP(bCheckOnly, FALSE))
        return TRUE;
    if (Command_OPEN_CCPP(bCheckOnly, FALSE))
        return TRUE;
    if (!bCheckOnly)
        ErrorBeep();
    return FALSE;
...
BOOL CViewCommander::Command_OPEN_HHPP(BOOL bCheckOnly, BOOL bBeepWhenMiss)
{
    ...
    return m_pCommanderView->OPEN_ExtFromtoExt(
        bCheckOnly, bBeepWhenMiss, source_ext, header_ext, _countof(source_ext), _countof(header_ext), LS(STR_ERR_CEDITVIEW_CMD08));
}
...
BOOL CViewCommander::Command_OPEN_CCPP(BOOL bCheckOnly, BOOL bBeepWhenMiss)
{
    ...
    static const WCHAR *header_ext[] = {L"h", L"hpp", L"hxx", L"hh", L"hp", L"h++"};
    return m_pCommanderView->OPEN_ExtFromtoExt(
        bCheckOnly, bBeepWhenMiss, header_ext, source_ext, _countof(header_ext), _countof(source_ext), LS(STR_ERR_CEDITVIEW_CMD09));
}
...
BOOL CViewCommander::Command_INSFILE(LPCWSTR filename, ECodeType nCharCode, int nFlgOpt)
{
    ...
    if (!IsValidCodeOrCPType(nSaveCharCode))
        nSaveCharCode = CODE_SJIS;
