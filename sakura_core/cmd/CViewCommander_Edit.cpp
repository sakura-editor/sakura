...
void CViewCommander::Command_WCHAR(wchar_t wcChar, bool bConvertEOL)
{
                ...
                if (NULL != pLine)
                {
					...
					*/
                    ...
                    for (nPos = CLogicInt(0); nPos < nLineLen && nPos < ptXY.GetX2();)
                    {
                        ...
                        /* その他のインデント文字 */
                        if (0 < nCharChars && pLine[nPos] != L'\0' // その他のインデント文字に L'\0' は含まれない	// 2009.02.04 ryoji L'\0'がインデントされてしまう問題修正
    ...
    if (WCODE::IsLineDelimiter(wcChar, GetDllShareData().m_Common.m_sEdit.m_bEnableExtEol) && m_pCommanderView->m_pTypeData->m_bRTrimPrevLine)
    { /* 改行時に末尾の空白を削除 */
        /* 前の行にある末尾の空白を削除する */
