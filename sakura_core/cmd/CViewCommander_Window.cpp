...
void CViewCommander::Command_CASCADE(void)
{
    ...
    if (nRowNum > 0)
    {
        ...
        ::SetWindowPos(
            pWndArr[i].hWnd, HWND_TOP, pWndArr[i].newX, pWndArr[i].newY, width, height, 0);

        // 残りを1つずつ下に入れていく
        while (--i >= 0)
        {
            ::SetWindowPos(
                pWndArr[i].hWnd, pWndArr[i + 1].hWnd, pWndArr[i].newX, pWndArr[i].newY, width, height, SWP_NOACTIVATE);
        }
...
void CViewCommander::Command_TILE_V(void)
{
    ...
    if (nRowNum > 0)
    {
        ...
        for (i = 0; i < count; ++i)
        {
            ...
            ::SetWindowPos(
                phwndArr[i], 0, rcDesktop.left, rcDesktop.top + height * i, //Mar. 19, 2004 crayonzen 上端調整
...
void CViewCommander::Command_TILE_H(void)
{
    ...
    if (nRowNum > 0)
    {
        ...
        for (i = 0; i < count; ++i)
        {
            ...
            ::ShowWindow(phwndArr[i], SW_RESTORE);
            ::SetWindowPos(
                phwndArr[i], 0, width * i + rcDesktop.left, rcDesktop.top, // Oct. 18, 2003 genta タスクバーが左にある場合を考慮
                width,
...
void CViewCommander::Command_MAXIMIZE_V(void)
{
    ...
    ::SetWindowPos(
        hwndFrame, 0, rcOrg.left, rcDesktop.top, rcOrg.right - rcOrg.left, rcDesktop.bottom - rcDesktop.top, SWP_NOOWNERZORDER | SWP_NOZORDER);
    return;
}
...
void CViewCommander::Command_MAXIMIZE_H(void)
{
    ...
    ::GetMonitorWorkRect(hwndFrame, &rcDesktop);
    ::SetWindowPos(
        hwndFrame, 0, rcDesktop.left, rcOrg.top, rcDesktop.right - rcDesktop.left, rcOrg.bottom - rcOrg.top, SWP_NOOWNERZORDER | SWP_NOZORDER);
    return;
}
...
void CViewCommander::Command_TRACEOUT(const wchar_t *outputstr, int nLen, int nFlgOpt)
{
    ...
    // 0x02 改行コードの有無
    if ((nFlgOpt & 0x02) == 0)
        CShareData::getInstance()->TraceOutString(L"\r\n");
}
