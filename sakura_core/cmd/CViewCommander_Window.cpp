...
void CViewCommander::Command_WINCLOSE(void)
{
	...
	::PostMessage(GetMainWindow(), MYWM_CLOSE, 0, // 2007.02.13 ryoji WM_CLOSE→MYWM_CLOSEに変更
				  (LPARAM)CAppNodeManager::getInstance()->GetNextTab(
					  GetMainWindow())); // タブまとめ時、次のタブに移動	2013/4/10 Uchi
	return;
}
...
void CViewCommander::Command_CASCADE(void)
{
	...
	if (nRowNum > 0)
	{
		...
		for (i = 0; i < nRowNum; ++i)
		{
			if (::IsIconic(pEditNodeArr[i].GetHwnd()))
			{ //	最小化しているウィンドウは無視。
				continue;
			}
			if (!::IsWindowVisible(pEditNodeArr[i].GetHwnd()))
			{ //	不可視ウィンドウは無視。
				continue;
			}
			//	Mar. 20, 2004 genta
		...
		//	Sep. 04, 2004 genta
		// -----------------------------------------
		for (i = 0; i < count; i++) { ::ShowWindow(pWndArr[i].hWnd, SW_RESTORE | SW_SHOWNA); }
		...
		::SetWindowPos(pWndArr[i].hWnd, HWND_TOP, pWndArr[i].newX, pWndArr[i].newY, width, height, 0);

		// 残りを1つずつ下に入れていく
		while (--i >= 0)
		{
			::SetWindowPos(pWndArr[i].hWnd, pWndArr[i + 1].hWnd, pWndArr[i].newX, pWndArr[i].newY, width, height,
						   SWP_NOACTIVATE);
		}
...
void CViewCommander::Command_TILE_V(void)
{
	...
	if (nRowNum > 0)
	{
		...
		for (i = 0; i < count; ++i)
		{
			...
			::SetWindowPos(phwndArr[i], 0, rcDesktop.left,
						   rcDesktop.top + height * i, // Mar. 19, 2004 crayonzen 上端調整
						   rcDesktop.right - rcDesktop.left, height, SWP_NOOWNERZORDER | SWP_NOZORDER);
		}
...
void CViewCommander::Command_TILE_H(void)
{
	...
	if (nRowNum > 0)
	{
		...
		for (i = 0; i < count; ++i)
		{
			//	Jul. 21, 2002 genta
			::ShowWindow(phwndArr[i], SW_RESTORE);
			::SetWindowPos(phwndArr[i], 0, width * i + rcDesktop.left,
						   rcDesktop.top, // Oct. 18, 2003 genta タスクバーが左にある場合を考慮
						   width, rcDesktop.bottom - rcDesktop.top, SWP_NOOWNERZORDER | SWP_NOZORDER);
		}
		::SetFocus(phwndArr[0]); // Aug. 17, 2002 MIK
		delete[] phwndArr;
		delete[] pEditNodeArr;
...
*/
void CViewCommander::Command_WINTOPMOST(LPARAM lparam) { GetEditWindow()->WindowTopMost(int(lparam)); }
...
void CViewCommander::Command_NEXTGROUP(void)
{
	...
	if (pcTabWnd->GetHwnd() == NULL) return;
	pcTabWnd->NextGroup();
}
...
void CViewCommander::Command_MAXIMIZE_V(void)
{
	...
	::SetWindowPos(hwndFrame, 0, rcOrg.left, rcDesktop.top, rcOrg.right - rcOrg.left, rcDesktop.bottom - rcDesktop.top,
...
void CViewCommander::Command_MAXIMIZE_H(void)
{
	...
	//	May 01, 2004 genta マルチモニタ対応
	::GetMonitorWorkRect(hwndFrame, &rcDesktop);
	::SetWindowPos(hwndFrame, 0, rcDesktop.left, rcOrg.top, rcDesktop.right - rcDesktop.left, rcOrg.bottom - rcOrg.top,
				   SWP_NOOWNERZORDER | SWP_NOZORDER);
	return;
}
...
void CViewCommander::Command_MINIMIZE_ALL(void)
{
	...
	if (0 == j) { return; }
	phWndArr = new HWND[j];
	for (i = 0; i < j; ++i) { phWndArr[i] = GetDllShareData().m_sNodes.m_pEditArr[i].GetHwnd(); }
	for (i = 0; i < j; ++i)
	{
		if (IsSakuraMainWindow(phWndArr[i]))
		{
			if (::IsWindowVisible(phWndArr[i])) ::ShowWindow(phWndArr[i], SW_MINIMIZE);
		}
	}
	delete[] phWndArr;
	return;
