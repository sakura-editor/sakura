								   ...
								   LPARAM lparam4)
{
	...
	if (m_pCommanderView->m_nAutoScrollMode && F_AUTOSCROLL != nCommand) { m_pCommanderView->AutoScrollExit(); }
	...
	if (!IsFuncEnable(GetDocument(), &GetDllShareData(), nCommand)) { return TRUE; }
	...
	/* キーリピート状態 */
	if (m_bPrevCommand == nCommand) { bRepeat = true; }
	...
	if (m_pCommanderView->ProcessCommand_isearch(nCommand, bRedraw, lparam1, lparam2, lparam3, lparam4)) return TRUE;
	...
	case F_FILESAVECLOSE:
		...
		if (Command_FILESAVE(false, true)) { Command_WINCLOSE(); }
		break;
	case F_FILECLOSE: //閉じて(無題)	//Oct. 17, 2000 jepro 「ファイルを閉じる」というキャプションを変更
		Command_FILECLOSE();
		break;
	case F_FILECLOSE_OPEN: /* 閉じて開く */ Command_FILECLOSE_OPEN(); break;
	case F_FILE_REOPEN:
	...
	// F_CHGMOD_EOL_xxx
	// はマクロに記録されないが、F_CHGMOD_EOLはマクロに記録されるので、マクロ関数を統合できるという手はず
	...
	case F_WIN_CLOSEALL: /* すべてのウィンドウを閉じる */		 // Oct. 7, 2000 jepro
														  // 「編集ウィンドウの全終了」を左記のように変更
