...
void CViewCommander::Command_COMPARE(void)
{
    ...
    if (FALSE != GetDllShareData().m_Common.m_sTabBar.m_bDispTabWnd && !GetDllShareData().m_Common.m_sTabBar.m_bDispTabWndMultiWin)
    {
...
void CViewCommander::Command_Diff(const WCHAR *_szDiffFile2, int nFlgOpt)
{
    ...
    if (GetDocument()->m_cDocEditor.IsModified() || saveCode != code || !GetDocument()->m_cDocFile.GetFilePathClass().IsValidPath() // 2014.06.25 Grep/アウトプットも対象にする
    )
    {
...
void CViewCommander::Command_Diff_Dialog(void)
{
    ...
    if (GetDocument()->m_cDocEditor.IsModified() || code != saveCode || !GetDocument()->m_cDocFile.GetFilePathClass().IsValidPath() // 2014.06.25 Grep/アウトプットも対象にする
    )
    {
        if (!m_pCommanderView->MakeDiffTmpFile(szTmpFile1, NULL, saveCode, GetDocument()->GetDocumentBomExist()))
        {
