	...
	bDifferent = true;
	{
		...
		for (;;)
		{
			...
			do
			{
				...
				// どちらかだけが、最終行に到達
				if (pLineSrc == NULL || 0 == nLineLenDes) { return true; }
				...
				else
				{
					...
					while (poDes.x < nDstEndPos)
					{
						...
						if (pLineSrc[poSrc.x] != pLineDes[poDes.x - nLineOffset]) { return true; }
						poSrc.x++;
						poDes.x++;
					}
				}
			...
			if (poSrc.x < nLineLenSrc) { return true; }
			poSrc.x = 0;
			poSrc.y++;
			poDes.x = 0;
			poDes.y++;
...
void CViewCommander::Command_COMPARE(void)
{
	...
	BOOL bDlgCompareResult = cDlgCompare.DoModal(G_AppInstance(), m_pCommanderView->GetHwnd(), (LPARAM)GetDocument(),
												 GetDocument()->m_cDocFile.GetFilePath(), szPath, &hwndCompareWnd);
	if (!bDlgCompareResult) { return; }
	/* 比較後、左右に並べて表示 */
	...
	*/
	CLogicPoint poSrc;
	GetDocument()->m_cLayoutMgr.LayoutToLogic(GetCaret().GetCaretLayoutPos(), &poSrc);
	...
	if (GetDllShareData().m_Common.m_sCompare.m_bCompareAndTileHorz)
	{
		...
		for (i = 0; i < 2; ++i)
		{
			if (::IsZoomed(phwndArr[i])) { ::ShowWindow(phwndArr[i], SW_RESTORE); }
		}
...
static ECodeType GetFileCharCode(LPCWSTR pszFile)
{
	...
	{ return CODE_ERROR; } return CCodeMediator(typeMini->m_encoding).CheckKanjiCodeOfFile(pszFile);
}

static ECodeType GetDiffCreateTempFileCode(ECodeType code)
{
	EEncodingTrait e = CCodePage::GetEncodingTrait(code);
	if (e != ENCODING_TRAIT_ASCII) { return CODE_UTF8; }
	return code;
}
...
void CViewCommander::Command_Diff(const WCHAR *_szDiffFile2, int nFlgOpt)
{
	...
	if (GetDocument()->m_cDocEditor.IsModified() || saveCode != code
	...
	bool bUTF8io = true;
	if (saveCode == CODE_SJIS) { bUTF8io = false; }
...
void CViewCommander::Command_Diff_Dialog(void)
{
	...
	// DIFF差分表示ダイアログを表示する
	int nDiffDlgResult = cDlgDiff.DoModal(G_AppInstance(), m_pCommanderView->GetHwnd(), (LPARAM)GetDocument(),
	...
	if (GetDocument()->m_cDocEditor.IsModified() || code != saveCode
		|| !GetDocument()->m_cDocFile.GetFilePathClass().IsValidPath() // 2014.06.25 Grep/アウトプットも対象にする
	)
	{
		if (!m_pCommanderView->MakeDiffTmpFile(szTmpFile1, NULL, saveCode, GetDocument()->GetDocumentBomExist()))
