	...
	case IDM_JUMPDICT:
		...
		{ /* キーワード辞書セレクトを使用する */ // 2006.04.10 fon
			...
			m_pCommanderView->TagJumpSub(
				...
				CMyPoint(1, m_pCommanderView->m_cTipWnd.m_nSearchLine), 0, true);
		}
		break;
...
int CViewCommander::Command_CUSTMENU(int nMenuIdx)
{
	...
	if (nMenuIdx < 0 || MAX_CUSTOM_MENU <= nMenuIdx) { return 0; }
	if (0 == GetDllShareData().m_Common.m_sCustomMenu.m_nCustMenuItemNumArr[nMenuIdx]) { return 0; }
