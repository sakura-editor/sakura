...
re_do:;
    ...
    {
        nSearchResult = CSearchAgent(&GetDocument()->m_cDocLineMgr).SearchWord(CLogicPoint(nIdx, nLineNumLogic),
                                                                               SEARCH_FORWARD, // 前方検索
                                                                               pcSelectLogic,
                                                                               m_pCommanderView->m_sSearchPattern);
    }
    if (nSearchResult)
    {
        ...
        //	Sep. 8, 2000 genta
        if (!bReplaceAll)
            m_pCommanderView->AddCurrentLineToHistory(); // 2002.02.16 hor すべて置換のときは不要
        if (NULL == pcSelectLogic)
        {
...
void CViewCommander::Command_REPLACE_ALL()
{
    ...
    if (((Int)GetDocument()->m_cDocLineMgr.GetLineCount() * 10 < (Int)GetDocument()->m_cLayoutMgr.GetLineCount()) && !(bSelectedArea || nPaste))
    {
    ...
    /* テキストが選択されているか */
    while ((!bFastMode && m_pCommanderView->GetSelectionInfo().IsTextSelected()) || (bFastMode && cSelectLogic.IsValid()))
    {
        /* キャンセルされたか */
        if (bCANCEL)
        {
        ...
        if (bSelectedArea)
        {
            ...
            else
            {
                ...
                // $$ 単位混在しまくりだけど、大丈夫？？
                if ((ptColLineP.y + linDif == (Int)ptOld.y && (ptColLineP.x + colDif < (Int)ptOld.x || ptColLineP.x == 0)) || ptColLineP.y + linDif < (Int)ptOld.y)
                {
                    break;
                }
            }
    ...
    else
    {
        if (bBeginBoxSelect)
        {
            ...
            if (sRangeA.GetTo().y < 0)
                sRangeA.SetToY(CLayoutInt(0));
        }
        else
        {
            // 普通の選択
