...
{
	GetEditWndPtr()->m_cToolbar.SetFocusSearchBox();
}
...
void CViewCommander::Command_SEARCH_DIALOG( void )
{
	...
	if( 0 < cmemCurText.GetStringLength() ){
		GetEditWndPtr()->m_cDlgFind.m_strText = cmemCurText.GetStringPtr();
	}
	/* 検索ダイアログの表示 */
	if( nullptr == GetEditWndPtr()->m_cDlgFind.GetHwnd() ){
		GetEditWndPtr()->m_cDlgFind.DoModeless( G_AppInstance(), m_pCommanderView->GetHwnd(), (LPARAM)&GetEditWndPtr()->GetActiveView() );
	}
	else{
		/* アクティブにする */
		ActivateFrameWindow( GetEditWndPtr()->m_cDlgFind.GetHwnd() );
		ApiWrap::DlgItem_SetText( GetEditWndPtr()->m_cDlgFind.GetHwnd(), IDC_COMBO_TEXT, cmemCurText.GetStringPtr() );
	}
	return;
}
...
void CViewCommander::Command_REPLACE_DIALOG( void )
{
	...
	/* 検索文字列を初期化 */
	if( 0 < cmemCurText.GetStringLength() ){
		GetEditWndPtr()->m_cDlgReplace.m_strText = cmemCurText.GetStringPtr();
	}
	if( 0 < GetDllShareData().m_sSearchKeywords.m_aReplaceKeys.size() ){
		if( GetEditWndPtr()->m_cDlgReplace.m_nReplaceKeySequence < GetDllShareData().m_Common.m_sSearch.m_nReplaceKeySequence ){
			GetEditWndPtr()->m_cDlgReplace.m_strText2 = GetDllShareData().m_sSearchKeywords.m_aReplaceKeys[0];	// 2006.08.23 ryoji 前回の置換後文字列を引き継ぐ
		}
	}
	...
	/* 置換オプションの初期化 */
	GetEditWndPtr()->m_cDlgReplace.m_nReplaceTarget=0;	/* 置換対象 */
	GetEditWndPtr()->m_cDlgReplace.m_nPaste=FALSE;		/* 貼り付ける？ */
// To Here 2001.12.03 hor

	/* 置換ダイアログの表示 */
	//	From Here Jul. 2, 2001 genta 置換ウィンドウの2重開きを抑止
	if( !::IsWindow( GetEditWndPtr()->m_cDlgReplace.GetHwnd() ) ){
		GetEditWndPtr()->m_cDlgReplace.DoModeless( G_AppInstance(), m_pCommanderView->GetHwnd(), (LPARAM)m_pCommanderView, bSelected );
	}
	else {
		/* アクティブにする */
		ActivateFrameWindow( GetEditWndPtr()->m_cDlgReplace.GetHwnd() );
		ApiWrap::DlgItem_SetText( GetEditWndPtr()->m_cDlgReplace.GetHwnd(), IDC_COMBO_TEXT, cmemCurText.GetStringPtr() );
	}
...
void CViewCommander::Command_REPLACE( HWND hwndParent )
{
	...
	//2002.02.10 hor
	int nPaste			=	GetEditWndPtr()->m_cDlgReplace.m_nPaste;
	int nReplaceTarget	=	GetEditWndPtr()->m_cDlgReplace.m_nReplaceTarget;
	...
	if( nPaste && !GetDocument()->m_cDocEditor.IsEnablePaste()){
		...
		::CheckDlgButton( GetEditWndPtr()->m_cDlgReplace.GetHwnd(), IDC_CHK_PASTE, FALSE );
		::EnableWindow( ::GetDlgItem( GetEditWndPtr()->m_cDlgReplace.GetHwnd(), IDC_COMBO_TEXT2 ), TRUE );
		return;	//	失敗return;
	}
...
//			bBeginBoxSelect = m_pCommanderView->GetSelectionInfo().IsBoxSelecting();
	...
	const CNativeW	cMemRepKey( GetEditWndPtr()->m_cDlgReplace.m_strText2.c_str() );
...
void CViewCommander::Command_REPLACE_ALL()
{
	...
	//2002.02.10 hor
	BOOL nPaste			= GetEditWndPtr()->m_cDlgReplace.m_nPaste;
	BOOL nReplaceTarget	= GetEditWndPtr()->m_cDlgReplace.m_nReplaceTarget;
	BOOL bRegularExp	= m_pCommanderView->m_sCurSearchOption.bRegularExp;
	BOOL bSelectedArea	= GetEditWndPtr()->m_cDlgReplace.m_bSelectedArea;
	BOOL bConsecutiveAll = GetEditWndPtr()->m_cDlgReplace.m_bConsecutiveAll;	/* 「すべて置換」は置換の繰返し */	// 2007.01.16 ryoji
	if( nPaste && nReplaceTarget == 3 ){
		// 置換対象：行削除のときは、クリップボードから貼り付けを無効にする
		nPaste = FALSE;
	}

	GetEditWndPtr()->m_cDlgReplace.m_bCanceled=false;
	GetEditWndPtr()->m_cDlgReplace.m_nReplaceCnt=0;
	...
	else
	{
		...
		cmemClip.SetString( GetEditWndPtr()->m_cDlgReplace.m_strText2.c_str() );
	}
	...
	if( bFastMode ){
		if( 0 < nReplaceNum ){
			...
			GetEditWndPtr()->ClearViewCaretPosInfo();
	...
	GetEditWndPtr()->m_cDlgReplace.m_bCanceled = (cDlgCancel.IsCanceled() != FALSE);
	GetEditWndPtr()->m_cDlgReplace.m_nReplaceCnt=nReplaceNum;
	m_pCommanderView->SetDrawSwitch(bDrawSwitchOld);
