...
#if 1 // ↓ここを残せば選択幅ゼロを最大にする（従来互換挙動）。無くても Command_INDENT() ver0
	  // が適切に動作するように変更されたので、削除しても特に不都合にはならない。
	// From Here 2001.12.03 hor
	/* SPACEorTABインンデントで矩形選択桁がゼロの時は選択範囲を最大にする */
	//	Aug. 14, 2005 genta 折り返し幅をLayoutMgrから取得するように
	if (INDENT_NONE != eIndent && m_pCommanderView->GetSelectionInfo().IsBoxSelecting()
...
void CViewCommander::Command_INDENT(const wchar_t *const pData, const CLogicInt nDataLen, EIndentType eIndent)
{
	...
	{ /* テキストが選択されているか */
		...
		else
		{
			...
			m_pCommanderView->InsertData_CEditView(
				GetCaret().GetCaretLayoutPos(), !bSoftTab ? pData : stabData,
				!bSoftTab ? nDataLen : stabData.Len(GetCaret().GetCaretLayoutPos().GetX2()), &ptInserted, true);
	...
	if (m_pCommanderView->GetSelectionInfo().IsBoxSelecting())
	{
		...
		*/
		bool alignFullWidthChar =
			eIndent == INDENT_TAB && 0 == rcSel.GetFrom().x % this->GetDocument()->m_cLayoutMgr.GetTabSpace();
#if 1 // ↓ここを残せば選択幅1のSPACEインデントで全角文字を揃える機能(2)が追加される。
		alignFullWidthChar =
			alignFullWidthChar || (eIndent == INDENT_SPACE && 1 == rcSel.GetTo().x - rcSel.GetFrom().x);
#endif
			...
			for (CLayoutInt nLineNum = rcSel.GetFrom().y; nLineNum <= rcSel.GetTo().y; ++nLineNum)
			{
				...
				if (pcLayout)
				{
					...
					for (int i = 0; 0 <= sortedKetas[i].keta; ++i)
					{
						...
						{
							if (sortedKetas[i].keta == it.getColumn()) { break; }
							it.scanNext();
							if (sortedKetas[i].keta < it.getColumn() + it.getColumnDelta()) { break; }
						}
				...
				const bool selectionIsOutOfLine =
					reachEndOfLayout
					&& ((pcLayout && pcLayout->GetLayoutEol() != EOL_NONE) ? xLayoutFrom == xLayoutTo
																		   : xLayoutTo < rcSel.GetFrom().x);
					...
					*/
					...
					if (alignFullWidthChar
						&& (ptInsert.x == rcSel.GetFrom().x
							|| (pcLayout && IsIndentChar(pcLayout->GetPtr()[nIdxFrom]))))
					{ // 文字の左側が範囲にぴったり収まっている
				...
				/* 現在位置にデータを挿入 */
				m_pCommanderView->InsertData_CEditView(ptInsert, !bSoftTab ? pData : stabData,
													   !bSoftTab ? nDataLen : stabData.Len(ptInsert.x), &ptInserted,
													   false);
				insertionWasDone = true;
				minOffset		 = t_min(
					   0 <= minOffset ? minOffset : this->GetDocument()->m_cLayoutMgr.GetMaxLineLayout(),
					   ptInsert.x <= ptInserted.x
						   ? ptInserted.x - ptInsert.x
						   : t_max(CLayoutInt(0), this->GetDocument()->m_cLayoutMgr.GetMaxLineLayout() - ptInsert.x));
		...
		if (0 < minOffset)
		{
			rcSel.GetFromPointer()->x =
				t_min(rcSel.GetFrom().x + minOffset, this->GetDocument()->m_cLayoutMgr.GetMaxLineLayout());
			rcSel.GetToPointer()->x =
				t_min(rcSel.GetTo().x + minOffset, this->GetDocument()->m_cLayoutMgr.GetMaxLineLayout());
		}
	...
	{ // 通常選択(1行内)
		...
		else
		{
			...
			m_pCommanderView->InsertData_CEditView(
				GetCaret().GetCaretLayoutPos(), !bSoftTab ? pData : stabData,
				!bSoftTab ? nDataLen : stabData.Len(GetCaret().GetCaretLayoutPos().GetX2()), &ptInserted, false);
			GetCaret().MoveCursor(ptInserted, true);
			GetCaret().m_nCaretPosX_Prev = GetCaret().GetCaretLayoutPos().GetX2();
		}
	}
	...
	{ // 通常選択(複数行)
		...
		for (CLayoutInt i = sSelectOld.GetFrom().GetY2(); i < sSelectOld.GetTo().GetY2(); i++)
		{
			...
			/* 現在位置にデータを挿入 */
			m_pCommanderView->InsertData_CEditView(CLayoutPoint(CLayoutInt(0), i), !bSoftTab ? pData : stabData,
												   !bSoftTab ? nDataLen : stabData.Len(CLayoutInt(0)), &ptInserted,
												   false);
			/* カーソルを移動 */
			GetCaret().MoveCursor(ptInserted, false);
			GetCaret().m_nCaretPosX_Prev = GetCaret().GetCaretLayoutPos().GetX2();
...
void CViewCommander::Command_UNINDENT(wchar_t wcChar)
{
	...
	{ /* テキストが選択されているか */
		...
		case WCODE::TAB:
			...
			break;
		case WCODE::SPACE: eIndent = INDENT_SPACE; break;
		default: eIndent = INDENT_NONE;
		}
	...
	else
	{
		...
		CWaitCursor cWaitCursor(m_pCommanderView->GetHwnd(),
								1000 < sSelectOld.GetTo().GetY() - sSelectOld.GetFrom().GetY());
		HWND		hwndProgress = NULL;
		int			nProgressPos = 0;
		if (cWaitCursor.IsEnable()) { hwndProgress = m_pCommanderView->StartProgress(); }
		...
		for (CLayoutInt i = sSelectOld.GetFrom().GetY2(); i < sSelectOld.GetTo().GetY2(); i++)
		{
			...
			if (NULL == pcLayout || pcLayout->GetLogicOffset() > 0)
			{ //折り返し以降の行はインデント処理を行わない
				continue;
			}
			...
			if (WCODE::TAB == wcChar)
			{
				...
				else
				{
					...
					{
						if (WCODE::SPACE != pLine[i]) { break; }
						//	Sep. 23, 2002 genta LayoutMgrの値を使う
						if (i >= nTabSpaces) { break; }
					}
					if (0 == i) { continue; }
					nDelLen = i;
				}
			...
			/* 指定位置の指定長データ削除 */
			m_pCommanderView->DeleteData2(CLayoutPoint(CLayoutInt(0), i),
...
*/
void CViewCommander::Command_TRIM(BOOL bLeft //!<  [in] FALSE: 右TRIM / それ以外: 左TRIM
)
{
	bool		 bBeDisableSelectArea = false;
	CViewSelect &cViewSelect		  = m_pCommanderView->GetSelectionInfo();

	if (!cViewSelect.IsTextSelected())
	{ //	非選択時は行選択に変更
		cViewSelect.m_sSelect.SetFrom(CLayoutPoint(CLayoutInt(0), GetCaret().GetCaretLayoutPos().GetY()));
		cViewSelect.m_sSelect.SetTo(
			CLayoutPoint(GetDocument()->m_cLayoutMgr.GetMaxLineLayout(), GetCaret().GetCaretLayoutPos().GetY()));
	...
	if (bBeDisableSelectArea) cViewSelect.DisableSelectArea(true);
}
...
inline int CNativeW_comp(const CNativeW &lhs, const CNativeW &rhs)
{
	...
	return wmemcmp(lhs.GetStringPtr(), rhs.GetStringPtr(), t_min(lhs.GetStringLength() + 1, rhs.GetStringLength() + 1));
}
...
inline int CStringRef_comp(const CStringRef &c1, const CStringRef &c2)
{
	...
	if (ret == 0) { return c1.GetLength() - c2.GetLength(); }
	return ret;
}
...
void CViewCommander::Command_SORT(BOOL bAsc) // bAsc:TRUE=昇順,FALSE=降順
{
	...
	if (m_pCommanderView->GetSelectionInfo().IsBoxSelecting())
	{
		...
		if (m_pCommanderView->GetSelectionInfo().m_sSelect.GetFrom().x
	...
	else
	{
		...
		if (sSelectOld.GetTo().x > 0)
		{
			...
			if (NULL != pcDocLine && EOL_NONE != pcDocLine->GetEol()) { sSelectOld.GetToPointer()->y++; }
		}
	}
	...
	//行選択されてない
	if (sSelectOld.IsLineOne()) { return; }
	...
	for (CLogicInt i = sSelectOld.GetFrom().GetY2(); i < sSelectOld.GetTo().y; i++)
	{
		...
		if (bBeginBoxSelectOld)
		{
			...
			if (nColumnTo < nLineLenWithoutEOL)
			{ // BOX選択範囲の右端が行内に収まっている場合
				// 2006.03.31 genta std::string::assignを使って一時変数削除
				pst->sKey = CStringRef(&pLine[nColumnFrom], nColumnTo - nColumnFrom);
			}
			else if (nColumnFrom < nLineLenWithoutEOL)
			{ // BOX選択範囲の右端が行末より右にはみ出している場合
	...
	}
	{
		...
		for (int k = 0; k < nSize; k++) { delete sta[k]; }
	}
	...
	m_pCommanderView->ReplaceData_CEditView3(sSelectOld_Layout, NULL, &repData, false,
											 m_pCommanderView->m_bDoing_UndoRedo ? NULL : GetOpeBlk(), opeSeq, NULL);
	...
	{ GetCaret().MoveCursor(m_pCommanderView->GetSelectionInfo().m_sSelect.GetFrom(), true); } else
	{
...
void CViewCommander::Command_MERGE(void)
{
	...
	if (m_pCommanderView->GetSelectionInfo().IsBoxSelecting()) { return; }
	...
	for (CLogicInt i = sSelectOld.GetFrom().GetY2(); i < sSelectOld.GetTo().y; i++)
	{
		...
		{ lineArr.push_back(CStringRef(pLine, nLineLen)); } else
		{
			bMerge = true;
		}
		pLinew	= pLine;
...
void CViewCommander::Command_Reconvert(void)
{
	...
	//領域確保
	PRECONVERTSTRING pReconv = (PRECONVERTSTRING)::HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, nSize);
