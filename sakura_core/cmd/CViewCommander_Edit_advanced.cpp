...
void CViewCommander::Command_INDENT(const wchar_t *const pData, const CLogicInt nDataLen, EIndentType eIndent)
{
    ...
    struct SSoftTabData
    {
        ...
        int Len(CLayoutInt nCol)
        {
            return (m_nTab + m_nXWidth - ((Int)nCol + m_nXWidth) % m_nTab) / m_nSpWidth;
        }
        wchar_t *m_szTab;
        int m_nTab;
        int m_nXWidth;
		...
		*/
        ...
        for (bool insertionWasDone = false;; alignFullWidthChar = false)
        {
            ...
            for (CLayoutInt nLineNum = rcSel.GetFrom().y; nLineNum <= rcSel.GetTo().y; ++nLineNum)
            {
                ...
                const bool selectionIsOutOfLine = reachEndOfLayout && ((pcLayout && pcLayout->GetLayoutEol() != EOL_NONE) ? xLayoutFrom == xLayoutTo : xLayoutTo < rcSel.GetFrom().x);
                ...
                if (INDENT_NONE != eIndent)
                {
					...
					*/
                    ...
                    /*
						全角文字の左側の桁揃え
					*/
                    if (alignFullWidthChar && (ptInsert.x == rcSel.GetFrom().x || (pcLayout && IsIndentChar(pcLayout->GetPtr()[nIdxFrom]))))
                    { // 文字の左側が範囲にぴったり収まっている
...
{
    return CNativeW_comp(*pst1->pCmemLine, *pst2->pCmemLine) < 0;
}
...
/*!	物理行のソートに使う関数(昇順) */
bool SortByKeyAsc(SORTDATA *pst1, SORTDATA *pst2)
{
    return CStringRef_comp(pst1->sKey, pst2->sKey) < 0;
}

/*!	物理行のソートに使う関数(降順) */
bool SortByKeyDesc(SORTDATA *pst1, SORTDATA *pst2)
{
    return CStringRef_comp(pst1->sKey, pst2->sKey) > 0;
}
