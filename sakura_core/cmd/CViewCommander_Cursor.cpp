...
void CViewCommander::Command_RIGHT(bool bSelect, bool bIgnoreCurrentSelection, bool bRepeat)
{
    ...
    for (int nRepCount = 0; nRepCount < nRepeat; ++nRepCount)
    {
        ...
        if (pcLayout)
        {
            ...
            // キャレットの移動先を決める。
            if (nextline_exists && (on_x_max == MOVE_NEXTLINE_IMMEDIATELY && x_max <= to_x || on_x_max == MOVE_NEXTLINE_NEXTTIME && x_max < to_x || on_x_max == MOVE_NEXTLINE_NEXTTIME_AND_MOVE_RIGHT && x_max < to_x))
            {
                ptTo.y = ptCaret.y + 1;
...
)
{
    ...
    if (!bLineTopOnly)
    {
        ...
        do
        {
            ...
            {
                if (WCODE::IsIndentChar(pLine[nPosX_Logic], bZenSpace != 0))
                    continue;
        ...
        if (0 == nPosX_Logic)
            nPosY_Layout = ptCaretPos.y; /* 物理行の移動なし */
...
void CViewCommander::Command_MODIFYLINE_NEXT(bool bSelect)
{
    ...
    for (int n = 0; n < 2; n++)
    {
        while (pcDocLine)
        {
            if (CModifyVisitor().IsLineModified(pcDocLine, nSaveSeq) != bModified || (0 == ptXY.y && CModifyVisitor().IsLineModified(pcDocLine, nSaveSeq) != false))
            {
