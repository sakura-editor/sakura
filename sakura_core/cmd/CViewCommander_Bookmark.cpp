...
{
	if( !GetEditWndPtr()->m_cDlgJump.DoModal(
...
void CViewCommander::Command_JUMP( void )
{
	...
	/* 行番号 */
	int	nLineNum; //$$ 単位混在
	nLineNum = GetEditWndPtr()->m_cDlgJump.m_nLineNum;

	if( !GetEditWndPtr()->m_cDlgJump.m_bPLSQL ){	/* PL/SQLソースの有効行か */
	...
	nMode = 0;
	nCurrentLine = GetEditWndPtr()->m_cDlgJump.m_nPLSQL_E2 - 1;

	int	nLineCount; //$$ 単位混在
	nLineCount = GetEditWndPtr()->m_cDlgJump.m_nPLSQL_E1 - 1;
