	...
	: CWnd(L"::CFuncKeyWnd")
{
	...
	for (i = 0; i < _countof(m_szFuncNameArr); ++i) { m_szFuncNameArr[i][0] = LTEXT('\0'); }
	//	2002.11.04 Moca Open()側で設定
	//	m_nButtonGroupNum = 4;

	for (i = 0; i < _countof(m_hwndButtonArr); ++i) { m_hwndButtonArr[i] = NULL; }
...
HWND CFuncKeyWnd::Open(HINSTANCE hInstance, HWND hwndParent, CEditDoc *pCEditDoc, bool bSizeBox)
{
	...
	if (1 > m_nButtonGroupNum || 12 < m_nButtonGroupNum) { m_nButtonGroupNum = 4; }
	...
	RegisterWC(hInstance,
			   ...
			   NULL /*MAKEINTRESOURCE( MYDOCUMENT )*/, // Pointer to a null-terminated character string that specifies
													   // the resource name of the class menu, as the name appears in
													   // the resource file.
	...
	CWnd::Create(hwndParent,
				 ...
				 WS_CHILD /* | WS_VISIBLE*/ | WS_CLIPCHILDREN, // window style	// 2006.06.17 ryoji WS_CLIPCHILDREN 追加
															   // // 2007.03.08 ryoji WS_VISIBLE 除去
...
/* ウィンドウ クローズ */
void CFuncKeyWnd::Close(void) { this->DestroyWindow(); }
...
LRESULT CFuncKeyWnd::OnSize(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		if (0 < i && 0 == (i % m_nButtonGroupNum)) { nX += 12; }
...
LRESULT CFuncKeyWnd::OnCommand(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	for (i = 0; i < _countof(m_hwndButtonArr); ++i)
	{
		if (hwndCtl == m_hwndButtonArr[i])
		{
			...
			{ ::SendMessageCmd(GetParentHwnd(), WM_COMMAND, MAKELONG(m_nFuncCodeArr[i], 0), (LPARAM)hwnd); } break;
		}
	}
...
LRESULT CFuncKeyWnd::OnTimer(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	//	return;
	if (NULL == GetHwnd()) { return 0; }

	if (::GetActiveWindow() != GetParentHwnd() && m_nCurrentKeyState != -1)
	{ //	2002/06/02 MIK	// 2006.12.20 ryoji 初回更新は処理する
		return 0;
	}
	...
	if (nIdx != m_nCurrentKeyState)
	{
		...
		for (i = 0; i < _countof(m_szFuncNameArr); ++i)
		{
			...
			EFunctionCode nFuncCode = CKeyBind::GetFuncCode((WORD)(((VK_F1 + i) | ((WORD)((BYTE)(nIdx))) << 8)),
	...
	if (m_nTimerCount > TIMER_CHECKFUNCENABLE || nIdx != m_nCurrentKeyState)
	{
		m_nTimerCount = 0;
		/* 機能が利用可能か調べる */
		for (i = 0; i < _countof(m_szFuncNameArr); ++i)
		{
			...
			{ ::EnableWindow(m_hwndButtonArr[i], TRUE); } else
			{
...
int CFuncKeyWnd::CalcButtonSize(void)
{
	...
	else
	{
		...
		::MoveWindow(m_hwndSizeBox, rc.right - rc.left - nCxVScroll, rc.bottom - rc.top - nCyHScroll, nCxVScroll,
					 nCyHScroll, TRUE);
...
void CFuncKeyWnd::CreateButtons(void)
{
	...
	for (i = 0; i < _countof(m_nFuncCodeArr); ++i) { m_nFuncCodeArr[i] = F_0; }
