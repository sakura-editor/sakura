...
const DWORD p_helpids[] = { // 12300
	...
	HIDCANCEL_CMP, //キャンセル
	IDC_BUTTON_HELP,
	HIDC_CMP_BUTTON_HELP, //ヘルプ
	IDC_CHECK_TILE_H,
	HIDC_CMP_CHECK_TILE_H, //左右に表示
	IDC_LIST_FILES,
	HIDC_CMP_LIST_FILES, //ファイル一覧
	IDC_STATIC_COMPARESRC,
	HIDC_CMP_STATIC_COMPARESRC, //ソースファイル
	0,
...
/* モーダルダイアログの表示 */
int CDlgCompare::DoModal(HINSTANCE hInstance, HWND hwndParent, LPARAM lParam, const WCHAR *pszPath,
						 WCHAR *pszCompareLabel, HWND *phwndCompareWnd)
{
	m_pszPath		  = pszPath;
...
BOOL CDlgCompare::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_COMPARE)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
		...
		for (i = 0; i < nRowNum; ++i)
		{
			...
			if (pEditNodeArr[i].GetHwnd() == CEditWnd::getInstance()->GetHwnd())
			{
				...
				CFileNameManager::getInstance()->GetMenuFullLabel_WinListNoEscape(
					szMenu, _countof(szMenu), pfi, pEditNodeArr[i].m_nId, -1, calc.GetDC());
				::DlgItem_SetText(GetHwnd(), IDC_STATIC_COMPARESRC, szMenu);
				continue;
			}
			// 番号は ウィンドウリストと同じになるようにする
			CFileNameManager::getInstance()->GetMenuFullLabel_WinListNoEscape(szMenu, _countof(szMenu), pfi,
																			  pEditNodeArr[i].m_nId, i, calc.GetDC());
...
int CDlgCompare::GetData(void)
{
	...
	else
	{
		*m_phwndCompareWnd = (HWND)List_GetItemData(hwndList, nItem);
		...
		CTextWidthCalc calc(hwndList);
		CFileNameManager::getInstance()->GetMenuFullLabel_WinListNoEscape(m_pszCompareLabel, _MAX_PATH /*長さ不明*/,
																		  pfi, nId, -1, calc.GetDC());
...
LPVOID CDlgCompare::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end
