...
const DWORD p_helpids[] = {IDC_LIST_PLUGIN_OPTIONS, HIDC_LIST_PLUGIN_OPTIONS,	  // オプションリスト
...
/* モーダルダイアログの表示 */
int CDlgPluginOption::DoModal(HINSTANCE hInstance, HWND hwndParent, CPropPlugin *cPropPlugin, int ID)
{
	// プラグイン番号（エディタがふる番号）
	m_ID		  = ID;
...
int CDlgPluginOption::GetData(void)
{
	...
	for (i = 0, it = m_cPlugin->m_options.begin(); it != m_cPlugin->m_options.end(); i++, it++) {
		...
		cOpt->GetKey(&sSection, &sKey);
		if (sSection.empty() || sKey.empty()) { continue; }
...
LPVOID CDlgPluginOption::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
...
void CDlgPluginOption::ChangeListPosition(void)
{
	...
	if (current == -1 || current == m_Line) { return; }

	WCHAR  buf[MAX_LENGTH_VALUE + 1];
	LVITEM lvi;

	// 戻し
	if (m_Line >= 0) { SetFromEdit(m_Line); }
