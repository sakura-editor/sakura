...
struct CDlgOpenFile_CommonItemDialog final : public IDlgOpenFile,
	...
	bool DoModalOpenDlg(SLoadInfo *pLoadInfo, std::vector<std::wstring> *pFileNames, bool bOptions) override;
	bool DoModalSaveDlg(SSaveInfo *pSaveInfo, bool bSimpleMode) override;

	bool	DoModalOpenDlgImpl0(bool bAllowMultiSelect, std::vector<std::wstring> *pFileNames, LPCWSTR fileName,
								const std::vector<COMDLG_FILTERSPEC> &specs);
	HRESULT DoModalOpenDlgImpl1(IFileOpenDialog *pFileOpenDialog, bool bAllowMultiSelect,
								std::vector<std::wstring> *pFileNames, LPCWSTR fileName,
								const std::vector<COMDLG_FILTERSPEC> &specs);
...
/* 初期化 */
void CDlgOpenFile_CommonItemDialog::Create(HINSTANCE hInstance, HWND hwndParent, const WCHAR *pszUserWildCard,
...
bool CDlgOpenFile_CommonItemDialog::DoModal_GetOpenFileName(WCHAR *pszPath, EFilter eAddFilter)
{
	...
	if (ret) { wcscpy(pszPath, fileNames[0].c_str()); }
	return ret;
}
...
#define RETURN_IF_FAILED                                                                                               \
	...
	if (m_customizeSetting.bUseCharCode) {
		...
		if (bCodeSel) {
			...
			hr = AddCheckButton(CtrlId::CHECK_CP, L"C&P", TRUE);
			RETURN_IF_FAILED
	...
	if (m_customizeSetting.bUseEol) {
		...
		RETURN_IF_FAILED
		hr = AddControlItem(CtrlId::COMBO_EOL, EOL_NONE, LS(STR_DLGOPNFL1));
		RETURN_IF_FAILED
		hr = AddControlItem(CtrlId::COMBO_EOL, EOL_CRLF, L"CR+LF");
		RETURN_IF_FAILED
		hr = AddControlItem(CtrlId::COMBO_EOL, EOL_LF, L"LF (UNIX)");
		RETURN_IF_FAILED
		hr = AddControlItem(CtrlId::COMBO_EOL, EOL_CR, L"CR (Mac)");
		RETURN_IF_FAILED
		hr = SetSelectedControlItem(CtrlId::COMBO_EOL, 0);
		RETURN_IF_FAILED
		hr = EndVisualGroup();
		RETURN_IF_FAILED
	}

	hr = StartVisualGroup(CtrlId::LABEL_MRU, LS(STR_FILEDIALOG_MRU));
	RETURN_IF_FAILED
	...
	hr = EndVisualGroup();
	RETURN_IF_FAILED
	hr = StartVisualGroup(CtrlId::LABEL_OPENFOLDER, LS(STR_FILEDIALOG_OPENFOLDER));
	RETURN_IF_FAILED
...
HRESULT CDlgOpenFile_CommonItemDialog::DoModalOpenDlgImpl1(IFileOpenDialog *pFileOpenDialog, bool bAllowMultiSelect,
														   std::vector<std::wstring> *pFileNames, LPCWSTR fileName,
														   const std::vector<COMDLG_FILTERSPEC> &specs)
{
...
#define RETURN_IF_FAILED                                                                                               \
	...
	if (bAllowMultiSelect) { options |= FOS_ALLOWMULTISELECT; }

	hr = pFileOpenDialog->SetOptions(options);
	RETURN_IF_FAILED
	hr = pFileOpenDialog->SetFileTypes(specs.size(), &specs[0]);
	RETURN_IF_FAILED
	hr = pFileOpenDialog->SetFileName(fileName);
	RETURN_IF_FAILED

	using namespace Microsoft::WRL;
	ComPtr<IShellItem> psiFolder;
	SHCreateItemFromParsingName(m_szInitialDir, NULL, IID_PPV_ARGS(&psiFolder));
	hr = pFileOpenDialog->SetFolder(psiFolder.Get());
	RETURN_IF_FAILED
	...
	hr = pFileOpenDialog->GetResults(&pShellItems);
	RETURN_IF_FAILED
	DWORD numItems;
	...
	for (DWORD i = 0; i < numItems; ++i) {
		...
		RETURN_IF_FAILED(*pFileNames)[i] = pszFilePath;
...
bool CDlgOpenFile_CommonItemDialog::DoModalOpenDlgImpl0(bool bAllowMultiSelect, std::vector<std::wstring> *pFileNames,
														LPCWSTR fileName, const std::vector<COMDLG_FILTERSPEC> &specs)
{
	using namespace Microsoft::WRL;
 ...
 */
bool CDlgOpenFile_CommonItemDialog::DoModalOpenDlg(SLoadInfo *pLoadInfo, std::vector<std::wstring> *pFileNames,
												   bool bOptions)
{
...
HRESULT CDlgOpenFile_CommonItemDialog::DoModalSaveDlgImpl1(IFileSaveDialog *pFileSaveDialog, WCHAR *pszPath)
{
	//カレントディレクトリを保存。関数から抜けるときに自動でカレントディレクトリは復元される。
	CCurrentDirectoryBackupPoint cCurDirBackup;
...
#define RETURN_IF_FAILED                                                                                               \
	...
	RETURN_IF_FAILED
	hr = pFileSaveDialog->SetFileTypes(specs.size(), &specs[0]);
	RETURN_IF_FAILED
	ComPtr<IShellItem> psiFolder;
	SHCreateItemFromParsingName(m_szInitialDir, NULL, IID_PPV_ARGS(&psiFolder));
	hr = pFileSaveDialog->SetFolder(psiFolder.Get());
	RETURN_IF_FAILED
...
bool CDlgOpenFile_CommonItemDialog::DoModalSaveDlg(SSaveInfo *pSaveInfo, bool bSimpleMode)
{
	...
	if (ret && !bSimpleMode) {
		...
		if (m_customizeSetting.bUseBom) { pSaveInfo->bBomExist = m_bBom; }
	}
	return ret;
}
	...
	/* [in] */ BOOL								   bChecked)
{
	...
	case CtrlId::CHECK_CP:
		...
		break;
	case CtrlId::CHECK_BOM: m_bBom = bChecked ? true : false; break;
	}
	return S_OK;
}
