...
const DWORD p_helpids[] = { // 13200
	IDC_BUTTON_DIFF_DST, HIDC_BUTTON_DIFF_DST, IDC_CHECK_DIFF_OPT_BLINE, HIDC_CHECK_DIFF_OPT_BLINE,
	IDC_CHECK_DIFF_OPT_CASE, HIDC_CHECK_DIFF_OPT_CASE, IDC_CHECK_DIFF_OPT_SPACE, HIDC_CHECK_DIFF_OPT_SPACE,
	IDC_CHECK_DIFF_OPT_SPCCHG, HIDC_CHECK_DIFF_OPT_SPCCHG, IDC_CHECK_DIFF_OPT_TABSPC, HIDC_CHECK_DIFF_OPT_TABSPC,
	IDC_EDIT_DIFF_DST, HIDC_EDIT_DIFF_DST, IDC_FRAME_DIFF_FILE12, HIDC_RADIO_DIFF_FILE1, IDC_RADIO_DIFF_FILE1,
	HIDC_RADIO_DIFF_FILE1, IDC_RADIO_DIFF_FILE2, HIDC_RADIO_DIFF_FILE2, IDC_FRAME_DIFF_DST, HIDC_RADIO_DIFF_DST1,
	IDC_RADIO_DIFF_DST1, HIDC_RADIO_DIFF_DST1, IDC_RADIO_DIFF_DST2, HIDC_RADIO_DIFF_DST2, IDC_LIST_DIFF_FILES,
	HIDC_LIST_DIFF_FILES, IDC_STATIC_DIFF_SRC, HIDC_STATIC_DIFF_SRC, IDOK, HIDC_DIFF_IDOK, IDCANCEL, HIDC_DIFF_IDCANCEL,
	IDC_BUTTON_HELP, HIDC_BUTTON_DIFF_HELP, IDC_CHECK_DIFF_EXEC_STATE,
...
/* モーダルダイアログの表示 */
int CDlgDiff::DoModal(HINSTANCE hInstance, HWND hwndParent, LPARAM lParam,
...
BOOL CDlgDiff::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_DIFF_DIALOG)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
	...
	case IDC_RADIO_DIFF_DST2:
		...
		//::EnableWindow( GetItemHwnd( IDC_LIST_DIFF_FILES ), TRUE );
		{
			...
			if (List_GetCurSel(hwndList) == LB_ERR) { List_SetCurSel(hwndList, m_nIndexSave); }
		}
		return TRUE;

	case IDC_RADIO_DIFF_FILE1: ::CheckDlgButton(GetHwnd(), IDC_RADIO_DIFF_FILE2, FALSE); return TRUE;

	case IDC_RADIO_DIFF_FILE2: ::CheckDlgButton(GetHwnd(), IDC_RADIO_DIFF_FILE1, FALSE); return TRUE;
	}
	...
	/* 編集中のファイル一覧を作成する */
	{
		...
		if (nRowNum > 0) {
			...
			for (i = 0; i < nRowNum; i++) {
				...
				// 番号はウィンドウ一覧と同じ番号を使う
				CFileNameManager::getInstance()->GetMenuFullLabel_WinListNoEscape(szName, _countof(szName), pFileInfo,
		...
		if (count == 0) {
			...
			//	ListBoxが選択されていなかったら，先頭のファイルを選択する．
			HWND hwndList = GetItemHwnd(IDC_LIST_DIFF_FILES);
			if (List_GetCurSel(hwndList) == LB_ERR) { List_SetCurSel(hwndList, selIndex); }
		}
...
LPVOID CDlgDiff::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
