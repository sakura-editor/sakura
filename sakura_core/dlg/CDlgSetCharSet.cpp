...
BOOL CDlgSetCharSet::OnBnClicked(int wID)
{
	...
	case IDOK:
		/* ダイアログデータの取得 */
		if (GetData()) { CloseDialog(TRUE); }
		return TRUE;
	case IDCANCEL: CloseDialog(FALSE); return TRUE;
	}
...
LPVOID CDlgSetCharSet::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
