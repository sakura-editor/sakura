...
BOOL CDlgExec::OnInitDialog(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	for (i = 0; i < _countof(codes); ++i) { Combo_AddString(hwndCombo, CCodeTypeName(codes[i]).Normal()); }
...
void CDlgExec::SetData(void)
{
	...
	{ //	From Here 2007.01.02 maru 引数を拡張のため
		...
		if (!m_bEditable) { nExecFlgOpt &= ~0x02; }
		...
		::EnableWindow(GetItemHwnd(IDC_COMBO_CODE_GET),
					   nExecFlgOpt & 0x01 ? TRUE
										  : FALSE); // 標準出力Off時、Unicodeを使用するをDesableする	2008/6/20 Uchi
		::EnableWindow(GetItemHwnd(IDC_COMBO_CODE_SEND),
					   nExecFlgOpt & 0x04 ? TRUE
										  : FALSE); // 標準入力Off時、Unicodeを使用するをDesableする	2008/6/20 Uchi
	...
	for (i = 0; i < nSize; ++i) { Combo_AddString(hwndCombo, m_pShareData->m_sHistory.m_aCommands[i]); }
...
int CDlgExec::GetData(void)
{
	...
	{ //	From Here 2007.01.02 maru 引数を拡張のため
		...
		nFlgOpt |= (BST_CHECKED == ::IsDlgButtonChecked(GetHwnd(), IDC_RADIO_EDITWINDOW))
					   ? 0x02
					   : 0; // 標準出力を編集中のウインドウへ
		nFlgOpt |= (BST_CHECKED == ::IsDlgButtonChecked(GetHwnd(), IDC_CHECK_SENDSTDIN))
					   ? 0x04
					   : 0; // 編集中ファイルを標準入力へ
		nFlgOpt |=
			(BST_CHECKED == ::IsDlgButtonChecked(GetHwnd(), IDC_CHECK_CUR_DIR)) ? 0x200 : 0; // カレントディレクトリ指定
		int sel;
...
BOOL CDlgExec::OnBnClicked(int wID)
{
	...
	{ //	From Here 2007.01.02 maru 引数を拡張のため
		...
		::EnableWindow(GetItemHwnd(IDC_RADIO_EDITWINDOW),
					   (bEnabled && m_bEditable) ? TRUE
												 : FALSE); // ビューモードや上書き禁止の条件追加	// 2009.02.21 ryoji
	}													   //	To Here 2007.01.02 maru 引数を拡張のため
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_EXECMD_DIALOG)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		break;
	...
	case IDOK: /* 下検索 */
		...
		CloseDialog(1);
		return TRUE;
	case IDCANCEL: CloseDialog(0); return TRUE;
	}
	return FALSE;
}

//@@@ 2002.01.18 add start
LPVOID CDlgExec::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end
