...
void CDlgProperty::SetData(void)
{
    ...
    if (INVALID_HANDLE_VALUE != (nFind = ::FindFirstFile(pCEditDoc->m_cDocFile.GetFilePath(), &wfd)))
    {
        ...
        auto_sprintf(szWork, LS(STR_DLGFLPROP_YMDHMS), ctimeCreation->wYear, ctimeCreation->wMonth, ctimeCreation->wDay, ctimeCreation->wHour, ctimeCreation->wMinute, ctimeCreation->wSecond);
        cmemProp.AppendString(szWork);
        cmemProp.AppendString(L"\r\n");

        cmemProp.AppendString(LS(STR_DLGFLPROP_UPDATE_DT));
        CFileTime ctimeLastWrite = wfd.ftLastWriteTime;
        auto_sprintf(szWork, LS(STR_DLGFLPROP_YMDHMS), ctimeLastWrite->wYear, ctimeLastWrite->wMonth, ctimeLastWrite->wDay, ctimeLastWrite->wHour, ctimeLastWrite->wMinute, ctimeLastWrite->wSecond);
        cmemProp.AppendString(szWork);
        cmemProp.AppendString(L"\r\n");

        cmemProp.AppendString(LS(STR_DLGFLPROP_ACCESS_DT));
        CFileTime ctimeLastAccess = wfd.ftLastAccessTime;
        auto_sprintf(szWork, LS(STR_DLGFLPROP_YMDHMS), ctimeLastAccess->wYear, ctimeLastAccess->wMonth, ctimeLastAccess->wDay, ctimeLastAccess->wHour, ctimeLastAccess->wMinute, ctimeLastAccess->wSecond);
        cmemProp.AppendString(szWork);
        cmemProp.AppendString(L"\r\n");
