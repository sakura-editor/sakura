...
const DWORD p_helpids[] = { // 12600
	...
	IDC_BUTTON_HELP, HIDC_PROP_BUTTON_HELP, IDC_EDIT_PROPERTY,
	HIDC_PROP_EDIT1, // IDC_EDIT1->IDC_EDIT_PROPERTY	2008/7/3 Uchi
					 //	IDC_STATIC,				-1,
...
BOOL CDlgProperty::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_PROPERTY_FILE)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
	case IDOK: /* 下検索 */
		/* ダイアログデータの取得 */
...
void CDlgProperty::SetData(void)
{
	...
	if (pCEditDoc->GetDocumentBomExist()) { cmemProp.AppendString(LS(STR_DLGFLPROP_WITH_BOM)); }
	...
	if (INVALID_HANDLE_VALUE != (nFind = ::FindFirstFile(pCEditDoc->m_cDocFile.GetFilePath(), &wfd))) {
		...
		cmemProp.AppendString(szWork);
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_ARCHIVE) { cmemProp.AppendString(LS(STR_DLGFLPROP_AT_ARCHIVE)); }
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_COMPRESSED) { cmemProp.AppendString(LS(STR_DLGFLPROP_AT_COMPRESS)); }
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) { cmemProp.AppendString(LS(STR_DLGFLPROP_AT_FOLDER)); }
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN) { cmemProp.AppendString(LS(STR_DLGFLPROP_AT_HIDDEN)); }
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_NORMAL) { cmemProp.AppendString(LS(STR_DLGFLPROP_AT_NORMAL)); }
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_OFFLINE) { cmemProp.AppendString(LS(STR_DLGFLPROP_AT_OFFLINE)); }
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_READONLY) { cmemProp.AppendString(LS(STR_DLGFLPROP_AT_READONLY)); }
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_SYSTEM) { cmemProp.AppendString(LS(STR_DLGFLPROP_AT_SYSTEM)); }
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_TEMPORARY) { cmemProp.AppendString(LS(STR_DLGFLPROP_AT_TEMP)); }
		cmemProp.AppendString(L"\r\n");

		cmemProp.AppendString(LS(STR_DLGFLPROP_CREATE_DT));
		CFileTime ctimeCreation = wfd.ftCreationTime;
		auto_sprintf(szWork, LS(STR_DLGFLPROP_YMDHMS), ctimeCreation->wYear, ctimeCreation->wMonth, ctimeCreation->wDay,
					 ctimeCreation->wHour, ctimeCreation->wMinute, ctimeCreation->wSecond);
		cmemProp.AppendString(szWork);
		cmemProp.AppendString(L"\r\n");

		cmemProp.AppendString(LS(STR_DLGFLPROP_UPDATE_DT));
		CFileTime ctimeLastWrite = wfd.ftLastWriteTime;
		auto_sprintf(szWork, LS(STR_DLGFLPROP_YMDHMS), ctimeLastWrite->wYear, ctimeLastWrite->wMonth,
					 ctimeLastWrite->wDay, ctimeLastWrite->wHour, ctimeLastWrite->wMinute, ctimeLastWrite->wSecond);
		cmemProp.AppendString(szWork);
		cmemProp.AppendString(L"\r\n");

		cmemProp.AppendString(LS(STR_DLGFLPROP_ACCESS_DT));
		CFileTime ctimeLastAccess = wfd.ftLastAccessTime;
		auto_sprintf(szWork, LS(STR_DLGFLPROP_YMDHMS), ctimeLastAccess->wYear, ctimeLastAccess->wMonth,
					 ctimeLastAccess->wDay, ctimeLastAccess->wHour, ctimeLastAccess->wMinute, ctimeLastAccess->wSecond);
		cmemProp.AppendString(szWork);
		cmemProp.AppendString(L"\r\n");
	...
	if (!in) { goto end_of_CodeTest; }
	nBufLen = in.GetLength();
	if (nBufLen > CheckKanjiCode_MAXREADLENGTH) { nBufLen = CheckKanjiCode_MAXREADLENGTH; }
...
LPVOID CDlgProperty::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end
