...
BOOL CDlgFind::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(
			GetHwnd(), HELP_CONTEXT,
			::FuncID_To_HelpContextID(
				F_SEARCH_DIALOG)); // Apr. 5, 2001 JEPRO 修正漏れを追加	// 2006.10.10 ryoji MyWinHelpに変更に変更
		break;
	case IDC_CHK_REGULAREXP: /* 正規表現 */
		...
		break;
	case IDC_BUTTON_SEARCHPREV: /* 上検索 */ // Feb. 13, 2001 JEPRO
											 // ボタン名を[IDC_BUTTON1]→[IDC_BUTTON_SERACHPREV]に変更
		/* ダイアログデータの取得 */
		nRet = GetData();
		if (0 < nRet)
		{
...
LPVOID CDlgFind::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end
