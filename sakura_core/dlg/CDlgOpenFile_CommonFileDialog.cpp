			...
			// 拡張子の補完を自前で行う	// 2006.11.10 ryoji
			{
				...
				if( pData->m_bIsSaveDialog ){
					...
					if( *pszCur == L'\"' ){	// 二重引用部で始まっている
						::wcsncpy_s(pData->m_szPath, _MAX_PATH, pData->m_pOf->lpstrFile, _TRUNCATE);
					}
					else{
						...
						if( szDefExt[0] == L'.' /* && szDefExt[1] != L'\0' */ ){	// 既に拡張子がついている	2文字目のチェックの削除	2008/6/14 Uchi
							...
							::wcsncpy_s(szBuf, _MAX_PATH + 1, pData->m_pOf->lpstrFile, _TRUNCATE);
							::wcscat(szBuf, szDefExt);
							::wcsncpy_s(pData->m_szPath, _MAX_PATH, szBuf, _TRUNCATE);
						}
					}
					...
					if( IsFileExists(pData->m_szPath, true) ){
						...
						::wcsncpy_s(szText, _MAX_PATH, pData->m_szPath, _TRUNCATE);
...
)
{
	...
	if( pSaveInfo->cFilePath[0] == L'\0' )
		::wcsncpy_s(pSaveInfo->cFilePath, _MAX_PATH, LS(STR_NO_TITLE2), _TRUNCATE);	// 無題
...
//	pData->m_ofn.lpstrDefExt = L"";
	...
	if( GetSaveFileNameRecover( &pData->m_ofn ) ){
		...
		if( pData->m_ofn.Flags & OFN_ENABLEHOOK )
			::wcsncpy_s(pSaveInfo->cFilePath, _MAX_PATH, pData->m_szPath, _TRUNCATE);	// 自前で拡張子の補完を行ったときのファイルパス	// 2006.11.10 ryoji
