...
struct CDlgOpenFile_CommonFileDialog final : public IDlgOpenFile
{
	...
	void Create(HINSTANCE hInstance, HWND hwndParent, const WCHAR *pszUserWildCard, const WCHAR *pszDefaultPath,
				const std::vector<LPCWSTR> &vMRU, const std::vector<LPCWSTR> &vOPENFOLDER) override;
...
)
{
	...
	case WM_SIZE:
	{
		...
		::SetWindowPos(pData->m_hwndComboMRU, 0, 0, 0, nWidth - po.x - nRightMargin, rc.bottom - rc.top,
					   SWP_NOMOVE | SWP_NOOWNERZORDER | SWP_NOZORDER);
		::SetWindowPos(pData->m_hwndComboOPENFOLDER, 0, 0, 0, nWidth - po.x - nRightMargin, rc.bottom - rc.top,
					   SWP_NOMOVE | SWP_NOOWNERZORDER | SWP_NOZORDER);
		return 0;
	}
	case WM_INITDIALOG:
	{
		...
		// Modified by KEITA for WIN64 2003.9.6
		pData->m_wpOpenDialogProc =
			(WNDPROC)::SetWindowLongPtr(pData->m_hwndOpenDlg, GWLP_WNDPROC, (LONG_PTR)OFNHookProcMain);
		...
		for (/*i = 0*/; i < cCodeTypes.GetCount(); ++i)
		{
			...
			if (cCodeTypes.GetCode(i) == pData->m_nCharCode) { nIdxSel = nIdx; }
		}
		if (nIdxSel != -1) { Combo_SetCurSel(pData->m_hwndComboCODES, nIdxSel); }
		else
		{
		...
		case CDN_FILEOK:
			// 拡張子の補完を自前で行う	// 2006.11.10 ryoji
			{
				...
				if (pData->m_bIsSaveDialog)
				{
					...
					CommDlg_OpenSave_GetSpec(pData->m_hwndOpenDlg, szBuf,
											 _MAX_PATH); // ファイル名入力ボックス内の文字列
					pszCur = szBuf;
					...
					else
					{
						...
						if (szDefExt[0] == L'.' /* && szDefExt[1] != L'\0' */)
						{ // 既に拡張子がついている 2文字目のチェックの削除
						  // 2008/6/14 Uchi
							// .のみの場合にも拡張子付きとみなす。
							lstrcpyn(pData->m_szPath, pData->m_pOf->lpstrFile, _MAX_PATH);
						}
						...
						{
							switch (pData->m_pOf->nFilterIndex)
							{		// 選択されているファイルの種類
							case 1: // ユーザー定義
								...
								while (*pszCur != L'.' && *pszCur != L'\0') // '.'まで読み飛ばす
									pszCur = ::CharNext(pszCur);
								i = 0;
								while (*pszCur != L';' && *pszCur != L'\0')
								{ // ';'までコピーする
									pszNext = ::CharNext(pszCur);
									while (pszCur < pszNext) szDefExt[i++] = *pszCur++;
								}
								szDefExt[i] = L'\0';
					...
					if (IsFileExists(pData->m_szPath, true))
					{
						...
						if (IDYES
							!= ::MessageBox(pData->m_hwndOpenDlg, szText, LS(STR_DLGOPNFL3),
	...
	case WM_COMMAND:
		...
		case CBN_SELCHANGE:
			...
			case IDC_COMBO_CODE:
			{
				...
				{
					::EnableWindow(pData->m_hwndCheckBOM, TRUE);
					if (lRes == pData->m_nCharCode) { fCheck = pData->m_bBom ? BST_CHECKED : BST_UNCHECKED; }
					else
					{
			...
			case IDC_COMBO_OPENFOLDER:
			{
				...
				if (CB_ERR != Combo_GetLBText((HWND)lParam, nIdx, szWork))
				{
					...
					if (!::IsWindow(hwndFilebox))
						hwndFilebox =
							::GetDlgItem(pData->m_hwndOpenDlg, edt1); // ファイル名エディット（レガシータイプ）
					if (::IsWindow(hwndFilebox))
					{
...
/* 初期化 */
void CDlgOpenFile_CommonFileDialog::Create(HINSTANCE hInstance, HWND hwndParent, const WCHAR *pszUserWildCard,
										   ...
										   const std::vector<LPCWSTR> &vOPENFOLDER)
{
	...
	{ //現在編集中のファイルのパス	//@@@ 2002.04.18
		...
		if (!::GetLongFileName(p, m_szInitialDir)) { wcscpy(m_szInitialDir, p); }
	}
	m_vMRU		  = vMRU;
...
bool CDlgOpenFile_CommonFileDialog::DoModal_GetOpenFileName(WCHAR *pszPath, EFilter eAddFilter)
{
	...
	switch (eAddFilter)
	{
	case EFITER_TEXT: cFileExt.AppendExtRaw(LS(STR_DLGOPNFL_EXTNAME2), L"*.txt"); break;
	case EFITER_MACRO:
	...
	if (0 != wcscmp(m_szDefaultWildCard, L"*.*")) { cFileExt.AppendExtRaw(LS(STR_DLGOPNFL_EXTNAME3), L"*.*"); }
	...
	// 2005/02/20 novice デフォルトのファイル名は何も設定しない
	{
		...
		if (szName[0] == L'\0') { pszPath[0] = L'\0'; }
		else
		{
			WCHAR szRelPath[_MAX_PATH];
			auto_sprintf(szRelPath, L"%s%s%s%s", szDrive, szDir, szName, szExt);
			const WCHAR *p = szRelPath;
			if (!::GetLongFileName(p, pszPath)) { wcscpy(pszPath, p); }
		}
	}
	...
	if (_GetOpenFileNameRecover(&pData->m_ofn)) { return true; }
	else
	{
...
bool CDlgOpenFile_CommonFileDialog::DoModal_GetSaveFileName(WCHAR *pszPath)
{
	...
	if (GetSaveFileNameRecover(&pData->m_ofn)) { return true; }
	else
	{
		//	May 29, 2004 genta 関数にまとめた
		DlgOpenFail();
...
/*! 「開く」ダイアログ モーダルダイアログの表示
	...
	@date 2005.02.20 novice 拡張子を省略したら補完する
*/
bool CDlgOpenFile_CommonFileDialog::DoModalOpenDlg(SLoadInfo *pLoadInfo, std::vector<std::wstring> *pFileNames,
												   bool bOptions)
{
	std::unique_ptr<CDlgOpenFileData> pData(new CDlgOpenFileData());
	...
	for (int i = 0; i < GetDllShareData().m_nTypesCount; i++)
	{
		...
		if (!CDocTypeManager().GetTypeConfigMini(CTypeConfig(i), &type)) { continue; }
	...
	pData->m_nHelpTopicID =
		::FuncID_To_HelpContextID(F_FILEOPEN); // Stonee, 2001/05/18 機能番号からヘルプトピック番号を調べるようにした
	...
	pData->m_ofn.lpstrInitialDir = m_szInitialDir;
	pData->m_ofn.Flags = OFN_EXPLORER | OFN_CREATEPROMPT | OFN_FILEMUSTEXIST | OFN_ENABLETEMPLATE | OFN_ENABLEHOOK
						 | OFN_SHOWHELP | OFN_ENABLESIZING;
	if (pData->m_bViewMode) pData->m_ofn.Flags |= OFN_READONLY;
	if (bMultiSelect) { pData->m_ofn.Flags |= OFN_ALLOWMULTISELECT; }
	pData->m_ofn.lpstrDefExt = L""; // 2005/02/20 novice 拡張子を省略したら補完する
	if (bOptions == false)
	{
	...
	if (bDlgResult)
	{
		if (bMultiSelect)
		{
			...
			{ pFileNames->push_back(pData->m_ofn.lpstrFile); } else
			{
				std::wstring path;
...
*/
bool CDlgOpenFile_CommonFileDialog::DoModalSaveDlg(SSaveInfo *pSaveInfo, bool bSimpleMode)
{
	std::unique_ptr<CDlgOpenFileData> pData(new CDlgOpenFileData());
	...
	if (pSaveInfo->cFilePath[0] == L'\0') lstrcpyn(pSaveInfo->cFilePath, LS(STR_NO_TITLE2), _MAX_PATH); // 無題
	...
	pData->m_ofn.lpstrDefExt =
		(pData->m_ofn.Flags & OFN_ENABLEHOOK) ? NULL : L""; // 2006.11.10 ryoji フックを使うときは自前で拡張子を補完する
	...
	pData->m_nHelpTopicID = ::FuncID_To_HelpContextID(
		F_FILESAVEAS_DIALOG); // Stonee, 2001/05/18 機能番号からヘルプトピック番号を調べるようにした
	if (GetSaveFileNameRecover(&pData->m_ofn))
	{
		pSaveInfo->cFilePath = pData->m_ofn.lpstrFile;
		if (pData->m_ofn.Flags & OFN_ENABLEHOOK)
			lstrcpyn(pSaveInfo->cFilePath, pData->m_szPath,
					 _MAX_PATH); // 自前で拡張子の補完を行ったときのファイルパス	// 2006.11.10 ryoji

		pSaveInfo->eCharCode = pData->m_nCharCode;

		//	Feb. 9, 2001 genta
		if (pData->m_bUseEol) { pSaveInfo->cEol = pData->m_cEol; }
		//	Jul. 26, 2003 ryoji BOM設定
		if (pData->m_bUseBom) { pSaveInfo->bBomExist = pData->m_bBom; }
		return true;
	}
	else
	{
...
void CDlgOpenFile_CommonFileDialog::DlgOpenFail(void)
{
	...
	ErrorBeep();
	TopErrorMessage(m_hwndParent, LS(STR_DLGOPNFL_ERR1), pszError);
}
...
/*! 初期レイアウト設定処理
	...
	@param hwndBaseCtrl [in]
   移動基準コントロール（ファイル名ボックスと左端を合わせるコントロール）のウィンドウハンドル
...
void CDlgOpenFile_CommonFileDialog::InitLayout(HWND hwndOpenDlg, HWND hwndDlg, HWND hwndBaseCtrl)
{
	...
	if (!::IsWindow(hwndFilelabel = ::GetDlgItem(hwndOpenDlg, stc3))) // ファイル名ラベル
		return;
	if (!::IsWindow(hwndFilebox = ::GetDlgItem(hwndOpenDlg, cmb13)))
	{ // ファイル名コンボ（Windows 2000タイプ）
