...
HWND CDlgReplace::DoModeless(HINSTANCE hInstance, HWND hwndParent, LPARAM lParam, BOOL bSelected)
{
	...
	m_bConsecutiveAll =
		m_pShareData->m_Common.m_sSearch.m_bConsecutiveAll; // 「すべて置換」は置換の繰返し	// 2007.01.16 ryoji
...
void CDlgReplace::SetCombosList(void)
{
	...
	Combo_GetText(hwndCombo, &vText[0], nBufferSize);
	if (m_strText2.compare(&vText[0]) != 0) { ::DlgItem_SetText(GetHwnd(), IDC_COMBO_TEXT2, m_strText2.c_str()); }
}
...
int CDlgReplace::GetData(void)
{
	...
	m_pShareData->m_Common.m_sSearch.m_bConsecutiveAll =
		m_bConsecutiveAll; // 1==「すべて置換」は置換の繰返し	// 2007.01.16 ryoji
	m_pShareData->m_Common.m_sSearch.m_bSelectedArea = m_bSelectedArea; // 選択範囲内置換
	m_pShareData->m_Common.m_sSearch.m_bNOTIFYNOTFOUND =
	...
	/* 置換 ダイアログを自動的に閉じる */
	m_pShareData->m_Common.m_sSearch.m_bAutoCloseDlgReplace =
		::IsDlgButtonChecked(GetHwnd(), IDC_CHECK_bAutoCloseDlgReplace);
	...
	if (0 < m_strText.size())
	{
		...
		if (m_sSearchOption.bRegularExp && !CheckRegexpSyntax(m_strText.c_str(), GetHwnd(), true, nFlag)) { return -1; }
		...
		if (m_strText2.size() < _MAX_PATH) { CSearchKeywordManager().AddToReplaceKeyArr(m_strText2.c_str()); }
...
BOOL CDlgReplace::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_REPLACE_DIALOG)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
	...
	case IDC_CHK_REGULAREXP: /* 正規表現 */
		...
		if (::IsDlgButtonChecked(GetHwnd(), IDC_CHK_REGULAREXP))
		{
			...
			{ ::CheckDlgButton(GetHwnd(), IDC_CHK_REGULAREXP, 0); } else
			{
...
LPVOID CDlgReplace::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end
