...
const DWORD p_helpids[] = { // 12900
	IDOK, HIDOK_ABOUT, IDC_EDIT_ABOUT, HIDC_ABOUT_EDIT_ABOUT,
...
#define VER_GITHASH "(GitHash " GIT_COMMIT_HASH ")"
	...
	// 1行目
	cmemMsg.AppendStringF(L"v%d.%d.%d.%d", HIWORD(dwVersionMS), LOWORD(dwVersionMS), HIWORD(dwVersionLS),
						  LOWORD(dwVersionLS) // e.g. {2, 3, 2, 0}
	);
	...
	cmemMsg.AppendStringF(_T(COMPILER_TYPE)
							  _T(TARGET_M_SUFFIX) L"%d " TSTR_TARGET_MODE L" WIN%03x/I%03x/C%03x/N%03x\r\n",
						  COMPILER_VER, WINVER, _WIN32_IE, MY_WIN32_WINDOWS, MY_WIN32_WINNT);

	// 更新日情報
	CFileTime cFileTime;
	GetLastWriteTimestamp(szFile, &cFileTime);
	cmemMsg.AppendStringF(L"      Last Modified: %d/%d/%d %02d:%02d:%02d\r\n", cFileTime->wYear, cFileTime->wMonth,
						  cFileTime->wDay, cFileTime->wHour, cFileTime->wMinute, cFileTime->wSecond);
...
LPVOID CDlgAbout::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
...
BOOL CUrlWnd::SetSubclassWindow(HWND hWnd)
{
	...
	if (m_hFont != NULL) SendMessageAny(hWnd, WM_SETFONT, (WPARAM)m_hFont, (LPARAM)FALSE);
...
LRESULT CALLBACK CUrlWnd::UrlWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DESTROY:
		...
		if (pUrlWnd->m_hFont != NULL) DeleteObject(pUrlWnd->m_hFont);
		pUrlWnd->m_hWnd		  = NULL;
...
bool CUrlWnd::OnSetText(_In_opt_z_ LPCWSTR pchText, _In_opt_ size_t cchText) const
{
	...
	auto retSetPos =
		::SetWindowPos(GetHwnd(), NULL, 0, 0, size.cx, size.cy, SWP_NOMOVE | SWP_NOZORDER | SWP_NOOWNERZORDER);
