...
INT_PTR CDialog::DoModal(HINSTANCE hInstance, HWND hwndParent, int nDlgTemplete, LPARAM lParam)
{
	...
	return ::DialogBoxParam(m_hLangRsrcInstance, MAKEINTRESOURCE(nDlgTemplete), m_hwndParent, MyDialogProc,
							(LPARAM)this);
}
...
HWND CDialog::DoModeless(HINSTANCE hInstance, HWND hwndParent, int nDlgTemplete, LPARAM lParam, int nCmdShow)
{
	...
	m_lParam	 = lParam;
	m_hLangRsrcInstance = CSelectLang::getLangRsrcInstance(); // メッセージリソースDLLのインスタンスハンドル
	m_hWnd = ::CreateDialogParam(m_hLangRsrcInstance, MAKEINTRESOURCE(nDlgTemplete), m_hwndParent, MyDialogProc,
								 (LPARAM)this);
	if (NULL != m_hWnd) { ::ShowWindow(m_hWnd, nCmdShow); }
	return m_hWnd;
}
...
BOOL CDialog::OnBnClicked(int wID)
{
	...
	case IDOK: CloseDialog(wID); return TRUE;
	}
	return FALSE;
}

BOOL CDialog::OnSize() { return CDialog::OnSize(0, 0); }
...
{
	/* サイズボックス */
	m_hwndSizeBox = ::CreateWindowEx(WS_EX_CONTROLPARENT,								 /* no extended styles */
...
INT_PTR CDialog::DispatchEvent(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE: m_hWnd = hwndDlg; return OnSize(wParam, lParam);
	case WM_MOVE: m_hWnd = hwndDlg; return OnMove(wParam, lParam);
...
BOOL CDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if (wID == IDOK || wID == IDCANCEL) { return OnBnClicked(wID); }
	...
	if (hwndCtl)
	{
		...
		else if (::lstrcmpi(szClass, L"ComboBox") == 0)
		{
			...
			case CBN_DROPDOWN:
				return OnCbnDropDown(hwndCtl, wID);
...
BOOL CDialog::OnPopupHelp(WPARAM wPara, LPARAM lParam)
{
	...
	MyWinHelp((HWND)p->hItemHandle, HELP_WM_HELP,
			  (ULONG_PTR)GetHelpIdTable()); // 2006.10.10 ryoji MyWinHelpに変更に変更
	return TRUE;
}
...
LPVOID CDialog::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end
...
BOOL CDialog::OnCbnDropDown(HWND hwndCtl, int wID) { return OnCbnDropDown(hwndCtl, false); }
/** コンボボックスのドロップダウン時処理
...
BOOL CDialog::OnCbnDropDown(HWND hwndCtl, bool scrollBar)
{
	...
	for (iItem = 0; iItem < nItem; iItem++)
	{
		...
		if (0 < nTextLen)
		{
			...
			{
				if (nWidth < sizeText.cx + nScrollWidth) nWidth = sizeText.cx + nScrollWidth;
			}
			delete[] pszText;
		}
	}
...
void CDialog::ResizeItem(HWND hTarget, const POINT &ptDlgDefault, const POINT &ptDlgNew, const RECT &rcItemDefault,
						 EAnchorStyle anchor, bool bUpdate)
{
	...
	::SetWindowPos(hTarget, NULL, pt.x, pt.y, width, height, SWP_NOOWNERZORDER | SWP_NOZORDER);
	if (bUpdate) { ::InvalidateRect(hTarget, NULL, TRUE); }
}
 ...
 */
static void DeleteRecentItem(HWND	  hwndCombo, //!< コンボボックスのハンドル
...
)
{
	...
	if (Combo_GetLBText(hwndCombo, nIndex, cItemText))
	{
		...
		if (0 <= nRecentIndex) { pRecent->DeleteItem(nRecentIndex); }
	}
}
...
LRESULT CALLBACK SubEditProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		if (wParam == VK_DELETE)
		{
			...
			if (bShow && 0 <= nIndex) { DeleteRecentItem(hwndCombo, nIndex, data->pRecent); }
		}
		break;
	}
	case WM_DESTROY:
