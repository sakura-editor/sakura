...
INT_PTR CDialog::DispatchEvent(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_ACTIVATE:
            return OnActivate(wParam, lParam); //@@@ 2003.04.08 MIK
        case WM_VKEYTOITEM:
            return OnVKeyToItem(wParam, lParam);
        case WM_CHARTOITEM:
            return OnCharToItem(wParam, lParam);
        case WM_HELP:
            return OnPopupHelp(wParam, lParam); //@@@ 2002.01.18 add
        case WM_CONTEXTMENU:
            return OnContextMenu(wParam, lParam); //@@@ 2002.01.18 add
    }
    return FALSE;
}
...
BOOL CDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
    ...
    if (hwndCtl)
    {
        ...
        else if (::lstrcmpi(szClass, L"Edit") == 0)
        {
            ...
            {
                case EN_CHANGE:
                    return OnEnChange(hwndCtl, wID);
                case EN_KILLFOCUS:
                    return OnEnKillFocus(hwndCtl, wID);
            }
        }
        else if (::lstrcmpi(szClass, L"ListBox") == 0)
        {
            ...
            {
                case LBN_SELCHANGE:
                    return OnLbnSelChange(hwndCtl, wID);
                case LBN_DBLCLK:
