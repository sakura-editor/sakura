...
)
{
	...
	while( fgets( szLineData, _countof( szLineData ), fp ) )
	{
		...
		if ('!' == szLineData[0]) {
			if (0 == strcmp_literal(szLineData + 1, "_TAG_")) {
				int  nRet;
				s[0][0] = s[1][0] = s[2][0] = 0;
				nRet = sscanf(
					szLineData,
				...
				if (0 == strcmp_literal(pTag, "FILE_FORMAT")) {
					n2 = atoi(s[1]);
					if (1 <= n2 && n2 <= 2) {
						*nTagFormat = n2;
					}
				}
				else if (0 == strcmp_literal(pTag, "FILE_SORTED")) {
					n2 = atoi(s[1]);
					*bSorted = (1 == n2);
					*bFoldcase = (2 == n2);
				...
				else if (0 == strcmp_literal(pTag, "S_SEARCH_NEXT")) {
				...
				else if (0 == strcmp_literal(pTag, "S_FILE_BASEDIR")) {
					WCHAR baseWork[1024];
					// 独自拡張:ファイル名の基準ディレクトリ
					if (state->m_bJumpPath) {
