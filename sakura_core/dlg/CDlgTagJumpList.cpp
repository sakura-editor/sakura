...
const DWORD p_helpids[] = {
    ...
    IDOK,
    HIDC_TAGJUMPLIST_IDOK, //OK
    IDCANCEL,
    HIDC_TAGJUMPLIST_IDCANCEL, //キャンセル
    IDC_BUTTON_HELP,
    HIDC_BUTTON_TAGJUMPLIST_HELP, //ヘルプ
    IDC_KEYWORD,
    HDIC_TAGJUMPLIST_KEYWORD, //キーワード
    IDC_CHECK_ICASE,
    HIDC_CHECK_ICASE,
    IDC_CHECK_ANYWHERE,
    HIDC_CHECK_ANYWHERE,
...
static const WCHAR *p_extentions[] = {
    ...
    L"f=function",
    /*beta*/ L"bet",
    L"f=fragment,p=pattern,s=slot,v=virtual",
    /*c*/ L"c,h",
    L"c=class,d=macro,e=enumerator,f=function,g=enum,m=member,n=namespace,p=prototype,s=struct,t=typedef,u=union,v=variable,x=externvar",
    ...
    L"c=class,d=macro,e=enumerator,f=function,g=enum,m=member,n=namespace,p=prototype,s=struct,t=typedef,u=union,v=variable,x=externvar",
    /*cobol*/ L"cbl,cob",
    L"d=data,f=file,g=group,p=paragraph,P=program,s=section",
    /*eiffel*/ L"e",
    L"c=class,f=feature,l=local",
    /*fortran*/ L"f,for,ftn,f77,f90,f95",
    L"b=block data,c=common,e=entry,f=function,i=interface,k=component,l=label,L=local,m=module,n=namelist,p=program,s=subroutine,t=type,v=variable",
    /*lisp*/ L"cl,clisp,el,l,lisp,lsp,ml",
    L"f=function",
    /*lua*/ L"lua",
    L"f=function",
    /*makefile*/ L"mak",
    L"m=macro",
    /*pascal*/ L"p,pas",
    L"f=function,p=procedure",
    /*perl*/ L"pl,pm,perl",
    L"s=subroutine,p=package",
    /*php*/ L"php,php3,phtml",
    L"c=class,f=function",
    /*python*/ L"py,python",
    L"c=class,f=function,m=member",
    /*rexx*/ L"cmd,rexx,rx",
    L"s=subroutine",
    /*ruby*/ L"rb",
    L"c=class,f=method,F=singleton method,m=mixin",
    /*scheme*/ L"sch,scheme,scm,sm",
    L"f=function,s=set",
    /*sh*/ L"sh,bsh,bash,ksh,zsh",
    L"f=function",
    /*slang*/ L"sl",
    L"f=function,n=namespace",
    /*sql*/ L"sql",
    L"c=cursor,d=prototype,f=function,F=field,l=local,P=package,p=procedure,r=record,s=subtype,t=table,T=trigger,v=variable",
    /*tcl*/ L"tcl,tk,wish,itcl",
    L"p=procedure,c=class,f=method",
    /*verilog*/ L"v",
    L"f=function,m=module,P=parameter,p=port,r=reg,t=task,v=variable,w=wire",
    /*vim*/ L"vim",
    L"f=function,v=variable",
    /*yacc*/ L"y",
    L"l=label",
...
bool CDlgTagJumpList::GetSelectedFullPathAndLine(WCHAR *fullPath, int count, int *lineNum, int *depth)
{
    ...
    {
        if (-1 == m_nIndex || m_nIndex >= m_pcList->GetCount())
            return false;
    }
    else
    {
...
int CDlgTagJumpList::SearchBestTag(void)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if ((nMatch1 == -1) && (_wcsicmp(lpPathInfo->szDriveSrc, lpPathInfo->szDriveDst) == 0) && (_wcsicmp(lpPathInfo->szPathSrc, lpPathInfo->szPathDst) == 0) && (_wcsicmp(lpPathInfo->szFileSrc, lpPathInfo->szFileDst) == 0))
        {
            //ファイル名まで一致
            nMatch1 = i;
        }

        if ((nMatch2 == -1) && (_wcsicmp(lpPathInfo->szDriveSrc, lpPathInfo->szDriveDst) == 0) && (_wcsicmp(lpPathInfo->szPathSrc, lpPathInfo->szPathDst) == 0))
        {
            //パス名まで一致
            nMatch2 = i;
        }

        if ((nMatch5 == -1) && (_wcsicmp(lpPathInfo->szPathSrc, lpPathInfo->szPathDst) == 0) && (_wcsicmp(lpPathInfo->szFileSrc, lpPathInfo->szFileDst) == 0) && (_wcsicmp(lpPathInfo->szExtSrc, lpPathInfo->szExtDst) == 0))
        {
            nMatch5 = i;
        }

        if ((nMatch6 == -1) && (_wcsicmp(lpPathInfo->szFileSrc, lpPathInfo->szFileDst) == 0) && (_wcsicmp(lpPathInfo->szExtSrc, lpPathInfo->szExtDst) == 0))
        {
            if ((lpPathInfo->nPathSrc >= lpPathInfo->nPathDst) && (_wcsicmp(&lpPathInfo->szPathSrc[lpPathInfo->nPathSrc - lpPathInfo->nPathDst], lpPathInfo->szPathDst) == 0))
            {
                nMatch6 = i;
            }
        }
        ...
        if ((nMatch3 == -1) && (_wcsicmp(lpPathInfo->szFileSrc, lpPathInfo->szFileDst) == 0) && (_wcsicmp(lpPathInfo->szExtSrc, lpPathInfo->szExtDst) == 0))
        {
            //ファイル名・拡張子が一致
            nMatch3 = i;
        }
...
void CDlgTagJumpList::FindNext(bool bNewFind)
{
    ...
    if (bNewFind)
    {
        ...
        if (-1 < m_psFind0Match->m_nDepth && (m_bTagJumpPartialMatch == m_bOldTagJumpPartialMatch || FALSE == m_bTagJumpPartialMatch) && (m_bTagJumpICase == m_bOldTagJumpICase || FALSE == m_bTagJumpICase) && 0 == wcsncmp(m_strOldKeyword.GetStringPtr(), szKey, m_strOldKeyword.GetStringLength()))
        {
    ...
    int nDefaultNextMode)
{
    ...
    for (; state.m_nDepth <= state.m_nLoop && state.m_nDepth < (_MAX_PATH / 2); state.m_nDepth++)
    {
        ...
        // 3 必ず次も検索
        if (0 == state.m_nNextMode)
            break;
        if (1 == state.m_nNextMode && 0 < state.m_nMatchAll)
            break;
        if (2 == state.m_nNextMode && bTagJumpExactMatch && 0 < state.m_nMatchAll)
            break;
    ...
    *n2                                   = 0;
    ...
    if (2 == nTagFormat)
    {
        ...
        if (nRet < 3)
            bRet = false;
        if (*n2 <= 0)
            bRet = false; //行番号不正(-excmd=nが指定されてないかも)
    }
    else
    {
        nRet = sscanf(
    ...
    int length)
{
    ...
    while (fgets(szLineData, _countof(szLineData), fp))
    {
        ...
        else if (0 < cmp)
        {
            ...
            //	2005.04.05 MIK バグ修正
            if ((!rule->bTagJumpICase) && bSorted && (!rule->bTagJumpPartialMatch))
                break;
            // 2010.07.21 Foldcase時も打ち切る。ただしtagsとサクラ側のソート順が同じでなければならない
            if (rule->bTagJumpICase && bFoldcase && (!rule->bTagJumpPartialMatch))
                break;
        }
    next_line:;
        szLineData[nLINEDATA_LAST_CHAR] = '\0';
...
int CDlgTagJumpList::CalcMaxUpDirectory(const WCHAR *p)
{
    ...
    if (loop < 0)
        loop = 0;
    if (loop > (_MAX_PATH / 2))
        loop = (_MAX_PATH / 2); //\A\B\C...のようなとき1フォルダで2文字消費するので...
    return loop;
}
...
/*!
	...
	@retval NULL   失敗
*/
WCHAR *CDlgTagJumpList::GetFullPathFromDepth(WCHAR *pszOutput, int count, WCHAR *basePath, const WCHAR *fileName, int depth)
{
