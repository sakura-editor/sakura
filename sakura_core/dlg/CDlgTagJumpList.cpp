...
const DWORD p_helpids[] = {IDC_LIST_TAGJUMP, HIDC_LIST_TAGJUMPLIST,		  //ファイル
						   ...
						   IDC_CHECK_ICASE, HIDC_CHECK_ICASE, IDC_CHECK_ANYWHERE, HIDC_CHECK_ANYWHERE,
						   IDC_BUTTON_NEXTTAG, HIDC_BUTTON_NEXTTAG, IDC_BUTTON_PREVTAG, HIDC_BUTTON_PREVTAG,
						   //	IDC_STATIC,				-1,
						   0, 0};
...
static const WCHAR *p_extentions[] = {
	...
	/*c*/ L"c,h",
	L"c=class,d=macro,e=enumerator,f=function,g=enum,m=member,n=namespace,p=prototype,s=struct,t=typedef,u=union,v="
	L"variable,x=externvar",
	/*c++*/ L"c++,cc,cp,cpp,cxx,h++,hh,hp,hpp,hxx",
	L"c=class,d=macro,e=enumerator,f=function,g=enum,m=member,n=namespace,p=prototype,s=struct,t=typedef,u=union,v="
	L"variable,x=externvar",
	/*java*/ L"java",
	L"c=class,d=macro,e=enumerator,f=function,g=enum,m=member,n=namespace,p=prototype,s=struct,t=typedef,u=union,v="
	...
	/*fortran*/ L"f,for,ftn,f77,f90,f95",
	L"b=block "
	L"data,c=common,e=entry,f=function,i=interface,k=component,l=label,L=local,m=module,n=namelist,p=program,s="
	L"subroutine,t=type,v=variable",
	...
	/*sql*/ L"sql",
	L"c=cursor,d=prototype,f=function,F=field,l=local,P=package,p=procedure,r=record,s=subtype,t=table,T=trigger,v="
	L"variable",
...
*/
int CDlgTagJumpList::DoModal(HINSTANCE hInstance, HWND hwndParent, LPARAM lParam)
{
...
void CDlgTagJumpList::SetData(void)
{
	...
	else
	{
		...
		{ Combo_AddString(hwndKey, cRecentTagJump.GetItemText(i)); } if (m_pszKeyword != NULL)
		{ ::DlgItem_SetText(GetHwnd(), IDC_KEYWORD, m_pszKeyword); } else if (cRecentTagJump.GetItemCount() > 0)
		{
...
BOOL CDlgTagJumpList::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_TAGJUMP_LIST)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
...
LPVOID CDlgTagJumpList::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
	...
	for (i = 0; p_extentions[i]; i += 2)
	{
		...
		while (token)
		{
			if (_wcsicmp(p, token) == 0)
			{
				...
				while (token)
				{
					if (token[0] == type) { return _wcsdup(&token[2]); }
...
int CDlgTagJumpList::SearchBestTag(void)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		if ((nMatch1 == -1) && (_wcsicmp(lpPathInfo->szDriveSrc, lpPathInfo->szDriveDst) == 0)
		...
		if ((nMatch2 == -1) && (_wcsicmp(lpPathInfo->szDriveSrc, lpPathInfo->szDriveDst) == 0)
			&& (_wcsicmp(lpPathInfo->szPathSrc, lpPathInfo->szPathDst) == 0))
		{
			//パス名まで一致
			nMatch2 = i;
		...
		if ((nMatch5 == -1) && (_wcsicmp(lpPathInfo->szPathSrc, lpPathInfo->szPathDst) == 0)
			&& (_wcsicmp(lpPathInfo->szFileSrc, lpPathInfo->szFileDst) == 0)
			&& (_wcsicmp(lpPathInfo->szExtSrc, lpPathInfo->szExtDst) == 0))
		{ nMatch5 = i; } if ((nMatch6 == -1) && (_wcsicmp(lpPathInfo->szFileSrc, lpPathInfo->szFileDst) == 0)
							 && (_wcsicmp(lpPathInfo->szExtSrc, lpPathInfo->szExtDst) == 0))
		{
			if ((lpPathInfo->nPathSrc >= lpPathInfo->nPathDst)
				&& (_wcsicmp(&lpPathInfo->szPathSrc[lpPathInfo->nPathSrc - lpPathInfo->nPathDst], lpPathInfo->szPathDst)
					== 0))
			{ nMatch6 = i; } }

		if ((nMatch7 == -1) && (_wcsicmp(lpPathInfo->szFileSrc, lpPathInfo->szFileDst) == 0))
		{
			if ((lpPathInfo->nPathSrc >= lpPathInfo->nPathDst)
				&& (_wcsicmp(&lpPathInfo->szPathSrc[lpPathInfo->nPathSrc - lpPathInfo->nPathDst], lpPathInfo->szPathDst)
					== 0))
		...
		if ((nMatch3 == -1) && (_wcsicmp(lpPathInfo->szFileSrc, lpPathInfo->szFileDst) == 0)
			&& (_wcsicmp(lpPathInfo->szExtSrc, lpPathInfo->szExtDst) == 0))
		{
			//ファイル名・拡張子が一致
			nMatch3 = i;
		...
		if ((nMatch4 == -1) && (_wcsicmp(lpPathInfo->szFileSrc, lpPathInfo->szFileDst) == 0))
		{
			//ファイル名が一致
			nMatch4 = i;
		}
...
void CDlgTagJumpList::find_key(const wchar_t *keyword)
{
	...
	find_key_core(m_nTop, keyword, FALSE != m_bTagJumpPartialMatch, FALSE != m_bTagJumpExactMatch,
				  FALSE != m_bTagJumpICase, IsDirectTagJump(),
				  IsDirectTagJump() ? (m_pShareData->m_Common.m_sSearch.m_nTagJumpMode)
									: m_pShareData->m_Common.m_sSearch.m_nTagJumpModeKeyword);
								   ...
								   int  nDefaultNextMode)
{
	...
	for (; state.m_nDepth <= state.m_nLoop && state.m_nDepth < (_MAX_PATH / 2); state.m_nDepth++)
	{
		...
		if (fp)
		{
			...
			// tagsファイルのパラメータを読みこみ
			bRet = ReadTagsParameter(fp, bTagJumpICaseByTags, &state, cList, &nTagFormat, &bSorted, &bFoldcase,
									 &bTagJumpICase, &szNextPath[0], &baseDirId);
			if (bRet)
			{
		...
		if (0 != state.m_nMatchAll && false == m_bNextItem)
		{
			...
			*m_psFindPrev = state;
			...
			DEBUG_TRACE(L"FindPrev udpate: d:%d m:%d n:%d L:%d j:%d\n", m_psFindPrev->m_nDepth,
						m_psFindPrev->m_nMatchAll, m_psFindPrev->m_nNextMode, m_psFindPrev->m_nLoop,
						(int)m_psFindPrev->m_bJumpPath);
		}
		if (0 == state.m_nMatchAll)
		{
			...
			*m_psFind0Match = state;
			...
			DEBUG_TRACE(L"Find0Match udpate: d:%d m:%d n:%d L:%d j:%d\n", m_psFind0Match->m_nDepth,
						m_psFind0Match->m_nMatchAll, m_psFind0Match->m_nNextMode, m_psFind0Match->m_nLoop,
						(int)m_psFind0Match->m_bJumpPath);
		}
		if (m_bNextItem) { break; }
	}
...
*/
bool CDlgTagJumpList::ReadTagsParameter(FILE *fp, bool bTagJumpICaseByTags, STagFindState *state,
										CSortedTagJumpList &cList, int *nTagFormat, bool *bSorted, bool *bFoldcase,
										bool *bTagJumpICase, PTCHAR szNextPath, int *baseDirId)
{
	...
	while (fgets(szLineData, _countof(szLineData), fp))
	{
		...
		if ('\0' != szLineData[nLINEDATA_LAST_CHAR] && '\n' != szLineData[nLINEDATA_LAST_CHAR]) { SkipLine(fp); }
		...
		if ('!' == szLineData[0])
		{
			if (0 == strncmp_literal(szLineData + 1, "_TAG_"))
			{
				...
				else if (0 == strncmp_literal(pTag, "S_SEARCH_NEXT"))
				{
					...
					{
						n2 = atoi(s[1]);
						if (0 <= n2 && n2 <= 3) { state->m_nNextMode = n2; }
						if (1 <= n2 && s[2][0])
						{
							// s[2] == 絶対パス(ディレクトリ)
							WCHAR baseWork[1024];
			...
			old_offset = ftell(fp);
			if (old_offset == -1)
			{ //読み取りエラー or ファイル終端に到達時はタグファイルとして扱わない
				return false;
			}
			continue;
		}
...
/*
	キーをtagsファイルから二分探索
*/
void CDlgTagJumpList::find_key_for_BinarySearch(FILE *fp, const ACHAR *paszKeyword, int nTagFormat,
												STagFindState *state, const STagSearchRule *rule)
{
	ACHAR				szLineData[1024]; //行バッファ
	ACHAR				s[4][1024];
	CSortedTagJumpList &cList = *m_pcList;
	...
	while (fgets(szLineData, _countof(szLineData), fp))
	{
		...
		if (!parseTagsLine(s, szLineData, &n2, nTagFormat)) { goto next_line; }
		...
		if (0 == cmp)
		{
			...
			else if (eSearchState == STATE_STEP_FORWARD)
			{
				...
				if ((rule->nTop) < (state->m_nMatchAll))
				{
					...
					{ cList.AddParamA(s[0], s[1], n2, s[2][0], s[3], state->m_nDepth, rule->baseDirId); } else
					{
						// 探索打ち切り(次ページでやり直し)
						m_bNextItem = true;
						break;
...
/*
	キーをtagsファイルから線形探索
*/
void CDlgTagJumpList::find_key_for_LinearSearch(FILE *fp, const ACHAR *paszKeyword, int nTagFormat,
												STagFindState *state, const STagSearchRule *rule, bool bSorted,
												bool bFoldcase, int length)
{
	ACHAR				szLineData[1024]; //行バッファ
	ACHAR				s[4][1024];
	CSortedTagJumpList &cList = *m_pcList;
...
*/
const WCHAR *CDlgTagJumpList::GetFileName(void) { return GetFileTitlePointer(GetFilePath()); }
...
/*!
	...
	@retval NULL   失敗
*/
WCHAR *CDlgTagJumpList::GetFullPathFromDepth(WCHAR *pszOutput, int count, WCHAR *basePath, const WCHAR *fileName,
											 int depth)
{
	...
	else
	{
		...
		if (-1 == auto_snprintf_s(pszOutput, count, L"%s%s", basePath, p)) { return NULL; }
	}
	return pszOutput;
}
