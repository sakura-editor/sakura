...
bool CDlgProfileMgr::TrySelectProfile(CCommandLine *pcCommandLine) noexcept
{
	...
	bool bDialog;
	if (pcCommandLine->IsProfileMgr())
	{ // コマンドラインでプロファイルマネージャの表示が指定されている
		bDialog = true;
	}
	else if (pcCommandLine->IsSetProfile())
	{ // コマンドラインでプロファイル名が指定されている
		bDialog = false;
	}
	else if (!bSettingLoaded)
	{ // プロファイル設定がなかった
		bDialog = false;
	}
...
{
	CDataProfile cProf;
	if (bWrite) { cProf.SetWritingMode(); }
	else
	{
