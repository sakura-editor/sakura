...
const DWORD p_helpids[] = {IDC_LIST_PROFILE,
						   HIDC_LIST_PROFILE, //プロファイル一覧
...
static std::wstring GetProfileMgrFileName(LPCWSTR profName = NULL)
{
	...
	if (bSet == false) {
		...
		if (szPath[0] == L'\0') { pszPath = szPath2; }
		bSet = true;
	}
...
/*! ダイアログデータの設定 */
void CDlgProfileMgr::SetData() { SetData(-1); }
...
void CDlgProfileMgr::SetData(int nSelIndex)
{
	...
	if (settings.m_nDefaultIndex == 0) { strdef += L"*"; }
...
template<size_t cchText> static bool MyList_GetText(HWND hwndList, int index, WCHAR (&szText)[cchText])
{
...
/*! ダイアログデータの取得 */
int CDlgProfileMgr::GetData() { return GetData(true); }
...
int CDlgProfileMgr::GetData(bool bStart)
{
	...
	if (m_strProfileName == L"(default)") { m_strProfileName = L""; }
	bool			 bDefaultSelect = IsDlgButtonCheckedBool(GetHwnd(), IDC_CHECK_PROF_DEFSTART);
	SProfileSettings settings;
	ReadProfSettings(settings);
	bool bWrtie = false;
	if (settings.m_bDefaultSelect != bDefaultSelect) { bWrtie = true; }
...
{
	switch (wID) {
	case IDC_BUTTON_PROF_CREATE: CreateProf(); break;

	case IDC_BUTTON_PROF_RENAME: RenameProf(); break;

	case IDC_BUTTON_PROF_DELETE: DeleteProf(); break;
...
void CDlgProfileMgr::UpdateIni()
{
	...
	for (int i = 0; i < nCount; i++) {
		...
		if (MyList_GetText(hwndList, i, szProfileName)) { settings.m_nDefaultIndex = i; }
		if (0 < i) {
	...
	if (!WriteProfSettings(settings)) { ErrorMessage(GetHwnd(), LS(STR_DLGPROFILE_ERR_WRITE)); }
}
...
static bool IsProfileDuplicate(HWND hwndList, LPCWSTR szProfName, int skipIndex)
{
	...
	for (int i = 0; i < nCount; i++) {
		...
		if (0 == wmemicmp(szProfName, szProfileName)) { return true; }
	}
	return false;
}
...
void CDlgProfileMgr::CreateProf()
{
	...
	szText[0]				= L'\0';
	if (!cDlgInput1.DoModal(::GetModuleHandle(NULL), GetHwnd(), strTitle.c_str(), strMessage.c_str(), max_size,
							szText)) {
		return;
	}
	if (szText[0] == L'\0') { return; }
...
void CDlgProfileMgr::DeleteProf()
{
	...
	UpdateIni();
	if (List_GetCount(hwndList) <= nCurIndex) { nCurIndex--; }
	SetData(nCurIndex);
}
...
static bool IOProfSettings(SProfileSettings &settings, bool bWrite)
{
	...
	if (!bWrite) {
		if (!cProf.ReadProfile(strIniName.c_str())) { return false; }
	}
	...
	if (nCount < settings.m_nDefaultIndex) { settings.m_nDefaultIndex = -1; }
	if (settings.m_nDefaultIndex < -1) { settings.m_nDefaultIndex = -1; }
	cProf.IOProfileData(pSection, L"szDllLanguage",
						StringBufferW(settings.m_szDllLanguage, _countof(settings.m_szDllLanguage)));
	cProf.IOProfileData(pSection, L"bDefaultSelect", settings.m_bDefaultSelect);

	if (bWrite) {
		if (!cProf.WriteProfile(strIniName.c_str(), L"Sakura Profile ini")) { return false; }
	}
	return true;
}
...
bool CDlgProfileMgr::WriteProfSettings(SProfileSettings &settings) { return IOProfSettings(settings, true); }

LPVOID CDlgProfileMgr::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
