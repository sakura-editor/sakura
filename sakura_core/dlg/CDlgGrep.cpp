...
static bool IsEscapeRequiredForExcludePattern(const std::wstring &pattern)
{
	...
	if (pattern.find(L'!') != NotFound) { return true; }
	if (pattern.find(L'#') != NotFound) { return true; }
	if (pattern.find(L'\x20') != NotFound) { return true; }
	if (pattern.find(L';') != NotFound) { return true; }
	return false;
}
...
BOOL CDlgGrep::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_GREP_DIALOG)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
	...
	case IDC_CHK_DEFAULTFOLDER:
		...
		{
			m_pShareData->m_Common.m_sSearch.m_bGrepDefaultFolder =
...
void CDlgGrep::SetData(void)
{
	...
	/* 文字コードセット */
	{
		...
		else
		{
			...
			if (nCurIdx == -1) { Combo_SetCurSel(hWndCombo, 0); }
		}
	}
...
int CDlgGrep::GetData(void)
{
	...
	{ m_nGrepOutputStyle = 3; } m_bGrepOutputFileOnly = IsDlgButtonCheckedBool(GetHwnd(), IDC_CHECK_FILE_ONLY);
	...
	if (0 < m_strText.size())
	{
		...
		{ return FALSE; } // To Here Jun. 26, 2001 genta 正規表現ライブラリ差し替え
...
LPVOID CDlgGrep::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end
