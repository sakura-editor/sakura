...
const DWORD p_helpids[] = {									 // 13700
	...
	IDOK, HIDC_TAG_MAKE_IDOK, IDCANCEL, HIDC_TAG_MAKE_IDCANCEL, IDC_BUTTON_HELP, HIDC_BUTTON_TAG_MAKE_HELP,
	//	IDC_STATIC,						-1,
	0, 0};
...
/* モーダルダイアログの表示 */
int CDlgTagsMake::DoModal(HINSTANCE hInstance, HWND hwndParent, LPARAM lParam,
...
BOOL CDlgTagsMake::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_TAGS_MAKE)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
	...
	case IDC_BUTTON_FOLDER_UP:
	{
		...
		if (DirectoryUp(szDir)) { ::SetWindowText(hwnd, szDir); }
	}
		return TRUE;
...
LPVOID CDlgTagsMake::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
