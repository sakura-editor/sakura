...
const DWORD p_helpids[] = { //13700
    ...
    HIDC_EDIT_TAG_MAKE_FOLDER, //タグ作成フォルダ
    IDC_BUTTON_TAG_MAKE_REF,
    HIDC_BUTTON_TAG_MAKE_REF, //参照
    IDC_BUTTON_FOLDER_UP,
    HIDC_BUTTON_TAG_MAKE_FOLDER_UP, // 上
    IDC_EDIT_TAG_MAKE_CMDLINE,
    HIDC_EDIT_TAG_MAKE_CMDLINE, //コマンドライン
    IDC_CHECK_TAG_MAKE_RECURSE,
    HIDC_CHECK_TAG_MAKE_RECURSE, //サブフォルダも対象
    IDOK,
    HIDC_TAG_MAKE_IDOK,
    IDCANCEL,
    HIDC_TAG_MAKE_IDCANCEL,
    IDC_BUTTON_HELP,
    HIDC_BUTTON_TAG_MAKE_HELP,
    //	IDC_STATIC,						-1,
    0,
    0};
...
void CDlgTagsMake::SetData(void)
{
    ...
    if (m_nTagsOpt & 0x0001)
        ::CheckDlgButton(GetHwnd(), IDC_CHECK_TAG_MAKE_RECURSE, TRUE);
...
int CDlgTagsMake::GetData(void)
{
    ...
    if (length > 0)
    {
        if (m_szPath[length - 1] != L'\\')
            wcscat(m_szPath, L"\\");
    }

    //CTAGSオプション
    m_nTagsOpt = 0;
    if (::IsDlgButtonChecked(GetHwnd(), IDC_CHECK_TAG_MAKE_RECURSE) == BST_CHECKED)
        m_nTagsOpt |= 0x0001;
