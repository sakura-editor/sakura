...
const DWORD p_helpids[] = {
    ...
    IDC_LIST_FAVORITE_FILE,
    HIDC_LIST_FAVORITE_FILE, //ファイル
    IDC_LIST_FAVORITE_FOLDER,
    HIDC_LIST_FAVORITE_FOLDER, //フォルダ
    IDC_LIST_FAVORITE_EXCEPTMRU,
    HIDC_LIST_FAVORITE_EXCEPTMRU, //MRU除外
    IDC_LIST_FAVORITE_SEARCH,
    HIDC_LIST_FAVORITE_SEARCH, //検索
    IDC_LIST_FAVORITE_REPLACE,
    HIDC_LIST_FAVORITE_REPLACE, //置換
    IDC_LIST_FAVORITE_GREP_FILE,
    HIDC_LIST_FAVORITE_GREPFILE, //GREPファイル
    IDC_LIST_FAVORITE_GREP_FOLDER,
    HIDC_LIST_FAVORITE_GREPFOLDER, //GREPフォルダ
    IDC_LIST_FAVORITE_CMD,
    HIDC_LIST_FAVORITE_CMD, //コマンド
    IDC_LIST_FAVORITE_CUR_DIR,
    HIDC_LIST_FAVORITE_CUR_DIR, //カレントディレクトリ
    //	IDC_STATIC_BUTTONS,				-1,
    IDC_BUTTON_CLEAR,
    HIDC_BUTTON_FAVORITE_CLEAR, //すべて	// 2006.10.10 ryoji
    IDC_BUTTON_DELETE_NOFAVORATE,
    HIDC_BUTTON_FAVORITE_DELETE_NOFAVORATE, //お気に入り以外
    IDC_BUTTON_DELETE_NOTFOUND,
    HIDC_BUTTON_FAVORITE_DELETE_NOTFOUND, //存在しない項目
    IDC_BUTTON_DELETE_SELECTED,
    HIDC_BUTTON_FAVORITE_DELETE_SELECTED, //選択項目
    IDC_BUTTON_ADD_FAVORITE,
    HIDC_BUTTON_ADD_FAVORITE, // 追加
    IDOK,
    HIDC_FAVORITE_IDOK, //閉じる
    IDC_BUTTON_HELP,
    HIDC_BUTTON_FAVORITE_HELP, //ヘルプ
...
BOOL CDlgFavorite::OnInitDialog(HWND hwndDlg, WPARAM wParam, LPARAM lParam)
{
    ...
    const int nListViewWidthClient = rc.right - rc.left - CTextWidthCalc::WIDTH_MARGIN_SCROLLBER - ::GetSystemMetrics(SM_CXVSCROLL);
    // 初期値は従来方式の%指定
...
bool CDlgFavorite::RefreshList(void)
{
    ...
    for (nTab = 0; NULL != m_aFavoriteInfo[nTab].m_pRecent; nTab++)
    {
        ...
        if (bret == true)
        {
            ...
            if (msg[0] != L'\0')
                wcscat(msg, LS(STR_DLGFAV_DELIMITER));
...
bool CDlgFavorite::RefreshListOne(int nIndex)
{
    ...
    if (-1 == nCurrentIndex)
        nCurrentIndex = ListView_GetNextItem(hwndList, -1, LVNI_FOCUSED);

    if (nItemCount != nCount)
        goto changed; //個数が変わったので再構築

    //お気に入り数が変わったので再構築
    if (m_aFavoriteInfo[nIndex].m_nViewCount != pRecent->GetViewCount())
        goto changed;
    ...
    for (i = 0; i < nCount; i++)
    {
        ...
        if (!bret)
            goto changed; //エラーなので再構築

        //アイテム内容が変わったので再構築
        if (lvitem.lParam != pRecent->FindItemByText(szText))
            goto changed;
    }
...
void CDlgFavorite::UpdateUIState()
{
    ...
    DlgItem_Enable(GetHwnd(), IDC_BUTTON_ADD_FAVORITE, m_aFavoriteInfo[m_nCurrentTab].m_bEditable && recent.GetItemCount() <= recent.GetArrayCount());

    // 削除の有効・無効化
    DlgItem_Enable(GetHwnd(), IDC_BUTTON_CLEAR, 0 < recent.GetItemCount());

    DlgItem_Enable(GetHwnd(), IDC_BUTTON_DELETE_NOFAVORATE, m_aFavoriteInfo[m_nCurrentTab].m_bHaveFavorite && 0 < recent.GetItemCount());

    DlgItem_Enable(GetHwnd(), IDC_BUTTON_DELETE_NOTFOUND, m_aFavoriteInfo[m_nCurrentTab].m_bFilePath && 0 < recent.GetItemCount());

    DlgItem_Enable(GetHwnd(), IDC_BUTTON_DELETE_SELECTED, 0 < recent.GetItemCount());
}
