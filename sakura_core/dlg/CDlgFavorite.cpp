...
CDlgFavorite::~CDlgFavorite()
{
	for (int nTab = 0; m_aFavoriteInfo[nTab].m_pRecent; nTab++) { m_aFavoriteInfo[nTab].m_pRecent->Terminate(); }
}

/* モーダルダイアログの表示 */
int CDlgFavorite::DoModal(HINSTANCE hInstance, HWND hwndParent, LPARAM lParam)
{
...
void CDlgFavorite::SetData(void)
{
	...
	for (nTab = 0; NULL != m_aFavoriteInfo[nTab].m_pRecent; nTab++) { SetDataOne(nTab, 0); }
...
void CDlgFavorite::SetDataOne(int nIndex, int nLvItemIndex)
{
	...
	if (-1 != nLvItemIndex && nLvItemIndex < nItemCount) { nNewFocus = nLvItemIndex; }

	//アイテムがあってどれも非選択なら、要求に近いアイテム(先頭か末尾)を選択
	if (nItemCount > 0 && -1 != nLvItemIndex && nNewFocus == -1)
	{ nNewFocus = (0 < nLvItemIndex ? nItemCount - 1 : 0); } if (-1 != nNewFocus)
	{
...
BOOL CDlgFavorite::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_FAVORITE)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
	...
	case IDC_BUTTON_DELETE_NOTFOUND:
	{
		...
		if (m_aFavoriteInfo[m_nCurrentTab].m_bFilePath)
		{
			...
			if (IDYES == nRet && pRecent)
			{
				...
				for (int i = pRecent->GetItemCount() - 1; i >= 0; i--)
				{
					...
					if (false == IsFileExists(szPath, false)) { pRecent->DeleteItem(i); }
				}
				pRecent->UpdateView();
				RefreshListOne(m_nCurrentTab);
				UpdateUIState();
		...
		return TRUE;
	//選択項目の削除
	case IDC_BUTTON_DELETE_SELECTED: { DeleteSelected();
	}
		return TRUE;
	case IDC_BUTTON_ADD_FAVORITE: { AddItem();
	}
		return TRUE;
	}
	...
	else
	{
		...
		if (hwndList == lpnmhdr->hwndFrom)
		{
			...
			case LVN_COLUMNCLICK:
				ListViewSort(m_aListViewInfo[m_nCurrentTab], m_aFavoriteInfo[m_nCurrentTab].m_pRecent, pnlv->iSubItem,
							 true);
				return TRUE;
			...
			case LVN_KEYDOWN:
				...
				if ((wKey == VK_NEXT && nIdx == _CTRL))
				{
					...
					if (_countof(m_aFavoriteInfo) - 1 <= next) { next = 0; }
...
void CDlgFavorite::TabSelectChange(bool bSetFocus)
{
	...
	if (-1 != nIndex)
	{
		...
		if (bSetFocus) { ::SetFocus(hwndList); }
...
LPVOID CDlgFavorite::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
...
void CDlgFavorite::AddItem()
{
	...
	{ return; } GetFavorite(m_nCurrentTab);
...
void CDlgFavorite::RightMenu(POINT &menuPos)
{
	...
	::InsertMenu(hMenu, iPos++, MF_BYPOSITION | MF_STRING | nEnable, MENU_DELETE_NOFAVORATE,
				 LS(STR_DLGFAV_MENU_DEL_NOTFAV));
	nEnable = (m_aFavoriteInfo[m_nCurrentTab].m_bFilePath && 0 < recent.GetItemCount() ? 0 : MF_GRAYED);
	::InsertMenu(hMenu, iPos++, MF_BYPOSITION | MF_STRING | nEnable, MENU_DELETE_NOTFOUND,
				 LS(STR_DLGFAV_MENU_DEL_INVALID));
	nEnable = (0 < recent.GetItemCount() ? 0 : MF_GRAYED);
	...
	case MENU_ADD_EXCEPT:
	{
		...
		if (pRecent)
		{
			...
			if (0 < nSelectedCount)
			{
				...
				if (bAddFalse)
				{
					WarningMessage(GetHwnd(),
								   LS(STR_DLGFAV_LIST_LIMIT_OVER)); // "除外リストがいっぱいで追加できませんでした。"
				}
	...
	break;
	case MENU_ADD_NEW: AddItem(); break;
	case MENU_DELETE_ALL: OnBnClicked(IDC_BUTTON_CLEAR); break;
	case MENU_DELETE_NOFAVORATE: OnBnClicked(IDC_BUTTON_DELETE_NOFAVORATE); break;
	case MENU_DELETE_NOTFOUND: OnBnClicked(IDC_BUTTON_DELETE_NOTFOUND); break;
	case MENU_DELETE_SELECTED: OnBnClicked(IDC_BUTTON_DELETE_SELECTED); break;
	}
}
...
static int ListView_GetLParamInt(HWND hwndList, int lvIndex)
{
	...
	lvitem.iSubItem = 0;
	if (ListView_GetItem(hwndList, &lvitem)) { return (int)lvitem.lParam; }
	return -1;
}
