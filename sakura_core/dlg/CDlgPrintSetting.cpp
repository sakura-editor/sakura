    ...
    switch (wID)
    {
        ...
        case IDC_BUTTON_FONT_HEAD:
        {
            ...
            if (MySelectFont(&lf, &nPointSize, GetHwnd(), false))
            {
                ...
                SetFontName(IDC_STATIC_FONT_HEAD, IDC_CHECK_USE_FONT_HEAD, m_PrintSettingArr[m_nCurrentPrintSetting].m_lfHeader, m_PrintSettingArr[m_nCurrentPrintSetting].m_nHeaderPointSize);
                UpdatePrintableLineAndColumn();
            }
        }
            return TRUE;
        case IDC_BUTTON_FONT_FOOT:
        {
            ...
            if (MySelectFont(&lf, &nPointSize, GetHwnd(), false))
            {
                ...
                SetFontName(IDC_STATIC_FONT_FOOT, IDC_CHECK_USE_FONT_FOOT, m_PrintSettingArr[m_nCurrentPrintSetting].m_lfFooter, m_PrintSettingArr[m_nCurrentPrintSetting].m_nFooterPointSize);
                UpdatePrintableLineAndColumn();
            }
        }
            return TRUE;
...
void CDlgPrintSetting::OnChangeSettingType(BOOL bGetData)
{
    ...
    // カラー印刷
    ::CheckDlgButton(GetHwnd(), IDC_CHECK_COLORPRINT, m_PrintSettingArr[m_nCurrentPrintSetting].m_bColorPrint ? BST_CHECKED : BST_UNCHECKED);
...
void CDlgPrintSetting::SetFontName(int idTxt, int idUse, LOGFONT &lf, int nPointSize)
{
    ...
    if (bUseFont)
    {
        ...
        auto_sprintf(szName, nPointSize % 10 ? L"%.32s(%.1fpt/%d.%dmm)" : L"%.32s(%.0fpt/%d.%dmm)", lf.lfFaceName, double(nPointSize) / 10, nMM / 10, nMM / 10);
    }
    else
    {
        szName[0] = L'\0';
