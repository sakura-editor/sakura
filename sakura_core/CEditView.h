...
public:
	...
	bool IsSpecialScrollMode( int );					/* キー・マウスボタン状態よりスクロールモードを判定する */		// 2009.01.12 nasukoji
...
protected:
	bool IsCurrentPositionURL( const CLayoutPoint& ptCaretPos, CLogicRange* pUrlRange, std::tstring* ptstrURL );/* カーソル位置にURLが有る場合のその範囲を調べる */
...
//2004.10.13 インクリメンタルサーチ関係
	...
	void Command_EXECEXTMACRO( const char* path, const char* type );	/* 名前を指定してマクロ実行 */
