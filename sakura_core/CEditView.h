...
public:
	...
	DWORD DoGrep( const CNativeW2*, const CNativeT*, const CNativeT*, BOOL, const SSearchOption&, ECodeType, BOOL, int );	/* Grep実行 */
	...
	int DoGrepFile( CDlgCancel*, HWND, const wchar_t*, int*, const TCHAR*, const SSearchOption&, ECodeType, BOOL, int, CBregexp*, int*, const TCHAR*, CNativeW2& );
	/* Grep結果をpszWorkに格納 */
	void SetGrepResult(
		/* データ格納先 */
		wchar_t*	pWork,
...
// 2002/01/19 novice public属性に変更
	BOOL GetSelectedData( CNativeW2*, BOOL, const wchar_t*, BOOL, BOOL bAddCRLFWhenCopy, enumEOLType neweol = EOL_UNKNOWN);/* 選択範囲のデータを取得 */
	//	Aug. 25, 2002 genta protected->publicに移動
	bool IsImeON( void );	// IME ONか	// 2006.12.04 ryoji
	int HokanSearchByFile( const wchar_t*, BOOL, CNativeW2**, int, int ); // 2003.06.25 Moca
	...
	*/
	...
	BOOL KeySearchCore( const CNativeW2* pcmemCurText );	// 2006.04.10 fon
...
public:
	...
	||  実装ヘルパ関数
	*/
	void GetCurrentTextForSearch( CNativeW2& );			/* 現在カーソル位置単語または選択範囲より検索等のキーを取得 */
	void GetCurrentTextForSearchDlg( CNativeW2& );		/* 現在カーソル位置単語または選択範囲より検索等のキーを取得（ダイアログ用） 2006.08.23 ryoji */
public:
//	CEOL GetCurrentInsertEOL( void );					/* 現在、Enterなどで挿入する改行コードの種類を取得 */
	...
	//取得
	BOOL MyGetClipboardData( CNativeW2&, BOOL*, BOOL* = NULL );			/* クリップボードからデータを取得 */
...
public:
	...
	void ReplaceData_CEditView(
		...
		CNativeW2*		pcmemCopyOfDeleted,	// 削除されたデータのコピー(NULL可能)
...
//2004.10.13 インクリメンタルサーチ関係
	...
	void ShowHokanMgr( CNativeW2& cmemData, BOOL bAutoDecided );	//	補完ウィンドウを表示する。Ctrl+Spaceや、文字の入力/削除時に呼び出されます。 YAZAKI 2002/03/11
