...
//	::ExtTextOutW2( hdc, 0, 0, ETO_OPAQUE, &rc, NULL, 0, NULL );
//	return;
//}
...
BOOL CEditView::KeyWordHelpSearchDict( LID_SKH nID, POINT* po, RECT* rc )
{
	...
	if( CNativeW2::IsEqual( cmemCurText, m_cTipWnd.m_cKey ) &&	/* 既に検索済みか */
...
*/
BOOL CEditView::KeySearchCore( const CNativeW2* pcmemCurText )
{
	CNativeW2*	pcmemRefKey;
		...
		if( 1 == m_pShareData->m_Types[nTypeNo].m_KeyHelpArr[i].m_nUse ){
			...
			if(nSearchResult){
				...
				if(m_pcEditDoc->GetDocumentAttribute().m_bUseKeyHelpAllSearch){	/* ヒットした次の辞書も検索 */	// 2006.04.10 fon
					...
					m_cTipWnd.m_cInfo.AppendStringW2( pszWork );
					delete pcmemRefText;
...
*/
BOOL CEditView::MyGetClipboardData( CNativeW2& cmemBuf, BOOL* pbColmnSelect, BOOL* pbLineSelect /*= NULL*/ )
{
	if(pbColmnSelect)
		*pbColmnSelect = FALSE;
...
void CEditView::GetCurrentTextForSearch( CNativeW2& cmemCurText )
{
...
*/
void CEditView::GetCurrentTextForSearchDlg( CNativeW2& cmemCurText )
{
	cmemCurText.SetString(L"");
...
//#endif
					...
					if( pLine[nPos] == L'\'' &&
						...
						for( i = nPos + 1; i <= nLineLen - 1; ++i ){
							...
							nCharChars_2 = CNativeW2::GetSizeOfChar( pLine, nLineLen, i );
							if( 0 == nCharChars_2 ){
...
// 2002/03/13 novice
				...
				else if (pLine[nPos] == L' ' && CTypeSupport(this,COLORIDX_SPACE).IsDisp() && (nCOMMENTMODE < 1000 || nCOMMENTMODE > 1099) )
				{
					...
					// 2005-09-02 D.S.Koba GetSizeOfChar
					nCharChars = CNativeW2::GetSizeOfChar( pLine, nLineLen, nPos );
					if( 0 == nCharChars ){
						nCharChars = CLogicInt(1);
					}
					if( !bSearchStringMode
