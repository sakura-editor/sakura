...
LRESULT CTabWnd::OnTabLButtonUp( WPARAM wParam, LPARAM lParam )
{
	...
	case DRAG_DRAG:
		...
		::SendMessage( TabCtrl_GetToolTips( m_hwndTab ), TTM_ACTIVATE, (WPARAM)TRUE, (LPARAM)0 );	// ツールチップ有効化
		break;
...
LRESULT CTabWnd::OnTabMouseMove( WPARAM wParam, LPARAM lParam )
{
	...
	case DRAG_CHECK:
		...
		::SendMessage( TabCtrl_GetToolTips( m_hwndTab ), TTM_ACTIVATE, (WPARAM)FALSE, (LPARAM)0 );	// ツールチップ無効化
...
void CTabWnd::LayoutTab( void )
{
	...
	if( 0 != memcmp( &m_lf, &m_pShareData->m_Common.m_sTabBar.m_lf, sizeof(m_lf) ) ){
		...
		int nHeight = TAB_WINDOW_HEIGHT;
		::SetWindowPos( m_hWnd, NULL, 0, 0, rcWnd.right - rcWnd.left, nHeight, SWP_NOMOVE | SWP_NOZORDER );
