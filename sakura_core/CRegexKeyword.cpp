...
#define MYDBGMSG(s)                                                                                                    \
	{                                                                                                                  \
		...
		fprintf(fp, "%08x: %ls  BMatch(%d)=%d, Use=%d, Idx=%d\n", &m_pTypes, s, &BMatch, BMatch, m_bUseRegexKeyword,   \
				m_nTypeIndex);                                                                                         \
		fclose(fp);                                                                                                    \
	}
#else
...
*/
BOOL CRegexKeyword::RegexIsKeyword(const CStringRef &cStr, //!< [in] 検索対象文字列
...
BOOL CRegexKeyword::RegexKeyCheckSyntax(const wchar_t *s)
{
	...
	static const wchar_t *kakomi[7 * 2] = {
		L"/", L"/k", L"m/", L"/k", L"m#", L"#k", L"/", L"/ki", L"m/", L"/ki", L"m#", L"#ki", NULL, NULL,
	};
...
/*static*/
DWORD CRegexKeyword::GetNewMagicNumber() { return ::GetTickCount(); }
