...
)
{
    ...
    //パラメータチェック
    if (NULL == pszItemArray)
        return false;
    if (NULL == pnItemCount)
        return false;
    if (nArrayCount <= 0)
        return false;
    if (pnViewCount && (*pnViewCount < 0 || nArrayCount < *pnViewCount))
        return false;
...
{
    if (*m_pnUserItemCount < 0)
        *m_pnUserItemCount = 0;
    if (*m_pnUserItemCount > m_nArrayCount)
        *m_pnUserItemCount = m_nArrayCount;

    if (m_pnUserViewCount)
    {
        if (*m_pnUserViewCount < 0)
            *m_pnUserViewCount = 0;
        if (*m_pnUserViewCount > m_nArrayCount)
            *m_pnUserViewCount = m_nArrayCount;
    }
}
...
bool CRecentImp<T, S>::AppendItem(ReceiveType pItemData)
{
    ...
    if (!pItemData)
        return false;
    if (false == ValidateReceiveType(pItemData))
        return false;
...
bool CRecentImp<T, S>::MoveItem(int nSrcIndex, int nDstIndex)
{
    ...
    if (!IsAvailable())
        return false;
    if (nSrcIndex < 0 || nSrcIndex >= *m_pnUserItemCount)
        return false;
    if (nDstIndex < 0 || nDstIndex >= *m_pnUserItemCount)
        return false;
...
bool CRecentImp<T, S>::CopyItem(int nSrcIndex, int nDstIndex)
{
    if (!IsAvailable())
        return false;
    if (nSrcIndex < 0 || nSrcIndex >= m_nArrayCount)
        return false;
    if (nDstIndex < 0 || nDstIndex >= m_nArrayCount)
        return false;
    ...
    //内部処理しないとだめ。
    if (m_pbUserItemFavorite)
        m_pbUserItemFavorite[nDstIndex] = m_pbUserItemFavorite[nSrcIndex];
...
{
    if (!IsAvailable() || nIndex < 0 || nIndex >= *m_pnUserItemCount)
        return NULL;
...
{
    if (!IsAvailable() || nIndex < 0 || nIndex >= m_nArrayCount)
        return NULL;
    return &m_puUserItemData[nIndex];
}
...
int CRecentImp<T, S>::GetOldestItem(int nIndex, bool bFavorite)
{
    if (!IsAvailable())
        return -1;
    if (nIndex >= *m_pnUserItemCount)
        nIndex = *m_pnUserItemCount - 1;

    for (int i = nIndex; i >= 0; i--)
    {
        if (IsFavorite(i) == bFavorite)
            return i;
    }
...
bool CRecentImp<T, S>::ChangeViewCount(int nViewCount)
{
    ...
    if (!IsAvailable())
        return false;
    if (nViewCount < 0 || nViewCount > m_nArrayCount)
        return false;
    ...
    if (nViewCount >= *m_pnUserItemCount)
        return true;

    //最も古いお気に入りを探す。
    i = GetOldestItem(*m_pnUserItemCount - 1, true);
    if (-1 == i)
        return true; //ないので何もしないで終了
