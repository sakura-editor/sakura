...
//	Nov. 20, 2000 genta
TYPE_NAME_ID<int> ImeStateArr[] = {{0, STR_IME_STATE_DEF},
...
static const wchar_t *aszEolStr[] = {
	L"CR+LF", L"LF (UNIX)", L"CR (Mac)", L"NEL", L"LS", L"PS",
};
static const EEolType aeEolType[] = {
	EOL_CRLF, EOL_LF, EOL_CR, EOL_NEL, EOL_LS, EOL_PS,
};
...
static bool isImeUndesirable(int id)
{
	...
	case IDC_EDIT_BACKIMG_TRANSPARENCY: return true;
	default: return false;
	}
}

/*! window メッセージ処理 */
INT_PTR CPropTypesWindow::DispatchEvent(HWND   hwndDlg, // handle to dialog box
										UINT   uMsg,	// message
...
)
{
		...
		case CBN_SELCHANGE:
		{
			...
			case IDC_COMBO_DEFAULT_CODETYPE:
				...
				if (CB_ERR != i)
				{
					...
					::CheckDlgButton(hwndDlg, IDC_CHECK_DEFAULT_BOM,
									 (cCodeTypeName.IsBomDefOn() ? BST_CHECKED : BST_UNCHECKED));
	...
	case WM_NOTIFY:
		...
		case PSN_SETACTIVE: m_nPageNum = ID_PROPTYPE_PAGENUM_WINDOW; return TRUE;
		}
		...
		case IDC_SPIN_LINENUMWIDTH:
			...
			if (nVal < LINENUMWIDTH_MIN) { nVal = LINENUMWIDTH_MIN; }
			if (nVal > LINENUMWIDTH_MAX) { nVal = LINENUMWIDTH_MAX; }
	...
	case WM_HSCROLL:
		if ((HWND)lParam == ::GetDlgItem(hwndDlg, IDC_TRACKBAR_BACKIMG_TRANSPARENCY))
		{
			...
			WORD pos;
			if (code == TB_THUMBPOSITION || code == TB_THUMBTRACK) { pos = HIWORD(wParam); }
			else
			{
...
void CPropTypesWindow::SetCombobox(HWND hwndWork, const int *nIds, int nCount, int select)
{
	...
	for (int i = 0; i < nCount; ++i) { Combo_AddString(hwndWork, LS(nIds[i])); }
...
void CPropTypesWindow::SetData(HWND hwndDlg)
{
	...
	/* 「文字コード」グループの設定 */
	{
		...
		for (i = 0; i < cCodeTypes.GetCount(); i++)
		{
			if (CCodeTypeName(cCodeTypes.GetCode(i)).CanDefault())
			{
				...
				if (m_Types.m_encoding.m_eDefaultCodetype == cCodeTypes.GetCode(i)) { nSel = j; }
				j++;
			}
		}
		if (nSel == -1)
		...
		if (!cCodeTypeName.UseBom()) m_Types.m_encoding.m_bDefaultBom = false;
		::CheckDlgButton(hwndDlg, IDC_CHECK_DEFAULT_BOM,
						 (m_Types.m_encoding.m_bDefaultBom ? BST_CHECKED : BST_UNCHECKED));
		...
		for (i = 0; i < _countof(aszEolStr); ++i) { ApiWrap::Combo_AddString(hCombo, aszEolStr[i]); }
		for (i = 0; i < _countof(aeEolType); ++i)
		{
			if (m_Types.m_encoding.m_eDefaultEoltype == aeEolType[i]) { break; }
		}
		if (i == _countof(aeEolType)) { i = 0; }
	...
	DlgItem_SetText(hwndDlg, IDC_EDIT_BACKIMG_PATH, m_Types.m_szBackImgPath);
	{
		static const int posNameId[] = {
			STR_IMAGE_POS1, STR_IMAGE_POS2, STR_IMAGE_POS3, STR_IMAGE_POS4, STR_IMAGE_POS5,
			STR_IMAGE_POS6, STR_IMAGE_POS7, STR_IMAGE_POS8, STR_IMAGE_POS9,
		};
		/*BGIMAGE_TOP_LEFT .. */
...
int CPropTypesWindow::GetData(HWND hwndDlg)
{
	...
	/* 「文字コード」グループの設定 */
	{
		...
		if (CB_ERR != i) { m_Types.m_encoding.m_eDefaultCodetype = ECodeType(Combo_GetItemData(hCombo, i)); }
		...
		hCombo = ::GetDlgItem(hwndDlg, IDC_COMBO_DEFAULT_EOLTYPE);
		i	  = Combo_GetCurSel(hCombo);
		if (CB_ERR != i) { m_Types.m_encoding.m_eDefaultEoltype = aeEolType[i]; }
	}

	/* 行番号の表示 false=折り返し単位／true=改行単位 */
	if (::IsDlgButtonChecked(hwndDlg, IDC_RADIO_LINENUM_LAYOUT)) { m_Types.m_bLineNumIsCRLF = false; }
	else
	{
	...
	m_Types.m_backImgPos =
		static_cast<EBackgroundImagePos>(Combo_GetCurSel(GetDlgItem(hwndDlg, IDC_COMBO_BACKIMG_POS)));
	...
	m_Types.m_backImgOpacity =
		255 - (BYTE)std::min(255U, GetDlgItemInt(hwndDlg, IDC_EDIT_BACKIMG_TRANSPARENCY, NULL, FALSE));

	/* 行番号区切り  0=なし 1=縦線 2=任意 */
	if (::IsDlgButtonChecked(hwndDlg, IDC_RADIO_LINETERMTYPE0)) { m_Types.m_nLineTermType = 0; }
	else if (::IsDlgButtonChecked(hwndDlg, IDC_RADIO_LINETERMTYPE1))
	{
		m_Types.m_nLineTermType = 1;
	}
	else if (::IsDlgButtonChecked(hwndDlg, IDC_RADIO_LINETERMTYPE2))
	{
	...
	if (m_Types.m_nLineNumWidth < LINENUMWIDTH_MIN) { m_Types.m_nLineNumWidth = LINENUMWIDTH_MIN; }
	if (m_Types.m_nLineNumWidth > LINENUMWIDTH_MAX) { m_Types.m_nLineNumWidth = LINENUMWIDTH_MAX; }
