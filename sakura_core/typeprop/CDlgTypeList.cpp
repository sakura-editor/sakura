...
BOOL CDlgTypeList::OnLbnDblclk(int wID)
{
	...
	case IDC_LIST_TYPES:
		...
		::EndDialog(GetHwnd(), List_GetCurSel(GetItemHwnd(IDC_LIST_TYPES)) | PROP_TEMPCHANGE_FLAG);
		return TRUE;
	}
	return FALSE;
}
...
BOOL CDlgTypeList::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_HELP:
		...
		MyWinHelp(GetHwnd(), HELP_CONTEXT,
				  ::FuncID_To_HelpContextID(F_TYPE_LIST)); // 2006.10.10 ryoji MyWinHelpに変更に変更
		return TRUE;
	...
	case IDC_BUTTON_TEMPCHANGE:
		...
		return TRUE;
	//	Nov. 29, 2000	To Here
	case IDOK: ::EndDialog(GetHwnd(), List_GetCurSel(GetItemHwnd(IDC_LIST_TYPES))); return TRUE;
	case IDCANCEL: ::EndDialog(GetHwnd(), -1); return TRUE;
	case IDC_BUTTON_IMPORT: Import(); return TRUE;
	case IDC_BUTTON_EXPORT: Export(); return TRUE;
	case IDC_BUTTON_INITIALIZE: InitializeType(); return TRUE;
	case IDC_BUTTON_COPY_TYPE: CopyType(); return TRUE;
	case IDC_BUTTON_UP_TYPE: UpType(); return TRUE;
	case IDC_BUTTON_DOWN_TYPE: DownType(); return TRUE;
	case IDC_BUTTON_ADD_TYPE: AddType(); return TRUE;
	case IDC_BUTTON_DEL_TYPE: DelType(); return TRUE;
	}
	/* 基底クラスメンバ */
...
BOOL CDlgTypeList::OnActivate(WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WA_ACTIVE:
	case WA_CLICKACTIVE: SetData(-1); return TRUE;
	...
	case WM_COMMAND:
	{
		...
		if (LOWORD(wParam) == IDC_LIST_TYPES)
		{
			...
			case LBN_SELCHANGE:
				...
				DlgItem_Enable(GetHwnd(), IDC_BUTTON_DOWN_TYPE,
							   nIdx != 0 && nIdx < GetDllShareData().m_nTypesCount - 1);
				...
				if (wcspbrk(ext, CDocTypeManager::m_typeExtWildcards) == NULL)
				{
					...
					{ //「右クリック」チェックON
						if ((nRet = RegistExt(ext, true)) != 0)
						{
							...
							::MessageBox(GetHwnd(), (wstring(LS(STR_DLGTYPELIST_ERR1)) + buf).c_str(), GSTR_APPNAME,
										 MB_OK);
							break;
						}
					}
					...
					{ //「右クリック」チェックOFF
						if ((nRet = UnregistExt(ext)) != 0)
						{
							WCHAR buf[BUFFER_SIZE] = {0};
							::FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, nRet, 0, buf, _countof(buf), NULL);
							::MessageBox(GetHwnd(), (wstring(LS(STR_DLGTYPELIST_ERR2)) + buf).c_str(), GSTR_APPNAME,
										 MB_OK);
							break;
						}
					}
...
/* ダイアログデータの設定 */
void CDlgTypeList::SetData(void) { SetData(m_nSettingType.GetIndex()); }
...
void CDlgTypeList::SetData(int selIdx)
{
	...
	if (-1 == selIdx)
	{
		...
		if (-1 == selIdx) { selIdx = 0; }
	}
	if (GetDllShareData().m_nTypesCount <= selIdx) { selIdx = GetDllShareData().m_nTypesCount - 1; }
...
LPVOID CDlgTypeList::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
//@@@ 2002.01.18 add end

static void SendChangeSetting()
{
	CAppNodeGroupHandle(0).SendMessageToAllEditors(MYWM_CHANGESETTING, (WPARAM)0, (LPARAM)PM_CHANGESETTING_ALL,
												   CEditWnd::getInstance()->GetHwnd());
}

static void SendChangeSettingType(int nType)
{
	CAppNodeGroupHandle(0).SendMessageToAllEditors(MYWM_CHANGESETTING, (WPARAM)nType, (LPARAM)PM_CHANGESETTING_TYPE,
												   CEditWnd::getInstance()->GetHwnd());
}

static void SendChangeSettingType2(int nType)
{
	CAppNodeGroupHandle(0).SendMessageToAllEditors(MYWM_CHANGESETTING, (WPARAM)nType, (LPARAM)PM_CHANGESETTING_TYPE2,
												   CEditWnd::getInstance()->GetHwnd());
}
...
bool CDlgTypeList::Import()
{
	...
	if (!CDocTypeManager().GetTypeConfigMini(CTypeConfig(nIdx), &typeMini)) { return false; }
...
bool CDlgTypeList::InitializeType(void)
{
	...
	{
		nRet = ::MYMESSAGEBOX(GetHwnd(), MB_YESNO | MB_ICONQUESTION, GSTR_APPNAME, LS(STR_DLGTYPELIST_INIT1),
							  typeMini->m_szTypeName);
		if (nRet != IDYES) { return false; }
	}

	iDocType = CDocTypeManager().GetDocumentTypeOfId(typeMini->m_id).GetIndex();
	if (-1 == iDocType) { return false; }
	...
	if (0 != iDocType)
	{
		...
		for (int i = 1; i < GetDllShareData().m_nTypesCount; i++)
		{
			...
			if (iDocType == i) { continue; }
			const STypeConfigMini *typeMini2 = NULL;
			if (!CDocTypeManager().GetTypeConfigMini(CTypeConfig(i), &typeMini2)) { continue; }
...
bool CDlgTypeList::CopyType()
{
	...
	for (int i = 0; i < nNewTypeIndex; i++)
	{
		if (bUpdate)
		{
			...
			{
				if (WCODE::Is09(type.m_szTypeName[k])) { p = &type.m_szTypeName[k]; }
				else
				{
					break;
				}
		...
		const STypeConfigMini *typeMini = NULL;
		if (!CDocTypeManager().GetTypeConfigMini(CTypeConfig(i), &typeMini)) { continue; }
	...
	if (!CDocTypeManager().AddTypeConfig(CTypeConfig(nNewTypeIndex))) { return false; }
...
bool CDlgTypeList::DelType()
{
	...
	int nRet = ConfirmMessage(hwndDlg, LS(STR_DLGTYPELIST_DEL), type.m_szTypeName);
	if (nRet != IDYES) { return false; }
	// ダイアログを出している間にタイプ別リストが更新されたかもしれないのでidから再検索
	CTypeConfig config = CDocTypeManager().GetDocumentTypeOfId(type.m_id);
	if (!config.IsValidType()) { return false; }
	iDocType = config.GetIndex();
	CDocTypeManager().DelTypeConfig(config);
	if (GetDllShareData().m_nTypesCount <= iDocType) { iDocType = GetDllShareData().m_nTypesCount - 1; }
	SetData(iDocType);
	SendChangeSetting();
	return true;
}
...
int CopyRegistry(HKEY srcRoot, const wstring &srcPath, HKEY destRoot, const wstring &destPath)
{
	...
	for (;;)
	{
		...
		{
			if ((errorCode = CopyRegistry(srcRoot, srcPath + L"\\" + szSubKey, destRoot, destPath + L"\\" + szSubKey)))
...
int RegistExt(LPCWSTR sExt, bool bDefProg)
{
	...
	{ keyExt_HKLM.GetValue(NULL, szProgID_HKLM, _countof(szProgID_HKLM)); } CRegKey keyExt;
	...
	if (!CRegKey::ExistsKey(HKEY_CURRENT_USER, sProgIDPath.c_str()))
	{
		...
		{
			if ((errorCode = CopyRegistry(HKEY_LOCAL_MACHINE, (sBasePath + szProgID_HKLM).c_str(), HKEY_CURRENT_USER,
										  sProgIDPath.c_str()))
				!= 0)
			{ return errorCode; } }
	}
	...
	if ((errorCode =
			 keyShellActionCommand.Open(HKEY_CURRENT_USER, sShellActionCommandPath.c_str(), KEY_READ | KEY_WRITE))
		!= 0)
	{
	...
	{ return errorCode; } if ((errorCode = keyShellAction.SetValue(NULL, L"Sakura &Editor")) != 0)
	{ return errorCode; } CRegKey keyShell;
	if ((errorCode = keyShell.Open(HKEY_CURRENT_USER, sShellPath.c_str(), KEY_READ | KEY_WRITE)) != 0)
	{ return errorCode; } WCHAR szShellValue[BUFFER_SIZE] = {0};
...
int UnregistExt(LPCWSTR sExt)
{
	...
	CRegKey keyExt;
	if ((errorCode = keyExt.Open(HKEY_CURRENT_USER, sDotExt.c_str(), KEY_READ | KEY_WRITE)) != 0) { return errorCode; }

	WCHAR szProgID[BUFFER_SIZE] = {0};
	keyExt.GetValue(NULL, szProgID, _countof(szProgID));

	if (szProgID[0] == L'\0') { return ERROR_SUCCESS; }
	...
	CRegKey keyShellAction;
	if ((errorCode = keyShellAction.Open(HKEY_CURRENT_USER, sShellActionPath.c_str(), KEY_READ | KEY_WRITE)) != 0)
	{ return ERROR_SUCCESS; } CRegKey keyShell;
	if ((errorCode = keyShell.Open(HKEY_CURRENT_USER, sShellPath.c_str(), KEY_READ | KEY_WRITE)) != 0)
	{ return errorCode; } CRegKey keyBackup;
	if ((errorCode = keyBackup.Open(HKEY_CURRENT_USER, sBackupPath.c_str(), KEY_READ | KEY_WRITE)) != 0)
	{ keyShell.DeleteValue(L""); } else
	{
	...
	if (wcsncmp_literal(szProgID, L"SakuraEditor_") == 0)
	{
		...
		if (szBackupValue[0] != L'\0') { keyExt.SetValue(NULL, szBackupValue); }
		else
		{
...
int CheckExt(LPCWSTR sExt, bool *pbRMenu, bool *pbDblClick)
{
	...
	*pbDblClick = false;
	...
	CRegKey keyExt;
	if ((errorCode = keyExt.Open(HKEY_CURRENT_USER, sDotExt.c_str(), KEY_READ)) != 0) { return ERROR_SUCCESS; }
	...
	wstring sShellActionPath = sShellPath + L"\\" + ACTION_NAME;
	if (!CRegKey::ExistsKey(HKEY_CURRENT_USER, sShellActionPath.c_str())) { return ERROR_SUCCESS; }
	*pbRMenu = true;
	...
	keyShell.GetValue(NULL, szShellValue, _countof(szShellValue));
	if (wcscmp(szShellValue, ACTION_NAME) == 0) { *pbDblClick = true; }
...
bool CDlgTypeList::AlertFileAssociation()
{
	...
	{
		if (IDYES
			== ::MYMESSAGEBOX(NULL, MB_YESNO | MB_ICONEXCLAMATION | MB_APPLMODAL | MB_TOPMOST, GSTR_APPNAME,
