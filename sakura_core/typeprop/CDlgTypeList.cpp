...
const DWORD p_helpids[] = { //12700
    IDC_BUTTON_TEMPCHANGE,
    HIDC_TL_BUTTON_TEMPCHANGE, //一時適用
    IDOK,
    HIDOK_TL, //設定
    IDCANCEL,
    HIDCANCEL_TL, //キャンセル
    IDC_BUTTON_HELP,
    HIDC_TL_BUTTON_HELP, //ヘルプ
    IDC_LIST_TYPES,
    HIDC_TL_LIST_TYPES, //リスト
    IDC_BUTTON_IMPORT,
    HIDC_TL_BUTTON_IMPORT, //インポート
    IDC_BUTTON_EXPORT,
    HIDC_TL_BUTTON_EXPORT, //エクスポート
    IDC_BUTTON_INITIALIZE,
    HIDC_TL_BUTTON_INIT, //初期化
    IDC_BUTTON_COPY_TYPE,
    HIDC_BUTTON_COPY_TYPE, //複製
    IDC_BUTTON_UP_TYPE,
    HIDC_BUTTON_UP_TYPE, //↑
    IDC_BUTTON_DOWN_TYPE,
    HIDC_BUTTON_DOWN_TYPE, //↓
    IDC_BUTTON_ADD_TYPE,
    HIDC_BUTTON_ADD_TYPE, //追加
    IDC_BUTTON_DEL_TYPE,
    HIDC_BUTTON_DEL_TYPE, //削除
    IDC_CHECK_EXT_RMENU,
    HIDC_TL_CHECK_RMENU, //右クリックメニューに追加
    IDC_CHECK_EXT_DBLCLICK,
    HIDC_TL_CHECK_DBLCLICK, //ダブルクリックで開く
    //	IDC_STATIC,				-1,
    0,
...
BOOL CDlgTypeList::OnLbnDblclk(int wID)
{
    switch (wID)
    {
        case IDC_LIST_TYPES:
            ...
            ::EndDialog(
                GetHwnd(),
                List_GetCurSel(GetItemHwnd(IDC_LIST_TYPES)) | PROP_TEMPCHANGE_FLAG);
            return TRUE;
    }
    return FALSE;
}
...
int DeleteRegistry(HKEY root, const wstring &path)
{
    ...
    for (;;)
    {
        ...
        {
            if ((errorCode = DeleteRegistry(root, path + L"\\" + szSubKey)) != 0)
            {
                return errorCode;
            }
        }
    ...
    keySrc.Close();
    if ((errorCode = CRegKey::DeleteKey(root, path.c_str())) != 0)
    {
        return errorCode;
    }
...
int RegistExt(LPCWSTR sExt, bool bDefProg)
{
    ...
    if ((errorCode = keyShellAction.SetValue(NULL, L"Sakura &Editor")) != 0)
    {
        return errorCode;
    }
...
bool CDlgTypeList::AlertFileAssociation()
{
    if (m_bAlertFileAssociation)
    {
        if (IDYES == ::MYMESSAGEBOX(
                         NULL, MB_YESNO | MB_ICONEXCLAMATION | MB_APPLMODAL | MB_TOPMOST, GSTR_APPNAME, LS(STR_DLGTYPELIST_ACC)))
        {
