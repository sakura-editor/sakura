...
static const DWORD p_helpids[] = { // 2006.10.10 ryoji
	...
	HIDOK_SAMECOLOR, // OK
	IDCANCEL,
	HIDCANCEL_SAMECOLOR, // キャンセル
	IDC_BUTTON_HELP,
	HIDC_BUTTON_SAMECOLOR_HELP, // ヘルプ
	IDC_LIST_COLORS,
	HIDC_LIST_SAMECOLOR_COLORS, // 変更対象の色
	IDC_BUTTON_SELALL,
	HIDC_BUTTON_SAMECOLOR_SELALL, // 全チェック
	IDC_BUTTON_SELNOTING,
	HIDC_BUTTON_SAMECOLOR_SELNOTING, // 全解除
	IDC_LIST_ITEMINFO,
	HIDC_LIST_SAMECOLOR_ITEMINFO, // 選択中の色に対応する項目のリスト
	IDC_STATIC_COLOR,
	HIDC_STATIC_COLOR, // 統一色
	0,
	0};

LPVOID CDlgSameColor::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
...
CDlgSameColor::~CDlgSameColor() { return; }
...
INT_PTR CDlgSameColor::DispatchEvent(HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{ OnSelChangeListColors((HWND)lParam); } break;
...
BOOL CDlgSameColor::OnInitDialog(HWND hwndDlg, WPARAM wParam, LPARAM lParam)
{
	...
	case IDC_BUTTON_SAMETEXTCOLOR:
		...
		{
			if (0 != (g_ColorAttributeArr[i].fAttribute & COLOR_ATTRIB_NO_TEXT)) continue;
	...
	default: CloseDialog(IDCANCEL); break;
	}
...
BOOL CDlgSameColor::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_SELNOTING:
		...
		for (i = 0; i < nItemNum; ++i) { List_SetItemData(hwndList, i, bCheck); }
...
BOOL CDlgSameColor::OnDrawItem(WPARAM wParam, LPARAM lParam)
{
	...
	if (FALSE != (BOOL)pDis->itemData) uState |= DFCS_CHECKED; // チェック状態
...
BOOL CDlgSameColor::OnSelChangeListColors(HWND hwndCtl)
{
	...
	if (LB_ERR != i)
	{
		...
		case IDC_BUTTON_SAMEBKCOLOR:
			...
			{
				if (0
					!= (g_ColorAttributeArr[j].fAttribute
