...
bool CImpExpKeybind::Import( const wstring& sFileName, wstring& sErrMsg )
{
	...
	if(wcscmp_literal(szHeader,WSTR_KEYBIND_HEAD4)==0)	bVer4=true;
	else if(wcscmp_literal(szHeader,WSTR_KEYBIND_HEAD3)==0)	bVer3=true;
	...
	if (!bVer3 && !bVer4) {
		...
		bVer2 = true;
		if ( wcscmp_literal(szLine.c_str(), WSTR_KEYBIND_HEAD2) != 0)	bVer2 = false;
		// カウントチェック
		int	i, cnt;
		if ( bVer2 ) {
			int	an;
