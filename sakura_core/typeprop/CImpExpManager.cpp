...
// static const wchar_t	WSTR_COLORDATA_HEAD21[]	=  L" テキストエディタ色設定 Ver2.1";	//Nov. 2, 2000 JEPRO 変更 [注].
// 0.3.9.0:ur3β10以降、設定項目の番号を入れ替えたため
static const wchar_t WSTR_COLORDATA_HEAD3[] =
	L" テキストエディタ色設定 Ver3"; // Jan. 15, 2001 Stonee  色設定Ver3ドラフト(設定ファイルのキーを連番→文字列に)
									 // //Feb. 11, 2001 JEPRO 有効にした
// static const wchar_t	WSTR_COLORDATA_HEAD4[]	=  L" テキストエディタ色設定 Ver4";		//2007.10.02 kobake
// UNICODE化に際してカラーファイル仕様も変更
...
static const wchar_t WSTR_KEYBIND_HEAD2[] =
	L"// テキストエディタキー設定 Ver2"; // (旧バージョン(ANSI版)） 読み込みのみ対応 2008/5/3 by Uchi
...
bool CImpExpManager::ImportUI(HINSTANCE hInstance, HWND hwndParent)
{
	...
	cDlgOpenFile.Create(hInstance, hwndParent, GetDefaultExtension(),
						GetDllShareData().m_sHistory.m_szIMPORTFOLDER // インポート用フォルダ
	);
	WCHAR szPath[_MAX_PATH + 1];
	szPath[0] = L'\0';
	if (!GetFileName().empty()) { wcscpy(szPath, GetFullPath().c_str()); }
	if (!cDlgOpenFile.DoModal_GetOpenFileName(szPath)) { return false; }
	...
	{
		if (sErrMsg.length() > 0) { ErrorMessage(hwndParent, L"%ls", sErrMsg.c_str()); }
		return false;
	}
	...
	if (sErrMsg.length() > 0) { InfoMessage(hwndParent, L"%ls", sErrMsg.c_str()); }
...
bool CImpExpManager::ExportUI(HINSTANCE hInstance, HWND hwndParent)
{
	...
	szPath[0] = L'\0';
	if (!GetFileName().empty()) { wcscpy(szPath, GetFullPath().c_str()); }
	if (!cDlgOpenFile.DoModal_GetSaveFileName(szPath)) { return false; }
	...
	if (sErrMsg.length() == 0) { sErrMsg = LS(STR_IMPEXP_OK_EXPORT) + sPath; }
...
// デフォルト拡張子の取得
const WCHAR *  CImpExpManager::GetDefaultExtension() { return L""; }
const wchar_t *CImpExpManager::GetOriginExtension() { return L""; }
...
bool CImpExpType::ImportAscertain(HINSTANCE hInstance, HWND hwndParent, const wstring &sFileName, wstring &sErrMsg)
{
	...
	if ((unsigned int)nStructureVersion != m_pShareData->m_vStructureVersion)
	{
		...
		int nRet = ConfirmMessage(hwndParent, LS(STR_IMPEXP_VER), GSTR_APPNAME, szKeyVersion, nStructureVersion);
	...
	// 確認
	if (!cDlgTypeAscertain.DoModal(hInstance, hwndParent, &sAscertainInfo)) { return false; }
...
bool CImpExpType::Import(const wstring &sFileName, wstring &sErrMsg)
{
	...
	if (m_cProfile.IOProfileData(szSecTypeEx, szKeyPluginOutlineId, MakeStringBufferW(szKeyData)))
	{
		...
		if (nIdx >= 0) { m_Types.m_eDefaultOutline = CPlug::GetOutlineType(CPlug::GetPluginFunctionCode(nIdx, nPlug)); }
	}
	//  スマートインデント
...
bool CImpExpType::Export(const wstring &sFileName, wstring &sErrMsg)
{
	...
	{
		cProfile.IOProfileData(szSecTypeEx, szKeyPluginOutlineName,
							   MakeStringBufferW(plugin.m_PluginTable[nPIdx].m_szName));
	...
	{
		cProfile.IOProfileData(szSecTypeEx, szKeyPluginSmartIndentName,
							   MakeStringBufferW(plugin.m_PluginTable[nPIdx].m_szName));
		wcscpyn(szId, plugin.m_PluginTable[nPIdx].m_szId, _countof(szId));
	...
	wchar_t		  wbuff[_MAX_PATH + 1];
	auto_sprintf(wbuff, L"%d.%d.%d.%d", HIWORD(pShare->m_sVersion.m_dwProductVersionMS),
...
bool CImpExpColors::Import(const wstring &sFileName, wstring &sErrMsg)
{
	...
	/* 色設定Ver3 */
	if (!cProfile.ReadProfile(strPath.c_str())) { return false; }
...
bool CImpExpColors::Export(const wstring &sFileName, wstring &sErrMsg)
{
	...
	cProfile.SetWritingMode();
	CShareData_IO::IO_ColorSet(&cProfile, szSecColor, m_ColorInfoArr);
	if (!cProfile.WriteProfile(sFileName.c_str(), WSTR_COLORDATA_HEAD3))
	{ // Jan. 15, 2001 Stonee
		sErrMsg = LS(STR_IMPEXP_ERR_EXPORT);
		sErrMsg += sFileName;
		return false;
...
bool CImpExpRegex::Import(const wstring &sFileName, wstring &sErrMsg)
{
	...
	for (int i = 0; i < count; i++) { m_Types.m_RegexKeywordArr[i] = regexKeyArr[i]; }
	for (int i = 0; i <= keywordPos; i++) { m_Types.m_RegexKeywordList[i] = pKeyword[i]; }
...
bool CImpExpKeyHelp::Import(const wstring &sFileName, wstring &sErrMsg)
{
	...
	while (in && i < MAX_KEYHELP_FILE)
	{
		...
		if (buff[0] == LTEXT('\0') || buff[0] == LTEXT('\n') || buff[0] == LTEXT('#') || buff[0] == LTEXT(';')
		...
		if (buff.length() < 10 || wmemcmp(buff.c_str(), LTEXT("KDct["), 5) != 0
		...
		if (b_enable_flag > 1) { b_enable_flag = 1; }
		// Path
		FILE *		 fp2;
		const WCHAR *p4 = p2;
		if ((fp2 = _wfopen_absini(p3, L"r")) == NULL)
		{ // 2007.02.03 genta 相対パスはsakura.exe基準で開く	//
...
bool CImpExpKeyHelp::Export(const wstring &sFileName, wstring &sErrMsg)
{
	...
	{
		out.WriteF(L"KDct[%02d]=%d,%s,%s\n", i, m_Types.m_KeyHelpArr[i].m_bUse ? 1 : 0,
				   m_Types.m_KeyHelpArr[i].m_szAbout, m_Types.m_KeyHelpArr[i].m_szPath.c_str());
	}
	out.Close();
...
bool CImpExpKeybind::Import(const wstring &sFileName, wstring &sErrMsg)
{
	...
	if (bVer3 || bVer4)
	{
		...
		if (sKeyBind.m_nKeyNameArrNum < 0 || sKeyBind.m_nKeyNameArrNum > KEYNAME_SIZE)
		{
			bVer3 = false;
			bVer4 = false;
		} //範囲チェック
	...
	if (!bVer3 && !bVer4)
	{
		...
		if (bVer2)
		{
			...
			for (i = 0; i < KEYNAME_SIZE; i++)
			{
				...
				//解析開始
				cnt = swscanf(szData, L"KeyBind[%03d]=%04x,%n", &n, &kc, &nc);
				if (cnt != 2 && cnt != 3)
				{
					bVer2 = false;
					break;
	...
	{
		sErrMsg = wstring(LS(STR_IMPEXP_KEY_FORMAT)) + sFileName;
		return false;
	}
	...
	{
		if ((bVer2 || bVer3) && sKeyBind.m_pKeyNameArr[j].m_nKeyCode <= 0)
		{ // マウスコードは先頭に固定されている KeyCodeが同じなのでKeyNameで判別
			for (int im = 0; im < MOUSEFUNCTION_KEYBEGIN; im++)
			{
				if (wcscmp(sKeyBind.m_pKeyNameArr[j].m_szKeyName, m_Common.m_sKeyBind.m_pKeyNameArr[im].m_szKeyName)
					== 0)
	...
	memcpy_raw(m_Common.m_sKeyBind.m_VKeyToKeyNameArr, sKeyBind.m_VKeyToKeyNameArr,
			   sizeof_raw(sKeyBind.m_VKeyToKeyNameArr));
...
bool CImpExpKeyWord::Import(const wstring &sFileName, wstring &sErrMsg)
{
	...
	if (bAddError) { sErrMsg = LS(STR_IMPEXP_KEYWORD); }
...
void CImpExpFileTree::IO_FileTreeIni(CDataProfile &cProfile, std::vector<SFileTreeItem> &data)
{
	...
	if (nItemCount < 0) { nItemCount = 0; }
	int i = 0;
	if (cProfile.IsReadingMode()) { data.resize(nItemCount); }
	for (; i < nItemCount; i++) { CShareData_IO::ShareData_IO_FileTreeItem(cProfile, data[i], pszSecName, i); }
}
