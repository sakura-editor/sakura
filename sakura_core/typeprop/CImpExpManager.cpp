...
bool CImpExpType::ImportAscertain( HINSTANCE hInstance, HWND hwndParent, const wstring& sFileName, wstring& sErrMsg )
{
	...
	if ((unsigned int)nStructureVersion != m_pShareData->m_vStructureVersion) {
		...
		m_cProfile.IOProfileData( szSecInfo, szKeyVersion, CStringBufW( szKeyVersion ) );
	...
	szLabel[0] = L'\0';
	m_cProfile.IOProfileData( szSecTypes, L"szTypeName", CStringBufW( szLabel ));
...
bool CImpExpType::Import( const wstring& sFileName, wstring& sErrMsg )
{
	...
	for (i=0; i < MAX_KEYWORDSET_PER_TYPE; i++) {
		...
		if (m_cProfile.IOProfileData( szSecTypeEx, szKeyName, CStringBufW( szKeyData ))) {
			...
			if (nIdx >= 0) {
				...
				szFileName[0] = L'\0';
				if (m_cProfile.IOProfileData( szSecTypeEx, szKeyName, CStringBufW( szFileName ))) {
	...
	if (m_cProfile.IOProfileData( szSecTypeEx, szKeyPluginOutlineId, CStringBufW( szKeyData ))) {
	...
	//  スマートインデント
	if (m_cProfile.IOProfileData( szSecTypeEx, szKeyPluginSmartIndentId, CStringBufW( szKeyData ))) {
		nDataLen = wcslen( szKeyData );
		pSlashPos = wcschr( szKeyData, L'/' );
		nIdx = -1;
		for (i = 0; i < MAX_PLUGIN; i++) {
...
bool CImpExpType::Export( const wstring& sFileName, wstring& sErrMsg )
{
	...
	for (i=0; i < MAX_KEYWORDSET_PER_TYPE; i++) {
		if (m_Types.m_nKeyWordSetIdx[i] >= 0) {
			...
			cProfile.IOProfileData( szSecTypeEx, szKeyName, CStringBufW( buff ));
			...
			if ( cImpExpKeyWord.Export( cImpExpKeyWord.GetFullPath(), sTmpMsg ) ) {
				...
				auto_sprintf( szKeyName, szKeyKeywordFileTemp, i+1 );
				if (cProfile.IOProfileData( szSecTypeEx, szKeyName, CStringBufW( szFileName ))) {
	...
	if ((nPIdx = CPlug::GetPluginId( static_cast<EFunctionCode>( m_Types.m_eDefaultOutline ))) >= 0) {
		cProfile.IOProfileData( szSecTypeEx, szKeyPluginOutlineName, CStringBufW(plugin.m_PluginTable[nPIdx].m_szName));
		...
		cProfile.IOProfileData( szSecTypeEx, szKeyPluginOutlineId, CStringBufW( szId ) );
	}
	//  スマートインデント
	if ((nPIdx = CPlug::GetPluginId( static_cast<EFunctionCode>( m_Types.m_eSmartIndent ))) >= 0) {
		cProfile.IOProfileData( szSecTypeEx, szKeyPluginSmartIndentName, CStringBufW(plugin.m_PluginTable[nPIdx].m_szName));
		wcscpyn( szId, plugin.m_PluginTable[nPIdx].m_szId, _countof(szId) );
		...
		cProfile.IOProfileData( szSecTypeEx, szKeyPluginSmartIndentId, CStringBufW( szId ) );
	}
	...
	cProfile.IOProfileData( szSecInfo, szKeyVersion, CStringBufW( wbuff ) );
...
bool CImpExpKeybind::Import( const wstring& sFileName, wstring& sErrMsg )
{
	...
	bVer3 = false;
	bVer2 = false;
	in.IOProfileData( szSecInfo, L"KEYBIND_VERSION", CStringBufW( szHeader ));
...
bool CImpExpCustMenu::Import( const wstring& sFileName, wstring& sErrMsg )
{
	...
	//バージョン確認
	WCHAR szHeader[256];
	cProfile.IOProfileData( szSecInfo, L"MENU_VERSION", CStringBufW( szHeader ));
...
bool CImpExpCustMenu::Export( const wstring& sFileName, wstring& sErrMsg )
{
	...
	//ヘッダ
	cProfile.IOProfileData( szSecInfo, L"MENU_VERSION", CStringBufW( WSTR_CUSTMENU_HEAD_V2 ) );
...
bool CImpExpMainMenu::Export( const wstring& sFileName, wstring& sErrMsg )
{
	...
	//ヘッダ
	cProfile.IOProfileData( szSecInfo, L"MENU_VERSION", CStringBufW( WSTR_MAINMENU_HEAD_V1 ) );
