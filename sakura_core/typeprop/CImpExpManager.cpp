...
bool CImpExpType::Export(const wstring &sFileName, wstring &sErrMsg)
{
    ...
    wchar_t wbuff[_MAX_PATH + 1];
    auto_sprintf(wbuff, L"%d.%d.%d.%d", HIWORD(pShare->m_sVersion.m_dwProductVersionMS), LOWORD(pShare->m_sVersion.m_dwProductVersionMS), HIWORD(pShare->m_sVersion.m_dwProductVersionLS), LOWORD(pShare->m_sVersion.m_dwProductVersionLS));
...
bool CImpExpRegex::Import(const wstring &sFileName, wstring &sErrMsg)
{
    ...
    while (in)
    {
        ...
        if (wcslen(buff) < 12)
            continue;
        if (wmemcmp(buff, L"RxKey[", 6) != 0)
            continue;
        if (wmemcmp(&buff[9], L"]=", 2) != 0)
            continue;
...
bool CImpExpKeyHelp::Import(const wstring &sFileName, wstring &sErrMsg)
{
    ...
    while (in && i < MAX_KEYHELP_FILE)
    {
        ...
        { // 2007.02.03 genta 相対パスはsakura.exe基準で開く	// 2007.05.19 ryoji 相対パスは設定ファイルからのパスを優先
