...
static const DWORD p_helpids1[] = {		   // 11300
	...
	IDC_SPIN_MAXLINELEN, HIDC_EDIT_MAXLINELEN, IDC_EDIT_CHARSPACE, HIDC_EDIT_CHARSPACE, //文字の間隔
	IDC_SPIN_CHARSPACE, HIDC_EDIT_CHARSPACE, IDC_EDIT_LINESPACE, HIDC_EDIT_LINESPACE,   //行の間隔
	IDC_SPIN_LINESPACE, HIDC_EDIT_LINESPACE, IDC_EDIT_TABSPACE, HIDC_EDIT_TABSPACE,		// TAB幅 // Sep. 19, 2002 genta
	IDC_SPIN_TABSPACE, HIDC_EDIT_TABSPACE, IDC_EDIT_TABVIEWSTRING, HIDC_EDIT_TABVIEWSTRING, // TAB表示文字列
...
TYPE_NAME_ID<EOutlineType> OlmArr[] = {
	{OUTLINE_C_CPP, STR_OUTLINE_CPP},	 {OUTLINE_C, STR2_OUTLINE_C},		{OUTLINE_CPP, STR2_OUTLINE_CPP},
	{OUTLINE_PLSQL, STR_OUTLINE_PLSQL},   {OUTLINE_JAVA, STR_OUTLINE_JAVA}, {OUTLINE_COBOL, STR_OUTLINE_COBOL},
	{OUTLINE_PERL, STR_OUTLINE_PERL},									// Sep. 8, 2000 genta
	{OUTLINE_ASM, STR_OUTLINE_ASM},		  {OUTLINE_VB, STR_OUTLINE_VB}, // 2001/06/23 N.Nakatani
...
static bool isImeUndesirable(int id)
{
	...
	case IDC_EDIT_TABSPACE: return true;
	default: return false;
	}
}

/* Screen メッセージ処理 */
INT_PTR CPropTypesScreen::DispatchEvent(HWND   hwndDlg, // handle to dialog box
										UINT   uMsg,	// message
...
)
{
	...
	case WM_INITDIALOG:
		...
		EditCtl_LimitText(GetDlgItem(hwndDlg, IDC_EDIT_OUTLINERULEFILE),
						  _countof2(m_Types.m_szOutlineRuleFilename) - 1); //	Oct. 5, 2002 genta 画面上でも入力制限
	...
	case WM_COMMAND:
		...
		case CBN_SELCHANGE:
			...
			case IDC_CHECK_TAB_ARROW:
			{
				...
				{ ::EnableWindow(::GetDlgItem(hwndDlg, IDC_EDIT_TABVIEWSTRING), TRUE); } else
				{
		...
		case BN_CLICKED:
			...
			*/
			...
			case IDC_BUTTON_RULEFILE_REF: /* アウトライン解析→ルールファイルの「参照...」ボタン */
			{
				...
				CDlgOpenFile::SelectFile(hwndDlg, GetDlgItem(hwndDlg, IDC_EDIT_OUTLINERULEFILE), L"*.rul;*.rule;*.txt",
										 true, EFITER_NONE);
			}
				return TRUE;
			...
			case IDC_BUTTON_TYPEFONT:
			{
				...
				if (m_Types.m_bUseTypeFont) { lf = m_Types.m_lf; }
				else
				{
				...
				if (MySelectFont(&lf, &nPointSize, hwndDlg, bFixedFont))
				{
					...
					HFONT hFont = SetFontLabel(hwndDlg, IDC_STATIC_TYPEFONT, m_Types.m_lf, m_Types.m_nPointSize,
											   m_Types.m_bUseTypeFont);
	...
	case WM_NOTIFY:
		...
		case IDC_SPIN_MAXLINELEN:
			...
			if (nVal < MINLINEKETAS) { nVal = MINLINEKETAS; }
			if (nVal > MAXLINEKETAS) { nVal = MAXLINEKETAS; }
		...
		case IDC_SPIN_LINESPACE:
			...
			//				nVal = 1;
			//			}
			if (nVal < -LINESPACE_MAX) { nVal = -LINESPACE_MAX; }
			//	To Here  Oct. 8, 2000
		...
		default:
			switch (pNMHDR->code)
			{
			case PSN_HELP: OnHelp(hwndDlg, IDD_PROP_SCREEN); return TRUE;
			case PSN_KILLACTIVE:
				/* ダイアログデータの取得 Screen */
				GetData(hwndDlg);
			...
			case PSN_SETACTIVE: m_nPageNum = ID_PROPTYPE_PAGENUM_SCREEN; return TRUE;
			}
			break;
		}
	...
	case WM_HELP:
	{
		...
		MyWinHelp((HWND)p->hItemHandle, HELP_WM_HELP,
				  (ULONG_PTR)(LPVOID)p_helpids1); // 2006.10.10 ryoji MyWinHelpに変更に変更
	}
		return TRUE;
		/*NOTREACHED*/
		//		break;
...
void CPropTypesScreen::SetData(HWND hwndDlg)
{
	...
	//レイアウト
	{
		...
		for (int i = 0; i < _countof(WrapMethodArr); ++i)
		{
			...
			if (WrapMethodArr[i].nMethod == m_Types.m_nTextWrapMethod)
			{ // テキストの折り返し方法
				nSelPos = i;
			}
		}
		...
		::EnableWindow(::GetDlgItem(hwndDlg, IDC_EDIT_TABVIEWSTRING),
					   m_Types.m_bTabArrow
						   == TABARROW_STRING); // Mar. 31, 2003 genta 矢印表示のON/OFFをTAB文字列設定に連動させる
		...
		for (int i = 0; i < _countof(TabArrowArr); ++i)
		{
			...
			if (TabArrowArr[i].nMethod == m_Types.m_bTabArrow) { nSelPos = i; }
		}
		Combo_SetCurSel(hwndCombo, nSelPos);

		::CheckDlgButtonBool(hwndDlg, IDC_CHECK_INS_SPACE,
							 m_Types.m_bInsSpace); // SPACEの挿入 [チェックボックス]	// From Here 2001.12.03 hor
		...
		for (int i = 0; i < _countof(TsvModeArr); ++i)
		{
			...
			if (TsvModeArr[i].nMethod == m_Types.m_nTsvMode) { nSelPos = i; }
		}
		Combo_SetCurSel(hwndCombo, nSelPos);
	}
	...
	//インデント
	{
		...
		{
			if (m_SIndentArr[i].pszName == NULL) { Combo_InsertString(hwndCombo, i, LS(m_SIndentArr[i].nNameId)); }
			else
			{
				Combo_InsertString(hwndCombo, i, m_SIndentArr[i].pszName);
			}
			if (m_SIndentArr[i].nMethod == m_Types.m_eSmartIndent)
			{ /* スマートインデント種別 */
				nSelPos = i;
			}
		}
		...
		for (int i = 0; i < _countof(IndentTypeArr); ++i)
		{
			...
			if (IndentTypeArr[i].nMethod == m_Types.m_nIndentLayout)
			{ /* 折り返しインデント種別 */
				nSelPos = i;
			}
		}
	...
	// 2002.04.01 YAZAKI ルールファイル関連追加
	{
		...
		for (int i = 0; i < nSize; ++i)
		{
			if (m_OlmArr[i].pszName == NULL)
			{
				...
				{ Combo_InsertString(hwndCombo, i, pszOutlineNames[m_OlmArr[i].nNameId]); } else
				{
			...
			if (m_OlmArr[i].nMethod == m_Types.m_eDefaultOutline)
			{ /* アウトライン解析方法 */
				nSelPos = i;
			}
		}
	...
	//フォント
	{
		...
		::EnableWindow(::GetDlgItem(hwndDlg, IDC_CHECK_USETYPEFONT), m_Types.m_bUseTypeFont);
		m_hTypeFont =
			SetFontLabel(hwndDlg, IDC_STATIC_TYPEFONT, m_Types.m_lf, m_Types.m_nPointSize, m_Types.m_bUseTypeFont);
	}
	...
	//その他
	{
		...
		{ //@@@ 2002.04.08 MIK start
			...
			::CheckDlgButtonBool(hwndDlg, IDC_CHECK_KINSOKURET, m_Types.m_bKinsokuRet);
			/* 改行文字をぶら下げる */ //@@@ 2002.04.13 MIK
			::CheckDlgButtonBool(hwndDlg, IDC_CHECK_KINSOKUKUTO, m_Types.m_bKinsokuKuto);
			/* 句読点をぶら下げる */ //@@@ 2002.04.17 MIK
			::CheckDlgButtonBool(hwndDlg, IDC_CHECK_KINSOKUHIDE,
								 m_Types.m_bKinsokuHide); // ぶら下げを隠す			// 2011/11/30 Uchi
			...
			EditCtl_LimitText(::GetDlgItem(hwndDlg, IDC_EDIT_KINSOKUKUTO),
							  _countof(m_Types.m_szKinsokuKuto) - 1); // 2009.08.07 ryoji
			...
			::EnableWindow(::GetDlgItem(hwndDlg, IDC_CHECK_KINSOKUHIDE),
						   (m_Types.m_bKinsokuRet || m_Types.m_bKinsokuKuto)
							   ? TRUE
							   : FALSE); // ぶら下げを隠すの有効化	2012/11/30 Uchi
		}								 //@@@ 2002.04.08 MIK end
	}
}
...
int CPropTypesScreen::GetData(HWND hwndDlg)
{
	...
	::DlgItem_GetText(hwndDlg, IDC_EDIT_TYPEEXTS, m_Types.m_szTypeExts,
					  _countof(m_Types.m_szTypeExts)); // ファイル拡張子
	...
	//レイアウト
	{
		...
		/* 折り返し桁数 */
		m_Types.m_nMaxLineKetas = CKetaXInt(
			t_max(MINLINEKETAS, t_min<int>(MAXLINEKETAS, ::GetDlgItemInt(hwndDlg, IDC_EDIT_MAXLINELEN, NULL, FALSE))));

		/* 文字の間隔 */
		m_Types.m_nColumnSpace = ::GetDlgItemInt(hwndDlg, IDC_EDIT_CHARSPACE, NULL, FALSE);
		if (m_Types.m_nColumnSpace < 0) { m_Types.m_nColumnSpace = 0; }
		if (m_Types.m_nColumnSpace > COLUMNSPACE_MAX)
		{ // Feb. 18, 2003 genta 最大値の定数化
		...
		if (m_Types.m_nLineSpace < -LINESPACE_MAX) { m_Types.m_nLineSpace = -LINESPACE_MAX; }
		...
		/* TAB幅 */
		m_Types.m_nTabSpace =
			CKetaXInt(t_max(1, t_min<int>(64, ::GetDlgItemInt(hwndDlg, IDC_EDIT_TABSPACE, NULL, FALSE))));
	...
	//インデント
	{
		...
		HWND hwndCombo = ::GetDlgItem(hwndDlg, IDC_COMBO_SMARTINDENT);
		int  nSelPos   = Combo_GetCurSel(hwndCombo);
		if (nSelPos >= 0) { m_Types.m_eSmartIndent = m_SIndentArr[nSelPos].nMethod; /* スマートインデント種別 */ }
	...
	// 2002.04.01 YAZAKI ルールファイル関連追加
	{
		...
		if (!::IsDlgButtonChecked(hwndDlg, IDC_RADIO_OUTLINERULEFILE))
		{
			HWND hwndCombo = ::GetDlgItem(hwndDlg, IDC_COMBO_OUTLINES);
			int  nSelPos   = Combo_GetCurSel(hwndCombo);
			if (nSelPos >= 0) { m_Types.m_eDefaultOutline = m_OlmArr[nSelPos].nMethod; /* アウトライン解析方法 */ }
		}
		//ルールファイル
		else
		{
		...
		::DlgItem_GetText(hwndDlg, IDC_EDIT_OUTLINERULEFILE, m_Types.m_szOutlineRuleFilename,
						  _countof2(m_Types.m_szOutlineRuleFilename));
	}
	...
	//その他
	{
		...
		{ //@@@ 2002.04.08 MIK start
			...
			m_Types.m_bKinsokuRet =
				::IsDlgButtonCheckedBool(hwndDlg, IDC_CHECK_KINSOKURET); // 改行文字をぶら下げる	//@@@ 2002.04.13 MIK
			m_Types.m_bKinsokuKuto =
				::IsDlgButtonCheckedBool(hwndDlg, IDC_CHECK_KINSOKUKUTO); // 句読点をぶら下げる	//@@@ 2002.04.17 MIK
			m_Types.m_bKinsokuHide =
				::IsDlgButtonCheckedBool(hwndDlg, IDC_CHECK_KINSOKUHIDE); // ぶら下げを隠す		// 2011/11/30 Uchi
			::DlgItem_GetText(hwndDlg, IDC_EDIT_KINSOKUHEAD, m_Types.m_szKinsokuHead,
							  _countof(m_Types.m_szKinsokuHead));
			::DlgItem_GetText(hwndDlg, IDC_EDIT_KINSOKUTAIL, m_Types.m_szKinsokuTail,
							  _countof(m_Types.m_szKinsokuTail));
			::DlgItem_GetText(hwndDlg, IDC_EDIT_KINSOKUKUTO, m_Types.m_szKinsokuKuto,
							  _countof(m_Types.m_szKinsokuKuto)); // 2009.08.07 ryoji
		}														  //@@@ 2002.04.08 MIK end
	}
