...
static const DWORD p_helpids3[] = {						 // 11500
	...
	IDC_CHECK_INDENTCPPSTR, HIDC_CHECK_INDENTCPPSTR, IDC_CHECK_INDENTCPPCMT, HIDC_CHECK_INDENTCPPCMT,
...
)
{
	...
	case WM_COMMAND:
		...
		case BN_CLICKED:
			...
			case IDC_BUTTON_TYPEOPENHELP: /* 外部ヘルプ１の「参照...」ボタン */
			{
				...
				CDlgOpenFile::SelectFile(hwndDlg, GetDlgItem(hwndDlg, IDC_EDIT_TYPEEXTHELP), L"*.hlp;*.chm;*.col", true,
										 EFITER_NONE);
			}
				return TRUE;
			case IDC_BUTTON_TYPEOPENEXTHTMLHELP: /* 外部HTMLヘルプの「参照...」ボタン */
			{
				// 2003.06.23 Moca 相対パスは実行ファイルからのパスとして開く
				// 2007.05.21 ryoji 相対パスは設定ファイルからのパスを優先
				CDlgOpenFile::SelectFile(hwndDlg, GetDlgItem(hwndDlg, IDC_EDIT_TYPEEXTHTMLHELP), L"*.chm;*.col", true,
										 EFITER_NONE);
			}
				return TRUE;
			}
	...
	case WM_NOTIFY:
		...
		case PSN_SETACTIVE: m_nPageNum = ID_PROPTYPE_PAGENUM_SUPPORT; return TRUE;
		}
		break;
...
void CPropTypesSupport::SetData(HWND hwndDlg)
{
	...
	::CheckDlgButton(hwndDlg, IDC_CHECK_TYPEHTMLHELPISSINGLE,
					 m_Types.m_bHtmlHelpIsSingle ? BST_CHECKED : BST_UNCHECKED);
