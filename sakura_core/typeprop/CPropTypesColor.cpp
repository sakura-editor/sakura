...
TYPE_NAME_ID<EStringLiteralType> StringLitteralArr[] = {
	{STRING_LITERAL_CPP, STR_STRINGESC_CPP},	   {STRING_LITERAL_PLSQL, STR_STRINGESC_PLSQL},
	{STRING_LITERAL_HTML, STR_STRINGESC_HTML},	 {STRING_LITERAL_CSHARP, STR_STRINGESC_CSHARP},
...
struct
{
	...
	int nCheckBoxID;
	int nTextID;
} const cLineComment[COMMENT_DELIMITER_NUM] = {{IDC_EDIT_LINECOMMENT, IDC_CHECK_LCPOS, IDC_EDIT_LINECOMMENTPOS},
...
LRESULT APIENTRY ColorList_SubclassProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_RBUTTONDOWN:
		...
		{ // 2009.02.22 ryoji 有効範囲の制限追加
			...
			/* 下線 */
			if (0
				== (g_ColorAttributeArr[nIndex].fAttribute
					& COLOR_ATTRIB_NO_UNDERLINE)) // 2006.12.18 ryoji フラグ利用で簡素化
			{
	...
	case WM_LBUTTONDBLCLK:
		...
		{ // 2009.02.22 ryoji 有効範囲の制限追加
			...
			if (0
				== (g_ColorAttributeArr[nIndex].fAttribute
					& COLOR_ATTRIB_NO_BOLD)) // 2006.12.18 ryoji フラグ利用で簡素化
			{
	...
	case WM_LBUTTONUP:
		...
		if (2 <= xPos && xPos <= 16
			&& (0
				== (g_ColorAttributeArr[nIndex].fAttribute
					& COLOR_ATTRIB_FORCE_DISP)) // 2006.12.18 ryoji フラグ利用で簡素化
		)
		{
			if (pColorInfo->m_bDisp)
			{ /* 色分け/表示する */
			...
			if (COLORIDX_GYOU == nIndex) { pColorInfo = (ColorInfo *)List_GetItemData(hwnd, nIndex); }
	...
	// 2005.11.30 Moca カスタム色保持
	case WM_DESTROY:
		if (::GetProp(hwnd, TSTR_PTRCUSTOMCOLORS)) { ::RemoveProp(hwnd, TSTR_PTRCUSTOMCOLORS); }
		break;
	}
...
static bool isImeUndesirable(int id)
{
	...
	case IDC_EDIT_LINECOMMENTPOS3: return true;
	default: return false;
	}
}

/* color メッセージ処理 */
INT_PTR CPropTypesColor::DispatchEvent(HWND   hwndDlg, // handle to dialog box
									   UINT   uMsg,	// message
...
)
{
	...
	case WM_COMMAND:
		...
		if (hwndListColor == hwndCtl)
		{
			...
			case LBN_SELCHANGE:
...

				{
					...
					::EnableWindow(::GetDlgItem(hwndDlg, IDC_CHECK_DISP),
								   (0 == (fAttribute & COLOR_ATTRIB_FORCE_DISP)) ? TRUE : FALSE);
					::EnableWindow(::GetDlgItem(hwndDlg, IDC_CHECK_BOLD),
								   (0 == (fAttribute & COLOR_ATTRIB_NO_BOLD)) ? TRUE : FALSE);
					::EnableWindow(::GetDlgItem(hwndDlg, IDC_CHECK_UNDERLINE),
								   (0 == (fAttribute & COLOR_ATTRIB_NO_UNDERLINE)) ? TRUE : FALSE);
					::EnableWindow(::GetDlgItem(hwndDlg, IDC_STATIC_MOZI),
								   (0 == (fAttribute & COLOR_ATTRIB_NO_TEXT)) ? TRUE : FALSE);
					::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_TEXTCOLOR),
								   (0 == (fAttribute & COLOR_ATTRIB_NO_TEXT)) ? TRUE : FALSE);
					::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_SAMETEXTCOLOR),
								   (0 == (fAttribute & COLOR_ATTRIB_NO_TEXT)) ? TRUE : FALSE);
					::EnableWindow(::GetDlgItem(hwndDlg, IDC_STATIC_HAIKEI),
								   (0 == (fAttribute & COLOR_ATTRIB_NO_BACK)) ? TRUE : FALSE);
					::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_BACKCOLOR),
								   (0 == (fAttribute & COLOR_ATTRIB_NO_BACK)) ? TRUE : FALSE);
					::EnableWindow(::GetDlgItem(hwndDlg, IDC_BUTTON_SAMEBKCOLOR),
								   (0 == (fAttribute & COLOR_ATTRIB_NO_BACK)) ? TRUE : FALSE);
				}
				...
				/* 太字で表示 */
				::CheckDlgButtonBool(hwndDlg, IDC_CHECK_BOLD,
									 m_Types.m_ColorInfoArr[m_nCurrentColorType].m_sFontAttr.m_bBoldFont);
				/* 下線を表示 */
				::CheckDlgButtonBool(hwndDlg, IDC_CHECK_UNDERLINE,
									 m_Types.m_ColorInfoArr[m_nCurrentColorType].m_sFontAttr.m_bUnderLine);
		...
		case BN_CLICKED:
			...
			case IDC_BUTTON_TEXTCOLOR: /* テキスト色 */
				/* 色選択ダイアログ */
				if (SelectColor(hwndDlg, &m_Types.m_ColorInfoArr[m_nCurrentColorType].m_sColorAttr.m_cTEXT,
								m_dwCustColors))
				{ ::InvalidateRect(::GetDlgItem(hwndDlg, IDC_BUTTON_TEXTCOLOR), NULL, TRUE); }
				/* 現在選択されている色タイプ */
			...
			case IDC_BUTTON_BACKCOLOR: /* 背景色 */
				/* 色選択ダイアログ */
				if (SelectColor(hwndDlg, &m_Types.m_ColorInfoArr[m_nCurrentColorType].m_sColorAttr.m_cBACK,
								m_dwCustColors))
				{ ::InvalidateRect(::GetDlgItem(hwndDlg, IDC_BUTTON_BACKCOLOR), NULL, TRUE); }
				/* 現在選択されている色タイプ */
				List_SetCurSel(hwndListColor, m_nCurrentColorType);
			...
			case IDC_CHECK_DISP: /* 色分け/表示 をする */
				...
				/* 現在選択されている色タイプ */
				List_SetCurSel(hwndListColor, m_nCurrentColorType);
				m_Types.m_nRegexKeyMagicNumber = CRegexKeyword::GetNewMagicNumber(); // Need Compile	//@@@ 2001.11.17
																					 // add MIK 正規表現キーワードのため
				return TRUE;
			case IDC_CHECK_BOLD: /* 太字か */
				m_Types.m_ColorInfoArr[m_nCurrentColorType].m_sFontAttr.m_bBoldFont =
					::IsDlgButtonCheckedBool(hwndDlg, IDC_CHECK_BOLD);
				/* 現在選択されている色タイプ */
				List_SetCurSel(hwndListColor, m_nCurrentColorType);
				return TRUE;
			case IDC_CHECK_UNDERLINE: /* 下線を表示 */
				m_Types.m_ColorInfoArr[m_nCurrentColorType].m_sFontAttr.m_bUnderLine =
					::IsDlgButtonCheckedBool(hwndDlg, IDC_CHECK_UNDERLINE);
				/* 現在選択されている色タイプ */
				List_SetCurSel(hwndListColor, m_nCurrentColorType);
				return TRUE;
			...
			case IDC_BUTTON_EXPORT: /* 色の設定をエクスポート */ Export(hwndDlg); return TRUE;
			...
			case IDC_BUTTON_KEYWORD_SELECT:
			{
				...
				if (CB_ERR == nIdx || 0 == nIdx) { m_nSet[0] = -1; }
				else
				{
					m_nSet[0] = nIdx - 1;
				}
				...
				//強調キーワード1を反映する。
				if (-1 == m_nSet[0]) { Combo_SetCurSel(hwndCombo, 0); }
				else
				{
			...
			case IDC_BUTTON_EDITKEYWORD:
			{
				...
				INT_PTR res = ::DialogBoxParam(CSelectLang::getLangRsrcInstance(), MAKEINTRESOURCE(IDD_PROP_KEYWORD),
											   hwndDlg, CPropKeyword::DlgProc_dialog, (LPARAM)pPropKeyword);
				if (res == IDOK)
				{
	...
	case WM_NOTIFY:
		...
		default:
			...
			case PSN_SETACTIVE: m_nPageNum = ID_PROPTYPE_PAGENUM_COLOR; return TRUE;
			}
			break; /* default */
		}
		break; /* WM_NOTIFY */
	case WM_DRAWITEM:
		...
		case IDC_BUTTON_BACKCOLOR: /* 背景色 */
			...
			return TRUE;
		case IDC_LIST_COLORS: /* 色種別リスト */ DrawColorListItem(pDis); return TRUE;
		}
		break;
	...
	case WM_HELP:
	{
		...
		MyWinHelp((HWND)p->hItemHandle, HELP_WM_HELP,
				  (ULONG_PTR)(LPVOID)p_helpids2); // 2006.10.10 ryoji MyWinHelpに変更に変更
	}
		return TRUE;
		/*NOTREACHED*/
		//		break;
...
void CPropTypesColor::SetData(HWND hwndDlg)
{
	...
	EditCtl_LimitText(::GetDlgItem(hwndDlg, IDC_EDIT_LINECOMMENT3),
					  COMMENT_DELIMITER_BUFFERSIZE - 1); // Jun. 01, 2001 JEPRO 追加
	...
	::DlgItem_SetText(hwndDlg, IDC_EDIT_BLOCKCOMMENT_FROM,
					  m_Types.m_cBlockComments[0].getBlockCommentFrom()); /* ブロックコメントデリミタ(From) */
	::DlgItem_SetText(hwndDlg, IDC_EDIT_BLOCKCOMMENT_TO,
					  m_Types.m_cBlockComments[0].getBlockCommentTo()); /* ブロックコメントデリミタ(To) */
	::DlgItem_SetText(hwndDlg, IDC_EDIT_BLOCKCOMMENT_FROM2,
					  m_Types.m_cBlockComments[1].getBlockCommentFrom()); /* ブロックコメントデリミタ2(From) */
	::DlgItem_SetText(hwndDlg, IDC_EDIT_BLOCKCOMMENT_TO2,
					  m_Types.m_cBlockComments[1].getBlockCommentTo()); /* ブロックコメントデリミタ2(To) */
	...
	for (i = 0; i < _countof(StringLitteralArr); ++i)
	{
		...
		if (StringLitteralArr[i].nMethod == m_Types.m_nStringType)
		{ // テキストの折り返し方法
			nSelPos = i;
		}
	}
	...
	//強調キーワード1～10の設定
	for (i = 0; i < MAX_KEYWORDSET_PER_TYPE; i++) { m_nSet[i] = m_Types.m_nKeyWordSetIdx[i]; }
...
void CPropTypesColor::SetDataKeyword(HWND hwndDlg)
{
	...
	if (0 < m_pCKeyWordSetMgr->m_nKeyWordSetNum)
	{
		...
		{ Combo_AddString(hwndWork, m_pCKeyWordSetMgr->GetTypeName(i)); } if (-1 == set[0])
		{
			/* セット名コンボボックスのデフォルト選択 */
...
int CPropTypesColor::GetData(HWND hwndDlg)
{
	...
	for (i = 0; i < COMMENT_DELIMITER_NUM; i++)
	{
		...
		::DlgItem_GetText(hwndDlg, cLineComment[i].nEditID, buffer,
						  COMMENT_DELIMITER_BUFFERSIZE); /* 行コメントデリミタ */
	...
	::DlgItem_GetText(hwndDlg, IDC_EDIT_BLOCKCOMMENT_FROM, szFromBuffer,
					  BLOCKCOMMENT_BUFFERSIZE); /* ブロックコメントデリミタ(From) */
	::DlgItem_GetText(hwndDlg, IDC_EDIT_BLOCKCOMMENT_TO, szToBuffer,
					  BLOCKCOMMENT_BUFFERSIZE); /* ブロックコメントデリミタ(To) */
	m_Types.m_cBlockComments[0].SetBlockCommentRule(szFromBuffer, szToBuffer);

	::DlgItem_GetText(hwndDlg, IDC_EDIT_BLOCKCOMMENT_FROM2, szFromBuffer,
					  BLOCKCOMMENT_BUFFERSIZE); /* ブロックコメントデリミタ(From) */
	::DlgItem_GetText(hwndDlg, IDC_EDIT_BLOCKCOMMENT_TO2, szToBuffer,
					  BLOCKCOMMENT_BUFFERSIZE); /* ブロックコメントデリミタ(To) */
	...
	if (nSelPos >= 0) { m_Types.m_nStringType = StringLitteralArr[nSelPos].nMethod; }
	...
	if (CB_ERR == nIdx || 0 == nIdx) { m_Types.m_nKeyWordSetIdx[0] = -1; }
	else
	{
	...
	//強調キーワード2～10の取得(1は別)
	for (nIdx = 1; nIdx < MAX_KEYWORDSET_PER_TYPE; nIdx++) { m_Types.m_nKeyWordSetIdx[nIdx] = m_nSet[nIdx]; }
	...
	while (i < MAX_VERTLINES)
	{
		...
		{ value = szVertLine[offset] - '0' + value * 10; } if (value <= 0)
		{ break; } if (szVertLine[offset] == '(')
		{
			offset++;
			int valueBegin = 0;
			int valueEnd   = 0;
			for (; '0' <= szVertLine[offset] && szVertLine[offset] <= '9'; offset++)
			{ valueBegin = szVertLine[offset] - '0' + valueBegin * 10; } if (valueBegin <= 0)
			{ break; } if (szVertLine[offset] == ',')
			{ offset++; } else if (szVertLine[offset] != ')')
			{
				break;
			}
			for (; '0' <= szVertLine[offset] && szVertLine[offset] <= '9'; offset++)
			{ valueEnd = szVertLine[offset] - '0' + valueEnd * 10; } if (valueEnd <= 0)
			{ valueEnd = MAXLINEKETAS; } if (szVertLine[offset] != ')')
			{ break; } offset++;
		...
		if (szVertLine[offset] != ',') { break; }
		offset++;
	}
	if (i < MAX_VERTLINES) { m_Types.m_nVertLineIdx[i] = CKetaXInt(0); }
	// to here 2005.11.30 Moca 指定位置縦線の設定
	return TRUE;
}
...
void CPropTypesColor::DrawColorListItem(DRAWITEMSTRUCT *pDis)
{
	...
	if (pDis->itemState & ODS_FOCUS) { ::DrawFocusRect(gr, &pDis->rcItem); }
	...
	if (0
		== (g_ColorAttributeArr[pColorInfo->m_nColorIdx].fAttribute
			& COLOR_ATTRIB_NO_BACK)) // 2006.12.18 ryoji フラグ利用で簡素化
	{
		/* 背景色 見本矩形 */
		rc1		 = pDis->rcItem;
