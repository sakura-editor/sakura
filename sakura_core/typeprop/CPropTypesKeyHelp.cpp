...
static const DWORD p_helpids[] = { // 2006.10.10 ryoji
	IDC_CHECK_KEYHELP,
	HIDC_CHECK_KEYHELP, //キーワードヘルプ機能を使う
	IDC_LIST_KEYHELP,
	HIDC_LIST_KEYHELP, // SysListView32
	IDC_BUTTON_KEYHELP_UPD,
	HIDC_BUTTON_KEYHELP_UPD, //更新(&E)
	IDC_EDIT_KEYHELP,
	HIDC_EDIT_KEYHELP, // EDITTEXT
	IDC_BUTTON_KEYHELP_REF,
	HIDC_BUTTON_KEYHELP_REF, //参照(&O)...
	IDC_BUTTON_KEYHELP_TOP,
	HIDC_BUTTON_KEYHELP_TOP, //先頭(&T)
	IDC_BUTTON_KEYHELP_UP,
	HIDC_BUTTON_KEYHELP_UP, //上へ(&U)
	IDC_BUTTON_KEYHELP_DOWN,
	HIDC_BUTTON_KEYHELP_DOWN, //下へ(&G)
	IDC_BUTTON_KEYHELP_LAST,
	HIDC_BUTTON_KEYHELP_LAST, //最終(&B)
	IDC_BUTTON_KEYHELP_INS,
	HIDC_BUTTON_KEYHELP_INS, //挿入(&S)
	IDC_BUTTON_KEYHELP_DEL,
	HIDC_BUTTON_KEYHELP_DEL, //削除(&D)
	IDC_CHECK_KEYHELP_ALLSEARCH,
	HIDC_CHECK_KEYHELP_ALLSEARCH, //全辞書検索する(&A)
	IDC_CHECK_KEYHELP_KEYDISP,
	HIDC_CHECK_KEYHELP_KEYDISP, //キーワードも表示する(&W)
	IDC_CHECK_KEYHELP_PREFIX,
	HIDC_CHECK_KEYHELP_PREFIX, //前方一致検索(&P)
	IDC_COMBO_MENU,
	HIDC_COMBO_KEYHELP_RMENU, //右クリックメニュー
	IDC_BUTTON_KEYHELP_IMPORT,
	HIDC_BUTTON_KEYHELP_IMPORT, //インポート
	IDC_BUTTON_KEYHELP_EXPORT,
	HIDC_BUTTON_KEYHELP_EXPORT, //エクスポート
	0,
	0};
...
)
{
	...
	case WM_COMMAND:
		...
		case BN_CLICKED:
			...
			{
				BOOL bEnable = TRUE;
				if (!IsDlgButtonChecked(hwndDlg, IDC_CHECK_KEYHELP)) { bEnable = FALSE; }
				// EnableWindow( GetDlgItem( hwndDlg, IDC_CHECK_KEYHELP ), FALSE );
				// //キーワードヘルプ機能を使う(&K)
			...
			case IDC_BUTTON_KEYHELP_UPD: /* 更新 */
				...
				for (i = 0; i < nIndex2; i++)
				{
					...
					{
						if ((wID == IDC_BUTTON_KEYHELP_UPD) && (i == nIndex))
						{ /* 更新時、変わっていなかったら何もしない */
						}
						else
						{
			...
			case IDC_BUTTON_KEYHELP_DEL: /* 削除 */
				...
				{
					ListView_SetItemState(hwndList, ListView_GetItemCount(hwndList) - 1, LVIS_SELECTED | LVIS_FOCUSED,
			...
			case IDC_BUTTON_KEYHELP_EXPORT: /* エクスポート */ Export(hwndDlg); return TRUE;
			}
			break;
		}
		break;
	...
	case WM_NOTIFY:
		...
		switch (pNMHDR->code)
		{
		case PSN_HELP: OnHelp(hwndDlg, IDD_PROP_KEYHELP); return TRUE;
		...
		case PSN_SETACTIVE: m_nPageNum = ID_PROPTYPE_PAGENUM_KEYHELP; return TRUE;
		...
		case LVN_ITEMCHANGED: /*リストの項目が変更された際の処理*/
			if (pNMHDR->hwndFrom == hwndList)
			{
				nIndex = ListView_GetNextItem(hwndList, -1, LVNI_ALL | LVNI_SELECTED);
				if (-1 == nIndex)
				{ //削除、範囲外でクリック時反映されないバグ修正	//@@@ 2003.06.17 MIK
...
void CPropTypesKeyHelp::SetData(HWND hwndDlg)
{
	...
	for (i = 0; i < (int)_countof(nKeyHelpRMenuType); i++) { Combo_AddString(hwndCombo, LS(nKeyHelpRMenuType[i])); }
	...
	for (i = 0; i < MAX_KEYHELP_FILE; i++)
	{
		...
		if (m_Types.m_KeyHelpArr[i].m_bUse)
		{ // ON
...
int CPropTypesKeyHelp::GetData(HWND hwndDlg)
{
	...
	for (i = 0; i < MAX_KEYHELP_FILE; i++)
	{
		if (i < nIndex)
		{
			...
			if (ListView_GetCheckState(hwndList, i)) bUse = true;
...
static WCHAR *strcnv(WCHAR *str)
{
	...
	p = str;
	if (NULL != (p = wcschr(p, L'\r'))) *p = L'\0';
	/* カンマの置換 */
	p = str;
	for (; (p = wcschr(p, L',')) != NULL;) { *p = L'.'; }
	return str;
}
