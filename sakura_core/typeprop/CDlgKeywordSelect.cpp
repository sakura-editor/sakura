...
static const int keyword_select_target_combo[KEYWORD_SELECT_NUM] = {IDC_COMBO1, IDC_COMBO2, IDC_COMBO3, IDC_COMBO4,
																	IDC_COMBO5, IDC_COMBO6, IDC_COMBO7, IDC_COMBO8,
																	IDC_COMBO9, IDC_COMBO10};
...
CDlgKeywordSelect::~CDlgKeywordSelect() { return; }
...
int CDlgKeywordSelect::DoModal(HINSTANCE hInstance, HWND hwndParent, int *pnSet)
{
	...
	for (i = 0; i < KEYWORD_SELECT_NUM; i++) { m_nSet[i] = pnSet[i]; }

	(void)CDialog::DoModal(hInstance, hwndParent, IDD_DIALOG_KEYWORD_SELECT, (LPARAM)NULL);

	for (i = 0; i < KEYWORD_SELECT_NUM; i++) { pnSet[i] = m_nSet[i]; }
...
LPVOID CDlgKeywordSelect::GetHelpIdTable(void) { return (LPVOID)p_helpids; }
