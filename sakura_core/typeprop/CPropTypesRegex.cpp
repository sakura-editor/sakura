...
static const DWORD p_helpids[] = {					   // 11600
	...
	IDC_LABEL_REGEX_KEYWORD, HIDC_EDIT_REGEX, IDC_LABEL_REGEX_COLOR, HIDC_COMBO_REGEX_COLOR, IDC_FRAME_REGEX,
	HIDC_LIST_REGEX, IDC_LABEL_REGEX_VERSION, HIDC_LABEL_REGEX_VERSION, //バージョン
																		//	IDC_STATIC,						-1,
	0, 0};
...
bool CPropTypesRegex::Import(HWND hwndDlg)
{
	...
	if (bImport) { SetDataKeywordList(hwndDlg); }
	return bImport;
}
...
/* 正規表現キーワード メッセージ処理 */
INT_PTR CPropTypesRegex::DispatchEvent(HWND   hwndDlg, // handle to dialog box
									   UINT   uMsg,	// message
	...
	case WM_COMMAND:
		...
		case BN_CLICKED:
			...
			case IDC_BUTTON_REGEX_INS: /* 挿入 */
			{
				...
				if (!CheckKeywordList(hwndDlg, &szKeyWord[0], -1)) { return FALSE; }
			...
			case IDC_BUTTON_REGEX_UPD: /* 更新 */
			{
				...
				::DlgItem_GetText(hwndDlg, IDC_EDIT_REGEX, &szKeyWord[0], nKeyWordSize);
				if (szKeyWord[0] == L'\0') return FALSE;
				if (!CheckKeywordList(hwndDlg, &szKeyWord[0], nIndex)) { return FALSE; }
				//追加するキー情報を取得する。
				wmemset(szColorIndex, 0, _countof(szColorIndex));
				::DlgItem_GetText(hwndDlg, IDC_COMBO_REGEX_COLOR, szColorIndex, _countof(szColorIndex));
				//キーを更新する。
			...
			case IDC_BUTTON_REGEX_EXPORT: /* エクスポート */ Export(hwndDlg); return TRUE;
			}
			break;
		}
		break;
...
*/
BOOL CPropTypesRegex::RegexKakomiCheck(const wchar_t *s) { return CRegexKeyword::RegexKeyCheckSyntax(s); }
...
bool CPropTypesRegex::CheckKeywordList(HWND hwndDlg, const WCHAR *szNewKeyWord, int nUpdateItem)
{
	...
	{
		nRet = ::MYMESSAGEBOX(hwndDlg, MB_OK | MB_ICONSTOP | MB_TOPMOST | MB_DEFBUTTON2, GSTR_APPNAME,
