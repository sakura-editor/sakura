...
// 共通ダイアログプロシージャ
INT_PTR CALLBACK PropTypesCommonProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam,
									 DISPATCH_EVENT_TYPE pDispatch)
{
	PROPSHEETPAGE *pPsp;
...
#define GEN_PROPTYPES_CALLBACK(FUNC, CLASS)                                                                            \
	...
	{                                                                                                                  \
		return PropTypesCommonProc(hwndDlg, uMsg, wParam, lParam,                                                      \
								   reinterpret_cast<pDispatchPage>(&CLASS::DispatchEvent));                            \
	}
...
INT_PTR CPropTypes::DoPropertySheet(int nPageNum)
{
	...
	if (-1 == nRet) {
		...
		PleaseReportToAuthor(NULL, LS(STR_PROPTYPE_ERR), psh.nStartPage, pszMsgBuf);
		::LocalFree(pszMsgBuf);
	}
