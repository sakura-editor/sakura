...
void CDocFileOperation::DoFileUnlock() { m_pcDocRef->m_cDocFile.FileUnlock(); }
	...
	std::vector<std::wstring> &files)
{
	...
	cDlgOpenFile.Create(G_AppInstance(), hwndParent, L"*.*",
...
//! ファイルを開く
bool CDocFileOperation::FileLoad(SLoadInfo *pLoadInfo //!< [in,out]
)
{
...
bool CDocFileOperation::FileLoadWithoutAutoMacro(SLoadInfo *pLoadInfo //!< [in,out]
)
{
...
//! 同一ファイルの再オープン
void CDocFileOperation::ReloadCurrentFile(ECodeType nCharCode //!< [in] 文字コード種別
)
{
	...
	CLayoutInt nViewTopLine =
		m_pcDocRef->m_pcEditWnd->GetActiveView().GetTextArea().GetViewTopLine(); /* 表示域の一番上の行(0開始) */
	CLayoutInt nViewLeftCol =
		m_pcDocRef->m_pcEditWnd->GetActiveView().GetTextArea().GetViewLeftCol(); /* 表示域の一番左の桁(0開始) */
	...
	sLoadInfo.bViewMode =
		CAppMode::getInstance()->IsViewMode(); // 2014.06.13 IsEditable->IsViewModeに戻す。かわりに bForceNoMsgを追加
	sLoadInfo.bWritableNoMsg =
		!m_pcDocRef->IsEditable(); // すでに編集できない状態ならファイルロックのメッセージを表示しない
	...
	m_pcDocRef->m_pcEditWnd->GetActiveView().GetCaret().MoveCursorProperly(
		ptCaretPosXY, true); // 2007.08.23 ryoji MoveCursor()->MoveCursorProperly()
	m_pcDocRef->m_pcEditWnd->GetActiveView().GetCaret().m_nCaretPosX_Prev =
		m_pcDocRef->m_pcEditWnd->GetActiveView().GetCaret().GetCaretLayoutPos().GetX2();
...
*/
bool CDocFileOperation::SaveFileDialog(SSaveInfo *pSaveInfo //!< [out]
)
{
	//拡張子指定
	...
	// ダイアログを表示
	CDlgOpenFile cDlgOpenFile;
	cDlgOpenFile.Create(G_AppInstance(), CEditWnd::getInstance()->GetHwnd(), szDefaultWildCard,
...
bool CDocFileOperation::DoSaveFlow(SSaveInfo *pSaveInfo)
{
	...
	try
	{
		...
		if (!GetDllShareData().m_Common.m_sFile.m_bEnableUnmodifiedOverwrite)
		{
			...
			{
				// 無変更の場合は警告音を出し、終了
				if (!m_pcDocRef->m_cDocEditor.IsModified() && pSaveInfo->cEol == EOL_NONE
					&& //※改行コード指定保存がリクエストされた場合は、「変更があったもの」とみなす
...
bool CDocFileOperation::FileSave()
{
	...
	if (!m_pcDocRef->m_cDocFile.GetFilePathClass().IsValidPath()) { return FileSaveAs(); }
...
bool CDocFileOperation::FileSaveAs(const WCHAR *filename, ECodeType eCodeType, EEolType eEolType, bool bDialog)
{
	...
	if (filename)
	{
		...
		if (EOL_NONE <= eEolType && eEolType < EOL_CODEMAX) { sSaveInfo.cEol = eEolType; }
	...
	if (DoSaveFlow(&sSaveInfo))
	{
		...
		// 提案時の Patches#1550557
		// に、「名前を付けて保存」でオープン後自動実行マクロが実行されることの是非について議論の経緯あり
...
bool CDocFileOperation::FileClose()
{
	...
	if (!m_pcDocRef->OnFileClose(false)) { return false; }
...
{
	/* ファイルを閉じるときのMRU登録 & 保存確認 & 保存実行 */
	if (!m_pcDocRef->OnFileClose(false)) { return; }
	...
	{
		std::vector<std::wstring> files;
		if (!OpenFileDialog(CEditWnd::getInstance()->GetHwnd(), NULL, &sLoadInfo, files)) { return; }
