...
bool CLineComment::Match( int nPos, std::wstring_view cStr ) const
{
	int i;
	for ( i=0; i<COMMENT_DELIMITER_NUM; i++ ){
		if (
			...
			nPos <= cStr.length() - m_nLineCommentLen[i] &&	/* 行コメントデリミタ */
			//0 == wmemicmp( &cStr.data()[nPos], m_pszLineComment[i], m_nLineCommentLen[i] )	//非ASCIIも大文字小文字を区別しない	//###locale 依存
