...
class CLayout
{
  ...
  public:
	...
	*/
    ...
    {
        return m_pCDocLine->GetPtr() + m_ptLogicPos.x;
    }
    ...
    {
        return m_nLength - (m_cEol.GetLen() ? 1 : 0);
    }
    //CLogicInt GetLength() const {	return m_nLength;	}	//	CMemoryIterator用（EOL含む）
    CLayoutInt GetIndent() const
    {
        return m_nIndent;
    } //!< このレイアウト行のインデントサイズを取得。単位は半角文字。	CMemoryIterator用

    //取得インターフェース
    CLogicInt GetLogicLineNo() const
    {
        if (this)
            return m_ptLogicPos.GetY2();
        else
            return CLogicInt(-1);
    } //$$$高速化
    CLogicInt GetLogicOffset() const
    {
        return m_ptLogicPos.GetX2();
    }
    CLogicPoint GetLogicPos() const
    {
        return m_ptLogicPos;
    }
    EColorIndexType GetColorTypePrev() const
    {
        return m_nTypePrev;
    } //#########汚っ
    CLayoutInt GetLayoutWidth() const
    {
        return m_nLayoutWidth;
    } // 2009.08.28 nasukoji	このレイアウト行の改行を含むレイアウト長を返す

    //変更インターフェース
    void OffsetLogicLineNo(CLogicInt n)
    {
        m_ptLogicPos.y += n;
    }
    ...
    void SetLayoutWidth(CLayoutInt nWidth)
    {
        m_nLayoutWidth = nWidth;
    }
    ...
    //! 文字列参照を取得
    CStringRef GetStringRef() const
    {
        return CStringRef(GetPtr(), GetLengthWithEOL());
    }

    //チェーン属性
    CLayout *GetPrevLayout()
    {
        return m_pPrev;
    }
    const CLayout *GetPrevLayout() const
    {
        return m_pPrev;
    }
    CLayout *GetNextLayout()
    {
        return m_pNext;
    }
    const CLayout *GetNextLayout() const
    {
        return m_pNext;
    }
    void _SetPrevLayout(CLayout *pcLayout)
    {
        m_pPrev = pcLayout;
    }
    void _SetNextLayout(CLayout *pcLayout)
    {
        m_pNext = pcLayout;
    }

    //実データ参照
    const CDocLine *GetDocLineRef() const
    {
        if (this)
            return m_pCDocLine;
        else
    ...
    //その他属性参照
    const CEol &GetLayoutEol() const
    {
        return m_cEol;
    }
    const CLayoutColorInfo *GetColorInfo() const
    {
        return m_cExInfo.GetColorInfo();
    }
