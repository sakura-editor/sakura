	...
	*/
	...
	CLogicInt GetLogicLineNo() const
	{
		if (this)
			...
			return CLogicInt(-1);
	} //$$$高速化 // TODO: Remove "this" check
	static CLogicInt GetLogicLineNo_Safe(const CLayout *layout)
	{
		if (layout) return layout->m_ptLogicPos.GetY2();
		return CLogicInt(-1);
	}
	...
	CLayoutInt CalcLayoutOffset(const CLayoutMgr &cLayoutMgr, CLogicInt nStartPos = CLogicInt(0),
...
private:
	...
	CEol	   m_cEol;
	CLayoutInt m_nLayoutWidth; //!< このレイアウト行の改行を含むレイアウト長（「折り返さない」選択時のみ）	//
							   //!< 2009.08.28 nasukoji
