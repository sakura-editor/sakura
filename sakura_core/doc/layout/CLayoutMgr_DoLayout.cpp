...
void CLayoutMgr::_DoGyotoKinsoku(SLayoutWork *pWork, PF_OnLine pfOnLine)
{
    if ((pWork->nPos + 1 < pWork->cLineStr.GetLength()) // 2007.02.17 ryoji 追加
        && (GetMaxLineLayout() - pWork->nPosX < 4) && (pWork->nPosX > pWork->nIndent) //	2004.04.09 pWork->nPosXの解釈変更のため，行頭チェックも変更
        && (pWork->eKinsokuType == KINSOKU_TYPE_NONE))
    {
        ...
        if (IsKinsokuPosHead(GetMaxLineLayout() - pWork->nPosX, nCharKetas2, nCharKetas3) && IsKinsokuHead(pWork->cLineStr.At(pWork->nPos + 1)) && !IsKinsokuHead(pWork->cLineStr.At(pWork->nPos)) //1文字前が行頭禁則でない
...
{
    if ((pWork->nPos + 1 < pWork->cLineStr.GetLength()) // 2007.02.17 ryoji 追加
        && (GetMaxLineKetas() - pWork->nPosX < 4) && (pWork->nPosX > pWork->nIndent) //	2004.04.09 pWork->nPosXの解釈変更のため，行頭チェックも変更
        && (pWork->eKinsokuType == KINSOKU_TYPE_NONE))
    ...
    while (NULL != pWork->pcDocLine)
    {
        ...
        if (nLineNumWork >= nLineNum)
        {
            if (pWork->pLayout && pWork->pLayout->GetNextLayout())
            {
                ...
                else if (pWork->exInfoPrev.GetColorInfo() && pWork->pLayout->GetNextLayout()->GetColorInfo() && !pWork->exInfoPrev.GetColorInfo()->IsEqual(pWork->pLayout->GetNextLayout()->GetColorInfo()))
                {
                    pWork->bNeedChangeCOMMENTMODE = true;
                }
