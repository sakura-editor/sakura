...
// 2008.07.27 kobake
static bool _GetKeywordLength(const CLayoutMgr &cLayoutMgr,
...
{
	return mgr->CreateLayout(this->pcDocLine, CLogicPoint(this->nBgn, this->nCurLine), this->nPos - this->nBgn,
							 this->colorPrev, this->nIndent, this->nPosX, this->exInfoPrev.DetachColorInfo());
}
	...
	if (KINSOKU_TYPE_NONE != pWork->eKinsokuType) {
		...
		if (pWork->nPos >= pWork->nWordBgn + pWork->nWordLen) {
			if (pWork->eKinsokuType == KINSOKU_TYPE_KINSOKU_KUTO && pWork->nPos == pWork->nWordBgn + pWork->nWordLen) {
				...
				if (!(m_pTypeConfig->m_bKinsokuRet && (pWork->nPos == pWork->cLineStr.GetLength() - nEol)
					  && nEol)) //改行文字をぶら下げる		//@@@ 2002.04.14 MIK
				{
					(this->*pfOnLine)(pWork);
				}
			}
...
void CLayoutMgr::_DoWordWrap(SLayoutWork *pWork, PF_OnLine pfOnLine)
{
	if (pWork->eKinsokuType == KINSOKU_TYPE_NONE) {
		...
		if (pWork->nPos >= pWork->nBgn && IS_KEYWORD_CHAR(pWork->cLineStr.At(pWork->nPos))) {
			// キーワード長を取得
			CLayoutInt nWordKetas = CLayoutInt(0);
			_GetKeywordLength(*this, pWork->cLineStr, pWork->nPos, &pWork->nWordBgn, &pWork->nWordLen, &nWordKetas);
...
void CLayoutMgr::_DoKutoBurasage(SLayoutWork *pWork)
{
	if ((GetMaxLineLayout() - pWork->nPosX < 2) && (pWork->eKinsokuType == KINSOKU_TYPE_NONE)) {
		...
		if (IsKinsokuPosKuto(GetMaxLineLayout() - pWork->nPosX, nCharKetas)
...
void CLayoutMgr::_DoGyomatsuKinsoku(SLayoutWork *pWork, PF_OnLine pfOnLine)
{
	if ((pWork->nPos + 1 < pWork->cLineStr.GetLength()) // 2007.02.17 ryoji 追加
		...
		&& (pWork->nPosX > pWork->nIndent) //	2004.04.09 pWork->nPosXの解釈変更のため，行頭チェックも変更
		&& (pWork->eKinsokuType
			== KINSOKU_TYPE_NONE)) { /* 行末禁則する && 行末付近 && 行頭でないこと(無限に禁則してしまいそう) */
		CLayoutInt nCharKetas2 = GetLayoutXOfChar(pWork->cLineStr, pWork->nPos);
		CLayoutInt nCharKetas3 = GetLayoutXOfChar(pWork->cLineStr, pWork->nPos + 1);

		if (IsKinsokuPosTail(GetMaxLineLayout() - pWork->nPosX, nCharKetas2, nCharKetas3)
...
void CLayoutMgr::_MakeOneLine(SLayoutWork *pWork, PF_OnLine pfOnLine)
{
	...
	while (pWork->nPos < nLength) {
		...
		if (_DoKinsokuSkip(pWork, pfOnLine)) {}
			...
			// 英文ワードラップをする
			if (m_pTypeConfig->m_bWordWrap) { _DoWordWrap(pWork, pfOnLine); }

			// 句読点のぶらさげ
			if (m_pTypeConfig->m_bKinsokuKuto) { _DoKutoBurasage(pWork); }

			// 行頭禁則
			if (m_pTypeConfig->m_bKinsokuHead) { _DoGyotoKinsoku(pWork, pfOnLine); }

			// 行末禁則
			if (m_pTypeConfig->m_bKinsokuTail) { _DoGyomatsuKinsoku(pWork, pfOnLine); }
		}
		...
		if (pWork->cLineStr.At(pWork->nPos) == WCODE::TAB) {
			if (_DoTab(pWork, pfOnLine)) { continue; }
		}
		else {
			if (pWork->nPos >= pWork->cLineStr.GetLength()) { break; }
...
void CLayoutMgr::_DoLayout(bool bBlockingHook)
{
	...
	*/
	...
	if (GetTabSpaceKetas() >= GetMaxLineKetas()) { m_nTabSpace = CKetaXInt(4); }
...
*/
CLayoutInt CLayoutMgr::DoLayout_Range(CLayout *pLayoutPrev, CLogicInt nLineNum, CLogicPoint _ptDelLogicalFrom,
									  EColorIndexType nCurrentLineType, CLayoutColorInfo *colorInfo,
									  const CalTextWidthArg *pctwArg, CLayoutInt *_pnExtInsLineNum)
{
	*_pnExtInsLineNum = CLayoutInt(0);
	...
	if (NULL == pWork->pLayout) { pWork->nCurLine = CLogicInt(0); }
	else {
			...
			if (pWork->pLayout && pWork->pLayout->GetNextLayout()) {
				if (pWork->colorPrev != pWork->pLayout->GetNextLayout()->GetColorTypePrev()) {
					...
					pWork->bNeedChangeCOMMENTMODE = true;
						 ...
						 && !pWork->exInfoPrev.GetColorInfo()->IsEqual(
...
void CLayoutMgr::CalculateTextWidth_Range(const CalTextWidthArg *pctwArg)
{
	if (m_pcEditDoc->m_nTextWrapMethodCur == WRAP_NO_TEXT_WRAP) { // 「折り返さない」
		...
		if ((pctwArg->nDelLines < CLayoutInt(0) && Int(m_nTextWidth) && Int(nInsLineNum) && Int(pctwArg->ptLayout.x)
			 && m_nTextWidthMaxLine == pctwArg->ptLayout.y)
			|| (pctwArg->nDelLines >= CLayoutInt(0) && Int(m_nTextWidth) && pctwArg->ptLayout.y <= m_nTextWidthMaxLine
		...
		// テキスト最大幅を算出する
		if (bCalTextWidth) {
			//			MYTRACE_W( L"CLayoutMgr::DoLayout_Range(%d) nCalTextWidthLinesFrom=%d
			// nCalTextWidthLinesTo=%d\n", testcount, nCalTextWidthLinesFrom, nCalTextWidthLinesTo );
...
#else
		// テキスト最大幅を算出する
		if (bCalTextWidth) CalculateTextWidth(FALSE, nCalTextWidthLinesFrom, nCalTextWidthLinesTo);
#endif
	}
}
