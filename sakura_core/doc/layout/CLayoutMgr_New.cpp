...
*/
bool CLayoutMgr::IsKinsokuHead(wchar_t wc) { return m_pszKinsokuHead_1.exist(wc); }
...
/*!
	...
	@retval false 禁則文字に該当しない
*/
bool CLayoutMgr::IsKinsokuTail(wchar_t wc) { return m_pszKinsokuTail_1.exist(wc); }
...
/*!
	...
	@retval false 禁則文字に該当しない
*/
bool CLayoutMgr::IsKinsokuKuto(wchar_t wc) { return m_pszKinsokuKuto_1.exist(wc); }

/*!
	@date 2005-08-20 D.S.Koba _DoLayout()とDoLayout_Range()から分離
*/
bool CLayoutMgr::IsKinsokuPosHead(CLayoutInt nRest,		 //!< [in] 行の残り文字数
...
/*!
	@date 2005-08-20 D.S.Koba _DoLayout()とDoLayout_Range()から分離
*/
bool CLayoutMgr::IsKinsokuPosTail(CLayoutInt nRest,		 //!< [in] 行の残り文字数
								  CLayoutInt nCharKetas, //!< [in] 現在位置の文字サイズ
								  CLayoutInt nCharKetas2 //!< [in] 現在位置の次の文字サイズ
)
{
...
/*!
	...
	@date 2002.10.01
*/
CLayoutInt CLayoutMgr::getIndentOffset_Normal(CLayout *) { return CLayoutInt(0); }
...
CLayoutInt CLayoutMgr::getIndentOffset_Tx2x(CLayout *pLayoutPrev)
{
	...
	//	前の行が折り返し行ならばそれに合わせる
	if (pLayoutPrev->GetLogicOffset() > 0) return nIpos;
	...
	while (!it.end())
	{
		...
		{ nIpos = it.getColumn() + it.getColumnDelta(); } it.addDelta();
	}
	...
	{
		nIpos = t_max(CLayoutInt(0),
					  GetMaxLineLayout()
						  - (GetTabSpace() + (2 * m_nCharLayoutXPerKeta))); // 2013.05.12 Chg:0だったのを最大幅に変更
	}
	return nIpos; //	インデント
}
...
BOOL CLayoutMgr::CalculateTextWidth(BOOL bCalLineLen, CLayoutInt nStart, CLayoutInt nEnd)
{
	...
	if (nStart < 0 && nEnd < 0) bOnlyExpansion = FALSE; // 最大幅の拡大・縮小をチェックする
	...
	for (CLayoutInt i = nStart; i < nEnd; i++)
	{
		...
		{
			CLayoutInt nWidth =
				pLayout->CalcLayoutWidth(*this) + CLayoutInt(pLayout->GetLayoutEol().GetLen() > 0 ? 1 + m_nSpacing : 0);
		...
		if (nMaxLen < pLayout->GetLayoutWidth())
		{
			...
			if (nMaxLen >= MAXLINEKETAS * GetWidthPerKeta() && !bCalLineLen) break;
		}
