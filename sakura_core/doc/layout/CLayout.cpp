...
void CLayout::DUMP(void)
{
	...
	DEBUG_TRACE(L"m_nTypePrev=%d\t\tタイプ 0=通常 1=行コメント 2=ブロックコメント 3=シングルクォーテーション文字列 "
				L"4=ダブルクォーテーション文字列 \n",
				m_nTypePrev);
	DEBUG_TRACE(L"======================\n");
	return;
...
CLayoutInt CLayout::CalcLayoutWidth(const CLayoutMgr &cLayoutMgr) const
{
	...
	for (CLogicInt i = m_ptLogicPos.GetX2(); i < nLen;)
	{
		...
		{ nWidth += cLayoutMgr.GetActualTsvSpace(nWidth, pText[i]); } else
		{
