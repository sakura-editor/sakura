...
CLayoutMgr::CLayoutMgr()
	: m_getIndentOffset(&CLayoutMgr::getIndentOffset_Normal) //	Oct. 1, 2002 genta	//	Nov. 16, 2002
															 //メンバー関数ポインタにはクラス名が必要
...
*/
void CLayoutMgr::SetLayoutInfo(bool bDoLayout, bool bBlockingHook, const STypeConfig &refType, CKetaXInt nTabSpace,
							   int nTsvMode, CKetaXInt nMaxLineKetas, CLayoutXInt nCharLayoutXPerKeta,
							   const LOGFONT *pLogfont)
{
	...
	if (nTsvModeOld != nTsvMode && nTsvMode != TSV_MODE_NONE) { m_tsvInfo.CalcTabLength(this->m_pcDocLineMgr); }
	...
	if (m_tsvInfo.m_nMaxCharLayoutX < m_nCharLayoutXPerKeta) { m_tsvInfo.m_nMaxCharLayoutX = m_nCharLayoutXPerKeta; }

	//	Oct. 1, 2002 genta タイプによって処理関数を変更する
	//	数が増えてきたらテーブルにすべき
	switch (refType.m_nIndentLayout)
	{ /* 折り返しは2行目以降を字下げ表示 */ //@@@ 2002.09.29 YAZAKI
	case 1:
		//	Nov. 16, 2002 メンバー関数ポインタにはクラス名が必要
		m_getIndentOffset = &CLayoutMgr::getIndentOffset_Tx2x;
		break;
	case 2: m_getIndentOffset = &CLayoutMgr::getIndentOffset_LeftSpace; break;
	default: m_getIndentOffset = &CLayoutMgr::getIndentOffset_Normal; break;
	}

	//句読点ぶら下げ文字	// 2009.08.07 ryoji
	// refType.m_szKinsokuKuto → m_pszKinsokuKuto_1
	m_pszKinsokuKuto_1.clear();
	if (refType.m_bKinsokuKuto)
	{ // 2009.08.06 ryoji m_bKinsokuKutoで振り分ける(Fix)
		for (const wchar_t *p = refType.m_szKinsokuKuto; *p; p++) { m_pszKinsokuKuto_1.push_back_unique(*p); }
	}
	...
	{
		if (m_pszKinsokuKuto_1.exist(*p)) { continue; }
		else
		{
	...
	for (const wchar_t *p = refType.m_szKinsokuTail; *p; p++) { m_pszKinsokuTail_1.push_back_unique(*p); }

	//レイアウト
	if (bDoLayout) { _DoLayout(bBlockingHook); }
}
...
*/
const CLayout *CLayoutMgr::SearchLineByLayoutY(CLayoutInt nLineLayout) const
{
	CLayoutInt nLineNum = nLineLayout;

	CLayout *  pLayout;
	CLayoutInt nCount;
	if (CLayoutInt(0) == m_nLines) { return NULL; }

	//	Mar. 19, 2003 Moca nLineNumが負の場合のチェックを追加
	if (CLayoutInt(0) > nLineNum || nLineNum >= m_nLines)
	{
		if (CLayoutInt(0) > nLineNum) { DEBUG_TRACE(L"CLayoutMgr::SearchLineByLayoutY() nLineNum = %d\n", nLineNum); }
		return NULL;
	}
	//	/*+++++++ 低速版 +++++++++*/
	...
	if (m_pLayoutPrevRefer == NULL || nLineNum < nPrevToLineNumDiff || m_nLines - nLineNum < nPrevToLineNumDiff)
	{
		if (nLineNum < (m_nLines / 2))
		{
			nCount  = CLayoutInt(0);
	...
	{
		if (nLineNum == m_nPrevReferLine) { return m_pLayoutPrevRefer; }
...
CLayout *CLayoutMgr::InsertLineNext(CLayout *pLayoutPrev, CLayout *pLayout)
{
	...
	else if (NULL == pLayoutPrev->GetNextLayout())
	{
		/* 最後に挿入 */
		m_pLayoutBot->m_pNext = pLayout;
		pLayout->m_pPrev	  = m_pLayoutBot;
...
*/
CLayout *CLayoutMgr::CreateLayout(CDocLine *pCDocLine, CLogicPoint ptLogicPos, CLogicInt nLength,
								  EColorIndexType nTypePrev, CLayoutInt nIndent, CLayoutInt nPosX,
								  CLayoutColorInfo *colorInfo)
{
	CLayout *pLayout = new (m_layoutMemRes->allocate(sizeof(CLayout)))
		CLayout(pCDocLine, ptLogicPos, nLength, nTypePrev, nIndent, colorInfo);

	if (EOL_NONE == pCDocLine->GetEol()) { pLayout->m_cEol.SetType(EOL_NONE); /* 改行コードの種類 */ }
	else
	{
		if (pLayout->GetLogicOffset() + pLayout->GetLengthWithEOL()
			> pCDocLine->GetLengthWithEOL() - pCDocLine->GetEol().GetLen())
		{ pLayout->m_cEol = pCDocLine->GetEol(); /* 改行コードの種類 */ } else
		{
...
const wchar_t *CLayoutMgr::GetLineStr(CLayoutInt nLine, CLogicInt *pnLineLen) const //#####いらんやろ
{
	...
	if (NULL == (pLayout = SearchLineByLayoutY(nLine))) { return NULL; }
...
{
	if (NULL == ((*ppcLayoutDes) = SearchLineByLayoutY(nLine))) { return NULL; }
...
|| 指定された位置がレイアウト行の途中の行末かどうか調べる
	...
	@date 2002/4/27 MIK
*/
bool CLayoutMgr::IsEndOfLine(const CLayoutPoint &ptLinePos)
{
	const CLayout *pLayout;

	if (NULL == (pLayout = SearchLineByLayoutY(ptLinePos.GetY2()))) { return false; }
...
*/
void CLayoutMgr::GetEndLayoutPos(CLayoutPoint *ptLayoutEnd //[out]
)
{
	if (-1 != m_nEOFLine)
	{
...
CLayout *CLayoutMgr::DeleteLayoutAsLogical(CLayout *pLayoutInThisArea, CLayoutInt nLineOf_pLayoutInThisArea,
										   CLogicInt nLineFrom, CLogicInt nLineTo, CLogicPoint ptDelLogicalFrom,
										   CLayoutInt *pnDeleteLines)
{
	...
	*pnDeleteLines = CLayoutInt(0);
	...
	if (NULL == pLayoutInThisArea) { return NULL; }
	...
	{ pLayoutWork = pLayoutWork->GetPrevLayout(); } if (NULL == pLayoutWork)
	{ pLayout = m_pLayoutTop; } else
	{
		pLayout = pLayoutWork->GetNextLayout();
	}
	while (NULL != pLayout)
	{
		if (pLayout->GetLogicLineNo() > nLineTo) { break; }
		pLayoutNext = pLayout->GetNextLayout();
		if (NULL == pLayoutWork)
		{
			/* 先頭行の処理 */
			m_pLayoutTop = pLayout->GetNextLayout();
			if (NULL != pLayout->GetNextLayout()) { pLayout->m_pNext->m_pPrev = NULL; }
		}
		else
		{
			pLayoutWork->m_pNext = pLayout->GetNextLayout();
			if (NULL != pLayout->GetNextLayout()) { pLayout->m_pNext->m_pPrev = pLayoutWork; }
		}
		...
		{ (*pnDeleteLines)++; } if (m_pLayoutPrevRefer == pLayout)
		{ DEBUG_TRACE(L"バグバグ\n"); } pLayout->~CLayout();
		m_layoutMemRes->deallocate(pLayout, sizeof(CLayout), alignof(CLayout));

		m_nLines--; /* 全物理行数 */
		if (NULL == pLayoutNext) { m_pLayoutBot = pLayoutWork; }
		pLayout = pLayoutNext;
	}
...
void CLayoutMgr::ShiftLogicalLineNum(CLayout *pLayoutPrev, CLogicInt nShiftLines)
{
	...
	CLayout *pLayout;
	if (0 == nShiftLines) { return; }
	if (NULL == pLayoutPrev) { pLayout = m_pLayoutTop; }
	else
	{
...
bool CLayoutMgr::ChangeLayoutParam(CKetaXInt nTabSize, int nTsvMode, CKetaXInt nMaxLineKetas)
{
	...
	int nTsvModeOld		 = m_tsvInfo.m_nTsvMode;
	m_tsvInfo.m_nTsvMode = nTsvMode;
	if (nTsvModeOld != nTsvMode) { m_tsvInfo.CalcTabLength(this->m_pcDocLineMgr); }
	m_nMaxLineKetas = nMaxLineKetas;
...
/* 現在位置の単語の範囲を調べる */
bool CLayoutMgr::WhereCurrentWord(CLayoutInt nLineNum, CLogicInt nIdx,
...
)
{
	...
	CLogicInt nFromX;
	CLogicInt nToX;
	bool	  nRetCode = CSearchAgent(m_pcDocLineMgr)
						.WhereCurrentWord(pLayout->GetLogicLineNo(), pLayout->GetLogicOffset() + CLogicInt(nIdx),
										  &nFromX, &nToX, pcmcmWord, pcmcmWordLeft);
...
/* 現在位置の左右の単語の先頭位置を調べる */
int CLayoutMgr::PrevOrNextWord(CLayoutInt nLineNum, CLogicInt nIdx, CLayoutPoint *pptLayoutNew, BOOL bLEFT,
							   BOOL bStopsBothEnds)
{
	const CLayout *pLayout = SearchLineByLayoutY(nLineNum);
	if (NULL == pLayout) { return FALSE; }
	...
	// 現在位置の左右の単語の先頭位置を調べる
	CLogicInt nPosNew;
	int		  nRetCode = CSearchAgent(m_pcDocLineMgr)
					   .PrevOrNextWord(pLayout->GetLogicLineNo(), pLayout->GetLogicOffset() + nIdx, &nPosNew, bLEFT,
									   bStopsBothEnds);

	if (nRetCode)
	{
		/* 論理位置→レイアウト位置変換 */
		LogicToLayout(CLogicPoint(nPosNew, pLayout->GetLogicLineNo()), pptLayoutNew, nLineNum);
	}
	return nRetCode;
}
...
/*
	@retval 0 見つからない
*/
int CLayoutMgr::SearchWord(CLayoutInt				   nLine,			 //!< [in] 検索開始レイアウト行
						   ...
						   const CSearchStringPattern &pattern)
{
	...
	nRetCode =
		CSearchAgent(m_pcDocLineMgr)
			.SearchWord(CLogicPoint(pLayout->GetLogicOffset() + nIdx, pLayout->GetLogicLineNo()), eSearchDirection,
						&cLogicRange, // pMatchRange,
						pattern);

	// 論理位置→レイアウト位置変換
	// cLogicRange -> pMatchRange
	if (nRetCode) { LogicToLayout(cLogicRange, pMatchRange); }
	return nRetCode;
}
...
*/
void CLayoutMgr::LogicToLayout(const CLogicPoint &ptLogic, CLayoutPoint *pptLayout, CLayoutInt nLineHint)
{
	pptLayout->Clear();
	...
	// 2013.05.15 ヒント、ありなしの処理を統合
	{
		...
		if (nCaretPosY <= m_nPrevReferLine && m_pLayoutPrevRefer && m_pLayoutPrevRefer->GetLogicLineNo() <= ptLogic.y)
		{
	...
	while (pLayout)
	{
		if (ptLogic.GetY2() == pLayout->GetLogicLineNo())
		{
			...
			for (i = CLogicInt(0); i < nDataLen; ++i)
			{
				...
				//ロジック加算
				if (pData[i] == WCODE::TAB) { nCharChars = CLogicInt(1); }
			...
			if (!pLayout->GetNextLayout())
			{
				...
				nCaretPosX = pLayout->CalcLayoutWidth(*this)
							 + CLayoutInt(pLayout->GetLayoutEol().GetLen() > 0 ? 1 + m_nSpacing : 0);
				break;
			}
	...
	pptLayout->Set(pLayout ? nCaretPosX : CLayoutInt(0), nCaretPosY);
}
...
*/
void CLayoutMgr::LayoutToLogicEx(const CLayoutPoint &ptLayout, //!< [in]  レイアウト位置
...
checkloop:;
	...
	for (i = CLogicInt(0); i < nDataLen; ++i)
	{
		...
		CLayoutInt nCharKetas;
		if (pData[i] == WCODE::TAB || (pData[i] == L',' && m_tsvInfo.m_nTsvMode == TSV_MODE_CSV))
		{ nCharKetas = GetActualTsvSpace(nX, pData[i]); } else
		{
			nCharKetas = GetLayoutXOfChar(pData, nDataLen, i);
		}
		//		if( nCharKetas == 0 )				// 削除 サロゲートペア対策	2008/7/5 Uchi
		...
		//レイアウト加算
		if (nX + nCharKetas > ptLayout.GetX2() && !bEOF) { break; }
		nX += nCharKetas;
