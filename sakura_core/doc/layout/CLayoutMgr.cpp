...
)
{
    ...
    CLogicInt nFromX;
    CLogicInt nToX;
    bool nRetCode = CSearchAgent(m_pcDocLineMgr).WhereCurrentWord(pLayout->GetLogicLineNo(), pLayout->GetLogicOffset() + CLogicInt(nIdx), &nFromX, &nToX, pcmcmWord, pcmcmWordLeft);
    ...
    BOOL bStopsBothEnds)
{
    ...
    // 現在位置の左右の単語の先頭位置を調べる
    CLogicInt nPosNew;
    int nRetCode = CSearchAgent(m_pcDocLineMgr).PrevOrNextWord(pLayout->GetLogicLineNo(), pLayout->GetLogicOffset() + nIdx, &nPosNew, bLEFT, bStopsBothEnds);
    ...
    const CSearchStringPattern &pattern)
{
    ...
    nRetCode = CSearchAgent(m_pcDocLineMgr).SearchWord(CLogicPoint(pLayout->GetLogicOffset() + nIdx, pLayout->GetLogicLineNo()), eSearchDirection,
    ...
    CLayoutInt nLineHint)
{
    ...
    // 2013.05.15 ヒント、ありなしの処理を統合
    {
        ...
        {
            while (pLayout && pLayout->GetPrevLayout() && pLayout->GetPrevLayout()->GetLogicLineNo() == ptLogic.GetY2() && ptLogic.x < pLayout->GetLogicOffset())
            {
                pLayout = pLayout->GetPrevLayout();
                nCaretPosY--;
            }
    ...
    while (pLayout)
    {
        if (ptLogic.GetY2() == pLayout->GetLogicLineNo())
        {
            ...
            if (pLayoutNext && ptLogic.GetY2() == pLayoutNext->GetLogicLineNo() && pLayoutNext->GetLogicOffset() <= ptLogic.x)
            {
                nCaretPosY++;
                pLayout = pLayout->GetNextLayout();
                continue;
