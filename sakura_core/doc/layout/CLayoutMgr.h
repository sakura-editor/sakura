...
public:
	...
	//レイアウトを探す
	const CLayout *SearchLineByLayoutY(
		CLayoutInt nLineLayout) const; /* 指定された物理行のレイアウトデータ(CLayout)へのポインタを返す */
	CLayout *SearchLineByLayoutY(CLayoutInt nLineLayout)
	{
		return const_cast<CLayout *>(static_cast<const CLayoutMgr *>(this)->SearchLineByLayoutY(nLineLayout));
	}

	//ワードを探す
	bool WhereCurrentWord(CLayoutInt, CLogicInt, CLayoutRange *pSelect, CNativeW *,
						  CNativeW *); /* 現在位置の単語の範囲を調べる */
	...
	{
		return CMemoryIterator(pcLayout, this->GetTabSpace(), this->m_tsvInfo, this->GetWidthPerKeta(),
							   this->GetCharSpacing());
	}
	...
	// -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
protected:
	int PrevOrNextWord(CLayoutInt, CLogicInt, CLayoutPoint *pptLayoutNew, BOOL,
					   BOOL bStopsBothEnds); /* 現在位置の左右の単語の先頭位置を調べる */
public:
	int PrevWord(CLayoutInt nLineNum, CLogicInt nIdx, CLayoutPoint *pptLayoutNew, BOOL bStopsBothEnds)
	{
		return PrevOrNextWord(nLineNum, nIdx, pptLayoutNew, TRUE, bStopsBothEnds);
	} /* 現在位置の左右の単語の先頭位置を調べる */
	int NextWord(CLayoutInt nLineNum, CLogicInt nIdx, CLayoutPoint *pptLayoutNew, BOOL bStopsBothEnds)
	{
		return PrevOrNextWord(nLineNum, nIdx, pptLayoutNew, FALSE, bStopsBothEnds);
	} /* 現在位置の左右の単語の先頭位置を調べる */

	int SearchWord(CLayoutInt nLine, CLogicInt nIdx, ESearchDirection eSearchDirection, CLayoutRange *pMatchRange,
				   const CSearchStringPattern &); /* 単語検索 */
...
public:
	...
	*/
	void SetLayoutInfo(bool bDoLayout, bool bBlockingHook, const STypeConfig &refType, CKetaXInt nTabSpace,
					   int nTsvMode, CKetaXInt nMaxLineKetas, CLayoutXInt nCharLayoutXPerKeta, const LOGFONT *pLogfont);

	/* 文字列置換 */
	void ReplaceData_CLayoutMgr(LayoutReplaceArg *pArg);
	...
	CLayoutXInt GetLayoutXOfChar(const wchar_t *pData, int nDataLen, int i) const
	{
		...
		if (m_nSpacing) { nSpace = CLayoutXInt(CNativeW::GetKetaOfChar(pData, nDataLen, i)) * m_nSpacing; }
...
private:
	...
	*/
	bool IsKinsokuPosKuto(CLayoutInt nRest, CLayoutInt nCharChars) const { return nRest < nCharChars; }
...
protected:
	...
	*/
	...
	CLayout *CreateLayout(CDocLine *pCDocLine, CLogicPoint ptLogicPos, CLogicInt nLength, EColorIndexType nTypePrev,
						  CLayoutInt nIndent, CLayoutInt nPosX, CLayoutColorInfo *);
