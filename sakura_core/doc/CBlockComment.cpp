...
) const
{
	if (
		...
		nPos <= cStr.length() - m_nBlockFromLen &&	/* ブロックコメントデリミタ(From) */
		//0 == wmemicmp( &cStr.data()[nPos], m_szBlockCommentFrom, m_nBlockFromLen )	//非ASCIIも大文字小文字を区別しない	//###locale 依存
...
{
	for( int i = nPos; i <= cStr.length() - m_nBlockToLen; ++i ){
		//if( 0 == wmemicmp( &cStr.data()[i], m_szBlockCommentTo, m_nBlockToLen ) ){	//非ASCIIも大文字小文字を区別しない	//###locale 依存
