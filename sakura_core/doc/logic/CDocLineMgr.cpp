...
CDocLineMgr::~CDocLineMgr() { DeleteAllLine(); }
...
void CDocLineMgr::DeleteLine(CDocLine *pcDocLineDel)
{
	...
	//参照切り離し
	if (m_pCodePrevRefer == pcDocLineDel) { m_pCodePrevRefer = pcDocLineDel->GetNextLine(); }
...
const CDocLine *CDocLineMgr::GetLine(CLogicInt nLine) const
{
	CLogicInt nCounter;
	CDocLine *pDocLine;
	if (CLogicInt(0) == m_nLines) { return NULL; }
	// 2004.03.28 Moca nLineが負の場合のチェックを追加
	if (CLogicInt(0) > nLine || nLine >= m_nLines) { return NULL; }
...
{
	if (!m_pDocLineTop) { m_pDocLineTop = pDocLineNew; }
	pDocLineNew->m_pPrev = m_pDocLineBot;

	if (m_pDocLineBot) { m_pDocLineBot->m_pNext = pDocLineNew; }
...
void CDocLineMgr::DUMP()
{
	...
	if (m_pDocLineBot->m_pNext != NULL) { MYTRACE(L"error: m_pDocLineBot->m_pNext != NULL\n"); }
	while (NULL != pDocLine)
	{
		if (m_pDocLineCurrent == pDocLine) { bIncludeCurrent = true; }
		if (m_pCodePrevRefer == pDocLine) { bIncludePrevRefer = true; }
	...
	if (pDocLineEnd != m_pDocLineBot) { MYTRACE(L"error: pDocLineEnd != m_pDocLineBot"); }

	if (nNum != m_nLines) { MYTRACE(L"error: nNum(%d) != m_nLines(%d)\n", nNum, m_nLines); }
