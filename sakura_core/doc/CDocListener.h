...
class CDocListener : public CListenerT<CDocSubject>
{
  public:
    ...
    // -- -- 属性 -- -- //
    CDocSubject *GetListeningDoc() const
    {
        return GetListeningSubject();
    }

    // -- -- 各種イベント -- -- //
    //ロード前後
    virtual ECallbackResult OnCheckLoad(SLoadInfo *pLoadInfo)
    {
        return CALLBACK_CONTINUE;
    } //!< 本当にロードを行うかの判定を行う
    virtual void OnBeforeLoad(SLoadInfo *sLoadInfo)
    {
        return;
    } //!< ロード事前処理
    virtual ELoadResult OnLoad(const SLoadInfo &sLoadInfo)
    {
        return LOADED_NOIMPLEMENT;
    } //!< ロード処理
    virtual void OnLoading(int nPer)
    {
        return;
    } //!< ロード処理の経過情報を受信
    virtual void OnAfterLoad(const SLoadInfo &sLoadInfo)
    {
        return;
    } //!< ロード事後処理
    virtual void OnFinalLoad(ELoadResult eLoadResult)
    {
        return;
    } //!< ロードフローの最後に必ず呼ばれる

    //セーブ前後
    virtual ECallbackResult OnCheckSave(SSaveInfo *pSaveInfo)
    {
        return CALLBACK_CONTINUE;
    } //!< 本当にセーブを行うかの判定を行う
    virtual ECallbackResult OnPreBeforeSave(SSaveInfo *pSaveInfo)
    {
        return CALLBACK_CONTINUE;
    } //!< セーブ事前おまけ処理 ($$ 仮)
    virtual void OnBeforeSave(const SSaveInfo &sSaveInfo)
    {
        return;
    } //!< セーブ事前処理
    virtual void OnSave(const SSaveInfo &sSaveInfo)
    {
        return;
    } //!< セーブ処理
    virtual void OnSaving(int nPer)
    {
        return;
    } //!< セーブ処理の経過情報を受信
    virtual void OnAfterSave(const SSaveInfo &sSaveInfo)
    {
        return;
    } //!< セーブ事後処理
    virtual void OnFinalSave(ESaveResult eSaveResult)
    {
        return;
    } //!< セーブフローの最後に必ず呼ばれる

    //クローズ前後
    virtual ECallbackResult OnBeforeClose()
    {
        return CALLBACK_CONTINUE;
    }
};
