				...
				if( bRegexReplace && pszToken ){
					...
					if( nullptr != pszGroupDel && 0 < pszWork[0] != L'\0' ){
						...
						if( nTitleLen < int(std::size(szText)) ){
							::wcsncpy_s(szText, pszWork, nTitleLen);
						}else{
							::wcsncpy_s(szText, pszWork, _TRUNCATE);
						}
...
void CDocOutline::MakeFuncList_RuleFile( CFuncInfoArr* pcFuncInfoArr, std::wstring& sTitleOverride )
{
	...
	bool bRegex;
	std::wstring title;
	int nCount = ReadRuleFile(GetTypeConfig().m_szOutlineRuleFilename, test.get(), 1024, bRegex, title );
	if ( nCount < 1 ){
		return;
	}
	if( 0 < title.size() ){
