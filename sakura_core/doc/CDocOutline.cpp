...
*/
int CDocOutline::ReadRuleFile(const WCHAR *pszFilename, SOneRule *pcOneRule, int nMaxCount, bool &bRegex,
							  std::wstring &title)
{
	...
	while (file.Good() && nCount < nMaxCount)
	{
		...
		if (NULL != pszWork && 0 < strLine.length() && strLine[0] != cComment)
		{
			...
			if (bRegex)
			{
				...
				if (bRegexReplace && pszToken)
				{
					...
					if (NULL != pszGroupDel && 0 < pszWork[0] != L'\0')
					{
						...
						if (nTitleLen < _countof(szText)) { wcsncpy_s(szText, _countof(szText), pszWork, nTitleLen); }
						else
						{
			...
			while (NULL != pszToken)
			{
				...
				nCount++;
				if (bTopDummy || bRegex) { pszToken = NULL; }
				else
				{
		...
		else
		{
			...
			{
				if (13 <= strLine.length() && strLine.length() <= 14
					&& 0 == wcsnicmp_literal(strLine.c_str() + 1, L"CommentChar="))
				{
					if (13 == strLine.length()) { cComment = L'\0'; }
					else
					{
						cComment = strLine[13];
					}
...
void CDocOutline::MakeFuncList_RuleFile(CFuncInfoArr *pcFuncInfoArr, std::wstring &sTitleOverride)
{
	...
	auto test = std::make_unique<SOneRule[]>(
		1024); // 1024個許可。 2007.11.29 kobake スタック使いすぎなので、ヒープに確保するように修正。
	bool		 bRegex;
	std::wstring title;
	int nCount = ReadRuleFile(m_pcDocRef->m_cDocType.GetDocumentAttribute().m_szOutlineRuleFilename, test.get(), 1024,
							  bRegex, title);
	if (nCount < 1) { return; }
	if (0 < title.size()) { sTitleOverride = title.c_str(); }
	...
	*/
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (test[i].nRegexMode == 1)
			{
				if (!pRegex[i].Compile(test[i].szMatch, test[i].szText, test[i].nRegexOption))
				{
					...
					str += L"\n";
					str += test[i].szText;
					ErrorMessage(NULL, LS(STR_DOCOUTLINE_REGEX), str.c_str(), pRegex[i].GetLastMessage());
					delete[] pRegex;
					return;
				}
			}
			...
			{
				ErrorMessage(NULL, LS(STR_DOCOUTLINE_REGEX), test[i].szMatch, pRegex[i].GetLastMessage());
				delete[] pRegex;
				return;
			}
		}
	...
	for (CLogicInt nLineCount = CLogicInt(0); nLineCount < m_pcDocRef->m_cDocLineMgr.GetLineCount(); ++nLineCount)
	{
		...
		if (!bRegex)
		{
			...
			{
				if (pLine[i] == L' ' || pLine[i] == L'\t' || pLine[i] == L'　') { continue; }
				break;
			}
			if (i >= nLineLen) { continue; }
		}
		...
		if (0 == wcscmp(szTitle, L"Except")) { continue; }
...
void CDocOutline::MakeFuncList_BookMark(CFuncInfoArr *pcFuncInfoArr)
{
	...
	BOOL		   bMarkUpBlankLineEnable =
		GetDllShareData().m_Common.m_sOutline.m_bMarkUpBlankLineEnable; //! 空行をマーク対象にするフラグ 20020119 aroka
	...
	for (nLineCount = CLogicInt(0); nLineCount < nLineLast; ++nLineCount)
	{
		...
		{ // 20020119 aroka
			if (nLineLen <= nNewLineLen && nLineCount < nLineLast) { continue; }
		} // LTrim
		for (leftspace = 0; leftspace < nLineLen; ++leftspace)
		{
			if (WCODE::IsBlank(pLine[leftspace])) { continue; }
			break;
		}

		if (bMarkUpBlankLineEnable)
		{ // 20020119 aroka
			if ((leftspace >= nLineLen - nNewLineLen && nLineCount < nLineLast) || (leftspace >= nLineLen))
			{ continue; } } // RTrim
		...
		while (k < nLineLen)
		{
			...
			if (1 == nCharChars)
			{
				if (!(WCODE::IsLineDelimiter(pLine[k], bExtEol) || pLine[k] == WCODE::SPACE || pLine[k] == WCODE::TAB
					  || WCODE::IsZenkakuSpace(pLine[k]) || pLine[k] == L'\0'))
