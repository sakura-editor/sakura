...
int _EncodeBase64(const char *pSrc, const int nSrcLen, CHAR_TYPE *pDest)
{
	...
	for (i = 0; i < nSrcLen; i += 3)
	{
		...
		// n 今回エンコードする長さ
		// j エンコード後のBASE64文字数
		for (k = 0; k < n; k++) { lDataSrc |= static_cast<unsigned long>(psrc[i + k]) << ((n - k - 1) * 8); }
...
int _DecodeUU_line(const CHAR_TYPE *pSrc, const int nSrcLen, char *pDest)
{
	...
	{
		lDataDes = 0;
		for (j = 0; j < 4; ++j) { lDataDes |= _UUDECODE_CHAR(pr[i + j]) << ((4 - j - 1) * 6); }
		for (j = 0; j < 3; ++j) { pDest[k + j] = (char)((lDataDes >> ((3 - j - 1) * 8)) & 0x000000ff); }
		k += 3;
	}
...
int _DecodeMimeHeader(const CHAR_TYPE *pSrc, const int nSrcLen, CMemory *pcMem_alt, ECodeType *peCodetype)
{
	...
	if (pSrc + 14 < pSrc + nSrcLen)
	{
		// JIS の場合
		if (sizeof(CHAR_TYPE) == 2)
		{ ncmpresult = wcsnicmp_literal(reinterpret_cast<const wchar_t *>(&pSrc[0]), L"=?ISO-2022-JP?"); } else
		{
	...
	if (pSrc + 8 < pSrc + nSrcLen)
	{
		// UTF-8 の場合
		if (sizeof(CHAR_TYPE) == 2)
		{ ncmpresult = wcsnicmp_literal(reinterpret_cast<const wchar_t *>(&pSrc[0]), L"=?UTF-8?"); } else
		{
	...
	if (peCodetype) { *peCodetype = CODE_NONE; }
	return 0;

finish_first_detect:;

	if (peCodetype) { *peCodetype = ecode; }
