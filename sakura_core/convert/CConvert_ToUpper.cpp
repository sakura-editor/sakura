...
bool CConvert_ToUpper::DoConvert(CNativeW *pcData)
{
	...
	while (p < end)
	{
		WCHAR &c = *p++;
		// a-z → A-Z
		if (c >= 0x0061 && c <= 0x007A) { c = 0x0041 + (c - 0x0061); }
		// ａ-ｚ → Ａ-Ｚ
		...
		{				// 2014.11.25 0x0131=>0x0130は間違い。0x0131=>0x0049に変更
			c = 0x0049; // I
		}
		...
		else if ((c >= 0x0180 && c <= 0x0185) || (c >= 0x0198 && c <= 0x0199) || (c >= 0x01A0 && c <= 0x01A5)
				 || (c >= 0x01AC && c <= 0x01AD) || (c >= 0x01B8 && c <= 0x01B9) || (c >= 0x01BC && c <= 0x01BD)
				 || (c >= 0x01DE && c <= 0x01EF) || (c >= 0x01F4 && c <= 0x01F5) || (c >= 0x01F8 && c <= 0x01FF))
		{
			c = (c & ~0x0001);
		}
		else if (((c >= 0x0187 && c <= 0x018c) || (c >= 0x0191 && c <= 0x0192) || (c >= 0x01A7 && c <= 0x01A8)
				  || (c >= 0x01AF && c <= 0x01B0) || (c >= 0x01B3 && c <= 0x01B6) || (c >= 0x01CD && c <= 0x01DC))
				 && ((c & 1) == 0))
		{
			c--;
		}
