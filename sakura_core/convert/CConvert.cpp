...
void CConvertMediator::ConvMemory(CNativeW *pCMemory, EFunctionCode nFuncCode, CKetaXInt nTabWidth, int nStartColumn)
{
	...
	//   2. xxx→SJIS 変換後にバッファ内容をUNICODE版相当に戻す（SJIS→Unicode）のと等価な結果を得るために xxx→Unicode
	//   変換する
	// SJIS2xxx:
	//   1. バッファ内容をANSI版相当に変換（Unicode→SJIS）後に SJIS→xxx 変換するのと等価な結果を得るために Unicode→xxx
	//   変換する
	...
	case F_CODECNV_UTF72SJIS: CShiftJis::UnicodeToSJIS(*pCMemory, pCMemory->_GetMemory()); break;
	//コード変換(SJIS2xxx)
	...
	case F_CODECNV_SJIS2UTF7: CShiftJis::SJISToUnicode(*(pCMemory->_GetMemory()), pCMemory); break;
	}
