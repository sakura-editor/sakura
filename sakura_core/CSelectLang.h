...
public:
	...
	*/
	...
	static LPCWSTR getDefaultLangString(
...
protected:
	...
	public:
		...
		{
			// バッファを取得していた場合は解放する。
			if (m_pszString && m_pszString != m_szString) { delete[] m_pszString; }
		}
...
public:
	...
	*/
	...
	||  Attributes & Operations
	*/
	/*virtual*/ LPCWSTR GetStringPtr() const
	{
		return m_cLoadStrBuffer.GetStringPtr();
	} // 読み込んだ文字列のポインタを返す
	  //	/*virtual*/ int GetBufferSize() const { return m_cLoadStrBuffer.GetBufferSize(); }		//
	  //読み込みバッファのサイズ（TCHAR単位）を返す
	/*virtual*/ int GetStringLength() const
	{
		return m_cLoadStrBuffer.GetStringLength();
	} // 読み込んだ文字数（TCHAR単位）を返す
