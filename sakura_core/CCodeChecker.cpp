...
ECallbackResult CCodeChecker::OnCheckSave(SSaveInfo *pSaveInfo)
{
    ...
    if (bTmpResult)
    {
        ...
        {
            case IDYES:
                pSaveInfo->cEol = pcDoc->m_cDocEditor.GetNewLineCode();
                break; //統一
            case IDNO:
                break; //続行
            case IDCANCEL:
    ...
    EConvertResult nTmpResult = _CheckSavingCharcode(
        pcDoc->m_cDocLineMgr, pSaveInfo->eCharCode, point, cmemChar);
    ...
    if (nTmpResult == RESULT_LOSESOME)
    {
        ...
        else
        {
            ...
            CUnicode().UnicodeToHex(cmemChar.GetStringPtr(), cmemChar.GetStringLength(), szCharCode, &GetDllShareData().m_Common.m_sStatusbar);
        }
        int nDlgResult = MYMESSAGEBOX(
            CEditWnd::getInstance()->GetHwnd(),
            MB_YESNOCANCEL | MB_ICONWARNING,
