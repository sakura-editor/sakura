...
)
{
	...
	if( bup_setting.GetBackupType() == 3 ||
		...
		for( ; i >= boundary; --i ){
			...
			if( ::DeleteFile( szPath ) == 0 ){
				::MessageBox( GetEditWndPtr()->GetHwnd(), szPath, LS(STR_BACKUP_ERR_DELETE), MB_OK );
				//	Jun.  5, 2005 genta 戻り値変更
				//	失敗しても保存は継続
				return 0;
				//	失敗した場合
		...
		for( ; i >= 0; --i ){
			...
			if( ::MoveFile( szPath, szNewPath ) == 0 ){
				//	失敗した場合
				//	後で考える
				::MessageBox( GetEditWndPtr()->GetHwnd(), szPath, LS(STR_BACKUP_ERR_MOVE), MB_OK );
				//	Jun.  5, 2005 genta 戻り値変更
				//	失敗しても保存は継続
				return 0;
			}
	...
	if( ::CopyFile( target_file, szPath, FALSE ) ){
		...
		if( bup_setting.m_bBackUpDustBox && !dustflag ){	//@@@ 2002.03.23 ネットワーク・リムーバブルドライブでない
			...
			SHFILEOPSTRUCT	fos;
			fos.hwnd   = GetEditWndPtr()->GetHwnd();
			fos.wFunc  = FO_DELETE;
