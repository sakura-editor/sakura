...
static TCHAR* nKeyHelpRMenuType[] = {
	_T("表示しない"),
	_T("先頭に表示"),
	_T("末尾に表示"),
};
...
void CPropTypesKeyHelp::SetData( HWND hwndDlg )
{
	...
	for( i = 0; i < (int)_countof(nKeyHelpRMenuType); i++ ){
		SendMessage(hwndCombo, CB_ADDSTRING, 0L, (LPARAM)nKeyHelpRMenuType[i]);
	}
	SendMessage(hwndCombo, CB_SETCURSEL, (WPARAM)m_Types.m_eKeyHelpRMenuShowType, 0L);
...
int CPropTypesKeyHelp::GetData( HWND hwndDlg )
{
	...
	m_Types.m_eKeyHelpRMenuShowType = (EKeyHelpRMenuType)SendMessage(GetDlgItem(hwndDlg, IDC_COMBO_MENU), CB_GETCURSEL, 0L, 0L);
